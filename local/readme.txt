...
See also https://agpudev.io/docs/apis/plugintypes/local for more
information.
...
* applications that extend agpu at the system level (hub server, amos server, etc.)
...
version tells agpu to invalidate all caches, do db upgrades if necessary,
...
$plugin->requires = 2010021900;   // Requires this agpu version
...
NOTE: events are not yet fully implemented in current agpu 2.0dev.
...
from /local/nicehack/db/services.php and are automatically installed/updated in agpu.
...
<?php
$defaults['agpu']['forcelogin'] = 1;  // new default for $CFG->forcelogin
$defaults['scorm']['maxgrade'] = 20;    // default for get_config('scorm', 'maxgrade')
$defaults['agpucourse']['numsections'] = 11;
$defaults['agpu']['hiddenuserfields'] = array('city', 'country');
...
---------------------------------
agpu supports other type of local customisation of standard language
...
agpudata/lang/en_local
<?php
  $string['login'] = 'Sign in';

See also http://docs.agpu.org/en/Language_editing
...
Examples:
* disable one specific agpu page without code modification
