...
console.log(`
${chalk.bold(chalk.underline(chalk.green('agpu Upgrade Notes Generator')))}

This tool is used to generate the upgrade notes for changes you make in agpu.
...
program
    ...
    .addArgument(
        new Argument('[version]', 'The agpu version to create the summary notes for')
    )
    .action((version) => generateAction(version));

program
    .command('release')
    .summary('Generate the markdown copies of the upgrade notes for a agpu release')
    .addArgument(
        new Argument('[version]', 'The agpu version to create the release notes for')
    )
    .addOption(
        new Option(
