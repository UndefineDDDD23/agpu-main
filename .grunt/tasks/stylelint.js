...
module.exports = grunt => {
    ...
    const registerStyleLintTasks = () => {
        ...
        const requestedFiles = grunt.agpuEnv.files;
        if (requestedFiles) {
            ...
            requestedFiles.forEach(changedFilePath => {
                ...
                matchesGlob = grunt.agpuEnv.scssSrc.some(watchedPathGlob => {
                ...
                matchesGlob = grunt.agpuEnv.cssSrc.some(watchedPathGlob => {
                    return glob.sync(watchedPathGlob).indexOf(changedFilePath) !== -1;
                });
                if (matchesGlob) {
            ...
            grunt.agpuEnv.scssSrc.forEach(path => {
                if (path.startsWith(grunt.agpuEnv.runDir)) {
                    scssFiles.push(path);
                    hasScss = true;
                }
            });

            grunt.agpuEnv.cssSrc.forEach(path => {
                if (path.startsWith(grunt.agpuEnv.runDir)) {
                    cssFiles.push(path);
                    hasCss = true;
                }
    ...
    const getCoreThemeMatches = () => {
        ...
        if (grunt.agpuEnv.inTheme) {
            return [scssMatch];
        }

        if (grunt.agpuEnv.runDir.startsWith('theme')) {
            return [`*/${scssMatch}`];
        }
