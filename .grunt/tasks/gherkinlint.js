...
module.exports = grunt => {
    ...
    const getGherkinLintTargets = () => {
        if (grunt.agpuEnv.files) {
            // Specific files were requested. Only check these.
            return grunt.agpuEnv.files;
        }

        if (grunt.agpuEnv.inComponent) {
            return [`${grunt.agpuEnv.runDir}/tests/behat/*.feature`];
        }
    ...
    grunt.config.merge({
        watch: {
            gherkinlint: {
                files: [grunt.agpuEnv.inComponent ? 'tests/behat/*.feature' : '**/tests/behat/*.feature'],
                tasks: ['gherkinlint'],
            },
        },
    });
    ...
    grunt.agpuEnv.startupTasks.push('gherkinlint');
