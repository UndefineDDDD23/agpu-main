...
/**
 * Helper functions for working with agpu component names, directories, and sources.
 *
...
/** @var {Object} A list of subsystems in agpu */
const componentData = {};

/**
 * Load details of all agpu modules.
 *
 * @returns {object}
 */
const fetchComponentData = () => {
    ...
    if (!Object.entries(componentData).length) {
        ...
        // Build the list of agpu subsystems.
    ...
    for (let library of libraries) {
        upgradableLibraries.push(
            getRepositoryTags(library.repository).then((tagMap) => {
                ...
                const currentVersion = library.version.replace(/agpu-/, '');
                ...
                if (versionCompare(currentVersion, library.latestVersion) > 0) {
                    // agpu somehow has a newer version than the latest version.
