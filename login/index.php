...
if (defined('BEHAT_SITE_RUNNING') && BEHAT_SITE_RUNNING) {
    ...
    if ($wantsurl !== '') {
        $SESSION->wantsurl = (new agpu_url($wantsurl))->out(false);
    }
}
...
if ($user !== false or $frm !== false or $errormsg !== '') {
    ...
    // Handles the case of another agpu site linking into a page on this site
...
if ($anchor && isset($SESSION->wantsurl) && strpos($SESSION->wantsurl, '#') === false) {
    $wantsurl = new agpu_url($SESSION->wantsurl);
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    if ($user) {
        ...
        if (empty($user->confirmed)) {       // This account was never confirmed
            ...
            $resendconfirmurl = new agpu_url('/login/index.php',
                [
    ...
    /// Currently supported only for ldap-authentication module
        ...
        redirect(new agpu_url(get_login_url(), array('testsession'=>$USER->id)));
...
if (!empty($CFG->alternateloginurl) && $loginredirect) {
    $loginurl = new agpu_url($CFG->alternateloginurl);
...
if (!empty($SESSION->loginerrormsg) || !empty($SESSION->logininfomsg)) {
    ...
    $loginurl = new agpu_url('/login/index.php');
...
if (isloggedin() and !isguestuser()) {
    ...
    $logout = new single_button(new agpu_url('/login/logout.php', array('sesskey'=>sesskey(),'loginpage'=>1)), get_string('logout'), 'post');
    $continue = new single_button(new agpu_url('/'), get_string('cancel'), 'get');
