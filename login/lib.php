...
/**
 *
 * Login library file of login/password related agpu functions.
 *
 * @package    core
 * @subpackage lib
...
function core_login_process_password_reset($username, $email) {
    ...
    if (empty($username) && empty($email)) {
        throw new \agpu_exception('cannotmailconfirm');
    }
    ...
    if ($user and !empty($user->confirmed)) {
        ...
        if (!$userauth->can_reset_password() or !is_enabled_auth($user->auth)
          or !has_capability('agpu/user:changeownpassword', $systemcontext, $user->id)) {
...
function core_login_process_password_set($token) {
    ...
    if ($user->auth === 'nologin' or !is_enabled_auth($user->auth)) {
        ...
        echo $OUTPUT->header();
        throw new \agpu_exception('forgotteninvalidurl');
        die; // Never reached.
    }

    // Check this isn't guest user.
    if (isguestuser($user)) {
        throw new \agpu_exception('cannotresetguestpwd');
    }
...
function core_login_get_return_url() {
    ...
    if ($urltogo == ($CFG->wwwroot . '/')) {
        ...
        // Go to my-agpu page instead of site homepage if defaulthomepage set to homepage_my.
