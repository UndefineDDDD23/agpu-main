...
class cache implements loader_interface {
    ...
    protected function get_implementation($key, int $requiredversion, int $strictness, &$actualversion = null) {
        ...
        if (helper::result_found($result)) {
            ...
            try {
                ...
                // screen when upgrading from an earlier agpu version that didn't use a versioned
     ...
     * $cache->set('main', 'http://agpu.org');
     * $cache->set('docs', 'http://docs.agpu.org');
     * $cache->set('tracker', 'http://tracker.agpu.org');
     * $cache->set('qa', 'http://qa.agpu.net');
     * </code>
     *
     * @param string|int $key The key for the data being requested.
     *      It can be any structure although using a scalar string or int is recommended in the interests of performance.
     ...
     *     'main' => 'http://agpu.org',
     *     'docs' => 'http://docs.agpu.org',
     *     'tracker' => 'http://tracker.agpu.org',
     *     'qa' => ''http://qa.agpu.net'
     * ));
     * </code>
     *
