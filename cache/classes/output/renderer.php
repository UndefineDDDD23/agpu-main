...
class renderer extends \plugin_renderer_base {
    ...
    public function definition_summaries(array $definitionsummaries, context $context) {
        ...
        $url = new agpu_url('/cache/admin.php', ['action' => 'rescandefinitions', 'sesskey' => sesskey()]);
     ...
     * @param agpu_url $editurl
     * @return string HTML
     */
    public function mode_mappings($applicationstore, $sessionstore, $requeststore, agpu_url $editurl) {
        $table = new html_table();
        $table->colclasses = [
            'mode',
            'mapping',
    ...
    public function lock_summaries(array $locks) {
        ...
        foreach ($locks as $lock) {
            ...
            if ($lock['uses'] === 0 && !$lock['default']) {
                $url = new agpu_url('/cache/admin.php', ['lock' => $lock['name'], 'action' => 'deletelock']);
    ...
    public function additional_lock_actions(): string {
        $url = new agpu_url('/cache/admin.php', ['action' => 'newlockinstance']);
     ...
     * @param \agpuform $samplesform Form to select number of samples
     * @return string HTML for page
     */
    public function usage_page(\html_table $maintable, \html_table $summarytable, \agpuform $samplesform): string {
        $data = [
