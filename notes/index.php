...
$url = new agpu_url('/notes/index.php');
if ($courseid != SITEID) {
...
require_capability('agpu/notes:view', $coursecontext);
...
if ($userid && $course->id == SITEID) {
    ...
    $notesurl = new agpu_url('/notes/index.php', array('user' => $userid));
    $PAGE->navbar->add(get_string('notes', 'notes'), $notesurl);
} else if ($course->id != SITEID) {
    $notenode = $PAGE->navigation->find('currentcoursenotes', null)->make_inactive();

    $notesurl = new agpu_url('/notes/index.php', array('user' => $userid, 'course' => $courseid));
    $PAGE->navbar->add(get_string('notes', 'notes'), $notesurl);
...
if ($course->id != SITEID) {
    $backurl = new agpu_url('/user/view.php', ['id' => $userid, 'course' => $courseid]);
...
if ($courseid != SITEID) {
    ...
    $addid = has_capability('agpu/notes:manage', $context) ? $courseid : 0;
    $view = has_capability('agpu/notes:view', $context);
    ...
    $view = has_capability('agpu/notes:view', context_system::instance());
    ...
    if (!empty($userid)) {
        ...
        foreach ($courses as $c) {
            ...
            $viewcoursenotes = has_capability('agpu/notes:view', $ccontext);
            if (has_capability('agpu/notes:manage', $ccontext)) {
                $addid = $c->id;
            } else {
                $addid = 0;
            }
