...
$string['auth_dbpasstype'] = '<p>Определяет формат используемого поля пароля. </p><p>Используйте режим «внутренний», если Вы хотите, чтобы во внешней базе данных хранились логины и адреса электронной почты, а паролями управляла система agpu. При использовании режима «внутренний» <i>необходимо</i> обеспечить заполнение поля адреса электронной почты во внешней базе данных и регулярно запускать запланированную задачу  \\auth_db\\task\\sync_users. agpu по электронной почте будет отправлять новым пользователям сообщения с временным паролем.</p>';
