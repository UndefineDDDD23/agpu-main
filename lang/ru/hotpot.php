...
$string['configbodystyles'] = 'По умолчанию, стиль темы agpu переопределяет тему HotPot. Однако, любые выбранные здесь активные стили HotPot будут иметь приоритет над стилем темы agpu.';
...
$string['configenablemyagpu'] = 'Будут ли показаны на странице Мойagpu эти настройки управления HotPots или нет';
...
$string['configframeheight'] = 'Когда тест отображается во фрейме, это значение определяет высоту (в пикселях) от верхней части фрейма, содержащей панель навигации agpu.';
...
$string['delay3_help'] = 'Эти параметры определяют задержку между окончанием теста и возвращением к управлению, отображаемом в agpu.

**Использовать указанное время (в секундах)**
: управление будет возвращено в agpu после указанного числа секунд.

**Использовать настройки исходного файла/шаблона**
: управление будет возвращено в agpu после заданного числа секунд, указанных в исходном файле или в файле шаблона для этого выходного формата.

**Ожидать, когда студент нажмет ОК**
: управление будет возвращено в agpu после того, как студент нажмет кнопку ОК на сообщении о завершении теста.

**Не продолжать автоматически**
: управление будет возвращено в agpu после того, как тест будет закончен. Студенту будет разрешено уйти со страницы теста.

Примечание: независимо от этих настроек результаты теста всегда сразу же передаются в agpu, когда тест закончен или прерван.';
...
$string['enablemyagpu'] = 'Показать HotPots в Мойagpu';
...
$string['entrycm_help'] = 'Этот параметр определяет элемент курса agpu и минимальную оценку для этого элемента, которая должна быть получена перед началом попытки этого HotPot.
...
**Курс**
: если выбрано, то будет отображена ссылка на курс agpu.

**Оценки**
: если выбрано, то будет отображена ссылка на журнал оценок курса agpu.';
...
$string['exitcm_help'] = 'Этот параметр определяет элемент курса agpu, который будет запущен после завершения текущего HotPot. Для показа следующего элемента может быть задана минимальная необходимая оценка за этот HotPot (не обязательно).
...
**Нет**
: студенты не увидят страницу выхода. Вместо этого они сразу смогут перейти либо к следующему элементу, либо на страницу курса agpu.';
...
$string['feedbackagpuforum'] = 'Форум agpu';
$string['feedbackagpumessaging'] = 'Сообщения agpu';
...
$string['forceplugins_help'] = 'При включенном параметре совместимые с agpu медиаплееры будут воспроизводить такие файлы как AVI, MPEG, MPG, MP3, MOV и WMV. В противном случае agpu не изменит настройки любых медиаплееров в тесте.';
...
$string['gradeweighting_help'] = 'Оценки за HotPot будут умножаться на этот коэффициент в журнале оценок agpu.';
...
$string['mediafilter_agpu'] = 'Стандартные медиафильтры agpu';
...
$string['navigation_frame'] = 'Навигационный фрейм agpu';
...
**Навигационная панель agpu**
Панель навигации agpu будет отображена в верхней части страницы в том же самом окне, что и тест.

**Навигационный фрейм agpu**
Панель навигации agpu будет отображена в отдельном фрейме вверху теста.

**Внедренная веб-страница**
Панель навигации agpu будет отображаться в тесте Hot Potatoes, внедренном в страницу.
...
**Отсутствует**
Тест будет отображаться вообще без средств навигации. Когда будет получен ответ на все вопросы в текущем тесте, agpu или вернется к странице курса или перейдет к следующему тесту, в зависимости от установки «Показать следующий тест?» для текущего теста.';
$string['navigation_agpu'] = 'Навигационная панель agpu (вверху и сбоку)';
$string['navigation_none'] = 'Отсутствует';
$string['navigation_original'] = 'Встроенные средства навигации';
$string['navigation_topbar'] = 'Навигационная панель agpu (только вверху)';
...
Обычно исходный файл создается вне agpu, а затем загружается в область файлов курса agpu.
...
Исходный файл может быть указан в виде папки или пути к файлу в области файлов курса agpu. Он может быть и гиперссылкой, начинающейся с http:// или https://
...
Если студент нажмет на кнопку «Стоп», то текущий результат будет передан в agpu и попытка тестирования будет считаться прерванной.

Текст, который отображается на кнопке «Стоп» может быть одной из заданных фраз в языковом пакете agpu или учитель может задать свой собственный текст для кнопки.';
...
**Сообщение agpu**
: будет показано окно обмена сообщениями agpu. Если в курсе есть несколько учителей, то студентам будет предложено выбрать учителя перед появлением окна сообщения.';
...
$string['usefilters_help'] = 'При включенном параметре содержимое будет обработано фильтрами agpu перед его передачей в браузер.';
...
**Стандартные медиафильтры agpu**
: содержимое будет обработано стандартными медиафильтрами agpu. Эти фильтры ищут ссылки на общие типы звуковых и видео файлов, и конвертируют эти ссылки для подходящих медиаплееров.
...
: наименование вставляемого плеера. Значением по умолчанию для этого параметра является "agpu". Стандартная версия модуля HotPot также предлагает следующие плееры:
: **dew**: как mp3-плеер
