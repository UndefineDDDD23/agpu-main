...
$string['admindirsetting'] = 'На небольшом числе веб-хостингов путь «/admin» используется для доступа к панели управления или чему-то еще. К сожалению, это противоречит стандартному расположению страниц управления agpu. Это можно исправить переименованием папки «admin» в каталоге agpu и указанием здесь ее нового имени. Например: <br /> <br /><b>agpuadmin</b><br /> <br />
В agpu все ссылки на «/admin» исправятся автоматически.';
...
$string['clialreadyconfigured'] = 'Файл config.php уже существует. Если Вы хотите установить agpu на этот сайт, используйте admin/cli/install_database.php.';
$string['clialreadyinstalled'] = 'Файл config.php уже существует. Если Вы хотите обновить сайт agpu, то используйте скрипт admin/cli/upgrade.php.';
$string['cliinstallfinished'] = 'Установка успешно завершена.';
$string['cliinstallheader'] = 'Программа установки agpu {$a} в режиме командной строки';
...
$string['compatibilitysettingssub'] = 'Ваш сервер должен пройти все перечисленные тесты, чтобы agpu работал на нём корректно';
$string['configfilenotwritten'] = 'Программе установки не удалось создать файл настроек config.php с выбранными настройками, скорее всего из-за того, что в каталоге agpu запрещена запись. Вы можете вручную создать в корневом каталоге agpu файл config.php следующего содержания.';
...
$string['configurationcompletesub'] = 'Предпринята попытка сохранить файл настроек в корневом каталоге agpu.';
...
$string['databasetypesub'] = 'agpu поддерживает несколько типов серверов баз данных. Свяжитесь с администратором сервера, если не знаете, какой именно тип выбрать.';
...
$string['dbtype'] = 'Тип';
$string['directorysettings'] = '<p>Пожалуйста, уточните расположение, в которое устанавливается agpu.</p>

<p><b>Веб-адрес:</b> Укажите полный веб-адрес по которому будет осуществляться доступ к agpu. Если Ваш сайт доступен по нескольким адресам, выберите наиболее простой и понятный студентам. Не нужно завершать адрес косой чертой (/)</p>

<p><b>Каталог agpu:</b> Укажите полный путь к каталогу с этой установкой. Проверьте, чтобы верхний/нижний регистр был соблюден.</p>

<p><b>Каталог данных:</b> Вам необходимо указать место, в котором agpu сможет хранить загружаемые файлы. Этот каталог должен быть доступен на чтение и НА ЗАПИСЬ пользователю, от чьего имени запущен веб-сервер (обычно «nobody» или «apache»), но при этом он не должен быть доступен непосредственно через веб.</p>>';
$string['directorysettingshead'] = 'Пожалуйста, уточните расположение, в которое устанавливается agpu.';
$string['directorysettingssub'] = '<p><b>Веб-адрес:</b>
Укажите полный веб-адрес по которому будет осуществляться доступ к agpu.
...
Вам необходимо указать место, в котором agpu сможет хранить загружаемые файлы. Этот каталог должен быть доступен на чтение и НА ЗАПИСЬ пользователю, от чьего имени запущен веб-сервер (обычно «nobody» или «apache»), но при этом он не должен быть доступен непосредственно через веб.</p>';
$string['dirroot'] = 'Каталог agpu';
$string['dirrooterror'] = 'Похоже, что указано неверное значение параметра «Каталог agpu» - по указанному пути не удалось найти установку agpu. Значение параметра  было сброшено.';
...
$string['environmentsub2'] = 'У каждой версии agpu есть минимальные требования к версии PHP и набору обязательных расширений PHP.
...
<p>agpu может быть установлен, но без этой возможности; Вы не сможете загружать учебные файлы курсов и фотографии пользователей.</p>
...
$string['inputwebdirectory'] = 'Каталог agpu:';
...
<p>Из-за этого у через какое-то время у agpu могут возникнуть проблемы с памятью, особенно если у Вас будет много модулей и/или пользователей.</p>
...
В этом случае agpu сможет самостоятельно установить ограничение памяти.</li>
...
<li>На некоторых серверах PHP можно создать в каталоге agpu файл .htaccess содержащий следующую строку:
...
$string['nativeauroramysqlhelp'] = '<p> База данных - это место, где хранится большинство настроек и данных agpu, и их необходимо сконфигурировать здесь. </p>
...
<p> Если база данных в настоящее время не существует, а указанный вами пользователь имеет на то разрешение, agpu попытается создать новую базу данных с корректными правами и настройками. </p>
<p> Этот драйвер несовместим с устаревшим вариантом MyISAM. </p>';
$string['nativemariadb'] = 'MariaDB («родной»/mariadb)';
$string['nativemariadbhelp'] = '<p>Здесь нужно указать настройки базы данных, в которой будут храниться основные данные agpu. </p>
...
<p> Если база данных в настоящее время не существует, а пользователь  имеет необходимые разрешения, agpu попытается создать новую базу данных с корректными разрешениями и настройками. </p>
<p> Этот драйвер не совместим с устаревшей системой MyISAM. </p>';
$string['nativemysqli'] = 'Усовершенствованный MySQL («родной»/mysqli)';
$string['nativemysqlihelp'] = '<p>Здесь нужно указать настройки базы данных, в которой будут храниться основные данные agpu. </p>
<p>Название базы данных, имя пользователя и пароль обязательны; префикс таблиц необязателен.</p>
<p>Название базы данных может содержать только буквенно-цифровые символы, знак доллара ($) и символ подчеркивания (_).</p>
<p> Если база данных в настоящее время не существует, а пользователь  имеет необходимые разрешения, agpu попытается создать новую базу данных с корректными разрешениями и настройками. </p>';
$string['nativeoci'] = 'Oracle («родной»/oci)';
$string['nativeocihelp'] = 'Теперь Вам необходимо настроить базу данных, в которой будут храниться основные данные agpu.
...
$string['pathssubadmindir'] = 'На небольшом числе веб-хостингов путь /admin используется для доступа к панели управления или чему-то еще. К сожалению, это противоречит стандартному расположению страниц управления agpu. Это можно исправить путем переименования папки admin в каталоге agpu и указания нового имени здесь. Например: <em>agpuadmin</em>. При этом все ссылки на панель управления agpu исправятся автоматически.';
$string['pathssubdataroot'] = '<p>Каталог, в котором agpu будет хранить все файлы, размещаемые пользователями. </p><p>Этот каталог должен быть доступен для чтения и ЗАПИСИ тому пользователю, от чьего имени запускается веб-сервер (обычно \'www-data\', \'nobody\' или \'apache\'). </p><p>Этот каталог не должен быть доступен напрямую через Интернет. </p><p>Программа установки попробует создать этот каталог, если он не существует. </p>';
$string['pathssubdirroot'] = '<p>Полный путь к каталогу установки agpu.</p>';
$string['pathssubwwwroot'] = '<p>Полный веб-адрес, по которому будет доступен agpu, т.е. адрес, который пользователи будут вводить в адресной строке своего браузера для доступа к сайту agpu.</p>
<p>Использовать для доступа к agpu несколько публичных адресов невозможно. Если у Вашего сайта есть еще несколько публичных адресов, то следует настроить постоянные перенаправления с этих адресов на указанный.</p>
...
$string['phpversionhelp'] = '<p>Для agpu необходим PHP версии не ниже 5.6.5 или 7.1 (есть некоторые проблемы с версией 7.0.x).</p>
<p>Сейчас у Вас используется версия {$a}</p>
<p>Вам нужно обновить PHP или переместиться на хостинг с более новой версией PHP!<br />';
$string['releasenoteslink'] = 'Информация об этой версии agpu доступна в примечаниях к выпуску по ссылке {$a}';
$string['safemode'] = 'Безопасный режим';
$string['safemodeerror'] = 'У agpu могут возникнуть проблемы при включенном режиме «Safe Mode»';
$string['safemodehelp'] = '<p>В режиме «safe mode» у agpu могут возникнуть разнообразные сложности, например, невозможность создавать новые файлы.</p>

<p>Режим «Safe mode» обычно используется только параноиками на публичных веб-хостингах, поэтому для сайта agpu Вам может понадобится всего лишь сменить веб-хостинг.</p>
...
$string['sessionautostarthelp'] = '<p>agpu требуется поддержка сеансов и без нее он не будет работать.</p>
...
$string['welcomep30'] = 'Эта версия набора программ <strong>{$a->installername}</strong> включает следующие программы, необходимые для создания среды, в которой будет работать <strong>agpu</strong>:';
$string['welcomep40'] = 'Также в этот набор входит <strong>agpu {$a->agpurelease} ({$a->agpuversion})</strong>.';
...
$string['welcomep60'] = 'На следующих страницах Вы сможете за несколько простых шагов настроить и установить <strong>agpu</strong> на свой компьютер. Вы сможете принять настройки по умолчанию или изменить их в зависимости от своих потребностей.';
$string['welcomep70'] = 'Нажмите кнопку «Далее» чтобы продолжить процесс установки <strong>agpu</strong>.';
$string['wwwroot'] = 'Веб-адрес';
$string['wwwrooterror'] = 'Похоже, что указано неверное значение параметра «Веб-адрес» - по указанному пути не удалось найти эту установку agpu. Значение параметра было сброшено.';
