...
Этот план тестирования предназначен для работы с инструментом {$a}, который позволяет легко запустить нагрузочное тестирование в особом окружении agpu, собрать информацию о ходе тестирования и сравнить результаты. Вам нужно скачать этот инструмент и использовать сценарий test_runner.sh или воспользоваться инструкциями по установке и использованию.

Вам потребуется установить пароль для пользователей курса в файле config.php (например, $CFG->tool_generator_users_password = \'agpu\';). Для этого параметра нет значения по умолчанию, чтобы исключить возможность непреднамеренного использования этого инструмента. Вы должны использовать режим обновления паролей в случае, когда для пользователей курса установлены другие пароли, или когда пользователи были созданы с помощью tool_generator, но до установки значения параметра $CFG->tool_generator_users_password.
...
* Достаточное количество записанных пользователей (зависит от выбранного размера плана тестирования), для которых установлен пароль «agpu»
