...
$string['expireyourkeyexplain'] = 'agpu автоматически изменяет Ваши ключи каждые 28 дней (по умолчанию), но Вы можете в любой момент <em>вручную</em> отметить этот ключа как просроченный. Это необходимо только в том случае, если Вы считаете, что ключ был скомпрометирован. Новый ключ будет создан автоматически сразу же после этого действия. <br />Удаление этого ключа сделает невозможным взаимодействие других приложений с Вашим сайтом до тех пор, пока Вы не свяжетесь с каждым из администраторов других сайтов сети MNet и не передадите им свой новый ключ.';
...
$string['agpu_home_help'] = 'Путь к домашней странице приложения MNet на удаленном сайте, например: /agpu/.';
$string['name'] = 'Имя';
...
$string['notagpuapplication'] = 'ПРЕДУПРЕЖДЕНИЕ: Это не приложение agpu, некоторые из методов проверки могут работать неверно';
$string['notpermittedtojump'] = 'У Вас нет прав для начала удаленного сеанса с этого сервера agpu.';
...
$string['trustedhostsexplain'] = '<p>Механизм доверенных адресов разрешает определенным машинам выполнять вызовы по протоколу XML-RPC к любой части agpu API. Это позволит управлять поведением agpu извне, поэтому установка этого параметра - очень опасное действие. Если Вы не знаете, что делать, лучше оставьте это поле пустым. </p>
