...
$string['configmessageinboundhost'] = 'Адрес сервера, на котором agpu должен проверять почту. Чтобы задать порт не по умолчанию, Вы можете использовать формат [сервер]:[порт]; например,  mail.example.com:993. Если оставить это поле пустым, то agpu будет использовать порт по умолчанию для указанного типа почтового сервера.';
...
$string['incomingmailserversettings_desc'] = 'agpu может подключаться к настроенным соответствующим образом серверам IMAP. Здесь Вы можете указать параметры подключения к серверу IMAP.';
...
$string['invalidrecipientfinal'] = 'Не удается проверить подлинность отправленного Вами сообщения с темой «{$a->subject}». Пожалуйста, убедитесь, что Вы отправляете сообщение с адреса электронной почты, указанного в Вашем профиле agpu.';
...
$string['messageinboundgeneralconfiguration_desc'] = 'Обработка входящих сообщений позволяет получать и обрабатывать электронную почту через agpu. Это можно использовать для того, чтобы путем отправки сообщений электронной почты отвечать на сообщения на форумах или добавлять файлы в область личных файлов пользователя.';
...
$string['messageinboundhostssl_desc'] = 'Некоторые почтовые серверы поддерживают дополнительный уровень защиты с помощью шифрования связи между agpu и сервером. Рекомендуется использовать шифрование SSL, если сервер его поддерживает.';
...
$string['messageinboundmailboxconfiguration_desc'] = 'При отправке сообщений используется формат «address+данные@example.com». Чтобы система agpu могла генерировать корректные адреса, укажите имя почтового ящика (будет использоваться в адресе перед знаком «@») и домен (будет использоваться после знака «@»). В приведенном примере имя почтового ящика будет «address», а домен электронной почты - «example.com». Для этой цели следует использовать отдельную учетную запись электронной почты.';
...
$string['validateaddress_help'] = 'При получении сообщения от пользователя, agpu сверяет адрес электронной почты отправителя с адресом электронной почты в профиле пользователя.
