...
$string['auth_changepasswordhelp_expl'] = 'Показать подсказку по смене пароля пользователям, которые забыли свой {$a} пароль. Она может отображаться как вместо, так и вместе со ссылкой на <strong>Адрес страницы смены пароля</strong> или внутренней страницей смены пароля системы agpu.';
...
$string['auth_fieldlocks_help'] = '<p>Вы можете запретить пользователям редактировать некоторые поля профиля. Это полезно для сайтов, где данные пользователей редактируются администратором вручную или загружаются с помощью команды «Загрузить пользователей». Если Вы блокируете поля, необходимые для работы agpu, то убедитесь, что предоставляете всю информацию, необходимую для создания учетной записи, иначе учетная запись не будет работать.</p><p>Чтобы избежать подобных проблем, можно попробовать установить блокировку в режим «Разблокировано, если пусто».</p>';
...
$string['forcechangepassword_help'] = 'Заставить пользователей сменить пароль при следующем входе в agpu.';
$string['forcechangepasswordfirst_help'] = 'Заставить пользователей сменить пароль при первом входе в agpu.';
$string['forgottenpassword'] = 'Если вы введете здесь URL веб-страницы, она будет использоваться для восстановления забытого пароля сайта. Эта настройка предназначена для сайтов, где управление паролями осуществляется вне agpu. Для использования стандартной процедуры восстановления пароля оставьте это поле пустым.';
...
$string['stdchangepassword_expl'] = 'Если внешняя система аутентификации позволяет изменение пароля через agpu — выберите «Да». Этот параметр преобладает над параметром «Адрес страницы смены пароля».';
