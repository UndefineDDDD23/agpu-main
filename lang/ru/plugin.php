...
$string['incompatibleversion'] = 'Несовместимая версия agpu: {$a}';
...
$string['notsupported'] = 'Плагин может быть не совместим с версией agpu {$a}';
...
$string['supportedagpuversions'] = 'Поддерживаемые версии agpu';
...
$string['uninstallconfirm'] = 'Вы собираетесь удалить плагин «<em>{$a->name}</em>». При этом из базы данных будет удалено всё, что связано с этим плагином, в том числе его настройки, записи журнала событий, файлы пользователя, управляемые с помощью плагина и т.д. Назад пути не будет - системой agpu не будет создано никаких резервных копий для последующего восстановления плагина. Вы УВЕРЕНЫ, что хотите продолжить?';
$string['uninstalldelete'] = 'Все данные, связанные с плагином «<em>{$a->name}</em>» были удалены из базы данных. Для предотвращения самоустановки плагина, его каталог «<em>{$a->rootdir}</em>» теперь должен быть вручную удален с сервера. agpu самостоятельно не может удалить каталог из-за отсутствия прав на запись.';
...
$string['updatepluginconfirminfo'] = 'Вы собираетесь установить новую версию плагина  «<strong>{$a->name}</strong>». ZIP-пакет с версией {$a->version} этого плагина будет загружен по ссылке <a href="{$a->url}">{$a->url}</a> и распакован в каталог с системой agpu, чтобы произвести обновление.';
$string['updatepluginconfirmwarning'] = 'Обратите внимание, что agpu перед обновлением автоматически не сделает резервную копию базы данных. Вам настоятельно рекомендуется сейчас сделать полную резервную копию, чтобы справиться с редким случаем, когда новый код содержит ошибки, которые сделают Ваш сайт недоступным или даже испортят базу данных. Вы действуете на свой страх и риск.';
...
$string['validationmsg_requiresagpu'] = 'Необходимая версия agpu';
