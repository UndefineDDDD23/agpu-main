...
/**
 * Strings for component 'auth_oidc', language 'ru', version '4.5'.
 *
 * @package     auth_oidc
 * @category    string
...
<br /><b>Примечание:</b> В случае, когда существуют конфликтующие логины - уже есть пользователь agpu с тем же логином, то используется приоритет подключаемого модуля аутентификации для определения того, какой пользователь выиграл.';
...
$string['cfg_debugmode_desc'] = 'При включенном параметре в журнал agpu будет заноситься информация, которая может помочь в выявлении проблем.';
...
$string['cfg_field_mapping_desc'] = 'Данные профиля пользователя agpu можно сопоставить с данными провайдера идентификации (IdP) OpenID Connect.<br/>
<ul>
<li>Основные данные профиля доступны из токенов ID всех провайдеров идентификации.</li>
<li>Если  в качестве IdP используется Azure AD, то дополнительные данные профиля можно сделать доступными, установив и настроив <a href="https://agpu.org/plugins/local_o365">подключаемый модуль интеграции Microsoft 365 (local_o365)</a>.</li>
<li>Если в подключаемом модуле local_o365 включена функция синхронизации профиля SDS, то определенные поля профиля можно синхронизировать из SDS в agpu при выполнении запланированной задачи "Синхронизация с SDS". <br/>При этом синхронизации не будет ни при выполнении запланированной задачи "Синхронизация пользователей с Azure AD", ни при входе пользователя в систему.</li>
</ul>';
...
$string['cfg_loginflow_authcode_desc'] = 'Используя этот метод, пользователь щелкает на названии провайдера идентификации (см. выше «Название провайдера») на странице входа в agpu и перенаправляется к провайдеру для входа. После успешного входа пользователь перенаправляется обратно в agpu, где вход в agpu происходит автоматически. Это стандартный и наиболее безопасный способ входа.';
...
$string['cfg_loginflow_rocreds_desc'] = 'Используя этот метод, пользователь вводит свои логин и пароль в форму входа в agpu, как при ручном входе. Это авторизует пользователя у провайдера идентификации, но не создает сеанс на сайте провайдера. Например, если вы используете  Microsoft 365 с OpenID Connect, то пользователь будет входить в agpu, но не через веб-приложения  Microsoft 365. Рекомендуется использовать запрос авторизации, если вы хотите, чтобы пользователи входили как в agpu, так и в провайдера идентификации. Обратите внимание, что не все провайдеры идентификации поддерживают этот метод. Эту опцию следует использовать только в том случае, если другие типы прав доступа недоступны.';
...
$string['errorauthloginfailednouser'] = 'Неудачный вход: пользователь не найден в agpu. Если на этом сайте включен параметр «authpreventaccountcreation», это может означать, что сначала вам понадобится администратор для создания учетной записи.';
...
$string['errorauthuserconnectedtodifferent'] = 'Аутентифицированный пользователь OpenID Connect уже подключен к пользователю agpu.';
$string['errorbadloginflow'] = 'Указан неверный тип аутентификации. Примечание: Если вы получили это сообщение после недавней установки или обновления, очистите кэш agpu.';
...
$string['errorlogintoconnectedaccount'] = 'Этот пользователь Microsoft 365 подключен к учетной записи agpu, но вход в систему OpenID Connect не включен для этой учетной записи agpu.
Войдите в учетную запись agpu, используя определенный метод аутентификации учетной записи, чтобы использовать функции Microsoft 365.';
...
$string['errorusermatched'] = 'Учетная запись Microsoft 365 "{$a->aadupn}" уже сопоставлена с пользователем agpu "{$a->username}". Чтобы завершить подключение, сначала войдите в систему как этот пользователь agpu и следуйте инструкциям в блоке Microsoft.';
...
$string['mismatched_details'] = 'Запись токена содержит имя пользователя "{$a->tokenusername}"; соответствующий пользователь agpu имеет имя пользователя "{$a->agpuusername}".';
...
$string['privacy:metadata:auth_oidc_prevlogin:userid'] = 'ID пользователя agpu';
...
$string['privacy:metadata:auth_oidc_token:userid'] = 'ID пользователя agpu.';
$string['privacy:metadata:auth_oidc_token:username'] = 'Логин пользователя agpu.';
...
$string['ucp_disconnect_details'] = 'Это отключит вашу учетную запись agpu от {$a}. Вам нужно будет создать логин и пароль для входа в agpu.';
$string['ucp_disconnect_title'] = 'Отключение {$a}';
$string['ucp_general_intro'] = 'Здесь вы можете управлять своим соединением с {$a}. Если включено, вы сможете использовать свою учетную запись {$a} для входа в agpu вместо отдельного логина и пароля. После подключения вам больше не придется запоминать логин и пароль для agpu, все входы в систему будут обрабатываться {$a}.';
$string['ucp_login_start'] = 'Начать использовать {$a} для входа в agpu';
$string['ucp_login_start_desc'] = 'Это переключит вашу учетную запись на использование {$a} для входа в agpu. Вы будете входить в систему, используя свои учетные данные {$a} - ваши логин и пароль agpu не будут работать. Вы  в любое время можете отключить свою учетную запись и вернуться к входу в систему в обычном режиме.';
$string['ucp_login_stop'] = 'Прекратить использовать {$a} для входа в agpu';
$string['ucp_login_stop_desc'] = 'Сейчас для входа в agpu вы используете {$a}. Нажатие «Прекратить использовать {$a} для входа в agpu» отключит вашу учетную запись agpu от {$a}. Вы больше не сможете войти в agpu под своей учетной записью {$a}. Вам будет предложено создать логин и пароль, и потом вы сможете напрямую войти в agpu.';
