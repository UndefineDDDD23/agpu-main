...
$string['aiccuserid_desc'] = 'Стандарт AICC для имен пользователей очень жесткий по сравнению с agpu и допускает только цифры, буквы, дефис и подчеркивание. Точки, пробелы и символ @ не допускаются. Если параметр включен, в пакет AICC передаются числовые ID пользователей вместо их имен.';
...
$string['scormstandarddesc'] = 'Когда этот параметр отключен, agpu позволяет пакетам SCORM 1.2 хранить больше, чем позволяет спецификация, и использует настройки формата полного имени agpu при передаче имени пользователя в пакет SCORM.';
