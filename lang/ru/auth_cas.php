...
$string['auth_cas_broken_password'] = 'Вы не можете обойтись без изменения пароля, однако нет доступных страниц для его изменения. Пожалуйста, свяжитесь с администратором agpu.';
...
$string['auth_cas_create_user'] = 'Включите этот параметр, если хотите внести CAS-аутентификацию пользователей в базу данных agpu. Если нет, то только те пользователи, которые уже существуют в базе данных agpu, смогут войти в систему.';
...
$string['auth_cas_logout_return_url'] = 'Введите URL-адрес, куда пользователи CAS должны быть перенаправлены после выхода из системы. <br /> Если оставить пустым, то пользователь будет перенаправлен в то место, куда  пользователей перенаправляет agpu.';
$string['auth_cas_logout_return_url_key'] = 'Альтернативный URL-адрес перенаправления при выходе';
$string['auth_cas_logoutcas'] = 'Выберите «Да», если хотите выйти из CAS при выходе из agpu';
...
$string['auth_casdescription'] = 'Этот метод использует сервер CAS (Центральная служба проверки подлинности) для аутентификации пользователей в контексте единого входа в систему (SSO). Вы также можете использовать простую LDAP аутентификацию. Если данное имя пользователя и его пароль действительны в соответствии с CAS, то, при необходимости, agpu создаст новую запись пользователя в своей базе данных, принимая атрибуты пользователя из LDAP. При следующем входе проверяются только имя пользователя и пароль.';
