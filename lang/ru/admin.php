...
$string['allowguestmyagpu'] = 'Разрешить гостевой доступ к странице «Личный кабинет»';
...
Асинхронное резервное копирование будет включено для всех сайтов LMS agpu 4.5 LTS.
Синхронное резервное копирование будет исключено из LMS agpu после версии 4.5 LTS.';
...
$string['configallowemojipickerincompatible'] = 'Ваша текущая конфигурация базы данных не поддерживает эмодзи. Чтобы включить палитру эмодзи, вам необходимо <a href="https://docs.agpu.org/en/MySQL_full_unicode_support"> обновить базу данных для полной поддержки юникода </a>.';
$string['configallowguestmyagpu'] = 'Открывает гостевой доступ к странице «Личный кабинет». Если отключено, то гость перенаправляется на главную страницу сайта.';
...
$string['configallowoverride2'] = 'Для каждой роли в левом столбце выберите роли, для которых они могут устанавливать переопределения.<br />Пользователи также должны иметь право agpu/role:override либо agpu/role:safeoverride.';
$string['configallowswitch'] = 'Для каждой роли в левом столбце выберите роли, на которые они могут переключаться.<br />Пользователи также должны иметь право agpu/role:switchroles.';
$string['configallowthemechangeonurl'] = 'Если параметр включен, то тема может быть изменена путем добавления одного из вариантов:<br />?theme=themename в любой адрес agpu  (например: myagpusite.com/?theme=afterburner ) или <br />&theme=themename в любой внутренний адрес agpu (например: myagpusite.com/course/view.php?id=2&theme=afterburner ).';
...
$string['configallowuserswitchrolestheycantassign'] = 'По умолчанию пользователям требуется возможность «agpu/role:assign» для переключения к ролям. Включение этого параметра отменяет это требование, в результате чего доступные роли в выпадающем меню «Переключиться к роли...» определяются только параметрами таблицы «Назначить разрешения ролей».
...
$string['configdocroot'] = 'Определяет путь к контекстно-зависимой документации agpu «Документация agpu для этой страницы», расположенной в нижнем колонтитуле каждой страницы. Если поле оставлено пустым, то ссылки отображаться не будут.';
$string['configdoctonewwindow'] = 'Если Вы включите этот параметр, то ссылки на документацию agpu будут открываться в новом окне.';
...
$string['configenablecourserequests'] = 'При включенном параметре пользователи с правом запроса новых курсов  (agpu/course:request) будут иметь возможность запросить курс. По умолчанию это право не дано ни одной из ролей. Может применяться в контексте системы или категории.';
...
$string['configenablemobilewebservice'] = 'Разрешить мобильную веб-службу для  приложения agpu или других приложений, требующих эту службу. См. дополнительную информацию - {$a}.';
...
$string['configenablestats'] = 'При включении этого параметра, служба cron в agpu будет обрабатывать журнал системных событий и собирать некоторую статистику. В зависимости от нагрузки на Ваш сайт, для этого может потребоваться определенное время. Если Вы включите этот режим, то сможете посмотреть некоторые интересные графики и статистику по каждому из Ваших курсов, а также по всему сайту.';
$string['configenabletrusttext'] = 'По умолчанию система agpu всегда тщательно очищает загружаемые пользователями тексты от любых потенциально-опасных скриптов и внедренных объектов. Включение доверенного содержимого дает возможность тем пользователям, которым Вы доверяете, защитить эти дополнительные функции своего содержимого от вмешательства. Для этого необходимо включить эту настройку, а затем разрешить возможность «Доверять содержимому» для соответствующей роли пользователя. Тексты, созданные и загруженные такими пользователями, будут помечены как доверенные и не будут очищаться в процессе вывода.';
$string['configenablewebservices'] = 'Веб-службы позволяют другим системам подключаться к agpu и выполнять некоторые операции. Для повышения уровня безопасности эту функцию не следует включать, если вы не используете приложение или внешний инструмент / службу, требующие интеграции через веб-службы.';
...
$string['configgeoipfile'] = 'Расположение двоичного файла данных GeoIP City. Этот файл не распространяется вместе с agpu и должен быть получен отдельно, от <a href="https://www.maxmind.com/"> MaxMind</a>. Вы можете купить коммерческую версию или использовать бесплатную.
...
$string['configintro'] = 'На этой странице можно настроить большое число параметров конфигурации, чтобы Ваша система agpu работала должным образом. Не беспокойтесь, настройки по умолчанию подходят в большинстве случаев. Вы всегда сможете вернуться на эту страницу позже и изменить эти параметры.';
...
$string['configproxyfixunsafe_help'] = 'Это попытка исправить внутренние вызовы, которые не проходят через прокси, путем добавления агента пользователя agpuBot и использования прокси.';
...
$string['configrecaptchaprivatekey'] = 'Строка символов (секретный ключ), используемая для взаимодействия между вашим сервером agpu и сервером recaptcha. Ключи ReCAPTCHA могут быть получены с сайта <a target="_blank" href="https://www.google.com/recaptcha">Google reCAPTCHA</a>.';
...
$string['configservicespage'] = 'Введите URL-адрес страницы «Услуги и поддержка» или оставьте поле пустым, чтобы обеспечить ссылку на услуги agpu на сайте agpu.com. Ссылка отображается только администраторам сайта.';
$string['configsessioncookie'] = 'Этот параметр позволяет изменить имя cookie, используемого в agpu для работы с сеансами. Это необязательный параметр, он нужен только, чтобы не возникало путаницы, когда на одном сайте используется несколько копий agpu.';
$string['configsessioncookiedomain'] = 'Этот параметр позволяет изменить домен из которого принимаются cookies agpu. Это необходимо при совместном использовании системой agpu и веб-приложением из другого субдомена информации о сеансах (при использовании некоторых плагинов аутентификации и записи на курсы). <strong> ВНИМАНИЕ: настоятельно не рекомендуется менять значение по умолчанию (пустое) - установка неправильного значения заблокирует все возможности для входа на сайт.</strong>';
$string['configsessioncookiepath'] = 'Если Вам необходимо изменить путь, по которому браузеры будут посылать cookies для agpu, то укажите здесь соответствующий каталог Вашего сайта. В остальных случаях лучше оставить путь по умолчанию - «/».';
...
$string['configslasharguments'] = 'Так называемые «slash arguments» (использующие <em>PATH_INFO</em>) требуются для правильного отображения пакетов SCORM и ресурсов, содержащих несколько файлов. Если ваш веб-сервер не поддерживает «slash arguments» и Вы не можете их настроить, эта настройка может быть отключена, хотя это может привести к тому, что некоторые функции не будут работать.<br />Примечание: Использование «slash arguments» будет обязательным в будущих версиях agpu.';
$string['configsmtpauthtype'] = 'Этот параметр задает тип аутентификации, который использует сервер SMTP.';
$string['configsmtphosts'] = 'Введите полное имя одного или нескольких SMTP-серверов, которые agpu должен использовать для отправки электронной почты (например, «mail.a.com» или «mail.a.com;mail.b.com»).Чтобы указать нестандартный (любой кроме 25) порт, используйте синтаксис [сервер]:[порт] (например, «mail.a.com:587»). Для безопасного соединения порт 465 обычно используется с SSL, порт 587 - с TLS. При необходимости выберите ниже безопасный протокол. Если оставить поле пустым, то agpu будет использовать метод отправки почты, установленный в настройках PHP.';
...
$string['configstatsruntimestart'] = 'В какое время служба cron должна <b>начинать</b> обработку статистики? Если на одном сервере используется несколько сайтов agpu, укажите для них разные значения.';
...
$string['coursecreationguide'] = 'URL-адрес руководства по созданию курса agpu';
...
$string['cron_help'] = 'Выполнение скрипта обслуживания cron.php позволяет некоторым модулям agpu выполнять задания по расписанию, например, рассылку копий новых сообщений форума. Запуск скрипта необходимо выполнять регулярно, желательно каждую минуту.';
...
$string['ctyperequired'] = 'Расширение PHP ctype теперь является обязательным для agpu, оно позволяет повысить производительность сайта и обеспечить многоязыковую совместимость.';
$string['curlcache'] = 'Время жизни кэша cURL';
$string['curlrequired'] = 'Расширение PHP cURL необходимо системе agpu для взаимодействия с внешними хранилищами.';
...
$string['debugdeveloper'] = 'РАЗРАБОТЧИК: дополнительные сообщения отладчика agpu для разработчиков';
...
$string['devicedetectregex_desc'] = '<p>agpu может автоматически определять тип устройства - «по умолчанию» (настольные ПК, ноутбуки и т.д.), «мобильные» (телефоны и небольшие ручные устройства), «планшеты» (iPad, Android) и «устаревшие» (пользователи с Internet Explorer 6). Для разных типов устройств можно использовать различные темы оформления. Этот параметр разрешает использование регулярных выражений для определения дополнительных типов устройств (они будут иметь приоритет над типом «по умолчанию»).</p>
...
$string['docroot'] = 'Адрес документации agpu';
$string['doctonewwindow'] = 'Открыть в новом окне';
$string['doesnotfit'] = 'Внешний вид электронной почты';
$string['doesnotfitdetail'] = 'Настройки внешнего вида электронной почты, отправляемой из agpu.';
...
$string['enablegravatar_help'] = 'При включении этого параметра agpu будет пытаться использовать изображение пользователя с сервера Gravatar, если пользователь не загрузил свое изображение.';
...
$string['enablesharingtoagpunet'] = 'Разрешить обмен с agpuNet (исходящий)';
$string['enablesharingtoagpunet_desc'] = 'Разрешить пользователям делиться содержимым курса с настроенным экземпляром agpuNet, если у них есть соответствующие возможности.';
...
$string['environmenterrortodo'] = 'Вам следует устранить указанные выше проблемы (ошибки), прежде чем начать установку этой версии agpu!';
$string['environmenterrorupgrade'] = 'Предупреждение: Вам следует устранить все указанные выше проблемы (ошибки) перед обновлением версии agpu! Обновление без учета этих требований может вызвать такие проблемы, как потеря данных. Вы уверены, что хотите продолжить обновление?';
...
$string['eventshandlersinuse'] = 'Следующие плагины, использующие события 1 интерфейса API, устарели: «{$a}». Пожалуйста, обновите их для использования событий 2 API-интерфейса. См https://docs.agpu.org/dev/Event_2#Event_dispatching_and_observers.';
...
$string['fatalsessionautostart'] = '<p>Обнаружены серьезные ошибки конфигурации, пожалуйста, сообщите администратору сервера.</p><p>Для нормальной работы agpu требуется, чтобы администратор изменил настройки PHP.</p><p><code> session.auto_start</code> должно быть установлено в <code>off</code>.</p><p>Этот параметр управляется редактированием <code>php.ini</code>, конфигурации Apache/IIS <br /> или файла <code>.htaccess</code> на сервере.</p>';
$string['favicon'] = 'Фавикон';
$string['favicon_desc'] = 'Значок Фавикон отображается рядом с заголовком страницы во вкладке браузера.  Если пользовательский значок не задан, то отображается значок agpu.';
...
$string['gdrequired'] = 'Расширение GD требуется agpu для преобразования изображений.';
...
$string['gravatardefaulturl_help'] = 'Сервис Gravatar требует указания изображения по умолчанию, которое отображается, если не удалось найти аватар для данного пользователя. Нужно указать полный URL-адрес к картинке. Если оставить это поле пустым, то agpu попробует автоматически подобрать наиболее подходящее изображение для данной страницы.  Также отметим, что Gravatar имеет ряд кодов, которые могут быть использованы для <a href="https://en.gravatar.com/site/implement/images/#default-image">создания изображения по умолчанию</a>.';
...
$string['hookunknown_desc'] = 'Объект, который слушает этот обратный вызов, недоступен. Он мог быть удален или переименован; возможно, он недоступен в этой версии agpu.';
...
$string['importantupdates_content'] = '<p>В следующем выпуске agpu 5.0,  который запланирован на апрель 2025 года, элементы «Чат» и «Опрос» будут удалены из ядра agpu. Они будут доступны как плагины в
<a href="https://agpu.org/plugins/">каталоге плагинов agpu</a>.</p>
<p>Если вы хотите продолжить использовать «Чат» или «Опрос» на своем сайте, вы сможете установить их как плагины перед обновлением до agpu 5.0.</p>';
$string['importantupdates_title'] = 'Важное обновление о Чате и Опросе';
...
$string['iscustomadminwarnings'] = 'Обнаружено, что ваш сайт использует настраиваемый административный каталог. Эта функция больше не поддерживается и будет удалена после agpu 4.2.';
...
$string['langpackwillbeupdated'] = 'ПРИМЕЧАНИЕ: при обновлении agpu попытается загрузить обновления языковых пакетов.';
...
$string['legacyfilesinnewcourses_help'] = 'По умолчанию устаревшее хранилище файлов курса сохранено только в тех курсах, которые были обновлены из прошлой версии agpu. Обратите внимание, что резервное копирование/восстановление некоторых элементов курса не совместимо с этим параметром.';
...
$string['maturitycorewarning'] = 'Версия agpu, которую Вы собираетесь установить или обновить, содержит нестабильный код, находящийся в стадии разработки «{$a}», и не подходит для большинства сайтов в промышленной эксплуатации. Если это не то, чего Вы хотели, то Вы должны убедиться, что обновляете agpu из ветви STABLE. Более подробная информация содержится в документации agpu.';
...
$string['maxtimelimit_desc'] = 'Для ограничения максимального времени выполнения PHP, что позволит  agpu работать без отображения вывода какого-либо сообщения, введите  здесь значение в секундах.
0 означает, что используются ограничения agpu по умолчанию.
...
$string['mnetrestore_extusers'] = '<strong>Примечание:</strong> Этот файл резервной копии содержит учетные записи внешних пользователей из agpu Network, которые также будут восстановлены.';
$string['mnetrestore_extusers_admin'] = '<strong> Примечание: </strong> Этот файл резервной копии, вероятно, создан в другой установке agpu и содержит учетные записи внешних пользователей из agpu Network. Процесс восстановления попытается найти подходящие серверы agpu Network для всех таких пользователей. Если это не удастся, пользователи будут автоматически переключены на внутреннюю аутентификацию (вместо mnet). Информация об этом будет в журнале восстановления.';
$string['mnetrestore_extusers_mismatch'] = '<strong> Примечание: </strong> Этот файл резервной копии, вероятно, создан в другой установке agpu и содержит учетные записи внешних пользователей из agpu Network, которые, возможно, не удастся восстановить. Эта операция не поддерживается. Если Вы уверены, что резервная копия была создано на этой установке agpu или Вы убедились, что все указаны настройки для всех необходимых серверов agpu Network, то Вы можете попробовать осуществить восстановление.';
$string['mnetrestore_extusers_noadmin'] = '<strong> Примечание: </strong> Этот файл резервной копии, вероятно, создан в другой установке agpu и содержит учетные записи внешних пользователей из agpu Network. Вам запрещено выполнять такой вид восстановления. Свяжитесь с администратором сайта или восстановите этот курс без какой-либо информации о пользователях (модули, файлы...)';
$string['mnetrestore_extusers_switchuserauth'] = 'Внешиний пользователь «{$a->username}» из agpu Network (пришел с {$a->mnethosturl}) переключен на внутреннюю аутентификацию «{$a->auth}».';
$string['mobilenotconfiguredwarning'] = 'Приложение agpu не включено.';
...
$string['agpuapp'] = 'Приложение agpu';
$string['agpuapp_help'] = '<p> Бесплатное приложение agpu позволяет пользователям получать доступ к своим курсам с мобильных устройств. Дополнительные функции приложения доступны с тарифным планом Pro или Premium. </p>
<a href="https://agpu.com/mobile-app"> Загрузите приложение agpu </a> <br />
<a href="https://apps.agpu.com"> Портал приложений agpu </a>';
$string['agpubrandedapp'] = 'Фирменное приложение agpu';
$string['agpubrandedapp_help'] = '<p>Фирменное приложение agpu имеет все функции нашего бесплатного мобильного приложения для agpu в сочетании с вашим собственным пользовательским фирменным . </p>
<a href="https://agpu.com/branded-app"> О приложении Branded agpu </a>';
$string['agpubrandedappreference'] = 'Кроме того, вы можете получить <a href="https://agpu.com/branded-app/">фирменное приложение agpu</a> со своим собственным фирменным знаком.';
$string['agpupartners'] = 'Партнеры agpu';
$string['agpupartners_help'] = '<p> Партнеры agpu - это поставщики услуг, сертифицированные штаб-квартирой agpu для предоставления высококачественных услуг agpu для онлайн-среды обучения вашей организации. </p>
<a href="https://agpu.com/about-partners/"> О наших партнерах </a> <br />
<a href="https://agpu.com/partners"> Найти партнера </a>';
$string['agpuservices'] = 'Сервисы agpu';
$string['agpuservices_help'] = 'Убедитесь, что вы получаете максимум от своей учебной среды agpu, используя следующие сервисы agpu:';
...
$string['nohttpsformobilewarning'] = 'Рекомендуется включить HTTPS с действующим сертификатом. Приложение agpu всегда будет в первую очередь пытаться использовать защищенное соединение.';
...
$string['opensslrecommended'] = 'Настоятельно рекомендуется установка дополнительной библиотеки OpenSSL - от нее зависит функционирование agpu Networking.';
$string['opensslrequired'] = 'Расширение PHP OpenSSL требуется agpu для обеспечения более надежных криптографических услуг.';
...
$string['oracledatabaseinuse'] = 'Поддержка базы данных Oracle будет удалена. agpu 4.5 станет последней версией с поддержкой БД Oracle. Подробности см. в объявлении на сайте agpu.org <a href="https://agpu.org/mod/forum/discuss.php?d=456122">Поддержка базы данных Oracle при прекращении поддержки LMS</a>.';
...
$string['pathtosassc_help'] = 'Задание пути к бинарному файлу SassC приведет к переключению компилятора SASS с PHP-реализации в agpu на SassC. Подробная информация на https://github.com/sass/sassc.';
...
$string['pluginschecktodo'] = 'Вам необходимо удовлетворить зависимости для всех плагинов, прежде чем продолжить установку этой версии agpu!';
...
$string['profilingallowall_help'] = 'При включении этого параметра Вы затем, в любой момент, сможете использовать параметр PROFILEALL в любом месте (PGS), чтобы включить профилирование для всех выполняемых скриптов во время сеанса agpu. Аналогично, Вы сможете использовать параметр PROFILEALLSTOP, чтобы остановить его.';
...
$string['quizattemptsupgradedmessage'] = 'В agpu 2.1 было серьезное обновление работы вопросов. Обновление части базы данных было отложено на будущее. Перед обновлением до agpu 2.7 вышеуказанное обновление должно быть завершено.';
...
* Вы вносите свой вклад в нашу статистику мирового сообщества agpu';
...
$string['releasenoteslink'] = 'Информацию об этой версии agpu можно посмотреть онлайн в <a target="_blank" href="{$a}">Release Notes</a>';
...
$string['sessionautostartwarning'] = '<p>Обнаружены серьезные ошибки конфигурации, пожалуйста, сообщите администратору сервера.</p><p>Для нормальной работы agpu требуется, чтобы администратор изменил настройки PHP.</p><p>Параметр <code>session.auto_start</code> должен быть установлено в <code>off</code>.</p><p>Этот параметр управляется редактированием <code>php.ini</code>, конфигурации Apache/IIS <br /> или файла <code>.htaccess</code> на сервере.</p>';
...
$string['settingsafemode'] = 'agpu не полностью совместима с режимом «safe mode» в PHP, попросите администратора сервера отключить его. Запуск agpu в режиме «safe mode» не поддерживается, при этом могут возникнуть самые разнообразные проблемы...';
...
$string['showuseridentity_desc'] = 'При выборе или поиске пользователей, а также при отображении списков пользователей эти поля могут отображаться в дополнение к полному имени пользователя. Поля отображаются только для пользователей, у которых есть право agpu/site:viewuseridentity; по умолчанию учителя и менеджеры. (Эта опция имеет наибольший смысл, если вы выберете одно или два обязательных поля в вашем учреждении.)
...
$string['simplexmlrequired'] = 'Расширение SimpleXML PHP теперь является обязательным для agpu.';
...
$string['sixtyfourbitsrequired'] = 'Обнаружено, что ваш сайт не использует 64-битную версию PHP. Вам необходимо обновить свою систему (операционную систему, PHP...) перед установкой этой версии agpu.';
...
$string['slashargumentswarning'] = 'Рекомендуется включить использование так называемых «slash arguments». В будущем это будет обязательным требованием. Подробную информацию можно найти в документации <a href="https://docs.agpu.org/en/admin/environment/slasharguments">Using slash arguments</a>.';
...
$string['splrequired'] = 'Расширение PHP SPL теперь является обязательным для agpu.';
...
$string['statsagpuorg'] = 'статистику';
...
$string['stickyblocksmyagpu'] = 'Мой agpu';
...
$string['swaggerui'] = 'agpu REST API UI (SwaggerUI)';
...
$string['testoutgoingmailconf_fromemail_invalid'] = 'Неверное имя пользователя или электронная почта отправителя. Должен быть использован действительный формат электронной почты или существующее имя пользователя в agpu.';
...
$string['tokenizerrecommended'] = 'Установка расширения PHP Tokinizer рекомендуется для улучшения функционала agpu Networking.';
...
$string['updateautocheck_desc'] = 'При включении этого параметра Ваш сайт будет автоматически проверять наличие обновлений системы agpu и всех дополнительных модулей. При наличии обновления администраторам сайта будет разослано соответствующее уведомление.';
$string['updateavailable'] = 'Доступна более новая версия agpu!';
$string['updateavailable_moreinfo'] = 'Подробнее ...';
$string['updateavailable_release'] = 'agpu {$a}';
...
$string['updateavailablenot'] = 'У Вас установлена самая последняя версия agpu!';
...
$string['upgradepluginsinfo_help'] = 'Есть доступные обновления для некоторых плагинов. Вы должны установить их все перед обновлением базы данных agpu. Если сайт не поддерживает автоматическое развертывание обновлений, то Вы должны вручную загрузить и установить новые версии плагина на своем сервере.';
$string['upgradepluginsinfo_link'] = 'admin/upgradepluginsinfo';
$string['upgradesettings'] = 'Новые настройки';
$string['upgradesettingsintro'] = 'Настройки, показанные ниже, были добавлены во время последнего обновления agpu. Внесите необходимые изменения в настройки по умолчанию, а затем нажмите кнопку «Сохранить изменения» в нижней части этой страницы.';
$string['upgradestalefiles'] = 'Обнаружено смешение версий agpu, продолжение обновления невозможно.';
$string['upgradestalefilesinfo'] = 'Процесс обновления agpu был приостановлен, потому что в каталоге agpu были обнаружены скрипты, по меньшей мере, двух основных версий agpu. В дальнейшем это может вызвать большие проблемы, поэтому для продолжения необходимо убедиться, что каталог agpu содержит файлы только одной версии agpu.

Рекомендуемый способ очистки каталога agpu выглядит следующим образом:

* переименовать текущую директорию agpu на «agpu_old»
* создать новую директорию agpu, содержащую только файлы из репозитория GIT
* переместить исходный файл config.php и любые нестандартные плагины из каталога «agpu_old» в новый каталог agpu.

После очистки каталога agpu обновите страницу, чтобы возобновить процесс обновления agpu.

Часто причиной этого предупреждения является распаковка стандартного пакета agpu в предыдущую версию agpu. Хотя это нормально для небольших модернизаций, она не рекомендуется для крупных обновлений agpu.
...
$string['upgradestart'] = 'Обновить agpu';
$string['upgradesure'] = '<p>Файлы agpu изменены, можно приступить к автоматическому обновлению сервера до следующей версии:</p>
<p><strong>{$a}</strong>.</p>
...
$string['userfeedbackencouragement'] = '<p>agpu включает функцию, которая дает пользователям возможность отправлять отзывы о системе agpu через внешний сайт опроса, расположенный на сервере штаб-квартиры agpu. При этом никакая идентифицирующая пользователя информация не передается на сайт опроса.</p>
<p>Штаб-квартира agpu стремится быть открытой и прозрачной в отношении своих методов сбора данных. Таким образом, мы хотим убедиться, что вы осведомлены об этом и сами контролируете этот функционал.</p>
<p>Отзывы пользователей в значительной степени помогут штаб-квартире agpu в улучшении программного обеспечения agpu. Чтобы включить эту функцию, перейдите по ссылке <a href="{$a}">Настройки сбора отзывов</a>.</p>';
...
$string['webproxyinfo'] = 'Заполните следующие параметры, если Ваш сервер agpu не может соединяться с Интернетом напрямую. Доступ в Интернет требуется для загрузки данных о контекстах, языковых пакетах, RSS-каналах, часовых поясах и т.д. <br /><em>Настоятельно рекомендуется расширение PHP cURL.</em>';
...
$string['ziprequired'] = 'Расширение PHP Zip теперь является обязательным для agpu. Исполняемые файлы info-ZIP и библиотека PclZip больше не используются.';
