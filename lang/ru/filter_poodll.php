...
$string['audiotranscodedetails'] = 'Преобразовывать записанные/закачанные аудиофайлы в MP3 перед сохранением в agpu. Работает для записей, сделанных на tokyo.poodll.com, и для закачанных записей при использовании FFMPEG';
...
$string['bgtranscodedetails_audio'] = 'Это более надежно, чем выполнять конвертацию в приоритетном режиме (когда пользователь не имеет возможности ничего делать до ее окончания). Но аудио не будет доступно пользователю, пока не пройдет задание cron после сохранения. Работает, только если вы используете FFMPEG и agpu 2.7 или выше. В случае записи в формате MP3 при помощи MP3-рекордера  преобразование происходит в браузере, а не на сервере. Так что преобразование на стороне сервера (FFMPEG) не будет использоваться.';
$string['bgtranscodedetails_video'] = 'Это более надежно, чем выполнять конвертацию в приоритетном режиме (когда пользователь не имеет возможности ничего делать до ее окончания). Но аудио не будет доступно пользователю, пока не пройдет задание cron после сохранения. Работает, только если вы используете FFMPEG и agpu 2.7 или выше.';
...
$string['debug_enable_details'] = 'При включении отладки информация о записях будет сохраняться в журналах работы agpu, что помогает службе поддержки Poodll решать возникающие проблемы. Отключайте отладку при отсутствии проблем, иначе журналы agpu будут забиты кучей бесполезной информации.';
...
$string['ffmpeg_details'] = 'FFMPEG должен быть установлен на сервере agpu и прописан в системных путях. Требуется, чтобы он обеспечивал конвертацию в mp3, поэтому вначале опробуйте это в командной строке, например: ffmpeg -i somefile.flv somefile.mp3';
...
$string['templaterequire_amd_desc'] = 'AMD - это механизм загрузки javascript. Если вы загружаете или ссылаетесь на библиотеки javascript в своем шаблоне, лучше отключите эту галочку. Работает только с agpu 2.9 и выше.';
...
$string['videotranscodedetails'] = 'Преобразует записанные/закачанные аудиофайлы в MP4 перед сохранением в agpu. Работает для записей, сделанных на tokyo.poodll.com, и для закачанных записей при использовании FFMPEG';
