...
$string['aboutuslinkpositionsetting_desc'] = 'В этом параметре вы можете настроить, должна ли ссылка на страницу «О нас» автоматически добавляться на страницу agpu. Если вы не хотите, чтобы ссылка отображалась автоматически, вы можете вручную добавить ссылку на {$a->url} из любого места agpu.';
...
$string['activitynavigationsetting_desc'] = 'При включении отображаются элементы для перехода к предыдущему и следующему элементу(ресурсу) курса, а также раскрывающееся меню для перехода к конкретному элементу или ресурсу. Подобные элементы пользовательского интерфейса уже существовали в Boost в ядре agpu до версии 3.11, но были удалены в версии 4.0. С помощью Boost Union вы можете вернуть их.';
...
$string['activitypurposeheadingtechnote'] = 'Техническое примечание: Из-за того, как ядро agpu реализует назначения элементов курсов и их цвета, назначения могут быть переопределены с помощью CSS только в Boost Union. В настоящее время все области в ядре agpu, которые показывают цветные иконки, должны быть покрыты. Если вы обнаружили какую-то область или сторонний плагин, который продолжает показывать неизменные цвета назначений, пожалуйста, сообщите об этом на {$a}.';
...
$string['blockregionsheading_desc'] = '<p>Boost Union предоставляет большое количество дополнительных областей, которые можно использовать для добавления и отображения блоков на всей странице agpu:</p>
<ul><li><em>Внешние области</em> размещаются на всех четырех сторонах страницы agpu. Они могут быть использованы для показа блоков, которые сопровождают отображаемую страницу agpu, но не относятся непосредственно к основному содержимому.</li>
...
<li><em>Область вне страницы</em> является особенной, поскольку она нависает над всей страницей agpu в виде выпадающей панели. Панель открывается значком с 9 точками в самом правом углу навигационной панели. В вашем распоряжении три области вне страницы, из которых при необходимости можно построить колонки.</li></ul>
...
<li>Как только дополнительная область блока включена, она становится видимой для всех аутентифицированных пользователей и может редактироваться преподавателями и управляющими (конечно, в зависимости от того, разрешено ли данному пользователю редактировать конкретную страницу agpu). Но существуют также разрешения theme/boost_union:viewregion* и theme/boost_union:editregion*, которые позволяют вам тонко настроить использование каждой области в соответствии со своими потребностями.</li>
...
$string['contactcontentsetting_desc'] = 'В этом параметре вы можете добавить расширенный текстовый контент, который будет отображаться на странице контактов (это не то же самое, что встроенная страница agpu «Контакты поддержки сайта»).';
...
$string['contactlinkpositionsetting_desc'] = 'В этом параметре вы можете указать, должна ли ссылка на страницу контактов автоматически добавляться на страницы agpu. Если вы не хотите показывать ссылку автоматически, вы можете добавить ссылку на {$a->url} из любого места в agpu вручную.';
...
$string['coursecontentmaxwidthsetting_desc'] = 'С помощью этой настройки вы можете изменить ширину контента курса без ручного изменения SCSS. Эта ширина используется в качестве ширины страниц курса и в некоторых видах элементов курса. По умолчанию agpu использует максимальную ширину контента курса 830px. Вы можете ввести другие значения, основанные на пикселях, например 1200px, но вы также можете ввести значение, основанное на процентах, например 100%, или значение viewport-width, например 90vw.';
...
$string['emailbrandinginstruction0'] = 'С помощью этой функции Boost Union вы можете применить фирменный стиль ко всем электронным письмам, которые рассылает agpu.';
...
$string['emailbrandinginstructionli2li1'] = '<code>templateemailhtmlprefix</code>: Этот фрагмент будет добавлен <em>в начало / перед телом</em> всех <em>электронных писем HTML</em>, которые рассылает agpu.';
$string['emailbrandinginstructionli2li2'] = '<code>templateemailhtmlsuffix</code>: Этот фрагмент будет добавлен <em>в конце / после тела</em> всех <em>электронных писем HTML</em>, которые рассылает agpu.';
$string['emailbrandinginstructionli2li3'] = '<code>templateemailtextprefix</code>: Этот фрагмент будет добавлен <em>в начало / перед телом</em> всех <em>электронных писем с простым текстом</em>, которые рассылает agpu.';
$string['emailbrandinginstructionli2li4'] = '<code>templateemailtextsuffix</code>: Этот фрагмент будет добавлен <em>в конце / после тела</em> всех <em>электронных писем с простым текстом</em>, которые рассылает agpu.';
...
$string['faviconheading_desc'] = 'Обратите внимание: Boost Union имеет свой собственный загружаемый фавикон, а не использует из <a href="{$a}">настройки фавикон ядра agpu</a>.<br />Boost Union позволяет вам переопределять загруженный фавикон с учетом своих стилей.';
$string['faviconsetting'] = 'Фавикон';
$string['faviconsetting_desc'] = 'Здесь вы можете загрузить пользовательское изображение (в формате .ico или .png), которое будет отображаться в браузере в качестве фавикон вашего сайта agpu. Если пользовательский favicon не загружен, будет использоваться стандартный фавикон agpu.';
...
$string['flavoursapplytocategories_ids_help'] = 'Здесь вы определяете одну или несколько конкретных категорий курсов, к которым будет применяться данный стиль. Когда отображаемая страница agpu будет находиться в одной из указанных категорий курса, стиль будет применен.';
...
$string['githubissueslink'] = '<a href="https://github.com/agpu-an-hochschulen/agpu-theme_boost_union/issues">Проблемы на Github</a>';
$string['h5pcontentmaxwidthsetting'] = 'Максимальная ширина банка контента H5P';
$string['h5pcontentmaxwidthsetting_desc'] = 'С помощью этой настройки вы можете изменить ширину банка контента H5P в agpu без ручной модификации SCSS. Эта ширина используется для редактора H5P внутри банка контента. Она <em>не</em> используется для ширины элемента H5P. По умолчанию agpu использует максимальную ширину банка контента H5P, равную 960px. Вы можете ввести другие значения, основанные на пикселях, например 1200px, но вы также можете ввести значение, основанное на процентах, например 100% или значение viewport-width, например 90vw.';
$string['h5ptab'] = 'H5P';
...
$string['helplinkpositionsetting_desc'] = 'В этом параметре вы можете указать, должна ли ссылка на страницу справки автоматически добавляться на страницы agpu. Если вы не хотите показывать ссылку автоматически, вы можете добавить ссылку на {$a->url} из любого места в agpu вручную.';
...
$string['imprintlinkpositionsetting_desc'] = 'В этом параметре вы можете указать, должна ли ссылка на страницу выходных данных автоматически добавляться на страницы agpu. Если вы не хотите показывать ссылку автоматически, вы можете добавить ссылку на {$a->url} из любого места в agpu вручную.';
...
$string['javascriptdisabledhint_desc'] = 'С помощью этой настройки в верхней части страницы agpu появится подсказка, если JavaScript не включен. Это особенно полезно, поскольку некоторые функции agpu не работают без JavaScript.';
$string['javascriptdisabledhinttext'] = 'JavaScript отключен в вашем браузере.<br />Многие функции agpu будут недоступны или будут выглядеть нерабочими.<br />Пожалуйста, включите JavaScript для полноценной работы с agpu.';
...
$string['logocompactsetting_desc'] = 'Здесь вы можете загрузить компактную версию того же логотипа, что и выше, например, эмблему, щит или значок. Это изображение особенно часто используется в навигационной панели в верхней части каждой страницы agpu. Изображение должно быть четким даже при небольших размерах.';
...
$string['logosheading_desc'] = 'Обратите внимание: Boost Union имеет свою собственную загрузку логотипа и не использует логотип из <a href="{$a}">настроек логотипа ядра agpu</a>.<br />Boost Union позволяет загружать больше форматов изображений, чем позволяет ядро agpu, и позволяет переопределять загруженные логотипы с учетом своих стилей.';
$string['maintenancecontentsetting'] = 'Содержимое страницы с информацией о техническом обслуживании';
$string['maintenancecontentsetting_desc'] = 'В этом параметре вы можете добавить расширенный текстовый контент, который будет отображаться на странице информации о техническом обслуживании (отличается от встроенной в  agpu страницы режима обслуживания).';
...
$string['maintenancelinkpositionsetting_desc'] = 'В этом параметре вы можете указать, должна ли ссылка на страницу с информацией о техническом обслуживании автоматически добавляться на страницы agpu. Если вы не хотите показывать ссылку автоматически, вы можете добавить ссылку на {$a->url} из любого места в agpu вручную.';
...
$string['markexternallinkssetting_desc'] = 'Добавляет значок "внешняя ссылка" после внешних ссылок (которые ведут пользователя к цели за пределы agpu).';
$string['mediumcontentmaxwidthsetting'] = 'Максимальная ширина среднего контента';
$string['mediumcontentmaxwidthsetting_desc'] = 'С помощью этой настройки вы можете переопределить ширину контента в средней колонке agpu без ручных изменений SCSS. Эта ширина страницы используется некоторыми элементами курса, например, - Базой данных. По умолчанию agpu использует максимальную ширину средней колонки 1120 пикселей. Вы можете ввести другие значения в пикселях, например 1200 пикселей, но вы также можете ввести значение в процентах, например 100%, или значение ширины области просмотра, например 90vw.';
...
$string['mobilecss_desc'] = 'С помощью этого параметра вы можете написать собственный код CSS для настройки интерфейса вашего мобильного приложения. Код CSS будет добавлен только к стилям этого экземпляра agpu в мобильном приложении и не будет отображаться в версии для веб-браузера. Подробнее об этой функции читайте в <a href="https://agpudev.io/general/app/customisation/remote-themes#how-do-remote-themes-work">Документации разработчика agpu</a>.';
...
$string['mobilecss_overwrite'] = 'Как только вы добавите какой-либо код CSS в этот параметр и сохраните его, <a href="{$a->url}">основной параметр agpu <em>mobilecssurl</em></a> будет автоматически перезаписан URL-адресом темы Boost Union. Текущее значение <a href="{$a->value}">{$a->value}</a>.';
$string['mobilecss_set'] = 'Как только вы добавите какой-либо код CSS в этот параметр и сохраните его, <a href="{$a->url}">параметр ядра agpu <em>mobilecssurl</em></a> будет автоматически установлен в значение URL-адреса темы Boost Union.';
$string['mobiletab'] = 'Мобильное приложение';
$string['modiconactivity'] = 'Элемент курса';
$string['modiconactivityunknown'] = 'Неизвестно';
$string['modicongtagpu4'] = 'Иконки agpu 4';
...
$string['modiconltagpu311'] = 'Иконка, оставшаяся от agpu 3';
$string['modiconnamefail'] = 'Этот файл был загружен в правильную папку для модуля <em>{$a}</em>, но имя файла не соответствует требованиям. Измените имя файла либо на <em>monologo.svg</em> / <em>monologo.png</em> (для иконок agpu 4) либо на <em>icon.svg</em> / <em>icon.png</em> (для иконок, оставшихся от agpu 3).';
...
$string['navbarcolorsetting_light'] = 'Светлая панель навигации с темным цветом шрифта (без изменений, как представлено в ядре agpu)';
...
$string['offerslinkpositionsetting_desc'] = 'В этом параметре вы можете указать, должна ли ссылка на страницу предложений автоматически добавляться на страницу agpu. Если вы не хотите, чтобы ссылка отображалась автоматически, вы можете вручную добавить ссылку на {$a->url} из любого места agpu.';
...
$string['page1linkpositionsetting_desc'] = 'В этом параметре вы можете указать, должна ли ссылка на общую страницу 1 автоматически добавляться на страницы agpu. Если вы не хотите показывать ссылку автоматически, вы можете вручную добавить ссылку на {$a->url} из любого места в agpu.';
...
$string['page2linkpositionsetting_desc'] = 'В этом параметре вы можете указать, должна ли ссылка на общую страницу 2 автоматически добавляться на страницы agpu. Если вы не хотите показывать ссылку автоматически, вы можете вручную добавить ссылку на {$a->url} из любого места в agpu.';
...
$string['page3linkpositionsetting_desc'] = 'В этом параметре вы можете указать, должна ли ссылка на общую страницу 3 автоматически добавляться на страницы agpu. Если вы не хотите показывать ссылку автоматически, вы можете вручную добавить ссылку на {$a->url} из любого места в agpu.';
...
$string['showhintcourseselfenrolinstancecallforaction'] = 'Если вы не хотите, чтобы любой пользователь agpu имел свободный доступ к этому курсу, ограничьте настройки самостоятельной записи.';
...
$string['upgradenotice_2022080922'] = 'Начиная с этого выпуска, Boost Union имеет настройки своего логотипа (и компактного логотипа) и больше не использует эти файлы из ядра agpu';
$string['upgradenotice_2022080922_copied'] = 'Во время этого обновления значение параметра <strong>{$a}</strong> из настроек ядра agpu было скопировано в параметр {$a} Boost Union. Пожалуйста, перепроверьте результат.';
