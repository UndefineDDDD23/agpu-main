...
$string['createnewcategories'] = 'Создавать новые (скрытые) категории курсов, если они не найдены в agpu';
...
$string['createnewcourses'] = 'Создавать новые (скрытые) курсы, если они не найдены в agpu';
$string['createnewcourses_desc'] = 'При включенном параметре плагин IMS  может создать новые курсы для любого из найденных в данных IMS, но не в базе данных в agpu. Любой вновь созданный курс изначально скрыт.';
$string['createnewusers'] = 'Создать учетные записи для пользователей, еще не зарегистрированных в agpu';
$string['createnewusers_desc'] = 'IMS Enterprise - типичные данные для регистрации, характеризующие группу пользователей. При включенном параметре учетная запись может быть создана для любого пользователя, отсутствующего в базе данных agpu. Пользователей сначала ищут по их «ID-номеру», потом - по их логину в agpu. Пароли не импортируются с помощью плагина IMS Enterprise.
...
$string['deleteusers_desc'] = 'При включенном параметре в данных IMS можно указать удаление учетных записей пользователей (если в параметре «recstatus» выбран п.3, который означает удаление аккаунта). Как это принято в agpu, запись пользователя не удаляется из базы данных agpu, но устанавливается флаг, который помечает аккаунт как удаленный.';
...
$string['imsrolesdescription'] = 'Спецификация IMS Enterprise включает в себя 8 различных типов ролей. Выберите, как Вы хотите  назначить их в agpu, при этом любая из них может быть проигнорирована.';
...
Обычно Вам не нужно беспокоиться об этом. Оставьте поле пустым и agpu всегда будет обрабатывать данные файла, независимо от того, задана ли цель или нет. В противном случае введите в поле точное название, которое будет присутствовать внутри тега <target>.';
$string['roles'] = 'Роли';
...
Некоторые системы не могут выводить информацию из поля <userid>. В этом случае Вы должны включить этот параметр - разрешить использовать поле <sourcedid> в качестве ID-номера пользователя agpu. В противном случае оставьте этот параметр отключенным.';
