...
$string['aim'] = 'Это инструмент управления помогает разработчикам и тестировщикам создавать файлы .feature, описывающие функционал agpu, и автоматически их запускать. Шаги, определяющие доступность для использования в файлах .feature, приведены ниже.';
...
$string['fieldvalueargument_help'] = 'Этот аргумент должен соответствовать значению поля. Есть много типов полей - простые, например: флажки или текстовая область или сложные, например: выбор даты. Подробную информацию об ожидаемых значениях полей см. в документации для разработчиков <a href="https://agpudev.io/general/development/tools/behat" target="_blank">Acceptance_testing</a>';
...
$string['wrongbehatsetup'] = 'Что-то не так с установкой behat и шаги определений не могут быть перечислены: <b>{$a->errormsg}.</b> <br/><br/> Пожалуйста, проверьте:  <ul><li> в config.php значения $CFG->behat_dataroot, $CFG->behat_prefix и $CFG->behat_wwwroot отличаются от значений, установленных в $CFG->dataroot, $CFG->prefix and $CFG->wwwroot. </li><li> Вы запускали "{$a->behatinit}" из корневого каталога agpu. </li><li> Зависимости установлены в vendor/ и файл {$a->behatcommand} имеет разрешения на выполнение. </li></ul>';
