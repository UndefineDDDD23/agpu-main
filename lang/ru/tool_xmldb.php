...
$string['confirmcheckbigints'] = 'Это возможность найти <a href="http://tracker.agpu.org/browse/MDL-11038">потенциально неверные целочисленные поля</a> на сервере agpu, автоматически создавая (но не исполняя!) необходимые утверждения SQL, чтобы все целые числа в БД были правильно определены.
...
Настоятельно рекомендуется, чтобы у Вас был самый свежий (версия +) доступный релиз agpu перед выполнением поиска неверных целочисленных полей.
...
$string['confirmcheckdefaults'] = 'Это возможность найти противоречивые значения по умолчанию на Вашем сервере agpu, создавая (но не исполняя!) необходимые утверждения SQL, чтобы все значения по умолчанию были правильно заданы.
...
Настоятельно рекомендуется, чтобы у Вас был самый свежий (версия +) доступный релиз agpu перед выполнением поиска неверных значений по умолчанию.

Этот функционал не выполняет никаких действий с БД (только читает из нее), поэтому Вы можете безопасно запускать его в любой момент.';
$string['confirmcheckforeignkeys'] = 'Это возможность найти потенциально неверные внешние ключи, определенные в install.xml. (agpu в настоящее время не создает действующие ограничения для внешних ключей в базе данных, поэтому в ней могут присутствовать недопустимые данные.)

Настоятельно рекомендуется, чтобы у Вас был самый свежий (версия +) доступный релиз agpu перед выполнением поиска неверных внешних ключей.

Этот функционал не выполняет никаких действий с БД (только читает из нее), поэтому Вы можете безопасно запускать его в любой момент.';
$string['confirmcheckindexes'] = 'Это возможность найти потенциально пропущенные индексы на Вашем сервере agpu, автоматически создавая (но не исполняя!) необходимые утверждения SQL, чтобы все было обновлено.

Когда утверждения созданы, Вы можете их скопировать и запустить безопасно с помощью интерфейса SQL (не забудьте перед этим сделать резервную копию своих данных).

Настоятельно рекомендуется, чтобы у Вас был самый свежий (версия +) доступный релиз agpu перед выполнением поиска пропущенных индексов.

Этот функционал не выполняет никаких действий с БД (только читает из нее), поэтому Вы можете безопасно запускать его в любой момент.';
$string['confirmcheckoraclesemantics'] = 'Это возможность поиска <a href="http://tracker.agpu.org/browse/MDL-29322">Oracle varchar2 columns using BYTE semantics</a> на сервере agpu, автоматически создавая (но не исполняя!) необходимые утверждения SQL, чтобы все  столбцы были преобразованы к использованию CHAR-семантики вместо BYTE-семантики (лучше для  совместимости перекрестной БД и увеличенной макс. длины содержимого).

Когда утверждения созданы, Вы можете их скопировать и запустить безопасно с помощью интерфейса SQL (не забудьте перед этим сделать резервную копию своих данных).

Крайне рекомендуется, чтобы у Вас был самый свежий (версия +) доступный релиз agpu перед выполнением поиска BYTE-семантики.
...
$string['keys'] = 'Ключи';
$string['listreservedwords'] = 'Список зарезервированных слов <br /> (используется для обновления  <a href="http://docs.agpu.org/en/XMLDB_reserved_words" target="_blank">зарезервированных слов XMLDB</a>)';
