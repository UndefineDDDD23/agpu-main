...
Некоторые провайдеры инструментов поддерживают передачу оценок обратно в agpu, опираясь на действия, которые выполняются в инструменте, и создавая более тесную интеграцию.';
...
$string['allowsetting'] = 'Разрешить инструменту хранить в agpu настройки - 8К';
...
$string['default_launch_container_help'] = 'Запуск инструмента влияет на его отображение при запуске из курса. Некоторые запуски обеспечивают более реальное отображение состояния инструмента, а другие обеспечивают большее объединение с интерфейсом agpu.

* **По умолчанию** - используется вид запуска, указанный в настройках инструмента.

* **Встроить** - инструмент отображается в существующем окне agpu, подобно большинству других элементов курса.

* **Встроить (без блоков)** - инструмент отображается в существующем окне agpu с управлением навигацией в верхней части страницы.
...
$string['errormisconfig'] = 'Инструмент не настроен. Обратитесь к администратору agpu, чтобы исправить настройки инструмента.';
...
$string['external_tool_type_help'] = '* **Автоматически, на основе URL-адреса запуска** — этот параметр следует использовать почти во всех случаях. agpu будет выбирать наиболее подходящие настройки инструмента на основе URL-адреса запуска. Будут использоваться инструменты, которые были настроены администратором или в этом курсе. Когда указан URL-адрес запуска, agpu будет обеспечивать обратную связь независимо от того, распознает он этот адрес или нет. Если agpu не распознает URL-адрес запуска, вам нужно вручную ввести настройки инструмента.

* **Конкретный предварительно настроенный инструмент** — Выбирая предварительно настроенный инструмент, вы можете заставить agpu использовать эту настройку инструмента при связи с провайдером внешнего инструмента. Если URL-адрес запуска выглядит не принадлежащим провайдеру инструмента, то будет показано предупреждение. В некоторых случаях не обязательно вводить URL-адрес запуска при предоставлении конкретного предварительно настроенного инструмента (если не запускается конкретный ресурс у провайдера инструмента).
...
$string['failedtoconnect'] = 'agpu не может общаться с системой «{$a}»';
...
$string['force_ssl_help'] = 'Выбор этого параметра принуждает использовать SSL при всех запусках поставляемого инструмента. Кроме того, все запросы веб-служб от поставщика инструмента будут использовать SSL. При использовании этого параметра убедитесь, что этот сайт agpu и поставщик инструмента поддерживают SSL.';
...
$string['launch_in_agpu'] = 'Запускать инструмент в agpu';
...
$string['launchinpopup_help'] = 'Вид запуска влияет на отображение инструмента при запуске из курса. Некоторые виды запуска обеспечивают на экране более реальное состояние для инструмента, а другие обеспечивают более комплексную интеграцию с окружением agpu.

* **По умолчанию** - используется вид запуска, указанный в настройках инструмента.

* **Встроить** - инструмент отображается в существующем окне agpu, подобно большинству других типов элементов курса.
.
* **Встроить (без блоков)** - инструмент отображается в существующем окне agpu с управлением навигацией в верхней части страницы.
...
* **Отклоненные** — так помечаются те поставщики инструментов, которые администратор не намерен делать доступными для всего сайта agpu. Учителя все еще могут использовать инструменты от этих поставщиков, если у них есть ключ клиента и общий секретный ключ, или если эти ключи не требуются.';
...
$string['module_class_type'] = 'Тип модуля agpu';
...
$string['notypes'] = 'В настоящее время в agpu не установлены инструменты LTI. Чтобы добавить их, нажмите выше ссылку Установить.';
...
$string['organizationid_default_help'] = 'Значение по умолчанию для ID организации. ID сайта идентифицирует эту установку agpu.';
$string['organizationidguid'] = 'ID организации';
$string['organizationidguid_help'] = 'Уникальный идентификатор этого экземпляра agpu, переданный инструменту как GUID экземпляра платформы.

Если это поле оставить пустым, будет использоваться значение по умолчанию.';
$string['organizationurl'] = 'URL-адрес организации';
$string['organizationurl_help'] = 'Основной URL-адрес этого сайта agpu. Если оставить это поле пустым, то по умолчанию будет использоваться значение конфигурации сайта.';
...
$string['password_admin_help'] = 'Общий секретный ключ можно рассматривать как пароль для доступа к инструменту. Он должен быть предоставлен поставщиком инструмента вместе с абонентским ключом. Инструменты, которые не требуют безопасной связи с agpu и не предоставляют дополнительные услуги (такие, как отчеты об оценках) могут не требовать общий ключ.';
$string['password_help'] = 'Для предварительно настроенных инструментов не стоит вводить здесь общий секретный ключ. Он будет задан в процессе настройки. Это поле должно быть заполнено, если создается ссылка на поставляемый инструмент, который еще не настроен. Если поставляемый инструмент будет использоваться более одного раза в этом курсе, то имеет смысл добавить в курс конфигурацию этого инструмента. Общий ключ можно рассматривать как пароль для доступа к инструменту. Он должен быть предоставлен поставщиком инструмента вместе с абонентским ключом. Инструменты, которые не требуют безопасной связи с agpu и не предоставляют дополнительные услуги (такие, как отчеты об оценках) могут не требовать общий ключ.';
...
$string['register_warning'] = 'Для открытия страницы регистрации может понадобиться некоторое время. Если она не открывается, убедитесь, что в параметрах конфигурации введен правильный URL-адрес. Если agpu использует https, убедитесь, что настраиваемый вами инструмент поддерживает https и вы используете https в URL.';
...
Он может быть использован поставщиком инструмента для однозначной идентификации сайта agpu, с которого пользователь запускает инструмент.
...
Инструменты, которые не требуют безопасной связи с agpu и не предоставляют дополнительные услуги (такие, как отчеты об оценках) могут не требовать ключ ресурса.';
...
$string['secure_icon_url_help'] = 'Подобен URL-адресу значка, но используется в случае доступа пользователя к agpu через защищенное соединение SSL. Это предотвратит появление предупреждения браузера при запросе показа незащищенного изображения.';
$string['secure_launch_url'] = 'Безопасный URL-адрес инструмента';
$string['secure_launch_url_help'] = 'Подобен URL-адресу инструмента, но используется вместо него при необходимости высокого уровня безопасности. agpu будет использовать безопасный URL-адрес вместо URL-адреса, если сайт agpu доступен через SSL или если в конфигурации инструмента задано - всегда запускать через SSL.
...
$string['typename_help'] = 'Название инструмента используется для идентификации в agpu поставляемого инструмента. Введенное название будет видно преподавателям при добавлении внешних инструментов в курсы.';
$string['types'] = 'Типы';
