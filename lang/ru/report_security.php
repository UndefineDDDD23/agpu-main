...
$string['check_nodemodules_details'] = '<p>Каталог <em>{$a->path}</em> содержит модули Node.js и их зависимости, обычно устанавливаемые утилитой NPM. Эти модули могут потребоваться для локальной разработки agpu, например, для использования фреймворка grunt. Они не нужны для промышленной эксплуатации сайта agpu, и могут содержать потенциально опасный код, подвергающий Ваш сайт угрозе атак.</p>
...
$string['check_riskbackup_detailsok'] = 'Нет ролей с явным разрешением на резервное копирование пользовательских данных. Однако, отметьте, что администраторы с правом «agpu/site:doanything», вероятно, будут в состоянии сделать это.';
...
$string['check_vendordir_details'] = '<p>Каталог <em>{$a->path}</em> содержит различные сторонние библиотеки и их зависимости, обычно устанавливаемые с помощью Composer (менеджера пакетов для PHP). Эти библиотеки могут потребоваться для локальной разработки agpu, например, для установки фреймворка PHPUnit. Они не нужны для промышленной эксплуатации сайта agpu, и могут содержать потенциально опасный код, подвергающий Ваш сайт угрозе атак.</p>
<p>Настоятельно рекомендуется удалить этот каталог, если сайт доступен через интернет, или, по крайней мере, запретить доступ к этому каталогу в конфигурации вашего веб-сервера.</p>';
