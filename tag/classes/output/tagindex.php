...
class tagindex implements templatable {
    ...
    public function __construct($tag, $component, $itemtype, $content,
        ...
        if ($totalpages > $page + 1) {
            $this->record->nextpageurl = new agpu_url($url, $urlparams + array('page' => $page + 1));
        }
        if ($page > 0) {
            $this->record->prevpageurl = new agpu_url($url, $urlparams + array('page' => $page - 1));
        }
        if (!$exclusivemode && ($totalpages > 1 || $page)) {
            $this->record->exclusiveurl = new agpu_url($url, $urlparams + array('excl' => 1));
        }
    ...
    public function export_for_template(renderer_base $output) {
        if ($this->record->nextpageurl && $this->record->nextpageurl instanceof agpu_url) {
            $this->record->nextpageurl = $this->record->nextpageurl->out(false);
        }
        if ($this->record->prevpageurl && $this->record->prevpageurl instanceof agpu_url) {
            $this->record->prevpageurl = $this->record->prevpageurl->out(false);
        }
        if ($this->record->exclusiveurl && $this->record->exclusiveurl instanceof agpu_url) {
