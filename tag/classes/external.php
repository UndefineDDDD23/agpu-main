...
class core_tag_external extends external_api {
    ...
    public static function update_tags($tags) {
        ...
        $canmanage = has_capability('agpu/tag:manage', $systemcontext);
        $canedit = has_capability('agpu/tag:edit', $systemcontext);
        $warnings = array();
        ...
        if (!empty($params['tag'])) {
            if (empty($params['tc'])) {
                ...
                if (count($tags) > 1) {
                    ...
                    throw new agpu_exception('Tag is in more that one collection, please indicate one.');
                ...
                if (!$tag = core_tag_tag::get_by_name($params['tc'], $params['tag'], '*')) {
                    ...
                    throw new agpu_exception('notagsfound', 'tag');
                }
            }
