...
class external_test extends externallib_advanced_testcase {
    ...
    public function test_update_tags(): void {
        ...
        $roleid = $this->assignUserCapability('agpu/tag:edit', $context->id);
        $result = \core_tag_external::update_tags(array($updatetag));
        $result = external_api::clean_returnvalue(\core_tag_external::update_tags_returns(), $result);
        ...
        $this->assertEquals($originaltag['rawname'], $result['tags'][0]['rawname']);
        $this->assertArrayNotHasKey('flag', $result['tags'][0]); // 'Flag' is not available unless 'agpu/tag:manage' cap exists.
        ...
        assign_capability('agpu/tag:manage', CAP_ALLOW, $roleid, $context->id);
        $this->assertTrue(has_capability('agpu/tag:manage', $context));
        $result = \core_tag_external::update_tags(array($updatetag));
        $result = external_api::clean_returnvalue(\core_tag_external::update_tags_returns(), $result);
        $this->assertEmpty($result['warnings']);
