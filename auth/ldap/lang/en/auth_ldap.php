...
$string['auth_ldapextrafields'] = 'These fields are optional.  You can choose to pre-fill some agpu user fields with information from the <b>LDAP fields</b> that you specify here. <p>If you leave these fields blank, then nothing will be transferred from LDAP and agpu defaults will be used instead.</p><p>In either case, the user will be able to edit all of these fields after they log in.</p>';
...
$string['auth_ldap_preventpassindb'] = 'Select yes to prevent passwords from being stored in agpu\'s DB.';
...
$string['auth_ldap_update_userinfo'] = 'Update user information (firstname, lastname, address..) from LDAP to agpu.  Specify "Data mapping" settings as you need.';
...
$string['auth_ntlmsso_enabled'] = 'Set to yes to attempt Single Sign On with the NTLM domain. Note that this requires additional setup on the server to work. For further details, see the documentation <a href="https://docs.agpu.org/en/NTLM_authentication">NTLM authentication</a>.';
...
$string['pagedresultsnotsupp'] = 'LDAP paged results not supported (either your PHP version lacks support, you have configured agpu to use LDAP protocol version 2 or agpu cannot contact your LDAP server to see if paged support is available.)';
...
$string['updateremfail'] = 'Error updating LDAP record. Error code: {$a->errno}; Error string: {$a->errstring}<br/>Key ({$a->key}) - old agpu value: \'{$a->ouvalue}\' new value: \'{$a->nuvalue}\'';
$string['updateremfailamb'] = 'Failed to update LDAP with ambiguous field {$a->key}; old agpu value: \'{$a->ouvalue}\', new value: \'{$a->nuvalue}\'';
