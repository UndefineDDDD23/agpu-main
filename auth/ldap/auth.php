...
class auth_plugin_ldap extends auth_plugin_base {
    ...
    function user_login($username, $password) {
        if (! function_exists('ldap_bind')) {
            throw new \agpu_exception('auth_ldapnotinstalled', 'auth_ldap');
            return false;
        }
        ...
        // password from agpu (otherwise we'll get stuck later in the login process).
     ...
     * this information to agpu user-table you should honor syncronization flags
     *
    ...
    function get_userinfo($username) {
        ...
        $user_entry = ldap_get_entries_agpu($ldapconnection, $user_info_result);
        if (empty($user_entry)) {
            $this->ldap_close();
     ...
     *
     * @param mixed $userobject  agpu userobject
    ...
    function user_create($userobject, $plainpass) {
        ...
        switch ($this->config->user_type)  {
            ...
            case 'rfc2307bis':
                ...
                // agpu without colliding with existing numbers and without
                // race conditions. As this user is supposed to be only used
                // with agpu (otherwise the user would exist beforehand) and
                ...
                // system level (agpu ignores this).
                ...
                // agpu is available, and the user can see the kind of
            ...
            case 'ad':
                ...
                if (!function_exists('mb_convert_encoding')) {
                    throw new \agpu_exception('auth_ldap_no_mbstring', 'auth_ldap');
                }

                // Check for invalid sAMAccountName characters.
                if (preg_match('#[/\\[\]:;|=,+*?<>@"]#', $extusername)) {
                    throw new \agpu_exception ('auth_ldap_ad_invalidchars', 'auth_ldap');
                }
                ...
                if (!ldap_add($ldapconnection, $userdn, $newuser)) {
                    throw new \agpu_exception('auth_ldap_ad_create_req', 'auth_ldap');
                }
                ...
                break;
            default:
               throw new \agpu_exception('auth_ldap_unsupportedusertype', 'auth_ldap', '', $this->config->user_type_name);
        }
        $this->ldap_close();
        return $uadd;
    }
    ...
    /**
     * Returns true if plugin allows resetting of password from agpu.
     *
     * @return bool
     */
    ...
    function user_signup($user, $notify=true) {
        ...
        if ($this->user_exists($user->username)) {
            throw new \agpu_exception('auth_ldap_user_exists', 'auth_ldap');
        }
        ...
        if (! $this->user_create($user, $plainslashedpassword)) {
            throw new \agpu_exception('auth_ldap_create_error', 'auth_ldap');
        }
        ...
        if (! send_confirmation_email($user)) {
            throw new \agpu_exception('noemail', 'auth_ldap');
        }
    ...
    function password_expire($username) {
        ...
        if ($sr)  {
            $info = ldap_get_entries_agpu($ldapconnection, $sr);
            if (!empty ($info)) {
                $info = $info[0];
    ...
    /**
     * Synchronise users from the external LDAP server to agpu's user table.
     *
    ...
    /**
     * Synchronise users from the external LDAP server to agpu's user table (callback).
     *
...
/// User Additions
    ...
    /**
     * Update users from the external LDAP server into agpu's user table.
     *
     * Sync helper
     *
    ...
    function user_activate($username) {
        ...
        switch ($this->config->user_type)  {
            ...
            case 'ad':
                ...
                break;
            default:
                throw new \agpu_exception('user_activatenotsupportusertype', 'auth_ldap', '', $this->config->user_type_name);
        }
     ...
     *
     * @deprecated since agpu 3.4 MDL-30634 - please do not use this function any more.
     */
                ...
                if ($newvalue !== null and $newvalue !== $oldvalue and !empty($this->config->{'field_updateremote_' . $key})) {
                    ...
                    foreach ($ldapkeys as $ldapkey) {
                        ...
                        if (!$ambiguous) {
                            ...
                            // Ambiguous. Value empty before in agpu (and LDAP) - use
    ...
    function user_update_password($user, $newpassword) {
        ...
        switch ($this->config->user_type) {
            case 'edir':
                ...
                $sr = ldap_read($ldapconnection, $user_dn, '(objectClass=*)', $search_attribs);
                if ($sr) {
                    $entry = ldap_get_entries_agpu($ldapconnection, $sr);
                    $info = $entry[0];
                    $newattrs = array();
    ...
    function ldap_expirationtime2unix($time, $ldapconnection, $user_dn) {
        ...
        switch ($this->config->user_type) {
            ...
            case 'ad':
                ...
                break;
            default:
                throw new \agpu_exception('auth_ldap_usertypeundefined', 'auth_ldap');
        }
        return $result;
    }
    ...
    function ldap_unix2expirationtime($time) {
        ...
        switch ($this->config->user_type) {
            ...
            case 'rfc2307bis':
                ...
                break;
            default:
                throw new \agpu_exception('auth_ldap_usertypeundefined2', 'auth_ldap');
        }
        return $result;
    ...
    /**
     * Returns user attribute mappings between agpu and LDAP
     *
     * @return array
     */

    function ldap_attributes() {
        $agpuattributes = array();
        ...
        foreach ($userfields as $field) {
            if (!empty($this->config->{"field_map_$field"})) {
                $agpuattributes[$field] = core_text::strtolower(trim($this->config->{"field_map_$field"}));
                if (preg_match('/,/', $agpuattributes[$field])) {
                    $agpuattributes[$field] = explode(',', $agpuattributes[$field]); // split ?
                }
            }
        }
        $agpuattributes['username'] = core_text::strtolower(trim($this->config->user_attribute));
        $agpuattributes['suspended'] = core_text::strtolower(trim($this->config->suspended_attribute));
        return $agpuattributes;
    }
    ...
    function ldap_get_userlist($filter='*') {
        ...
        foreach ($contexts as $context) {
            ...
            do {
                ...
                $users = ldap_get_entries_agpu($ldapconnection, $ldap_result);
    ...
    /**
     * Indicates if password hashes should be stored in local agpu database.
     *
    ...
    function change_password_url() {
        if (empty($this->config->stdchangepassword)) {
            if (!empty($this->config->changepasswordurl)) {
                return new agpu_url($this->config->changepasswordurl);
            } else {
                return null;
            }
        } else {
     ...
     * (The caller should define('NO_agpu_COOKIES', true) before including config.php)
     *
     */
    ...
    function ldap_get_ad_pwdexpire($pwdlastset, $ldapconn, $user_dn){
        ...
        $entry = ldap_get_entries_agpu($ldapconn, $sr);
        $info = $entry[0];
        ...
        if ($sr = ldap_read($ldapconn, $user_dn, '(objectClass=*)', array('msDS-ResultantPSO'))) {
            if ($entry = ldap_get_entries_agpu($ldapconn, $sr)) {
                $info = $entry[0];
     ...
     * settings. It's actually a wrapper around ldap_connect_agpu()
     *
    ...
    function ldap_connect() {
        ...
        if($ldapconnection = ldap_connect_agpu($this->config->host_url, $this->config->ldap_version,
        ...
        throw new \agpu_exception('auth_ldap_noconnect_all', 'auth_ldap', '', $debuginfo);
    }
