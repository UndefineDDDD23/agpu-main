...
class auth_ldap_test extends \advanced_testcase {
    ...
    public function test_auth_ldap(int $pagesize, int $subcontext): void {
        ...
        $debuginfo = '';
        if (!$connection = ldap_connect_agpu(TEST_AUTH_LDAP_HOST_URL, 3, 'rfc2307', TEST_AUTH_LDAP_BIND_DN, TEST_AUTH_LDAP_BIND_PW, LDAP_DEREF_NEVER, $debuginfo, false)) {
        ...
        $topdn = 'dc=agputest,'.TEST_AUTH_LDAP_DOMAIN;

        $this->recursive_delete($connection, TEST_AUTH_LDAP_DOMAIN, 'dc=agputest');

        $o = array();
        $o['objectClass'] = array('dcObject', 'organizationalUnit');
        $o['dc']         = 'agputest';
        $o['ou']         = 'agpuTEST';
        if (!ldap_add($connection, 'dc=agputest,'.TEST_AUTH_LDAP_DOMAIN, $o)) {
