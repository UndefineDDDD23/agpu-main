 ...
 * that are matching our four supported agpu database drivers.
 *
 * @package    auth_db
 * @category   phpunit
...
class db_test extends \advanced_testcase {
    ...
    public function test_clean_data(): void {
        ...
        // Get the user on agpu user table.
    ...
    public function test_deleting_with_many_users(): void {
        ...
        // Set to delete from agpu when missing from DB.
        set_config('removeuser', AUTH_REMOVEUSER_FULLDELETE, 'auth_db');
        $auth->config->removeuser = AUTH_REMOVEUSER_FULLDELETE;
        ...
        $agpuuser = $DB->get_record('user', array('email' => $user->email, 'auth' => 'db'));
        $this->assertNotNull($agpuuser);
        $this->assertEquals($user->username, $agpuuser->username);
        ...
        $agpuuser = $DB->get_record('user', array('id' => $agpuuser->id));
        $this->assertFalse($auth->user_login($user->username, 'heslo'));
        $this->assertEquals(1, $agpuuser->deleted);
