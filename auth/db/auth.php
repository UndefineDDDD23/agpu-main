...
class auth_plugin_db extends auth_plugin_base {
    ...
    function db_init() {
        if ($this->is_configured() === false) {
            throw new agpu_exception('auth_dbcantconnect', 'auth_db');
        }
    ...
    /**
     * Returns user attribute mappings between agpu and the external database.
     *
     * @return array
     */
    function db_attributes() {
        ...
        foreach ($userfields as $field) {
            if (!empty($this->config->{"field_map_$field"})) {
                $agpuattributes[$field] = $this->config->{"field_map_$field"};
            }
        }
        $agpuattributes['username'] = $this->config->fielduser;
    ...
    /**
     * Synchronizes user from external db to agpu user table.
     *
     * Sync should be done by using idnumber attribute, not username.
     * You need to pass firstsync parameter to function to fill in
     * idnumbers if they don't exists in agpu user table.
     *
    ...
    function user_update($olduser, $newuser) {
        ...
        if (!empty($update)) {
            ...
            if (!$authdb->Execute($sql, array($this->ext_addslashes($extusername)))) {
                throw new \agpu_exception('auth_dbupdateerror', 'auth_db');
            }
        }
        $authdb->Close();
        return true;
