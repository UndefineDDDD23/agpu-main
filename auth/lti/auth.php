...
class auth_plugin_lti extends \auth_plugin_base {
     ...
     * On first launch, for a previously unbound user, this mode dictates that a new agpu account will be created automatically
     ...
     *
     * This method ensures a agpu user account has been found or is created, that the user is linked to the relevant
     ...
     * @param agpu_url $returnurl the local URL to return to if authentication workflows are required.
     ...
     */
    public function complete_login(array $launchdata, agpu_url $returnurl, int $provisioningmode,
        ...
        switch ($provisioningmode) {
            ...
            default:
                ...
                redirect(new agpu_url('/auth/lti/login.php', [
                    'sesskey' => sesskey(),
                ]));
                break;
        }
    ...
    /**
     * Get a agpu user account for the LTI user based on the user details returned by a NRPS 2 membership call.
     *
     ...
     * @return stdClass a agpu user record.
     */
    ...
    /**
     * Get a agpu user account for the LTI user corresponding to the user defined in a link launch.
     *
     ...
     * @return stdClass the agpu user who is mapped to the platform user identified in the JWT data.
     */
     ...
     * @param int $userid the id of the agpu user account to bind.
     */
    ...
    /**
     * Gets the id of the linked agpu user account for an LTI user, or null if not found.
     *
     ...
     * @return int|null the id of the corresponding agpu user record, or null if not found.
     */
     ...
     * @return stdClass a complete agpu user record.
     */
     ...
     *
     * @param stdClass $user the agpu user account to update.
     * @param array $userdata the user data coming from either a launch or membership service call.
     * @param string $iss the issuer to which the user belongs.
     */
     ...
     * @throws agpu_exception if there were any problems updating the picture.
     */
    protected function update_user_picture(int $userid, string $url): void {
        ...
        try {
            ...
            throw new agpu_exception(get_string($e->errorcode, $e->module, $e->a));
        }
        ...
        if (!$iconfile = $iconfile->copy_content_to_temp()) {
            $fs->delete_area_files($context->id, 'user', 'newicon');
            throw new agpu_exception('There was a problem copying the profile picture to temp.');
        }
