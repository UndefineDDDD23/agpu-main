...
class renderer extends \plugin_renderer_base {
    ...
    public function render_account_binding_options_page(int $provisioningmode): string {
        ...
        $formaction = new \agpu_url('/auth/lti/login.php');
        $notification = new notification(get_string('firstlaunchnotice', 'auth_lti'), \core\notification::INFO, false);
        $cancreateaccounts = !get_config('agpu', 'authpreventaccountcreation');
     ...
     * @param \agpu_url $returnurl the URL to return to when the user clicks continue on the rendered page.
     * @return string the rendered HTML
     */
    public function render_account_binding_complete(notification $notification, \agpu_url $returnurl): string {
        $context = (object) [
