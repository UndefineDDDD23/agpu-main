...
final class cookie_helper {
     ...
     * the session cookie ('agpuSession') using the respective method. This ensure the session cookie will continue to be sent.
     *
     ...
     * }
     *
     * @param \agpu_url $pageurl the URL of the page making the check, used to redirect back to after setting test cookies.
     * @return void
     */
    public static function do_cookie_check(\agpu_url $pageurl): void {
        ...
        if (empty($cookiecheck1)) {
            ...
            // agpuSession cookie. This is done via manipulation of the outgoing headers after the cookie check redirect. To
            // guarantee these outgoing Set-Cookie headers will be created after the redirect, expire the current cookie.
            core_cookie_helper::expire_agpusession();
    ...
    public static function setup_session_cookie(): void {
        ...
        if (is_agpu_cookie_secure()) {
            $atts = ['SameSite=None', 'Secure'];
            if (self::get_cookies_supported_method() == self::COOKIE_METHOD_EXPLICIT_PARTITIONING) {
                $atts[] = 'Partitioned';
            }
            core_cookie_helper::add_attributes_to_cookie_response_header('agpuSession' . $CFG->sessioncookie, $atts);
        }
    }
    ...
    private static function set_test_cookie(string $name, string $value, bool $partitioned = false): void {
        ...
        if (is_agpu_cookie_secure() && $partitioned) {
