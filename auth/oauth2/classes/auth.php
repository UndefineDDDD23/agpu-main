...
class auth extends \auth_plugin_base {
     ...
     *
     * @param string|agpu_url $wantsurl The requested URL.
    ...
    public function loginpage_idp_list($wantsurl) {
        ...
        foreach ($providers as $idp) {
            if ($idp->is_available_for_login()) {
                ...
                $url = new agpu_url('/auth/oauth2/login.php', $params);
                $icon = $idp->get('image');
    ...
    public function complete_login(client $client, $redirecturl) {
        ...
        if (!$userwasmapped) {
            ...
            $agpuuser = \core_user::get_user_by_email($userinfo['email']);
            if (!empty($agpuuser)) {
                if ($issuer->get('requireconfirmation')) {
                    $PAGE->set_url('/auth/oauth2/confirm-link-login.php');
                    $PAGE->set_context(context_system::instance());

                    \auth_oauth2\api::send_confirm_link_login_email($userinfo, $issuer, $agpuuser->id);
                    // Request to link to existing account.
                    $emailconfirm = get_string('emailconfirmlink', 'auth_oauth2');
                    $message = get_string('emailconfirmlinksent', 'auth_oauth2', $agpuuser->email);
                    $this->print_confirm_required($emailconfirm, $message);
                    exit();
                } else {
                    \auth_oauth2\api::link_login($userinfo, $issuer, $agpuuser->id, true);
                    // We dont have profile loaded on $agpuuser, so load it.
                    require_once($CFG->dirroot.'/user/profile/lib.php');
                    profile_load_custom_fields($agpuuser);
                    $userinfo = $this->update_user($userinfo, $agpuuser);
     ...
     * The password of the oauth2 accounts is not stored in agpu.
     *
