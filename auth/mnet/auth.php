...
/**
 * Authentication Plugin: agpu Network Authentication
 * Multiple host authentication support for agpu Network.
 *
 * @package auth_mnet
...
/**
 * agpu Network authentication plugin.
 */
class auth_plugin_mnet extends auth_plugin_base {
    ...
    function user_login($username, $password) {
        return false; // Throw agpu_exception("mnetlocal").
    }
    ...
    function start_jump_session($mnethostid, $wantsurl, $wantsurlbackhere=false) {
        ...
        if (\core\session\manager::is_loggedinas()) {
            throw new \agpu_exception('notpermittedtojumpas', 'mnet');
        }

        // check remote login permissions
        if (! has_capability('agpu/site:mnetlogintoremote', context_system::instance())
                or is_mnet_remote_user($USER)
                or isguestuser()
                or !isloggedin()) {
            throw new \agpu_exception('notpermittedtojump', 'mnet');
        }

        // check for SSO publish permission first
        if ($this->has_service($mnethostid, 'sso_sp') == false) {
            throw new \agpu_exception('hostnotconfiguredforsso', 'mnet');
        }
    ...
    function confirm_mnet_session($token, $remotepeer) {
        ...
        if (! $remotehost = $DB->get_record('mnet_host', array('wwwroot' => $remotepeer->wwwroot, 'deleted' => 0))) {
            throw new \agpu_exception('notpermittedtoland', 'mnet');
        }
        ...
        if ($mnetrequest->send($remotepeer) === true) {
            ...
            foreach ($mnetrequest->error as $errormessage) {
                ...
                if($code == 702) {
                    $site = get_site();
                    throw new \agpu_exception('mnet_session_prohibited', 'mnet', $remotepeer->wwwroot,
                        format_string($site->fullname));
                    exit;
                }
                $message .= "ERROR $code:<br/>$errormessage<br/>";
            }
            throw new \agpu_exception("rpcerror", '', '', $message);
        }
        unset($mnetrequest);

        if (empty($remoteuser) or empty($remoteuser->username)) {
            throw new \agpu_exception('unknownerror', 'mnet');
            exit;
        }

        if (user_not_fully_set_up($remoteuser, false)) {
            throw new \agpu_exception('notenoughidpinfo', 'mnet');
            exit;
        }
        ...
        // the user may roam from agpu 1.x where lang has _utf8 suffix
        ...
        if (empty($localuser) || ! $localuser->id) {
            ...
            if (empty($this->config->auto_add_remote_users)) {
                throw new \agpu_exception('nolocaluser', 'mnet');
        ...
        if (!$this->can_login_remotely($localuser->username, $remotehost->id)) {
            throw new \agpu_exception('sso_mnet_login_refused', 'mnet', '',
            ...
            if ($key == '_mnet_userpicture_timemodified' and empty($CFG->disableuserimages) and isset($remoteuser->picture)) {
                ...
                if (!empty($val) and $localtimemodified < $val) {
                    ...
                    if ($fetchrequest->send($remotepeer) === true) {
                        ...
                        // note that since agpu 2.0 we ignore $fetchrequest->response['f2']
        ...
        if (!$firsttime) {
            ...
            if (is_array($courses) && !empty($courses)) {
                ...
                if (!empty($studentroles)) {
                    ...
                    throw new agpu_exception('unknownrole', 'error', '', 'student');
                }
            } else {
     ...
     * table
     *
     * @deprecated since agpu 2.8 Please don't use this function for recording mnet logs.
     ...
     *
     * This is called by require_logout in agpulib
     *
     * @return   void
     */
     ...
     *
     * The mimetype information was added in agpu 2.0. In agpu 1.x, images are always jpegs.
     *
     ...
     * is permitted to login to this agpu.
     *
     * @param string $username   The username
    ...
    function loginpage_idp_list($wantsurl) {
        ...
        foreach ($hosts as $host) {
            $idps[] = array(
                'url'  => new agpu_url($host->wwwroot . $host->sso_jump_url, array('hostwwwroot' => $CFG->wwwroot, 'wantsurl' => $wantsurl, 'remoteurl' => 1)),
