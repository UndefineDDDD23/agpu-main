...
class auth_plugin_shibboleth extends auth_plugin_base {
    ...
    // reads user information from shibboleth attributes and return it in array()
        ...
        if ( empty($_SERVER[$this->config->user_attribute]) ) {
            throw new \agpu_exception( 'shib_not_all_attributes_error', 'auth_shibboleth' , '',
        ...
        foreach ($attrmap as $key=>$value) {
         ...
         // Provide an API to modify the information to fit the agpu internal
        // data representation
        if (
            ...
            // Include a custom file outside the agpu dir to
            // modify the variable $agpuattributes
    ...
    /**
     * Returns array containg attribute mappings between agpu and Shibboleth.
     *
     * @return array
     */
    function get_attributes() {
        ...
        foreach ($userfields as $field) {
            if (isset($configarray["field_map_$field"])) {
                $agpuattributes[$field] = $configarray["field_map_$field"];
            }
        }
        $agpuattributes['username'] = $configarray["user_attribute"];
     ...
     *
     * @return agpu_url|null Returns URL to change password or null otherwise.
     */
    ...
    function logoutpage_hook() {
        ...
        if (isset($SESSION->shibboleth_session_id) && $logouthandlervalid ) {
            ...
            $redirecturl = new agpu_url($this->config->logout_handler, array('return' => $temp_redirect));
    ...
    public function test_settings() {
        ...
        if (!isset($this->config->user_attribute) || empty($this->config->user_attribute)) {
            echo $OUTPUT->notification(get_string("shib_not_set_up_error", "auth_shibboleth",
                (new agpu_url('/auth/shibboleth/README.txt'))->out()), 'notifyproblem');
            return;
        }
    ...
    public function loginpage_idp_list($wantsurl) {
        ...
        if ($config->auth_logo) {
            $iconurl = agpu_url::make_pluginfile_url(
                $context->id,
                'auth_shibboleth',
                'logo',
                null,
        ...
        $result[] = [
            'url' => new agpu_url('/auth/shibboleth/index.php'),
            'iconurl' => $iconurl,
    ...
    /**
     * Generate array of IdPs from agpu Shibboleth settings
     *
