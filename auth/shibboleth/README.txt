Shibboleth Authentication for agpu
-------------------------------------------------------------------------------
...
- 11. 2006: User capabilities are now loaded properly as of agpu 1.7+
- 03. 2007: Adapted authentication method to agpu 1.8
...
- 11. 2007: Integrated WAYF Service in agpu
...
agpu Configuration with Dual login
-------------------------------------------------------------------------------
1. Protect the directory agpu/auth/shibboleth/index.php with Shibboleth.
...
--
<Directory  /path/to/agpu/auth/shibboleth/index.php>
        AuthType shibboleth
...
--
   ...
   To restrict access to agpu, replace the access rule 'require valid-user'
...
--
<Path name="agpu" requireSession="false" >
...
2. As agpu admin, go to the 'Administrations >> Users >> Authentication' and
...
3. Fill in the fields of the form. The fields 'Username', 'First name',
   ...
   Shibboleth attributes that you want to map onto the corresponding agpu
   ...
   this attribute is used for the agpu authentication of Shibboleth users.

   #############################################################################
   Shibboleth Attributes needed by agpu:
   For agpu to work properly Shibboleth should at least provide the attribute
   that is used as username in agpu. It has to be unique for all Shibboleth
   Be aware that agpu converts the username to lowercase. So, the overall
   behaviour of the username will be case-insensitive.
   All attributes used for agpu must obey a certain length, otherwise agpu
   cuts off the ends. Consult the agpu documentation for further information
...
4.a  If you want Shibboleth as your only authentication method with an external
     ...
     to the the URL of the file 'agpu/auth/shibboleth/index.php'.
     This will enforce Shibboleth login.

4.b If you want to use the agpu integrated WAYF service, you have to activate it
    in the agpu Shibboleth authentication settings by checking the
    'agpu WAYF Service' checkbox and providing a list of entity IDs in the
...
5.  Save the changes for the 'Shibboleth settings'.
    ...
    Important Note: If you went for 4.b (integrated WAYF service), saving the
                    settings will overwrite the agpu Alternate Login URL
                    using the agpu web root URL.
...
6.  If you want to use Shibboleth in addition to another authentication method
    ...
    'Administrations >> Users >> Manage authentication' to contain a link to the
     agpu/auth/shibboleth/index.php file which is protected by
     ...
     Note: As of now you cannot use dual login together with the integrated
           WAYF service provided by agpu (4.b).

7. Save the authentication changes.

How the Shibboleth authentication works
--------------------------------------------------------------------------------
To get Shibboleth authenticated in agpu a user basically must access the
...
his home organization in the agpu WAYF service or if the alternate login URL
...
agpu basically checks whether the Shibboleth attribute that you mapped
as the username is present. This attribute should only be present if a user is
Shibboleth authenticated.

If the user's agpu account has not existed yet, it gets automatically created.

To prevent that every Shibboleth user can access your agpu site you have to
...
authentication method unless they have two accounts in agpu.
...
form that sends 'username' and 'password' to agpu/login/index.php. Set this
web page then als alternate login page.
Consult the agpu documentation for further instructions and requirements.

How to customize the way the Shibboleth user data is used in agpu
--------------------------------------------------------------------------------
Among the Shibboleth settings in agpu there is a field that should contain a
...
attributes are used in agpu. Due to security reasons this file cannot be
...
Example 1: Your Shibboleth federation uses an attribute that specifies the
           ...
           compatible with the agpu data representation, e.g. the Shibboleth
           attribute contains 'German' but agpu needs a two letter value like
           'de'.
Example 2: The country, city and street are provided in one Shibboleth attribute
           and you want these values to be used in the agpu user profile. So
...
agpu/auth/shibboleth/auth.php, especially the function 'get_userinfo'
...
$_SERVER['sn'].
Because agpu needs to know what Shibboleth attributes it shall map onto which
agpu user profile field, one has to make sure the mapping is updated as well
after the Service Provider upgrade.

********************************************************************************
Because you risk locking yourself out of agpu it is strongly
...
3. After the SP upgrade, use this account to log into agpu and adapt the
...
--------------------------------------------------------------------------------
In order make agpu support Shibboleth logout, one has to make the Shibboleth
Service Provider (SP) aware of the agpu logout capability. Only then the SP
can trigger agpu's front or back channel logout handler.

To make the SP aware of the agpu logout, you have to add the following to the
...
<Notify
    Channel="back"
    Location="https://#YOUR_agpu_HOSTNAME#/agpu/auth/shibboleth/logout.php" />
--

Then restart the Shibboleth daemon and check the log file for errors. If there
were no errors, you can test the logout feature by accessing agpu,
authenticating via Shibboleth and the access the URL:
#YOUR_agpu_HOSTNAME#/Shibboleth.sso/Logout (assuming you have a standard
Shibboleth installation). If everything worked well, you should see a Shibboleth
page saying that you were successfully logged out and if you go back to agpu
you also should be logged out from agpu.
...
- /agpu/auth/shibboleth/logout.php *must not* be protected by Shibboleth!
  In case all of agpu is protected with Shibboleth, you have to add something
  like this to your Apache configuration after all the other require rules

--
<Directory /path/to/agpu/auth/shibboleth/logout.php>
    AuthType shibboleth
...
logout. Hopefully, the agpu logout helps to motivate the developers to
