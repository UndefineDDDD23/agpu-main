...
class course_competency_settings_test extends \advanced_testcase {
    ...
    public function test_who_can_change_settings(): void {
        ...
        assign_capability('agpu/competency:coursecompetencyconfigure', CAP_ALLOW, $role, $syscontext->id);
        assign_capability('agpu/competency:competencygrade', CAP_ALLOW, $role, $syscontext->id);
        assign_capability('agpu/competency:coursecompetencyview', CAP_ALLOW, $role, $syscontext->id);
        assign_capability('agpu/competency:planview', CAP_ALLOW, $role, $syscontext->id);
        $gradedrole = create_role('Graded role', 'graded', 'Someone who can be graded');
        assign_capability('agpu/competency:coursecompetencygradable', CAP_ALLOW, $gradedrole, $syscontext->id);
