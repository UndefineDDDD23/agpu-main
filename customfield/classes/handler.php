 ...
 * @copyright 2018 David Matamoros <davidmc@agpu.com>
...
abstract class handler {
    ...
    public static function get_handler(string $component, string $area, int $itemid = 0): handler {
        ...
        throw new \agpu_exception('unknownhandler', 'core_customfield', '', $a);
    }
     ...
     * @return \agpu_url
     */
    abstract public function get_configuration_url(): \agpu_url;
    ...
    protected function validate_category(category_controller $category): category_controller {
        ...
        if (!array_key_exists($category->get('id'), $categories)) {
            throw new \agpu_exception('categorynotfound', 'core_customfield');
        }
    ...
    protected function validate_field(field_controller $field): field_controller {
        if (!array_key_exists($field->get('categoryid'), $this->get_categories_with_fields())) {
            throw new \agpu_exception('fieldnotfound', 'core_customfield');
        }
     ...
     *
     * This method is called from agpuform::definition_after_data and allows to tweak
     * mform with some data coming directly from the field plugin data controller.
     *
     * @param \agpuQuickForm $mform
     * @param int $instanceid
     */
    public function instance_form_definition_after_data(\agpuQuickForm $mform, int $instanceid = 0) {
    ...
    /**
     * Validates the given data for custom fields, used in agpuform validation() function
     *
     * Example:
     ...
     */
    public function instance_form_definition(\agpuQuickForm $mform, int $instanceid = 0,
     ...
     * @param \agpuQuickForm $mform
     */
    public function config_form_definition(\agpuQuickForm $mform) {
    }
