...
define('NO_agpu_COOKIES', true); // No need for a session here.
...
if ($badge->status != BADGE_STATUS_INACTIVE) {
    if (is_null($action)) {
        ...
        $urlimage = agpu_url::make_pluginfile_url($context->id, 'badges', 'badgeimage', $badge->id, '/', 'f3')->out(false);

        $url = new agpu_url('/badges/badge_json.php', array('id' => $badge->id));
        ...
        if ($badge->imageauthorname ||
                ...
                $badge->imagecaption) {
            $urlimage = agpu_url::make_pluginfile_url($context->id,
            ...
            if ($badge->imageauthorname || $badge->imageauthoremail || $badge->imageauthorurl) {
                $authorimage = new agpu_url('/badges/image_author_json.php', array('id' => $badge->id));
        ...
        if (!empty($relatedbadges)) {
            foreach ($relatedbadges as $related) {
                $relatedurl = new agpu_url('/badges/badge_json.php', array('id' => $related->id));
        ...
        if (!empty($endorsement)) {
            $endorsementurl = new agpu_url('/badges/endorsement_json.php', array('id' => $badge->id));
    ...
    $badgeurl = new agpu_url('/badges/badge_json.php', array('id' => $id));
