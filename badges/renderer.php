...
class core_badges_renderer extends plugin_renderer_base {
    ...
    public function print_badges_list($badges, $userid, $profile = false, $external = false) {
        ...
        foreach ($badges as $badge) {
            if (!$external) {
                ...
                $bname = $badge->name;
                $imageurl = agpu_url::make_pluginfile_url($context->id, 'badges', 'badgeimage', $badge->id, '/', 'f3', false);
            } else {
                $bname = '';
                $imageurl = '';
                if (!empty($badge->name)) {
            ...
            if (($userid == $USER->id) && !$profile) {
                ...
                if (!empty($CFG->badges_allowexternalbackpack) && $notexpiredbadge && $userbackpack) {
                    $assertion = new agpu_url('/badges/assertion.php', array('b' => $badge->uniquehash));
                    ...
                    if (badges_open_badges_backpack_api($userbackpack->id) == OPEN_BADGES_V2) {
                        $addurl = new agpu_url('/badges/backpack-add.php', array('hash' => $badge->uniquehash));
                        $push = $this->output->action_icon($addurl, $icon);
                    } else if (badges_open_badges_backpack_api($userbackpack->id) == OPEN_BADGES_V2P1) {
                        $addurl = new agpu_url('/badges/backpack-export.php', array('hash' => $badge->uniquehash));
                        $push = $this->output->action_icon($addurl, $icon);
                    }
                }
                ...
                if ($badge->visible) {
                    $url = new agpu_url('mybadges.php', array('hide' => $badge->issuedid, 'sesskey' => sesskey()));
                    $status = $this->output->action_icon($url, new pix_icon('t/hide', get_string('makeprivate', 'badges')));
                } else {
                    $url = new agpu_url('mybadges.php', array('show' => $badge->issuedid, 'sesskey' => sesskey()));
                    $status = $this->output->action_icon($url, new pix_icon('t/show', get_string('makepublic', 'badges')));
                }
            }

            if (!$profile) {
                $url = new agpu_url('badge.php', array('hash' => $badge->uniquehash));
            } else {
                if (!$external) {
                    $url = new agpu_url('/badges/badge.php', array('hash' => $badge->uniquehash));
                } else {
                    $hash = hash('md5', $badge->hostedUrl);
                    $url = new agpu_url('/badges/external.php', array('hash' => $hash, 'user' => $userid));
                }
            }
    ...
    public function print_badge_overview($badge, $context) {
        ...
        if ($badge->has_criteria()) {
            ...
            if (has_capability('agpu/badges:configurecriteria', $context)) {
                $display .= $this->output->single_button(
                    new agpu_url('/badges/criteria.php', array('id' => $badge->id)),
                    get_string('addcriteria', 'badges'), 'POST', array('class' => 'activatebadge'));
            }
        }

        // Awards details if any.
        if (has_capability('agpu/badges:viewawarded', $context)) {
            $display .= $this->heading(get_string('awards', 'badges'), 3);
            if ($badge->has_awards()) {
                $url = new agpu_url('/badges/recipients.php', array('id' => $badge->id));
                $a = new stdClass();
            ...
            if (has_capability('agpu/badges:awardbadge', $context) &&
                $badge->has_manual_award_criteria() &&
                $badge->is_active()) {
                $display .= $this->output->single_button(
                        new agpu_url('/badges/award.php', array('id' => $badge->id)),
    ...
    public function print_badge_table_actions($badge, $context) {
        ...
        if (has_capability('agpu/badges:configuredetails', $context) && $badge->has_criteria()) {
            ...
            if ($badge->status == BADGE_STATUS_INACTIVE || $badge->status == BADGE_STATUS_INACTIVE_LOCKED) {
                ...
                $url = new agpu_url('/badges/action.php');
                $url->param('id', $badge->id);
                $url->param('activate', true);
                $url->param('sesskey', sesskey());
                $return = new agpu_url(qualified_me());
        ...
        if ($badge->has_manual_award_criteria() &&
                has_capability('agpu/badges:awardbadge', $context) &&
                $badge->is_active()) {
            $url = new agpu_url('/badges/award.php', array('id' => $badge->id));
            $actions .= $this->output->action_icon($url, new pix_icon('t/award', get_string('award', 'badges'))) . " ";
        }

        // Edit badge.
        if (has_capability('agpu/badges:configuredetails', $context)) {
            $url = new agpu_url('/badges/edit.php', array('id' => $badge->id, 'action' => 'badge'));
            $actions .= $this->output->action_icon($url, new pix_icon('t/edit', get_string('edit'))) . " ";
        }

        // Duplicate badge.
        if (has_capability('agpu/badges:createbadge', $context)) {
            $url = new agpu_url('/badges/action.php', array('copy' => '1', 'id' => $badge->id, 'sesskey' => sesskey()));
            $actions .= $this->output->action_icon($url, new pix_icon('t/copy', get_string('copy'))) . " ";
        }

        // Delete badge.
        if (has_capability('agpu/badges:deletebadge', $context)) {
            $url = new agpu_url(qualified_me());
    ...
    protected function render_badge_user_collection(\core_badges\output\badge_user_collection $badges) {
        ...
        $mybackpack = new agpu_url('/badges/mybackpack.php');
        ...
        $actionhtml = $this->output->single_button(
                    new agpu_url('/badges/mybadges.php', array('downloadall' => true, 'sesskey' => sesskey())),
        ...
        if (!empty($CFG->badges_allowexternalbackpack)) {
            ...
            $backpacksettings = html_writer::link(new agpu_url('/badges/mybackpack.php'), $label, $attr);
     ...
     * @todo MDL-80455 this will be removed in agpu 4.8
     */
    protected function render_badge_collection(\core_badges\output\badge_collection $badges) {
        ...
        foreach ($badges->badges as $badge) {
            ...
            if ($badge->dateissued) {
                ...
                $badgeurl = new agpu_url('/badges/badge.php', array('hash' => $badge->uniquehash));
    ...
    protected function render_badge_management(\core_badges\output\badge_management $badges) {
        ...
        foreach ($badges->badges as $b) {
            ...
            $name = html_writer::link(new agpu_url('/badges/overview.php', array('id' => $b->id)), $forlink, $style);
            $status = $b->statstring;
            $criteria = self::print_badge_criteria($b, 'short');

            if (has_capability('agpu/badges:viewawarded', $this->page->context)) {
                $awards = html_writer::link(new agpu_url('/badges/recipients.php', array('id' => $b->id)), $b->awards);
            } else {
                $awards = $b->awards;
            }
    ...
    /**
     * @deprecated since agpu 4.0 - Use the manage_badge_action_bar tertiary navigation instead.
     */
    ...
    public function print_badge_status_box(badge $badge) {
        if (has_capability('agpu/badges:configurecriteria', $badge->get_context())) {

            if (!$badge->has_criteria()) {
                $criteriaurl = new agpu_url('/badges/criteria.php', array('id' => $badge->id));
        ...
        if (!$badge->is_active() && !$badge->is_locked()) {
            ...
            if (!empty($potential)) {
                ...
                $output .= $this->output->single_select(
                    new agpu_url('/badges/criteria_settings.php', array('badgeid' => $badge->id, 'add' => true)),
                    'type',
                    $select,
                    '',
                    array('' => 'choosedots'),
    ...
    protected function render_badge_recipients(\core_badges\output\badge_recipients $recipients) {
        ...
        foreach ($recipients->userids as $holder) {
            ...
            $fullname = html_writer::link(
                            new agpu_url('/user/profile.php', array('id' => $holder->userid)),
                            $fullname
                        );
            $awarded  = userdate($holder->dateissued);
            $badgeurl = html_writer::link(
                            new agpu_url('/badges/badge.php', array('hash' => $holder->uniquehash)),
    ...
    protected function helper_search_form($search) {
        ...
        $mform = new agpuQuickForm('searchform', 'POST', $this->page->url);
    ...
    protected function print_badge_related(badge $badge) {
        ...
        if (!empty($relatedbadges)) {
            ...
            foreach ($relatedbadges as $related) {
                $relatedurl = new agpu_url('/badges/overview.php', array('id' => $related->id));
    ...
    protected function print_badge_alignments(badge $badge) {
        ...
        if (!empty($alignments)) {
            ...
            foreach ($alignments as $alignment) {
                $urlaligment = new agpu_url('alignment.php',
    ...
    protected function render_badge_related(\core_badges\output\badge_related $related) {
        ...
        foreach ($related->badges as $badge) {
            ...
            $name = html_writer::link(new agpu_url('/badges/overview.php', array('id' => $badgeobject->id)), $forlink, $style);
            ...
            if (!$currentbadge->is_active() && !$currentbadge->is_locked()) {
                $action = $this->output->action_icon(
                    new agpu_url('/badges/related_action.php', [
        ...
        foreach ($alignments->alignments as $item) {
            ...
            if (!$currentbadge->is_active() && !$currentbadge->is_locked()) {
                $delete = $this->output->action_icon(
                    new agpu_url('/badges/alignment_action.php', [
                        'id' => $currentbadge->id,
                        'alignmentid' => $item->id,
                        'sesskey' => sesskey(),
                        'action' => 'remove'
