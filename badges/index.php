...
$returnurl = new agpu_url('/badges/index.php', $urlparams);
...
if ($type == BADGE_TYPE_SITE) {
    ...
    navigation_node::override_active_url(new agpu_url('/badges/index.php', ['type' => BADGE_TYPE_SITE]), true);
    ...
    navigation_node::override_active_url(
        new agpu_url('/badges/index.php', ['type' => BADGE_TYPE_COURSE, 'id' => $course->id])
    );
...
if (!has_any_capability([
        ...
        'agpu/badges:deletebadge'], $PAGE->context)) {
...
if ($delete || $archive) {
    ...
    require_capability('agpu/badges:deletebadge', $badge->get_context());
    if (!$confirm) {
        echo $output->header();
        // Archive this badge?
        echo $output->heading(get_string('archivebadge', 'badges', $badge->name));
        $archivebutton = $output->single_button(
                            new agpu_url($PAGE->url, ['archive' => $badge->id, 'confirm' => 1]),
        ...
        $deletebutton = $output->single_button(
                            new agpu_url($PAGE->url, ['delete' => $badge->id, 'confirm' => 1]),
...
if ($deactivate && has_capability('agpu/badges:configuredetails', $PAGE->context)) {
    require_sesskey();
