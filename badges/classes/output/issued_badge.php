...
class issued_badge implements renderable {
    ...
    public function export_for_template(renderer_base $output): stdClass {
        ...
        if (!empty($relatedbadges)) {
            ...
            foreach ($relatedbadges as $related) {
                if (isloggedin() && ($context instanceof context_course && !is_guest($context))) {
                    $related->url = (new agpu_url('/badges/overview.php', ['id' => $related->id]))->out(false);
                }
        ...
        if ($USER->id == $this->recipient->id && !empty($CFG->enablebadges)) {
            $data->downloadurl = (new agpu_url('/badges/badge.php', ['hash' => $this->hash, 'bake' => true]))->out(false);
            ...
            if (!empty($CFG->badges_allowexternalbackpack) && ($expiration > $now)
                ...
                if (badges_open_badges_backpack_api($userbackpack->id) == OPEN_BADGES_V2P1) {
                    $addtobackpackurl = new agpu_url('/badges/backpack-export.php', ['hash' => $this->hash]);
                } else {
                    $addtobackpackurl = new agpu_url('/badges/backpack-add.php', ['hash' => $this->hash]);
                }
