...
class badge {
    ...
    public function __construct($badgeid) {
        ...
        if (empty($data)) {
            throw new agpu_exception('error:nosuchbadge', 'badges', '', $badgeid);
        }
    ...
    public function save() {
        ...
        if ($DB->update_record_raw('badge', $fordb)) {
            ...
            throw new agpu_exception('error:save', 'badges');
            return false;
        }
    }
    ...
    public function make_clone() {
        ...
        if ($new = $DB->insert_record('badge', $fordb, true)) {
            ...
            throw new agpu_exception('error:clone', 'badges');
            return false;
        }
    }
    ...
    public function review_all_criteria() {
        ...
        foreach ($this->criteria as $crit) {
            ...
            if ($this->type == BADGE_TYPE_SITE) {
                ...
                list($enrolledsql, $enrolledparams) = get_enrolled_sql($this->get_context(), 'agpu/badges:earnbadge', 0, true);
    ...
    public function get_badge_issuer(?int $obversion = null) {
        return [
            ...
            'id' => (new agpu_url('/badges/issuer_json.php', ['id' => $this->id]))->out(false),
