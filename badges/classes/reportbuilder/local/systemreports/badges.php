...
class badges extends system_report {
    ...
    protected function can_view(): bool {
        return has_any_capability([
            ...
            'agpu/badges:deletebadge'], $this->get_context());
    }
    ...
    public function add_columns(string $badgeissuedalias): void {
        ...
        if ($canviewdraftbadges) {
            ...
            $this->add_column((new column(
                ...
                ->set_callback(function(int $count): string {
                    if (!has_capability('agpu/badges:viewawarded', $this->get_context())) {
                        return (string) $count;
                    }

                    return html_writer::link(new agpu_url('/badges/recipients.php', ['id' => $this->badgeid]), $count);
                }));
        }
        ...
        $this->get_column('badge_issued:issued')
            ...
            ->set_callback(static function(?int $value, stdClass $row) {
                ...
                $badgeurl = new agpu_url('/badges/badge.php', ['hash' => $row->uniquehash]);
    ...
    protected function add_actions(): void {
        ...
        $this->add_action((new action(
            ...
            return has_capability('agpu/badges:configuredetails', $badge->get_context()) &&
                $badge->has_criteria() &&
        ...
        $this->add_action((new action(
            ...
            $badge = new \core_badges\badge($row->id);
            return has_capability('agpu/badges:awardbadge', $badge->get_context()) &&
        ...
        $this->add_action((new action(
            ...
            return has_capability('agpu/badges:configuredetails', $context);

        }));

        // Duplicate action.
        $this->add_action((new action(
            new agpu_url('/badges/action.php', [
                'id' => ':id',
                'copy' => 1,
                'sesskey' => sesskey(),
            ]),
            ...
            $context = self::get_badge_context((int)$row->type, (int)$row->courseid);
            return has_capability('agpu/badges:createbadge', $context);
        }));

        // Delete action.
        $this->add_action((new action(
            new agpu_url('/badges/index.php', [
                'delete' => ':id',
                'type' => ':type',
                'id' => ':courseid',
            ]),
            ...
            $context = self::get_badge_context((int)$row->type, (int)$row->courseid);
            return has_capability('agpu/badges:deletebadge', $context);
        }));
    }
