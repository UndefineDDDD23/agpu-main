...
if (!empty($badgeid)) {
    ...
    if ($action == 'message') {
        require_capability('agpu/badges:configuremessages', $context);
    } else {
        require_capability('agpu/badges:configuredetails', $context);
    }

    $cancelurl = new agpu_url('/badges/overview.php', ['id' => $badgeid]);
} else {
    // New badge.
    if ($courseid) {
    ...
    // Check capabilities.
    require_capability('agpu/badges:createbadge', $context);

    $cancelurl = new agpu_url('/badges/index.php', ['type' => $badge->type, 'id' => $courseid]);
}
...
$navurl = new agpu_url('/badges/index.php', ['type' => $badge->type]);
if ($badge->type == BADGE_TYPE_COURSE) {
    require_login($badge->courseid);
    $heading = format_string($course->fullname, true, ['context' => $context]);
    $navurl = new agpu_url('/badges/index.php', ['type' => $badge->type, 'id' => $badge->courseid]);
...
$currenturl = new agpu_url('/badges/edit.php', $params);
    ...
    switch ($action) {
        case 'new':
            ...
            if (has_capability('agpu/badges:configurecriteria', $context)) {
                redirect(new agpu_url('/badges/criteria.php', ['id' => $badgeid]));
            }
            redirect(new agpu_url('/badges/overview.php', ['id' => $badgeid]));
            break;
