...
/**
 * For a given user, this returns the $page information for their My agpu page
 *
...
function my_get_page(?int $userid, int $private = MY_PAGE_PRIVATE, string $pagename = MY_PAGE_DEFAULT) {
    ...
    if (empty($CFG->forcedefaultmyagpu) && $userid) {  // Ignore custom My agpu pages if admin has forced them
...
/**
 * For a given user, this deletes their My agpu page and returns them to the system default.
 *
 * @param int $userid the id of the user whose page should be reset
 * @param int $private either MY_PAGE_PRIVATE or MY_PAGE_PUBLIC
 * @param string $pagetype either my-index or user-profile
...
class my_syspage_block_manager extends block_manager {
     ...
     *
     * This is needed because the My agpu pages set the page context to the
