...
foreach ($accounts as $account) {
    $gateways = [];
    $canmanage = has_capability('agpu/payment:manageaccounts', $account->get_context());
...
if (has_capability('agpu/site:config', context_system::instance())) {
    ...
    $link = html_writer::link(new agpu_url('/admin/settings.php', ['section' => 'managepaymentgateways']),
...
echo html_writer::div(html_writer::link(new agpu_url($PAGE->url, ['showarchived' => !$showarchived]),
    $showarchived ? get_string('hidearchived', 'payment') : get_string('showarchived', 'payment')), 'mdl-right');

echo $OUTPUT->single_button(new agpu_url('/payment/manage_account.php'), get_string('createaccount', 'payment'), 'get');
