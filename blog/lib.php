 ...
 * the capability agpu/blog:create, or if they have the capability
 * agpu/blog:manageentries.
 *
...
function blog_user_can_edit_entry($blogentry) {
    ...
    if (has_capability('agpu/blog:manageentries', $sitecontext)) {
        return true; // Can edit any blog entry.
    }

    if ($blogentry->userid == $USER->id && has_capability('agpu/blog:create', $sitecontext)) {
...
function blog_user_can_view_user_entry($targetuserid, $blogentry=null) {
    ...
    if ($blogentry && $blogentry->publishstate == 'draft' && !has_capability('agpu/blog:viewdrafts', $sitecontext)) {
    ...
    switch ($CFG->bloglevel) {
        ...
        default:
            ...
            return has_capability('agpu/user:readuserblogs', $personalcontext);
        break;
 ...
 * for new entries not yet copied into agpu.
...
function blog_delete_external_entries($externalblog) {
    global $DB;
    require_capability('agpu/blog:manageexternal', context_system::instance());
 ...
 *
 * @param agpu_page $page The page to load for (normally $PAGE)
 * @param stdClass $userid Load for a specific user
 * @return array An array of options organised by type.
 */
function blog_get_all_options(agpu_page $page, ?stdClass $userid = null) {
    global $CFG, $DB, $USER;
    ...
    if (!empty($CFG->enableblogs)
        ...
        && has_capability('agpu/blog:view', context_system::instance())) {

        $options[CONTEXT_SYSTEM] = array('viewsite' => array(
            'string' => get_string('viewsiteentries', 'blog'),
            'link' => new agpu_url('/blog/index.php')
        ));
    }
...
function blog_get_options_for_user(?stdClass $user=null) {
    ...
    $sitecontext = context_system::instance();
    $canview = has_capability('agpu/blog:view', $sitecontext);
    ...
    if (!$iscurrentuser && $canview && ($CFG->bloglevel >= BLOG_SITE_LEVEL)) {
        ...
        $options['userentries'] = array(
            ...
            'link' => new agpu_url('/blog/index.php', array('userid' => $user->id))
        );
    } else {
        ...
        if (has_capability('agpu/blog:create', $sitecontext)) {
            // We can add to our own blog.
            $options['add'] = array(
                'string' => get_string('addnewentry', 'blog'),
                'link' => new agpu_url('/blog/edit.php', array('action' => 'add'))
            );
        }
    }
    if ($canview && $CFG->enablerssfeeds) {
        $options['rss'] = array(
            ...
            'link' => new agpu_url(rss_get_url($sitecontext->id, $USER->id, 'blog', 'user/'.$user->id))
        );
    }
...
function blog_get_options_for_course(stdClass $course, ?stdClass $user=null) {
    ...
    if (has_capability('agpu/blog:create', $sitecontext)) {
        ...
        $options['courseadd'] = array(
            ...
            'link' => new agpu_url('/blog/edit.php', array('action' => 'add', 'courseid' => $course->id))
        );
    }
...
function blog_get_options_for_module($module, $user=null) {
    ...
    if (has_capability('agpu/blog:view', $sitecontext)) {
        ...
        if ($CFG->bloglevel >= BLOG_SITE_LEVEL) {
            ...
            $options['moduleview'] = array(
                ...
                'link' => new agpu_url('/blog/index.php', array('modid' => $module->id))
            );
        }
        // View MY entries about this module.
        $options['moduleviewmine'] = array(
            'string' => get_string('viewmyentriesaboutmodule', 'blog', $modulename),
            'link' => new agpu_url('/blog/index.php', array('modid' => $module->id, 'userid' => $USER->id))
        );
        if (!empty($user) && ($CFG->bloglevel >= BLOG_SITE_LEVEL)) {
    ...
    if (has_capability('agpu/blog:create', $sitecontext)) {
        ...
        $options['moduleadd'] = array(
            ...
            'link' => new agpu_url('/blog/edit.php', array('action' => 'add', 'modid' => $module->id))
        );
    }
    // Cache the options.
 ...
 * 4. url:     The agpu_url object used as the base for add and view links
...
function blog_get_headers($courseid=null, $groupid=null, $userid=null, $tagid=null, $tag=null, $modid=null, $entryid=null,
    ...
    $blogurl = new agpu_url('/blog/index.php');
    ...
    if (!empty($entryid)) {
        ...
        $PAGE->set_title($entry->subject . agpu_page::TITLE_SEPARATOR . $blogentryby);
    ...
    if (!empty($userid) && empty($entryid) && ((empty($courseid) && empty($modid)) || !$CFG->useblogassociations)) {
        ...
        $PAGE->set_title(fullname($user) . ': ' . get_string('blogentries', 'blog') . agpu_page::TITLE_SEPARATOR . $cm->name);
    ...
    if (!in_array($action, array('edit', 'add'))) {
        ...
        // Append Search info.
        if (!empty($search) && has_capability('agpu/blog:search', $sitecontext)) {
...
function blog_comment_validate($commentparam) {
    ...
    $sitecontext = context_system::instance();
    return has_capability('agpu/blog:view', $sitecontext) &&
...
function core_blog_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    $url = new agpu_url("/blog/index.php", array('userid' => $user->id));
    if (!empty($course)) {
...
function blog_get_tagged_posts($tag, $exclusivemode = false, $fromctx = 0, $ctx = 0, $rec = true, $page = 0) {
    ...
    $content = '';
    if (empty($CFG->enableblogs) || !has_capability('agpu/blog:view', $systemcontext)) {
        ...
        if (!empty($blogs)) {
            ...
            foreach ($blogs as $blog) {
                ...
                $url = new agpu_url('/blog/index.php', array('entryid' => $blog->id));
                $subject = html_writer::link($url, $subject, array('class' => $class));

                $fullname = fullname($user);
                if (user_can_view_profile($user)) {
                    $profilelink = new agpu_url('/user/view.php', array('id' => $blog->userid));
            ...
            $allblogsurl = new agpu_url('/blog/index.php', $urlparams);
...
function blog_validate_access($courseid, $modid, $groupid, $entryid, $userid) {
    ...
    if (!$userid && has_capability('agpu/blog:view', $sitecontext) && $CFG->bloglevel > BLOG_USER_LEVEL) {
        if ($entryid) {
            if (!$entryobject = $DB->get_record('post', array('id' => $entryid))) {
                throw new \agpu_exception('nosuchentry', 'blog');
            }
    ...
    if (!empty($modid)) {
        if ($CFG->bloglevel < BLOG_SITE_LEVEL) {
            throw new \agpu_exception('courseblogdisable', 'blog');
        }
        if (!$mod = $DB->get_record('course_modules', array('id' => $modid))) {
            throw new \agpu_exception('invalidmoduleid', 'error', '', $modid);
        }
        $courseid = $mod->course;
    }
    ...
    if ((empty($courseid) ? true : $courseid == SITEID) && empty($userid)) {
        if ($CFG->bloglevel < BLOG_SITE_LEVEL) {
            throw new \agpu_exception('siteblogdisable', 'blog');
        }
        if (!has_capability('agpu/blog:view', $sitecontext)) {
            throw new \agpu_exception('cannotviewsiteblog', 'blog');
        }
    ...
    if (!empty($courseid)) {
        ...
        if (!has_capability('agpu/blog:view', $sitecontext)) {
            throw new \agpu_exception('cannotviewcourseblog', 'blog');
        }
    } else {
    ...
    if (!empty($groupid)) {
        if ($CFG->bloglevel < BLOG_SITE_LEVEL) {
            throw new \agpu_exception('groupblogdisable', 'blog');
        }

        if (! $group = groups_get_group($groupid)) {
            throw new \agpu_exception('invalidgroupid', 'blog');
        }
        ...
        if (!has_capability('agpu/blog:view', $sitecontext)) {
            throw new \agpu_exception('cannotviewcourseorgroupblog', 'blog');
        }

        if (groups_get_course_groupmode($course) == SEPARATEGROUPS &&
                !has_capability('agpu/site:accessallgroups', $coursecontext)) {

            if (!groups_is_member($groupid)) {
                throw new \agpu_exception('notmemberofgroup');
            }
        }
    }
    ...
    if (!empty($userid)) {
        if ($CFG->bloglevel < BLOG_USER_LEVEL) {
            throw new \agpu_exception('blogdisable', 'blog');
        }
        ...
        if ($user->deleted) {
            throw new \agpu_exception('userdeleted');
        }

        if ($USER->id == $userid) {
            if (!has_capability('agpu/blog:create', $sitecontext)
              && !has_capability('agpu/blog:view', $sitecontext)) {
                throw new \agpu_exception('donothaveblog', 'blog');
            }
        } else {
            if (!has_capability('agpu/blog:view', $sitecontext) || !blog_user_can_view_user_entry($userid)) {
                throw new \agpu_exception('cannotviewcourseblog', 'blog');
            }
        }
    }
