...
$returnurl = new agpu_url('/blog/index.php');
...
if (!has_capability('agpu/blog:create', $sitecontext) && !has_capability('agpu/blog:manageentries', $sitecontext)) {
    throw new \agpu_exception('cannoteditentryorblog', 'blog');
}

// Make sure that the person trying to edit has access right.
if ($id) {
    if (!blog_user_can_edit_entry($entry)) {
        throw new \agpu_exception('notallowedtoedit', 'blog');
    }
    ...
    if (!has_capability('agpu/blog:create', $sitecontext)) {
        throw new \agpu_exception('noentry', 'blog'); // The capability "manageentries" is not enough for adding.
    }
}
...
if ($action === 'delete') {
    ...
    if (data_submitted() && $confirm && confirm_sesskey()) {
        ...
        echo $OUTPUT->confirm(get_string('blogdeleteconfirm', 'blog', format_string($entry->subject)),
                              new agpu_url('edit.php', $optionsyes),
