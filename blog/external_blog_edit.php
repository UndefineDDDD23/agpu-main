...
require_capability('agpu/blog:manageexternal', $context);
...
$url = new agpu_url('/blog/external_blog_edit.php');
if ($id !== null) {
    $url->param('id', $id);
}
$PAGE->set_url($url);
...
$returnurl = new agpu_url('/blog/external_blogs.php');
...
if (!empty($id)) {
    if (!$external = $DB->get_record('blog_external', array('id' => $id, 'userid' => $USER->id))) {
        throw new \agpu_exception('wrongexternalid', 'blog');
    }
...
if ($externalblogform->is_cancelled()) {
    ...
    switch ($action) {
        case 'add':
            $rss = new agpu_simplepie($data->url);
...
navigation_node::override_active_url(new agpu_url('/blog/external_blogs.php'));
