...
class externallib_test extends externallib_advanced_testcase {
    ...
    public function test_send_instant_messages_no_capability(): void {
        ...
        $this->unassignUserCapability('agpu/site:sendmessage', $contextid, $userrole->id);
    ...
    public function test_block_user_when_ineffective(): void {
        ...
        assign_capability('agpu/site:messageanyuser', CAP_ALLOW, $authenticateduser->id, \context_system::instance(), true);
    ...
    public function test_get_messages(): void {
        ...
        $im1 = message_post_message($user1, $user2, 'some random text 1', FORMAT_agpu);
        $im2 = message_post_message($user1, $user3, 'some random text 2', FORMAT_agpu);
        $im3 = message_post_message($user2, $user3, 'some random text 3', FORMAT_agpu);
        $im4 = message_post_message($user3, $user2, 'some random text 4', FORMAT_agpu);
        $im5 = message_post_message($user3, $user1, 'some random text 5', FORMAT_agpu);
        $im6 = message_post_message($user1, $user2, 'some random text 6', FORMAT_agpu);
        ...
        $eventdata->modulename        = 'agpu';
        $eventdata->component         = 'enrol_paypal';
        $eventdata->name              = 'paypal_enrolment';
        $eventdata->userfrom          = get_admin();
        $eventdata->userto            = $user1;
        $eventdata->subject           = "agpu: PayPal payment";
        ...
        $message->contexturl        = (string)new \agpu_url('/course/view.php', array('id' => $course->id));
    ...
    public function test_delete_message(): void {
        ...
        $m1to2 = message_post_message($user1, $user2, 'some random text 1', FORMAT_agpu);
        $m2to3 = message_post_message($user2, $user3, 'some random text 3', FORMAT_agpu);
        $m3to2 = message_post_message($user3, $user2, 'some random text 4', FORMAT_agpu);
        $m3to4 = message_post_message($user3, $user4, 'some random text 4', FORMAT_agpu);
    ...
    /**
     * Tests searching users as another user having the 'agpu/user:viewdetails' capability.
     */
    ...
    /**
     * Tests searching users as another user without the 'agpu/user:viewdetails' capability.
     */
    ...
    public function test_get_unread_conversations_count(): void {
        ...
        $this->send_message($user1, $user4, 'Hey mate, you see the new messaging UI in agpu?');
    ...
    public function test_get_user_message_preferences(): void {
        ...
        // Set a couple of preferences to test.
        set_user_preference('message_provider_agpu_instantmessage_enabled', 'email', $user);
        ...
        foreach ($prefs['preferences']['components'] as $component) {
            foreach ($component['notifications'] as $prefdata) {
                if ($prefdata['preferencekey'] != 'message_provider_agpu_instantmessage') {
                    continue;
                }
                foreach ($prefdata['processors'] as $processor) {
