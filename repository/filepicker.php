...
$home_url = new agpu_url('/repository/draftfiles_manager.php', $params);

$params['savepath'] = $savepath;
$params['repo_id'] = $repo_id;
$url = new agpu_url("/repository/filepicker.php", $params);
...
case 'search':
    ...
    try {
        ...
        $purl = new agpu_url($url, array('search_paging' => 1, 'action' => 'search', 'repo_id' => $repo_id));
        ...
        if (!empty($list['upload'])) {
            ...
            if (!empty($list['path'])) {
                foreach ($list['path'] as $p) {
                    ...
                    $pathurl = new agpu_url($url, array(
                        'p'=>$p['path'],
                        'action'=>'list',
                        'draftpath'=>$draftpath,
                        'savepath'=>$savepath
            ...
            if (!empty($list['page'])) {
                ...
                $pagingurl = new agpu_url("/repository/filepicker.php?action=list&itemid=$itemid&ctx_id=$contextid&repo_id=$repo_id&course=$courseid&sesskey=".  sesskey());
...
case 'download':
    ...
    if (!$repo->file_is_accessible($fileurl)) {
        throw new \agpu_exception('storedfilecannotread');
    }
    $record = new stdClass();
    ...
    if ($repo->has_agpu_files()) {
        ...
        if (!empty($thefile['path'])) {
            ...
            if ($maxbytes != -1 && $filesize>$maxbytes) {
                unlink($thefile['path']);
                throw new \agpu_exception('maxbytes');
            }
            ...
            if (file_is_draft_areas_limit_reached($USER->id)) {
                unlink($thefile['path']);
                throw new \agpu_exception('maxdraftitemids');
            }
            try {
            ...
            throw new \agpu_exception('cannotdownload', 'repository');
        }
    }
