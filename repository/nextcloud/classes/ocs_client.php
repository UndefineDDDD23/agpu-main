...
class ocs_client extends rest {
     ...
     * @throws \agpu_exception when trying to construct a agpuurl
     */
    public function __construct(client $oauthclient) {
        ...
        $this->ocsendpoint = new \agpu_url($ocsendpoint);
    ...
    public function get_api_functions() {
        return [
            'create_share' => [
                ...
                'method' => 'post',
                'args' => [
                    'path' => PARAM_TEXT, // Could be PARAM_PATH, we really don't want to enforce a agpu understanding of paths.
                    'shareType' => PARAM_INT,
    ...
    /**
     * In POST requests, agpu's REST API assumes that params are
     ...
     * Curl assumes the content type to be `multipart/form-data` then, but the agpu REST API tries to put
