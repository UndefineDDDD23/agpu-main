...
class repository_upload extends repository {
    ...
    public function process_upload($saveasfilename, $maxbytes, $types = '*', $savepath = '/', $itemid = 0,
        ...
        if (!isset($_FILES[$elname])) {
            throw new agpu_exception('nofile');
        }
        if (!empty($_FILES[$elname]['error'])) {
            switch ($_FILES[$elname]['error']) {
                case UPLOAD_ERR_INI_SIZE:
                    throw new agpu_exception('upload_error_ini_size', 'repository_upload');
                    break;
                case UPLOAD_ERR_FORM_SIZE:
                    throw new agpu_exception('upload_error_form_size', 'repository_upload');
                    break;
                case UPLOAD_ERR_PARTIAL:
                    throw new agpu_exception('upload_error_partial', 'repository_upload');
                    break;
                case UPLOAD_ERR_NO_FILE:
                    throw new agpu_exception('upload_error_no_file', 'repository_upload');
                    break;
                case UPLOAD_ERR_NO_TMP_DIR:
                    throw new agpu_exception('upload_error_no_tmp_dir', 'repository_upload');
                    break;
                case UPLOAD_ERR_CANT_WRITE:
                    throw new agpu_exception('upload_error_cant_write', 'repository_upload');
                    break;
                case UPLOAD_ERR_EXTENSION:
                    throw new agpu_exception('upload_error_extension', 'repository_upload');
                    break;
                default:
                    throw new agpu_exception('nofile');
            }
        ...
        if (!$this->check_valid_contents($_FILES[$elname]['tmp_name'])) {
            throw new agpu_exception('upload_error_invalid_file', 'repository_upload', '', $record->filename);
        }
        ...
        if ($this->mimetypes != '*') {
            ...
            if (!in_array($filemimetype, $this->mimetypes)) {
                throw new agpu_exception('invalidfiletype', 'repository', '',
        ...
        if (repository::draftfile_exists($record->itemid, $record->filepath, $record->filename)) {
            ...
            if ($overwriteexisting) {
                ...
                $event['newfile']->url = agpu_url::make_draftfile_url($record->itemid, $record->filepath,
                    $unusedfilename)->out(false);

                $event['existingfile'] = new stdClass;
                $event['existingfile']->filepath = $record->filepath;
                $event['existingfile']->filename = $existingfilename;
                $event['existingfile']->url = agpu_url::make_draftfile_url($record->itemid, $record->filepath,
        ...
        return array(
            'url' => agpu_url::make_draftfile_url($record->itemid, $record->filepath, $record->filename)->out(false),
            'id' => $record->itemid,
