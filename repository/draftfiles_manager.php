...
$filepicker_url = new agpu_url("/repository/filepicker.php", $params);
...
case 'renameform':
    ...
    echo '  <input type="submit" value="'.s(get_string('rename', 'agpu')).'" />';
    echo ' </form>';
    echo $OUTPUT->footer();
    break;
...
case 'downloaddir':
    ...
    if ($newfile = $zipper->archive_to_storage(array('/' => $file), $user_context->id, 'user', 'draft', $newdraftitemid, '/', $filename, $USER->id)) {
        $fileurl = agpu_url::make_draftfile_url($newdraftitemid, '/', $filename)->out();
        header('Location: ' . $fileurl);
    } else {
        throw new \agpu_exception('cannotdownloaddir', 'repository');
    }
    break;
...
case 'mkdirform':
    ...
    echo '  <input name="draftpath" type="hidden" value="'.s($draftpath).'" />';
    echo '  <input type="submit" value="'.s(get_string('makeafolder', 'agpu')).'" />';
    echo ' </form>';
    echo $OUTPUT->footer();
    break;
...
default:
    ...
    if ($env == 'filemanager') {
        if (!empty($subdirs)) {
            ...
            echo ' <a href="'.$home_url->out().'">'.get_string('makeafolder', 'agpu').'</a>';
        }
        if (!empty($files->list)) {
    ...
    if (!empty($files->list)) {
        ...
        foreach ($files->list as $file) {
            if ($file->type != 'folder') {
                ...
                // a file
                echo '<li>';
                echo $OUTPUT->pix_icon(file_file_icon($file), '', 'agpu', array('class' => 'iconsmall'));
