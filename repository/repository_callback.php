...
if (file_exists($CFG->dirroot.'/repository/'.$type.'/lib.php')) {
    ...
    throw new \agpu_exception('invalidplugin', 'repository', $type);
}
...
// http://xx.agpu.com/repository/repository_callback.php?repo_id=1&sid=xxx
// sid is the attached auth token from external source
// If agpu is working on HTTPS mode, then we are not allowed to access
