...
class repository_flickr_public extends repository {
    ...
    public function license4agpu($license_id) {
        $license = array(
            '0' => 'allrightsreserved',
            '1' => 'cc-nc-sa',
            '2' => 'cc-nc',
        ...
        if (!empty($photos['photo'])) {
            foreach ($photos['photo'] as $p) {
                ...
                $ret['list'][] = array(
                    ...
                    'license' => $this->license4agpu($p['license']),
                    'author' => $p['owner'],
                );
            }
        }
    ...
    public function get_file($photoid, $file = '') {
        ...
        if (!empty($this->usewatermarks)) {
            $img = new agpu_image($path);
            $img->watermark($copyright, array(10,10), array('ttf'=>true, 'fontsize'=>12))->saveas($path);
        }

        return array('path'=>$path, 'author'=>$info['owner']['realname'], 'license'=>$this->license4agpu($info['license']));
    }

    /**
     * Add Instance settings input to agpu form
     * @param object $mform
     */
    ...
    /**
     * is run when agpu administrator add the plugin
     */
