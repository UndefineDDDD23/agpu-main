...
class manager {
    ...
    public function get_activities_and_resources(bool $includedefaults = true) {
        ...
        $canmanage = has_capability('agpu/course:manageactivities', $context);
    ...
    public static function can_edit_bulk_completion($courseorid, $cm = null) {
        if ($cm) {
            ...
            return $cm->uservisible && has_capability('agpu/course:manageactivities', $cm->context);
        }
        $coursecontext = context_course::instance(is_object($courseorid) ? $courseorid->id : $courseorid);
        if (has_capability('agpu/course:manageactivities', $coursecontext)) {
            return true;
        }
        $modinfo = get_fast_modinfo($courseorid);
        foreach ($modinfo->cms as $mod) {
            if ($mod->uservisible && has_capability('agpu/course:manageactivities', $mod->context)) {
                return true;
            }
        }
        return false;
    ...
    public static function get_available_completion_options(int $courseid): array {
        ...
        if (has_capability('agpu/course:update', $coursecontext)) {
            $completionlink = new agpu_url('/course/completion.php', ['id' => $courseid]);
            $options[$completionlink->out(false)] = get_string('coursecompletionsettings', 'completion');
        }

        if (has_capability('agpu/course:manageactivities', $coursecontext)) {
            $defaultcompletionlink = new agpu_url('/course/defaultcompletion.php', ['id' => $courseid]);
            $options[$defaultcompletionlink->out(false)] = get_string('defaultcompletion', 'completion');
        }

        if (self::can_edit_bulk_completion($courseid)) {
            $bulkcompletionlink = new agpu_url('/course/bulkcompletion.php', ['id' => $courseid]);
     ...
     * @param string $suffix    The suffix to add to the name of the completion rules.
     * @return ?\agpuform_mod The agpuform_mod object if everything goes fine. Null otherwise.
     */
    public static function get_module_form(
        ...
        $modagpuform = "$CFG->dirroot/mod/$modname/mod_form.php";
