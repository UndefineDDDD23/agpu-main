...
class core_enrol_external extends external_api {
            ...
            array (
                'coursecapabilities' => new external_multiple_structure(
                    new external_single_structure(
                        array (
                            'courseid' => new external_value(PARAM_INT, 'Course ID number in the agpu course table'),
                 ...
                 'options'  => new external_multiple_structure(
                    new external_single_structure(
                        array(
                            ...
                            * groupid (integer) return only users in this group id. Requires \'agpu/site:accessallgroups\' .
                            * onlyactive (integer) only users with active enrolments. Requires \'agpu/course:enrolreview\' .
    ...
    public static function get_enrolled_users_with_capability($coursecapabilities, $options) {
        ...
        foreach ($params['coursecapabilities'] as $coursecapability) {
            ...
            if (!$coursecontext) {
                throw new agpu_exception('cannotfindcourse', 'error', '', null,
            ...
            try {
                ...
                throw new agpu_exception(get_string('errorcoursecontextnotvalid' , 'webservice', $exceptionparam));
            }
            ...
            if (!empty($groupid) && groups_is_member($groupid)) {
                require_capability('agpu/site:accessallgroups', $coursecontext);
            }
            // The course:enrolereview capability is needed to use this option.
            if ($onlyactive) {
                require_capability('agpu/course:enrolreview', $coursecontext);
            }

            // To see the permissions of others role:review capability is required.
            require_capability('agpu/role:review', $coursecontext);
    ...
    public static function get_users_courses($userid, $returnusercount = true) {
        ...
        foreach ($courses as $course) {
            ...
            if (!$canviewlastaccess) {
                $canviewlastaccess = has_capability('agpu/course:viewhiddenuserfields', $context);
            }
                ...
                'options'  => new external_multiple_structure(
                    new external_single_structure(
                        [
                            ...
                            * withcapability (string) return only users with this capability. This option requires \'agpu/role:review\' on the course context.
                            * groupid (integer) return only users in this group id. If the course has groups enabled and this param
                                                ...
                                                This option requires \'agpu/site:accessallgroups\' on the course context if the
                            ...
                            * onlyactive (integer) return only users with active enrolments and matching time restrictions.
                                                This option requires \'agpu/course:enrolreview\' on the course context.
                            ...
                            * onlysuspended (integer) return only suspended users. This option requires
                                            \'agpu/course:enrolreview\' on the course context. Please note that this option can\'t
    ...
    public static function get_enrolled_users_returns() {
        return new external_multiple_structure(
            new external_single_structure(
                [
                    'id'    => new external_value(PARAM_INT, 'ID of the user'),
                    'username'    => new external_value(PARAM_RAW, 'Username policy is defined in agpu security config', VALUE_OPTIONAL),
                    'firstname'   => new external_value(PARAM_NOTAGS, 'The first name(s) of the user', VALUE_OPTIONAL),
                    'lastname'    => new external_value(PARAM_NOTAGS, 'The family name of the user', VALUE_OPTIONAL),
                    'fullname'    => new external_value(PARAM_NOTAGS, 'The fullname of the user'),
    ...
    public static function get_course_enrolment_methods($courseid) {
        ...
        if (!core_course_category::can_view_course_info($course) && !can_access_course($course)) {
            throw new agpu_exception('coursehidden');
        }
...
class core_role_external extends external_api {
    ...
    public static function assign_roles($assignments) {
        ...
        foreach ($params['assignments'] as $assignment) {
            ...
            self::validate_context($context);
            require_capability('agpu/role:assign', $context);
