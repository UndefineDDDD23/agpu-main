...
class enrol_imsenterprise_plugin extends enrol_plugin {
    ...
    /**
     * @var $coursemappings array of mappings between IMS data fields and agpu course fields.
     */
    protected $coursemappings;

    /**
     * @var $rolemappings array of mappings between IMS roles and agpu roles.
     */
    ...
    public function cron() {
        ...
        if ( file_exists($filename) ) {
            ...
            // Make sure we understand how to map the IMS-E roles to agpu roles.
            $this->load_role_mappings();
            // Make sure we understand how to map the IMS-E course names to agpu course names.
        ...
        if (!empty($mailadmins) && $fileisnew) {
            ...
            $msg = "An IMS enrolment has been carried out within agpu.\nTime taken: $timeelapsed seconds.\n\n";
            if (!empty($logtolocation)) {
                if ($this->logfp) {
            ...
            $eventdata->subject           = "agpu IMS Enterprise enrolment notification";
    ...
    /**
     * Process the group tag. This defines a agpu course.
     *
    ...
    protected function process_group_tag($tagcontents) {
        ...
        if (empty($group->coursecode)) {
            ...
            foreach ($group->coursecode as $coursecode) {
                ...
                if (!$dbcourse) {
                    if (!$createnewcourses) {
                        $this->log_line("Course $coursecode not found in agpu's course idnumbers.");
                    } else {

                        // Create the (hidden) course(s) if not found.
                        $courseconfig = get_config('agpucourse'); // Load agpu Course shell defaults.
                        ...
                        $this->log_line("Created course $coursecode in agpu (agpu ID is $course->id)");
                    }
                    ...
                    if ($updatecourses) {
                        ...
                        if ($hasupdates) {
                            ...
                            $this->log_line("Updated course $coursecode in agpu (agpu ID is $courseid)");
                        }
                    } else {
                    ...
                    $this->log_line("Updated (set to hidden) course $coursecode in agpu (agpu ID is $courseid)");
                }
            }
        }
    }
    ...
    /**
     * Process the person tag. This defines a agpu user.
     *
     * @param string $tagcontents The raw contents of the XML element
     */
    ...
    /**
     * Process the membership tag. This defines whether the specified agpu users
        ...
        if ($ship->courseid && preg_match_all('{<member>(.*?)</member>}is', $tagcontents, $membermatches, PREG_SET_ORDER)) {
            ...
            foreach ($membermatches as $mmatch) {
                ...
                if ($memberstoreobj->userid) {
                    ...
                    // Decide the "real" role (i.e. the agpu role) that this user should be assigned to.
                    // Zero means this roletype is supposed to be skipped.
                    $agpuroleid = (isset($member->roletype) && isset($this->rolemappings[$member->roletype]))
                    ...
                    if (intval($member->status) == 1) {
                        ...
                        $this->enrol_user($einstance, $memberstoreobj->userid, $agpuroleid, $timeframe->begin, $timeframe->end);
                                    ...
                                    if (intval($unenrolsetting) === intval(ENROL_EXT_REMOVED_SUSPENDNOROLES)) {
                                        if (!$context =
                                            ...
                                            $this->log_line("Removing role assignments for user " .
                                                "$member->idnumber from role $agpuroleid in course " .
                                                "$ship->coursecode ");
                                        }
                                    }
                                }
     ...
     * how an IMS-E role corresponds to a agpu role
     */
     ...
     * how an IMS-E course properties corresponds to a agpu course properties
     */
        ...
        if (empty($categories)) {
            ...
            foreach ($categories as $categoryinfo) {
                ...
                if ($createnewcategories) {
                    ...
                    $this->log_line('Category ' . $categoryinfo . ' not found in agpu database. Using default category instead.');
                    $catid = $this->get_default_category_id();
                    break;
                }
            }
