...
class plugin_test extends \advanced_testcase {
    ...
    public function test_edit_instance_validation(): void {
        ...
        unassign_capability('agpu/course:viewhiddencourses', $teacherrole->id);
        $this->setUser($user);
        ...
        $errors = $metaplugin->edit_instance_validation($data, [], $instance, $coursecontext);
        $this->assertEquals('Sorry, but you do not currently have permissions to do that (agpu/course:viewhiddencourses).',
            $errors['customint1']);

        // Revert some changes from the last assertion to reuse the course.
        $metacourse2->visible = 1;
        $DB->update_record('course', $metacourse2);
        assign_capability('agpu/course:viewhiddencourses', CAP_ALLOW,
