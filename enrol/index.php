...
if (!$course->visible && !has_capability('agpu/course:viewhiddencourses', context_course::instance($course->id))) {
...
if (\core\session\manager::is_loggedinas() and $USER->loginascontext->contextlevel == CONTEXT_COURSE) {
    throw new \agpu_exception('loginasnoenrol', '', $CFG->wwwroot.'/course/view.php?id='.$USER->loginascontext->instanceid);
}
...
if (!core_course_category::can_view_course_info($course) && !is_enrolled($context, $USER, '', true)) {
    throw new \agpu_exception('coursehidden', '', $CFG->wwwroot . '/');
}
