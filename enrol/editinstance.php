...
if (!$plugin) {
    throw new agpu_exception('invaliddata', 'error');
}

require_login($course);

if (!has_any_capability(['enrol/' . $type . ':config', 'agpu/course:editcoursewelcomemessage'], $context)) {
    throw new \agpu_exception('nopermissiontoaccesspage', 'error');
}

$url = new agpu_url('/enrol/editinstance.php', ['courseid' => $course->id, 'id' => $instanceid, 'type' => $type]);
$PAGE->set_url($url);
$PAGE->set_pagelayout('admin');
$PAGE->set_docs_path('enrol/' . $type . '/edit');

if (empty($return)) {
    $return = new agpu_url('/enrol/instances.php', array('id' => $course->id));
}
...
if ($instanceid) {
    ...
    if ($instance->status == ENROL_INSTANCE_DISABLED) { // The instance is currently disabled.
        navigation_node::override_active_url(new agpu_url('/enrol/instances.php', ['id' => $course->id]));
        $name = $instance->name ?: get_string('pluginname', 'enrol_' . $type);
        $PAGE->navbar->add($name, $url);
    }

} else {
    require_capability('agpu/course:enrolconfig', $context);
    // No instance yet, we have to add new instance.
    navigation_node::override_active_url(new agpu_url('/enrol/instances.php', array('id' => $course->id)));
