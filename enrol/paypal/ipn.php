...
if (!enrol_is_enabled('paypal')) {
    ...
    throw new agpu_exception('errdisabled', 'enrol_paypal');
}

/// Keep out casual intruders
if (empty($_POST) or !empty($_GET)) {
    http_response_code(400);
    throw new agpu_exception('invalidrequest', 'core_error');
}
...
foreach ($_POST as $key => $value) {
    if ($key !== clean_param($key, PARAM_ALPHANUMEXT)) {
        throw new agpu_exception('invalidrequest', 'core_error', '', null, $key);
    }
    if (is_array($value)) {
        throw new agpu_exception('invalidrequest', 'core_error', '', null, 'Unexpected array param: '.$key);
    }
...
if (empty($data->custom)) {
    throw new agpu_exception('invalidrequest', 'core_error', '', null, 'Missing request param: custom');
}
...
if (empty($custom) || count($custom) < 3) {
    throw new agpu_exception('invalidrequest', 'core_error', '', null, 'Invalid value of the request param: custom');
}
...
if ($c->get_errno()) {
    throw new agpu_exception('errpaypalconnect', 'enrol_paypal', '', array('url' => $paypaladdr, 'result' => $result),
        json_encode($data));
}
...
if (strlen($result) > 0) {
    if (strcmp($result, "VERIFIED") == 0) {          // VALID PAYMENT!
        ...
        if ($users = get_users_by_capability($context, 'agpu/course:update', 'u.*', 'u.id ASC',
                                             '', '', '', '', false, true)) {
        ...
        throw new agpu_exception('erripninvalid', 'enrol_paypal', '', null, json_encode($data));
    }
}
