...
class course_enrolment_manager {
    ...
    /**
     * The current agpu_page object
     * @var agpu_page
     */
    protected $agpupage = null;
     ...
     */
    public function __construct(agpu_page $agpupage, $course, $instancefilter = null,
            $rolefilter = 0, $searchfilter = '', $groupfilter = 0, $statusfilter = -1) {
        $this->agpupage = $agpupage;
    ...
    /**
     * Returns the current agpu page
     * @return agpu_page
     */
    public function get_agpupage() {
        return $this->agpupage;
    }
    ...
    public function search_users(string $search = '', bool $searchanywhere = false, int $page = 0, int $perpage = 25,
        ...
        if ($groupmode == SEPARATEGROUPS && !has_capability('agpu/site:accessallgroups', $context)) {
    ...
    public function get_assignable_roles($otherusers = false) {
        ...
        if ($otherusers) {
            if (!is_array($this->_assignablerolesothers)) {
                ...
                list($courseviewroles, $ignored) = get_roles_with_cap_in_context($this->context, 'agpu/course:view');
    ...
    public function unassign_role_from_user($userid, $roleid) {
        ...
        if (!is_siteadmin() and !array_key_exists($roleid, $this->get_assignable_roles())) {
            if (defined('AJAX_SCRIPT')) {
                throw new agpu_exception('invalidrole');
            }
            return false;
        }
    ...
    public function assign_role_to_user($roleid, $userid) {
        require_capability('agpu/role:assign', $this->context);
    ...
    public function add_user_to_group($user, $groupid) {
        require_capability('agpu/course:managegroups', $this->context);
     ...
     * @return array
     */
    public function get_other_users_for_display(core_enrol_renderer $renderer, agpu_url $pageurl, $sort, $direction, $page, $perpage) {
    ...
    public function get_users_for_display(course_enrolment_manager $manager, $sort, $direction, $page, $perpage) {
        $pageurl = $manager->get_agpupage()->url;
        ...
        $context    = $this->get_context();
        $canmanagegroups = has_capability('agpu/course:managegroups', $context);

        $url = new agpu_url($pageurl, $this->get_url_params());
        // TODO Does not support custom user profile fields (MDL-70456).
        $extrafields = fields::get_identity_fields($context, false);
     ...
     *
     * Please note that this function does not check capability for agpu/coures:viewhiddenuserfields
     *
    ...
    private function prepare_user_for_display($user, $extrafields, $now) {
        $details = array(
            ...
            'userfullnamedisplay' => fullname($user, has_capability('agpu/site:viewfullnames', $this->get_context())),
...
class enrol_user_button extends single_button {
     ...
     */
    public function __construct(agpu_url $url, $label, $method = 'post') {
        static $count = 0;
        $count ++;
    ...
    public function require_yui_module($modules, $function, ?array $arguments = null, $galleryversion = null, $ondomready = false) {
        if ($galleryversion != null) {
            debugging('The galleryversion parameter to yui_module has been deprecated since agpu 2.3.', DEBUG_DEVELOPER);
        }
     ...
     * @param mixed $a
     */
    public function strings_for_js($identifiers, $component = 'agpu', $a = null) {
        $string = new stdClass;
     ...
     */
    public function initialise_js(agpu_page $page) {
...
class enrol_ajax_exception extends agpu_exception {
    /**
     * Constructor
...
abstract class enrol_bulk_enrolment_operation {
    ...
    /**
     * Returns a agpuform used for this operation, or false if no form is required and the action
     * should be immediatly processed.
     *
     * @param agpu_url|string $defaultaction
     * @param mixed $defaultcustomdata
     * @return enrol_bulk_enrolment_change_form|agpuform|false
     */
