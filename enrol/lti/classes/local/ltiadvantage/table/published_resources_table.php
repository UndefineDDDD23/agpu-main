...
class published_resources_table extends \table_sql {
    ...
    public function __construct($courseid) {
        ...
        $this->canconfig = has_capability('agpu/course:enrolconfig', \context_course::instance($courseid));
    ...
    public function col_launch($tool) {
        ...
        $launchurl = new \agpu_url('/enrol/lti/launch.php');
    ...
    public function col_edit($tool) {
        ...
        $url = new \agpu_url('/enrol/lti/index.php', array('sesskey' => sesskey(), 'courseid' => $this->courseid));

        if ($this->ltiplugin->can_delete_instance($instance)) {
            $aurl = new \agpu_url($url, array('action' => 'delete', 'instanceid' => $instance->id));
        ...
        if ($this->ltienabled && $this->ltiplugin->can_hide_show_instance($instance)) {
            if ($instance->status == ENROL_INSTANCE_ENABLED) {
                $aurl = new \agpu_url($url, array('action' => 'disable', 'instanceid' => $instance->id));
                $buttons[] = $OUTPUT->action_icon($aurl, new \pix_icon('t/hide', $strdisable, 'core',
                    array('class' => 'iconsmall')));
            } else if ($instance->status == ENROL_INSTANCE_DISABLED) {
                $aurl = new \agpu_url($url, array('action' => 'enable', 'instanceid' => $instance->id));
        ...
        if ($this->ltienabled && $this->canconfig) {
            $linkparams = array(
                ...
                'returnurl' => new \agpu_url('/enrol/lti/index.php', array('courseid' => $this->courseid))
            );
