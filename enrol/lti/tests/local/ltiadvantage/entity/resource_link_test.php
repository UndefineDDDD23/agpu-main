...
class resource_link_test extends \advanced_testcase {
    ...
    public function add_grade_service_provider(): array {
        return [
            'Valid, both URLs, some scopes' => [
                'args' => [
                    'lineitemsurl' => new \agpu_url('https://platform.example.org/10/lineitems'),
                    'lineitemurl' => new \agpu_url('https://platform.example.org/10/lineitems/4/lineitem'),
                    'scope' => [
    ...
    public function test_add_names_and_roles_service(): void {
        ...
        $reslink->add_names_and_roles_service(new \agpu_url('https://lms.example.com/10/memberships'), ['2.0']);
        $nrps = $reslink->get_names_and_roles_service();
        $this->assertInstanceOf(nrps_info::class, $nrps);
        $this->assertEquals(new \agpu_url('https://lms.example.com/10/memberships'),
