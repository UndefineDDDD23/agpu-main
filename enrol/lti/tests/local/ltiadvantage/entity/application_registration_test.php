...
class application_registration_test extends \advanced_testcase {
        ...
        return [
            'Valid, only required args provided' => [
                'args' => [
                    ...
                    'authrequesturl' => new \agpu_url('https://lms.example.com/auth'),
    ...
    public function test_complete_registration(): void {
        ...
        $draft->set_platformid(new \agpu_url('https://lms.example.com'));
        $draft->set_clientid('bcgd23');
        $draft->set_authenticationrequesturl(new \agpu_url('https://lms.example.com/auth'));
        $draft->set_jwksurl(new \agpu_url('https://lms.example.com/jwks'));
        $draft->set_accesstokenurl(new \agpu_url('https://lms.example.com/token'));
        $this->assertFalse($draft->is_complete());
