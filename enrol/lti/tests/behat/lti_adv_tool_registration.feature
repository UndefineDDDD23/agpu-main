...
Feature: Register a platform with the tool
  In order to share and consume a agpu resource or activity over LTI Advantage
  As an admin
  ...
  Scenario: An admin can add deployment ids for a given platform registration
    ...
    And I set the following fields to these values:
      | Deployment name | Sitewide deployment of agpu on platform x |
      | Deployment ID   | 1a2b3c                                      |
    And I press "Cancel"
    And I should not see "Sitewide deployment of agpu on platform x"
    And I should see "No tool deployments found"
    And I follow "Add a deployment"
    And I set the following fields to these values:
      | Deployment name | Sitewide deployment of agpu on platform x |
    ...
    And I should see "1a2b3c" in the "Sitewide deployment of agpu on platform x" "table_row"
    And I should not see "No tool deployments found"
    And "Delete" "link" should exist in the "Sitewide deployment of agpu on platform x" "table_row"
    And I navigate to "Plugins > Enrolments > Publish as LTI tool > Tool registration" in site administration
    ...
    And I follow "Add a deployment"
    And I set the following fields to these values:
      | Deployment name | Course context deployment of agpu on platform x |
      | Deployment ID | 4d5e6f                                              |
    And I press "Save changes"
    And I should see "4d5e6f" in the "Course context deployment of agpu on platform x" "table_row"
    And I navigate to "Plugins > Enrolments > Publish as LTI tool > Tool registration" in site administration
