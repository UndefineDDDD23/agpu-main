...
$validuris = [
    (new agpu_url('/enrol/lti/launch.php'))->out(false), // Resource link launches.
    (new agpu_url('/enrol/lti/launch_deeplink.php'))->out(false) // Deep linking launches.
];
...
// Before beginning the OIDC authentication, ensure the agpuSession cookie can be used. Browser-specific steps may need to be
...
if (!isloggedin()) {
    cookie_helper::do_cookie_check(new agpu_url('/enrol/lti/login.php', [
        'iss' => $iss,
    ...
    if (!cookie_helper::cookies_supported()) {
        ...
        $PAGE->set_url(new agpu_url('/enrol/lti/login.php'));
