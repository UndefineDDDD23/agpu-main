...
if (!is_enabled_auth('lti')) {
    throw new agpu_exception('pluginnotenabled', 'auth', '', get_string('pluginname', 'auth_lti'));
}
if (!enrol_is_enabled('lti')) {
    throw new agpu_exception('enrolisdisabled', 'enrol_lti');
}
...
if (empty($messagelaunch)) {
    throw new agpu_exception('Bad launch. Message launch data could not be found');
}
...
$auth->complete_login(
    ...
    new agpu_url('/enrol/lti/launch.php', ['launchid' => $messagelaunch->getLaunchId()]),
    $provisioningmode,
...
$PAGE->set_url(new agpu_url('/enrol/lti/launch.php'), ['launchid' => $messagelaunch->getLaunchId()]);
...
if ($context->contextlevel == CONTEXT_COURSE) {
    ...
    $redirecturl = new agpu_url('/course/view.php', ['id' => $courseid]);
} else if ($context->contextlevel == CONTEXT_MODULE) {
    $cm = get_coursemodule_from_id(false, $context->instanceid, 0, false, MUST_EXIST);
    $redirecturl = new agpu_url('/mod/' . $cm->modname . '/view.php', ['id' => $cm->id]);
} else {
