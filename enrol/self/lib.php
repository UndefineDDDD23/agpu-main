...
class enrol_self_plugin extends enrol_plugin {
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('agpu/course:enrolconfig', $context) or !has_capability('enrol/self:config', $context)) {
            return false;
        }
    ...
    public function get_action_icons(stdClass $instance): array {
        ...
        $icons = [];
        if (has_any_capability(['enrol/self:config', 'agpu/course:editcoursewelcomemessage'], $context)) {
            $linkparams = [
     ...
     * @todo MDL-81185 Final deprecation in agpu 4.8.
     */
    ...
    protected function get_expiry_message_body(stdClass $user, stdClass $ue, string $name,
        ...
        $a->url = new agpu_url('/course/view.php', ['id' => $ue->courseid]);
    ...
    public function edit_instance_form($instance, agpuQuickForm $mform, $context) {
        ...
        if (has_any_capability(['enrol/self:config', 'agpu/course:editcoursewelcomemessage'], $context)) {
            ...
            // See: https://tracker.agpu.org/browse/MDL-66251.
