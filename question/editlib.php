...
function question_build_edit_resources($edittab, $baseurl, $params,
    ...
    $thispageurl = new agpu_url($baseurl);
    ...
    if (is_null($cmid) && is_null($courseid)) {
        throw new \agpu_exception('Must provide a cmid or courseid');
    }
    ...
    if (!empty($pagevars['cat'])){
        ...
        if (!$catparts[0] || (false !== array_search($catparts[1], $contextlistarr)) ||
            ...
            throw new \agpu_exception('invalidcategory', 'question');
        }
    } else {
 ...
 * @param agpu_url $thispageurl if the value has been explicitly set, we add
 *      it to this URL.
