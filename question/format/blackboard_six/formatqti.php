...
class qformat_blackboard_six_qti extends qformat_blackboard_six_base {
    ...
    protected function readquestions($text) {
        ...
        foreach ($rawquestions as $quest) {
            ...
            // Warning : rawquestions are not agpu questions.
            $question = $this->create_raw_question($quest);
            // Third step : convert a rawquestion into a agpu question.
    ...
    /**
     * Creates a cleaner object to deal with for processing into agpu.
     * The object returned is NOT a agpu question object.
     ...
     * @param object $quest rawquestion
     * @param array $questions array of agpu questions already done
     */
    ...
    public function process_essay($quest, &$questions) {
        ...
        foreach ($quest->feedback as $feedback) {
            ...
            // instructor gives as the agpu answer for an essay question.
    ...
    public function process_matching($quest, &$questions) {
        ...
        // Blackboard matching questions can't be imported in core agpu without a loss in data,
        ...
        foreach ($subq->choices as $choiceid => $choice) {
            ...
            if ($subanswertext != '') { // Only import non empty subanswers.
                ...
                $fiber = agpu_array_keys_filter($mappings, $choiceid);
