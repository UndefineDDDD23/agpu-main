...
class qformat_default {
     ...
     * @param course object agpu course variable
     */
    ...
    protected function create_category_path($catpath, $lastcategoryinfo = null) {
        ...
        if ($this->contextfromfile && $contextid !== false) {
            ...
            require_capability('agpu/question:add', $context);
        } else {
        ...
        foreach ($catnames as $key => $catname) {
            if ($parent == 0) {
                ...
                require_capability('agpu/question:managecategory', $context);
    ...
    protected function defaultquestion() {
        ...
        $question->questiontextformat = FORMAT_agpu;
        $question->generalfeedback = '';
        $question->generalfeedbackformat = FORMAT_agpu;
     ...
     * object suitable for processing and insertion into agpu.
     *
    ...
    public function exportprocess($checkcapabilities = true) {
        ...
        if ($count==0) {
            throw new \agpu_exception('noquestions', 'question', $continuepath);
        }
    ...
    protected function get_category_path($id, $includecontext = true) {
        ...
        if (!$category = $DB->get_record('question_categories', array('id' => $id))) {
            throw new \agpu_exception('cannotfindcategory', 'error', '', $id);
        }
...
class qformat_based_on_xml extends qformat_default {
    ...
    /**
     * Return the array agpu is expecting
