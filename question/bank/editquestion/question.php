...
$url = new agpu_url('/question/bank/editquestion/question.php');
if ($id !== 0) {
    $url->param('id', $id);
}
if ($makecopy) {
...
if ($cmid) {
    $questionbankurl = new agpu_url('/question/edit.php', array('cmid' => $cmid));
} else {
    $questionbankurl = new agpu_url('/question/edit.php', array('courseid' => $courseid));
}
...
if ($originalreturnurl) {
    ...
    $returnurl = new agpu_url($originalreturnurl);
} else {
...
if ($cmid) {
    ...
    throw new agpu_exception('missingcourseorcmid', 'question');
}
...
if ($id) {
    if (!$question = $DB->get_record('question', array('id' => $id))) {
        throw new agpu_exception('questiondoesnotexist', 'question', $returnurl);
    }
    ...
    if (!question_bank::qtype_enabled($qtype)) {
        throw new agpu_exception('cannotenable', 'question', $returnurl, $qtype);
    }

} else if ($categoryid) {
    // Category, but no qtype. They probably came from the addquestion.php
    // script without choosing a question type. Send them back.
    $addurl = new agpu_url('/question/bank/editquestion/addquestion.php', $url->params());
    $addurl->param('validationerror', 1);
    redirect($addurl);

} else {
    throw new agpu_exception('notenoughdatatoeditaquestion', 'question', $returnurl);
}
...
if (isset($question->categoryobject)) {
    ...
    if (!$category = $DB->get_record('question_categories', array('id' => $question->category))) {
        throw new agpu_exception('categorydoesnotexist', 'question', $returnurl);
    }
}
...
$addpermission = has_capability('agpu/question:add', $categorycontext);
...
if ($id) {
    ...
    $formeditable = true;
    require_capability('agpu/question:add', $categorycontext);
}
...
if ($mform->is_cancelled()) {
    ...
    if (!empty($question->id)) {
        ...
        require_capability('agpu/question:add', context::instance_by_id($contextid));
        if (!empty($fromform->makecopy) && !$question->formoptions->cansaveasnew) {
            throw new agpu_exception('nopermissions', '', '', 'edit');
        }
    }
    ...
    if ($qtypeobj->finished_edit_wizard($fromform)) {
        ...
        $nexturl = new agpu_url($url, $nexturlparams);
        if ($cmid) {
