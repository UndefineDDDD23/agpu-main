...
if (!$category = $DB->get_record("question_categories", ['id' => $catid])) {
    throw new agpu_exception('nocategory', 'question');
}
...
if ($form = $importform->get_data()) {
    ...
    if (!is_readable($formatfile)) {
        throw new agpu_exception('formatnotfound', 'question', '', $form->format);
    }
    ...
    if (!$qformat->importpreprocess()) {
        throw new agpu_exception('cannotimport', '', $thispageurl->out());
    }
    ...
    echo $OUTPUT->continue_button(new agpu_url('/question/edit.php', $params));
    echo $OUTPUT->footer();
    exit;
}
