...
$url = new agpu_url('/question/bank/bulkmove/move.php');
...
if ($category) {
    ...
    if (! $tocategory = $DB->get_record('question_categories',
        ...
        throw new \agpu_exception('cannotfindcate', 'question');
    }
}
...
if ($movequestionselected && $confirm && confirm_sesskey()) {
    ...
    redirect(new agpu_url($returnurl, ['filter' => $returnfilters]));
}
...
if ($moveselected) {
    ...
    $moveurl = new \agpu_url($url, $moveparam);

    $addcontexts = $contexts->having_cap('agpu/question:add');
