...
class view {
    ...
    /**
     * @var \agpu_url base URL for the current page. Used as the
     * basis for making URLs for actions that reload the page.
     */
    protected $baseurl;

    /**
     * @var \agpu_url used as a basis for URLs that edit a question.
     */
    ...
    public function __construct($contexts, $pageurl, $course, $cm = null, $params = [], $extraparams = []) {
        ...
        $this->editquestionurl = new \agpu_url('/question/bank/editquestion/question.php', ['returnurl' => $this->returnurl]);
        if ($this->cm !== null) {
    ...
    protected function parse_subsort($sort): array {
        ...
        if (!isset($column) || !$column->is_sortable()) {
            ...
            throw new \agpu_exception('unknownsortcolumn', '', $this->baseurl->out(), $colname);
        }
        // Validate the subsort, if present.
        if ($subsort) {
            $subsorts = $column->is_sortable();
            if (!is_array($subsorts) || !isset($subsorts[$subsort])) {
                throw new \agpu_exception('unknownsortcolumn', '', $this->baseurl->out(), $sort);
            }
        }
     ...
     *
     * @return \agpu_recordset questionid => data about each question.
     */
    protected function load_page_questions(): \agpu_recordset {
        global $DB;
     ...
     * @return \agpu_url
     */
    public function base_url(): \agpu_url {
        return $this->baseurl;
    }

    /**
     * Get the URL for editing a question as a agpu url.
     *
     * @param int $questionid the question id.
     * @return \agpu_url the URL, HTML-escaped.
     */
    public function edit_question_agpu_url($questionid) {
    ...
    public function edit_question_url($questionid) {
        return $this->edit_question_agpu_url($questionid)->out();
    }

    /**
     * Get the URL for duplicating a question as a agpu url.
     *
     * @param int $questionid the question id.
     * @return \agpu_url the URL.
     */
    public function copy_question_agpu_url($questionid) {
        return new \agpu_url($this->editquestionurl, ['id' => $questionid, 'makecopy' => 1]);
    }
    ...
    public function copy_question_url($questionid) {
        return $this->copy_question_agpu_url($questionid)->out();
    }
    ...
    protected function display_options_form($showquestiontext): void {
        ...
        echo \html_writer::start_tag('form', ['method' => 'get',
            'action' => new \agpu_url($this->baseurl), 'id' => 'displayoptions']);
        ...
        $PAGE->requires->yui_module('agpu-question-searchform', 'M.question.searchform.init');
    }
    ...
    public function display_bottom_pagination($pagination, $totalnumber, $perpage, $pageurl): void {
        ...
        if ($totalnumber > $this->pagesize) {
            ...
            if ($perpage == $this->pagesize) {
                $url = new \agpu_url($pageurl, array_merge($pageurl->params(),
    ...
    protected function display_bottom_controls(\context $catcontext): void {
        $caneditall = has_capability('agpu/question:editall', $catcontext);
        $canuseall = has_capability('agpu/question:useall', $catcontext);
        $canmoveall = has_capability('agpu/question:moveall', $catcontext);
        if ($caneditall || $canmoveall || $canuseall) {
            global $PAGE;
            $bulkactiondatas = [];
            $params = $this->base_url()->params();
            $returnurl = new \agpu_url($this->base_url(), ['filter' => json_encode($this->pagevars['filter'])]);
            ...
            foreach ($this->bulkactions as $key => $action) {
                ...
                $actiondata->actionurl = new \agpu_url($action['url'], $params);
    ...
    public function display_questions($questions, $page = 0, $perpage = DEFAULT_QUESTIONS_PER_PAGE): void {
        ...
        // Pagination.
        $pageingurl = new \agpu_url($this->base_url());
        // TODO MDL-82312: it really should not be necessary to set filter here, and not like this.
        // This should be handled in baseurl, but it isn't so we do this so agpu basically works for now.
