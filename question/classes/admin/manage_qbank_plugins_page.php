...
class manage_qbank_plugins_page extends \admin_setting {
    ...
    public function output_html($data, $query = ''): string {
        ...
        foreach ($types as $type) {
            $url = new \agpu_url('/admin/qbankplugins.php', ['sesskey' => sesskey(), 'name' => $type->name]);
            ...
            if ($type->is_enabled()) {
                $hideshow = \html_writer::link($url->out(false, ['action' => 'disable']),
                        $OUTPUT->pix_icon('t/hide', $txt->disable, 'agpu', ['class' => 'iconsmall']));
            } else {
                $class = 'dimmed_text';
                $hideshow = \html_writer::link($url->out(false, ['action' => 'enable']),
                        $OUTPUT->pix_icon('t/show', $txt->enable, 'agpu', ['class' => 'iconsmall']));
            }
