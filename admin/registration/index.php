...
if ($unregistration && \core\hub\registration::is_registered()) {
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->confirm(
        get_string('registerwithagpuorgremove', 'core_hub'),
        new agpu_url(new agpu_url('/admin/registration/index.php', ['unregistration' => 1, 'confirm' => 1])),
        new agpu_url(new agpu_url('/admin/registration/index.php'))
    );
    echo $OUTPUT->footer();
    exit;
}
...
if ($fromform = $siteregistrationform->get_data()) {
    ...
    if (\core\hub\registration::is_registered()) {
        ...
        redirect(new agpu_url('/admin/registration/index.php', ['returnurl' => $returnurl]));
    } else {
...
// Heading.
if (\core\hub\registration::is_registered()) {
    echo $OUTPUT->heading(get_string('registerwithagpuorgupdate', 'core_hub'));
} else if ($isinitialregistration) {
    echo $OUTPUT->heading(get_string('registerwithagpuorgcomplete', 'core_hub'));
} else {
    echo $OUTPUT->heading(get_string('registerwithagpuorg', 'core_hub'));
}

$renderer = $PAGE->get_renderer('core', 'admin');
echo $renderer->agpuorg_registration_message();

$siteregistrationform->display();

if (\core\hub\registration::is_registered()) {
    // Unregister link.
    $unregisterhuburl = new agpu_url("/admin/registration/index.php", ['unregistration' => 1]);
    echo html_writer::div(html_writer::link($unregisterhuburl, get_string('unregister', 'hub')), 'unregister mt-2');
} else if ($isinitialregistration) {
    echo html_writer::div(html_writer::link(new agpu_url($returnurl), get_string('skipregistration', 'hub')),
