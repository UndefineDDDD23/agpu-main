...
$baseurl = new agpu_url('/admin/repository.php');
...
if (($action == 'edit') || ($action == 'new')) {
    ...
    if ($mform->is_cancelled()){
        ...
        if ($success) {
            ...
            throw new \agpu_exception('instancenotsaved', 'repository', $baseurl);
        }
        exit;
    } else {
        echo $OUTPUT->header();
    ...
    if ($sure) {
        ...
        if ($repositorytype->delete($downloadcontents)) {
            ...
            throw new \agpu_exception('instancenotdeleted', 'repository', $baseurl);
        }
        exit;
    } else {
        echo $OUTPUT->header();
        ...
        $removeurl = new agpu_url($baseurl, [
            'action' =>'delete',
            'repos' => $repository,
            'sure' => 'yes',
        ]);

        $removeanddownloadurl = new agpu_url($removeurl, [
            'downloadcontents' => 1,
        ]);
    ...
    if (!empty($repositorytypes)) {
        ...
        foreach ($repositorytypes as $i) {
            ...
            if (!empty($typeoptionnames) || !empty($instanceoptionnames)) {
                ...
                $settings = html_writer::link(new agpu_url($baseurl, ['action' => 'edit', 'repos' => $typename]), $settingsstr);
            ...
            // Active toggle.
            $selectaction = new agpu_url($baseurl, ['sesskey' => sesskey(), 'repos' => $typename]);
            ...
            if ($updowncount > 1) {
                $moveupaction = new agpu_url($baseurl, [
                    'sesskey' => sesskey(),
                    'action' => 'moveup',
                    'repos' => $typename,
                ]);
            ...
            if ($updowncount < $totalrepositorytypes) {
                $movedownaction = new agpu_url($baseurl, [
                    'sesskey' => sesskey(),
                    'action' => 'movedown',
                    'repos' => $typename,
                ]);
    ...
    if (!empty($plugins)) {
        foreach ($plugins as $plugin => $dir) {
            ...
            if (!in_array($plugin, $alreadyplugins)) {
                $selectaction = new agpu_url($baseurl, ['sesskey' => sesskey(), 'repos' => $plugin]);
