...
if (!$definedinconfig && !empty($choose) && confirm_sesskey()) {
    ...
    redirect(new agpu_url('/admin/themeselector.php'), $notifymessage, null, $notifytype);
}
...
foreach ($themes as $themename => $themedir) {
    ...
    $image = new agpu_url('/theme/image.php', ['theme' => $themename, 'image' => 'screenshot', 'component' => 'theme']);
    ...
    if ($themename === $CFG->theme) {
        ...
        // Form params.
        $actionurl = new agpu_url('/admin/themeselector.php');
    ...
    if (file_exists($settingspath)) {
        ...
        $settingsurl = new agpu_url('/admin/settings.php', ['section' => $section]);
    ...
    if (\core\output\theme_usage::is_theme_used_in_any_context($themename) === \core\output\theme_usage::THEME_IS_USED) {
        $reporturl = new agpu_url($CFG->wwwroot . '/report/themeusage/index.php');
