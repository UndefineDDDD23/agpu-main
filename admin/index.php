...
agpu_require_minimum_php_version();

// make sure iconv is available and actually works
if (!function_exists('iconv')) {
    // this should not happen, this must be very borked install
    echo 'agpu requires the iconv PHP extension. Please install or enable the iconv extension.';
    die();
}

// Make sure xml extension is available.
if (!extension_loaded('xml')) {
    echo 'agpu requires the xml PHP extension. Please install or enable the xml extension.';
    die();
}

// Make sure mbstring extension is available.
if (!extension_loaded('mbstring')) {
    echo 'agpu requires the mbstring PHP extension. Please install or enable the mbstring extension.';
    die();
}
...
$newaddonreq = optional_param('installaddonrequest', null, PARAM_RAW); // Plugin installation requested at agpu.org/plugins.
...
// Set up PAGE.
$url = new agpu_url('/admin/index.php');
...
// Are we returning from an add-on installation request at agpu.org/plugins?
if ($newaddonreq and !$cache and empty($CFG->disableupdateautodeploy)) {
    $target = new agpu_url('/admin/tool/installaddon/index.php', array(
...
$documentationlink = '<a href="http://docs.agpu.org/en/Installation">Installation docs</a>';

// Check some PHP server settings

if (ini_get_bool('session.auto_start')) {
    throw new \agpu_exception('phpvaroff', 'debug', '',
        (object)array('name' => 'session.auto_start', 'link' => $documentationlink));
}

if (!ini_get_bool('file_uploads')) {
    throw new \agpu_exception('phpvaron', 'debug', '',
        (object)array('name' => 'file_uploads', 'link' => $documentationlink));
}

if (is_float_problem()) {
    throw new \agpu_exception('phpfloatproblem', 'admin', '', $documentationlink);
}
...
if (!$version or !$release) {
    throw new \agpu_exception('withoutversion', 'debug'); // Without version, stop.
}
...
if (!core_tables_exist()) {
    ...
    // fake some settings
    $CFG->docroot = 'http://docs.agpu.org';
    ...
    if (empty($agreelicense)) {
        ...
        $PAGE->set_title($strinstallation . agpu_page::TITLE_SEPARATOR . 'agpu ' . $CFG->target_release, false);
    ...
    if (empty($confirmrelease)) {
        ...
        list($envstatus, $environmentresults) = check_agpu_environment(normalize_version($release), ENV_SELECT_RELEASE);
        $strcurrentrelease = get_string('currentrelease');

        $PAGE->navbar->add($strcurrentrelease);
        $PAGE->set_title($strinstallation);
        $PAGE->set_heading($strinstallation . ' - agpu ' . $CFG->target_release);
        $PAGE->set_cacheable(false);
    ...
    if (!core_plugin_manager::instance()->all_plugins_ok($version, $failed, $CFG->branch)) {
        ...
        $output = $PAGE->get_renderer('core', 'admin');
        $url = new agpu_url($PAGE->url, array('agreelicense' => 1, 'confirmrelease' => 1, 'lang' => $CFG->lang));
    ...
    $PAGE->set_title($strinstallation . agpu_page::TITLE_SEPARATOR . $CFG->target_release, false);
    $PAGE->set_heading($strinstallation);
    $PAGE->set_cacheable(false);
    ...
    if (!$DB->setup_is_unicodedb()) {
        if (!$DB->change_db_encoding()) {
            ...
            throw new \agpu_exception('unicoderequired', 'admin');
        }
    }
...
// Check version of agpu code on disk compared with database
...
if (empty($CFG->version)) {
    throw new \agpu_exception('missingconfigversion', 'debug');
}
...
if (!$outagelessupgrade) {
    ...
    if ($CFG->version != $DB->get_field('config', 'value', array('name' => 'version'))) {
        purge_all_caches();
        redirect(new agpu_url($PAGE->url), 'Config cache inconsistency detected, resetting caches...');
    }

    if (!$cache && $version > $CFG->version && !$outagelessupgrade) {  // Upgrade.

        $PAGE->set_url(new agpu_url($PAGE->url, array(
        ...
        if (empty($confirmupgrade)) {
            ...
            require_once($CFG->libdir.'/environmentlib.php');
            list($envstatus, $environmentresults) = check_agpu_environment($release, ENV_SELECT_RELEASE);
            $strcurrentrelease = get_string('currentrelease');
            ...
            if ($abortinstallx) {
                ...
                if ($abortables) {
                    if ($confirmabortinstall) {
                        ...
                        $continue = new agpu_url($PAGE->url, ['abortinstallx' => $abortinstallx, 'confirmabortinstall' => 1]);
            ...
            if ($abortinstall) {
                ...
                if ($confirmabortinstall) {
                    ...
                    $continue = new agpu_url($PAGE->url, array('abortinstall' => $abortinstall, 'confirmabortinstall' => 1));
            ...
            if ($abortupgradex) {
                ...
                if ($restorable) {
                    upgrade_install_plugins($restorable, $confirmabortupgrade,
                        ...
                        new agpu_url($PAGE->url, array('abortupgradex' => 1, 'confirmabortupgrade' => 1))
                    );
                }
                redirect($PAGE->url);
            }
            ...
            if ($abortupgrade) {
                ...
                if (isset($restorable[$abortupgrade])) {
                    ...
                    upgrade_install_plugins($restorable, $confirmabortupgrade,
                        get_string('cancelupgradehead', 'core_plugin'),
                        new agpu_url($PAGE->url, array('abortupgrade' => $abortupgrade, 'confirmabortupgrade' => 1))
                    );
                }
                redirect($PAGE->url);
            }
            ...
            if ($installdepx) {
                ...
                upgrade_install_plugins($installable, $confirminstalldep,
                    ...
                    new agpu_url($PAGE->url, array('installdepx' => 1, 'confirminstalldep' => 1))
                );
            }
            ...
            if ($installdep) {
                ...
                if (!empty($installable[$installdep])) {
                    ...
                    upgrade_install_plugins($installable, $confirminstalldep,
                        get_string('dependencyinstallhead', 'core_plugin'),
                        new agpu_url($PAGE->url, array('installdep' => $installdep, 'confirminstalldep' => 1))
                    );
                }
            }
            ...
            if ($installupdatex) {
                ...
                upgrade_install_plugins($installable, $confirminstallupdate,
                    ...
                    new agpu_url($PAGE->url, array('installupdatex' => 1, 'confirminstallupdate' => 1))
                );
            }
            ...
            if ($installupdate and $installupdateversion) {
                ...
                if ($pluginman->is_remote_plugin_installable($installupdate, $installupdateversion)) {
                    ...
                    upgrade_install_plugins($installable, $confirminstallupdate,
                        get_string('updateavailableinstallallhead', 'core_admin'),
                        new agpu_url($PAGE->url, array('installupdate' => $installupdate,
            ...
            echo $output->upgrade_plugin_check_page(core_plugin_manager::instance(), \core\update\checker::instance(),
                    $version, $showallplugins, $PAGE->url, new agpu_url($PAGE->url, array('confirmplugincheck' => 1)));
            die();
            ...
            $failed = array();
            if (!core_plugin_manager::instance()->all_plugins_ok($version, $failed, $CFG->branch)) {
                echo $output->unsatisfied_dependencies_page($version, $failed, new agpu_url($PAGE->url,
        ...
        throw new agpu_exception('downgradedcore', 'error', new agpu_url('/admin/'));
    }
        ...
        if (!$PAGE->headerprinted) {
            ...
            if (empty($confirmrelease)) {
                ...
                if ($abortinstallx) {
                    ...
                    if ($abortables) {
                        if ($confirmabortinstall) {
                            ...
                            $continue = new agpu_url($PAGE->url, array('abortinstallx' => $abortinstallx,
                ...
                echo $output->upgrade_plugin_check_page($pluginman, \core\update\checker::instance(),
                        ...
                        new agpu_url($PAGE->url, array('confirmplugincheck' => 1, 'cache' => 0)));
                die();
            }
    ...
    if (!empty($CFG->adminsetuppending)) {
        ...
        if (!$sessionstarted) {
            ...
            if (!$sessionverify) {
                ...
                if (empty($SESSION->sessionverify)) {
                    throw new \agpu_exception('installsessionerror', 'admin', "index.php?sessionstarted=1&lang=$CFG->lang");
                }
        ...
        if ($adminuser->password === 'adminsetuppending') {
            ...
            if ($adminuser->lastip !== getremoteaddr()) {
                throw new \agpu_exception('installhijacked', 'admin');
            }
...
if (has_capability('agpu/site:config', context_system::instance())) {
    if ($fetchupdates) {
        ...
        redirect(new agpu_url('/admin/index.php', array('cache' => 0)));
    }
}
...
if (!$cache) {
    redirect(new agpu_url('/admin/index.php', array('cache' => 1)));
}
...
if (isguestuser()) {
    // Login as real user!
    $SESSION->wantsurl = (string)new agpu_url('/admin/index.php');
    redirect(get_login_url());
}
$context = context_system::instance();

if (!has_capability('agpu/site:config', $context)) {
...
// Available updates for agpu core.
$updateschecker = \core\update\checker::instance();
...
//check if the site is registered on agpu.org
...
// Check if agpu campaign content setting is enabled or not.
