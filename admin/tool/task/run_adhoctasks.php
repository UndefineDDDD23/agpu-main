...
if ($taskid) {
    ...
    if (!$record) {
        throw new \agpu_exception('invalidtaskid');
    }
    ...
    if (!$classname) {
        throw new \agpu_exception('noclassname', 'tool_task');
    }
...
if (!$confirmed) {
    echo $OUTPUT->confirm(get_string('runadhoc_confirm', 'tool_task'),
            new single_button(new agpu_url($runurl, array_merge($params, ['confirm' => 1])),
            get_string('runadhoc', 'tool_task')),
            new single_button(new agpu_url($tasksurl, $params),
...
if ($repeat) {
    echo html_writer::div(
        $OUTPUT->single_button(
            new agpu_url($runurl, array_merge($params, ['confirm' => 1])),
...
echo html_writer::div(
    html_writer::link(
        new agpu_url($tasksurl, $taskid ? ['classname' => $classname] : []),
