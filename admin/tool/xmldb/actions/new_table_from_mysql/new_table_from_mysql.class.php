 ...
 * mysql table present in the agpu DB to one xmldb_table structure
 *
 * @package    tool_xmldb
 * @copyright  2003 onwards Eloy Lafuente (stronk7) {@link http://stronk7.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class new_table_from_mysql extends XMLDBAction {
    ...
    function init() {
        ...
        if ($DB->get_dbfamily() !== 'mysql') {
            throw new agpu_exception('DB family not supported');
        }
    ...
    function invoke() {
        ...
        // go to the table edit action
            ...
            if (empty($tableparam) || empty($afterparam)) {
                throw new agpu_exception('Invalid param value detected.');
            }
