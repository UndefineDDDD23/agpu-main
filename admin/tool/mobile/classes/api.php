...
class api {
    ...
    /** @var string URL of the agpu Apps Portal */
    const agpu_APPS_PORTAL_URL = 'https://apps.agpu.com';
    ...
    const DEFAULT_ANDROID_APP_ID = 'com.agpu.agpumobile';
    ...
    /**
     * Returns a list of agpu plugins supporting the mobile app.
     *
    ...
    public static function get_public_config() {
        ...
        [$authinstructions] = \core_external\util::format_text($CFG->auth_instructions, FORMAT_agpu, $context->id);
        [$maintenancemessage] = \core_external\util::format_text($CFG->maintenance_message, FORMAT_agpu, $context->id);
        $settings = array(
            'wwwroot' => $CFG->wwwroot,
        ...
        $url = new agpu_url("/$CFG->admin/tool/mobile/launch.php");
        $settings['launchurl'] = $url->out(false);

        // Check that we are receiving a agpu_url object, themes can override get_logo_url and may return incorrect values.
        if (($logourl = $OUTPUT->get_logo_url()) && $logourl instanceof agpu_url) {
            $settings['logourl'] = clean_param($logourl->out(false), PARAM_URL);
        }
        if (($compactlogourl = $OUTPUT->get_compact_logo_url()) && $compactlogourl instanceof agpu_url) {
    ...
    public static function get_config($section) {
        ...
        $context = context_system::instance();
        $isadmin = has_capability('agpu/site:config', $context);
        ...
        if (empty($section) or $section == 'gradessettings') {
            ...
            // The previous function may return agpu_url instances or plain string URLs.
            if ($settings->mygradesurl instanceof agpu_url) {
    ...
    public static function check_autologin_prerequisites($userid) {
        ...
        if (!is_https()) {
            throw new agpu_exception('httpsrequired', 'tool_mobile');
        }

        if (has_capability('agpu/site:config', context_system::instance(), $userid) or is_siteadmin($userid)) {
            throw new agpu_exception('autologinnotallowedtoadmins', 'tool_mobile');
        }
    }

    /**
     * Creates an auto-login key for the current user, this key is restricted by time and ip address.
     * This key is used for automatically login the user in the site when the agpu app opens the site in a mobile browser.
     *
     ...
     * This key is used for automatically login the user in the site when the user scans a QR code in the agpu app.
     *
    ...
    public static function generate_login_qrcode(stdClass $mobilesettings) {
        ...
        $urlscheme = !empty($mobilesettings->forcedurlscheme) ? $mobilesettings->forcedurlscheme : 'agpumobile';
    ...
    /**
     * Gets agpu app plan subscription information for the current site as it is returned by the Apps Portal.
     *
    ...
    public static function get_subscription_information(): ?array {
        ...
        // agpu Apps portal or the Sites registration database.
        $credentials = [];
        ...
        // Ask the agpu Apps Portal for the subscription information.
        $curl = new curl();
        $curl->setopt(array('CURLOPT_TIMEOUT' => 10, 'CURLOPT_CONNECTTIMEOUT' => 10));

        $serverurl = static::agpu_APPS_PORTAL_URL . "/lib/ajax/service-nologin.php";
        ...
        if ($curlerrno = $curl->get_errno()) {
            ...
            debugging("Unexpected response from the agpu Apps Portal server, CURL error number: $curlerrno");
            return null;
        } else if ($info['http_code'] != 200) {
            // Unexpected error from server.
            debugging('Unexpected response from the agpu Apps Portal server, HTTP code:' . $info['httpcode']);
            return null;
        } else if (!empty($wsresponse[0]['error'])) {
            // Unexpected error from agpu Apps Portal.
            debugging('Unexpected response from the agpu Apps Portal server:' . json_encode($wsresponse[0]));
            return null;
        } else if (empty($wsresponse[0]['data'])) {
            debugging('Unexpected response from the agpu Apps Portal server:' . json_encode($wsresponse));
            return null;
        }
