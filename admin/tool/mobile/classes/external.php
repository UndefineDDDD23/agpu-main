...
class external extends external_api {
     ...
     *
     * Please note that it only works if the request comes from the agpu mobile or desktop app.
     *
    ...
    public static function get_autologin_key($privatetoken) {
        ...
        // Only requests from the agpu mobile or desktop app. This enhances security to avoid any type of XSS attack.
        ...
        if (!\core_useragent::is_agpu_app()) {
            throw new agpu_exception('apprequired', 'tool_mobile');
        }
        api::check_autologin_prerequisites($USER->id);

        if (isset($_GET['privatetoken']) or empty($privatetoken)) {
            throw new agpu_exception('invalidprivatetoken', 'tool_mobile');
        }
        ...
        if ($timenow - $last < $mintimereq) {
            ...
            throw new agpu_exception('autologinkeygenerationlockout', 'tool_mobile', '', $minutes);
        }
        ...
        $autologinurl = new agpu_url("/$CFG->admin/tool/mobile/autologin.php");
    ...
    public static function get_tokens_for_qr_login($qrloginkey, $userid) {
        ...
        if ($qrcodetype != api::QR_CODE_LOGIN) {
            throw new agpu_exception('qrcodedisabled', 'tool_mobile');
        }
        ...
        $service = $DB->get_record('external_services', ['shortname' => agpu_OFFICIAL_MOBILE_SERVICE, 'enabled' => 1]);
        if (empty($service)) {
