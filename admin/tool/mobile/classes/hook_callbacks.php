...
class hook_callbacks {
    ...
    public static function before_standard_footer_html_generation(
        ...
        $hook->add_html(
            html_writer::div(
                html_writer::link($url, get_string('getagpuonyourmobile', 'tool_mobile'), ['class' => 'mobilelink']),
            ),
        );
    }
    ...
    public static function after_login_completed(
        ...
        if (!NO_agpu_COOKIES && !empty($_COOKIE['tool_mobile_launch'])) {
            if (empty($SESSION->wantsurl) || strpos($SESSION->wantsurl, '/tool/mobile/launch.php') === false) {
                ...
                $SESSION->wantsurl = (new \agpu_url("/$CFG->admin/tool/mobile/launch.php", $params))->out(false);
            }
        }
