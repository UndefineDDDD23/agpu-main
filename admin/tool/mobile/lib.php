...
/**
 * Generate the app download url to promote agpu mobile.
 *
 * @return agpu_url|void App download agpu_url object or return if setuplink is not set.
 */
function tool_mobile_create_app_download_url() {
    ...
    $downloadurl = new agpu_url($mobilesettings->setuplink);

    // Do not update the URL if it is a custom one (we may break it completely).
    if ($mobilesettings->setuplink != 'https://download.agpu.org/mobile') {
        return $downloadurl;
    }
...
function tool_mobile_get_token($userid) {
    ...
    $sql = "SELECT t.*
             ...
             WHERE t.externalserviceid = s.id
               AND s.enabled = 1
               AND s.shortname IN ('agpu_mobile_app', 'local_mobile')
               AND t.userid = ?";
 ...
 *
 * Used add a section about the agpu mobile app.
 *
        ...
        if ($usertoken) {
            ...
            if ($usertoken->lastaccess) {
                ...
                if ($iscurrentuser && $validtoken) {
                    $url = new agpu_url('/'.$CFG->admin.'/tool/mobile/logout.php', ['sesskey' => sesskey()]);
