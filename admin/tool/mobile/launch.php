...
$urlscheme         = optional_param('urlscheme', 'agpumobile', PARAM_NOTAGS); // The URL scheme the app supports.
...
if (!preg_match('/^[a-zA-Z][a-zA-Z0-9-\+\.]*$/', $urlscheme)) {
    throw new agpu_exception('Invalid parameter: the value of urlscheme isn\'t valid. ' .
...
if (!empty($oauthsso) && is_enabled_auth('oauth2')) {
    $wantsurl = new agpu_url('/admin/tool/mobile/launch.php',
        array('service' => $serviceshortname, 'passport' => $passport, 'urlscheme' => $urlscheme, 'confirmed' => $confirmed));
    $oauthurl = new agpu_url('/auth/oauth2/login.php',
...
if (empty($SESSION->justloggedin) &&
    ...
    throw new agpu_exception('pluginnotenabledorconfigured', 'tool_mobile');
}
...
if (\core_useragent::is_agpu_app()) {
    \core\session\utility\cookie_helper::add_attributes_to_cookie_response_header(
        cookiename: "agpuSession{$CFG->sessioncookie}",
...
$siteadmin = has_capability('agpu/site:config', context_system::instance(), $USER->id);
...
if ($confirmed or $isios) {
    ...
    if ($confirmed) {
        ...
        echo $OUTPUT->single_button(new agpu_url('/course/'), get_string('courses'));
        echo $OUTPUT->box_end();
    }
