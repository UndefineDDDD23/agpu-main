...
/**
 * Parser of agpu strings defined as associative array.
 *
 * agpu core just includes this file format directly as normal PHP code. However
...
class phpparser {
    ...
    /**
     * Parses the given data in agpu PHP string format
     *
        ...
        foreach ($tokens as $token) {
            ...
            if ($expect == 'LEFT_BRACKET') {
                if ($foundtype === 'char' and $founddata === '[') {
                    ...
                    throw new agpu_exception('Parsing error. Expected character [ at line '.$line);
                }
            }
            ...
            if ($expect == 'STRING_ID') {
                if ($foundtype === T_CONSTANT_ENCAPSED_STRING) {
                    ...
                    throw new agpu_exception('Parsing error. Expected T_CONSTANT_ENCAPSED_STRING array key at line '.$line);
                }
            }
            ...
            if ($expect == 'SEMICOLON') {
                if (is_null($id) or is_null($text)) {
                    throw new agpu_exception('Parsing error. NULL string id or value at line '.$line);
                }
                if ($foundtype === 'char' and $founddata === ';') {
                    if (!empty($id)) {
                        $strings[$id] = $text;
    ...
    protected function decapsulate(string $text): string {
        ...
        if (strlen($text) < 2) {
            throw new agpu_exception('Parsing error. Expected T_CONSTANT_ENCAPSED_STRING in decapsulate()');
        }
