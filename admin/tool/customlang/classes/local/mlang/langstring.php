...
class langstring {
     ...
     * The agpu 1.x part is keep on puspose to make it easier the copy paste from both codes.
     ...
     * - for get_string() in agpu 1.6 - 1.9 use $format == 1
     * - for get_string() in agpu 2.0 and higher use $format == 2
     *
    ...
    public static function fix_syntax(string $text, int $format = 2, ?int $from = null): string {
        ...
        if (($format === 2) && ($from === 2)) {
            ...
            throw new agpu_exception('Unknown get_string() format version');
        }
        return $clean;
    }
}
