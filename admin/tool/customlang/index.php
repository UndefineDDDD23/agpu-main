...
if ($action === 'checkout') {
    ...
    echo $output->continue_button(new agpu_url("/admin/tool/customlang/{$next}.php", array('lng' => $lng)), 'get');
    echo $output->footer();
    ...
    if (!$confirm) {
        ...
        if ($numofmodified != 0) {
            ...
            echo $output->confirm(get_string('confirmcheckin', 'tool_customlang'),
                                  new agpu_url($PAGE->url, array('action'=>'checkin', 'lng'=>$lng, 'confirm'=>1)),
                                  new agpu_url($PAGE->url, array('lng'=>$lng)));
        } else {
            echo $output->heading(get_string('modifiedno', 'tool_customlang', $numofmodified), 3);
            echo $output->continue_button(new agpu_url($PAGE->url, array('lng' => $lng)));
        }
        echo $output->footer();
        die();
...
if (has_capability('tool/customlang:edit', context_system::instance())) {
    $menu['checkout'] = array(
        ...
        'url'       => new agpu_url($PAGE->url, array('action' => 'checkout', 'lng' => $lng)),
        'method'    => 'post',
    );
    if ($numofmodified != 0) {
        $menu['checkin'] = array(
            'title'     => get_string('checkin', 'tool_customlang'),
            'url'       => new agpu_url($PAGE->url, array('action' => 'checkin', 'lng' => $lng)),
            'method'    => 'post',
        );
    }
    $menu['import'] = array(
        'title'     => get_string('import', 'tool_customlang'),
        'url'       => new agpu_url($PAGE->url, ['action' => 'checkout', 'lng' => $lng, 'next' => 'import']),
        'method'    => 'post',
    );
}
if (has_capability('tool/customlang:export', context_system::instance())) {
    ...
    if (check_dir_exists(dirname($langdir)) && count(glob("$langdir/*"))) {
        $menu['export'] = [
            ...
            'url'       => new agpu_url("/admin/tool/customlang/export.php", ['lng' => $lng]),
            'method'    => 'post',
        ];
    }
}
