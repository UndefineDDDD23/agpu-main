 ...
 * plugin & agpu 3.9 functions. The 3.9 Functions have a base understanding that certain
 ...
 *
 * @module     tool_agpunet/instance_form
 * @copyright  2020 Mathew May <mathew.solutions>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

define(['tool_agpunet/validator',
        'tool_agpunet/selectors',
        'core/loadingicon',
        'core/templates',
        'core/notification',
        'jquery'],
    ...
    /**
     * Given a user wishes to see the agpuNet profile url form transition them there.
     *
     * @method chooserNavigateToMnet
     * @param {HTMLElement} showagpuNet The chooser's area for ment
     ...
     */
    var chooserNavigateToMnet = function(showagpuNet, footerData, carousel, modal) {
        showagpuNet.innerHTML = '';

        // Add a spinner.
        var spinnerPromise = LoadingIcon.addIconToContainer(showagpuNet);
        ...
        $.when(
            ...
            transitionPromise
                ...
                Templates.replaceNodeContents(showagpuNet, footerData.customcarouseltemplate, '');
                return;
        }).catch(Notification.exception);

        // We apply our handlers in here to minimise plugin dependency in the Chooser.
        registerListenerEvents(showagpuNet);
        ...
        carousel.carousel(2);
        modal.setFooter(Templates.render('tool_agpunet/chooser_footer_close_mnet', {}));
    };

    /**
     * Given a user no longer wishes to see the agpuNet profile url form transition them from there.
     *
    ...
    var footerClickListener = function(e, footerData, modal) {
        if (e.target.matches(Selectors.action.showagpuNet) || e.target.closest(Selectors.action.showagpuNet)) {
            e.preventDefault();
            const carousel = $(modal.getBody()[0].querySelector(Selectors.region.carousel));
            const showagpuNet = carousel.find(Selectors.region.agpuNet)[0];

            chooserNavigateToMnet(showagpuNet, footerData, carousel, modal);
        }
