...
/**
 * This is the main endpoint which agpuNet instances POST to.
 *
 * agpuNet instances send the user agent to this endpoint via a form POST.
 * Then:
 * 1. The POSTed resource information is put in a session store for cross-request access.
 * 2. This page makes a GET request for admin/tool/agpunet/index.php (the import confirmation page).
...
use tool_agpunet\local\remote_resource;
use tool_agpunet\local\url;
...
if (!empty($_POST)) {
    ...
    redirect(new agpu_url('/admin/tool/agpunet/index.php', ['id' => $importinfo->get_id()]));
