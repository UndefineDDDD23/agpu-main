...
class remote_resource {
     ...
     * @throws \agpu_exception if the file cannot be downloaded.
     */
    public function download_to_requestdir(): array {
        ...
        // In future, use a timeout (download and/or connection) controlled by a tool_agpunet setting.
        $downloadtimeout = 30;

        $result = $this->curl->download_one($this->url->get_value(), null, ['filepath' => $fullpathwithname,
            'timeout' => $downloadtimeout]);
        if ($result !== true) {
            throw new \agpu_exception('errorduringdownload', 'tool_agpunet', '', $result);
        }
