...
class external extends external_api {
    ...
    public static function verify_webfinger(string $profileurl, int $course, int $section) {
        ...
        try {
            $mnetprofile = new agpunet_user_profile($params['profileurl'], $USER->id);
        } catch (\Exception $e) {
            return [
                'result' => false,
        ...
        $userlink = profile_manager::get_agpunet_profile_link($mnetprofile);

        // There were no problems verifying the account so lets store it.
        if ($userlink['result'] === true) {
            profile_manager::save_agpunet_user_profile($mnetprofile);
        ...
        if ($arrcourses = \core_course_category::search_courses(array('search' => $params['searchvalue']))) {
            foreach ($arrcourses as $course) {
                if (has_capability('agpu/course:manageactivities', \context_course::instance($course->id))) {
                    ...
                    $viewurl = new \agpu_url('/admin/tool/agpunet/options.php', $options);
