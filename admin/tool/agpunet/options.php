...
/**
 * Page to select WHAT to do with a given resource stored on agpuNet.
 *
 ...
 */
use tool_agpunet\local\import_handler_registry;
use tool_agpunet\local\import_processor;
use tool_agpunet\local\import_info;
use tool_agpunet\local\import_strategy_file;
use tool_agpunet\local\import_strategy_link;
...
require_capability('agpu/course:manageactivities', context_course::instance($config->course));
...
if ($cancel) {
    redirect(new agpu_url('/course/view.php', ['id' => $config->course]));
}
...
if ($import && $module) {
    ...
    redirect(new agpu_url('/course/view.php', ['id' => $course->id]));
}

// Setup the page and display the form.
$PAGE->set_context(context_course::instance($course->id));
$PAGE->set_pagelayout('base');
$PAGE->set_title(get_string('coursetitle', 'agpu', array('course' => $course->fullname)));
$PAGE->set_heading($course->fullname);
$PAGE->set_url(new agpu_url('/admin/tool/agpunet/options.php'));
...
echo $OUTPUT->header();
echo $PAGE->get_renderer('core')->render_from_template('tool_agpunet/import_options_select', $context);
echo $OUTPUT->footer();
