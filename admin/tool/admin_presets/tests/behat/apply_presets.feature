...
Feature: I can apply presets
  ...
  Scenario: Applying Starter agpu preset changes status and settings
#   Checking the settings before applying Full agpu preset (we're only testing one of each type).
...
#   Apply the Starter preset.
  ...
  Scenario: Re-applying Starter agpu preset does not display setting changes
