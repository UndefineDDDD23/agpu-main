...
class load extends base {
    ...
    public function execute(): void {
        ...
        $url = new \agpu_url('/admin/tool/admin_presets/index.php', ['action' => 'load', 'mode' => 'execute']);
        $this->agpuform = new load_form($url);

        if ($this->agpuform->is_cancelled()) {
            redirect(new \agpu_url('/admin/tool/admin_presets/index.php?action=base'));
        }

        if ($this->agpuform->is_submitted() && $this->agpuform->is_validated() && ($this->agpuform->get_data())) {
            ...
            $this->agpuform = new continue_form($url);
        }
    }
     ...
     *
     * It checks the agpu version and it only allows users to import
    ...
    protected function display_preset(bool $displayform = true, bool $raiseexception = true) {
        ...
        if (!$preset = $DB->get_record('adminpresets', ['id' => $data->id])) {
            if ($raiseexception) {
                throw new agpu_exception('errornopreset', 'core_adminpresets');
            } else {
