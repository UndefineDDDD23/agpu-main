...
switch ($context->contextlevel) {
    ...
    default:
        throw new \agpu_exception('invalidcontext', 'tool_recyclebin');
    break;
}

if (!$recyclebin::is_enabled()) {
    throw new \agpu_exception('notenabled', 'tool_recyclebin');
}
...
foreach ($items as $item) {
    ...
    if ($canrestore && ($context->contextlevel == CONTEXT_COURSECAT || isset($modules[$item->module]))) {
        $restoreurl = new agpu_url($PAGE->url, array(
            'contextid' => $contextid,
            'itemid' => $item->id,
            'action' => 'restore',
            'sesskey' => sesskey()
    ...
    if ($recyclebin->can_delete()) {
        $showempty = true;
        $delete = new agpu_url($PAGE->url, array(
            'contextid' => $contextid,
            'itemid' => $item->id,
            'action' => 'delete',
            'sesskey' => sesskey()
...
if ($showempty) {
    $emptylink = new agpu_url($PAGE->url, array(
        'contextid' => $contextid,
        'action' => 'empty',
        'sesskey' => sesskey()
    ));
