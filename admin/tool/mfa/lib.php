...
/**
 * Triggered as soon as practical on every agpu bootstrap after config has
...
function tool_mfa_bulk_user_actions(): array {
    ...
    return [
        'tool_mfa_reset_factors' => new action_link(
            new agpu_url('/admin/tool/mfa/reset_factor.php'),
...
function tool_mfa_output_fragment_factor_action_confirmation_form(
    ...
    foreach ($args as $key => $arg) {
        if (empty($arg)) {
            throw new \agpu_exception('missingparam', 'error', '', $key);
        }
    }
