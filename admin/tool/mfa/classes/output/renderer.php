...
class renderer extends \plugin_renderer_base {
            ...
            if (!$active) {
                ...
                if ($factor->show_setup_buttons()) {
                    ...
                    $button = new \single_button(
                        url: new \agpu_url('action.php', $params),
     ...
     * @todo Final deprecation in agpu 4.8 MDL-80995
     */
    public function setup_factor(object $factor): string {
        ...
        $setupurl = new \agpu_url('action.php', $setupparams);
    ...
    public function not_enough_factors(): string {
        ...
        // Logout button.
        $url = new \agpu_url('/admin/tool/mfa/auth.php', ['logout' => 1]);
    ...
    public function factors_locked_table(): string {
        ...
        foreach ($factors as $factor) {
            ...
            $actions = \html_writer::link( new \agpu_url($this->page->url,
                ['reset' => $factor->name, 'sesskey' => sesskey()]), get_string('performbulk', 'tool_mfa'));
            $lockedusers = \html_writer::link(new \agpu_url($this->page->url, ['view' => $factor->name]), $lockedusers);
    ...
    public function factor_locked_users_table(object_factor $factor): string {
        ...
        foreach ($records as $record) {
            ...
            $proflink = \html_writer::link(new \agpu_url('/user/profile.php',
                ['id' => $record->id]), fullname($record));

            // IP link.
            $creatediplink = \html_writer::link(new \agpu_url('/iplookup/index.php',
                ['ip' => $record->createdfromip]), $record->createdfromip);
            $lastiplink = \html_writer::link(new \agpu_url('/iplookup/index.php',
                ['ip' => $record->lastip]), $record->lastip);

            // Deep link to logs.
            $logicon = $this->pix_icon('i/report', get_string('userlogs', 'tool_mfa'));
            $actions = \html_writer::link(new \agpu_url('/report/log/index.php', [
                'id' => 1, // Site.
                'user' => $record->id,
            ]), $logicon);
            ...
            $actions .= $this->action_link(
                new \agpu_url($this->page->url, ['reset' => $factor->name, 'id' => $record->id, 'sesskey' => sesskey()]),
