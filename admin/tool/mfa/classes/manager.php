...
class manager {
    ...
    public static function cannot_login(): void {
        ...
        if (!preg_match("~(\/admin\/tool\/mfa\/auth.php)~", $ME)) {
            ...
            redirect(new \agpu_url('/admin/tool/mfa/auth.php'));
        }
    ...
    public static function resolve_mfa_status(bool $shouldreload = false): void {
        ...
        if ($state == factor::STATE_PASS) {
            ...
            if (isset($SESSION->tool_mfa_has_been_redirected)) {
                ...
                redirect(new \agpu_url($wantsurl));
            } else {
            ...
            $SESSION->tool_mfa_has_been_redirected = true;
            $authurl = new \agpu_url('/admin/tool/mfa/auth.php');
            redirect($authurl);
        }
    }
    ...
    public static function set_pass_state(): void {
        ...
        if (!isset($SESSION->tool_mfa_authenticated)) {
            ...
            foreach ($enabledfactors as $factor) {
                ...
                if ($factorpref) {
                    $url = new \agpu_url('/admin/tool/mfa/user_preferences.php');
     ...
     * @return int
     */
    public static function should_require_mfa(string|\agpu_url $url, bool|null $preventredirect): int {
        global $CFG, $USER, $SESSION;

        // If no cookies then no session so cannot do MFA.
        // Unit testing based on defines is not viable.
        if (NO_agpu_COOKIES && !PHPUNIT_TEST) {
        ...
        if (is_siteadmin()) {
            ...
            $upgradesettings = new \agpu_url('/admin/upgradesettings.php');
        ...
        $logourl = new \agpu_url('/pluginfile.php/1/core_admin/logocompact/');
        ...
        // Enrolment.
        $enrol = new \agpu_url('/enrol/index.php');
    ...
    public static function get_no_redirect_urls(): array {
        ...
        $urls = [
            new \agpu_url('/login/logout.php'),
            new \agpu_url('/admin/tool/mfa/guide.php'),
        ];
        foreach ($factors as $factor) {
        ...
        if (empty($SESSION->tool_mfa_authenticated) || !$SESSION->tool_mfa_authenticated) {
            ...
            if ($redir == self::REDIRECT) {
                ...
                if (!empty($SESSION->mfa_redir_referer)) {
                    throw new \agpu_exception('redirecterrordetected', 'tool_mfa',
