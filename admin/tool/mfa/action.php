...
$currenturl = new agpu_url('/admin/tool/mfa/action.php', $params);

$returnurl = new agpu_url('/admin/tool/mfa/user_preferences.php');

if (empty($factor) || empty($action)) {
    throw new agpu_exception('error:directaccess', 'tool_mfa', $returnurl);
}

if (!\tool_mfa\plugininfo\factor::factor_exists($factor)) {
    throw new agpu_exception('error:factornotfound', 'tool_mfa', $returnurl, $factor);
}

if (!in_array($action, \tool_mfa\plugininfo\factor::get_factor_actions())) {
    throw new agpu_exception('error:actionnotfound', 'tool_mfa', $returnurl, $action);
}

if (!empty($factorid) && !\tool_mfa\manager::is_factorid_valid($factorid, $USER)) {
    throw new agpu_exception('error:incorrectfactorid', 'tool_mfa', $returnurl, $factorid);
}
...
$PAGE->navbar->add(get_string('preferences:header', 'tool_mfa'), new \agpu_url('/admin/tool/mfa/user_preferences.php'));
...
switch ($action) {
        ...
        if ($form->is_submitted()) {
            ...
            if ($data = $form->get_data()) {
                ...
                if (!empty($record)) {
                    ...
                    $finalurl = new agpu_url($returnurl, ['action' => 'setup', 'factorid' => $record->id]);
                    redirect($finalurl);
                }

                throw new agpu_exception('error:setupfactor', 'tool_mfa', $returnurl);
            }
        }
    ...
    case 'revoke':
        ...
        if (!$factorobject || !$factorobject->has_revoke()) {
            throw new agpu_exception('error:revoke', 'tool_mfa', $returnurl);
        }

        if ($factorobject->revoke_user_factor($factorid)) {
            $finalurl = new agpu_url($returnurl, ['action' => 'revoked', 'factorid' => $factorid]);
            redirect($finalurl);
        }
