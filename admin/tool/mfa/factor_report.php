...
if (!empty($reset) && confirm_sesskey()) {
    ...
    if (!$factor instanceof \tool_mfa\local\factor\object_factor_base) {
        throw new agpu_exception('error:factornotfound', 'tool_mfa');
    }
    ...
    if (!empty($userid)) {
        ...
        redirect(new agpu_url($PAGE->url, ['view' => $factor->name]), get_string('resetsuccess', 'tool_mfa', $stringarr));
    }
    ...
    redirect(new agpu_url('/admin/user/user_bulk.php'));
}
...
if (!empty($view)) {
    ...
    $backbutton = new single_button(new agpu_url($PAGE->url), get_string('back'));
