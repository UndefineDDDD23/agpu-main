...
require_capability('agpu/competency:templatemanage', $context);

// We keep the original context in the URLs, so that we remain in the same context.
$url = new agpu_url("/admin/tool/lp/edittemplate.php", [
    'id' => $id,
...
if ($data) {
    if (empty($data->id)) {
        ...
        $returnurl = new agpu_url('/admin/tool/lp/templatecompetencies.php', [
