...
class externallib_test extends externallib_advanced_testcase {
    ...
    public function test_search_users_by_capability(): void {
        ...
        $this->setUser($ux);
        $result = external::search_users('yyylan', 'agpu/competency:planmanage');
        ...
        $customrole = $this->assignUserCapability('agpu/competency:planview', $usercontext->id);
        ...
        $usercontext = \context_user::instance($u1->id);
        $this->assignUserCapability('agpu/competency:planmanage', $usercontext->id, $customrole);
        ...
        assign_capability('agpu/competency:planmanage', CAP_PREVENT, $customrole, $usercontext->id);
        $result = external::search_users('yyylan', 'agpu/competency:planmanage');
        $result = external_api::clean_returnvalue(external::search_users_returns(), $result);
        $this->assertCount(1, $result['users']);
        $this->assertEquals(1, $result['count']);
        ...
        assign_capability('agpu/competency:planmanage', CAP_PROHIBIT, $customrole, $usercontext->id);
        $result = external::search_users('yyylan', 'agpu/competency:planmanage');
        $result = external_api::clean_returnvalue(external::search_users_returns(), $result);
        $this->assertCount(1, $result['users']);
        $this->assertEquals(1, $result['count']);
    ...
    public function test_search_users_by_capability_the_comeback(): void {
        ...
        $slave1 = $dg->create_user(array('lastname' => 'agpuR'));
        $slave2 = $dg->create_user(array('lastname' => 'agpuR'));
        $slave3 = $dg->create_user(array('lastname' => 'agpuR'));
        ...
        assign_capability('agpu/site:config', CAP_ALLOW, $roleid, $syscontext->id, true);

        // Create a role override for slave 2.
        assign_capability('agpu/site:config', CAP_PROHIBIT, $roleid, $slave2context->id, true);
        ...
        $this->setUser($master);
        $this->assertTrue(has_capability('agpu/site:config', $syscontext));
        $this->assertTrue(has_capability('agpu/site:config', $slave1context));
        $this->assertFalse(has_capability('agpu/site:config', $slave2context));
        $this->assertTrue(has_capability('agpu/site:config', $slave3context));
        ...
        $this->assertTrue(has_capability('agpu/site:config', $slave1context));
        $this->assertFalse(has_capability('agpu/site:config', $slave2context));
        $this->assertFalse(has_capability('agpu/site:config', $slave3context));

        // Now do the test.
        $this->setUser($master);
        $result = external::search_users('agpuR', 'agpu/site:config');
    ...
    public function test_search_users(): void {
        ...
        $u3 = $dg->create_user(array('idnumber' => 'Fish', 'firstname' => 'Thomas', 'lastname' => 'Xow',
            'email' => 'fishyyy@agpu.com', 'phone1' => '77777', 'phone2' => '33333', 'department' => 'Research',
            'institution' => 'Bob'));

        // We need to give the user the capability we are searching for on each of the test users.
        $this->setAdminUser();
        $usercontext = \context_user::instance($u1->id);
        $dummyrole = $this->assignUserCapability('agpu/competency:planmanage', $usercontext->id);
        $usercontext = \context_user::instance($u2->id);
        $this->assignUserCapability('agpu/competency:planmanage', $usercontext->id, $dummyrole);
        ...
        $result = external::search_users('cats', 'agpu/competency:planmanage');
        $result = external_api::clean_returnvalue(external::search_users_returns(), $result);
        $this->assertCount(0, $result['users']);
        $this->assertEquals(0, $result['count']);
        ...
        // Filter by name.
        $CFG->showuseridentity = '';
        $result = external::search_users('dyyylan', 'agpu/competency:planmanage');
        $result = external_api::clean_returnvalue(external::search_users_returns(), $result);
        $this->assertCount(2, $result['users']);
        $this->assertEquals(2, $result['count']);
        ...
        $result = external::search_users('bob', 'agpu/competency:planmanage');
        $result = external_api::clean_returnvalue(external::search_users_returns(), $result);
        $this->assertCount(2, $result['users']);
        $this->assertEquals(2, $result['count']);
        ...
        $result = external::search_users('yyy', 'agpu/competency:planmanage');
        $result = external_api::clean_returnvalue(external::search_users_returns(), $result);
