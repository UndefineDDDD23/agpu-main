 ...
 *
 * @package    tool_policy
 * @copyright  2023 Juan Leyva <juan@agpu.com>
...
class get_user_acceptances extends external_api {
    ...
    public static function execute(int $userid = 0): array {
        ...
        $canviewfullnames = has_capability('agpu/site:viewfullnames', $systemcontext);
            ...
            foreach ($userpolicy->versions as $version) {
                ...
                if (!empty($version->acceptance)) {
                    ...
                    if ($version->acceptance->usermodified && $version->acceptance->usermodified != $user->id) {
                        ...
                        $override = $canviewfullnames || has_capability('agpu/site:viewfullnames', context_user::instance($version->acceptance->usermodified));
                        $policy['acceptance']['modfullname'] = fullname($usermodified, $override);
                    }
                    if (!empty($version->acceptance->note)) {
                        [$policy['acceptance']['note']] = util::format_text($version->acceptance->note, FORMAT_agpu, $systemcontext);
                    }
                }
