...
class acceptances implements renderable, templatable {
     ...
     *
     * @param int $userid
     * @param string|agpu_url $returnurl
     */
    public function __construct($userid, $returnurl = null) {
        $this->userid = $userid;
        $this->returnurl = $returnurl ? (new agpu_url($returnurl))->out(false) : null;
    }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        foreach ($policies as $policy) {
            foreach ($policy->versions as $version) {
                ...
                $returnurl = new agpu_url('/admin/tool/policy/user.php', ['userid' => $this->userid]);
                $version->viewurl = (new agpu_url('/admin/tool/policy/view.php', [
                    'policyid' => $policy->id,
                ...
                if ($version->acceptance !== null) {
                    ...
                    if ($onbehalf) {
                        ...
                        $profileurl = new \agpu_url('/user/profile.php', array('id' => $usermodified->id));
                        $version->acceptedby = \html_writer::link($profileurl, fullname($usermodified, $canviewfullnames ||
                            has_capability('agpu/site:viewfullnames', \context_user::instance($acceptance->usermodified))));
