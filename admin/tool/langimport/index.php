...
/**
 * Fetches language packages from download.agpu.org server
 *
 * Language packages are available at https://download.agpu.org/langpack/
...
if (empty($CFG->langotherroot)) {
    throw new agpu_exception('missingcfglangotherroot', 'tool_langimport');
}
...
if (!empty($CFG->skiplangupgrade)) {
    ...
    echo $OUTPUT->single_button(new agpu_url($PAGE->url, array('purgecaches' => 1)), get_string('purgestringcaches', 'tool_langimport'));
    echo $OUTPUT->footer();
    die;
}
...
$uninstallurl = new agpu_url('/admin/tool/langimport/index.php', array('mode' => DELETION_OF_SELECTED_LANG));
$updateurl = null;
if ($remote) {
    $updateurl = new agpu_url('/admin/tool/langimport/index.php', array('mode' => UPDATE_ALL_LANG));
}
$installurl = new agpu_url('/admin/tool/langimport/index.php', array('mode' => INSTALLATION_OF_SELECTED_LANG));
