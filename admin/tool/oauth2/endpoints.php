...
if ($mform && $mform->is_cancelled()) {
    redirect(new agpu_url('/admin/tool/oauth2/endpoints.php', ['issuerid' => $issuerid]));
} else if ($action == 'edit') {
    ...
    if (!optional_param('confirm', false, PARAM_BOOL)) {
        ...
        $continueurl = new agpu_url('/admin/tool/oauth2/endpoints.php', $continueparams);
        $cancelurl = new agpu_url('/admin/tool/oauth2/endpoints.php');
        echo $OUTPUT->header();
        $strparams = [ 'issuer' => s($issuer->get('name')), 'endpoint' => s($endpoint->get('name')) ];
    ...
    $addurl = new agpu_url('/admin/tool/oauth2/endpoints.php', ['action' => 'edit', 'issuerid' => $issuerid]);
