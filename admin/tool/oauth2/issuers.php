 ...
 *
 * @package    tool_oauth2
 * @copyright  2017 Damyon Wiese <damyon@agpu.com>
...
if ($mform && $mform->is_cancelled()) {
    redirect(new agpu_url('/admin/tool/oauth2/issuers.php'));
} else if ($action == 'edit') {
    ...
    if (!optional_param('confirm', false, PARAM_BOOL)) {
        ...
        $continueurl = new agpu_url('/admin/tool/oauth2/issuers.php', $continueparams);
        $cancelurl = new agpu_url('/admin/tool/oauth2/issuers.php');
        echo $OUTPUT->header();
    ...
    if (!optional_param('confirm', false, PARAM_BOOL)) {
        ...
        if (core\oauth2\api::connect_system_account($issuer, new agpu_url('/admin/tool/oauth2/issuers.php', $params))) {
    ...
    $addurl = new agpu_url('/admin/tool/oauth2/issuers.php', $params);
    ...
    // agpuNet template.
    $docs = 'admin/tool/oauth2/issuers/agpunet';
    $params = ['action' => 'edittemplate', 'type' => 'agpunet', 'sesskey' => sesskey(), 'docslink' => $docs];
    $addurl = new agpu_url('/admin/tool/oauth2/issuers.php', $params);
    echo $renderer->single_button($addurl, get_string('agpunet_service', 'tool_oauth2'));

    // Generic issuer.
    $addurl = new agpu_url('/admin/tool/oauth2/issuers.php', ['action' => 'edit']);
