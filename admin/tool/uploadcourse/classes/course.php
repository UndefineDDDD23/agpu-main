...
class tool_uploadcourse_course {
    ...
    public function prepare() {
        ...
        if (!empty($coursedata['lang'])) {
            if ($exists) {
                ...
                if (!has_capability('agpu/course:setforcedlanguage', context_course::instance($courseid))) {
        ...
        if (!$exists || !array_key_exists('numsections', $coursedata)) {
            if (isset($this->rawdata['numsections']) && is_numeric($this->rawdata['numsections'])) {
                ...
                if ($numsections != 0) {
                    ...
                    $coursedata['numsections'] = get_config('agpucourse', 'numsections');
                }
            } else {
                $coursedata['numsections'] = get_config('agpucourse', 'numsections');
            }
        ...
        if (isset($coursedata['downloadcontent'])) {
            if (!$CFG->downloadcoursecontentallowed ||
                    !has_capability('agpu/course:configuredownloadcontent', $context)) {
    ...
    public function proceed() {
        ...
        if (!$this->prepared) {
            ...
            throw new agpu_exception('Cannot proceed, errors were detected.');
