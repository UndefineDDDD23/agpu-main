...
class permissions {
    ...
    public static function check_permission_to_delete(string $shortname): ?lang_string {
        ...
        if (!has_capability('agpu/course:delete', context_course::instance($course->id))) {
            return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:delete'));
        }
        return null;
    }
    ...
    public static function check_permission_to_update(array $coursedata): ?lang_string {
        ...
        if (!has_capability('agpu/course:update', context_course::instance($course->id))) {
            return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:update'));
        }
        ...
        if (!empty($coursedata['category']) && $coursedata['category'] != $course->category) {
            ...
            if (!has_capability('agpu/course:changecategory', context_coursecat::instance($course->category))) {
                return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:changecategory'));
            }

            if (!has_capability('agpu/course:changecategory', context_coursecat::instance($coursedata['category']))) {
                return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:changecategory'));
            }
        }
        ...
        if (array_key_exists('idnumber', $coursedata) && $coursedata['idnumber'] !== $course->idnumber &&
                !has_capability('agpu/course:changeidnumber', $context)) {
            return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:changeidnumber'));
        }
        ...
        if (array_key_exists('shortname', $coursedata) && $coursedata['shortname'] !== $course->shortname &&
                !has_capability('agpu/course:changeshortname', $context)) {
            return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:changeshortname'));
        }

        // Check permission to change course fullname.
        if (array_key_exists('fullname', $coursedata) && $coursedata['fullname'] !== $course->fullname &&
                !has_capability('agpu/course:changefullname', $context)) {
            return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:changefullname'));
        }

        // Check permission to change course summary.
        if (array_key_exists('summary', $coursedata) && $coursedata['summary'] !== $course->summary &&
                !has_capability('agpu/course:changesummary', $context)) {
            return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:changesummary'));
        }

        // Check permission to change course visibility.
        if (array_key_exists('visible', $coursedata) && $coursedata['visible'] !== $course->visible &&
                !has_capability('agpu/course:visibility', $context)) {
            return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:visibility'));
        }
        ...
        if (core_tag_tag::is_enabled('core', 'course') &&
            ...
            ($error = self::check_capability(tool_uploadcourse_course::DO_UPDATE, $coursedata, 'agpu/course:tag'))) {
            return $error;
        }
    ...
    public static function check_permission_to_create(array $coursedata): ?lang_string {
        ...
        if (!has_capability('agpu/course:create', $catcontext)) {
            return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:create'));
        }

        // If lang is specified, check the user is allowed to set that field.
        if (!empty($coursedata['lang'])) {
            if (!guess_if_creator_will_have_course_capability('agpu/course:setforcedlanguage', $catcontext)) {
                return new lang_string('cannotforcelang', 'tool_uploadcourse');
            }
        }
        ...
        if (array_key_exists('visible', $coursedata) && !$coursedata['visible'] &&
                !guess_if_creator_will_have_course_capability('agpu/course:visibility', $catcontext)) {
            return new lang_string('nopermissions', 'error', get_capability_string('agpu/course:visibility'));
        }
        ...
        if (core_tag_tag::is_enabled('core', 'course') &&
                (array_key_exists('tags', $coursedata) && strval($coursedata['tags']) !== '') &&
                ($error = self::check_capability(tool_uploadcourse_course::DO_CREATE, $coursedata, 'agpu/course:tag'))) {
            return $error;
        }
    ...
    public static function check_permission_to_reset(array $coursedata): ?lang_string {
        return self::check_capability(tool_uploadcourse_course::DO_UPDATE, $coursedata, 'agpu/course:reset');
    }
    ...
    public static function check_permission_to_restore(int $do, array $coursedata): ?lang_string {
        return self::check_capability($do, $coursedata, 'agpu/restore:restorecourse');
    }
}
