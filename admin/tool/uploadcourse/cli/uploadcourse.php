...
echo "agpu course uploader running ...\n";
...
$defaults['enddate'] = $defaults['startdate'] + intval(get_config('agpucourse', 'courseduration'));
...
unset($content);
if ($readcount === false) {
    throw new \agpu_exception('csvfileerror', 'tool_uploadcourse', '', $cir->get_error());
} else if ($readcount == 0) {
    throw new \agpu_exception('csvemptyfile', 'error', '', $cir->get_error());
}
