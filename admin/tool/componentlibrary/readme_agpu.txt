...
The agpu UI component Library is built with the aim to provide a single repository
of agpu UI components to be used for new and existing agpu pages.

A agpu UI component is a self-contained UI element originating from one source file
that can be used in different locations in core code, for example a button.

This plugin generates the documentation of the agpu User Interface components. It
...
The agpu grunt tasks were setup to include the node modules required for building a static site using
...
all the build tasks for the library are included in agpu's core grunt tasks
...
### Example form
The component library includes an example agpu form in `/admin/tool/componentlibrary/classes/exampleform.php` that renders most form fields inside a component library page. Since the rendering of form fields require php the form fields cannot be included as mustache templates only. The example form does not contain all possible fields but enough to serve as a testing page when any of the form CSS changes
