...
if (empty($courseid)) {
    ...
    if (!has_capability('tool/monitor:subscribe', $context)) {
        ...
        if (tool_monitor_can_subscribe()) {
            ...
            // return error.
            throw new \agpu_exception('rulenopermission', 'tool_monitor');
        }
    }
} else {
...
// Set up the page.
$indexurl = new agpu_url('/admin/tool/monitor/index.php', array('courseid' => $courseid));
    ...
    switch ($action) {
        ...
        case 'unsubscribe' :
            ...
            if (!$subscription = $DB->record_exists('tool_monitor_subscriptions', array('id' => $subscriptionid))) {
                redirect(new agpu_url('/admin/tool/monitor/index.php', array('courseid' => $courseid)));
            }

            // Set the URLs.
            $confirmurl = new agpu_url('/admin/tool/monitor/index.php', array('subscriptionid' => $subscriptionid,
                'courseid' => $courseid, 'action' => 'unsubscribe', 'confirm' => true,
                'sesskey' => sesskey()));
            $cancelurl = new agpu_url('/admin/tool/monitor/index.php', array('subscriptionid' => $subscriptionid,
