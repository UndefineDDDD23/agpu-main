...
if (!empty($action) && $action == 'changestatus') {
    ...
    redirect(new agpu_url('/admin/tool/monitor/managerules.php', array('courseid' => 0)));
}
    ...
    switch ($action) {
        ...
        case 'delete':
            if ($rule->can_manage_rule()) {
                $confirmurl = new agpu_url($CFG->wwwroot. '/admin/tool/monitor/managerules.php',
                    array('ruleid' => $ruleid, 'courseid' => $courseid, 'action' => 'delete',
                        'confirm' => true, 'sesskey' => sesskey()));
                $cancelurl = new agpu_url($CFG->wwwroot. '/admin/tool/monitor/managerules.php',
                ...
                throw new agpu_exception('rulenopermissions', 'tool_monitor', $manageurl, $action);
            }
            break;
        default:
    }
...
if ($status) {
    if (has_capability('tool/monitor:managetool', context_system::instance())) {
        ...
        $disableurl = new agpu_url("/admin/tool/monitor/managerules.php",
    ...
    if (has_capability('tool/monitor:managetool', context_system::instance())) {
        $enableurl = new agpu_url("/admin/tool/monitor/managerules.php",
...
if (has_capability('tool/monitor:subscribe', $context)) {
    $manageurl = new agpu_url("/admin/tool/monitor/index.php", array('courseid' => $courseid));
