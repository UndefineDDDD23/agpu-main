 ...
 * @param agpu_url $returnurl return url in case of any error
 * @return array list of fields
 */
function uu_validate_user_upload_columns(csv_import_reader $cir, $stdfields, $profilefields, agpu_url $returnurl) {
    $columns = $cir->get_columns();

    if (empty($columns)) {
        $cir->close();
        $cir->cleanup();
        throw new \agpu_exception('cannotreadtmpfile', 'error', $returnurl);
    }
    if (count($columns) < 2) {
        $cir->close();
        $cir->cleanup();
        throw new \agpu_exception('csvfewcolumns', 'error', $returnurl);
    }
    ...
    foreach ($columns as $key=>$unused) {
        ...
        if (in_array($field, $stdfields) or in_array($lcfield, $stdfields)) {
            ...
            $cir->close();
            $cir->cleanup();
            throw new \agpu_exception('invalidfieldname', 'error', $returnurl, $field);
        }
        if (in_array($newfield, $processed)) {
            $cir->close();
            $cir->cleanup();
            throw new \agpu_exception('duplicatefieldname', 'error', $returnurl, $newfield);
        }
