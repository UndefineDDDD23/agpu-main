...
class process {
    ...
    public function get_file_columns(): array {
        if ($this->filecolumns === null) {
            $returnurl = new \agpu_url('/admin/tool/uploaduser/index.php');
    ...
    public function process_line(array $line) {
        ...
        if (!empty($user->deleted)) {
            if (!$this->get_allow_deletes() or $remoteuser or
                    !has_capability('agpu/user:delete', context_system::instance())) {
                $this->usersskipped++;
        ...
        if ($existinguser) {
            ...
            $this->upt->track('username', \html_writer::link(
                new \agpu_url('/user/profile.php', ['id' => $existinguser->id]), s($existinguser->username)), 'normal', false);
            ...
            $this->upt->track('username', \html_writer::link(
                new \agpu_url('/user/profile.php', ['id' => $user->id]), s($user->username)), 'normal', false);
            ...
            if (!empty($user->$column)) {
                ...
                if (!isset($this->cohorts[$addcohort])) {
                    if (is_number($addcohort)) {
                        ...
                        if (empty($cohort) && has_capability('agpu/cohort:manage', \context_system::instance())) {
