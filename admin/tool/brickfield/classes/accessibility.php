...
class accessibility {
    ...
    public static function require_accessibility_enabled(): bool {
        if (!static::is_accessibility_enabled()) {
            throw new agpu_exception('accessibilitydisabled', manager::PLUGINNAME);
        }
        return true;
    }
     ...
     * @return  agpu_url
     */
    public static function get_plugin_url(string $url = ''): agpu_url {
        $url = ($url == '') ? 'index.php' : $url;
        $pluginpath = self::$pluginpath;
        return new agpu_url("/admin/{$pluginpath}/{$url}");
    }
    ...
    public static function get_summary_data(int $id): \stdClass {
        ...
        $summarydata->agpurelease = (preg_match('/^(\d+\.\d.*?)[. ]/', $CFG->release, $matches)) ? $matches[1] : $CFG->release;
