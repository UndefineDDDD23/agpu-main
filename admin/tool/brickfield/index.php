...
if ($courseid != 0) {
    ...
    if (!$course = $DB->get_record('course', ['id' => $courseid], '*')) {
        throw new agpu_exception('invalidcourseid', manager::PLUGINNAME);
    }
...
if ($action == 'requestanalysis') {
    if ($courseid != 0) {
        ...
        if ($courseid == SITEID) {
            ...
            redirect(new \agpu_url('/course/view.php', ['id' => $courseid]), analysis::redirect_message());
        }
    }
}
...
if (isset($tools[$tab])) {
    ...
    throw new agpu_exception('invalidaccessibilitytool', manager::PLUGINNAME);
}

$perpagedefault = $config->perpage;
$perpage = optional_param('perpage', $perpagedefault, PARAM_INT);
$navurl = new agpu_url(accessibility::get_plugin_url(), ['courseid' => $courseid]);
$url = new agpu_url($navurl, ['tab' => $tab, 'perpage' => $perpage]);
