 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class user_bulk_action_form extends agpuform {
    ...
    protected function build_actions(): array {
        ...
        $canaccessbulkactions = has_any_capability(['agpu/user:update', 'agpu/user:delete'], context_system::instance());

        $syscontext = context_system::instance();
        $actions = [];
        if (has_capability('agpu/user:update', $syscontext)) {
            $actions['confirm'] = new action_link(
                new agpu_url('/admin/user/user_bulk_confirm.php'),
                get_string('confirm'));
        }
        if ($canaccessbulkactions && has_capability('agpu/site:readallmessages', $syscontext) && !empty($CFG->messaging)) {
            $actions['message'] = new action_link(
                new agpu_url('/admin/user/user_bulk_message.php'),
                get_string('messageselectadd'));
        }
        if (has_capability('agpu/user:delete', $syscontext)) {
            $actions['delete'] = new action_link(
                new agpu_url('/admin/user/user_bulk_delete.php'),
                get_string('delete'));
        }
        if ($canaccessbulkactions) {
            $actions['displayonpage'] = new action_link(
                new agpu_url('/admin/user/user_bulk_display.php'),
                get_string('displayonpage'));
        }

        if (has_capability('agpu/user:update', $syscontext)) {
            $actions['download'] = new action_link(
                new agpu_url('/admin/user/user_bulk_download.php'),
                get_string('download', 'admin'));
        }

        if (has_capability('agpu/user:update', $syscontext)) {
            $actions['forcepasswordchange'] = new action_link(
                new agpu_url('/admin/user/user_bulk_forcepasswordchange.php'),
                get_string('forcepasswordchange'));
        }
        if ($canaccessbulkactions && has_capability('agpu/cohort:assign', $syscontext)) {
            $actions['addtocohort'] = new action_link(
                new agpu_url('/admin/user/user_bulk_cohortadd.php'),
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class user_bulk_form extends agpuform {
