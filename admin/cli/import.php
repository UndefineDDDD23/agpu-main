...
if (!$admin = get_admin()) {
    throw new \agpu_exception('noadmins');
}
...
if (!empty($options['srccourseid']) && !empty($options['dstcourseid'])) {
    ...
    $bc = new backup_controller(backup::TYPE_1COURSE, $srccourse->id, backup::FORMAT_agpu,
    ...
    if (!$cm = $DB->get_record('course_modules', ['id' => $options['srccmid']], 'id')) {
        throw new \agpu_exception('invalidcoursemoduleid', 'error', '', $options['srccmid']);
    }
    ...
    throw new \agpu_exception('invalidoption');
}
