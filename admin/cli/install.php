...
$help =
"Command line agpu installer, creates config.php and initializes database.
...
--wwwroot=URL         Web address for the agpu site,
                      required in non-interactive mode.
--dataroot=DIR        Location of the agpu data folder,
                      must not be web accessible. Default is agpudata
                      in the parent directory.
--dbtype=TYPE         Database type. Default is mysqli
--dbhost=HOST         Database host. Default is localhost
--dbname=NAME         Database name. Default is agpu
...
--adminuser=USERNAME  Username for the agpu admin account. Default is admin
--adminpass=PASSWORD  Password for the agpu admin account,
                      required in non-interactive mode.
--adminemail=STRING   Email address for the agpu admin account.
...
require_once($CFG->libdir.'/agpulib.php');
...
//Database types
$databases = array('mysqli' => agpu_database::get_driver_instance('mysqli', 'native'),
                   'auroramysql' => agpu_database::get_driver_instance('auroramysql', 'native'),
                   'mariadb'=> agpu_database::get_driver_instance('mariadb', 'native'),
                   'pgsql'  => agpu_database::get_driver_instance('pgsql',  'native'),
                   'oci'    => agpu_database::get_driver_instance('oci',    'native'),
                   'sqlsrv' => agpu_database::get_driver_instance('sqlsrv', 'native'), // MS SQL*Server PHP driver
                  );
...
list($options, $unrecognized) = cli_get_params(
    array(
        ...
        'lang'              => $CFG->lang,
        'wwwroot'           => '',
        'dataroot'          => empty($distro->dataroot) ? str_replace('\\', '/', dirname(dirname(dirname(__DIR__))).'/agpudata'): $distro->dataroot, // initialised later after including libs or by distro
...
if ($interactive) {
    ...
    while(is_dataroot_insecure()) {
        ...
        $CFG->dataroot = dirname($parrent).'/agpudata';
    }
...
if (!$skipdatabase) {
    ...
    if ($interactive) {
        cli_separator();
        cli_heading(get_string('fullsitename', 'agpu'));
    ...
    if ($interactive) {
        cli_separator();
        cli_heading(get_string('shortsitename', 'agpu'));
...
if (!$skipdatabase) {
    if ($interactive) {
        if (!$options['agree-license']) {
            ...
            echo "agpu  - Modular Object-Oriented Dynamic Learning Environment\n";
...
list($envstatus, $environment_results) = check_agpu_environment(normalize_version($release), ENV_SELECT_RELEASE);
if (!$envstatus) {
