...
if (empty($settingspage) or !($settingspage instanceof admin_category)) {
    throw new \agpu_exception('categoryerror', 'error', "$CFG->wwwroot/$CFG->admin/");
}
...
$hassiteconfig = has_capability('agpu/site:config', $PAGE->context);
if ($hassiteconfig) {
...
foreach ($settingspage->children as $childpage) {
    ...
    if ($childpage instanceof admin_externalpage) {
        ...
        $outputhtml .= $OUTPUT->heading(html_writer::link(new agpu_url('/'.$CFG->admin.'/settings.php', array('section' => $childpage->name)), $childpage->visiblename), 3);
        ...
        $outputhtml .= $OUTPUT->heading(html_writer::link(new agpu_url('/'.$CFG->admin.'/category.php', array('category' => $childpage->name)), get_string('admincategory', 'admin', $childpage->visiblename)), 3);
    }
}
if ($savebutton) {
...
$PAGE->set_title(implode(agpu_page::TITLE_SEPARATOR, $settingspage->visiblepath));
...
echo html_writer::input_hidden_params(new agpu_url($PAGE->url, array('sesskey' => sesskey(), 'return' => $return)));
