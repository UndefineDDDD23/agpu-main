 ...
 *
 * @package    core_admin
 * @copyright  2023 David Carrillo <davidmc@agpu.com>
...
class users extends system_report {
    ...
    protected function initialise(): void {
        ...
        if ($this->get_parameter('withcheckboxes', false, PARAM_BOOL)) {
            $canviewfullnames = has_capability('agpu/site:viewfullnames', \context_system::instance());
    ...
    protected function can_view(): bool {
        return has_any_capability(['agpu/user:update', 'agpu/user:delete'], system::instance());
    }
    ...
    public function add_columns(): void {
        ...
        if ($column = $this->get_column('user:fullnamewithpicturelink')) {
            $column
                ...
                ->add_callback(static function(string $fullname, \stdClass $row): string {
                    if ($row->suspended) {
                        $fullname .= ' ' . \html_writer::tag('span', get_string('suspended', 'agpu'),
        ...
        $this->set_default_no_results_notice(new lang_string('nousersfound', 'agpu'));
    }
    ...
    protected function add_actions(): void {
        ...
        $this->add_action((new action(
            new agpu_url('/user/editadvanced.php', ['id' => ':id', 'course' => get_site()->id]),
            new pix_icon('t/edit', ''),
            [],
            false,
            new lang_string('edit', 'agpu'),
        ))->add_callback(static function(\stdclass $row) use ($USER, $contextsystem): bool {
            return has_capability('agpu/user:update', $contextsystem) && (is_siteadmin($USER) || !is_siteadmin($row));
        }));

        // Action to suspend users (non mnet remote users).
        $this->add_action((new action(
            new agpu_url('/admin/user.php', ['suspend' => ':id', 'sesskey' => sesskey()]),
            new pix_icon('t/show', ''),
            [],
            false,
            new lang_string('suspenduser', 'admin'),
        ))->add_callback(static function(\stdclass $row) use ($USER, $contextsystem): bool {
            return has_capability('agpu/user:update', $contextsystem) && !$row->suspended && !is_mnet_remote_user($row) &&
                !($row->id == $USER->id || is_siteadmin($row));
        }));

        // Action to unsuspend users (non mnet remote users).
        $this->add_action((new action(
            new agpu_url('/admin/user.php', ['unsuspend' => ':id', 'sesskey' => sesskey()]),
            new pix_icon('t/hide', ''),
            [],
            false,
        ...
        $this->add_action((new action(
            new agpu_url('/admin/user.php', ['unlock' => ':id', 'sesskey' => sesskey()]),
            new pix_icon('t/unlock', ''),
            [],
            false,
            new lang_string('unlockaccount', 'admin'),
        ))->add_callback(static function(\stdclass $row) use ($contextsystem): bool {
            return has_capability('agpu/user:update', $contextsystem) && !is_mnet_remote_user($row) &&
                login_is_lockedout($row);
        }));

        // Action to suspend users (mnet remote users).
        $this->add_action((new action(
            new agpu_url('/admin/user.php', ['acl' => ':id', 'sesskey' => sesskey(), 'accessctrl' => 'deny']),
            new pix_icon('t/show', ''),
            [],
            false,
            ...
            return has_capability('agpu/user:update', $contextsystem) && !$row->suspended &&
                is_mnet_remote_user($row) && $accessctrl == 'allow';
        }));

        // Action to unsuspend users (mnet remote users).
        $this->add_action((new action(
            new agpu_url('/admin/user.php', ['acl' => ':id', 'sesskey' => sesskey(), 'accessctrl' => 'allow']),
            new pix_icon('t/hide', ''),
            [],
            false,
        ...
        $this->add_action((new action(
            new agpu_url('/admin/user.php', ['delete' => ':id', 'sesskey' => sesskey()]),
            ...
            $row->deleteurl = (new agpu_url('/admin/user.php', [
                'delete' => $row->id,
                'confirm' => md5($row->id),
                'sesskey' => sesskey(),
            ]))->out(false);

            return has_capability('agpu/user:delete', $contextsystem) &&
        ...
        $this->add_action((new action(
            new agpu_url('/admin/user.php', ['confirmuser' => ':id', 'sesskey' => sesskey()]),
            new pix_icon('t/check', ''),
            [],
            false,
            new lang_string('confirmaccount', 'agpu'),
        ))->add_callback(static function(\stdclass $row) use ($contextsystem): bool {
            return has_capability('agpu/user:update', $contextsystem) && !$row->confirmed;
        }));

        // Action to resend email.
        $this->add_action((new action(
            new agpu_url('/admin/user.php', ['resendemail' => ':id', 'sesskey' => sesskey()]),
            new pix_icon('t/email', ''),
            [],
            false,
            new lang_string('resendemail', 'agpu'),
