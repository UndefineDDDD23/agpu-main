 ...
 * from agpu.org be able to check more and more versions.
 *
 * @package    core
 * @subpackage admin
...
if ($action == 'updatecomponent' && confirm_sesskey()) {
    ...
    if ($cd = new component_installer('https://download.agpu.org',
                                      'environment',
                                      'environment.zip')) {
        ...
        switch ($status) {
            case COMPONENT_ERROR:
                if ($cd->get_error() == 'remotedownloaderror') {
                    $a = new stdClass();
                    $a->url  = 'https://download.agpu.org/environment/environment.zip';
                    $a->dest = $CFG->dataroot . '/';
                    throw new \agpu_exception($cd->get_error(), 'error', $PAGE->url, $a);
                    die();

                } else {
                    throw new \agpu_exception($cd->get_error(), 'error', $PAGE->url);
                    die();
                }
...
list($envstatus, $environment_results) = check_agpu_environment($version, ENV_SELECT_NEWER);
