        ...
        if (!empty($form->submit) && $form->submit == get_string('savechanges')) {
            if (in_array($form->mode, array("off", "strict", "dangerous"))) {
                if (set_config('mnet_dispatcher_mode', $form->mode)) {
                    ...
                    throw new \agpu_exception('invalidaction', '', 'index.php');
                }
            }
            ...
            $formcontinue = new single_button(new agpu_url('index.php', array('confirm' => md5($mnet->public_key))), get_string('yes'));
            $formcancel = new single_button(new agpu_url('index.php', array()), get_string('no'));
            ...
            if (!isset($form->confirm)) {
                redirect(
                    new agpu_url('/admin/mnet/index.php'),
            ...
            if ($key != md5(sha1($mnet->keypair['keypair_PEM']))) {
                // fail - you're being attacked?
                throw new \agpu_exception ('deletewrongkeyvalue', 'mnet', 'index.php');
                exit;
            }
