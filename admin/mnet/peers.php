...
if (!empty($updra)) {
    ...
    redirect(new agpu_url('/admin/mnet/peers.php'), get_string('changessaved'));
}
...
if (!function_exists('curl_init') ) {
    throw new \agpu_exception('nocurl', 'mnet');
}
...
if ($formdata = $reviewform->get_data()) {
    ...
    if ($mnet_peer->commit()) {
        redirect(new agpu_url('/admin/mnet/peers.php', array('hostid' => $mnet_peer->id)), get_string('changessaved'));
    } else {
        throw new \agpu_exception('invalidaction', 'error', 'index.php');
    }
...
$registerrow = '';
$registerstr = '';
$registerurl = new agpu_url('/admin/mnet/peers.php', array('updateregisterall' => 1));
...
$baseurl = new agpu_url('/admin/mnet/peers.php');
$deleted = array();
foreach($hosts as $host) {
    $hosturl = new agpu_url($baseurl, array('hostid' => $host->id));
    if (trim($host->name) === '') {
    ...
    $table->data[] = array(
        ...
        $last_connect,
        $OUTPUT->single_button(new agpu_url('/admin/mnet/delete.php', array('hostid' => $host->id)), get_string('delete'))
    );
}
echo html_writer::table($table);
