<?php
/**
 * A service browser for remote agpus
 *
 * This script 'remotely' executes the reflection methods on a remote agpu,
...
if ($CFG->mnet_dispatcher_mode === 'off') {
    throw new \agpu_exception('mnetdisabled', 'mnet');
}
...
if (!extension_loaded('openssl')) {
    throw new \agpu_exception('requiresopenssl', 'mnet', '', null, true);
}
...
$agpuapplicationid = $DB->get_field('mnet_application', 'id', array('name' => 'agpu'));

$url = new agpu_url('/admin/mnet/testclient.php');
$PAGE->set_url($url);
...
foreach ($hosts as $id => $host) {
    ...
    $newurl = new agpu_url($url, array('hostid' => $host->id));
    echo '<p>' . html_writer::link($newurl, $host->wwwroot) . '</p>';
}

if (!empty($hostid) && array_key_exists($hostid, $hosts)) {
    $host = $hosts[$hostid];
    if ($host->applicationid != $agpuapplicationid) {
        echo $OUTPUT->notification(get_string('notagpuapplication', 'mnet'));
    }
    ...
    foreach ($services as $id => $servicedata) {
        ...
        $newurl = new agpu_url($url, array('hostid' => $host->id, 'servicename' => $servicedata['name']));
        $table->data[] = array(
            $servicedata['name'],
    ...
    foreach ($methods as $id => $method) {
        ...
        $newurl = new agpu_url($url, $params);
        $table->data[] = array(
            $method,
