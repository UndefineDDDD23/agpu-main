...
// Allows the admin to control user logins from remote agpus.
...
if (!extension_loaded('openssl')) {
    throw new \agpu_exception('requiresopenssl', 'mnet');
}
...
if (!empty($action) and confirm_sesskey()) {
    ...
    if (!has_capability('agpu/user:delete', $sitecontext)) {
        throw new \agpu_exception('nomodifyacl', 'mnet');
    }

    // fetch the record in question
    $id = required_param('id', PARAM_INT);
    if (!$idrec = $DB->get_record('mnet_sso_access_control', array('id'=>$id))) {
        throw new \agpu_exception('recordnoexists', 'mnet', "$CFG->wwwroot/$CFG->admin/mnet/access_control.php");
    }
    ...
    switch ($action) {
        ...
        case "acl":
            ...
            if ($accessctrl != 'allow' and $accessctrl != 'deny') {
                throw new \agpu_exception('invalidaccessparam', 'mnet', "$CFG->wwwroot/$CFG->admin/mnet/access_control.php");
            }
        ...
        default:
            throw new \agpu_exception('invalidactionparam', 'mnet', "$CFG->wwwroot/$CFG->admin/mnet/access_control.php");
    }
}
...
if ($form = data_submitted() and confirm_sesskey()) {
    ...
    if (!has_capability('agpu/user:delete', $sitecontext)) {
        throw new \agpu_exception('nomodifyacl', 'mnet', "$CFG->wwwroot/$CFG->admin/mnet/access_control.php");
    }
...
if (!empty($table)) {
    ...
    echo '<p>&nbsp;</p>';
    $baseurl = new agpu_url('/admin/mnet/access_control.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage));
