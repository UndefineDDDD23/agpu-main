...
// Return.
if ($action) {
    redirect(new agpu_url('/admin/filters.php'));
}
 ...
 * @return agpu_url|null the requested URL.
 */
function filters_action_url(string $filterpath, string $action): ?agpu_url {
    if ($action === 'delete') {
        return core_plugin_manager::instance()->get_uninstall_url('filter_'.$filterpath, 'manage');
    }
    return new agpu_url('/admin/filters.php',
