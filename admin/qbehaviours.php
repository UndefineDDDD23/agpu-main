...
$thispageurl = new agpu_url('/admin/qbehaviours.php');
...
if (($disable = optional_param('disable', '', PARAM_PLUGIN)) && confirm_sesskey()) {
    if (!isset($behaviours[$disable])) {
        throw new \agpu_exception('unknownbehaviour', 'question', $thispageurl, $disable);
    }
...
if (($enable = optional_param('enable', '', PARAM_PLUGIN)) && confirm_sesskey()) {
    if (!isset($behaviours[$enable])) {
        throw new \agpu_exception('unknownbehaviour', 'question', $thispageurl, $enable);
    }

    if (!$archetypal[$enable]) {
        throw new \agpu_exception('cannotenablebehaviour', 'question', $thispageurl, $enable);
    }
...
if (($up = optional_param('up', '', PARAM_PLUGIN)) && confirm_sesskey()) {
    if (!isset($behaviours[$up])) {
        throw new \agpu_exception('unknownbehaviour', 'question', $thispageurl, $up);
    }
...
if (($down = optional_param('down', '', PARAM_PLUGIN)) && confirm_sesskey()) {
    if (!isset($behaviours[$down])) {
        throw new \agpu_exception('unknownbehaviour', 'question', $thispageurl, $down);
    }
...
function question_behaviour_icon_html($action, $behaviour, $icon, $alt, $tip) {
    global $OUTPUT;
    return $OUTPUT->action_icon(new agpu_url('/admin/qbehaviours.php',
