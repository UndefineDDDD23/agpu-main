...
$url = new agpu_url('/admin/roles/permissions.php', array('contextid' => $contextid));
...
require_capability('agpu/role:review', $context);
...
$allowoverrides     = has_capability('agpu/role:override', $context);
$allowsafeoverrides = has_capability('agpu/role:safeoverride', $context);
    ...
    if (isset($overridableroles[$roleid]) && ($prevent || $unprohibit)) {
        ...
        if ($prevent) {
            if ($confirm && data_submitted() && confirm_sesskey()) {
                ...
                $continueurl = new agpu_url($PAGE->url,
...
if ($overridableroles) {
    $overrideurl = new agpu_url('/admin/roles/override.php', array('contextid' => $context->id));
