...
class provider_test extends provider_testcase {
    ...
    public function test_get_contexts_for_userid(): void {
        ...
        // Role capabilities.
        $this->setUser($user);
        $result = assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $student->id, $cmcontext->id);
    ...
    public function test_export_user_data(): void {
        ...
        assign_capability('agpu/backup:backupactivity', CAP_ALLOW, $student->id, $cmcontext->id, true);
        assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $student->id, $coursecontext->id, true);
        assign_capability('agpu/category:manage', CAP_ALLOW, $student->id, $coursecatcontext->id, true);
        assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $student->id, $systemcontext->id, true);
        assign_capability('agpu/block:edit', CAP_ALLOW, $student->id, $blockcontext->id, true);
        assign_capability('agpu/competency:evidencedelete', CAP_ALLOW, $student->id, $usercontext2->id, true);
        ...
        foreach ($contextlist as $context) {
            ...
            if ($context->contextlevel == CONTEXT_MODULE) {
                ...
                if ($data = (array)$writer->get_data($subcontextrc)) {
                    $this->assertEquals('agpu/backup:backupactivity', reset($data)->capability);
            ...
            if ($context->contextlevel == CONTEXT_COURSE) {
                ...
                if ($data = (array)$writer->get_data($subcontextrc)) {
                    $this->assertEquals('agpu/backup:backupcourse', reset($data)->capability);
                }
            }
            if ($context->contextlevel == CONTEXT_COURSECAT) {
                ...
                if ($data = (array)$writer->get_data($subcontextrc)) {
                    $this->assertEquals('agpu/category:manage', reset($data)->capability);
                }
            }
            ...
            if ($context->contextlevel == CONTEXT_BLOCK) {
                ...
                if ($data = (array)$writer->get_data($subcontextrc)) {
                    $this->assertEquals('agpu/block:edit', reset($data)->capability);
                }
            }
            if ($context->contextlevel == CONTEXT_USER) {
                ...
                if ($data = (array)$writer->get_data($subcontextrc)) {
                    $this->assertEquals('agpu/competency:evidencedelete', reset($data)->capability);
                }
            }
        }
    }
    ...
    public function test_get_users_in_context(): void {
        ...
        // Role capabilities.
        $this->setUser($user1);
        assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $studentrole->id, $cmcontext->id);
