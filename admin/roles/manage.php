...
switch ($action) {
    case 'delete':
        if (isset($undeletableroles[$roleid])) {
            throw new \agpu_exception('cannotdeletethisrole', '', $baseurl);
        }
        if (!$confirmed) {
            // Show confirmation.
            echo $OUTPUT->header();
            ...
            $formcontinue = new single_button(new agpu_url($baseurl, $optionsyes), get_string('yes'));
            $formcancel = new single_button(new agpu_url($baseurl), get_string('no'), 'get');
        ...
        if (!delete_role($roleid)) {
            // The delete failed.
            throw new \agpu_exception('cannotdeleterolewithid', 'error', $baseurl, $roleid);
        }
    ...
    case 'moveup':
        if (confirm_sesskey()) {
            ...
            if (is_null($thisrole) || is_null($prevrole)) {
                throw new \agpu_exception('cannotmoverolewithid', 'error', '', $roleid);
            }
...
echo $OUTPUT->single_button(new agpu_url($defineurl, array('action' => 'add')), get_string('addrole', 'core_role'), 'get');
