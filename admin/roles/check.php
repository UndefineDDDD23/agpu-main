...
$url = new agpu_url('/admin/roles/check.php', array('contextid' => $contextid));
...
if (!has_any_capability(array('agpu/role:assign', 'agpu/role:safeoverride', 'agpu/role:override', 'agpu/role:manage'), $context)) {
    throw new \agpu_exception('nopermissions', 'error', '', get_string('checkpermissions', 'core_role'));
}

navigation_node::override_active_url($url);
$pageurl = new agpu_url($url);
if ($returnurl) {
...
// Admins (people with agpu/role:manage) can run this report for any user.
...
if (!is_null($reportuser)) {
    ...
    if (!empty($roleassignments)) {
        ...
        foreach ($roleassignments as $ra) {
            ...
            if (has_capability('agpu/role:manage', $systemcontext)) {
                $rolename = html_writer::link(new agpu_url('/admin/roles/define.php',
...
if (!$PAGE->has_secondary_navigation() && $context->contextlevel > CONTEXT_USER) {
    ...
    if ($returnurl) {
        $backurl = new agpu_url($returnurl);
    } else {
