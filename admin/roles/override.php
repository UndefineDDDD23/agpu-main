...
$url = new agpu_url('/admin/roles/override.php', array('contextid' => $contextid, 'roleid' => $roleid));
...
if (!has_capability('agpu/role:override', $context)) {
    require_capability('agpu/role:safeoverride', $context);
...
if ($context->contextlevel == CONTEXT_USER and $USER->id != $context->instanceid) {
    ...
    navigation_node::override_active_url(new agpu_url('/admin/roles/permissions.php',
        array('contextid'=>$context->id, 'userid'=>$context->instanceid, 'courseid'=>$course->id)));

} else {
    $PAGE->set_context($context);
    navigation_node::override_active_url(new agpu_url('/admin/roles/permissions.php', array('contextid'=>$context->id)));
}

$courseid = $course->id;

$returnurl = new agpu_url('/admin/roles/permissions.php', array('contextid' => $context->id));
...
switch ($context->contextlevel) {
    case CONTEXT_SYSTEM:
        throw new \agpu_exception('cannotoverridebaserole', 'error');
        break;
    case CONTEXT_USER:
...
if (empty($overridableroles[$roleid])) {
    ...
    throw new \agpu_exception('cannotoverriderolehere', '', $context->get_url(), $a);
}
