...
$url = new agpu_url('/admin/roles/assign.php', array('contextid' => $contextid));
...
if ($roleid && !isset($assignableroles[$roleid])) {
    ...
    throw new \agpu_exception('cannotassignrolehere', '', $context->get_url(), $a);
}
...
if ($roleid) {
    ...
    $backurl = new agpu_url('/enrol/otherusers.php', ['id' => $course->id]);
} else if ($returnurl) {
...
if ($roleid) {
    ...
    // Print the form.
    $assignurl = new agpu_url($PAGE->url, array('roleid'=>$roleid));
?>
