...
if ($hassiteconfig or has_any_capability($capabilities, $systemcontext)) { // speedup for non-admins, add all caps used on this page
    ...
    // blog
    $temp = new admin_settingpage('blog', new lang_string('blog','blog'), 'agpu/site:config', empty($CFG->enableblogs));
    ...
    if (!isset($CFG->enabledashboard) || $CFG->enabledashboard) {
        $choices[HOMEPAGE_MY] = new lang_string('myagpu', 'admin');
    }
    ...
    if (!isset($CFG->enabledashboard) || $CFG->enabledashboard) {
        $temp->add(new admin_setting_configcheckbox(
            'allowguestmyagpu',
            new lang_string('allowguestmyagpu', 'admin'),
            new lang_string('configallowguestmyagpu', 'admin'),
            1
        ));
    }
    ...
    $ADMIN->add('appearance', new admin_externalpage('resetemoticons', new lang_string('emoticonsreset', 'admin'),
        new agpu_url('/admin/resetemoticons.php'), 'agpu/site:config', true));

    // "documentation" settingpage
    $temp = new admin_settingpage('documentation', new lang_string('agpudocs'));
    $temp->add(new admin_setting_configtext('docroot', new lang_string('docroot', 'admin'), new lang_string('configdocroot', 'admin'), 'https://docs.agpu.org', PARAM_URL));
    ...
    $temp->add(new admin_setting_configtext(
        ...
        'https://agpu.academy/coursequickstart',
        PARAM_URL
    ));
    $ADMIN->add('appearance', $temp);
    ...
    $url = new agpu_url('/tag/manage.php');
    $hidden = empty($CFG->usetags);
    $page = new admin_externalpage('managetags', new lang_string('managetags', 'tag'), $url, 'agpu/tag:manage', $hidden);
    ...
    $defaultsettingcustomusermenuitems = [
        'profile,agpu|/user/profile.php',
        'grades,grades|/grade/report/mygrades.php',
        'calendar,core_calendar|/calendar/view.php?view=month',
        'privatefiles,agpu|/user/files.php',
