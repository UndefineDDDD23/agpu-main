...
if ($hassiteconfig) {
    ...
    $temp = new admin_externalpage('authtestsettings', get_string('testsettings', 'core_auth'), new agpu_url("/auth/test_settings.php"), 'agpu/site:config', true);
    $ADMIN->add('authsettings', $temp);
    ...
    $temp = new admin_externalpage('enroltestsettings', get_string('testsettings', 'core_enrol'), new agpu_url("/enrol/test_settings.php"), 'agpu/site:config', true);
    $ADMIN->add('enrolments', $temp);
...
/// Editor plugins
    ...
    $url = new agpu_url('/report/status/index.php');
...
/// Filter plugins
    ...
    $temp->add(new admin_setting_description(
        ...
        new lang_string('gotopaymentaccounts', 'payment',
            html_writer::link(new agpu_url('/payment/accounts.php'), get_string('paymentaccounts', 'payment')))
    ));
    ...
    $ADMIN->add('portfoliosettings', $temp);
    $ADMIN->add('portfoliosettings', new admin_externalpage('portfolionew', new lang_string('addnewportfolio', 'portfolio'), $url, 'agpu/site:config', true));
    $ADMIN->add('portfoliosettings', new admin_externalpage('portfoliodelete', new lang_string('deleteportfolio', 'portfolio'), $url, 'agpu/site:config', true));
    $ADMIN->add('portfoliosettings', new admin_externalpage('portfoliocontroller', new lang_string('manageportfolios', 'portfolio'), $url, 'agpu/site:config', true));
    ...
    $ADMIN->add('repositorysettings', new admin_externalpage('repositorynew',
        new lang_string('addplugin', 'repository'), $url, 'agpu/site:config', true));
    $ADMIN->add('repositorysettings', new admin_externalpage('repositorydelete',
        new lang_string('deleterepository', 'repository'), $url, 'agpu/site:config', true));
    $ADMIN->add('repositorysettings', new admin_externalpage('repositorycontroller',
        new lang_string('manage', 'repository'), $url, 'agpu/site:config', true));
    $ADMIN->add('repositorysettings', new admin_externalpage('repositoryinstancenew',
        new lang_string('createrepository', 'repository'), $url, 'agpu/site:config', true));
    $ADMIN->add('repositorysettings', new admin_externalpage('repositoryinstanceedit',
        new lang_string('editrepositoryinstance', 'repository'), $url, 'agpu/site:config', true));
    ...
    foreach (core_component::get_plugin_list('coursereport') as $report => $path) {
        ...
        if (file_exists($file)) {
            $settings = new admin_settingpage('coursereport' . $report,
                    new lang_string('pluginname', 'coursereport_' . $report), 'agpu/site:config');
...
foreach (core_component::get_plugin_list('report') as $report => $plugindir) {
    ...
    if (file_exists($settings_path)) {
        $settings = new admin_settingpage('report' . $report,
                new lang_string('pluginname', 'report_' . $report), 'agpu/site:config');
...
if ($hassiteconfig) {
    ...
    foreach (core_component::get_plugin_list('search') as $engine => $plugindir) {
        ...
        if (file_exists($settingspath)) {
            $settings = new admin_settingpage('search' . $engine,
                    new lang_string('pluginname', 'search_' . $engine), 'agpu/site:config');
    ...
    $ADMIN->add('searchplugins', new admin_externalpage('searchareas', new lang_string('searchareas', 'admin'),
        new agpu_url('/admin/searchareas.php')));
...
if ($hassiteconfig) {
    ...
    foreach (core_component::get_plugin_list('cachestore') as $plugin => $path) {
        ...
        if (file_exists($settingspath)) {
            $settings = new admin_settingpage('cachestore_'.$plugin.'_settings', new lang_string('pluginname', 'cachestore_'.$plugin), 'agpu/site:config');
            include($settingspath);
