...
if ($hassiteconfig
 or has_capability('agpu/user:create', $systemcontext)
 or has_capability('agpu/user:update', $systemcontext)
 or has_capability('agpu/user:delete', $systemcontext)
 or has_capability('agpu/role:manage', $systemcontext)
 or has_capability('agpu/role:assign', $systemcontext)
 or has_capability('agpu/cohort:manage', $systemcontext)
 or has_capability('agpu/cohort:view', $systemcontext)) { // Speedup for non-admins, add all caps used on this page.


    // Stuff under the "accounts" subcategory.
    $ADMIN->add('accounts', new admin_externalpage('editusers', new lang_string('userlist','admin'), "$CFG->wwwroot/$CFG->admin/user.php", array('agpu/user:update', 'agpu/user:delete')));
    $ADMIN->add('accounts', new admin_externalpage('userbulk', new lang_string('userbulk','admin'), "$CFG->wwwroot/$CFG->admin/user/user_bulk.php", array('agpu/user:update', 'agpu/user:delete')));
    $ADMIN->add('accounts', new admin_externalpage('addnewuser', new lang_string('addnewuser'), "$CFG->wwwroot/user/editadvanced.php?id=-1", 'agpu/user:create'));
    ...
    $ADMIN->add('accounts', new admin_externalpage('profilefields', new lang_string('profilefields','admin'), "$CFG->wwwroot/user/profile/index.php", 'agpu/site:config'));
    $ADMIN->add('accounts', new admin_externalpage('cohorts', new lang_string('cohorts', 'cohort'), $CFG->wwwroot . '/cohort/index.php', array('agpu/cohort:manage', 'agpu/cohort:view')));
    $ADMIN->add(
        'accounts',
        new admin_externalpage(
            ...
            ['agpu/cohort:configurecustomfields']
        )
    );
    ...
    if ($ADMIN->fulltree) {
        ...
        $temp->add(new admin_setting_configmultiselect('hiddenuserfields', new lang_string('hiddenuserfields', 'admin'),
                   new lang_string('confighiddenuserfields', 'admin'), array(),
                       array('description' => new lang_string('description'),
                             ...
                             'agpunetprofile' => new lang_string('agpunetprofile', 'user'),
        ...
        // with agpu/site:viewuseridentity).
    ...
    $ADMIN->add('roles', new admin_externalpage('defineroles', new lang_string('defineroles', 'role'), "$CFG->wwwroot/$CFG->admin/roles/manage.php", 'agpu/role:manage'));
    $ADMIN->add('roles', new admin_externalpage('assignroles', new lang_string('assignglobalroles', 'role'), "$CFG->wwwroot/$CFG->admin/roles/assign.php?contextid=".$systemcontext->id, 'agpu/role:assign'));
    $ADMIN->add('roles', new admin_externalpage('checkpermissions', new lang_string('checkglobalpermissions', 'role'), "$CFG->wwwroot/$CFG->admin/roles/check.php?contextid=".$systemcontext->id, array('agpu/role:assign', 'agpu/role:safeoverride', 'agpu/role:override', 'agpu/role:manage')));
...
if ($hassiteconfig) {
    ...
    $url = new agpu_url('/admin/settings.php?section=supportcontact');
    $url = $url->out();
