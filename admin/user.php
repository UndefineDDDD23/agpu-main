<?php
    ...
    $returnurl = new agpu_url('/admin/user.php');
    ...
    if ($confirmuser and confirm_sesskey()) {
        ...
        if ($user->confirmed) {
            throw new \agpu_exception('alreadyconfirmed', 'agpu');
        }
        ...
        require_capability('agpu/user:delete', $sitecontext);

        $user = $DB->get_record('user', array('id'=>$delete, 'mnethostid'=>$CFG->mnet_localhost_id), '*', MUST_EXIST);

        if ($user->deleted) {
            throw new \agpu_exception('usernotdeleteddeleted', 'error');
        }
        ...
        if ($confirm != md5($delete)) {
            ...
            $deleteurl = new agpu_url($returnurl, $optionsyes);
        ...
        if (!has_capability('agpu/user:update', $sitecontext)) {
            throw new \agpu_exception('nopermissions', 'error', '', 'modify the NMET access control list');
        }
        if (!$user = $DB->get_record('user', array('id'=>$acl))) {
            throw new \agpu_exception('nousers', 'error');
        }
        if (!is_mnet_remote_user($user)) {
            throw new \agpu_exception('usermustbemnet', 'error');
        }
        $accessctrl = strtolower(required_param('accessctrl', PARAM_ALPHA));
        if ($accessctrl != 'allow' and $accessctrl != 'deny') {
            throw new \agpu_exception('invalidaccessparameter', 'error');
        }
    ...
    if (has_capability('agpu/user:create', $sitecontext)) {
        echo html_writer::start_div('d-flex mb-2');
        $url = new agpu_url('/user/editadvanced.php', ['id' => -1]);
        echo html_writer::link($url, get_string('addnewuser', 'agpu'), [
    ...
    $bulkactions = new user_bulk_action_form(new agpu_url('/admin/user/user_bulk.php'),
