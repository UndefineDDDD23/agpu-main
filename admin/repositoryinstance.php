...
admin_externalpage_setup($pagename, '', null, new agpu_url('/admin/repositoryinstance.php'));

// The URL used for redirection, and that all edit related URLs will be based off.
$parenturl = new agpu_url('/admin/repository.php', ['action' => 'edit']);
...
if (!empty($edit) || !empty($new)) {
    ...
    if ($mform->is_cancelled()){
        ...
        if ($success) {
            ...
            throw new \agpu_exception('instancenotsaved', 'repository', $parenturl);
        }
        exit;
    } else {
        echo $OUTPUT->header();
    ...
    if ($sure) {
        ...
        if ($instance->delete($downloadcontents)) {
            ...
            throw new \agpu_exception('instancenotdeleted', 'repository', $parenturl);
        }
        exit;
    }
    ...
    $continueanddownloadurl = new agpu_url($continueurl, array(
        'downloadcontents' => 1
    ));
