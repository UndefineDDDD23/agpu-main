...
if ($action) {
    ...
    if ($areaid) {
        ...
        if ($area === false) {
            throw new agpu_exception('invalidrequest');
        }
    }
    ...
    if (in_array($action, ['delete', 'indexall', 'reindexall', 'deleteall'])) {
        ...
        if ($indexingenabled) {
            ...
            if (!optional_param('confirm', 0, PARAM_INT)) {
                ...
                $actionurl = new agpu_url('/admin/searchareas.php', $actionparams);
                $cancelurl = new agpu_url('/admin/searchareas.php');
                echo $OUTPUT->header();
                ...
                redirect(new agpu_url('/admin/searchareas.php'));
            }
        }
...
if (isset($searchmanager) && $indexingenabled) {
    ...
    $params = (object) [
        'url' => (new agpu_url("/admin/settings.php?section=manageglobalsearch#admin-searchindexwhendisabled"))->out(false)
    ];
...
foreach ($searchareas as $area) {
    ...
    if ($area->is_enabled()) {
        $columns[] = $OUTPUT->action_icon(admin_searcharea_action_url('disable', $areaid),
            new pix_icon('t/hide', get_string('disable'), 'agpu', array('title' => '', 'class' => 'iconsmall')),
        ...
        if ($areasconfig && $indexingenabled) {
            ...
            if ($area->supports_get_document_recordset()) {
                $actions[] = $OUTPUT->pix_icon('i/reload', '') . html_writer::link(
                        new agpu_url('searchreindex.php', ['areaid' => $areaid]),
        ...
        $columns[] = $OUTPUT->action_icon(admin_searcharea_action_url('enable', $areaid),
            new pix_icon('t/show', get_string('enable'), 'agpu', array('title' => '', 'class' => 'iconsmall')),
...
function admin_searcharea_action_url($action, $areaid = false) {
    ...
    return new agpu_url('/admin/searchareas.php', $params);
}
