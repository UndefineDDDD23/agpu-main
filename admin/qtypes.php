...
require_capability('agpu/question:config', $systemcontext);
$canviewreports = has_capability('report/questioninstances:view', $systemcontext);

$thispageurl = new agpu_url('/admin/qtypes.php');
...
if (($disable = optional_param('disable', '', PARAM_PLUGIN)) && confirm_sesskey()) {
    if (!isset($qtypes[$disable])) {
        throw new \agpu_exception('unknownquestiontype', 'question', $thispageurl, $disable);
    }
...
if (($enable = optional_param('enable', '', PARAM_PLUGIN)) && confirm_sesskey()) {
    if (!isset($qtypes[$enable])) {
        throw new \agpu_exception('unknownquestiontype', 'question', $thispageurl, $enable);
    }

    if (!$qtypes[$enable]->menu_name()) {
        throw new \agpu_exception('cannotenable', 'question', $thispageurl, $enable);
    }
...
if (($up = optional_param('up', '', PARAM_PLUGIN)) && confirm_sesskey()) {
    if (!isset($qtypes[$up])) {
        throw new \agpu_exception('unknownquestiontype', 'question', $thispageurl, $up);
    }
...
if (($down = optional_param('down', '', PARAM_PLUGIN)) && confirm_sesskey()) {
    if (!isset($qtypes[$down])) {
        throw new \agpu_exception('unknownquestiontype', 'question', $thispageurl, $down);
    }
...
foreach ($sortedqtypes as $qtypename => $localname) {
    ...
    if ($counts[$qtypename]->numquestions + $counts[$qtypename]->numhidden + $counts[$qtypename]->numdraft > 0) {
        ...
        if ($canviewreports) {
            $row[] = html_writer::link(new agpu_url('/report/questioninstances/index.php',
    ...
    if ($settings instanceof admin_externalpage) {
        ...
        $row[] = html_writer::link(new agpu_url('/admin/settings.php',
...
function question_type_icon_html($action, $qtypename, $icon, $alt, $tip) {
    global $OUTPUT;
    return $OUTPUT->action_icon(new agpu_url('/admin/qtypes.php',
            array($action => $qtypename, 'sesskey' => sesskey())),
            new pix_icon($icon, $alt, 'agpu', array('title' => '', 'class' => 'iconsmall')),
