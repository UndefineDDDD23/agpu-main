...
$url = new agpu_url('/backup/copy.php', array('id' => $courseid));
...
if ($returnurl != '') {
    ...
    $returnurl = new agpu_url('/course/management.php', array('categoryid' => $course->category));
} else {
    // Redirect back to course page if we came from there.
    $returnurl = new agpu_url('/course/view.php', array('id' => $courseid));
}
...
if ($mform->is_cancelled()) {
    ...
    if (!empty($mdata->submitdisplay)) {
        ...
        $progressurl = new agpu_url('/backup/copyprogress.php', array('id' => $courseid));
        redirect($progressurl);
    } else {
        // Redirect to the course view page.
        $coursesurl = new agpu_url('/course/view.php', array('id' => $courseid));
        redirect($coursesurl);
    }
