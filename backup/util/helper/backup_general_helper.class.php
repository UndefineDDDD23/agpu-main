...
abstract class backup_general_helper extends backup_helper {
    ...
    /**
     * Load all the blocks information needed for a given path within agpu2 backup
     *
    ...
    /**
     * Load and format all the needed information from agpu_backup.xml
     *
     * This function loads and process all the agpu_backup.xml
    ...
    public static function get_backup_information($tempdir) {
        ...
        $agpufile = $backuptempdir . '/' . $tempdir . '/agpu_backup.xml';
        if (!file_exists($agpufile)) { // Shouldn't happen ever, but...
            throw new backup_helper_exception('missing_agpu_backup_xml_file', $agpufile);
        }

        $agpufilesha1 = sha1_file($agpufile);
        if ($agpufilesha1 === $cachesha1) {
            return clone $cache;
        }

        // Load the entire file to in-memory array
        $xmlparser = new progressive_parser();
        $xmlparser->set_file($agpufile);
        $xmlprocessor = new restore_agpuxml_parser_processor();
        $xmlparser->set_processor($xmlprocessor);
        $xmlparser->process();
        $infoarr = $xmlprocessor->get_all_chunks();
        if (count($infoarr) !== 1) { // Shouldn't happen ever, but...
            throw new backup_helper_exception('problem_parsing_agpu_backup_xml_file');
        }
        $infoarr = $infoarr[0]['tags']; // for commodity

        // Let's build info
        $info->agpu_version = $infoarr['agpu_version'];
        $info->agpu_release = $infoarr['agpu_release'];
        ...
        // agpu backup file don't have this option before 2.3
     ...
     *
     * This will only extract the agpu_backup.xml file from an MBZ
    ...
    public static function get_backup_information_from_mbz($filepath, ?file_progress $progress = null) {
        ...
        if (!is_readable($filepath)) {
            throw new backup_helper_exception('missing_agpu_backup_file', $filepath);
        }

        // Extract agpu_backup.xml.
        ...
        $extracted = $fp->extract_to_pathname($filepath, $tmpdir, array('agpu_backup.xml'), $progress);
        $agpufile =  $tmpdir . '/' . 'agpu_backup.xml';
        if (!$extracted || !is_readable($agpufile)) {
            throw new backup_helper_exception('missing_agpu_backup_xml_file', $agpufile);
        }
    ...
    /**
     * Given the information fetched from agpu_backup.xml file
    ...
    public static function detect_backup_format($tempdir) {
        ...
        if (convert_helper::detect_agpu2_format($tempdir)) {
            return backup::FORMAT_agpu;
        }
