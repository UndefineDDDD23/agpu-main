...
abstract class convert_helper {
    ...
    /**
     * Detects if the given folder contains an unpacked agpu2 backup
     *
     * @param string $tempdir the name of the backup directory
     * @return boolean true if agpu2 format detected, false otherwise
     */
    public static function detect_agpu2_format($tempdir) {
        ...
        $filepath = $dirpath . '/agpu_backup.xml';
        ...
        if (stripos($firstchars, '<?xml version="1.0" encoding="UTF-8"?>') !== false &&
            strpos($firstchars, '<agpu_backup>') !== false &&
    ...
    /**
     * Converts the given directory with the backup into agpu2 format
     *
     ...
     */
    public static function to_agpu2_format($tempdir, $format = null, $logger = null) {
        ...
        if ($logger instanceof base_logger) {
            backup_helper::log('conversion path established', backup::LOG_INFO,
                implode(' => ', array_merge($path, array('agpu2'))), 0, false, $logger);
        }
        ...
        // make sure we ended with agpu2 format
        if (!self::detect_agpu2_format($tempdir)) {
    ...
    */
     ...
     * the target agpu2 format. The method returns a list of converters that should be
     * called, in order.
     *
     ...
     * @author David Mudrak <david@agpu.com>
    ...
    protected static function choose_conversion_path($format, array $descriptions) {
        ...
        if (is_null($dist[backup::FORMAT_agpu])) {
 ...
 * @author David Mudrak <david@agpu.com>
 */
class convert_helper_exception extends agpu_exception {
