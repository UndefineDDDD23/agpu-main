...
abstract class restore_dbops {
     ...
     *
     * Rule2: The 'agpu/question:managecategory' and 'agpu/question:add' capabilities will be
    ...
    public static function prechek_precheck_qbanks_by_level($restoreid, $courseid, $userid, $samesite, $contextlevel) {
        ...
        // 8) Check if backup is from agpu >= 3.5 and error if more than one top-level category in the context.
        ...
        foreach ($contexts as $contextid => $contextlevel) {
            ...
            if ($targetcontext = self::restore_find_best_target_context($categories, $courseid, $contextlevel)) {
                $canmanagecategory = has_capability('agpu/question:managecategory', $targetcontext, $userid);
                $canadd            = has_capability('agpu/question:add', $targetcontext, $userid);
            }
            // 1) Iterate over each qcat in the context, matching by stamp for the found target context
            ...
            // 8) Check if backup is made on agpu >= 3.5 and there are more than one top-level category in the context.
    ...
    public static function send_files_to_pool($basepath, $restoreid, $component, $filearea,
        ...
        $fs = get_file_storage();         // Get agpu file storage
        ...
        foreach ($rs as $rec) {
            ...
            if (empty($file->repositoryid)) {
                ...
                if ($includesfiles) {
                    ...
                    // This backup does not include the files - they should be available in agpu filestorage already.
    ...
    public static function create_included_users($basepath, $restoreid, $userid,
        ...
        foreach ($rs as $recuser) {
            ...
            if (empty($user->password)) { // Only if restore comes without password
                ...
                // If agpu is responsible for storing/validating pwd and reset functionality is available, mark
    ...
    */
    ...
    public static function precheck_included_users($restoreid, $courseid, $userid, $samesite,
        ...
        // to 'agpu/restore:createuser' and 'agpu/restore:userinfo'
        // and also observe $CFG->disableusercreationonrestore
        $cancreateuser = false;
        if (has_capability('agpu/restore:createuser', $context, $userid) and
            has_capability('agpu/restore:userinfo', $context, $userid) and
