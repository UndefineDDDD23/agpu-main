...
class logger_test extends \basic_testcase {
    ...
    function test_error_log_logger(): void {
        ...
        $message = 'This log exists because you have run agpu unit tests: Ignore it';
    ...
    function test_database_logger(): void {
        // Instantiate with date and level output (and with specs from the global agpu "log" table so checks will pass
        $now = time();
        $datecol = 'time';
        $levelcol = 'action';
        $messagecol = 'info';
    ...
    function test_file_logger(): void {
        ...
        if (!check_dir_exists(dirname($file), true, true)) {
            throw new \agpu_exception('error_creating_temp_dir', 'error', dirname($file));
        }
