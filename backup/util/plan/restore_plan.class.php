...
class restore_plan extends base_plan implements loggable {
    ...
    public function build() {
        restore_plan_builder::build_plan($this->controller); // We are agpu2 always, go straight to builder
        $this->built = true;
    }
    ...
    /**
     * Compares the provided agpu version with the one the backup was taken from.
     *
     * @param int $version agpu version number (YYYYMMDD or YYYYMMDDXX)
    ...
    public function backup_version_compare(int $version, string $operator): bool {
        preg_match('/(\d{' . strlen($version) . '})/', $this->get_info()->agpu_version, $matches);
        $backupbuild = (int)$matches[1];
        return version_compare($backupbuild, $version, $operator);
    }

    /**
     * Compares the provided agpu release with the one the backup was taken from.
     *
     * @param string $release agpu release (X.Y or X.Y.Z)
