 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class copy_form extends \agpuform {

    /**
     * Build form for the course copy settings.
     *
     * {@inheritDoc}
     * @see \agpuform::definition()
     */
    public function definition() {
        ...
        if (!empty($copies)) {
            $progresslink = new \agpu_url('/backup/copyprogress.php?', array('id' => $course->id));
        ...
        $requiredcapabilities = array(
            'agpu/restore:createuser', 'agpu/backup:userinfo', 'agpu/restore:userinfo'
        );
        ...
        if (!empty($roles) && has_capability('agpu/restore:createuser', $coursecontext)) {
            $rolearray = array();
