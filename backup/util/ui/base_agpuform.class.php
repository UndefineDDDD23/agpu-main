 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class base_agpuform extends agpuform {
        ...
        if ($setting->get_visibility() == backup_setting::VISIBLE) {
            ...
            switch ($setting->get_status()) {
                case backup_setting::LOCKED_BY_PERMISSION:
                    $icon = ' '.$OUTPUT->pix_icon('i/permissionlock', get_string('lockedbypermission', 'backup'), 'agpu',
                            array('class' => 'smallicon lockedicon permissionlock'));
                    break;
                case backup_setting::LOCKED_BY_CONFIG:
                    $icon = ' '.$OUTPUT->pix_icon('i/configlock', get_string('lockedbyconfig', 'backup'), 'agpu',
                            array('class' => 'smallicon lockedicon configlock'));
                    break;
                case backup_setting::LOCKED_BY_HIERARCHY:
                    $icon = ' '.$OUTPUT->pix_icon('i/hierarchylock', get_string('lockedbyhierarchy', 'backup'), 'agpu',
                            array('class' => 'smallicon lockedicon configlock'));
                    break;
                default:
                    $icon = '';
     ...
     *
     * @param string $elementname
     * @return HTML_QuickForm_input|agpuQuickForm_group
     */
    ...
    public function display() {
        ...
        $PAGE->requires->strings_for_js(array('select', 'all', 'none'), 'agpu');
