...
require_once($CFG->dirroot . '/backup/agpu2/backup_plan_builder.class.php');
...
class core_backup_renderer extends plugin_renderer_base {
    ...
    public function backup_details($details, $nextstageurl) {
        ...
        $html .= $this->backup_detail_pair(get_string('agpuversion', 'backup'),
                html_writer::tag('span', s($details->agpu_release), array('class' => 'agpu_release')).
                html_writer::tag('span', '[' . s($details->agpu_version) .']', array('class' => 'agpu_version sub-detail')));
        $html .= $this->backup_detail_pair(get_string('backupversion', 'backup'),
                html_writer::tag('span', s($details->backup_release), array('class' => 'agpu_release')).
                html_writer::tag('span', '[' . s($details->backup_version) . ']', array('class' => 'agpu_version sub-detail')));
     ...
     *
     * @param agpu_url $nextstageurl URL to send user to
     ...
     * @return string HTML code to display
     */
    public function backup_details_unknown(agpu_url $nextstageurl) {
     ...
     * @return string
     */
    public function course_selector(agpu_url $nextstageurl, $wholecourse = true, ?restore_category_search $categories = null,
     ...
     * @return string
     */
    public function import_course_selector(agpu_url $nextstageurl, ?import_course_search $courses = null) {
    ...
    public function render_backup_files_viewer(backup_files_viewer $viewer) {
        ...
        if ($filestodisplay || $async) {
            ...
            foreach ($files as $file) {
                ...
                $restoreurl = new agpu_url('/backup/restorefile.php', $params);
                ...
                if ($params['filearea'] == 'automated') {
                    if (!has_capability('agpu/restore:viewautomatedfilearea', $viewer->currentcontext)) {
                        $restorelink = '';
                    }
        ...
        if ($canmanagebackups) {
            $html .= $this->output->single_button(
                new agpu_url('/backup/backupfilesedit.php', array(
     ...
     *
     * @param int $userid The agpu user id.
     * @param \context $context The agpu context for these restores.
    ...
    public function copy_progress_viewer(int $userid, int $courseid): string {
        ...
        foreach ($copies as $copy) {
            $sourceurl = new \agpu_url('/course/view.php', array('id' => $copy->sourceid));
