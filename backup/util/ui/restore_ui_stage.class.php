 ...
 *  - initialise_stage_form : To get a restore_agpuform instance for the stage
 *
 * @package   core_backup
 * @copyright 2010 Sam Hemelryk
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
    ...
    public function get_progress_bar() {
        ...
        while ($stage > 0) {
            ...
            if ($stage < $currentstage && $currentstage < restore_ui::STAGE_COMPLETE) {
                ...
                // $item['link'] = new agpu_url($PAGE->url, array('restore' => $this->get_restoreid(), 'stage' => $stage));
            }
...
class restore_ui_stage_confirm extends restore_ui_independent_stage implements file_progress {
    ...
    protected function extract_file_to_dir($source) {
        ...
        $fb = get_file_packer('application/vnd.agpu.backup');
    ...
    public function display(core_backup_renderer $renderer) {
        ...
        $prevstageurl = new agpu_url('/backup/restorefile.php', array('contextid' => $this->contextid));
        $nextstageurl = new agpu_url('/backup/restore.php', array(
        ...
        } else if ($format !== backup::FORMAT_agpu) {
...
class restore_ui_stage_destination extends restore_ui_independent_stage {
    ...
    public function __construct($contextid) {
        ...
        $url = new agpu_url($PAGE->url, array(
            'filepath' => $this->filepath,
            'contextid' => $this->contextid,
            'stage' => restore_ui::STAGE_DESTINATION));
    ...
    public function display(core_backup_renderer $renderer) {
        ...
        if ($format === backup::FORMAT_agpu) {
            // Standard agpu 2 format, let use get the type of the backup.
        ...
        if ($context->contextlevel == CONTEXT_COURSE and has_capability('agpu/restore:restorecourse', $context)) {
...
class restore_ui_stage_settings extends restore_ui_stage {
     ...
     *
     * @return backup_agpuform|base_agpuform|restore_settings_form
     * @throws coding_exception
     */
...
class restore_ui_stage_process extends restore_ui_stage {
    ...
    public function process(?base_agpuform $form = null) {
        if (optional_param('cancel', false, PARAM_BOOL)) {
            redirect(new agpu_url('/course/view.php', array('id' => $this->get_ui()->get_controller()->get_courseid())));
        }
