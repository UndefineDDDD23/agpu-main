    ...
    public static function check_security($restore_controller, $apply) {
        ...
        switch ($type) {
            case backup::TYPE_1COURSE :
                $typecapstocheck['agpu/restore:restorecourse'] = $coursectx;
                break;
            case backup::TYPE_1SECTION :
                $typecapstocheck['agpu/restore:restoresection'] = $coursectx;
                break;
            case backup::TYPE_1ACTIVITY :
                $typecapstocheck['agpu/restore:restoreactivity'] = $coursectx;
                break;
            default :
        ...
        switch ($mode) {
            case backup::MODE_IMPORT:
                if (!has_capability('agpu/restore:restoretargetimport', $coursectx, $userid)) {
                    $a = new stdclass();
                    $a->userid = $userid;
                    $a->courseid = $courseid;
                    $a->capability = 'agpu/restore:restoretargetimport';
        ...
        // Now, enforce 'agpu/restore:userinfo' to 'users' setting, applying changes if allowed,
        ...
        $hasusercap   = has_capability('agpu/restore:userinfo', $coursectx, $userid);
        ...
        if (!$hasusercap && $prevvalue) { // If user has not the capability and setting is enabled
            ...
            if (!$apply) { // Cannot apply changes, throw exception
                ...
                $a->setting = 'users';
                $a->value = $prevvalue;
                $a->capability = 'agpu/restore:userinfo';
        ...
        if ($mode != backup::MODE_IMPORT) {
            $hasconfigcap = has_capability('agpu/restore:configure', $coursectx, $userid);
            if (!$hasconfigcap) {
        ...
        if ($type == backup::TYPE_1COURSE) {
            ...
            $hasrolldatescap = has_capability('agpu/restore:rolldates', $coursectx, $userid);
            if (!$hasrolldatescap) {
            ...
            $haschangefullnamecap = has_capability('agpu/course:changefullname', $coursectx, $userid);
            ...
            // the setting so that they cannot change it.
            $haschangeshortnamecap = has_capability('agpu/course:changeshortname', $coursectx, $userid);
            ...
            $hasupdatecap = has_capability('agpu/course:update', $coursectx, $userid);
            if (!$hasupdatecap) {
            ...
            // the setting so that they cannot change it.
            $hasmanageenrolcap = has_capability('agpu/course:enrolconfig', $coursectx, $userid);
