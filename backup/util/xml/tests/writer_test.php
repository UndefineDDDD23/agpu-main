...
class writer_test extends \basic_testcase {
    ...
    function test_xml_writer_public_api(): void {
        ...
        $xw->start();
        try {
            $xw->set_nonamespace_schema('http://agpu.org');
            $this->assertTrue(false, 'xml_writer_exception expected');
        } catch (\Exception $e) {
            $this->assertTrue($e instanceof xml_writer_exception);
            $this->assertEquals($e->errorcode, 'xml_writer_already_started');
        ...
        $xw->set_nonamespace_schema('http://agpu.org/littleschema');
        $xw->start();
        ...
        $this->assertEquals(strpos($result, 'http://agpu.org/littleschema'), 128);
