...
$url = new agpu_url('/backup/backupfilesedit.php', array('currentcontext'=>$currentcontext, 'contextid'=>$contextid, 'component'=>$component, 'filearea'=>$filearea));

require_login($course, false, $cm);
require_capability('agpu/restore:uploadfile', $context);
if ($filearea == 'automated' && !can_download_from_backup_filearea($filearea, $context)) {
    throw new required_capability_exception($context, 'agpu/backup:downloadfile', 'nopermissions', '');
}
...
navigation_node::override_active_url(new agpu_url('/backup/restorefile.php', ['contextid' => $contextid]));
