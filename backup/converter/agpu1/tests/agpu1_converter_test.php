...
use convert_helper;
use agpu1_converter;
use agpu1_convert_empty_storage_exception;
use agpu1_convert_exception;
use agpu1_convert_storage_exception;

defined('agpu_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot . '/backup/converter/agpu1/lib.php');

/**
 * Unit tests for the agpu1 converter
 *
 * @package    core_backup
 ...
 */
class agpu1_converter_test extends \advanced_testcase {

    /** @var string the name of the directory containing the unpacked agpu 1.9 backup */
    protected $tempdir;

    /** @var string the full name of the directory containing the unpacked agpu 1.9 backup */
    ...
    protected function setUp(): void {
        ...
        copy(
            "$CFG->dirroot/backup/converter/agpu1/tests/fixtures/agpu.xml",
            "$this->tempdirpath/agpu.xml"
        );
        copy(
            "$CFG->dirroot/backup/converter/agpu1/tests/fixtures/icon.gif",
    ...
    public function test_detect_format(): void {
        $detected = agpu1_converter::detect_format($this->tempdir);
        $this->assertEquals(backup::FORMAT_agpu1, $detected);
    }

    public function test_convert_factory(): void {
        $converter = convert_factory::get_converter('agpu1', $this->tempdir);
        $this->assertInstanceOf('agpu1_converter', $converter);
    }

    public function test_stash_storage_not_created(): void {
        $converter = convert_factory::get_converter('agpu1', $this->tempdir);
        $this->expectException(agpu1_convert_storage_exception::class);
    ...
    public function test_stash_requiring_empty_stash(): void {
        ...
        try {
            ...
            $this->expectException(agpu1_convert_empty_storage_exception::class);
            $converter->drop_stash_storage();
            throw new agpu1_convert_empty_storage_exception('rethrowing');
        }
    }
    ...
    public function test_convert_path_implicit_recipes(): void {
        ...
        $this->assertEquals('agpur', $data['martin']);
...
as it is parsed from the backup file. <br /><br /><img border="0" width="110" vspace="0" hspace="0" height="92" title="News" alt="News" src="$@FILEPHP@$$@SLASH@$pics$@SLASH@$news.gif" /><a href="$@FILEPHP@$$@SLASH@$pics$@SLASH@$news.gif$@FORCEDOWNLOAD@$">download image</a><br />
    ...
    <br /><a href=\'$@FILEPHP@$$@SLASH@$MANUAL.DOC$@FORCEDOWNLOAD@$\'>download manual</a><br />';
        ...
        $files = agpu1_converter::find_referenced_files($text);
        ...
        $text = agpu1_converter::rewrite_filephp_usage($text, array('/pics/news.gif', '/another/file/notused.txt'));
...
<a href="$@FILEPHP@$$@SLASH@$illegal pics%2Bmovies$@SLASH@$romeo%2Bjuliet.avi">Download the full AVI for free! (only plus)</a>';
        ...
        $text = agpu1_converter::rewrite_filephp_usage($text, $files);
...
<a href="$@FILEPHP@$$@SLASH@$illegal pics%2Bmovies$@SLASH@$romeo%2Bjuliet.avi">Download the full AVI for free! (only plus)</a>', $text);
    ...
    public function test_question_bank_conversion(): void {
        ...
        copy(
            "$CFG->dirroot/backup/converter/agpu1/tests/fixtures/questions.xml",
            "$this->tempdirpath/agpu.xml"
        );
        $converter = convert_factory::get_converter('agpu1', $this->tempdir);
        $converter->convert();
