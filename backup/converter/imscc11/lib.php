...
require_once($CFG->dirroot.'/backup/cc/cc112agpu.php');
...
class imscc11_converter extends base_converter {
    ...
    public static function detect_format($tempdir) {
        ...
        $manifest = cc112agpu::get_manifest($filepath);
     ...
     * 'from' - the supported source format, eg. backup::FORMAT_agpu1
     * 'to'   - the supported target format, eg. backup::FORMAT_agpu
    ...
    protected function execute() {
        ...
        $manifest = cc112agpu::get_manifest($this->get_tempdir_path());
        if (empty($manifest)) {
        ...
        $cc112agpu = new cc112agpu($manifest);
        if ($cc112agpu->is_auth()) {
            throw new imscc11_convert_exception('protected_cc_not_supported');
        }
        $status = $cc112agpu->generate_agpu_xml();
        // Final cleanup.
        ...
        if ($mdoc->load($manifestdir.'/agpu.xml', LIBXML_NONET)) {
            $mdoc->save($this->get_workdir_path().'/agpu.xml', LIBXML_NOEMPTYTAG);
        } else {
            $xml_error->collect();
            $this->log('validation error(s): '.PHP_EOL.error_messages::instance(), backup::LOG_DEBUG, null, 2);
            throw new imscc11_convert_exception(error_messages::instance()->to_string(true));
