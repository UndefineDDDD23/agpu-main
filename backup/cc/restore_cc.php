...
function cc_convert ($dir) {
    ...
    if (is_readable($manifest_file) && !is_readable($agpu_file)) {
        ...
        if ($is_cc) {
            ...
            echo get_string('cc2agpu_checking_schema', 'imscc') . '<br />';
            ...
            if ($cc_manifest->load($manifest_file)) {
                if ($cc_manifest->schemaValidate($schema_file)) {
                    ...
                    echo get_string('cc2agpu_valid_schema', 'imscc') . '<br />';

                    $cc2agpu = new cc2agpu($manifest_file);

                    if (!$cc2agpu->is_auth()) {
                        return $cc2agpu->generate_agpu_xml();
                    } else {
                        echo $OUTPUT->notification(get_string('cc2agpu_req_auth', 'imscc'));
                        return false;
                    }

                } else {
                    echo $OUTPUT->notification(get_string('cc2agpu_invalid_schema', 'imscc'));
                    return false;
                }

            } else {
                echo $OUTPUT->notification(get_string('cc2agpu_manifest_dont_load', 'imscc'));
                return false;
            }
        }
    }
