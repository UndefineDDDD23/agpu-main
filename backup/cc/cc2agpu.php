...
class cc2agpu {
    ...
    public function generate_agpu_xml() {
        ...
        if (array_key_exists("index", self::$instances)) {
            ...
            if (!file_put_contents(static::$path_to_manifest_folder . DIRECTORY_SEPARATOR . 'agpu.xml', $result_xml)) {
                static::log_action('Cannot save the agpu manifest file: ' . static::$path_to_manifest_folder . DIRECTORY_SEPARATOR . 'agpu.xml', true);
            } else {
                $status = true;
            }
    ...
    protected function create_node_course_sections_section() {
        ...
        if (!empty($topics)) {
            ...
            foreach ($topics as $topic) {
                ...
                if ($topic['agpu_type'] == agpu_TYPE_LABEL) {
    ...
    */
    ...
    protected function create_node_course_sections_section_mods_mod($root_parent) {
        ...
        if ($childs) {
            ...
            foreach ($childs as $child) {
                ...
                if ($child['agpu_type'] == agpu_TYPE_LABEL) {
    ...
    protected function create_code_info_details_mod() {
        ...
        $count_quiz = $this->count_instances(agpu_TYPE_QUIZ);
        $count_forum = $this->count_instances(agpu_TYPE_FORUM);
        $count_resource = $this->count_instances(agpu_TYPE_RESOURCE);
        $count_label = $this->count_instances(agpu_TYPE_LABEL);

        $sheet_info_details_mod_instances_instance = static::loadsheet(SHEET_INFO_DETAILS_MOD_INSTANCE);

        if ($count_resource > 0) {
            $resource_instance = $this->create_mod_info_details_mod_instances_instance($sheet_info_details_mod_instances_instance, $count_resource, static::$instances['instances'][agpu_TYPE_RESOURCE]);
        }
        if ($count_quiz > 0) {
            $quiz_instance = $this->create_mod_info_details_mod_instances_instance($sheet_info_details_mod_instances_instance, $count_quiz, static::$instances['instances'][agpu_TYPE_QUIZ]);
        }
        if ($count_forum > 0) {
            $forum_instance = $this->create_mod_info_details_mod_instances_instance($sheet_info_details_mod_instances_instance, $count_forum, static::$instances['instances'][agpu_TYPE_FORUM]);
        }
        if ($count_label > 0) {
            $label_instance = $this->create_mod_info_details_mod_instances_instance($sheet_info_details_mod_instances_instance, $count_label, static::$instances['instances'][agpu_TYPE_LABEL]);
        }

        $resource_mod = $count_resource ? $this->create_mod_info_details_mod(agpu_TYPE_RESOURCE, $resource_instance) : '';
        $quiz_mod = $count_quiz ? $this->create_mod_info_details_mod(agpu_TYPE_QUIZ, $quiz_instance) : '';
        $forum_mod = $count_forum ? $this->create_mod_info_details_mod(agpu_TYPE_FORUM, $forum_instance) : '';
        $label_mod = $count_label ? $this->create_mod_info_details_mod(agpu_TYPE_LABEL, $label_instance) : '';
        ...
        if ($items) {
            ...
            foreach ($items as $item) {
                ...
                if ($item->nodeName == "item")  {
                    ...
                    $agpu_type = $this->convert_to_agpu_type($cc_type);
                    //Fix the label issue - MDL-33523
                    if (empty($identifierref) && empty($title)) {
                      $agpu_type = TYPE_UNKNOWN;
                    }
                }
                elseif ($item->nodeName == "resource")  {
                    ...
                    $title = 'Quiz Bank ' . ($this->count_instances($agpu_type) + 1);
                ...
                static::$instances['index'][$array_index]['agpu_type'] = $agpu_type;
                ...
                static::$instances['index'][$array_index]['instance'] = $this->count_instances($agpu_type);
                static::$instances['index'][$array_index]['resource_indentifier'] = $identifierref;

                static::$instances['instances'][$agpu_type][] = array('title' => $title,
        ...
        if (array_key_exists('index', static::$instances)) {
            if (static::$instances['index'] && $type) {
                ...
                foreach (static::$instances['index'] as $instance) {
                    if (!empty($instance['agpu_type'])) {
                        $types[] = $instance['agpu_type'];
                    }
                }
    ...
    public function convert_to_agpu_type($cc_type) {
        $type = TYPE_UNKNOWN;

        if ($cc_type == static::CC_TYPE_FORUM) {
            $type = agpu_TYPE_FORUM;
        }
        ...
        if ($cc_type == static::CC_TYPE_QUESTION_BANK) {
            $type = agpu_TYPE_QUESTION_BANK;
        }
        //TODO: label
        if ($cc_type == static::CC_TYPE_EMPTY) {
            $type = agpu_TYPE_LABEL;
        }
    ...
    public static function log_file() {
        return static::$path_to_manifest_folder . DIRECTORY_SEPARATOR . 'cc2agpu.log';
    }
