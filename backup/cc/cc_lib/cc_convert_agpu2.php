...
abstract class cc_convert_agpu2 {
    ...
    public static function convert($packagedir, $outdir) {
        ...
        if ($doc->load($coursefile)) {
            ...
            $agpu_backup = $dir . DIRECTORY_SEPARATOR . 'agpu_backup.xml';
            $secp = new XMLGenericDocument();
            $docp = new XMLGenericDocument();
            if ($docp->load($agpu_backup)) {
                //sections
                $sections = array();
                ...
                $section_list = $docp->nodeList('/agpu_backup/information/contents/sections/section');
    ...
    */
    protected static function process_sequence(cc_i_item &$item, cc_i_manifest &$manifest, array $sequence, $packageroot, $outdir) {
        $agpu_backup = $packageroot . DIRECTORY_SEPARATOR . 'agpu_backup.xml';
        $doc = new XMLGenericDocument();
        if(!$doc->load($agpu_backup)) {
            return;
        }
        $activities = $doc->nodeList('/agpu_backup/information/contents/activities/activity');
        if (!empty($activities)) {
