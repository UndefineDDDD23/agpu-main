...
class cc11_quiz extends entities11 {
    ...
    public function generate_node_course_modules_mod() {
        ...
        cc112agpu::log_action('Creating Quiz mods');
    ...
    private function create_node_course_modules_mod_quiz_feedback() {
        ...
        $sheet_question_mod_feedback = cc112agpu::loadsheet(SHEET_COURSE_SECTIONS_SECTION_MODS_MOD_QUIZ_FEEDBACK);
    ...
    private function generate_instances() {
        ...
        $types = array(agpu_TYPE_QUIZ, agpu_TYPE_QUESTION_BANK);

        foreach ($types as $type) {

            if (!empty(cc112agpu::$instances['instances'][$type])) {

                foreach (cc112agpu::$instances['instances'][$type] as $instance) {

                    if ($type == agpu_TYPE_QUIZ) {
                        $is_question_bank = 0;
                    } else {
                        $is_question_bank = 1;
                    }
                    ...
                    if (!empty($assessment_file)) {
                        ...
                        $assessment = $this->load_xml_resource(cc112agpu::$path_to_manifest_folder . DIRECTORY_SEPARATOR . $assessment_file);
    ...
    private function create_node_course_modules_mod($instance) {
        ...
        $sheet_question_mod = cc112agpu::loadsheet(SHEET_COURSE_SECTIONS_SECTION_MODS_MOD_QUIZ);
    ...
    private function get_global_config($assessment, $option, $default_value, $replace_values = '') {
        ...
        $xpath = cc112agpu::newx_path($assessment, cc112agpu::getquizns());
    ...
    private function create_node_course_modules_mod_quiz_question_instances($instance) {
        ...
        $sheet_question_mod_instance = cc112agpu::loadsheet(SHEET_COURSE_SECTIONS_SECTION_MODS_MOD_QUIZ_QUESTION_INSTANCE);
    ...
    private function create_node_course_question_categories($instances) {
        ...
        $sheet_question_categories = cc112agpu::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES);
    ...
    private function create_node_course_question_categories_question_category($instance) {
        ...
        $sheet_question_categories_quetion_category = cc112agpu::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY);
    ...
    private function create_node_course_question_categories_question_category_question($instance) {
        ...
        $sheet_question_categories_question = cc112agpu::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION);
        ...
        if (!empty($questions)) {
            ...
            foreach ($questions as $question) {
                ...
                $question_agpu_type = $question['agpu_type'];
                $question_cc_type = $question['cc_type'];

                $question_type_node = '';

                $question_type_node = ($question_agpu_type == agpu_QUIZ_MULTIPLE_CHOICE) ? $this->create_node_course_question_categories_question_category_question_multiple_choice($question) : $question_type_node;
                $question_type_node = ($question_agpu_type == agpu_QUIZ_TRUE_FALSE) ? $this->create_node_course_question_categories_question_category_question_true_false($question) : $question_type_node;
                $question_type_node = ($question_agpu_type == agpu_QUIZ_ESSAY) ? $this->create_node_course_question_categories_question_category_question_eesay($question) : $question_type_node;
                $question_type_node = ($question_agpu_type == agpu_QUIZ_SHORTANSWER) ? $this->create_node_course_question_categories_question_category_question_shortanswer($question) : $question_type_node;
        ...
        foreach ($questions_items as $question_item) {
            ...
            if (!empty($question_identifier)) {
                ...
                if (!empty($question_type['agpu'])) {
                    ...
                    // This attribute is not IMSCC spec, but it is included in agpu 2.x export of IMS1.1
                    ...
                    $questions[$question_identifier]['agpu_type'] = $question_type['agpu'];
    ...
    private function get_defaultgrade($assessment, $question_identifier) {
        $result = 1;
        $xpath = cc2agpu::newx_path($assessment, cc2agpu::getquizns());
    ...
    private function get_score($assessment, $identifier, $question_identifier) {
        ...
        // This method (get_score) is only used by T/F & M/C questions in CC, therefore it's either 0 or 1 in agpu.
    ...
    private function create_node_course_question_categories_question_category_question_multiple_choice($question) {
        ...
        $sheet_question_categories_question = cc112agpu::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION_MULTIPLE_CHOICE);
    ...
    private function create_node_course_question_categories_question_category_question_eesay($question) {
        ...
        $sheet_question_categories_question = cc112agpu::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION_EESAY);
    ...
    private function create_node_course_question_categories_question_category_question_shortanswer($question) { //, &$fib_questions) {
        ...
        $sheet_question_categories_question = cc112agpu::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION_SHORTANSWER);
        $node_course_question_categories_question_answer = '';
    ...
    private function create_node_course_question_categories_question_category_question_true_false($question) {
        ...
        $sheet_question_categories_question = cc112agpu::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION_TRUE_FALSE);
    ...
    private function create_node_course_question_categories_question_category_question_answer($answer) {
        ...
        $sheet_question_categories_question_answer = cc112agpu::loadsheet(SHEET_COURSE_QUESTION_CATEGORIES_QUESTION_CATEGORY_QUESTION_ANSWER);
    ...
    private function get_question_type($identifier, $assessment) {
        ...
        if ($type == CC_QUIZ_MULTIPLE_CHOICE) {
            $return_type['agpu'] = agpu_QUIZ_MULTIPLE_CHOICE;
        }
        if ($type == CC_QUIZ_MULTIPLE_RESPONSE) {
            $return_type['agpu'] = agpu_QUIZ_MULTIPLE_CHOICE;
        }
        if ($type == CC_QUIZ_TRUE_FALSE) {
            $return_type['agpu'] = agpu_QUIZ_TRUE_FALSE;
        }
        if ($type == CC_QUIZ_ESSAY) {
            $return_type['agpu'] = agpu_QUIZ_ESSAY;
        }
        if ($type == CC_QUIZ_FIB) {
            $return_type['agpu'] = agpu_QUIZ_SHORTANSWER;
        }
