...
require_capability('agpu/restore:restoretargetimport', $context);

// Set up the page
$PAGE->set_title($course->shortname . ': ' . get_string('import'));
$PAGE->set_heading($course->fullname);
$PAGE->set_url(new agpu_url('/backup/import.php', array('id'=>$courseid)));
...
if ($importcourseid === false || $searchcourses) {
    ...
    $url = new agpu_url('/backup/import.php', array('id'=>$courseid));
...
require_capability('agpu/backup:backuptargetimport', $importcontext);
...
if (!($bc = backup_ui::load_controller($backupid))) {
    $bc = new backup_controller(backup::TYPE_1COURSE, $importcourse->id, backup::FORMAT_agpu,
...
if ($backup->get_stage() == backup_ui::STAGE_FINAL) {
    ...
    if (!file_exists($tempdestination) || !is_dir($tempdestination)) {
        throw new \agpu_exception('unknownbackupexporterror'); // Shouldn't happen ever.
    }
    ...
    if (!$rc->execute_precheck()) {
        ...
        if (is_array($precheckresults)) {
            if (!empty($precheckresults['errors'])) { // If errors are found, terminate the import.
                ...
                echo $OUTPUT->continue_button(new agpu_url('/course/view.php', array('id'=>$course->id)));
                echo $OUTPUT->footer();
                die();
            }
