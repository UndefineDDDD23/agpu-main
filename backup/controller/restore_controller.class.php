...
class restore_controller extends base_controller {
    ...
    protected $format; // Format of backup (agpu, imscc)
    ...
    public function __construct($tempdir, $courseid, $interactive, $mode, $userid, $target,
        ...
        // If format is not agpu2, set to conversion needed
        if ($this->format !== backup::FORMAT_agpu) {
            $this->set_status(backup::STATUS_REQUIRE_CONV);

        // Else, format is agpu2, load plan, apply security and set status based on interactivity
        } else {
            // Load plan
            $this->load_plan();
...
// checksumable interface methods
    ...
    /**
     * Converts from current format to backup::agpu format
     */
    public function convert() {
        ...
        if (!convert_helper::to_agpu2_format($this->get_tempdir(), $this->format, $this->get_logger())) {
        ...
        $this->format = backup::FORMAT_agpu;
...
// Protected API starts here
    ...
    protected function load_plan() {
        // First of all, we need to introspect the agpu_backup.xml file
