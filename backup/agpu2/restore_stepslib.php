...
class restore_gradebook_structure_step extends restore_structure_step {
    ...
    protected function check_minmaxtouse() {
        ...
        if ($minmaxtouse === false &&
            ...
            $version = $this->get_task()->get_info()->agpu_version;
...
class restore_groups_structure_step extends restore_structure_step {
    ...
    public function process_group($data) {
        ...
        if (isset($data->idnumber) and has_capability('agpu/course:changeidnumber', $context, $this->task->get_userid())) {
...
class restore_scales_structure_step extends restore_structure_step {
    ...
    protected function process_scale($data) {
        ...
        if (!$scadb = $DB->get_record_sql("SELECT *
            ...
            if ($data->courseid == 0 && !has_capability('agpu/course:managescales', $systemctx , $this->task->get_userid())) {
                $data->courseid = $this->get_courseid();
            }
...
class restore_outcomes_structure_step extends restore_structure_step {
    ...
    protected function process_outcome($data) {
        ...
        if (!$outdb = $DB->get_record_sql('SELECT *
            ...
            $systemctx = context_system::instance();
            if (is_null($data->courseid) && !has_capability('agpu/grade:manageoutcomes', $systemctx , $this->task->get_userid())) {
                $data->courseid = $this->get_courseid();
            }
...
class restore_section_structure_step extends restore_structure_step {
    ...
    public function process_section($data) {
        ...
        // by a lot of people to "hide" sections on purpose (so this remains as used to be in agpu 1.x)
     ...
     * in agpu 2.7+ but we still support restore.
     *
     ...
     * exist in agpu 2.7+ but we still support restore.
     *
     * @param stdClass $data Record data
     */
...
class restore_course_structure_step extends restore_structure_step {
    ...
    public function process_course($data) {
        ...
        $canchangeidnumber = $isnewcourse || has_capability('agpu/course:changeidnumber', $context, $userid);
        $canchangesummary = $isnewcourse || has_capability('agpu/course:changesummary', $context, $userid);
        $canforcelanguage = has_capability('agpu/course:setforcedlanguage', $context, $userid);
    ...
    protected function after_execute() {
        ...
        if ($this->legacyrestrictmodules) {
            ...
            list($roleids) = get_roles_with_cap_in_context($context, 'agpu/course:manageactivities');
            list($managerroleids) = get_roles_with_cap_in_context($context, 'agpu/site:config');
        ...
        if ($newroleid && $this->task->get_contextid()) {
            if (!$capability = get_capability_info($data->capability)) {
                ...
                if (in_array($newrole->info['shortname'], $overrideableroles) &&
                    (has_capability('agpu/role:override', $context, $userid) ||
                            ($safecapability && has_capability('agpu/role:safeoverride', $context, $userid)))
                ) {
...
class restore_fix_restorer_access_step extends restore_execution_step {
    protected function define_execution() {
        ...
        if (is_enrolled($context, $userid, 'agpu/course:update', true) or is_viewing($context, $userid, 'agpu/course:update')) {
            // Current user may access the course (admin, category manager or restored teacher enrolment usually)
            return;
        }

        // Try to add role only - we do not need enrolment if user has agpu/course:view or is already enrolled
...
class restore_module_structure_step extends restore_structure_step {
    ...
    protected function process_module($data) {
        ...
        if (!has_capability('agpu/course:setforcedlanguage', context_course::instance($data->course))) {
            unset($data->lang);
        }
 ...
 *
 * @copyright 2012 David Mudrak <david@agpu.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
        ...
        foreach ($rs as $record) {
            ...
            if ($info->oldfile->repositorytype === 'local' || $info->oldfile->repositorytype === 'coursefiles'
                ...
                if ($source) {
                    ...
                    // This is a reference to some agpu file that was not contained in the backup
...
abstract class restore_questions_activity_structure_step extends restore_activity_structure_step {
    ...
    /**
     * Attach below $element (usually attempts) the needed restore_path_elements
     * to restore question attempt data from agpu 2.0.
     *
