...
/**
 * Tests for agpu 2 format backup operation.
 *
 * @package core_backup
 * @copyright 2014 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class agpu2_test extends \advanced_testcase {
    ...
    /**
     * The availability data format was changed in agpu 2.7. This test
     * ensures that a agpu 2.6 backup with this data can still be correctly
     * restored.
     */
    public function test_restore_legacy_availability(): void {
        ...
        get_file_packer('application/vnd.agpu.backup')->extract_to_pathname(
    ...
    public function test_restore_frontpage(): void {
        ...
        $frontpagebc = new backup_controller(backup::TYPE_1COURSE, $frontpage->id,
                backup::FORMAT_agpu, backup::INTERACTIVE_NO, backup::MODE_IMPORT,
                $USER->id);
    ...
    protected function duplicate($course, $cmid) {
        ...
        // Do backup.
        $bc = new backup_controller(backup::TYPE_1ACTIVITY, $cmid, backup::FORMAT_agpu,
    ...
    protected function prepare_for_enrolments_test($target, $additionalcaps = []) {
        ...
        $caps = array_merge([
            ...
            'agpu/backup:configure',
            'agpu/backup:backuptargetimport',
            'agpu/restore:restorecourse',
            'agpu/role:assign',
            'agpu/restore:configure',
        ], $additionalcaps);
        ...
        // create the directory and not zip it.
        $bc = new backup_controller(backup::TYPE_1COURSE, $course->id,
            backup::FORMAT_agpu, backup::INTERACTIVE_NO, backup::MODE_SAMESITE,
            $user->id);
        $backupid = $bc->get_backupid();
    ...
    public function test_restore_with_users_without_enrolments(): void {
        ...
        list($course, $newcourseid, $rc) = $this->prepare_for_enrolments_test(backup::TARGET_NEW_COURSE,
            ['agpu/backup:userinfo', 'agpu/restore:userinfo']);
    ...
    public function test_restore_with_users_with_enrolments(): void {
        ...
        list($course, $newcourseid, $rc) = $this->prepare_for_enrolments_test(backup::TARGET_NEW_COURSE,
            ['agpu/backup:userinfo', 'agpu/restore:userinfo', 'agpu/course:enrolconfig']);
    ...
    /**
     * The Question category hierarchical structure was changed in agpu 3.5.
     * From 3.5, all question categories in each context are a child of a single top level question category for that context.
     * This test ensures that both agpu 3.4 and 3.5 backups can still be correctly restored.
     */
