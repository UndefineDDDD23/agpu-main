...
class backup_course_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        // Add 'numsections' in order to be able to restore in previous versions of agpu.
        // Even though agpu does not officially support restore into older verions of agpu from the
 ...
 * using one specialised nested_element, will copy them form agpu storage
 * to backup storage
 */
...
/**
 * Structure step in charge of creating the main agpu_backup.xml file
...
class backup_main_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        $info['agpu_version'] = $CFG->version;
        $info['agpu_release'] = $CFG->release;
        ...
        list($dinfo, $cinfo, $sinfo) = backup_controller_dbops::get_agpu_backup_information(
                $this->get_backupid(), $this->get_task()->get_progress());

        // Define elements

        $agpu_backup = new backup_nested_element('agpu_backup');

        $information = new backup_nested_element('information', null, array(
            'name', 'agpu_version', 'agpu_release', 'backup_version',
        ...
        $agpu_backup->add_child($information);
        ...
        // Prepare some information to be sent to main agpu_backup.xml file
        return $agpu_backup;
    }
...
class backup_zip_contents extends backup_execution_step implements file_progress {
    ...
    protected function define_execution() {
        ...
        if (file_exists($logfilepath)) {
             $files['agpu_backup.log'] = $logfilepath;
        }
        ...
        // Get the zip packer
        $zippacker = get_file_packer('application/vnd.agpu.backup');
