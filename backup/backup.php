...
$courseurl = new agpu_url('/course/view.php', array('id' => $courseid));
$url = new agpu_url('/backup/backup.php', array('id'=>$courseid));
if ($sectionid !== null) {
...
switch ($type) {
    ...
    case backup::TYPE_1SECTION :
        require_capability('agpu/backup:backupsection', $coursecontext);
    ...
    case backup::TYPE_1ACTIVITY :
        ...
        require_capability('agpu/backup:backupactivity', $activitycontext);
        $contextid = $activitycontext->id;
        $heading = get_string('backupactivity', 'backup', $cm->name);
        break;
    default :
        throw new \agpu_exception('unknownbackuptype');
}
...
if (!async_helper::is_async_pending($id, 'course', 'backup')) {
    ...
    if (!($bc = backup_ui::load_controller($backupid))) {
        $bc = new backup_controller($type, $id, backup::FORMAT_agpu,
    ...
    if ($backup->get_stage() == backup_ui::STAGE_FINAL) {
        ...
        if ($backupmode != backup::MODE_ASYNC) {
            ...
            $restoreurl = new agpu_url('/backup/restorefile.php', array('contextid' => $contextid));
            $progresssetup = array(
                    'backupid' => $backupid,
                    'contextid' => $contextid,
