...
class settings {
    ...
    /** @var bool whether the current user has agpu/site:config capability  */
    ...
    protected function add_preupload_settings(): void {
        ...
        if ($this->admin->fulltree) {
            // This feature only works if curl is installed (but it is as now required by agpu). The checks have been removed.
            $item = new admin_setting_heading(
            ...
            // Note: checks on curl library have been removed as it is a requirement from agpu.
            $filemanageroptions = [];
