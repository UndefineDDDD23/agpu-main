 ...
 *
 * This class acts as a proxy between agpu and the BigBlueButton API server,
...
class bigbluebutton_proxy extends proxy_base {
     ...
     * @return agpu_url
     */
    private static function get_avatar_url(stdClass $user): agpu_url {
        global $PAGE;
    ...
    public static function get_server_not_available_url(instance $instance): string {
        if ($instance->is_admin()) {
            return new agpu_url('/admin/settings.php', ['section' => 'modsettingbigbluebuttonbn']);
        } else if ($instance->is_moderator()) {
            return new agpu_url('/course/view.php', ['id' => $instance->get_course_id()]);
        } else {
            return new agpu_url('/course/view.php', ['id' => $instance->get_course_id()]);
        }
    }
