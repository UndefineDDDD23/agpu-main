...
abstract class proxy_base {
     ...
     * can be converted to agpu error messages
     */
    const BBB_TO_agpu_ERROR_CODE = [
    ...
    protected static function sanitized_url(): string {
        ...
        if (PHPUNIT_TEST) {
            $serverurl = (new agpu_url(TEST_MOD_BIGBLUEBUTTONBN_MOCK_SERVER))->out(false);
        }
    ...
    protected static function assert_returned_xml($xml, ?array $additionaldetails = null): void {
        ...
        if (empty($xml) || static::is_known_server_unavailable_errorcode($messagekey)) {
            ...
            throw new server_not_available_exception(
                $errorcode,
                plugin::COMPONENT,
                (new agpu_url('/admin/settings.php?section=modsettingbigbluebuttonbn'))->out(),
            );
        }
    ...
    /**
     * Get agpu error code from returned Message Key
     *
    ...
    private static function get_errorcode_from_xml_messagekey(string $messagekey): string {
        ...
        if ($messagekey) {
            $errorcode = self::BBB_TO_agpu_ERROR_CODE[$messagekey] ?? $errorcode;
        }
        return $errorcode;
    }
