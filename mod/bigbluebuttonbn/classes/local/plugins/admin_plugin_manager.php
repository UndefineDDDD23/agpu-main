...
class admin_plugin_manager {
    ...
    public function __construct() {
        $this->pageurl = new agpu_url(admin_page_manage_extensions::ADMIN_PAGE_URL);
    }
        ...
        foreach ($plugins as $idx => $plugin) {
            ...
            if (!empty($pluginversion) && $exists) {
                $row[] = html_writer::link(
                    new agpu_url('/admin/settings.php', ['section' => $componentname]),
                    get_string('settings')
                );
            } else {
                $row[] = '&nbsp;';
    ...
    private function format_icon_link(string $action, string $plugin, string $icon, string $alt): string {
        ...
        return $OUTPUT->action_icon(
                ...
                new pix_icon($icon, $alt, 'agpu', ['title' => $alt]),
                null,
                ['title' => $alt]
            );
    }
