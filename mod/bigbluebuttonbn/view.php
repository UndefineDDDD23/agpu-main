...
if (!$instance) {
    throw new agpu_exception('view_error_url_missing_parameters', plugin::COMPONENT);
}
...
if (config::server_credentials_invalid()) {
    if (has_capability('agpu/site:config', context_system::instance())) {
        $settingslink = new agpu_url('/admin/settings.php', ['section' => 'modsettingbigbluebuttonbn']);
    ...
    } else if (has_capability('agpu/course:manageactivities', context_course::instance($course->id))) {
...
if (!$instance->can_join() && $instance->get_type() != instance::TYPE_RECORDING_ONLY) {
    if (isguestuser()) {
        ...
        notice(
            ...
            new agpu_url('/course/view.php', ['id' => $course->id])
        );
    }
}
