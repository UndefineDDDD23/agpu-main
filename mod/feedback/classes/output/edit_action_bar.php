...
class edit_action_bar extends base_action_bar {
    /** @var agpu_url $currenturl The current page url */
    private $currenturl;
     ...
     * @param agpu_url $pageurl The current page url
     * @param int|null $lastposition Index of the last question in the feedback
     */
    public function __construct(int $cmid, agpu_url $pageurl, ?int $lastposition = null) {
    ...
    private function get_add_question_menu(): action_menu {
        ...
        foreach (feedback_load_feedback_items_options() as $key => $value) {
            $addselect->add(new action_menu_link(
                new agpu_url('/mod/feedback/edit_item.php', $addselectparams + ['typ' => $key]),
                null,
                $value,
                false,
            ));
    ...
    private function get_edit_actions_menu(): action_menu {
        ...
        if ($DB->record_exists('feedback_item', ['feedback' => $this->feedback->id])) {
            $exporturl = new agpu_url('/mod/feedback/export.php', $this->urlparams + ['action' => 'exportfile']);
        ...
        // Import.
        $importurl = new agpu_url('/mod/feedback/import.php', $this->urlparams);
        $actionsselect->add(new action_menu_link(
            $importurl,
