 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_feedback_complete_form extends agpuform {
    ...
    private function remove_button($buttonname) {
        ...
        foreach ($el->_elements as $idx => $button) {
            if ($button instanceof agpuQuickForm_submit && $button->getName() === $buttonname) {
    ...
    public function add_form_element($item, $element, $addrequiredrule = true, $setdefaultvalue = true) {
        ...
        if ($item->required) {
            $required = $OUTPUT->pix_icon('req', get_string('requiredelement', 'form'), 'agpu', ['class' => 'ms-2']);
    ...
    /**
     * Returns the CSS id attribute that will be assigned by agpuform later to this element
     * @param stdClass $item
     * @param HTML_QuickForm_element $element
     */
