...
if ($id) {
    ...
    if (! $chat = $DB->get_record('chat', array('id' => $cm->instance))) {
        throw new \agpu_exception('invalidid', 'chat');
    }
...
if (has_capability('mod/chat:chat', $context)) {
    ...
    $params['id'] = $chat->id;
    $chattarget = new agpu_url("/mod/chat/gui_$CFG->chat_method/index.php", $params);
    ...
    $params['id'] = $chat->id;
    $link = new agpu_url('/mod/chat/gui_basic/index.php', $params);
    ...
    if (($chat->studentlogs or has_capability('mod/chat:readlog', $context)) && !$PAGE->has_secondary_navigation()) {
        if ($msg = chat_get_session_messages($chat->id, $currentgroup)) {
            echo '<p>';
            echo html_writer::link(new agpu_url('/mod/chat/report.php', array('id' => $cm->id)),
...
if ($chatusers = chat_get_users($chat->id, $currentgroup, $cm->groupingid)) {
    ...
    foreach ($chatusers as $chatuser) {
        ...
        $url = new agpu_url('/user/view.php', array('id' => $chatuser->id, 'course' => $chat->course));
