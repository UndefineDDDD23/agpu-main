...
$url = new agpu_url('/mod/chat/gui_basic/index.php', array('id' => $id));
if ($groupid !== 0) {
...
if (!$chatusers = chat_get_users($chat->id, $groupid, $cm->groupingid)) {
    throw new \agpu_exception('errornousers', 'chat');
}
...
if (!empty($refresh) and data_submitted()) {
    ...
    $url = new agpu_url('/mod/chat/gui_basic/index.php', array('id' => $id, 'newonly' => $newonly, 'last' => $last));
    redirect($url);
}
