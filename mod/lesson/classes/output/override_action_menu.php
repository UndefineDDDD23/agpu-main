...
class override_action_menu implements templatable, renderable {
    ...
    protected $cmid;
    /** @var agpu_url The current url for the page. */
     ...
     * @param agpu_url $currenturl The current url for the page.
     * @param bool $canoverride Whether can add user or group override (depending on the override type).
     */
    public function __construct(int $cmid, agpu_url $currenturl, bool $canoverride = false) {
        $this->cmid = $cmid;
    ...
    protected function create_override_select_menu(): \url_select {
        $userlink = new agpu_url('/mod/lesson/overrides.php', ['cmid' => $this->cmid, 'mode' => 'user']);
        $grouplink = new agpu_url('/mod/lesson/overrides.php', ['cmid' => $this->cmid, 'mode' => 'group']);
        $menu = [
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        if ($this->canoverride) {
            $data['addoverride'] = [
                'text' => $text,
                'link' => (new agpu_url('/mod/lesson/overrideedit.php', [
