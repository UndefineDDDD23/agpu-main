...
$url = new agpu_url('/mod/lesson/essay.php', array('id'=>$id));
if ($mode !== 'display') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
switch ($mode) {
    case 'grade':
        ...
        if (empty($attempt)) {
            throw new \agpu_exception('cannotfindattempt', 'lesson');
        }
        if (empty($user)) {
            throw new \agpu_exception('cannotfinduser', 'lesson');
        }
        if (empty($answer)) {
            throw new \agpu_exception('cannotfindanswer', 'lesson');
        }
        break;
    ...
    case 'update':
        ...
        if ($form = $mform->get_data()) {
            ...
            redirect(new agpu_url('/mod/lesson/essay.php', array('id'=>$cm->id)));
        } else {
    ...
    case 'email':
        ...
        if (!$attempts = $DB->get_records_select('lesson_attempts', "pageid $usql".$queryadd, $params)) {
            throw new \agpu_exception('nooneansweredthisquestion', 'lesson');
        }
        // Get the answers
        ...
        foreach ($attempts as $attempt) {
            ...
            if ($essayinfo->graded && !$essayinfo->sent) {
                ...
                // Context url.
                $contexturl = new agpu_url('/grade/report/user/index.php', array('id' => $course->id));
...
switch ($mode) {
    case 'display':
        ...
        foreach (array_keys($studentessays) as $userid) {
            ...
            foreach ($studentessays[$userid] as $page => $tries) {
                ...
                foreach($tries as $try) {
                    ...
                    // link for each essay
                    $url = new agpu_url('/mod/lesson/essay.php', array('id'=>$cm->id,'mode'=>'grade','attemptid'=>$essay->id,'sesskey'=>sesskey()));
            ...
            $url = new agpu_url('/mod/lesson/essay.php', array('id'=>$cm->id,'mode'=>'email','userid'=>$userid,'sesskey'=>sesskey()));
        ...
        $url = new agpu_url('/mod/lesson/essay.php', array('id'=>$cm->id,'mode'=>'email','sesskey'=>sesskey()));
