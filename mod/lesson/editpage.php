...
if (!empty($returnto)) {
    $returnto = new agpu_url($returnto);
} else {
    $returnto = new agpu_url('/mod/lesson/edit.php', array('id' => $id));
...
// in agpuform::_process_submission because the array key check doesn't
...
if ($edit) {
    ...
    $mform->set_data($data);
    $PAGE->navbar->add(get_string('edit'), new agpu_url('/mod/lesson/edit.php', array('id'=>$id)));
