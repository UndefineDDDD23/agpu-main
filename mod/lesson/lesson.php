...
$url = new agpu_url('/mod/lesson/lesson.php', array('id'=>$id,'action'=>$action));
$PAGE->set_url($url);
...
switch ($action) {
        ...
        if ($answers = $DB->get_records_select("lesson_answers", "lessonid = :lessonid AND jumpto = :pageid + 1", $params)) {
            ...
            foreach ($answers as $answer) {
                if (!$title = $DB->get_field("lesson_pages", "title", array("id" => $answer->pageid))) {
                    throw new \agpu_exception('cannotfindpagetitle', 'lesson');
                }
                echo $title."<br />\n";
            }
        }
        ...
        while (true) {
            ...
            if ($page->nextpageid) {
                if (!$page = $DB->get_record("lesson_pages", array("id" => $page->nextpageid))) {
                    throw new \agpu_exception('cannotfindnextpage', 'lesson');
                }
            } else {
                // last page reached
                break;
    ...
    case 'duplicate':
            ...
            redirect(new agpu_url('/mod/lesson/edit.php', array('id' => $cm->id)));
        break;
    default:
