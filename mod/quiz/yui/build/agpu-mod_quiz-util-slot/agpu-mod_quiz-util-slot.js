YUI.add('agpu-mod_quiz-util-slot', function (Y, NAME) {

/**
 * A collection of utility classes for use with slots.
 *
 * @module agpu-mod_quiz-util
 * @submodule agpu-mod_quiz-util-slot
 */

Y.namespace('agpu.mod_quiz.util.slot');

/**
 * A collection of utility classes for use with slots.
 *
 * @class agpu.mod_quiz.util.slot
 * @static
 */
Y.agpu.mod_quiz.util.slot = {
    CSS: {
        SLOT: 'slot',
    ...
    CONSTANTS: {
        SLOTIDPREFIX: 'slot-',
        QUESTION: M.util.get_string('question', 'agpu')
    },
    SELECTORS: {
        SLOT: 'li.slot',
    ...
    reorderSlots: function() {
        ...
        slots.each(function(slot) {
            ...
            if (!Y.agpu.mod_quiz.util.page.getPageFromSlot(slot)) {
                // Move the next page to the front.
                var nextpage = slot.next(Y.agpu.mod_quiz.util.page.SELECTORS.PAGE);
                slot.swap(nextpage);
            }
    ...
    remove: function(slot) {
        var page = Y.agpu.mod_quiz.util.page.getPageFromSlot(slot);
        slot.remove();
        // Is the page empty.
        if (!Y.agpu.mod_quiz.util.page.isEmpty(page)) {
            return;
        }
        // If so remove it. Including add menu and page break.
        Y.agpu.mod_quiz.util.page.remove(page);
    },
    ...
    reorderPageBreaks: function() {
        ...
        slots.each(function(slot, key) {
            ...
            var action = '';
            var iconname = '';
            if (Y.agpu.mod_quiz.util.page.isPage(nextitem)) {
...
}, '@VERSION@', {"requires": ["node", "agpu-mod_quiz-util-base"]});
