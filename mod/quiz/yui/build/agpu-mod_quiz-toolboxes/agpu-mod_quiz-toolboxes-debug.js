YUI.add('agpu-mod_quiz-toolboxes', function (Y, NAME) {
...
},
{
    ...
    ATTRS: {
        /**
         * The ID of the agpu Course being edited.
         *
         * @attribute courseid
         * @default 0
         * @type Number
...
Y.extend(RESOURCETOOLBOX, TOOLBOX, {
    ...
    delete_with_confirmation: function(ev, button, activity) {
        ...
        require(['core/notification'], function(Notification) {
            Notification.saveCancelPromise(
                M.util.get_string('confirm', 'agpu'),
                M.util.get_string('confirmremovequestion', 'quiz', qtypename),
                M.util.get_string('yes', 'agpu')
            ).then(function() {
                var spinner = this.add_spinner(element);
                var data = {
                    'class': 'resource',
                    'action': 'DELETE',
                    'id': Y.agpu.mod_quiz.util.slot.getId(element)
                };
                this.send_request(data, spinner, function(response) {
                    if (response.deleted) {
                        // Actually remove the element.
                        Y.agpu.mod_quiz.util.slot.remove(element);
    ...
    delete_multiple_with_confirmation: function(ev) {
        ...
        Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
            var slot = Y.agpu.mod_quiz.util.slot.getSlotFromComponent(node);
            ids += ids === '' ? '' : ',';
            ids += Y.agpu.mod_quiz.util.slot.getId(slot);
            slots.push(slot);
        });
            ...
            Notification.saveCancelPromise(
                ...
                this.send_request(data, spinner, function(response) {
                    ...
                    if (response.deleted) {
                        // Actually remove the element.
                        Y.all(SELECTOR.SELECTMULTIPLECHECKBOX + ':checked').each(function(node) {
                            Y.agpu.mod_quiz.util.slot.remove(node.ancestor('li.activity'));
                        });
    ...
    edit_maxmark: function(ev, button, activity) {
        ...
        var instancemaxmark = activity.one(SELECTOR.INSTANCEMAXMARK),
            ...
            data = {
                'class': 'resource',
                'field': 'getmaxmark',
                'id': Y.agpu.mod_quiz.util.slot.getId(activity)
            };
        ...
        this.send_request(data, null, function(response) {
            ...
            var editinstructions = Y.Node.create('<span class="' + CSS.EDITINSTRUCTIONS + '" id="id_editinstructions" />')
                .set('innerHTML', M.util.get_string('edittitleinstructions', 'agpu'));
    ...
    update_page_break: function(ev, button, activity, action) {
        ...
        var data = {
            ...
            'id':    Y.agpu.mod_quiz.util.slot.getId(nextactivity),
            'value': value
        };
        ...
        this.send_request(data, spinner, function(response) {
            if (response.slots) {
                if (action === 'addpagebreak') {
                    Y.agpu.mod_quiz.util.page.add(activity);
                } else {
                    var page = activity.next(Y.agpu.mod_quiz.util.page.SELECTORS.PAGE);
                    Y.agpu.mod_quiz.util.page.remove(page, true);
                }
                this.reorganise_edit_page();
            }
        });
    ...
    update_dependency: function(ev, button, activity, action) {
        ...
        this.send_request(data, spinner, function(response) {
            if (response.hasOwnProperty('requireprevious')) {
                Y.agpu.mod_quiz.util.slot.updateDependencyIcon(activity, response.requireprevious);
            }
        });
    ...
    reorganise_edit_page: function() {
        Y.agpu.mod_quiz.util.slot.reorderSlots();
        Y.agpu.mod_quiz.util.slot.reorderPageBreaks();
        Y.agpu.mod_quiz.util.page.reorderPages();
        Y.agpu.mod_quiz.util.slot.updateOneSlotSections();
        Y.agpu.mod_quiz.util.slot.updateAllDependencyIcons();
    },
 ...
 *
 * @module agpu-mod_quiz-toolboxes
...
M.mod_quiz.init_section_toolbox = function(config) {
    ...
    "requires": [
        ...
        "event-key",
        "io",
        "agpu-mod_quiz-quizbase",
        "agpu-mod_quiz-util-slot",
        "agpu-core-notification-ajaxexception"
    ]
});
