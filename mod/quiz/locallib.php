 ...
 * Functions that are also called by core agpu are in {@link lib.php}
...
function quiz_create_attempt(quiz_settings $quizobj, $attemptnumber, $lastattempt, $timenow, $ispreview = false, $userid = null) {
    ...
    if ($quiz->sumgrades < grade_calculator::ALMOST_ZERO && $quiz->grade > grade_calculator::ALMOST_ZERO) {
        throw new agpu_exception('cannotstartgradesmismatch', 'quiz',
                new agpu_url('/mod/quiz/view.php', ['q' => $quiz->id]),
    ...
    if ($attemptnumber == 1 || !$quiz->attemptonlast) {
        ...
        if (empty($lastattempt)) {
            throw new \agpu_exception('cannotfindprevattempt', 'quiz');
        }
        $attempt = $lastattempt;
    }
...
function quiz_start_new_attempt($quizobj, $quba, $attempt, $attemptnumber, $timenow,
    ...
    foreach ($quizobj->get_questions(null, false) as $questiondata) {
        ...
        if ($questiondata->status == \core_question\local\bank\question_version_status::QUESTION_STATUS_DRAFT) {
            throw new agpu_exception('questiondraftonly', 'mod_quiz', '', $questiondata->name);
        }
    ...
    if ($randomfound) {
        ...
        foreach ($quizobj->get_questions(null, false) as $questiondata) {
            ...
            if ($questionid === null) {
                throw new agpu_exception('notenoughrandomquestions', 'quiz',
...
function quiz_start_attempt_built_on_last($quba, $attempt, $lastattempt) {
    ...
    foreach ($oldquba->get_attempt_iterator() as $oldslot => $oldqa) {
        ...
        if ($question->status == \core_question\local\bank\question_version_status::QUESTION_STATUS_DRAFT) {
            throw new agpu_exception('questiondraftonly', 'mod_quiz', '', $question->name);
        }
...
function quiz_delete_attempt($attempt, $quiz) {
    ...
    if (!$attempt->preview) {
        ...
        // This class callback is deprecated, and will be removed in agpu 4.8 (MDL-80327).
...
function quiz_override_summary(stdClass $quiz, cm_info|stdClass $cm, int $currentgroup = 0): array {
    ...
    $accessallgroups = ($quizgroupmode == NOGROUPS) ||
            has_capability('agpu/site:accessallgroups', context_module::instance($cm->id));
...
function quiz_get_user_timeclose($courseid) {
    ...
    if (has_capability('agpu/course:update', context_course::instance($courseid))) {
...
function quiz_question_edit_button($cmid, $question, $returnurl, $contentaftericon = '') {
    ...
    // Build the icon.
    if ($action) {
        if ($returnurl instanceof agpu_url) {
            $returnurl = $returnurl->out_as_local_url(false);
        }
        $questionparams = ['returnurl' => $returnurl, 'cmid' => $cmid, 'id' => $question->id];
        $questionurl = new agpu_url("$CFG->wwwroot/question/bank/editquestion/question.php", $questionparams);
 ...
 * @return agpu_url to preview this question with the options from this quiz.
 */
...
function quiz_get_flag_option($attempt, $context) {
    global $USER;
    if (!has_capability('agpu/question:flag', $context)) {
...
function quiz_get_review_options($quiz, $attempt, $context) {
    ...
    if (!empty($attempt->id)) {
        $options->questionreviewlink = new agpu_url('/mod/quiz/reviewquestion.php',
                ['attempt' => $attempt->id]);
    }
    ...
    if (!empty($attempt->id) && $attempt->state == quiz_attempt::FINISHED && !$attempt->preview &&
        ...
        $options->manualcommentlink = new agpu_url('/mod/quiz/comment.php',
                ['attempt' => $attempt->id]);
    }

    if (!is_null($context) && !$attempt->preview &&
            has_capability('mod/quiz:viewreports', $context) &&
            has_capability('agpu/grade:viewhidden', $context)) {
 ...
 *      subject and body.
 * @param bool $studentisonline is the student currently interacting with agpu?
 *
...
function quiz_send_notification_messages($course, $quiz, $attempt, $context, $cm, $studentisonline) {
    ...
    if (is_array($groups) && count($groups) > 0) {
        ...
        // 'agpu/site:accessallgroups' get notified.
        $groups = -1;
    } else {
        $groups = '';
    }
 ...
 * @deprecated Since agpu 4.3 MDL-72321
 * @todo Final deprecation in agpu 4.7 MDL-78091
 */
    ...
    if ($lastattempt && ($lastattempt->state == quiz_attempt::IN_PROGRESS ||
        ...
        if ($lastattempt->state == quiz_attempt::ABANDONED || $lastattempt->state == quiz_attempt::FINISHED) {
            if ($redirect) {
                ...
                throw new agpu_exception('attemptalreadyclosed', 'quiz', $quizobj->view_url());
            }
        }
...
function quiz_create_attempt_handling_errors($attemptid, $cmid = null) {
    try {
        ...
        if (!empty($cmid)) {
            ...
            $continuelink = new agpu_url('/mod/quiz/view.php', ['id' => $cmid]);
            $context = context_module::instance($cm->id);
            if (has_capability('mod/quiz:preview', $context)) {
                throw new agpu_exception('attempterrorcontentchange', 'quiz', $continuelink);
            } else {
                throw new agpu_exception('attempterrorcontentchangeforuser', 'quiz', $continuelink);
            }
        } else {
            throw new agpu_exception('attempterrorinvalid', 'quiz');
        }
    }
