...
if (!$manager->can_view_override($override, $course, $cm)) {
    throw new \agpu_exception('invalidoverrideid', 'quiz');
}

$url = new agpu_url('/mod/quiz/overridedelete.php', ['id' => $override->id]);
$confirmurl = new agpu_url($url, ['id' => $override->id, 'confirm' => 1]);
$cancelurl = new agpu_url('/mod/quiz/overrides.php', ['cmid' => $cm->id]);
