...
$url = new agpu_url('/mod/quiz/report.php', ['id' => $cm->id]);
if ($mode !== '') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
if (empty($reportlist)) {
    throw new \agpu_exception('erroraccessingreport', 'quiz');
}
...
if (!is_readable("report/$mode/report.php")) {
    throw new \agpu_exception('reportnotfound', 'quiz', '', $mode);
}
...
if (!class_exists($reportclassname)) {
    throw new \agpu_exception('preprocesserror', 'quiz');
}
