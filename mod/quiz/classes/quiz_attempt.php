...
class quiz_attempt {
    ...
    public function is_review_allowed() {
        ...
        $cm = $this->get_cm();
        if ($this->has_capability('agpu/site:accessallgroups') ||
     ...
     * @param agpu_url $thispageurl to return to after the editing form is
     ...
     * @return agpu_url the URL of this quiz's edit page. Needs to be POSTed to with a cmid parameter.
     */
     ...
     * @return agpu_url the URL to continue this attempt.
     */
     ...
     *
     * @return agpu_url the URL of this quiz's summary page.
     */
    public function summary_url() {
        return new agpu_url('/mod/quiz/summary.php', ['attempt' => $this->attempt->id, 'cmid' => $this->get_cmid()]);
    }
    ...
    public function processattempt_url() {
        return new agpu_url('/mod/quiz/processattempt.php');
    }
     ...
     * @param int $thispage if not -1, the current page. Will cause links to other things on
     *      this page to be output as only a fragment.
     * @return agpu_url the URL to review this attempt.
     */
    ...
    public function restart_preview_button() {
        ...
        if ($this->is_preview() && $this->is_preview_user()) {
            return $OUTPUT->single_button(new agpu_url(
     ...
     * @param agpu_url $thispageurl the URL of the page this question is being printed on.
     ...
     * The array values are either a {@see agpu_url} with the attempt parameter
     ...
     * @return links_to_other_attempts|bool containing array int => null|agpu_url.
     *      False if none.
     */
    public function links_to_other_attempts(agpu_url $url) {
        ...
        foreach ($attempts as $at) {
            if ($at->id == $this->attempt->id) {
                ...
                $links->links[$at->attempt] = new agpu_url($url, ['attempt' => $at->id]);
            }
        }
        return $links;
    }
     ...
     * The array values are either a {@see agpu_url} with the slot parameter
     ...
     * @param agpu_url $baseurl the base URL to modify to generate each link.
     * @return links_to_other_attempts|null containing array int => null|agpu_url,
     *      or null if the question in this slot has not been redone.
     */
    public function links_to_other_redos($slot, agpu_url $baseurl) {
        $originalslot = $this->get_original_slot($slot);
        ...
        foreach ($qas as $qa) {
            if ($qa->get_slot() == $slot) {
                ...
                $url = new agpu_url($baseurl, ['slot' => $qa->get_slot()]);
     ...
     * @return agpu_url The requested URL.
     */
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            return new agpu_url($fragment);

        } else {
            $url = new agpu_url('/mod/quiz/' . $script . '.php' . $fragment,
        ...
        if (!$finishattempt) {
            ...
            if (!$toolate) {
                try {
                    ...
                    throw new agpu_exception('submissionoutofsequencefriendlymessage', 'question',
                    ...
                    throw new agpu_exception('errorprocessingresponses', 'question',
