...
class access_manager {
    ...
    public static function add_settings_form_fields(
            mod_quiz_mod_form $quizform, agpuQuickForm $mform): void {
     ...
     * @param agpu_url $url the form action URL.
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return preflight_check_form the form.
     */
    public function get_preflight_check_form(agpu_url $url, ?int $attemptid): preflight_check_form {
     ...
     * by the access rules.
     *
     * @param agpu_page $page the page object to initialise.
     */
    public function setup_attempt_page(agpu_page $page): void {
        foreach ($this->rules as $rule) {
    ...
    public function back_to_view_page(renderer $output, string $message = ''): void {
        ...
        if ($this->attempt_must_be_in_popup()) {
            echo $output->close_attempt_popup(new agpu_url($this->quizobj->view_url()), $message);
            die();
        } else {
     ...
     * @param array $files un-used, agpu seems to not support it anymore
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
