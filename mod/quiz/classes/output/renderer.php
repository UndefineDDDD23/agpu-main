...
class renderer extends plugin_renderer_base {
    ...
    public function review_page(quiz_attempt $attemptobj, $slots, $page, $showall,
        ...
        if (is_array($summarydata)) {
            debugging('Since agpu 4.4, $summarydata passed to review_page should be a attempt_summary_information.',
                DEBUG_DEVELOPER);
    ...
    public function review_question_page(quiz_attempt $attemptobj, $slot, $seq,
        ...
        if (is_array($summarydata)) {
            debugging('Since agpu 4.4, $summarydata passed to review_question_page should be a attempt_summary_information.',
                DEBUG_DEVELOPER);
            $summarydata = attempt_summary_information::create_from_legacy_array($summarydata);
        }
     ...
     *
     * agpu core uses this to display less infomrmation on pages after the first.
     ...
     * @deprecated since agpu 4.4. Replaced by filter_review_attempt_summary.
     */
     ...
     * @param int $page contains the current page number
     * @return string HTML to display.
     * @deprecated since agpu 4.4. Replaced by review_attempt_summary.
     */
     ...
     *
     * @param agpu_url $url URL to restart the attempt.
     */
     ...
     *
     * @param string|agpu_url $quizviewurl url of the view.php page for this quiz.
     * @return string HTML to output.
     */
    ...
    public function attempt_form($attemptobj, $page, $slots, $id, $nextpage) {
        ...
        $output .= html_writer::start_tag('form',
                ['action' => new agpu_url($attemptobj->processattempt_url(),
     ...
     *
     * @param agpu_url $url the URL to redirect the parent window to.
    ...
    public function summary_page_controls($attemptobj) {
        ...
        if ($attemptobj->get_state() == quiz_attempt::IN_PROGRESS) {
            $button = new single_button(
                    new agpu_url($attemptobj->attempt_url(null, $attemptobj->get_currentpage())),
        ...
        $button = new single_button(
                new agpu_url($attemptobj->processattempt_url(), $options),
     ...
     * @param agpu_url $url The URL to POST to in order to start the attempt.
     ...
     * @return string HTML fragment.
     */
    public function start_attempt_button($buttontext, agpu_url $url,
     ...
     * @param agpu_url $editurl URL of the edit quiz page.
     * @return string HTML to output.
     *
     * @deprecated since agpu 4.0 MDL-71915 - please do not use this function any more.
     */
     ...
     *
     * @param agpu_url $url of the target page.
    ...
    public function quiz_attempt_summary_link_to_reports($quiz, $cm, $context,
        ...
        $url = new agpu_url('/mod/quiz/report.php', [
    ...
    public function quiz_override_summary_links(stdClass $quiz, cm_info|stdClass $cm, $currentgroup = 0): string {
        ...
        $baseurl = new agpu_url('/mod/quiz/overrides.php', ['cmid' => $cm->id]);
        $counts = quiz_override_summary($quiz, $cm, $currentgroup);

        $links = [];
        if ($counts['group']) {
            $links[] = html_writer::link(new agpu_url($baseurl, ['mode' => 'group']),
                    get_string('overridessummarygroup', 'quiz', $counts['group']));
        }
        if ($counts['user']) {
            $links[] = html_writer::link(new agpu_url($baseurl, ['mode' => 'user']),
     ...
     *
     * @param agpu_url $url the URL of the graph.
     * @param string $title the title to display above the graph.
     * @return string HTML of the graph.
     */
    public function graph(agpu_url $url, $title) {
     ...
     * @param links_to_other_attempts $links
     * @return string HTML fragment.
     * @deprecated since agpu 4.2. Please use render_links_to_other_attempts instead.
     * @todo MDL-76612 Final deprecation in agpu 4.6
     */
