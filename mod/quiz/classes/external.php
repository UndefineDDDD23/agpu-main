...
class mod_quiz_external extends external_api {
    ...
    public static function start_attempt($quizid, $preflightdata = [], $forcenew = false) {
        ...
        if (!$quizobj->has_questions()) {
            throw new agpu_exception('noquestionsfound', 'quiz', $quizobj->view_url());
        }
        ...
        if (!$quizobj->is_preview_user() && $messages) {
            ...
            if ($accessmanager->is_preflight_check_required($currentattemptid)) {
                ...
                if (!empty($errors)) {
                    throw new agpu_exception(array_shift($errors), 'quiz', $quizobj->view_url());
                }
            ...
            if ($currentattemptid) {
                if ($lastattempt->state == quiz_attempt::OVERDUE) {
                    throw new agpu_exception('stateoverdue', 'quiz', $quizobj->view_url());
                } else {
                    throw new agpu_exception('attemptstillinprogress', 'quiz', $quizobj->view_url());
                }
            }
    ...
    protected static function validate_attempt($params, $checkaccessrules = true, $failifoverdue = true) {
        ...
        if ($attemptobj->get_userid() != $USER->id) {
            throw new agpu_exception('notyourattempt', 'quiz', $attemptobj->view_url());
        }
        ...
        if ($checkaccessrules) {
            ...
            if (!$ispreviewuser && $messages) {
                throw new agpu_exception('attempterror', 'quiz', $attemptobj->view_url());
            }
        }

        // Attempt closed?.
        if ($attemptobj->is_finished()) {
            throw new agpu_exception('attemptalreadyclosed', 'quiz', $attemptobj->view_url());
        } else if ($failifoverdue && $attemptobj->get_state() == quiz_attempt::OVERDUE) {
            throw new agpu_exception('stateoverdue', 'quiz', $attemptobj->view_url());
        }
        ...
        if ($accessmanager->is_preflight_check_required($attemptobj->get_attemptid())) {
            ...
            if (!empty($errors)) {
                throw new agpu_exception(array_shift($errors), 'quiz', $attemptobj->view_url());
            }
            // Pre-flight check passed.
        ...
        if (isset($params['page'])) {
            ...
            if ($params['page'] != $attemptobj->force_page_number_into_range($params['page'])) {
                throw new agpu_exception('Invalid page number', 'quiz', $attemptobj->view_url());
            }

            // Prevent out of sequence access.
            if (!$attemptobj->check_page_access($params['page'])) {
                throw new agpu_exception('Out of sequence access', 'quiz', $attemptobj->view_url());
            }
            ...
            if (empty($slots)) {
                throw new agpu_exception('noquestionsfound', 'quiz', $attemptobj->view_url());
            }
        }
     ...
     * @since  agpu 3.1
     * @since agpu 3.2 blockedbyprevious parameter added.
     */
    ...
    protected static function validate_attempt_review($params) {
        ...
        if ($attemptobj->is_own_attempt()) {
            if (!$attemptobj->is_finished()) {
                throw new agpu_exception('attemptclosed', 'quiz', $attemptobj->view_url());
            } else if (!$displayoptions->attempt) {
                throw new agpu_exception('noreview', 'quiz', $attemptobj->view_url(), null,
                    $attemptobj->cannot_review_message());
            }
        } else if (!$attemptobj->is_review_allowed()) {
            throw new agpu_exception('noreviewattempt', 'quiz', $attemptobj->view_url());
        }
    ...
    public static function get_quiz_feedback_for_grade($quizid, $grade) {
        ...
        $result = [];
        $result['feedbacktext'] = '';
        $result['feedbacktextformat'] = FORMAT_agpu;
