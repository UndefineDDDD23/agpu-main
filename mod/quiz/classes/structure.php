...
class structure {
    ...
    public function has_use_capability(int $slotnumber): bool {
        ...
        if (is_numeric($slot->questionid)) {
            ...
            return has_capability('agpu/question:useall', $context);
        }
    }
    ...
    public function check_can_be_edited() {
        if (!$this->can_be_edited()) {
            ...
            throw new \agpu_exception('cannoteditafterattempts', 'quiz',
                    new \agpu_url('/mod/quiz/edit.php', ['cmid' => $this->get_cmid()]), $reportlink);
        }
    }
    ...
    public function move_slot($idmove, $idmoveafter, $page) {
        ...
        if (empty($movingslot)) {
            throw new \agpu_exception('Bad slot ID ' . $idmove);
        }
     ...
     * It is only possible to add a random question if the user has the agpu/question:useall capability
    ...
    public function can_add_random_questions() {
        if ($this->canaddrandom === null) {
            ...
            $usablecontexts = $relatedcontexts->having_cap('agpu/question:useall');
    ...
    public function add_random_questions(int $addonpage, int $number, array $filtercondition): void {
        ...
        if (!$category) {
            new \agpu_exception('invalidcategoryid');
        }

        $catcontext = \context::instance_by_id($category->contextid);
        require_capability('agpu/question:useall', $catcontext);
