...
class seb_quiz_settings extends persistent {
    ...
    private function process_seb_upload_config() {
        ...
        if (empty($file)) {
            throw new agpu_exception('noconfigfilefound', 'quizaccess_seb', '', $this->get('cmid'));
        } else {
    ...
    private function process_required_enforced_settings() {
        ...
        $quizurl = new agpu_url($CFG->wwwroot . "/mod/quiz/view.php", ['id' => $this->get('cmid')]);
    ...
    /**
     * Use the boolean map to add agpu boolean setting to config PList.
     */
     ...
     *
     * @return array agpu setting as key, SEB setting as value.
     */
