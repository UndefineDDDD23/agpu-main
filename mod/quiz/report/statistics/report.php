...
class quiz_statistics_report extends report_base {
    ...
    public function display($quiz, $cm, $course) {
        ...
        $reporturl = new agpu_url('/mod/quiz/report.php', $pageoptions);
        ...
        if ($everything) { // Implies is downloading.
            ...
            if (!$questionstats->has_subq($qid, $variantno)) {
                throw new \agpu_exception('questiondoesnotexist', 'question');
            }
            ...
            if ($variantno === null &&
                ...
                $this->table->define_baseurl(new agpu_url($reporturl, ['slot' => $slot]));
     ...
     *
     * @param agpu_url $reporturl the base URL of the report.
     * @return string HTML.
     */
    protected function everything_download_options(agpu_url $reporturl) {
        global $OUTPUT;
     ...
     *                                   QUIZ_GRADEAVERAGE, QUIZ_GRADEHIGHEST, QUIZ_ATTEMPTLAST or QUIZ_ATTEMPTFIRST
     *                                   we calculate stats based on which attempts would affect the grade for each student.
     * @param agpu_url $reporturl url for this report
     * @return string HTML.
     */
    protected function output_caching_info($lastcachetime, $quizid, $groupstudentsjoins, $whichattempts, $reporturl) {
        ...
        $recalcualteurl = new agpu_url($reporturl,
