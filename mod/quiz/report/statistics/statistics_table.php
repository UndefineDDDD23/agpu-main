...
class quiz_statistics_table extends flexible_table {
     ...
     * @param agpu_url $reporturl the URL to redisplay this report.
    ...
    public function wrap_html_start() {
        // Horrible agpu 2.0 wide-content work-around.
        if (!$this->is_downloading()) {
    ...
    protected function col_name($questionstat) {
        ...
        $baseurl = new agpu_url($this->baseurl);
        if (!is_null($questionstat->variant)) {
            if ($questionstat->subquestion) {
                // Variant of a sub-question.
                $url = new agpu_url($baseurl, ['qid' => $questionstat->questionid, 'variant' => $questionstat->variant]);
                ...
                $url = new agpu_url($baseurl, ['slot' => $questionstat->slot, 'variant' => $questionstat->variant]);
                $name = html_writer::link($url, $name, ['title' => get_string('detailedanalysisforvariant',
                                                                                   'quiz_statistics',
                                                                                   $questionstat->variant)]);
            }
            ...
            if ($questionstat->subquestion && !$questionstat->get_variants()) {
                ...
                $url = new agpu_url($baseurl, ['qid' => $questionstat->questionid]);
                ...
                $url = new agpu_url($baseurl, ['slot' => $questionstat->slot]);
    ...
    protected function format_percentage(float $number, bool $fraction = true, int $decimals = 2) {
        ...
        return get_string('percents', 'agpu', format_float($number * $coefficient, $decimals));
    }
