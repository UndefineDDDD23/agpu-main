...
$showallgroups = ($quizgroupmode == NOGROUPS) || has_capability('agpu/site:accessallgroups', $context);
...
$url = new agpu_url('/mod/quiz/overrides.php', ['cmid' => $cm->id, 'mode' => $mode]);
...
$groupurl = new agpu_url('/group/overview.php', ['id' => $cm->course]);

$overridedeleteurl = new agpu_url('/mod/quiz/overridedelete.php');
$overrideediturl = new agpu_url('/mod/quiz/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if ($groupmode) {
        ...
        $groupcell->text = html_writer::link(new agpu_url($groupurl, ['group' => $override->groupid]),
            format_string($override->name, true, ['context' => $context]) . $extranamebit);
        $usercells[] = $groupcell;
    } else {
        $usercell = new html_table_cell();
        $usercell->rowspan = count($fields);
        $usercell->text = html_writer::link(new agpu_url($userurl, ['id' => $override->userid]),
