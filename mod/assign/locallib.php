 ...
 * @todo MDL-81977 This will be deleted in agpu 4.8.
 */
...
class assign {
    ...
    public function register_return_link($action, $params) {
        ...
        if ($cm) {
            $currenturl = new agpu_url('/mod/assign/view.php', array('id' => $cm->id));
        } else {
            $currenturl = new agpu_url('/mod/assign/index.php', array('id' => $this->get_course()->id));
        }
    ...
    public function view($action='', $args = array()) {
        ...
        if ($action == 'redirect') {
            $nextpageurl = new agpu_url('/mod/assign/view.php', $nextpageparams);
            $messages = '';
        ...
        if ($callplugins) {
            ...
            foreach ($this->submissionplugins as $plugin) {
                if (!$this->update_plugin_instance($plugin, $formdata)) {
                    throw new \agpu_exception($plugin->get_error());
                    return false;
                }
            }
     ...
     * @return void
     */
    protected function add_plugin_grade_elements($grade, agpuQuickForm $mform, stdClass $data, $userid) {
        foreach ($this->feedbackplugins as $plugin) {
     ...
     * @param agpuQuickForm $mform The form to add the configuration settings to.
     ...
     * @return void
     */
    protected function add_plugin_settings(assign_plugin $plugin, agpuQuickForm $mform, & $pluginsenabled) {
        global $CFG;
     ...
     * @return void
     */
    public function add_all_plugin_settings(agpuQuickForm $mform) {
    ...
    public function should_provide_intro_attachments(int $userid): bool {
        ...
        if (has_capability('agpu/course:manageactivities', $this->get_context())) {
            return true;
        }
     ...
     *
     * Cron function to be run periodically according to the agpu cron.
     *
     * @return bool
     */
     ...
     *
     * @param agpuform $mform - Used for validation of the submitted data
     * @return string
     */
    ...
    public function get_submission_group_members($groupid, $onlyids, $excludesuspended = false) {
        ...
        if ($excludesuspended || !has_capability('agpu/course:viewsuspendedusers', $this->context)) {
    ...
    private function view_course_index() {
        ...
        if (!$cms = get_coursemodules_in_course('assign', $course->id, 'm.duedate')) {
            $o .= $this->get_renderer()->notification(get_string('thereareno', 'agpu', $strplural));
            $o .= $this->get_renderer()->continue_button(new agpu_url('/course/view.php', array('id' => $course->id)));
            return $o;
        }
        ...
        foreach ($modinfo->instances['assign'] as $cm) {
            ...
            if (has_capability('mod/assign:submit', $context) &&
                !has_capability('agpu/site:config', $context)) {
                $cangrade = false;
    ...
    protected function view_plugin_page() {
        ...
        if (!$plugin) {
            throw new \agpu_exception('invalidformdata', '');
            return;
        }
    ...
    public function can_view_grades($groupid = null) {
        ...
        if ($this->get_course_module()->effectivegroupmode == SEPARATEGROUPS) {
            ...
            $groupflag = has_capability('agpu/site:accessallgroups', $this->get_context());
    ...
    protected function download_submissions($userids = null) {
        ...
        $url = new agpu_url('/mod/assign/view.php', ['id' => $cm->id, 'action' => 'grading']);
    ...
    protected function view_single_grading_panel($args) {
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $viewfullnames = has_capability('agpu/site:viewfullnames', $this->get_context());
    ...
    protected function view_remove_submission_confirm() {
        ...
        if (!$this->can_edit_submission($userid, $USER->id)) {
            throw new \agpu_exception('nopermission');
        }
        ...
        $urlparams = array('id' => $this->get_course_module()->id,
                           ...
                           'userid' => $userid,
                           'sesskey' => sesskey());
        $confirmurl = new agpu_url('/mod/assign/view.php', $urlparams);

        $urlparams = array('id' => $this->get_course_module()->id,
                           'action' => 'view');
        $cancelurl = new agpu_url('/mod/assign/view.php', $urlparams);
    ...
    protected function view_return_links() {
        ...
        $url = new agpu_url('/mod/assign/view.php', $params);
    ...
    protected function view_grading_table() {
        ...
        if ($suspendedparticipantsfilter !== null &&
                has_capability('agpu/course:viewsuspendedusers', $this->get_context())) {
        ...
        $currenturl = new agpu_url('/mod/assign/view.php', ['id' => $this->get_course_module()->id, 'action' => 'grading']);
        ...
        if (count($users) != 0 && $this->can_grade()) {
            ...
            $jsparams['message'] = !empty($CFG->messaging)
                && has_all_capabilities(['agpu/site:sendmessage', 'agpu/course:bulkmessaging'], $this->context);
    ...
    protected function view_notices($title, $notices) {
        ...
        $url = new agpu_url('/mod/assign/view.php', array('id'=>$this->get_course_module()->id, 'action'=>'view'));
        ...
        if ($this->is_blind_marking()) {
            ...
            if ($hasviewblind) {
                return get_string('participant', 'assign') . ' ' . $uniqueid . ' (' .
                        fullname($user, has_capability('agpu/site:viewfullnames', $this->get_context())) . ')';
            } else {
                return get_string('participant', 'assign') . ' ' . $uniqueid;
            }
        } else {
            return fullname($user, has_capability('agpu/site:viewfullnames', $this->get_context()));
        }
    }
    ...
    public function can_view_submission($userid) {
        ...
        if (!$this->is_active_user($userid) && !has_capability('agpu/course:viewsuspendedusers', $this->context)) {
            return false;
        }
     ...
     *
     * @param agpuform $mform - null unless form validation has failed
     * @return string
     */
    ...
    public function can_edit_submission($userid, $graderid = 0) {
        ...
        if (groups_get_activity_groupmode($cm) == SEPARATEGROUPS &&
                !has_capability('agpu/site:accessallgroups', $this->context, $graderid)) {
     ...
     *
     * @param agpuform|null $mform If validation failed when submitting this form - this is the agpuform.
     *               It can be null.
     ...
     * Save extension date.
     *
     * @param agpuform $mform The submitted form
     * @return boolean
     */
     ...
     * @todo Final deprecation in agpu 6.0. See MDL-82876.
     * @return void
     */
    ...
    public function save_submission(stdClass $data, & $notices) {
        ...
        // Get the flags to check if it is locked.
        if ($flags && $flags->locked) {
            throw new \agpu_exception('submissionslocked', 'assign');
            return true;
        }
     ...
     * @return void
     */
    public function add_grade_form_elements(agpuQuickForm $mform, stdClass $data, $params) {
        ...
        $capabilitylist = array('gradereport/grader:view', 'agpu/grade:viewall');
        ...
        if (has_all_capabilities($capabilitylist, $this->get_course_context())) {
            ...
            $url = new agpu_url('/grade/report/grader/index.php', $urlparams);
        ...
        if ($this->get_instance()->markingworkflow &&
            ...
            $viewfullnames = has_capability('agpu/site:viewfullnames', $this->context);
            foreach ($markers as $marker) {
     ...
     * @return void
     */
    public function add_submission_form_elements(agpuQuickForm $mform, stdClass $data) {
        global $USER;
    ...
    protected function process_save_grade(&$mform) {
        ...
        if (!$userid) {
            if (empty($SESSION->mod_assign_useridlist[$this->get_useridlist_key($useridlistid)])) {
                ...
                $url = new agpu_url('/mod/assign/view.php', array('id' => $this->get_course_module()->id));
                throw new agpu_exception('useridlistnotcached', 'mod_assign', $url);
            }
        ...
        if (is_null($this->showonlyactiveenrol)) {
            ...
            if (!is_null($this->context)) {
                $this->showonlyactiveenrol = $this->showonlyactiveenrol ||
                            !has_capability('agpu/course:viewsuspendedusers', $this->context);
            }
        }
    ...
    protected function add_grade_notices() {
        if (has_capability('mod/assign:grade', $this->get_context()) && get_config('assign', 'has_rescaled_null_grades_' . $this->get_instance()->id)) {
            $link = new \agpu_url('/mod/assign/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades'));
    ...
    protected function view_fix_rescaled_null_grades() {
        ...
        if ($confirm) {
            ...
            $continue = new \agpu_url('/mod/assign/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades', 'confirm' => true, 'sesskey' => sesskey()));
            $cancel = new \agpu_url('/mod/assign/view.php', array('id' => $this->get_course_module()->id));
    ...
    protected function get_submissionstatement($adminconfig, $instance, $context) {
        ...
        if (!$instance->teamsubmission) {
            ...
            if (!empty($adminconfig->submissionstatement)) {
                ...
                $submissionstatement = format_text($adminconfig->submissionstatement, FORMAT_agpu, $options);
            }
