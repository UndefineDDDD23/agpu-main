 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assign_override_form extends agpuform {
    ...
    protected function definition() {
        ...
        $accessallgroups = ($assigngroupmode == NOGROUPS) || has_capability('agpu/site:accessallgroups', $this->context);
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new agpu_url('/mod/assign/overrides.php', array('cmid' => $cm->id));
                    throw new \agpu_exception('groupsnone', 'assign', $link);
                }
            ...
            if ($this->userid) {
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new agpu_url('/mod/assign/overrides.php', array('cmid' => $cm->id));
                    throw new \agpu_exception('usersnone', 'assign', $link);
                }
