...
class assignfeedback_file_zip_importer {
    ...
    public function is_valid_filename_for_import($assignment, $fileinfo, $participants, & $users, & $plugin, & $filename) {
        ...
        while (!empty($pathparts)) {
            ...
            // export in agpu >= 4.1) resp. fullname_userid_plugintype_pluginname_ (as created by earlier versions). We ensure
    ...
    /**
     * Returns a mapping from unique user / group ids in folder names to array of agpu users.
     *
     * @param assign $assignment  - The assignment instance
