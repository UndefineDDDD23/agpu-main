...
EOD;
    ...
    protected static function generate_page_images_for_attempt($assignment, $userid, $attemptnumber, $resetrotation = true) {
        ...
        if ($status === combined_document::STATUS_FAILED) {
            throw new \agpu_exception('Could not generate combined pdf.');
        ...
        for ($i = 0; $i < $pagecount; $i++) {
            try {
                if (empty($images[$i])) {
                    throw new \agpu_exception('error image');
                }
                $image = $images[$i];
                if (!$resetrotation) {
                ...
                // We catch only agpu_exception here as other exceptions indicate issue with setup not the pdf.
    ...
    public static function get_page_images_for_attempt($assignment, $userid, $attemptnumber, $readonly = false) {
        ...
        if (empty($pages) && $readonly) {
            throw new \agpu_exception('Could not find readonly pages for grade ' . $grade->id);
        }
