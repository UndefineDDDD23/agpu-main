...
if ($action === 'pollconversions') {
    ...
    try {
        ...
        if (in_array($response->status, $completestatuslist)) {
            ...
            foreach ($pages as $id => $pagefile) {
                ...
                $page->url = agpu_url::make_pluginfile_url($context->id,
    ...
    $response->url = '';
    if ($file) {
        $url = agpu_url::make_pluginfile_url($assignment->get_context()->id,
                                               'assignfeedback_editpdf',
    ...
    $page = new stdClass();
    $page->url = agpu_url::make_pluginfile_url($context->id, document_services::COMPONENT, $filearea,
