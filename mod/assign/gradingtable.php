...
class assign_grading_table extends table_sql implements renderable {
    ...
    public function __construct(assign $assignment,
        ...
        $url = new agpu_url($CFG->wwwroot . '/mod/assign/view.php', $urlparams);
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($markers === null) {
            ...
            $viewfullnames = has_capability('agpu/site:viewfullnames', $this->assignment->get_context());
        ...
        if ($this->is_downloading()) {
            if (isset($markers[$row->allocatedmarker])) {
                return fullname($markers[$row->allocatedmarker],
                        has_capability('agpu/site:viewfullnames', $this->assignment->get_context()));
            } else {
                return '';
            }
        }
     ...
     * @todo Final deprecation in agpu 6.0. See MDL-82336.
     */
    ...
    public function col_status(stdClass $row) {
        ...
        if (!$this->is_downloading() && $this->hasgrade) {
            ...
            $baseactionurl = new agpu_url('/mod/assign/view.php', [
                'id' => $this->assignment->get_course_module()->id,
                'userid' => $row->id,
                'sesskey' => sesskey(),
                'page' => $this->currpage,
    ...
    public function can_view_all_grades() {
        ...
        return has_capability('gradereport/grader:view', $context) &&
               has_capability('agpu/grade:viewall', $context);
    }
