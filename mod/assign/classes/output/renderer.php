...
class renderer extends \plugin_renderer_base {
    ...
    public function render_assign_user_summary(\assign_user_summary $summary) {
        ...
        if ($summary->blindmarking) {
            ...
            $url = new \agpu_url('/user/view.php', $urlparams);
    ...
    public function render_assign_submit_for_grading_page($page) {
        ...
        $cancelurl = new \agpu_url('/mod/assign/view.php', array('id' => $page->coursemoduleid));
        if (count($page->notifications)) {
            ...
            $o .= $this->agpuform($page->confirmform);
        }
        $o .= $this->output->container_end();
    ...
    public function render_assign_grading_summary(\assign_grading_summary $summary) {
        ...
        if (isset($summary->cm)) {
            $currenturl = new \agpu_url('/mod/assign/view.php', array('id' => $summary->cm->id));
    ...
    public function render_assign_attempt_history(\assign_attempt_history $history) {
        ...
        $this->page->requires->yui_module('agpu-mod_assign-history', 'Y.one("#' . $containerid . '").history');
    ...
    public function render_assign_course_index_summary(\assign_course_index_summary $indexsummary) {
        ...
        foreach ($indexsummary->assignments as $info) {
            ...
            $link = \html_writer::link(new \agpu_url('/mod/assign/view.php', $params),
                                      $info['cmname']);
            $due = $info['timedue'] ? userdate($info['timedue']) : '-';

            if ($info['cangrade']) {
                $params['action'] = 'grading';
                $gradeinfo = \html_writer::link(new \agpu_url('/mod/assign/view.php', $params),
     ...
     * @return string HTML
     */
    protected function agpuform(\agpuform $mform) {
