...
class grading_actionmenu implements templatable, renderable {
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        $resetlink = new agpu_url('/mod/assign/view.php', ['id' => $this->cmid, 'action' => 'grading']);
        ...
        if ($showquickgrading) {
            $quickgradingbaseurl = new agpu_url('/mod/assign/view.php', [
                'id' => $this->assign->get_course_module()->id,
                'action' => 'grading',
            ]);
            if ($userid) {
        ...
        if ($this->showdownload) {
            $downloadasfoldersbaseurl = new agpu_url('/mod/assign/view.php', [
                'id' => $this->assign->get_course_module()->id,
                'action' => 'grading',
            ]);
            if ($userid) {
        ...
        if ($actions) {
            ...
            foreach ($actions as $groupkey => $actiongroup) {
                foreach ($actiongroup as $label => $url) {
                    $menu->add(new \action_menu_link_secondary(new \agpu_url($url), null, $label));
                }
    ...
    private function get_actions() {
        ...
        if (
            ...
            && has_capability('agpu/grade:viewall', $this->assign->get_course_context())
        ) {
            $url = new agpu_url('/grade/report/grader/index.php', ['id' => $this->assign->get_course()->id]);
    ...
    private function get_extra_filters_dropdown(): ?dialog {
        ...
        $dropdowncontentdata = [
            'actionurl' => (new agpu_url('/mod/assign/view.php'))->out(false),
            'id' => $this->assign->get_course_module()->id,
            'action' => 'grading',
            'filters' => [],
        ];
        ...
        if (has_capability('agpu/course:viewsuspendedusers', $this->assign->get_context())) {
