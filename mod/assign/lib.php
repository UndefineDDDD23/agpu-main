...
/**
 * This file contains the agpu hooks for the assign module.
 *
...
/**
 * Return the list if agpu features this module supports
 *
...
function assign_extend_settings_navigation(settings_navigation $settings, navigation_node $navref) {
    ...
    if (has_capability('mod/assign:manageoverrides', $settings->get_page()->cm->context)) {
        $url = new agpu_url('/mod/assign/overrides.php', ['cmid' => $settings->get_page()->cm->id, 'mode' => 'user']);
    ...
    if (has_capability('mod/assign:revealidentities', $context)) {
        ...
        if ($assignment && $assignment->blindmarking && !$assignment->revealidentities) {
            ...
            $url = new agpu_url('/mod/assign/view.php', $urlparams);
    ...
    if ($assign->can_view_grades()) {
        $url = new agpu_url('/mod/assign/view.php', ['id' => $settings->get_page()->cm->id, 'action' => 'grading']);
        $navref->add(
    ...
    foreach ($submissions as $submission) {
        ...
        if (empty($showrecentsubmissions)) {
            if (!array_key_exists($cm->id, $grader)) {
                $grader[$cm->id] = has_capability('agpu/grade:viewall', $context);
            }
            if (!$grader[$cm->id]) {
                continue;
            }
        ...
        if ($groupmode == SEPARATEGROUPS &&
                !has_capability('agpu/site:accessallgroups',  $context)) {
            if (isguestuser()) {
...
function assign_get_recent_mod_activity(&$activities,
    ...
    $grader          = has_capability('agpu/grade:viewall', $cmcontext);
    $accessallgroups = has_capability('agpu/site:accessallgroups', $cmcontext);
    $viewfullnames   = has_capability('agpu/site:viewfullnames', $cmcontext);
...
function assign_get_extra_capabilities() {
    return ['gradereport/grader:view', 'agpu/grade:viewall'];
}
...
function assign_get_file_info($browser,
    ...
    if ($filearea === ASSIGN_INTROATTACHMENT_FILEAREA || $filearea === ASSIGN_ACTIVITYATTACHMENT_FILEAREA) {
        if (!has_capability('agpu/course:managefiles', $context)) {
...
function assign_user_outline($course, $user, $coursemodule, $assignment) {
    ...
    $result = new stdClass();
    if (!$gradingitem->hidden || has_capability('agpu/grade:viewhidden', context_course::instance($course->id))) {
 ...
 *
 * This is used to determine global visibility of an event in all places throughout agpu. For example,
...
function mod_assign_core_calendar_provide_event_action(calendar_event $event,
    ...
    if ($event->eventtype == ASSIGN_EVENT_TYPE_GRADINGDUE) {
        ...
        $url = new \agpu_url('/mod/assign/view.php', [
            'id' => $cm->id,
            'action' => 'grader'
        ]);
