...
function url_print_workaround($url, $cm, $course) {
    ...
    $fullurl = new agpu_url(url_get_full_url($url, $cm, $course));
...
function url_display_embed($url, $cm, $course) {
    ...
    $agpuurl = new agpu_url($fullurl);
    $link = html_writer::link($agpuurl, format_string($cm->name));
    ...
    } else if ($mediamanager->can_embed_url($agpuurl, $embedoptions)) {
        // Media (audio/video) file.
        $code = $mediamanager->embed_url($agpuurl, $title, 0, 0, $embedoptions);
...
function url_guess_icon($fullurl, $unused = null) {
    ...
    try {
        ...
        // That will make agpu_url class to throw an exception, so we need to catch the exception to prevent errors.
        $agpuurl = new agpu_url($fullurl);
        $fullurl = $agpuurl->out_omit_querystring();
