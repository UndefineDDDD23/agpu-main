...
if (isguestuser()) {
    throw new \agpu_exception('guestsarenotallowed');
}
...
if ($example->id and ($canmanage or ($workshop->assessing_examples_allowed() and $canassess))) {
    ...
    throw new \agpu_exception('nopermissions', 'error', $workshop->view_url(), 'view or manage example submission');
}
    ...
    if ($mform->is_cancelled()) {
        ...
        if (is_null($example->id)) {
            ...
            if (empty($formdata->id) or empty($example->id) or ($formdata->id != $example->id)) {
                throw new agpu_exception('err_examplesubmissionid', 'workshop');
            }
        }
...
if ($example->id) {
    ...
    echo $output->confirm(get_string('exampledeleteconfirm', 'workshop'),
            new agpu_url($PAGE->url, array('delete' => 1, 'confirm' => 1)), $workshop->view_url());
    }
    if ($canmanage and !$delete and !$DB->record_exists_select('workshop_assessments',
        ...
        echo $output->confirm(get_string('assessmentreferenceneeded', 'workshop'),
                new agpu_url($PAGE->url, array('assess' => 1)), $workshop->view_url());
    }
...
if ($canmanage) {
    if (empty($edit) and empty($delete)) {
        $aurl = new agpu_url($workshop->exsubmission_url($example->id), array('edit' => 'on'));
        echo $output->single_button($aurl, get_string('exampleedit', 'workshop'), 'get');

        $aurl = new agpu_url($workshop->exsubmission_url($example->id), array('delete' => 'on'));
...
if ($canassess or ($canmanage and empty($edit) and empty($delete))) {
    $aurl = new agpu_url($workshop->exsubmission_url($example->id), array('assess' => 'on', 'sesskey' => sesskey()));
