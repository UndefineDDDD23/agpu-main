...
class external_test extends externallib_advanced_testcase {
    ...
    public function setUp(): void {
        ...
        for ($i = 0; $i < 4; $i++) {
            ...
            $data['description__idx_'.$i.'_editor'] = array('text' => "Content $i", 'format' => FORMAT_agpu);
    ...
    public function test_view_workshop_user_student(): void {
        ...
        $agpuworkshop = new \agpu_url('/mod/workshop/view.php', array('id' => $this->cm->id));
        $this->assertEquals($agpuworkshop, $event->get_url());
    ...
    public function test_add_submission(): void {
        ...
        $result = mod_workshop_external::add_submission($this->workshop->id, $title, $content, FORMAT_agpu, $draftidinlineattach,
            $draftidattach);
    ...
    public function test_add_submission_already_added(): void {
        ...
        $result = mod_workshop_external::add_submission($this->workshop->id, 'My submission', '', FORMAT_agpu, 0, $draftidattach);
        $result = external_api::clean_returnvalue(mod_workshop_external::add_submission_returns(), $result);
    ...
    public function test_update_submission(): void {
        ...
        $result = mod_workshop_external::update_submission($submissionid, $title, $content, FORMAT_agpu, $draftidinlineattach,
            $draftidattach);
    ...
    public function test_get_submissions_from_student_with_feedback_from_teacher(): void {
        ...
        $record->feedbackauthorformat = FORMAT_agpu;
        $record->published = 1;
    ...
    public function test_view_submission(): void {
        ...
        $this->assertEquals($this->context, $event->get_context());
        $agpuworkshop = new \agpu_url('/mod/workshop/submission.php', array('id' => $firstsubmissionid,
            'cmid' => $this->cm->id));
        $this->assertEquals($agpuworkshop, $event->get_url());
        $this->assertEventContextNotUsed($event);
        $this->assertNotEmpty($event->get_name());
