...
if ($canmanage) {
    ...
    throw new \agpu_exception('nopermissions', 'error', $workshop->view_url(), 'compare example assessment');
}
...
if ($isreviewer) {
    ...
    if ($workshop->assessing_examples_allowed()) {
        $assessment->add_action(
            new agpu_url($workshop->exsubmission_url($example->id), array('assess' => 'on', 'sesskey' => sesskey())),
