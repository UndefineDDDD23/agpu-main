...
class mod_workshop_external extends external_api {
    ...
    public static function get_user_plan($workshopid, $userid = 0) {
        ...
        foreach ($userplan['phases'] as $phasecode => $phase) {
            ...
            foreach ($userplan['phases'][$phasecode]['tasks'] as $taskcode => $task) {
                $task->code = $taskcode;
                if ($task->link instanceof agpu_url) {
            ...
            foreach ($userplan['phases'][$phasecode]['actions'] as $actioncode => $action) {
                if ($action->url instanceof agpu_url) {
    ...
    public static function add_submission_parameters() {
        return new external_function_parameters(array(
            ...
            'contentformat' => new external_value(PARAM_INT, 'The format used for the content', VALUE_DEFAULT, FORMAT_agpu),
     ...
     * @throws agpu_exception
     */
    public static function add_submission($workshopid, $title, $content = '', $contentformat = FORMAT_agpu,
        ...
        if (!$canaddsubmission) {
            throw new agpu_exception('nopermissions', 'error', '', 'add submission');
        }
        ...
        if (empty($submission->title)) {
            throw new agpu_exception('errorinvalidparam', 'webservice', '', 'title');
        }
     ...
     * @throws agpu_exception
     */
    public static function update_submission($submissionid, $title, $content = '', $contentformat = FORMAT_agpu,
            $inlineattachmentsid = 0, $attachmentsid = 0) {
        global $USER, $DB;
        ...
        if (!$canupdatesubmission) {
            throw new agpu_exception('nopermissions', 'error', '', 'update submission');
        }
    ...
    public static function delete_submission($submissionid) {
        ...
        if (!has_capability('mod/workshop:deletesubmissions', $context)) {
            ...
            if (!$candeletesubmission) {
                throw new agpu_exception('nopermissions', 'error', '', 'delete submission');
            }
        }
    ...
    protected static function validate_submission($submission, workshop $workshop) {
        ...
        if ($canview) {
            ...
            throw new agpu_exception('nopermissions', 'error', '', 'view submission');
        }
    }
    ...
    protected static function check_view_submission_assessments($submission, workshop $workshop) {
        ...
        if ($canview) {
            ...
            throw new agpu_exception('nopermissions', 'error', '', 'view assessment');
        }
    }
    ...
    public static function get_reviewer_assessments($workshopid, $userid = 0) {
        ...
        if (empty($params['userid']) || $params['userid'] == $USER->id) {
            ...
            if ($workshop->phase < workshop::PHASE_ASSESSMENT) {    // Can view assessments only in assessment phase onwards.
                throw new agpu_exception('nopermissions', 'error', '', 'view assessments');
            }
        } else {
    ...
    public static function evaluate_assessment_parameters() {
        return new external_function_parameters(
            array(
                ...
                'feedbackformat' => new external_value(PARAM_INT, 'The feedback format for text.', VALUE_DEFAULT, FORMAT_agpu),
     ...
     * @throws agpu_exception
     */
    public static function evaluate_assessment($assessmentid, $feedbacktext = '', $feedbackformat = FORMAT_agpu, $weight = 1,
            $gradinggradeover = '') {
        global $DB;
        ...
        if (!$canoverridegrades && !$cansetassessmentweight) {
            throw new agpu_exception('nopermissions', 'error', '', 'evaluate assessments');
        }
    ...
    public static function get_grades_report($workshopid, $groupid = 0, $sortby = 'lastname', $sortdirection = 'ASC',
        ...
        throw new agpu_exception('nothingfound', 'workshop');
    }
     ...
     * @throws agpu_exception
     */
    public static function evaluate_submission($submissionid, $feedbacktext = '', $feedbackformat = FORMAT_agpu, $published = 1,
            $gradeover = '') {
        global $DB;
        ...
        if (!$canpublish && !$canoverride) {
            throw new agpu_exception('nopermissions', 'error', '', 'evaluate submission');
        }
