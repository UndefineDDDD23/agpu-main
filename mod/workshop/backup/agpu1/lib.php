 ...
 */
class agpu1_mod_workshop_handler extends agpu1_mod_handler {
    ...
    /** @var agpu1_file_manager for the current workshop */
    protected $fileman = null;

    /** @var agpu1_inforef_manager */
    protected $inforefman = null;

    /** @var array list of agpu1_workshopform_handler instances */
     ...
     *
     * Note that the path /agpu_BACKUP/COURSE/MODULES/MOD/WORKSHOP does not
    ...
    public function get_paths() {
        return array(
            new convert_path('workshop', '/agpu_BACKUP/COURSE/MODULES/MOD/WORKSHOP'),
            new convert_path('workshop_elements', '/agpu_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS'),
            new convert_path(
                'workshop_element', '/agpu_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS/ELEMENT',
                array(
                    'dropfields' => array(
                        'stddev',
                        'totalassessments',
            ...
            new convert_path('workshop_element_rubric', '/agpu_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS/ELEMENT/RUBRICS/RUBRIC'),
        );
    }

    /**
     * This is executed every time we have one /agpu_BACKUP/COURSE/MODULES/MOD/WORKSHOP
     * data available
     */
    public function process_workshop($data, $raw) {
        ...
        $this->currentworkshop['intro'] = agpu1_converter::migrate_referenced_files($this->currentworkshop['intro'], $this->fileman);
    ...
    /**
     * Processes one <ELEMENT> tag from agpu.xml
     */
    ...
    /**
     * Processes one <RUBRIC> tag from agpu.xml
     */
     ...
     * @return agpu1_workshopform_handler the instance of the handler
     */
    protected function get_strategy_handler($strategy) {
        ...
        if (is_null($this->strategyhandlers)) {
            ...
            foreach ($subplugins as $name => $dir) {
                ...
                $handlerclass = "agpu1_workshopform_{$name}_handler";
                ...
                if (!$this->strategyhandlers[$name] instanceof agpu1_workshopform_handler) {
                    throw new agpu1_convert_exception('wrong_handler_class', get_class($this->strategyhandlers[$name]));
                }
            }
        }

        if (!isset($this->strategyhandlers[$strategy])) {
            throw new agpu1_convert_exception('usupported_subplugin', 'workshopform_'.$strategy);
        }
 ...
 */
abstract class agpu1_workshopform_handler extends agpu1_submod_handler {

    /**
     * @param agpu1_mod_handler $workshophandler the handler of a module we are subplugin of
     * @param string $subpluginname the name of the subplugin
     */
    public function __construct(agpu1_mod_handler $workshophandler, $subpluginname) {
