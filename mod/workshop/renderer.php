...
class mod_workshop_renderer extends plugin_renderer_base {
    ...
    protected function render_workshop_assessment(workshop_assessment $assessment) {
        ...
        if (($assessment->url instanceof agpu_url) and ($this->page->url != $assessment->url)) {
        ...
        if (!is_null($assessment->form)) {
            ...
            $o .= $this->output->container(self::agpuform($assessment->form), 'assessment-form');
     ...
     * @param agpu_url $url base URL object.
     * @return string HTML.
     */
    public function initials_bars(workshop $workshop, agpu_url $url): string {
    ...
    protected function helper_submission_attachments($submissionid, $format = 'html') {
        ...
        foreach ($files as $file) {
            ...
            $fileurl    = agpu_url::make_pluginfile_url($ctx->id, 'mod_workshop', 'submission_attachment',
                            $submissionid, $filepath, $filename, true);
            $embedurl   = agpu_url::make_pluginfile_url($ctx->id, 'mod_workshop', 'submission_attachment',
                            $submissionid, $filepath, $filename, false);
            $embedurl   = new agpu_url($embedurl, array('preview' => 'bigthumb'));
            $type       = $file->get_mimetype();
            $image      = $this->output->pix_icon(file_file_icon($file), get_mimetype_description($file), 'agpu', array('class' => 'icon'));
    ...
    protected function helper_grading_report_submission(stdclass $participant) {
        ...
        if (is_null($participant->submissionid)) {
            ...
            $url = new agpu_url('/mod/workshop/submission.php',
    ...
    protected function helper_grading_report_assessment($assessment, $shownames, array $userinfo, $separator) {
        ...
        $url = new agpu_url('/mod/workshop/assessment.php',
    ...
    /**
     * Helper method dealing with the fact we can not just fetch the output of agpuforms
     *
     * @param agpuform $mform
     * @return string HTML
     */
    protected static function agpuform(agpuform $mform) {
            ...
            case workshop::PHASE_SUBMISSION:
                ...
                if (has_capability('mod/workshop:submit', $this->page->context) && (!$examplesmust || $examplesdone)) {
                    if (!$workshop->get_submission_by_author($USER->id)) {
                        $btnurl = new agpu_url($workshop->submission_url(), ['edit' => 'on']);
        ...
        switch ($workshop->phase) {
            case workshop::PHASE_SETUP:
                ...
                if ($workshop->useexamples && has_capability('mod/workshop:manageexamples', $this->page->context)) {
                    ...
                    if ($workshop->grading_strategy_instance()->form_ready()) {
                        ...
                        $aurl = new agpu_url($workshop->exsubmission_url(0), ['edit' => 'on']);
            ...
            case workshop::PHASE_SUBMISSION:
                ...
                if (has_capability('mod/workshop:viewallsubmissions', $this->page->context)) {
                    ...
                    if ($groupmode == SEPARATEGROUPS && !has_capability('agpu/site:accessallgroups', $workshop->context)) {
                    ...
                    if ($data) {
                        ...
                        $baseurl = new agpu_url($this->page->url, ['sortby' => $sortby, 'sorthow' => $sorthow]);
            ...
            case workshop::PHASE_EVALUATION:
                if (has_capability('mod/workshop:viewallassessments', $this->page->context)) {
                    ...
                    if ($data) {
                        ...
                        if (has_capability('mod/workshop:overridegrades', $this->page->context)) {
                            ...
                            $form = $evaluator->get_settings_form(new agpu_url($workshop->aggregate_url(),
                ...
                if (has_capability('mod/workshop:overridegrades', $workshop->context)) {
                    ...
                    $url = new agpu_url($workshop->toolbox_url('clearaggregatedgrades'));
                    ...
                    // Clear assessments.
                    $url = new agpu_url($workshop->toolbox_url('clearassessments'));
