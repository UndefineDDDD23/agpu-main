...
if ($submission->id and ($ownsubmission or $canviewall or $isreviewer)) {
    ...
    throw new \agpu_exception('nopermissions', 'error', $workshop->view_url(), 'view or create submission');
}
...
if ($deletable and $delete) {
    ...
    echo $output->confirm($prompt, new agpu_url($PAGE->url, ['delete' => 1, 'confirm' => 1]), $workshop->view_url());
}
...
if (!$delete) {
    ...
    if ($editable) {
        if ($submission->id) {
            $btnurl = new agpu_url($PAGE->url, array('edit' => 'on', 'id' => $submission->id));
            $btntxt = get_string('editsubmission', 'workshop');
        } else {
            $btnurl = new agpu_url($PAGE->url, array('edit' => 'on'));
    ...
    if ($submission->id and $deletable) {
        $url = new agpu_url($PAGE->url, array('delete' => 1));
    ...
    if ($submission->id and !$edit and !$isreviewer and $canallocate and $workshop->assessing_allowed($USER->id)) {
        $url = new agpu_url($PAGE->url, array('assess' => 1));
