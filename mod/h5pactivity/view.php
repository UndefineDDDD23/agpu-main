...
$file = reset($files);
$fileurl = agpu_url::make_pluginfile_url($file->get_contextid(), $file->get_component(),
...
if (!$manager->can_submit()) {
    ...
    if (!$manager->is_tracking_enabled()) {
        if (has_capability('agpu/course:manageactivities', $context)) {
            $url = new agpu_url('/course/modedit.php', ['update' => $cm->id]);
...
if ($manager->can_view_all_attempts() && $manager->is_tracking_enabled()) {
    $extraactions[] = new action_link(
        new agpu_url('/mod/h5pactivity/report.php', ['id' => $cm->id]),
