...
function resource_display_embed($resource, $cm, $course, $file) {
    ...
    $agpuurl = agpu_url::make_pluginfile_url($context->id, 'mod_resource', 'content', $resource->revision,
    ...
    if (file_mimetype_in_typegroup($mimetype, 'web_image')) {  // It's an image
        $code = resourcelib_embed_image($agpuurl->out(), $title);

    } else if ($mimetype === 'application/pdf') {
        // PDF document
        $code = resourcelib_embed_pdf($agpuurl->out(), $title, $clicktoopen);
        ...
        $agpuurl->param('embed', 1);

        // anything else - just try object tag enlarged as much as possible
        $code = resourcelib_embed_general($agpuurl, $title, $clicktoopen, $mimetype);
    }
