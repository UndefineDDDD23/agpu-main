...
$url = new agpu_url('/mod/glossary/import.php', array('id'=>$id));
if ($mode !== 'letter') {
    $url->param('mode', $mode);
}
if ($hook !== 'ALL') {
...
if ($form->is_cancelled()) {
    redirect(new agpu_url('view.php', ['id' => $id]));
}
...
if ($xml = glossary_read_imported_file($result)) {
    ...
    if ($data->dest == 'newglossary') {
        ...
        if ( $xmlglossary['NAME'][0]['#'] ) {
            ...
            $glossary->introformat = isset($xmlglossary['INTROFORMAT'][0]['#']) ? $xmlglossary['INTROFORMAT'][0]['#'] : FORMAT_agpu;
    ...
    for($i = 0; $i < $sizeofxmlentries; $i++) {
        ...
        if (!is_string($definition)) {
            throw new \agpu_exception('errorparsingxml', 'glossary');
        }
