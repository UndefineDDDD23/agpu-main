...
$url = new agpu_url('/mod/glossary/exportentry.php', array('id'=>$id,'prevmode'=>$prevmode));
if ($confirm !== 0) {
...
if (!$entry = $DB->get_record('glossary_entries', array('id'=>$id))) {
    throw new \agpu_exception('invalidentry');
}
...
if (!data_submitted() or !$confirm or !confirm_sesskey()) {
    ...
    echo $OUTPUT->confirm($areyousure, new agpu_url($linkyes, $optionsyes), new agpu_url($linkno, $optionsno));
    echo '</div>';
    echo $OUTPUT->footer();
    die;
