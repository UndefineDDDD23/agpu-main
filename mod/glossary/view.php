...
$url = new agpu_url('/mod/glossary/view.php', array('id'=>$cm->id));
if (isset($mode) && $mode) {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
if ($allentries) {
    ...
    //Build paging bar
    $baseurl = new agpu_url('/mod/glossary/view.php', ['id' => $id, 'mode' => $mode, 'hook' => $hook,
        ...
        if ($printpivot) {
            ...
            if ($currentpivot != $upperpivot) {
                ...
                // printing the user icon if defined (only when browsing authors)
                    ...
                    $pivottoshow = fullname($user, has_capability('agpu/site:viewfullnames', context_course::instance($course->id)));
                } else {
                    echo '<th >';
                }
    ...
    if ($allentries instanceof agpu_recordset) {
        $allentries->close();
    }
}
if ( !$entriesshown ) {
