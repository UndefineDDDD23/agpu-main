...
$url = new agpu_url('/mod/forum/user.php', array('id' => $userid));
if ($isspecificcourse) {
...
$hasparentaccess = $isparent && has_all_capabilities(array('agpu/user:viewdetails', 'agpu/user:readuserposts'), $usercontext);
...
if (empty($result->posts)) {
    ...
    if ($iscurrentuser) {
        ...
        if (isset($courseid) && $courseid != SITEID) {
            ...
            if ($mode == 'posts') {
                $navbar = $PAGE->navbar->add(get_string('posts', 'forum'), new agpu_url('/mod/forum/user.php',
                        array('id' => $user->id, 'course' => $courseid)));
            } else {
                $navbar = $PAGE->navbar->add(get_string('discussions', 'forum'), new agpu_url('/mod/forum/user.php',
