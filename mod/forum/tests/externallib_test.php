 ...
 * @package    mod_forum
 * @category   external
 * @copyright  2012 Mark Nelson <markn@agpu.com>
...
final class externallib_test extends externallib_advanced_testcase {
     ...
     * @param array $values
     * @param \agpu_url|null $url
     * @return array
     */
    protected function get_expected_attachment(\stored_file $file, array $values  = [], ?\agpu_url $url = null): array {
        if (!$url) {
    ...
    public function test_mod_forum_get_forums_by_courses(): void {
        ...
        // Execute real agpu enrolment as we'll call unenrol() method on the instance later.
    ...
    public function test_mod_forum_get_discussion_posts(): void {
        ...
        $expectedposts['posts'][] = array(
            ...
            'urls' => [
                ...
                'delete' =>null,
                'split' => null,
                'reply' => (new \agpu_url('/mod/forum/post.php#mformforum', [
    ...
    public function test_add_discussion_post(): void {
        ...
        global $DB; // Yes, we are going to use DB facilities too, because cannot rely on other functions for checking
                    ...
                    // processing.
        $formats = [FORMAT_PLAIN, FORMAT_agpu, FORMAT_MARKDOWN, FORMAT_HTML];
        $options = [];
        ...
        $createdpost = mod_forum_external::add_discussion_post($discussion->firstpost,
            'interesting subject', 'with some https://example.com link', $options, FORMAT_agpu);
        $createdpost = external_api::clean_returnvalue(mod_forum_external::add_discussion_post_returns(), $createdpost);
                ...
                'userposts' => [
                    [
                        ...
                        'urls' => [
                            ...
                            'edit' => (new \agpu_url('/mod/forum/post.php', [
                                'edit' => $discussion1reply1->id
                            ]))->out(false),
                            'delete' => (new \agpu_url('/mod/forum/post.php', [
                                'delete' => $discussion1reply1->id
                            ]))->out(false),
                            'split' => (new \agpu_url('/mod/forum/post.php', [
    ...
    /**
     * Test that we can update the message format of a post to {@see FORMAT_agpu}
     */
    public function test_update_discussion_post_set_message_format_agpu(): void {
        global $DB, $USER;
        ...
        $result = external_api::clean_returnvalue(
            mod_forum_external::update_discussion_post_returns(),
            mod_forum_external::update_discussion_post($discussion->firstpost, '', 'Update discussion message', FORMAT_agpu)
        );
        $this->assertTrue($result['status']);
