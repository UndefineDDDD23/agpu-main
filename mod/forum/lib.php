    ...
    if ($forum->type == 'single') {  // Update related discussion and post.
        ...
        if (!empty($discussions)) {
            ...
            if (! $discussion = $DB->get_record('forum_discussions', array('forum'=>$forum->id))) {
                throw new \agpu_exception('cannotadd', 'forum');
            }
        }
        if (! $post = $DB->get_record('forum_posts', array('id'=>$discussion->firstpost))) {
            throw new \agpu_exception('cannotfindfirstpost', 'forum');
        }
...
/**
 * @deprecated since agpu 3.3, when the block_course_overview block was removed.
 */
...
function forum_is_user_group_discussion(cm_info $cm, $discussiongroupid) {
    ...
    if (has_capability('agpu/site:accessallgroups', context_module::instance($cm->id)) ||
...
function forum_print_recent_activity($course, $viewfullnames, $timestart) {
    ...
    foreach ($printposts as $post) {
        ...
        $discussionurl = new agpu_url('/mod/forum/discuss.php', ['d' => $post->discussion]);
...
function forum_get_readable_forums($userid, $courseid=0) {
    ...
    if (!$forummod = $DB->get_record('modules', array('name' => 'forum'))) {
        throw new \agpu_exception('notinstalled', 'forum');
    }
    ...
    foreach ($courses as $course) {
        ...
        foreach ($modinfo->instances['forum'] as $forumid => $cm) {
         ...
         /// group access
            if (groups_get_activity_groupmode($cm, $course) == SEPARATEGROUPS and !has_capability('agpu/site:accessallgroups', $context)) {
...
function forum_get_discussions($cm, $forumsort="", $fullpost=true, $unused=-1, $limit=-1,
    ...
    if ($groupmode) {
        ...
        if ($groupmode == VISIBLEGROUPS or has_capability('agpu/site:accessallgroups', $modcontext)) {
            if ($currentgroup) {
...
function forum_get_discussion_neighbours($cm, $discussion, $forum) {
    ...
    $groupselect = '';
    if ($groupmode) {
        if ($groupmode == VISIBLEGROUPS || has_capability('agpu/site:accessallgroups', $modcontext)) {
            if ($currentgroup) {
...
function forum_rating_validate($params) {
    ...
    if ($discussion->groupid > 0 and $groupmode = groups_get_activity_groupmode($cm, $course)) {   // Groups are being used
        ...
        if (!groups_is_member($discussion->groupid) and !has_capability('agpu/site:accessallgroups', $context)) {
...
function forum_get_discussion_subscription_icon($forum, $discussionid, $returnurl = null, $includetext = false) {
    ...
    $subscriptionlink = new agpu_url('/mod/forum/subscribe.php', array(
        'sesskey' => sesskey(),
        'id' => $forum->id,
        'd' => $discussionid,
        'returnurl' => $returnurl,
...
function forum_print_mode_form($id, $mode, $forumtype='') {
    ...
    if ($forumtype == 'single') {
        $select = new single_select(
            new agpu_url("/mod/forum/view.php",
            array('f' => $id)),
            'mode',
        ...
        $select = new single_select(
            new agpu_url("/mod/forum/discuss.php",
            array('d' => $id)),
            'mode',
            forum_get_layout_modes($useexperimentalui),
            $mode,
...
function forum_print_attachments($post, $cm, $type) {
    ...
    if ($files) {
        ...
        foreach ($files as $file) {
            ...
            $iconimage = $OUTPUT->pix_icon(file_file_icon($file), get_mimetype_description($file), 'agpu', array('class' => 'icon'));
...
function forum_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    ...
    if ($discussion->groupid > 0 && !has_capability('agpu/site:accessallgroups', $context)) {
        $groupmode = groups_get_activity_groupmode($cm, $course);
...
function forum_add_new_post($post, $mform, $unused = null) {
    ...
    // Let agpu know that assessable content is uploaded (eg for plagiarism detection)
...
function forum_post_subscription($fromform, $forum, $discussion) {
    ...
    if (\mod_forum\subscriptions::is_forcesubscribed($forum)) {
        ...
        if ($subscribed && !has_capability('agpu/course:manageactivities', context_course::instance($forum->course), $USER->id)) {
...
function forum_get_subscribe_link($forum, $context, $messages = array(), $cantaccessagroup = false, $unused1 = true,
    ...
    if (\mod_forum\subscriptions::is_forcesubscribed($forum)) {
        ...
        $url = new agpu_url('/mod/forum/subscribe.php', $options);
...
function forum_user_can_post($forum, $discussion, $user=NULL, $cm=NULL, $course=NULL, $context=NULL) {
    ...
    if (has_capability('agpu/site:accessallgroups', $context)) {
        return true;
    }

    if ($groupmode == VISIBLEGROUPS) {
        if ($discussion->groupid == -1) {
            // allow students to reply to all participants discussions - this was not possible in agpu <1.8
            return true;
        }
...
function forum_user_can_see_group_discussion($discussion, $cm, $context) {
    ...
    if ($discussion->groupid > 0) {
        $groupmode = groups_get_activity_groupmode($cm);
        if ($groupmode == SEPARATEGROUPS) {
            return groups_is_member($discussion->groupid) || has_capability('agpu/site:accessallgroups', $context);
        }
    }
...
function forum_user_can_see_post($forum, $discussion, $post, $user = null, $cm = null, $checkdeleted = true) {
    ...
    if (!$canviewdiscussion && !has_all_capabilities(array('agpu/user:viewdetails', 'agpu/user:readuserposts'), context_user::instance($post->userid))) {
        return false;
    }
...
function forum_get_recent_mod_activity(&$activities, &$index, $timestart, $courseid, $cmid, $userid=0, $groupid=0)  {
    ...
    $accessallgroups = has_capability('agpu/site:accessallgroups', $cm_context);
...
function forum_print_recent_mod_activity($activity, $courseid, $detail, $modnames, $viewfullnames) {
    ...
    $discussionurl = new agpu_url('/mod/forum/discuss.php', ['d' => $content->discussion]);
    ...
    if ($authorhidden) {
        ...
        $userurl = new agpu_url('/user/view.php');
...
function forum_tp_count_forum_unread_posts($cm, $course, $resetreadcache = false) {
    ...
    $forumcontext = context_module::instance($cm->id);
    if (has_any_capability(['agpu/site:accessallgroups', 'mod/forum:readprivatereplies'], $forumcontext)) {
        return $readcache[$course->id][$forumid];
    }
...
function forum_check_blocking_threshold($thresholdwarning) {
    if (!empty($thresholdwarning) && !$thresholdwarning->canpost) {
        throw new \agpu_exception($thresholdwarning->errorcode,
...
/**
 * This function is used by the reset_course_userdata function in agpulib.
...
function forum_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $forumnode) {
    ...
    if ($canmanage) {
        ...
        $allowchoiceaction = new action_link(
            new agpu_url('/mod/forum/subscribe.php', [
                'id' => $forumobject->id,
    ...
    if (has_capability('mod/forum:viewsubscribers', $settingsnav->get_page()->context)) {
        $url = new agpu_url('/mod/forum/subscribers.php', ['id' => $forumobject->id, 'edit' => 'off']);
    ...
    if (isloggedin() && !isguestuser()) {
        ...
        foreach ($reportnames as $reportname) {
            if (has_capability("forumreport/{$reportname}:view", $settingsnav->get_page()->context)) {
                ...
                $reportlink = new agpu_url("/mod/forum/report/{$reportname}/index.php", $reportlinkparams);
    ...
    if ($enrolled && forum_tp_can_track_forums($forumobject)) { // keep tracking info for users with suspended enrolments
        if ($forumobject->trackingtype == FORUM_TRACKING_OPTIONAL
            ...
            $url = new agpu_url('/mod/forum/settracking.php', array(
                    'id' => $forumobject->id,
                    'sesskey' => sesskey(),
                ));
    ...
    if ($enablerssfeeds && $forumobject->rsstype && $forumobject->rssarticles && $hascourseaccess) {
        ...
        $url = new agpu_url(rss_get_url($settingsnav->get_page()->cm->context->id, $userid, "mod_forum",
            $forumobject->id));
    ...
    if ($capabilitymanager->can_export_forum($USER)) {
        $url = new agpu_url('/mod/forum/export.php', ['id' => $forumobject->id]);
 ...
 *
 * @global agpu_database $DB The database connection
...
function forum_get_posts_by_user($user, array $courses, $musthaveaccess = false, $discussionsonly = false, $limitfrom = 0, $limitnum = 50) {
    ...
    $hascapsonuser = $hascapsonuser && has_all_capabilities(array('agpu/user:viewdetails', 'agpu/user:readuserposts'), $usercontext);
    ...
    foreach ($courses as $course) {
        ...
        if ($iscurrentuser || $hascapsonuser) {
            ...
            if (!is_viewing($coursecontext, $user) && !is_enrolled($coursecontext, $user)) {
                ...
                if ($musthaveaccess) {
                    throw new \agpu_exception('errorenrolmentrequired', 'forum');
                }
                continue;
            }
        } else {
            ...
            if (groups_get_course_groupmode($course) == SEPARATEGROUPS && $course->groupmodeforce
              && !has_capability('agpu/site:accessallgroups', $coursecontext) && !has_capability('agpu/site:accessallgroups', $coursecontext, $user->id)) {
                ...
                if (empty($intersect)) {
                    ...
                    if ($musthaveaccess) {
                        throw new \agpu_exception("groupnotamember", '', $CFG->wwwroot."/course/view.php?id=$course->id");
                    }
                    continue;
                }
            }
    ...
    if (empty($return->courses)) {
        ...
        if ($musthaveaccess) {
            throw new \agpu_exception('permissiondenied');
        } else {
            return $return;
        }
    }
    ...
    foreach ($return->courses as $course) {
        ...
        foreach ($modinfo->get_instances_of('forum') as $forumid => $cm) {
            ...
            if (!$iscurrentuser && !$hascapsonuser) {
                ...
                if (groups_get_activity_groupmode($cm, $course) == SEPARATEGROUPS and !has_capability('agpu/site:accessallgroups', $cm->context)) {
...
function forum_set_user_maildigest($forum, $maildigest, $user = null) {
    ...
    if (!isset($digestoptions[$maildigest])) {
        throw new agpu_exception('invaliddigestsetting', 'mod_forum');
    }
...
function mod_forum_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    $postsurl = new agpu_url('/mod/forum/user.php', array('id' => $user->id));
    if (!empty($course)) {
    ...
    $discussionssurl = new agpu_url('/mod/forum/user.php', array('id' => $user->id, 'mode' => 'discussions'));
    if (!empty($course)) {
...
function mod_forum_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        get_string('view'),
        new \agpu_url('/mod/forum/view.php', ['id' => $cm->id]),
        $itemcount,
        true
    );
}
...
function mod_forum_extend_navigation_user(
    ...
    if (!empty($CFG->navadduserpostslinks) && $coursecontext instanceof \core\context\system) {
        ...
        $forumtab->add(
            ...
            new agpu_url('/mod/forum/user.php', $baseargs),
        );
        $forumtab->add(
            get_string('discussions', 'forum'),
            new agpu_url('/mod/forum/user.php',
