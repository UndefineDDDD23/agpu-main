...
$PAGE->set_url(new agpu_url('/mod/forum/search.php', $params));
...
if (!$search || $showform) {
    ...
    $url = new agpu_url('/mod/forum/index.php', array('id' => $course->id));
    $PAGE->navbar->add($strforums, $url);
    $url = new agpu_url('/mod/forum/search.php', array('id' => $course->id));
...
$PAGE->navbar->add($strsearch, new agpu_url('/mod/forum/search.php', array('id'=>$course->id)));
...
$url    = new agpu_url("/mod/forum/search.php", $params);
foreach ($tags as $tag) {
    $url .= "&tags[]=$tag";
}
...
$url = new agpu_url('search.php', array('search' => $search, 'id' => $course->id, 'perpage' => $perpage));
