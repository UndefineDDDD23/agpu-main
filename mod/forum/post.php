...
$prefilledpostformat = optional_param('postformat', FORMAT_agpu, PARAM_INT);
...
if (!isloggedin() or isguestuser()) {
    ...
    if (!empty($forum)) {
        ...
        if (empty($forumentity)) {
            throw new \agpu_exception('invalidforumid', 'forum');
        }
    } else if (!empty($reply)) {
        // User is writing a new reply.
        $forumentity = $forumvault->get_from_post_id($reply);
        if (empty($forumentity)) {
            throw new \agpu_exception('invalidparentpostid', 'forum');
        }
    }
...
if (!empty($forum)) {
        ...
        if (!isguestuser()) {
            if (!is_enrolled($coursecontext)) {
                if (enrol_selfenrol_available($course->id)) {
                    ...
                    redirect(new agpu_url('/enrol/index.php', array('id' => $course->id,
                        'returnurl' => '/mod/forum/view.php?f=' . $forum->id)),
                        get_string('youneedtoenrol'));
                }
            }
        }
        throw new \agpu_exception('nopostforum', 'forum');
    }

    if (!$cm->visible and !has_capability('agpu/course:viewhiddenactivities', $modcontext)) {
        redirect(
    ...
    if (empty($discussionentity)) {
        throw new \agpu_exception('notpartofdiscussion', 'forum');
    }
    ...
    if (!$capabilitymanager->can_reply_to_post($USER, $discussionentity, $parententity)) {
        if (!isguestuser()) {
            ...
            if (forum_discussion_is_locked($forum, $discussion)) {
                redirect(new agpu_url('/mod/forum/discuss.php', array('d' => $discussion->id)));
            }
        }
        throw new \agpu_exception('nopostforum', 'forum');
    }
    ...
    if ($groupmode == SEPARATEGROUPS and !has_capability('agpu/site:accessallgroups', $modcontext)) {
    ...
    if (!$cm->visible and !has_capability('agpu/course:viewhiddenactivities', $modcontext)) {
        throw new \agpu_exception("activityiscurrentlyhidden");
    }

    if ($parententity->is_private_reply()) {
        throw new \agpu_exception('cannotreplytoprivatereply', 'forum');
    }
    ...
    if (empty($postentity)) {
        throw new \agpu_exception('invalidpostid', 'forum');
    }
    ...
    if (!($forum->type == 'news' && !$post->parent && $discussion->timestart > time())) {
        if (((time() - $post->created) > $CFG->maxeditingtime) and
            ...
            throw new \agpu_exception('maxtimehaspassed', 'forum', '', format_time($CFG->maxeditingtime));
        }
    }
    if (($post->userid <> $USER->id) and
        !has_capability('mod/forum:editanypost', $modcontext)) {
        throw new \agpu_exception('cannoteditposts', 'forum');
    }
    ...
    if ($prunemform->is_cancelled()) {
        ...
        $PAGE->navbar->add($subjectstr, new agpu_url('/mod/forum/discuss.php', array('d' => $discussion->id)));
...
if ($mformpost->is_cancelled()) {
    ...
    if ($fromform->edit) {
        ...
        if (!forum_update_post($updatepost, $mformpost)) {
            throw new \agpu_exception("couldnotupdate", "forum", $errordestination);
        }
        ...
        if ($fromform->id = forum_add_new_post($addpost, $mformpost)) {
            ...
            throw new \agpu_exception("couldnotadd", "forum", $errordestination);
        }
        exit;
        ...
        $redirectto = new agpu_url('/mod/forum/view.php', array('f' => $fromform->forum));
        ...
        foreach ($groupstopostto as $group) {
            if (!$capabilitymanager->can_create_discussions($USER, $group)) {
                throw new \agpu_exception('cannotcreatediscussion', 'forum');
            }
...
if (!empty($parententity) && !$capabilitymanager->can_view_post($USER, $discussionentity, $parententity)) {
    throw new \agpu_exception('cannotreply', 'forum');
}
