...
$url = new agpu_url('/mod/forum/discuss.php', array('d'=>$d));
if ($parent !== 0) {
...
if (!$discussion) {
    throw new \agpu_exception('errordiscussionnotfound', 'mod_forum');
}
...
if (!$forum) {
    throw new \agpu_exception('errorforumnotfound', 'mod_forum');
}
...
if (!$capabilitymanager->can_view_discussions($USER)) {
    throw new agpu_exception('noviewdiscussionspermission', 'mod_forum');
}
...
if ($move > 0 && confirm_sesskey()) {
    ...
    if (!$forumto = $DB->get_record('forum', ['id' => $move])) {
        throw new \agpu_exception('cannotmovetonotexist', 'forum', $return);
    }

    if (!$capabilitymanager->can_move_discussions($USER)) {
        if ($forum->get_type() == 'single') {
            throw new \agpu_exception('cannotmovefromsingleforum', 'forum', $return);
        } else {
            throw new \agpu_exception('nopermissions', 'error', $return, get_capability_string('mod/forum:movediscussions'));
        }
    }

    if ($forumto->type == 'single') {
        throw new \agpu_exception('cannotmovetosingleforum', 'forum', $return);
    }
    ...
    if (!array_key_exists($forumto->id, $forums)) {
        throw new \agpu_exception('cannotmovetonotfound', 'forum', $return);
    }

    $cmto = $forums[$forumto->id];
    if (!$cmto->uservisible) {
        throw new \agpu_exception('cannotmovenotvisible', 'forum', $return);
    }
...
if ($pin !== -1 && confirm_sesskey()) {
    if (!$capabilitymanager->can_pin_discussions($USER)) {
        throw new \agpu_exception('nopermissions', 'error', $return, get_capability_string('mod/forum:pindiscussions'));
    }
...
if (!$post = $postvault->get_from_id($parent)) {
    throw new \agpu_exception("notexists", 'forum', "$CFG->wwwroot/mod/forum/view.php?f={$forum->get_id()}");
}

if (!$capabilitymanager->can_view_post($USER, $discussion, $post)) {
    throw new \agpu_exception('noviewdiscussionspermission', 'forum', "$CFG->wwwroot/mod/forum/view.php?id={$forum->get_id()}");
}
