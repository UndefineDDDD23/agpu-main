...
if ($forumid) {
    if (!isset($forumsvisibletouser[$forumid])) {
        throw new \agpu_exception('A valid forum ID is required to generate a summary report.');
    }
    ...
    $redirecturl = new agpu_url('/mod/forum/view.php', ['id' => $forumid]);
    $numforums = 1;
...
$pageurl = new agpu_url('/mod/forum/report/summary/index.php', $pageurlparams);
...
navigation_node::override_active_url(new agpu_url('/mod/forum/report/summary/index.php',
    ['courseid' => $courseid, 'forumid' => $forumid]));

$allowbulkoperations = !$download && !empty($CFG->messaging) && has_capability('agpu/course:bulkmessaging', $context);
...
if ($download) {
    ...
    $reporturl = new agpu_url('/mod/forum/report/summary/index.php', ['courseid' => $courseid]);
