...
$url = new agpu_url('/mod/forum/subscribers.php', array('id'=>$id));
if ($group !== 0) {
    ...
    if ($subscribe) {
        ...
        foreach ($users as $user) {
            if (!\mod_forum\subscriptions::subscribe_user($user->id, $forum)) {
                throw new \agpu_exception('cannotaddsubscriber', 'forum', '', $user->id);
            }
        }
    } else if ($unsubscribe) {
        ...
        foreach ($users as $user) {
            if (!\mod_forum\subscriptions::unsubscribe_user($user->id, $forum)) {
                throw new \agpu_exception('cannotremovesubscriber', 'forum', '', $user->id);
            }
        }
    }
...
function mod_forum_filter_hidden_users(stdClass $cm, context_module $context, array $users) {
    if ($cm->visible) {
        ...
        $filteredusers = array();
        $hiddenviewers = get_users_by_capability($context, 'agpu/course:viewhiddenactivities');
