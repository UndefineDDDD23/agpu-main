...
$url = new agpu_url('/mod/forum/subscribe.php', array('id'=>$id));
if (!is_null($mode)) {
    $url->param('mode', $mode);
}
if ($user !== 0) {
...
if (!is_null($discussionid)) {
    ...
    if (!$discussion = $DB->get_record('forum_discussions', array('id' => $discussionid, 'forum' => $id))) {
        throw new \agpu_exception('invaliddiscussionid', 'forum');
    }
}
$PAGE->set_url($url);
...
if ($user) {
    ...
    if (!has_capability('mod/forum:managesubscriptions', $context)) {
        throw new \agpu_exception('nopermissiontosubscribe', 'forum');
    }
...
if ($groupmode && !$issubscribed && !has_capability('agpu/site:accessallgroups', $context)) {
    if (!groups_get_all_groups($course->id, $USER->id)) {
        throw new \agpu_exception('cannotsubscribe', 'forum');
    }
}
...
if (is_null($mode) and !is_enrolled($context, $USER, '', true)) {   // Guests and visitors can't subscribe - only enrolled
    ...
    if (isguestuser()) {
        ...
        echo $OUTPUT->confirm(get_string('subscribeenrolledonly', 'forum').'<br /><br />'.get_string('liketologin'),
                     get_login_url(), new agpu_url('/mod/forum/view.php', array('f'=>$id)));
        echo $OUTPUT->footer();
        exit;
    } else {
        // There should not be any links leading to this place, just redirect.
        redirect(
                new agpu_url('/mod/forum/view.php', array('f'=>$id)),
...
$subscribersurl = new agpu_url('/mod/forum/subscribers.php', ['id' => $id, 'edit' => $edit]);
...
if (!is_null($mode) and has_capability('mod/forum:managesubscriptions', $context)) {
    ...
    switch ($mode) {
        ...
        case FORUM_DISALLOWSUBSCRIBE : // 3
            ...
            break;
        default:
            throw new \agpu_exception(get_string('invalidforcesubscribe', 'forum'));
    }
}
...
if ($issubscribed) {
    if (is_null($sesskey)) {
        ...
        $viewurl = new agpu_url('/mod/forum/view.php', array('f' => $id));
        if ($discussionid) {
            $a = new stdClass();
    ...
    if ($discussionid === null) {
        if (\mod_forum\subscriptions::unsubscribe_user($user->id, $forum, $context, true)) {
            ...
            throw new \agpu_exception('cannotunsubscribe', 'forum', get_local_referer(false));
        }
    } else {
    ...
    if (\mod_forum\subscriptions::subscription_disabled($forum) && !has_capability('mod/forum:managesubscriptions', $context)) {
        throw new \agpu_exception('disallowsubscribe', 'forum', get_local_referer(false));
    }
    if (!has_capability('mod/forum:viewdiscussion', $context)) {
        throw new \agpu_exception('noviewdiscussionspermission', 'forum', get_local_referer(false));
    }
    if (is_null($sesskey)) {
        // We came here via link in email.
        $PAGE->set_title($course->shortname);
