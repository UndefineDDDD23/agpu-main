 ...
 *
 * It contains the great majority of functions defined by agpu
...
function wiki_search_form($cm, $search = '', $subwiki = null) {
    ...
    $data = [
        'action' => new agpu_url('/mod/wiki/search.php'),
...
function wiki_extend_navigation(navigation_node $navref, stdClass $course, stdClass $instance, cm_info $cm) {
    ...
    if (wiki_can_create_pages($context)) {
        $link = new agpu_url('/mod/wiki/create.php', ['action' => 'new', 'swid' => $subwiki->id]);
...
function wiki_get_extra_capabilities() {
    return array('agpu/comment:view', 'agpu/comment:post', 'agpu/comment:delete');
}
...
function mod_wiki_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        get_string('view'),
        new \agpu_url('/mod/wiki/view.php', ['id' => $cm->id]),
        1,
        true
    );
}
