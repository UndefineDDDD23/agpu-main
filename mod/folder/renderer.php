...
class mod_folder_renderer extends plugin_renderer_base {
    ...
    public function display_folder(stdClass $folder) {
        ...
        $canmanagecourseactivities = has_capability('agpu/course:manageactivities', $context);
        if ($canmanagefolderfiles && ($folder->display != FOLDER_DISPLAY_INLINE || !$canmanagecourseactivities)) {
            $editbutton = new single_button(new agpu_url('/mod/folder/edit.php', ['id' => $cm->id]),
        ...
        if ($downloadable) {
            $downloadbutton = new single_button(new agpu_url('/mod/folder/download_folder.php', ['id' => $cm->id]),
    ...
    protected function htmllize_tree($tree, $dir) {
        ...
        foreach ($dir['subdirs'] as $subdir) {
            $image = $this->output->pix_icon(file_folder_icon(), $subdir['dirname'], 'agpu');
        ...
        foreach ($dir['files'] as $file) {
            ...
            $url = agpu_url::make_pluginfile_url($file->get_contextid(), $file->get_component(),
            ...
            if (file_extension_in_typegroup($filename, 'web_image')) {
                ...
                $image = $this->output->pix_icon(file_file_icon($file), $filenamedisplay, 'agpu');
            }
            $filename = html_writer::tag('span', $image, array('class' => 'fp-icon')).
