...
if (!empty($typeid)) {
    ...
    if ($type->course != $courseid || $type->course == get_site()->id) {
        throw new agpu_exception('You do not have permissions to edit this tool type.');
    }
} else {
...
// Page setup.
$url = new agpu_url('/mod/lti/coursetooledit.php', ['courseid' => $courseid]);
...
if ($form->is_cancelled()) {
    ...
    redirect(new agpu_url('/mod/lti/coursetools.php', ['id' => $courseid]));
    ...
    if (!empty($data->typeid)) {
        ...
        $redirecturl = new agpu_url('/mod/lti/coursetools.php', ['id' => $courseid]);
