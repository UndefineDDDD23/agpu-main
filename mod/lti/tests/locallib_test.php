 ...
 *
 * @package    mod_lti
 * @copyright  Copyright (c) 2012 agpurooms Inc. (http://www.agpurooms.com)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
';
    ...
    public function test_lti_ensure_url_is_https(): void {
        $this->assertEquals('https://agpu.org', lti_ensure_url_is_https('http://agpu.org'));
        $this->assertEquals('https://agpu.org', lti_ensure_url_is_https('agpu.org'));
        $this->assertEquals('https://agpu.org', lti_ensure_url_is_https('https://agpu.org'));
    }
    ...
    public function test_lti_get_url_thumbprint(): void {
        ...
        $this->assertEquals('agpu.org/', lti_get_url_thumbprint('http://agpu.ORG'));
        $this->assertEquals('agpu.org/', lti_get_url_thumbprint('http://www.agpu.org'));
        $this->assertEquals('agpu.org/', lti_get_url_thumbprint('https://www.agpu.org'));
        $this->assertEquals('agpu.org/', lti_get_url_thumbprint('agpu.org'));
        $this->assertEquals('agpu.org//this/is/agpu', lti_get_url_thumbprint('http://agpu.org/this/is/agpu'));
        $this->assertEquals('agpu.org//this/is/agpu', lti_get_url_thumbprint('https://agpu.org/this/is/agpu'));
        $this->assertEquals('agpu.org//this/is/agpu', lti_get_url_thumbprint('agpu.org/this/is/agpu'));
        $this->assertEquals('agpu.org//this/is/agpu', lti_get_url_thumbprint('agpu.org/this/is/agpu?'));
        $this->assertEquals('agpu.org//this/is/agpu?foo=bar', lti_get_url_thumbprint('agpu.org/this/is/agpu?foo=bar'));
    }
    ...
    public function test_lti_load_type_from_cartridge(): void {
        ...
        $this->assertEquals('http://download.agpu.org/unittest/test.jpg', $type->lti_icon);
        $this->assertEquals('https://download.agpu.org/unittest/test.jpg', $type->lti_secureicon);
    }
    ...
    public function test_lti_load_tool_from_cartridge(): void {
        ...
        $this->assertEquals('http://download.agpu.org/unittest/test.jpg', $lti->icon);
        $this->assertEquals('https://download.agpu.org/unittest/test.jpg', $lti->secureicon);
    }
    ...
    public function test_lti_build_standard_message_institution_name_set(): void {
        ...
        $this->assertEquals('agpu-2', $message['ext_lms']);
        $this->assertEquals('agpu', $message['tool_consumer_info_product_family_code']);
...
-----END PUBLIC KEY-----';
    ...
    public function test_lti_sign_jwt(): void {
        ...
        $endpoint = 'https://www.example.com/agpu';
...
-----END PUBLIC KEY-----';
    ...
    public function test_get_tool_type_config(): void {
        ...
        $this->assertEquals('https://www.example.com/agpu', $typeconfig['platformid']);
        $this->assertEquals($type->clientid, $typeconfig['clientid']);
        $this->assertEquals($typeid, $typeconfig['deploymentid']);
        $this->assertEquals('https://www.example.com/agpu/mod/lti/certs.php', $typeconfig['publickeyseturl']);
        $this->assertEquals('https://www.example.com/agpu/mod/lti/token.php', $typeconfig['accesstokenurl']);
        $this->assertEquals('https://www.example.com/agpu/mod/lti/auth.php', $typeconfig['authrequesturl']);
    }
    ...
    /**
     * Test default orgid is host if not specified in config (tool installed in earlier version of agpu).
     */
    ...
    /**
     * Verify that empty curl responses lead to the proper agpu_exception, not to XML ValueError.
     *
