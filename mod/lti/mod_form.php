...
class mod_lti_mod_form extends agpuform_mod {
     ...
     * @throws agpu_exception if trying to init the form for the creation of a manual instance, which is no longer supported.
     */
    public function __construct($current, $section, $cm, $course) {
        ...
        if ($PAGE->has_set_url() && str_contains($PAGE->url, '/course/modedit.php')) {
            if (empty($this->typeid) && empty($current->id)) {
                throw new agpu_exception('lti:addmanualinstanceprohibitederror', 'mod_lti');
            }
        }
    ...
    protected function legacy_instance_form_definition(array $instancetypes): void {
        ...
        $launchoptions = [
            ...
            LTI_LAUNCH_CONTAINER_REPLACE_agpu_WINDOW => get_string('existing_window', 'lti'),
    ...
    public function definition() {
        ...
        if ($supportscontentitemselection) {
            $contentitemurl = new agpu_url('/mod/lti/contentitem.php');
            ...
            $selectcontentindicatorinner = $iscontentitem ?
                $OUTPUT->pix_icon('i/valid', get_string('contentselected', 'mod_lti'), 'agpu', ['class' => 'me-1'])
