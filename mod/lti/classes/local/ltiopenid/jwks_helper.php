...
class jwks_helper {
    ...
    public static function get_jwks() {
        ...
        if (!isset($details['rsa']['e']) || !isset($details['rsa']['n'])) {
            throw new \agpu_exception('Error: essential openssl keys not set');
        }
    ...
    public static function fix_jwks_alg(array $jwks, string $jwt): array {
        ...
        if (!isset($jwtheader['kid'])) {
            throw new \agpu_exception('Error: kid must be provided in JWT header.');
        }
        ...
        foreach ($jwks['keys'] as $index => $key) {
            ...
            if (!isset(static::$ltisupportedalgs[$jwtheader['alg']]) ||
                ...
                throw new \agpu_exception('Error: Alg specified in the JWT header is incompatible with the JWK key type');
            }
