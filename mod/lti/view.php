...
$url = new agpu_url('/mod/lti/view.php', array('id' => $cm->id));
$PAGE->set_url($url);
...
} else if ($launchcontainer == LTI_LAUNCH_CONTAINER_REPLACE_agpu_WINDOW) {
    if (!$forceview) {
        $url = new agpu_url('/mod/lti/launch.php', array('id' => $cm->id));
        redirect($url);
    }
...
$launchurl = new agpu_url('/mod/lti/launch.php', ['id' => $cm->id, 'triggerview' => 0]);
if ($action) {
