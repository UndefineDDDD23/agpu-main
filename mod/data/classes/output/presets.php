...
class presets implements templatable, renderable {
    ...
    /** @var agpu_url $formactionurl The action url for the form. */
     ...
     * @param agpu_url $formactionurl The action url for the form
     * @param bool $manage Whether the manage preset options should be displayed
     */
    public function __construct(manager $manager, array $presets, agpu_url $formactionurl, bool $manage = false) {
    ...
    private function get_preset_action_menu(renderer_base $output, $preset, ?int $userid): stdClass {
        ...
        $usepreseturl = new agpu_url('/mod/data/preset.php', [
            'action' => 'usepreset',
            'cmid' => $this->cmid,
        ]);
        ...
        $previewpreseturl = new agpu_url('/mod/data/preset.php', [
            'fullname' => $preset->get_fullname(),
            'action' => 'preview',
            'id' => $this->cmid,
        ]);
        ...
        if (!$preset->isplugin) {
            // Edit.
            if ($canmanage) {
                $editactionurl = new agpu_url('/mod/data/preset.php', [
                    'action' => 'edit',
                    'd' => $this->id,
                ]);
            ...
            // Export.
            $exporturl = new agpu_url('/mod/data/preset.php', [
                'presetname' => $preset->name,
                'action' => 'export',
                'd' => $this->id,
            ]);
            ...
            if ($canmanage) {
                ...
                $deleteactionurl = new agpu_url('/mod/data/preset.php', [
                    'action' => 'delete',
                    'd' => $this->id,
                ]);
     ...
     * @param agpu_url $actionurl
     * @param array $otherattributes
     * @return void
     */
    private function add_action_menu(action_menu &$actionmenu, string $actionlabel, agpu_url $actionurl,
