...
class preset_preview implements templatable, renderable {
     ...
     *
     * @param agpu_page $page the current page instance
     */
    public function prepare_page(agpu_page $page) {
        $instance = $this->manager->get_instance();
        $preset = $this->preset;
        // Add CSS and JS.
        $csscontent = $preset->get_template_content('csstemplate');
        if (!empty($csscontent)) {
            $url = new agpu_url('/mod/data/css.php', ['d' => $instance->id, 'preset' => $preset->get_fullname()]);
            $page->requires->css($url);
        }
        $jscontent = $preset->get_template_content('jstemplate');
        if (!empty($jscontent)) {
            $url = new agpu_url('/mod/data/js.php', ['d' => $instance->id, 'preset' => $preset->get_fullname()]);
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        $useurl = new agpu_url('/mod/data/field.php');
