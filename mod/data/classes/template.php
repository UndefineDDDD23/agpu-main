 ...
 *
 * @package    mod_data
 * @copyright  2022 Ferran Recio <ferran@agpu.com>
...
class template {
     ...
     *  - baseurl: an alternative entry url (agpu_url)
    ...
    public function add_options(array $options = []) {
        ...
        $baseurl = $options['baseurl'] ?? new agpu_url('/mod/data/view.php', ['id' => $cm->id]);
    ...
    protected function get_tag_edit_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        $backurl = new agpu_url($this->baseurl, [
            'rid' => $entry->id,
            'mode' => 'single',
        ]);
        $url = new agpu_url('/mod/data/edit.php', $this->baseurl->params());
        $url->params([
            'rid' => $entry->id,
            'sesskey' => sesskey(),
    ...
    protected function get_tag_actionsmenu_replacement(stdClass $entry, bool $canmanageentry): string {
        ...
        // Show more.
        if ($this->showmore) {
            $showmoreurl = new agpu_url($this->baseurl, [
                'rid' => $entry->id,
                'filter' => 1,
            ]);
        ...
        if ($canmanageentry) {
            ...
            $editurl = new agpu_url('/mod/data/edit.php', $this->baseurl->params());
            $editurl->params([
                'rid' => $entry->id,
                'sesskey' => sesskey(),
                'backto' => urlencode($backurl->out(false))
            ...
            // Delete entry.
            $deleteurl = new agpu_url($this->baseurl, [
                'delete' => $entry->id,
                'sesskey' => sesskey(),
                'mode' => 'single',
            ]);
        ...
        if (has_capability('mod/data:approve', $context) && $this->instance->approval) {
            if ($entry->approved) {
                $disapproveurl = new agpu_url($this->baseurl, [
                    'disapprove' => $entry->id,
                    'sesskey' => sesskey(),
                ]);
                $actionmenu->add(new action_menu_link_secondary(
                ...
                $approveurl = new agpu_url($this->baseurl, [
                    'approve' => $entry->id,
                    'sesskey' => sesskey(),
                ]);
                $actionmenu->add(new action_menu_link_secondary(
        ...
        if (!empty($CFG->enableportfolios)) {
            ...
            if ($canexportall || (data_isowner($entry->id) && $canexportown)) {
                ...
                $button->set_formats($formats);
                $exporturl = $button->to_html(PORTFOLIO_ADD_agpu_URL);
