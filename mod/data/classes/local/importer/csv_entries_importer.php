...
class csv_entries_importer extends entries_importer {
    ...
    public function import_csv(stdClass $cm, stdClass $data, string $encoding, string $fielddelimiter): void {
        ...
        if (empty($readcount)) {
            throw new \agpu_exception('csvfailed', 'data', "{$CFG->wwwroot}/mod/data/edit.php?d={$data->id}");
        } else {
            if (!$fieldnames = $cir->get_columns()) {
                throw new \agpu_exception('cannotreadtmpfile', 'error');
            }
            ...
            if (!empty($errorfield)) {
                throw new \agpu_exception('fieldnotmatched', 'data',
            ...
            while ($record = $cir->next()) {
                ...
                if ($recordid = data_add_record($data, 0, $authorid, $approved)) { // Add instance to data_record.
                    ...
                    $this->addedrecordsmessages[] = get_string('added', 'agpu',
