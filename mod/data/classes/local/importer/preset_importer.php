...
abstract class preset_importer {
     ...
     * @param string|null $dir the directory to look in. null if using the agpu file storage
    ...
    public function finish_import_process(bool $overwritesettings, stdClass $instance): void {
        ...
        $backurl = new \agpu_url('/mod/data/field.php', ['d' => $instance->id]);
        redirect($backurl);
    }
     ...
     * @throws \agpu_exception when the file provided as parameter (POST or GET) does not exist
     */
    ...
    public static function create_from_plugin_or_directory(manager $manager, string $pluginordirectory): preset_importer {
        ...
        if (!$pluginordirectory) {
            throw new \agpu_exception('emptypresetname', 'mod_data');
        }
        try {
            ...
            throw new \agpu_exception('errorpresetnotfound', 'mod_data', '', $pluginordirectory);
        }
    }
