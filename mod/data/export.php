...
$url = new agpu_url('/mod/data/export.php', array('d' => $d));
$PAGE->set_url($url);

if (! $data = $DB->get_record('data', array('id'=>$d))) {
    throw new \agpu_exception('wrongdataid', 'data');
}
...
if(empty($fieldrecords)) {
    if (has_capability('mod/data:managetemplates', $context)) {
        ...
        throw new \agpu_exception('nofieldindatabase', 'data');
    }
}
...
$mform = new mod_data_export_form(new agpu_url('/mod/data/export.php', ['d' => $data->id,
    'backto' => $redirectbackto]), $fields, $cm, $data);

if ($mform->is_cancelled()) {
    $redirectbackto = !empty($redirectbackto) ? $redirectbackto :
        new \agpu_url('/mod/data/view.php', ['d' => $data->id]);
