...
$url = new agpu_url('/mod/data/preset.php', array('d' => $data->id));
...
if ($action === 'export') {
    if (headers_sent()) {
        throw new \agpu_exception('headersent');
    }
...
if ($action == 'importzip') {
    ...
    redirect(new agpu_url('/mod/data/field.php', ['id' => $cm->id]));
    exit(0);
}
...
if ($action === 'preview') {
    ...
    if (!$manager->can_view_preset($preset)) {
        throw new \agpu_exception('cannotaccesspresentsother', manager::PLUGINNAME);
    }
...
$presets = new \mod_data\output\presets($manager, $presets, new \agpu_url('/mod/data/field.php'), true);
