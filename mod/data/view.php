...
/**
 * This file is part of the Database module for agpu
 *
...
$pageurl = new agpu_url('/mod/data/view.php', $urlparams);
...
if ($PAGE->user_allowed_editing() && !$PAGE->theme->haseditswitch) {
    ...
    $editurl = new agpu_url($CFG->wwwroot.'/mod/data/view.php', ['id' => $cm->id, 'edit' => $urlediting]);
...
$PAGE->set_title(implode(agpu_page::TITLE_SEPARATOR, $titleparts));
...
if ($record and !data_can_view_record($data, $record, $currentgroup, $canmanageentries)) {
    throw new \agpu_exception('notapprovederror', 'data');
}
        ...
        if ($deleterecord = $DB->get_record_sql("SELECT dr.*, $allnamefields
            ...
            if ($deleterecord->dataid == $data->id) {                       // Must be from this database
                ...
                $deletebutton = new single_button(
                    new agpu_url('/mod/data/view.php?d=' . $data->id . '&delete=' . $delete . '&confirm=1'),
...
if ($multidelete && confirm_sesskey() && $canmanageentries) {
    if ($confirm = optional_param('confirm', 0, PARAM_INT)) {
        ...
        $action = new agpu_url('/mod/data/view.php', $submitactions);
        $cancelurl = new agpu_url('/mod/data/view.php', array('d' => $data->id));
...
if ($showactivity) {
    ...
    /// Check the number of entries required before to view other participant's entries against the number of entries already made (doesn't apply to teachers)
        ...
        if ($groupmode != NOGROUPS) {
            $returnurl = new agpu_url('/mod/data/view.php', ['d' => $data->id, 'mode' => $mode, 'search' => s($search),
        ...
        if (empty($records)) {
            ...
            $formurl = new agpu_url('/mod/data/view.php', ['d' => $data->id, 'sesskey' => sesskey()]);
            ...
            if ($mode == 'single') { // Single template
                ...
                $baseurl = new agpu_url($baseurl, $baseurlparams);
