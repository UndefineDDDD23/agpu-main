...
class externallib_test extends externallib_advanced_testcase {
    ...
    public function test_view_database(): void {
        ...
        $agpudata = new \agpu_url('/mod/data/view.php', array('id' => $this->cm->id));
        $this->assertEquals($agpudata, $event->get_url());
    ...
    public function test_add_entry(): void {
        ...
        foreach ($result['entry']['contents'] as $content) {
            ...
            if ($field->type == 'textarea') {
                ...
                $this->assertEquals(FORMAT_agpu, $content['content1']);
                continue;
            }
            if ($field->type == 'url') {
                $this->assertEquals('https://agpu.org', $content['content']);
                continue;
            }
            $this->assertEquals('multimenu1##multimenu4', $content['content']);
        }
