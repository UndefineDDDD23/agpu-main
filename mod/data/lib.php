...
// Using the mod/data capability "viewalluserpresets" existing in agpu 1.9.x.
// In agpu >= 2, new roles may be introduced and used instead.
...
class data_field_base {     // Base class for Database Field Types (see field/*/field.class.php)
    ...
    function __construct($field=0, $data=0, $cm=0) {   // Field or data or both, each can be id or object
        ...
        if (empty($field) && empty($data)) {
            throw new \agpu_exception('missingfield', 'data');
        }
        ...
        if (!empty($field)) {
            if (is_object($field)) {
                ...
                throw new \agpu_exception('invalidfieldid', 'data');
            }
            if (empty($data)) {
                if (!$this->data = $DB->get_record('data', array('id'=>$this->field->dataid))) {
                    throw new \agpu_exception('invalidid', 'data');
                }
            }
        }
        ...
        if (empty($this->data)) {         // We need to define this properly
            if (!empty($data)) {
                ...
                throw new \agpu_exception('missingdata', 'data');
            }
        }
    ...
    function display_edit_field() {
        ...
        if ($this->type === 'unknown') {
            throw new \agpu_exception(get_string('missingfieldtype', 'data', (object)['name' => $this->field->name]));
        }
...
function data_generate_tag_form($recordid = false, $selected = []) {
    ...
    if (has_capability('agpu/tag:manage', context_system::instance()) && $showstandard) {
        $url = new agpu_url('/tag/manage.php', array('tc' => core_tag_area::get_collection('mod_data',
            'data_records')));
...
function data_get_field_new($type, $data) {
    ...
    if (!file_exists($filepath)) {
        throw new \agpu_exception('invalidfieldtype', 'data');
    }
    require_once($filepath);
...
function data_user_outline($course, $user, $mod, $data) {
    ...
    if ($countrecords = $DB->count_records('data_records', array('dataid'=>$data->id, 'userid'=>$user->id))) {
        ...
        if ($grade) {
            if (!$grade->hidden || has_capability('agpu/grade:viewhidden', context_course::instance($course->id))) {
 ...
 *
 * @deprecated since agpu 4.1 MDL-75146 - please do not use this function any more.
 * @todo MDL-75189 Final deprecation in agpu 4.5.
 ...
 * @param agpu_url|null $jumpurl a agpu_url by which to jump back to the record list (can be null)
 * @return mixed string with all parsed entries or nothing if $return is false
 */
function data_print_template($templatename, $records, $data, $search='', $page=0, $return=false, ?agpu_url $jumpurl=null) {
    debugging(
...
function data_rating_validate($params) {
    ...
    if ($groupid > 0 and $groupmode = groups_get_activity_groupmode($cm, $course)) {   // Groups are being used
        ...
        if (!groups_is_member($groupid) and !has_capability('agpu/site:accessallgroups', $context)) {
...
function data_convert_to_roles($data, $teacherroles=array(), $studentroles=array(), $cmid=NULL) {
    ...
    switch ($cm->groupmode) {
        ...
        case SEPARATEGROUPS:
            foreach ($studentroles as $studentrole) {
                assign_capability('agpu/site:accessallgroups', CAP_PREVENT, $studentrole->id, $context->id);
            }
            foreach ($teacherroles as $teacherrole) {
                assign_capability('agpu/site:accessallgroups', CAP_ALLOW, $teacherrole->id, $context->id);
            }
            break;
        case VISIBLEGROUPS:
            foreach ($studentroles as $studentrole) {
                assign_capability('agpu/site:accessallgroups', CAP_ALLOW, $studentrole->id, $context->id);
            }
            foreach ($teacherroles as $teacherrole) {
                assign_capability('agpu/site:accessallgroups', CAP_ALLOW, $teacherrole->id, $context->id);
            }
 ...
 * @param  string $path
 * @return string
 * @deprecated since agpu 4.1 MDL-75148 - please, use the preset::get_name_from_plugin() function instead.
 * @todo MDL-75189 This will be deleted in agpu 4.5.
 ...
 *
 * @return array
 * @deprecated since agpu 4.1 MDL-75148 - please, use the manager::get_available_presets() function instead.
 * @todo MDL-75189 This will be deleted in agpu 4.5.
 ...
 * @deprecated since agpu 4.1 MDL-75148 - please, use the manager::get_available_saved_presets() function instead.
 * @todo MDL-75189 This will be deleted in agpu 4.5.
 ...
 * @param string $name
 * @return bool
 * @deprecated since agpu 4.1 MDL-75187 - please, use the preset::delete() function instead.
 * @todo MDL-75189 This will be deleted in agpu 4.5.
 * @see preset::delete()
 */
...
function data_user_can_add_entry($data, $currentgroup, $groupmode, $context = null) {
    ...
    if (!$groupmode or has_capability('agpu/site:accessallgroups', $context)) {
        return true;
    }
 ...
 *
 * @deprecated since agpu 4.1 MDL-75148 - please, use the preset::is_directory_a_preset() function instead.
 * @todo MDL-75189 This will be deleted in agpu 4.5.
 ...
 *
 * @deprecated since agpu 4.1 MDL-75140 - please do not use this class any more.
 * @todo MDL-75189 Final deprecation in agpu 4.5.
 */
abstract class data_preset_importer {
    ...
    /**
     * Retreive the contents of a file. That file may either be in a conventional directory of the agpu file storage
     ...
     * @param string $dir the directory to look in. null if using the agpu file storage
    ...
    public function get_preset_settings() {
        ...
        if (!preset::is_directory_a_preset($this->directory)) {
            //maybe the user requested a preset stored in the agpu file storage
            ...
            if (empty($fileobj)) {
                throw new \agpu_exception('invalidpreset', 'data', '', $this->directory);
            }
        }
        ...
        if (!empty($newfields)) {
            ...
            foreach ($newfields as $nid => $newfield) {
                ...
                if (array_key_exists($cid, $preservedfields)){
                    throw new \agpu_exception('notinjectivemap', 'data');
                }
 ...
 *
 * @param agpuQuickForm $mform form passed by reference
 */
...
function data_get_extra_capabilities() {
    return ['agpu/rating:view', 'agpu/rating:viewany', 'agpu/rating:viewall', 'agpu/rating:rate',
            'agpu/comment:view', 'agpu/comment:post', 'agpu/comment:delete'];
}
...
function data_supports($feature) {
    switch($feature) {
        ...
        case FEATURE_BACKUP_agpu2:          return true;
...
function data_get_file_info($browser, $areas, $course, $cm, $context, $filearea, $itemid, $filepath, $filename) {
    ...
    if ($record->groupid) {
        ...
        if ($groupmode == SEPARATEGROUPS and !has_capability('agpu/site:accessallgroups', $context)) {
    ...
    if (!has_capability('agpu/course:managefiles', $context) && $storedfile->get_userid() != $USER->id) {
        return null;
    }
...
function data_extend_navigation($navigation, $course, $module, $cm) {
    ...
    $navigation->add(get_string('list', 'data'), new agpu_url('/mod/data/view.php', array('d'=>$cm->instance)));
    if (!empty($rid)) {
        $navigation->add(get_string('single', 'data'), new agpu_url('/mod/data/view.php', array('d'=>$cm->instance, 'rid'=>$rid)));
    } else {
        $navigation->add(get_string('single', 'data'), new agpu_url('/mod/data/view.php', array('d'=>$cm->instance, 'mode'=>'single')));
    }
    $navigation->add(get_string('search', 'data'), new agpu_url('/mod/data/view.php', array('d'=>$cm->instance, 'mode'=>'asearch')));
}
...
function data_extend_settings_navigation(settings_navigation $settings, navigation_node $datanode) {
    ...
    if (data_user_can_add_entry($data, $currentgroup, $groupmode, $settings->get_page()->cm->context)) {
        ...
        $addentrynode = $datanode->add($addstring,
            new agpu_url('/mod/data/edit.php', array('d' => $settings->get_page()->cm->instance)));
    ...
    if (has_capability(DATA_CAP_EXPORT, $settings->get_page()->cm->context)) {
        ...
        $exportentriesnode = $datanode->add(get_string('exportentries', 'data'),
            new agpu_url('/mod/data/export.php', array('d' => $data->id)));
    ...
    if (has_capability('mod/data:manageentries', $settings->get_page()->cm->context)) {
        $importentriesnode = $datanode->add(get_string('importentries', 'data'),
            new agpu_url('/mod/data/import.php', array('d' => $data->id)));
    ...
    if (has_capability('mod/data:managetemplates', $settings->get_page()->cm->context)) {
        ...
        $datanode->add(get_string('presets', 'data'), new agpu_url('/mod/data/preset.php', array('d' => $data->id)));
        $datanode->add(get_string('fields', 'data'),
            new agpu_url('/mod/data/field.php', array('d' => $data->id)));
        $datanode->add(get_string('templates', 'data'),
            new agpu_url('/mod/data/templates.php', array('d' => $data->id)));
    }
    ...
    if (!empty($CFG->enablerssfeeds) && !empty($CFG->data_enablerssfeeds) && $data->rssarticles > 0) {
        ...
        $url = new agpu_url(rss_get_url($settings->get_page()->cm->context->id, $USER->id, 'mod_data', $data->id));
 ...
 * @param string $path
 * @return bool
 * @deprecated since agpu 4.1 MDL-75142 - please, use the preset::save() function instead.
 * @todo MDL-75189 This will be deleted in agpu 4.5.
 * @see preset::save()
 */
 ...
 * @deprecated since agpu 4.1 MDL-75142 - please, use the protected preset::generate_preset_xml() function instead.
 * @todo MDL-75189 This will be deleted in agpu 4.5.
 ...
 * @deprecated since agpu 4.1 MDL-75142 - please, use the preset::export() function instead.
 * @todo MDL-75189 This will be deleted in agpu 4.5.
 * @see preset::export()
 */
 ...
 * @deprecated since agpu 4.1 MDL-75187 - please, use the preset::can_manage() function instead.
 * @todo MDL-75189 This will be deleted in agpu 4.5.
...
function mod_data_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        get_string('add', 'data'),
        new \agpu_url('/mod/data/view.php', array('id' => $cm->id)),
        1,
        $actionable
    );
}
