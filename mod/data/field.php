...
$url = new agpu_url('/mod/data/field.php');
if ($fid !== 0) {
    $url->param('fid', $fid);
}
if ($newtype !== '') {
...
switch ($mode) {
    ...
    case 'usepreset':
        ...
        if (!$importer->needs_mapping() || $action == 'notmapping') {
            $backurl = new agpu_url('/mod/data/field.php', ['id' => $cm->id]);
...
if (($mode == 'new') && (!empty($newtype))) { // Adding a new field.
    ...
    foreach ($fieldrecords as $fieldrecord) {
        ...
        $baseurl = new agpu_url('/mod/data/field.php', array(
            'd'         => $data->id,
            'fid'       => $field->field->id,
            'sesskey'   => sesskey(),
        ));

        $displayurl = new agpu_url($baseurl, array(
            'mode'      => 'display',
        ));

        $deleteurl = new agpu_url($baseurl, array(
            'mode'      => 'delete',
        ));
