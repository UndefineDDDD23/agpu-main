...
class data_field_picture extends data_field_base {
    ...
    function display_browse_field($recordid, $template) {
        ...
        if ($template == 'listtemplate') {
            ...
            $width = '';
            $height = '';
            $url = new agpu_url('/mod/data/view.php', ['d' => $this->field->dataid, 'rid' => $recordid]);
        } else {
            $filename = $content->content;
            $url = null;
        }
        $imgurl = agpu_url::make_pluginfile_url($this->context->id, 'mod_data', 'content', $content->id, '/', $filename);
            ...
            case 'file':
                ...
                if (count($files) == 0) {
                    ...
                    if ($file->get_imageinfo() === false) {
                        $url = new agpu_url('/mod/data/edit.php', array('d' => $this->field->dataid));
