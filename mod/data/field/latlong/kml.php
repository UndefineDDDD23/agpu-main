...
$url = new agpu_url('/mod/data/field/latlong/kml.php', array('d'=>$d, 'fieldid'=>$fieldid));
if ($rid !== 0) {
    $url->param('rid', $rid);
}
$PAGE->set_url($url);
...
if ($rid) {
    if (! $record = $DB->get_record('data_records', array('id'=>$rid))) {
        throw new \agpu_exception('invalidrecord', 'data');
    }
    ...
    if (! $content = $DB->get_record('data_content', array('fieldid'=>$fieldid, 'recordid'=>$rid))) {
        throw new \agpu_exception('nofieldcontent', 'data');
    }
} else {   // We must have $d
...
header('Content-Disposition: attachment; filename="agpuearth-'.$d.'-'.$rid.'-'.$fieldid.'.kml"');
...
if($rid) { // List one single item
    ...
    $pm->description = "&lt;a href='$CFG->wwwroot/mod/data/view.php?d=$d&amp;rid=$rid'&gt;Item #$rid&lt;/a&gt; in agpu data activity";
    ...
    foreach($contents as $content) {
        $pm->name = data_latlong_kml_get_item_name($content, $field);
        $pm->description = "&lt;a href='$CFG->wwwroot/mod/data/view.php?d=$d&amp;rid=$content->recordid'&gt;Item #$content->recordid&lt;/a&gt; in agpu data activity";
        $pm->long = $content->content1;
        $pm->lat = $content->content;
        echo data_latlong_kml_placemark($pm);
    }
