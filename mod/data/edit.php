...
$url = new agpu_url('/mod/data/edit.php');
...
if (!has_capability('mod/data:manageentries', $context)) {
    if ($rid) {
        ...
        if (!data_user_can_manage_entry($record, $data, $context)) {
            throw new \agpu_exception('noaccess', 'data');
        }
    ...
    if ($processeddata->validated) {
        ...
        if ($recordid) {
            ...
            if (!empty($datarecord->saveandadd)) {
                ...
                $viewurl = new agpu_url('/mod/data/view.php', [
                    'd' => $data->id,
                    'rid' => $recordid,
                ]);
                redirect($viewurl);
...
if (empty($redirectbackto)) {
    $redirectbackto = new \agpu_url('/mod/data/view.php', ['id' => $cm->id]);
}
