...
class actionbar implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output): array {
        ...
        $basicreportlink = new agpu_url('/mod/scorm/report.php', ['id' => $this->id, 'mode' => 'basic']);
        $graphreportlink = new agpu_url('/mod/scorm/report.php', ['id' => $this->id, 'mode' => 'graphs']);
        $interactionreportlink = new agpu_url('/mod/scorm/report.php', ['id' => $this->id, 'mode' => 'interactions']);
        $objectivesreportlink = new agpu_url('/mod/scorm/report.php', ['id' => $this->id, 'mode' => 'objectives']);
        ...
        if ($this->download) {
            ...
            $downloadodslink = new agpu_url($PAGE->url, $options);

            $options['download'] = 'Excel';
            $downloadexcellink = new agpu_url($PAGE->url, $options);

            $options['download'] = 'CSV';
            $downloadtextlink = new agpu_url($PAGE->url, $options);
        }

        $url = new agpu_url('/mod/scorm/report.php', $PAGE->url->remove_params('attemptsmode'));
