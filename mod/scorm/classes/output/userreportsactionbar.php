...
class userreportsactionbar implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output): array {
        $data = [
            'backurl' => (new agpu_url('/mod/scorm/report.php', ['id' => $this->id, 'mode' => $this->mode]))->out(false)
        ];

        if (!$this->scoid) {
            $learnobjects = new agpu_url('/mod/scorm/report/userreport.php',
                    ['id' => $this->id, 'user' => $this->userid, 'attempt' => $this->attempt, 'mode' => $this->mode]);
            $interactions = new agpu_url('/mod/scorm/report/userreportinteractions.php',
                    ['id' => $this->id, 'user' => $this->userid, 'attempt' => $this->attempt, 'mode' => $this->mode]);
