...
$url = new agpu_url('/mod/survey/report.php', array('id' => $id));
if ($action !== '') {
...
if (!$survey = $DB->get_record("survey", array("id" => $cm->instance))) {
    throw new agpu_exception('invalidsurveyid', 'survey');
}

if (!$template = $DB->get_record("survey", array("id" => $survey->template))) {
    throw new agpu_exception('invalidtmptid', 'survey');
}
...
navigation_node::override_active_url(new agpu_url('/mod/survey/report.php', ['id' => $id, 'action' => 'summary']));
...
if ($groupmode != NOGROUPS) {
    ...
    if ($currentgroup === 0 && $groupmode == SEPARATEGROUPS && !has_capability('agpu/site:accessallgroups', $context)) {
        throw new agpu_exception('notingroup');
    }

    $groupsactivitymenu = groups_print_activity_menu($cm, new agpu_url('/mod/survey/report.php',
...
switch ($action) {
    ...
    case "question":
        if (!$question = $DB->get_record("survey_questions", array("id" => $qid))) {
            throw new \agpu_exception('cannotfindquestion', 'survey');
        }
        $question->text = get_string($question->text, "survey");
    ...
    case "student":
        ...
        if ($currentgroup && !array_key_exists($user->id, $users)) {
            throw new agpu_exception('usernotavailable', 'error');
        }
    ...
    case "download":
        ...
        if ($numusers > 0) {
            ...
            $options["type"] = "ods";
            echo $OUTPUT->single_button(new agpu_url("download.php", $options), get_string("downloadods"));

            $options["type"] = "xls";
            echo $OUTPUT->single_button(new agpu_url("download.php", $options), get_string("downloadexcel"));

            $options["type"] = "txt";
            echo $OUTPUT->single_button(new agpu_url("download.php", $options), get_string("downloadtext"));
