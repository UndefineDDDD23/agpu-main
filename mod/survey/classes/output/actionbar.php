...
class actionbar implements renderable, templatable {
     ...
     */
    public function __construct(int $id, string $action, agpu_url $currenturl) {
        $this->id = $id;
        $this->action = $action;
    ...
    private function create_select_menu(): url_select {
        ...
        foreach ($actions as $action => $straction) {
            $url = new agpu_url($this->currenturl, ['id' => $this->id, 'action' => $action]);
    ...
    public function export_for_template(renderer_base $output): array {
        ...
        if (has_capability('mod/survey:download', $PAGE->cm->context)) {
            $downloadlink = (new agpu_url('/mod/survey/report.php', ['id' => $this->id, 'action' => 'download']))->out(false);
            $data['download'] = [
                'link' => $downloadlink,
