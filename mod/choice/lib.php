...
function choice_user_submit_response($formanswer, $choice, $userid, $course, $cm) {
    ...
    $continueurl = new agpu_url('/mod/choice/view.php', array('id' => $cm->id));

    if (empty($formanswer)) {
        throw new \agpu_exception('atleastoneoption', 'choice', $continueurl);
    }

    if (is_array($formanswer)) {
        if (!$choice->allowmultiple) {
            throw new \agpu_exception('multiplenotallowederror', 'choice', $continueurl);
        }
    ...
    foreach ($formanswers as $key => $val) {
        if (!isset($options[$val])) {
            throw new \agpu_exception('cannotsubmit', 'choice', $continueurl);
        }
    }
    ...
    if (!($choice->limitanswers && $choicesexceeded)) {
        ...
        $choicelock->release();
        throw new \agpu_exception('choicefull', 'choice', $continueurl);
    }
...
function prepare_choice_show_results($choice, $course, $cm, $allresponses) {
    ...
    $display->fullnamecapability = has_capability('agpu/site:viewfullnames', $context);
...
function choice_extend_settings_navigation(settings_navigation $settings, navigation_node $choicenode) {
    if (has_capability('mod/choice:readresponses', $settings->get_page()->cm->context)) {
        $choicenode->add(get_string('responses', 'choice'),
            new agpu_url('/mod/choice/report.php', array('id' => $settings->get_page()->cm->id)));
    }
}
...
function mod_choice_core_calendar_provide_event_action(calendar_event $event,
    ...
    return $factory->create_instance(
        ...
        new \agpu_url('/mod/choice/view.php', array('id' => $cm->id)),
        1,
        $actionable
    );
}
