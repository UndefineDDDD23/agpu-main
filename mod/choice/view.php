...
$url = new agpu_url('/mod/choice/view.php', array('id'=>$id));
if ($action !== '') {
...
if (!$choiceformshown) {
    ...
    if (isguestuser()) {
        ...
        echo $OUTPUT->confirm(get_string('noguestchoose', 'choice').'<br /><br />'.get_string('liketologin'),
                     get_login_url(), new agpu_url('/course/view.php', array('id'=>$course->id)));
        ...
        echo $OUTPUT->single_button(new agpu_url('/enrol/index.php?', array('id'=>$course->id)), get_string('enrolme', 'core_enrol', $courseshortname));
...
if (choice_can_view_results($choice, $current, $choiceopen)) {
    ...
    if ($groupmode) { // If group mode is enabled, display the groups selector.
        ...
        $groupsactivitymenu = groups_print_activity_menu($cm, new agpu_url('/mod/choice/view.php', ['id' => $id]),
            true);
