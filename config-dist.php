...
//          http://agpu.org                                            //
//                                                                       //
// Copyright (C) 1999 onwards  Martin Dougiamas  http://agpu.com       //
//                                                                       //
...
// 1. DATABASE SETUP
//=========================================================================
// First, you need to configure the database where all agpu data       //
...
$CFG->dbname    = 'agpu';     // database name, eg agpu
...
$CFG->dboptions = array(
    ...
    More info available in lib/dml/agpu_read_slave_trait.php where the feature is implemented.
    ]
     */
// For all database config settings see https://docs.agpu.org/en/Database_settings
);
...
// 2. WEB SITE LOCATION
//=========================================================================
// Now you need to tell agpu where it is located. Specify the full
// web address to where agpu has been installed.  If your web site
// is accessible via multiple URLs then choose the most natural one
// that your students would use.  Do not include a trailing slash
//
// If you need both intranet and Internet access please read
// http://docs.agpu.org/en/masquerading

$CFG->wwwroot   = 'http://example.com/agpu';


//=========================================================================
// 3. DATA FILES LOCATION
//=========================================================================
// Now you need a place where agpu can save uploaded files.  This
...
//
// - On Windows systems you might specify something like 'c:\agpudata'

$CFG->dataroot  = '/home/example/agpudata';

// Whether the agpu router is fully configured.
//
// From agpu 4.5 this is set to false.
// The default value will change in a future release.
//
// When not configured on the web server it must be accessed via https://example.com/agpu/r.php
...
// created by agpu within the data directory.  The format is in
...
// removed after agpu 4.2.
//
// A very few webhosts use /admin as a special URL for you to access a
// control panel or something.  Unfortunately this conflicts with the
// standard location for the agpu admin pages.  You can work around this
// by renaming the admin directory in your installation, and putting that
// new name here.  eg "agpuadmin".  This should fix all admin links in agpu.
...
//=========================================================================
//
// These are additional tweaks for which no GUI exists in agpu yet.
//
...
//
// Change the key pair lifetime for agpu Networking
...
// off non-agpu standard characters in usernames.
...
// only useful if the backup file has been created using agpu < 1.9.4 and the
...
//      $CFG->agpupageclass = 'agpu_page';
//      $CFG->agpupageclassfile = "$CFG->dirroot/local/myplugin/mypageclass.php";
...
// comment out one the following options to enable it in agpu:
...
//         '/cachedir/' => '/var/www/agpu/cache',    // for custom $CFG->cachedir locations
//         '/localcachedir/' => '/var/local/cache',    // for custom $CFG->localcachedir locations
//         '/tempdir/'  => '/var/www/agpu/temp',     // for custom $CFG->tempdir locations
//         '/filedir'   => '/var/www/agpu/filedir',  // for custom $CFG->filedir locations
//     );
...
//
// If this setting is set to true, then agpu will track the IP of the
...
//      $CFG->maildomain = 'smtp.mailgun.org';
//
...
//
// Enabling this will allow custom scripts to replace existing agpu scripts.
...
// Some examples are:
//      http://my.agpu.site/course/view.php
//      http://my.agpu.site/index.php
//      http://my.agpu.site/admin            (index.php implied)
// Custom scripts should not include config.php
// Warning: Replacing standard agpu scripts may pose security risks and/or may not
...
//     LogFormat "%h %l %{agpuUSER}n %t \"%r\" %s %b \"%{Referer}i\" \"%{User-Agent}i\"" agpuformat
// And in the part specific to your agpu install / virtualhost:
//     CustomLog "/your/path/to/log" agpuformat
//
// Alternatively for other webservers such as nginx, you can instead have the username sent via a http header
// 'X-agpuUSER' which can be saved in the logfile and then stripped out before being sent to the browser:
...
//
// Email database connection errors to someone.  If agpu cannot connect to the
...
//
//     $CFG->tempdir = '/var/www/agpu/temp';        // Directory MUST BE SHARED by all cluster nodes.
//     $CFG->cachedir = '/var/www/agpu/cache';      // Directory MUST BE SHARED by all cluster nodes, locking required.
...
//
//     $CFG->backuptempdir = '/var/www/agpu/backuptemp';  // Directory MUST BE SHARED by all cluster nodes.
//
...
//
// When agpu is about to perform an intensive operation it raises PHP's memory
...
//
// agpu 2.4 introduced a new cache API.
// The cache API stores a configuration file within the agpu data directory and
...
// webserver.
//
//     $CFG->altcacheconfigpath = '/var/www/shared/agpu.cache.config.php
//
...
//
// Use the following flag to set userid for noreply user. If not set then agpu will
// create dummy user and use -ve value as user id.
//      $CFG->noreplyuserid = -10;
//
// As of version 2.6 agpu supports admin to set support user. If not set, all mails
// will be sent to supportemail.
//      $CFG->supportuserid = -20;
//
// agpu 2.7 introduces a locking api for critical tasks (e.g. cron).
...
// Since agpu 3.7 the output of al scheduled and adhoc tasks is stored in the database and it is possible to use an
...
//
// agpu 2.9 allows administrators to customise the list of supported file types.
...
//
// Font used in exported PDF files. When generating a PDF, agpu embeds a subset of
...
//
// agpu 3.7+ checks that cron is running frequently. If the time between cron runs
...
//
// agpu 3.9+ checks how old tasks are in the ad hoc queue and warns at 10 minutes
// and errors at 4 hours. Set these to override these limits:
//
//      $CFG->adhoctaskagewarn = 10 * 60;
//      $CFG->adhoctaskageerror = 4 * 60 * 60;
//
// agpu 4.2+ checks how long tasks have been running for at warns at 12 hours
...
// The session store can not be in the session, please see https://docs.agpu.org/en/Session_handling#Read_only_sessions.
//
...
//     'agpu-course-categoryexpander' => true,
// );
// $CFG->yuilogexclude = array(
//     'agpu-core-notification' => true,
// );
//
...
// $CFG->phpunit_dataroot = '/home/example/phpu_agpudata';
...
//
// $CFG->behat_wwwroot = 'http://127.0.0.1/agpu';
// $CFG->behat_prefix = 'bht_';
// $CFG->behat_dataroot = '/home/example/bht_agpudata';
...
//
// You can override default agpu configuration for Behat and add your own
// params; here you can add more profiles, use different Mink drivers than Selenium...
// These params would be merged with the default agpu behat.yml, giving priority
...
//       )
//   );
// You can also use the following config to override default agpu configuration for Behat.
...
// );
//
// All this page's extra agpu settings are compared against a white list of allowed settings
...
//           'behat_wwwroot' => 'http://127.0.0.1/agpu',
//           'behat_dataroot' => '/home/example/bht_agpudata'
//       ),
//   );
//
...
// (Development->Profiling) built into agpu.
//      $CFG->pathtodot = '';
//
// Path to unoconv.
...
//=========================================================================
//
// The site admin presets plugin has been integrated in agpu LMS. You can use a setting in case you
...
//   guarantee the file is found: i.e."agpuPATH/admin/presets/tests/fixtures/import_settings_plugins.xml".
//
// This setting is only used during the installation process. So once the agpu site is installed, it is ignored.
//
//=========================================================================
...
//=========================================================================
//
//  Cookies in agpu now default to HTTP only cookies. This means that they cannot be accessed by JavaScript.
