...
$participantsperpage = intval(get_config('agpucourse', 'participantsperpage'));
...
if ($participanttable->get_page_size() < $participanttable->totalrows) {
    ...
    $label = get_string('selectalluserswithcount', 'agpu', $participanttable->totalrows);
...
$displaylist = array();
if (!empty($CFG->messaging) && has_all_capabilities(['agpu/site:sendmessage', 'agpu/course:bulkmessaging'], $context)) {
    $displaylist['#messageselect'] = get_string('messageselectadd');
}
if (!empty($CFG->enablenotes) && has_capability('agpu/notes:manage', $context) && $context->id != $frontpagectx->id) {
...
foreach ($formats as $format) {
    if ($format->is_enabled()) {
        ...
        $url = new agpu_url('bulkchange.php', $params);
