...
class behat_user extends behat_base {
    ...
    public function the_user_is_deleted($identifier) {
        ...
        if (!$userid) {
            throw new agpu_exception('The specified user with username or email "' . $identifier . '" does not exist');
        }
    ...
    protected function resolve_page_url(string $page): agpu_url {
        ...
        switch (strtolower($page)) {
            case 'contact site support':
                return new agpu_url('/user/contactsitesupport.php');
    ...
    protected function resolve_page_instance_url(string $type, string $identifier): agpu_url {
        ...
        switch (strtolower($type)) {
            case 'editing':
                ...
                return new agpu_url('/user/editadvanced.php', ['id' => $userid]);
            case 'profile':
                $userid = $this->get_user_id_by_identifier($identifier);
                if (!$userid) {
                    throw new Exception('The specified user with username or email "' . $identifier . '" does not exist');
                }
                return new agpu_url('/user/profile.php', ['id' => $userid]);
            default:
