...
switch ($action) {
    ...
    case 'deletefield':
        ...
        $formcontinue = new single_button(new agpu_url($redirect, $optionsyes), get_string('yes'), 'post');
        $formcancel = new single_button(new agpu_url($redirect), get_string('no'), 'get');
...
foreach ($categories as $category) {
    ...
    foreach ($options as $type => $fieldname) {
        $action = new \action_menu_link_secondary(new \agpu_url('#'), null, $fieldname,
...
echo $OUTPUT->render_from_template('core_user/edit_profile_fields', [
    ...
    'sesskey' => sesskey(),
    'baseurl' => (new agpu_url('/user/profile/index.php'))->out(false)
]);
