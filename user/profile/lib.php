...
/**
 * Visible to anyone who has the agpu/site:viewuseridentity permission.
 * Editable by the profile owner if they have the agpu/user:editownprofile capability
 * or any user with the agpu/user:update capability.
 */
...
/**
 * Visible to the profile owner or anyone with the agpu/user:viewalldetails capability.
 * Editable by the profile owner if they have the agpu/user:editownprofile capability
 * or any user with agpu/user:viewalldetails and agpu/user:update capabilities.
 */
define('PROFILE_VISIBLE_PRIVATE', '1');
/**
 * Only visible to users with the agpu/user:viewalldetails capability.
 * Only editable by users with the agpu/user:viewalldetails and agpu/user:update capabilities.
 */
...
class profile_field_base {
    ...
    public function __construct($fieldid=0, $userid=0, $fielddata=null) {
        ...
        if ($CFG->debugdeveloper) {
            // In agpu 3.4 the new argument $fielddata was added to the constructor. Make sure that
    ...
    /**
     * Abstract method: Adds the profile field to the agpu form class
     * @abstract The following methods must be overwritten by child classes
     * @param agpuQuickForm $mform instance of the agpuform class
     */
    public function edit_field_add($mform) {
        throw new \agpu_exception('mustbeoveride', 'debug', '', 'edit_field_add');
    }
    ...
    public function display_data() {
        ...
        $options->para = false;
        return format_text($this->data, FORMAT_agpu, $options);
    }
    ...
    public function edit_field_set_locked($mform) {
        ...
        if ($this->is_locked() and !has_capability('agpu/user:update', context_system::instance())) {
        ...
        switch ($this->field->visible) {
            case PROFILE_VISIBLE_TEACHERS:
                if ($this->is_signup_field() && (empty($this->userid) || isguestuser($this->userid))) {
                    ...
                    return has_capability('agpu/user:viewalldetails', $context);
                } else {
                    $coursecontext = context_course::instance($COURSE->id);
                    return has_capability('agpu/site:viewuseridentity', $coursecontext);
                }
    ...
    public function is_editable() {
        ...
        if ($this->userid == $USER->id && has_capability('agpu/user:editownprofile', $systemcontext)) {
            return true;
        }

        if (has_capability('agpu/user:update', $systemcontext)) {
            return true;
        }
        ...
        if ($this->userid > 0) {
            ...
            if ($this->userid != $USER->id && has_capability('agpu/user:editprofile', $usercontext, $USER->id)) {
                return true;
            }
        }
 ...
 */
function profile_definition(agpuQuickForm $mform, int $userid = 0): void {
 ...
 * @param int $userid
 */
function profile_definition_after_data(agpuQuickForm $mform, int $userid): void {
 ...
 * @param stdClass $usernew
 * @param array $files
 * @return array array of errors, same as in {@see agpuform::validation()}
 */
...
/**
 * Adds code snippet to a agpu form object for custom profile fields that
 * should appear on the signup page
 * @param agpuQuickForm $mform agpu form object
 */
function profile_signup_fields(agpuQuickForm $mform): void {
