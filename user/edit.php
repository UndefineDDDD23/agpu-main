...
// Guest can not edit.
if (isguestuser()) {
    throw new \agpu_exception('guestnoeditprofile');
}
...
if (is_mnet_remote_user($user)) {
    if (user_not_fully_set_up($user, true)) {
        ...
        throw new \agpu_exception('usernotfullysetup', 'mnet', '', $hostwwwroot);
    }
...
if ($user->id == $USER->id) {
    ...
    if (!has_capability('agpu/user:editownprofile', $systemcontext)) {
        throw new \agpu_exception('cannotedityourprofile');
    }
    ...
    if (isguestuser($user->id)) {
        throw new \agpu_exception('guestnoeditprofileother');
    }
...
// Create form.
$userform = new user_edit_form(new agpu_url($PAGE->url, array('returnto' => $returnto)), array(
...
if ($returnto === 'profile') {
    if ($course->id != SITEID) {
        $returnurl = new agpu_url('/user/view.php', array('id' => $user->id, 'course' => $course->id));
    } else {
        $returnurl = new agpu_url('/user/profile.php', array('id' => $user->id));
    }
} else {
    $returnurl = new agpu_url('/user/preferences.php', array('userid' => $user->id));
}
...
if ($userform->is_cancelled()) {
    ...
    if ($CFG->emailchangeconfirmation) {
        // Users with 'agpu/user:update' can change their email address immediately.
        // Other users require a confirmation email.
        if (isset($usernew->email) and $user->email != $usernew->email && !has_capability('agpu/user:update', $systemcontext)) {
            $a = new stdClass();
    ...
    if (!$authplugin->user_update($user, $usernew)) {
        // Auth update failed.
        throw new \agpu_exception('cannotupdateprofile');
    }
