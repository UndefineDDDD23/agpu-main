...
M.core_user.init_user_selector = function(Y, name, hash, extrafields, lastsearch, searchtype) {
    ...
    var user_selector = {
        ...
        send_query : function(forceresearch) {
            ...
            this.listbox.setStyle('background', 'url(' + M.util.image_url('i/loading', 'agpu') + ') no-repeat center center');
        ...
        output_options : function(data) {
            ...
            if (!count) {
                var searchstr = (this.lastsearch != '') ? this.insert_search_into_str(M.util.get_string('nomatchingusers', 'agpu'), this.lastsearch) : M.util.get_string('none', 'agpu');
            ...
            if (this.get_option('preserveselected') && selectedusers) {
                this.output_group(this.insert_search_into_str(M.util.get_string('previouslyselectedusers', 'agpu'), this.lastsearch), selectedusers, true, false);
            }
