...
if (isguestuser()) {     // Guests can never edit their profile.
    ...
    if ($currentuser) {
        $PAGE->set_blocks_editing_capability('agpu/user:manageownblocks');
    } else {
        $PAGE->set_blocks_editing_capability('agpu/user:manageblocks');
    }
}
...
if ($PAGE->user_allowed_editing()) {
    if ($reset !== null) {
        if (!is_null($userid)) {
            ...
            redirect(new agpu_url('/user/profile.php', array('id' => $userid)));
        }
    ...
    $reseturl = new agpu_url("$CFG->wwwroot/user/profile.php", array('edit' => 1, 'reset' => 1, 'id' => $userid));
    ...
    $url = new agpu_url("$CFG->wwwroot/user/profile.php", $params);
    $button = '';
...
$hiddenfields = [];
if (!has_capability('agpu/user:viewhiddendetails', $usercontext)) {
