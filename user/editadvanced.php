...
if ($id == -1) {
    ...
    $user->deleted = 0;
    $user->timezone = '99';
    require_capability('agpu/user:create', $systemcontext);
    admin_externalpage_setup('addnewuser', '', array('id' => -1));
    $PAGE->set_primary_active_tab('siteadminnode');
    $PAGE->navbar->add(get_string('addnewuser', 'agpu'), $PAGE->url);
} else {
    // Editing existing user.
    require_capability('agpu/user:update', $systemcontext);
...
// Create form.
$userform = new user_editadvanced_form(new agpu_url($PAGE->url, array('returnto' => $returnto)), array(
...
if ($userform->is_cancelled()) {
    ...
    if ($usernew->id == -1) {
        ...
        if (!$authplugin->user_update($user, $usernew)) {
            // Auth update failed.
            throw new \agpu_exception('cannotupdateuseronexauth', '', '', $user->auth);
        }
        ...
        if (!empty($usernew->newpassword)) {
            if ($authplugin->can_change_password()) {
                if (!$authplugin->user_update_password($usernew, $usernew->newpassword)) {
                    throw new \agpu_exception('cannotupdatepasswordonextauth', '', '', $usernew->auth);
                }
...
if ($user->id == -1 or ($user->id != $USER->id)) {
    if ($user->id == -1) {
        ...
        $PAGE->set_title("$streditmyprofile: $userfullname" . agpu_page::TITLE_SEPARATOR . $coursename);
        echo $OUTPUT->header();
