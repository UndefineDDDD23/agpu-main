...
function useredit_shared_definition(&$mform, $editoroptions, $filemanageroptions, $user) {
    ...
    if (get_config('tool_agpunet', 'enableagpunet')) {
        $mform->addElement('text', 'agpunetprofile', get_string('agpunetprofile', 'user'), 'maxlength="255" size="30"');
        $mform->setType('agpunetprofile', PARAM_NOTAGS);
        $mform->addHelpButton('agpunetprofile', 'agpunetprofile', 'user');
    }
    ...
    if (empty($USER->newadminuser)) {
        $mform->addElement('header', 'agpu_picture', get_string('pictureofuser'));
        $mform->setExpanded('agpu_picture', true);
    ...
    if (count($disabledusernamefields) > 0) {
        $mform->addElement('header', 'agpu_additional_names', get_string('additionalnames'));
    ...
    if (core_tag_tag::is_enabled('core', 'user') and empty($USER->newadminuser)) {
        $mform->addElement('header', 'agpu_interests', get_string('interests'));
        $mform->addElement('tags', 'interests', get_string('interestslist'),
            array('itemtype' => 'user', 'component' => 'core'));
        $mform->addHelpButton('interests', 'interestslist');
    }

    // agpu optional fields.
    $mform->addElement('header', 'agpu_optional', get_string('optional', 'form'));
