...
function user_create_user($user, $updatepassword = true, $triggerevent = true) {
    ...
    if (trim($user->username) === '') {
        throw new agpu_exception('invalidusernameblank');
    }

    if ($user->username !== core_text::strtolower($user->username)) {
        throw new agpu_exception('usernamelowercase');
    }

    if ($user->username !== core_user::clean_field($user->username, 'username')) {
        throw new agpu_exception('invalidusername');
    }
    ...
    if ($updatepassword && isset($user->password)) {
        ...
        if (!check_password_policy($user->password, $errmsg, $user)) {
            throw new agpu_exception($errmsg);
        }
 ...
 *          it can be changed with $CFG->agpuwstextformatlinkstoimagesfile
 *
 * @throws agpu_exception
 * @param stdClass $user user record from mdl_user
 * @param stdClass $course agpu course
...
function user_get_user_details($user, $course = null, array $userfields = array()) {
    ...
    foreach ($userfields as $thefield) {
        if (!in_array($thefield, $defaultfields)) {
            throw new agpu_exception('invaliduserfield', 'error', '', $thefield);
        }
    }
    ...
    if (!empty($course)) {
        ...
        $canviewdetailscap = (has_capability('agpu/user:viewdetails', $context) || has_capability('agpu/user:viewdetails', $usercontext));
    } else {
        $context = context_user::instance($user->id);
        $usercontext = $context;
        $canviewdetailscap = has_capability('agpu/user:viewdetails', $usercontext);
    }
    ...
    if (!empty($course)) {
        $canviewhiddenuserfields = has_capability('agpu/course:viewhiddenuserfields', $context);
    } else {
        $canviewhiddenuserfields = has_capability('agpu/user:viewhiddendetails', $context);
    }
    $canviewfullnames = has_capability('agpu/site:viewfullnames', $context);
    if (!empty($course)) {
        $canviewuseremail = has_capability('agpu/course:useremail', $context);
    } else {
    ...
    if (!empty($course)) {
        $canaccessallgroups = has_capability('agpu/site:accessallgroups', $context);
    } else {
    ...
    if (in_array('username', $userfields)) {
        if ($currentuser or has_capability('agpu/user:viewalldetails', $context)) {
    ...
    if (in_array('idnumber', $userfields) && $user->idnumber) {
        if (in_array('idnumber', $showuseridentityfields) or $currentuser or
                has_capability('agpu/user:viewalldetails', $context)) {
...
function can_view_user_details_cap($user, $course = null) {
    ...
    $usercontext = context_user::instance($user->id);
    $result = has_capability('agpu/user:viewdetails', $usercontext);
    // Otherwise can $USER see them at course context.
    if (!$result && !empty($course)) {
        $context = context_course::instance($course->id);
        $result = has_capability('agpu/user:viewdetails', $context);
    }
    return $result;
}
 ...
 * @param agpu_page $page the current page
 * @return array
 */
function user_convert_text_to_menu_items($text, $page) {
    ...
    foreach ($lines as $line) {
        ...
        if (!array_key_exists(1, $bits) or empty($bits[1])) {
            ...
            // Make sure the url is a agpu url.
            $bits[1] = new agpu_url(trim($bits[1]));
        }
        $child->url = $bits[1];
 ...
 * @param agpu_page $page page object.
 ...
 *          userprofileurl agpu_url the url of the user's profile
 ...
 *          realuserprofileurl agpu_url the url of the user's profile
...
function user_get_user_navigation_info($user, $page, $options = array()) {
    ...
    $returnobject->metadata['userprofileurl'] = new agpu_url('/user/profile.php', array(
        'id' => $user->id
    ));
    ...
    $preferences->url = new agpu_url('/user/preferences.php');
    $preferences->title = get_string('preferences');
    $preferences->titleidentifier = 'preferences,agpu';
    ...
    if (is_role_switched($course->id)) {
        if ($role = $DB->get_record('role', array('id' => $user->access['rsw'][$context->path]))) {
            ...
            $rolereturn->url = new agpu_url('/course/switchrole.php', array(
                'id' => $course->id,
                'sesskey' => sesskey(),
                'switchrole' => 0,
            ...
            $rolereturn->titleidentifier = 'switchrolereturn,agpu';
        ...
        if (is_array($roles) && (count($roles) > 0)) {
            ...
            $switchrole->url = new agpu_url('/course/switchrole.php', array(
                'id' => $course->id,
                'switchrole' => -1,
                'returnurl' => $page->url->out_as_local_url(false)
            ));
            $switchrole->title = get_string('switchroleto');
            $switchrole->titleidentifier = 'switchroleto,agpu';
    ...
    if ($returnobject->metadata['asotheruser'] = \core\session\manager::is_loggedinas()) {
        ...
        $returnobject->metadata['realuserprofileurl'] = new agpu_url('/user/profile.php', [
            'id' => $realuser->id
        ]);
        ...
        $userrevert->url = new agpu_url('/course/loginas.php', [
            'id' => $course->id,
            'sesskey' => sesskey()
        ]);
        $userrevert->title = get_string('logout');
        $userrevert->titleidentifier = 'logout,agpu';
        $returnobject->navitems[] = $userrevert;
    } else {
        // Build a logout link.
        $logout = new stdClass();
        $logout->itemtype = 'link';
        $logout->url = new agpu_url('/login/logout.php', ['sesskey' => sesskey()]);
        $logout->title = get_string('logout');
        $logout->titleidentifier = 'logout,agpu';
...
/**
 * Remove a user device from the agpu database (for PUSH notifications usually).
 *
...
function user_mygrades_url($userid = null, $courseid = SITEID) {
    ...
    if (isset($CFG->grade_mygrades_report) && $CFG->grade_mygrades_report != 'external') {
        if (isset($userid) && $USER->id != $userid) {
            ...
            $url = new agpu_url('/grade/report/' . $CFG->grade_mygrades_report . '/index.php',
...
function user_can_view_profile($user, $course = null, $usercontext = null) {
    ...
    if (isset($course)) {
        ...
        if (has_capability('agpu/user:viewdetails', $usercontext) || has_capability('agpu/user:viewalldetails', $usercontext)) {
            return true;
        }
    ...
    foreach ($userscourses as $userscourse) {
        ...
        if (has_capability('agpu/user:viewdetails', $coursecontext) ||
            has_capability('agpu/user:viewalldetails', $coursecontext)) {
 ...
 *
 * @param string $uuid The device UUID.
 * @param string $appid The app id, usually something like com.agpu.agpumobile.
