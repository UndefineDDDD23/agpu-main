...
$url = new agpu_url('/user/portfolio.php', array('courseid' => $courseid));
...
if (!empty($config)) {
    navigation_node::override_active_url(new agpu_url('/user/portfolio.php', array('courseid' => $courseid)));
    ...
    if ($mform->is_cancelled()) {
        ...
        if (!confirm_sesskey()) {
            throw new \agpu_exception('confirmsesskeybad', '', $baseurl);
        }
...
if ($display) {
    ...
    if (!$instances = portfolio_instances(true, false)) {
        throw new \agpu_exception('noinstances', 'portfolio', $CFG->wwwroot . '/user/view.php');
    }
    ...
    foreach ($instances as $i) {
        ...
        if ($i->has_user_config()) {
            $configurl = new agpu_url($baseurl);
        ...
        $showhideurl = new agpu_url($baseurl);
