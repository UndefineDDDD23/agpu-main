...
if (!$currentuser) {
    ...
    $url = new agpu_url('/user/preferences.php', array('userid' => $userid));
    $navbar = $PAGE->navbar->add(get_string('preferences', 'agpu'), $url);
    ...
    if (!$PAGE->settingsnav->can_view_user_preferences($userid)) {
        throw new agpu_exception('cannotedituserpreferences', 'error');
    }
} else {
