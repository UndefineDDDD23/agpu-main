...
if (array_search($formaction, $actions) === false) {
    throw new \agpu_exception('unknownuseraction');
}
...
if ($formaction == 'bulkchange.php') {
    ...
    $default = new agpu_url('/user/index.php', ['id' => $course->id]);
    $returnurl = new agpu_url(optional_param('returnto', $default, PARAM_LOCALURL));
    ...
    if (empty($plugin) AND $operationname == 'download_participants') {
        ...
        if (!$instance) {
            throw new \agpu_exception('errorwithbulkoperation', 'enrol');
        }
