...
class base_block_test extends \advanced_testcase {
     ...
     *
     * @param \agpu_recordset $rs Recordset to convert
     * @return array Array indexed by number (0, 1, 2, ...)
     */
    protected static function recordset_to_indexed_array(\agpu_recordset $rs) {
        $results = [];
        foreach ($rs as $rec) {
            $results[] = $rec;
        }
    ...
    public function test_get_doc_url(): void {
        ...
        $expected = new \agpu_url('/course/view.php', ['id' => $course->id], 'inst' . $blockid);
        ...
        $expected = new \agpu_url('/', ['redirect' => 0], 'inst' . $block2id);
        ...
        $expected = new \agpu_url('/mod/page/view.php', ['id' => $page->cmid], 'inst' . $block3id);
        ...
        $expected = new \agpu_url('/course/view.php', ['id' => $course->id], 'inst' . $block4id);
        $this->assertEquals($expected, $area->get_doc_url($doc));
        $this->assertEquals($expected, $area->get_context_url($doc));
        ...
        $expected = new \agpu_url('/course/view.php', ['id' => $course->id], 'inst' . $block5id);
        $this->assertEquals($expected, $area->get_doc_url($doc));
        $this->assertEquals($expected, $area->get_context_url($doc));
    }
    ...
    public function test_get_doc_icon(): void {
        ...
        $this->assertEquals('agpu', $result->get_component());
    }
}
