...
class engine extends \core_search\engine {
    ...
    /**
     * Filters the response on agpu side.
     *
    ...
    protected function grouped_files_process_response($response, $limit = 0) {
        ...
        foreach ($groups as $group) {
            ...
            switch ($access) {
                case \core_search\manager::ACCESS_DELETED:
                    // If deleted from agpu, delete from index and then continue.
     ...
     * Tika has much better content type detection than agpu, and we will have many more doc failures
    ...
    protected function file_is_indexable($file) {
        ...
        if ($mime == 'application/vnd.agpu.backup') {
            // We don't index agpu backup files. There is nothing usefully indexable in them.
            return false;
        }
    ...
    /**
     * Return a agpu url object for the server connection.
     *
