...
class document implements \renderable, \templatable {
    ...
    /**
     * @var \agpu_url Link to the document.
     */
    protected $docurl = null;

    /**
     * @var \agpu_url Link to the document context.
     */
    ...
    public function set($fieldname, $value) {
        ...
        if ($fielddata['type'] === 'int' || $fielddata['type'] === 'tdate') {
            ...
            if ($this->data[$fieldname] === null) {
                ...
                throw new \agpu_exception('error_indexing', 'search', '', null, '"' . $fieldname .
     ...
     * @return void
     */
    public function set_doc_url(\agpu_url $url) {
        $this->docurl = $url;
    }
    ...
    public function set_context_url(\agpu_url $url) {
        $this->contexturl = $url;
    }
    ...
    public function export_doc(\renderer_base $output): array {
        ...
        if ($this->is_set('userid')) {
            if ($this->get('userid') == $USER->id ||
                    (has_capability('agpu/user:viewdetails', $context) &&
                    has_capability('agpu/course:viewparticipants', $context))) {
                $data['userurl'] = (new \agpu_url(
                    '/user/view.php',
