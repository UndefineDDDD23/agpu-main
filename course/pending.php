...
if (has_capability('agpu/site:approvecourse', $context)) {
    ...
    $categories = core_course_category::make_categories_list('agpu/site:approvecourse');
    if (!$categories) {
        require_capability('agpu/site:approvecourse', $context);
    }
    $PAGE->set_context($context);
    $PAGE->set_url(new agpu_url('/course/pending.php'));
}
...
if (!empty($approve) and confirm_sesskey()) {
    ...
    if ($courseid !== false) {
        if (has_capability('agpu/course:update', context_course::instance($courseid))) {
            redirect(new agpu_url('/course/edit.php', ['id' => $courseid, 'returnto' => 'pending']));
        } else {
            redirect(new agpu_url('/course/view.php', ['id' => $courseid]));
        }
    } else {
        throw new \agpu_exception('courseapprovedfailed');
    }
}
...
if (empty($pending)) {
    ...
    if ($role) {
        ...
        $userpoliciesurl = new agpu_url('/admin/settings.php', ['section' => 'userpolicies']);
...
/// Build a table of all the requests.
    ...
    foreach ($pending as $course) {
        ...
        $row[] = $OUTPUT->single_button(new agpu_url($baseurl, array('approve' => $course->id, 'sesskey' => sesskey())), get_string('approve'), 'get') .
                 $OUTPUT->single_button(new agpu_url($baseurl, array('reject' => $course->id)), get_string('rejectdots'), 'get');
