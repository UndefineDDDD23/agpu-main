...
class core_course_renderer extends plugin_renderer_base {
    ...
    function course_section_add_cm_control($course, $section, $sectionreturn = null, $displayoptions = array()) {
        ...
        if (!has_capability('agpu/course:manageactivities', context_course::instance($course->id))
     ...
     * @param agpu_url $paginationurl url to view more or url to form links to the other pages in paging bar
    ...
    protected function course_name(coursecat_helper $chelper, core_course_list_element $course): string {
        ...
        $coursenamelink = html_writer::link(new agpu_url('/course/view.php', ['id' => $course->id]),
        ...
        if ($chelper->get_show_courses() < self::COURSECAT_SHOW_COURSES_EXPANDED) {
            if ($course->has_summary() || $course->has_course_contacts() || $course->has_course_overviewfiles()
                ...
                $url = new agpu_url('/course/info.php', ['id' => $course->id]);
    ...
    protected function course_overview_files(core_course_list_element $course): string {
        ...
        foreach ($course->get_course_overviewfiles() as $file) {
            ...
            $url = agpu_url::make_file_url("$CFG->wwwroot/pluginfile.php",
            ...
            if ($isimage) {
                ...
                $image = $this->output->pix_icon(file_file_icon($file), $file->get_filename(), 'agpu');
    ...
    protected function course_category_name(coursecat_helper $chelper, core_course_list_element $course): string {
        ...
        if ($chelper->get_show_courses() == self::COURSECAT_SHOW_COURSES_EXPANDED_WITH_CAT) {
            if ($cat = core_course_category::get($course->category, IGNORE_MISSING)) {
                ...
                $content .= html_writer::link(new agpu_url('/course/index.php', ['categoryid' => $cat->id]),
    ...
    protected function coursecat_subcategories(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        if ($totalcount > count($subcategories)) {
            if ($paginationurl) {
                ...
                if ($viewmoreurl->compare(new agpu_url('/course/index.php'), URL_MATCH_BASE)) {
    ...
    protected function coursecat_include_js() {
        ...
        $this->page->requires->yui_module('agpu-course-categoryexpander',
                'Y.agpu.course.categoryexpander.init');
    }
    ...
    protected function coursecat_category_content(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        if ($chelper->get_show_courses() > core_course_renderer::COURSECAT_SHOW_COURSES_COUNT) {
            ...
            if ($viewmoreurl = $chelper->get_courses_display_option('viewmoreurl')) {
                ...
                if ($viewmoreurl->compare(new agpu_url('/course/index.php'), URL_MATCH_BASE)) {
                    $chelper->set_courses_display_option('viewmoreurl', new agpu_url($viewmoreurl, array('categoryid' => $coursecat->id)));
                }
            }
    ...
    protected function coursecat_category(coursecat_helper $chelper, $coursecat, $depth) {
        ...
        $categoryname = html_writer::link(new agpu_url('/course/index.php',
    ...
    protected function coursecat_tree(coursecat_helper $chelper, $coursecat) {
        ...
        if ($coursecat->get_children_count()) {
            ...
            $content .= html_writer::end_tag('div');
            $this->page->requires->strings_for_js(array('collapseall', 'expandall'), 'agpu');
        }
    ...
    public function course_category($category) {
        ...
        $baseurl = new agpu_url('/course/index.php');
        if ($coursecat->id) {
        ...
        if ($browse === 'courses' || !$coursecat->get_children_count()) {
            ...
            $coursedisplayoptions['paginationurl'] = new agpu_url($baseurl, array('browse' => 'courses'));
            $catdisplayoptions['nodisplay'] = true;
            $catdisplayoptions['viewmoreurl'] = new agpu_url($baseurl, array('browse' => 'categories'));
            $catdisplayoptions['viewmoretext'] = new lang_string('viewallsubcategories');
        } else if ($browse === 'categories' || !$coursecat->get_courses_count()) {
            $coursedisplayoptions['nodisplay'] = true;
            $catdisplayoptions['offset'] = $page * $perpage;
            $catdisplayoptions['paginationurl'] = new agpu_url($baseurl, array('browse' => 'categories'));
            $coursedisplayoptions['viewmoreurl'] = new agpu_url($baseurl, array('browse' => 'courses'));
            $coursedisplayoptions['viewmoretext'] = new lang_string('viewallcourses');
        } else {
            // we have a category that has both subcategories and courses, display pagination separately
            $coursedisplayoptions['viewmoreurl'] = new agpu_url($baseurl, array('browse' => 'courses', 'page' => 1));
            $catdisplayoptions['viewmoreurl'] = new agpu_url($baseurl, array('browse' => 'categories', 'page' => 1));
        }
    ...
    public function coursecat_ajax() {
        ...
        if ($type === self::COURSECAT_TYPE_CATEGORY) {
            ...
            $chelper = new coursecat_helper();
            $baseurl = new agpu_url('/course/index.php', array('categoryid' => $categoryid));
            $coursedisplayoptions = array(
                'limit' => $CFG->coursesperpage,
                'viewmoreurl' => new agpu_url($baseurl, array('browse' => 'courses', 'page' => 1))
            );
            $catdisplayoptions = array(
                'limit' => $CFG->coursesperpage,
                'viewmoreurl' => new agpu_url($baseurl, array('browse' => 'categories', 'page' => 1))
            );
    ...
    public function search_courses($searchcriteria) {
        ...
        if (!empty($searchcriteria)) {
            ...
            $displayoptions['paginationurl'] = new agpu_url('/course/search.php', $searchcriteria);
                ...
                foreach ($courses as $course) {
                    ...
                    if ($showcategories && ($cat = core_course_category::get($course->category, IGNORE_MISSING))) {
                        $details = get_string('category').': '.
                                html_writer::link(new agpu_url('/course/index.php', array('categoryid' => $cat->id)),
    ...
    protected function frontpage_remote_course(stdClass $course) {
        $url = new agpu_url('/auth/mnet/jump.php', array(
    ...
    public function frontpage_my_courses() {
        ...
        if (!empty($courses) || !empty($rcourses) || !empty($rhosts)) {
            ...
            if (count($courses) > $CFG->frontpagecourselimit) {
                ...
                $chelper->set_courses_display_options(array(
                        'viewmoreurl' => new agpu_url('/my/courses.php'),
                        'viewmoretext' => new lang_string('mycourses')
                    ));
            } else if (core_course_category::top()->is_uservisible()) {
                // All enrolled courses are displayed, display link to 'All courses' if there are more courses in system.
                $chelper->set_courses_display_options(array(
                        'viewmoreurl' => new agpu_url('/course/index.php'),
    ...
    public function frontpage_available_courses() {
        ...
        if (!$totalcount && !$this->page->user_is_editing() && has_capability('agpu/course:create', context_system::instance())) {
    ...
    public function add_new_course_button() {
        ...
        $url = new agpu_url('/course/edit.php', array('category' => $CFG->defaultrequestcategory, 'returnto' => 'topcat'));
    ...
    protected function frontpage_news($forum) {
        ...
        if (isloggedin()) {
            ...
            $suburl = new agpu_url('/mod/forum/subscribe.php', array('id' => $forum->id, 'sesskey' => sesskey()));
