...
if (!$course = $DB->get_record('course', array('id'=>$id))) {
    throw new \agpu_exception("That's an invalid course id");
}
...
if (!empty($param->user)) {
    if (!$u = $DB->get_record('user', array('id'=>$param->user))) {
        throw new \agpu_exception("That's an invalid user!");
    }
    $userinfo = fullname($u);
}

$strrecentactivity = get_string('recentactivity');
$PAGE->navbar->add($strrecentactivity, new agpu_url('/course/recent.php', array('id'=>$course->id)));
    ...
    foreach ($activities as $key => $activity) {
        ...
        if (($activity->type == 'section') && ($param->sortby == 'default')) {
            ...
            if ($param->sortby == 'default') {
                ...
                $link = html_writer::link(new agpu_url("/mod/$cm->modname/view.php",
            ...
            if (!isset($viewfullnames[$activity->cmid])) {
                ...
                $viewfullnames[$activity->cmid] = has_capability('agpu/site:viewfullnames', $cm_context);
            }
