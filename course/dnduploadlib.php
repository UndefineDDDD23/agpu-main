...
function dndupload_add_to_course($course, $modnames) {
    ...
    $jsmodule = array(
        ...
        'strings' => array(
            array('addfilehere', 'agpu'),
            array('dndworkingfiletextlink', 'agpu'),
            array('dndworkingfilelink', 'agpu'),
            array('dndworkingfiletext', 'agpu'),
            array('dndworkingfile', 'agpu'),
            array('dndworkingtextlink', 'agpu'),
            array('dndworkingtext', 'agpu'),
            array('dndworkinglink', 'agpu'),
            array('namedfiletoolarge', 'agpu'),
            array('actionchoice', 'agpu'),
            array('servererror', 'agpu'),
            array('filereaderror', 'agpu'),
            array('upload', 'agpu'),
            array('cancel', 'agpu'),
            array('changesmadereallygoaway', 'agpu')
        ),
...
class dndupload_handler {
    ...
    public function __construct($course, $modnames = null) {
        ...
        $this->register_type('url', array('url', 'text/uri-list', 'text/x-moz-url'), get_string('addlinkhere', 'agpu'),
                        get_string('nameforlink', 'agpu'), get_string('whatforlink', 'agpu'), 10);
        $this->register_type('text/html', array('text/html'), get_string('addpagehere', 'agpu'),
                        get_string('nameforpage', 'agpu'), get_string('whatforpage', 'agpu'), 20);
        $this->register_type('text', array('text', 'text/plain'), get_string('addpagehere', 'agpu'),
                        get_string('nameforpage', 'agpu'), get_string('whatforpage', 'agpu'), 30);
...
class dndupload_ajax_processor {
    ...
    public function process($displayname = null, $content = null) {
        require_capability('agpu/course:manageactivities', $this->context);

        if ($this->is_file_upload()) {
            require_capability('agpu/course:managefiles', $this->context);
            if ($content != null) {
                throw new agpu_exception('fileuploadwithcontent', 'agpu');
            }
        } else {
            if (empty($content)) {
                throw new agpu_exception('dnduploadwithoutcontent', 'agpu');
            }
        }
    ...
    protected function handle_file_upload() {
        ...
        if (empty($repo)) {
            throw new agpu_exception('errornouploadrepo', 'agpu');
        }
    ...
    protected function handle_other_upload($content) {
        ...
        if (!$this->dnduploadhandler->has_type_handler($this->module->name, $this->type)) {
            ...
            throw new agpu_exception('moddoesnotsupporttype', 'agpu', $info);
        }
    ...
    protected function finish_setup_course_module($instanceid) {
        ...
        if (!$instanceid) {
            ...
            throw new agpu_exception('errorcreatingactivity', 'agpu', '', $this->module->name);
        }
