...
$url = new agpu_url('/course/loginas.php', array('id'=>$id));
$PAGE->set_url($url);
...
if (\core\session\manager::is_loggedinas()) {
    ...
    redirect(new agpu_url($url, array('redirect'=>1)));
}
...
if (has_capability('agpu/user:loginas', $systemcontext)) {
    if (is_siteadmin($userid)) {
        throw new \agpu_exception('nologinas');
    }
    $context = $systemcontext;
    $PAGE->set_context($context);
} else {
    require_login($course);
    require_capability('agpu/user:loginas', $coursecontext);
    if (is_siteadmin($userid)) {
        throw new \agpu_exception('nologinas');
    }
    if (!is_enrolled($coursecontext, $userid)) {
        throw new \agpu_exception('usernotincourse');
    }
...
if ($course->id != SITEID) {
    ...
    $returnurl = new agpu_url('/', ['redirect' => 1]);
}
