...
require_capability('agpu/course:update', context_course::instance($course->id));
require_sesskey();
...
if ($desirednumsections > $maxsections) {
    ...
    \core\notification::warning(get_string('maxsectionslimit', 'agpu', $maxsections));
    $increase = null;
    $insertsection = null;
    $numsections = 0;
...
if (isset($courseformatoptions['numsections']) && $increase !== null) {
    ...
    if ($insertsection) {
        ...
        require_capability('agpu/course:movesections', context_course::instance($course->id));
    }
    $sections = [];
