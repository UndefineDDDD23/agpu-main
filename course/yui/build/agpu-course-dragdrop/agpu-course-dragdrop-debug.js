YUI.add('agpu-course-dragdrop', function (Y, NAME) {

/* eslint-disable no-unused-vars */
/**
 * Drag and Drop for course sections and course modules.
 *
 * @module agpu-course-dragdrop
 */
...
Y.extend(DRAGSECTION, M.core.dragdrop, {
    ...
    drop_hit: function(e) {
        ...
        if (dragnodeid === dropnodeindex) {
            Y.log("Skipping move - same location moving " + dragnodeid + " to " + dropnodeindex, 'debug', 'agpu-course-dragdrop');
            return;
        }

        Y.log("Moving from position " + dragnodeid + " to position " + dropnodeindex, 'debug', 'agpu-course-dragdrop');
...
Y.extend(DRAGRESOURCE, M.core.dragdrop, {
    ...
    setup_for_resource: function(baseselector) {
        Y.Node.all(baseselector).each(function(resourcesnode) {
            ...
            if (move) {
                ...
                move.replace(this.get_drag_handle(M.util.get_string('movecoursemodule', 'agpu'),
    ...
    drop_hit: function(e) {
        ...
        var cmid = Number(Y.agpu.core_course.util.cm.getId(dragnode));
        var beforeid = null;
        ...
        params.field = 'move';
        params.id = cmid;
        params.sectionId = Y.agpu.core_course.util.section.getId(dropnode.ancestor(M.course.format.get_section_wrapper(Y), true));

        if (dragnode.next()) {
            beforeid = Number(Y.agpu.core_course.util.cm.getId(dragnode.next()));
