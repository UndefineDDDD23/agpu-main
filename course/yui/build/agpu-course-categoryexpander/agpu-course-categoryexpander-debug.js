YUI.add('agpu-course-categoryexpander', function (Y, NAME) {
 ...
 *
 * @module agpu-course-categoryexpander
 */
 ...
 *
 * @constructor
 * @class Y.agpu.course.categoryexpander
 */
...
var CSS = {
    ...
    NS = Y.namespace('agpu.course.categoryexpander'),
    TYPE_CATEGORY = 0,
    TYPE_COURSE = 1,
...
NS.setup_keyboard_listeners = function() {
    ...
    Y.log('Setting the tabindex for all expandable course nodes', 'info', 'agpu-course-categoryexpander');
...
NS.expand_category = function(categorynode) {
    ...
    Y.use('io-base', 'json-parse', 'agpu-core-notification', 'anim-node-plugin', function() {
...
NS.update_collapsible_actions = function(ancestor) {
    ...
    if (foundmaximisedchildren) {
        ...
        togglelink.setHTML(M.util.get_string('collapseall', 'agpu'))
        ...
        togglelink.setHTML(M.util.get_string('expandall', 'agpu'))
