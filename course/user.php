...
$url = new agpu_url('/course/user.php', array('id'=>$id,'user'=>$user, 'mode'=>$mode));

$course = $DB->get_record('course', array('id'=>$id), '*', MUST_EXIST);
$user = $DB->get_record("user", array("id"=>$user, 'deleted'=>0), '*', MUST_EXIST);

if ($mode === 'outline' or $mode === 'complete') {
    $url = new agpu_url('/report/outline/user.php', array('id'=>$user->id, 'course'=>$course->id, 'mode'=>$mode));
    redirect($url);
}
if ($mode === 'todaylogs' or $mode === 'alllogs') {
    $logmode = ($mode === 'todaylogs') ? 'today' : 'all';
    $url = new agpu_url('/report/log/user.php', array('id'=>$user->id, 'course'=>$course->id, 'mode'=>$logmode));
    redirect($url);
}
if ($mode === 'stats') {
...
if ($mode === 'coursecompletions' or $mode === 'coursecompletion') {
    $url = new agpu_url('/report/completion/user.php', array('id'=>$user->id, 'course'=>$course->id));
    redirect($url);
}
...
if (has_capability('agpu/user:viewuseractivitiesreport', $personalcontext) and !is_enrolled($coursecontext)) {
...
$anyreport  = has_capability('agpu/user:viewuseractivitiesreport', $personalcontext);
...
} else if ($course->showgrades and $user->id == $USER->id and has_capability('agpu/grade:view', $coursecontext)) {
    //ok - can view own grades
    $modes[] = 'grade';

} else if ($course->showgrades and has_capability('agpu/grade:viewall', $personalcontext)) {
...
if (empty($modes)) {
    require_capability('agpu/user:viewuseractivitiesreport', $personalcontext);
}
    ...
    case "grade":
        ...
        if ($USER->id == $user->id) {
            ...
            if (!preg_match('/^user\d{0,}$/', $activenode->key ?? '')) { // No user name found.
                $userurl = new agpu_url('/user/view.php', array('id' => $user->id, 'course' => $course->id));
                // Add the user name.
                $usernode = $activenode->add(fullname($user), $userurl, navigation_node::TYPE_SETTING);
                $usernode->add(get_string('grades'));
            } else {
                $url = new agpu_url('/course/user.php', array('id' => $id, 'user' => $user->id, 'mode' => $mode));
            ...
            $gradeurl = new agpu_url('/course/user.php', array('id' => $id, 'user' => $user->id, 'mode' => $mode));
        ...
        if ($course->id !== SITEID) {
            $userheading = array(
                'heading' => fullname($user, has_capability('agpu/site:viewfullnames', $PAGE->context)),
                'user' => $user,
                'usercontext' => $personalcontext,
            );
            echo $OUTPUT->context_header($userheading, 2);

            echo $OUTPUT->heading(get_string('grades', 'agpu'), 2, 'main mt-4 mb-4');
        }
