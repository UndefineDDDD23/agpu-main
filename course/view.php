...
if (!empty($name)) {
    ...
    throw new \agpu_exception('unspecifycourseid', 'error');
}
...
if ($switchrole > 0 && confirm_sesskey() &&
    has_capability('agpu/role:switchroles', $context)) {
...
$PAGE->set_other_editing_capability('agpu/course:update');
$PAGE->set_other_editing_capability('agpu/course:manageactivities');
$PAGE->set_other_editing_capability('agpu/course:activityvisibility');
if (course_format_uses_sections($course->format)) {
    $PAGE->set_other_editing_capability('agpu/course:sectionvisibility');
    $PAGE->set_other_editing_capability('agpu/course:movesections');
}
...
if ($PAGE->user_allowed_editing()) {
    if (($edit == 1) && confirm_sesskey()) {
        ...
        if ($course->id == SITEID) {
            ...
            $url = new agpu_url($PAGE->url, ['notifyeditingon' => 1]);
            redirect($url);
        }
    ...
    if (has_capability('agpu/course:sectionvisibility', $context)) {
    ...
    if (
        ...
        && has_capability('agpu/course:update', $context) && confirm_sesskey()
    ) {
    ...
    if (!empty($section) && !empty($move) &&
            has_capability('agpu/course:movesections', $context) && confirm_sesskey()) {
...
if ($section && $section > 0 && course_format_uses_sections($course->format)) {
    ...
    $PAGE->set_title(get_string('coursetitle' . $editingtitle, 'agpu', ['course' => $course->fullname]));
}
...
if (has_capability('agpu/course:update', $context)) {
