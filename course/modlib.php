...
function add_moduleinfo($moduleinfo, $course, $mform = null) {
    ...
    if (has_capability('agpu/course:setforcedlanguage', context_course::instance($course->id))) {
    ...
    if (!$moduleinfo->coursemodule = add_course_module($newcm)) {
        throw new \agpu_exception('cannotaddcoursemodule');
    }
    ...
    if (!$returnfromfunc or !is_number($returnfromfunc)) {
        ...
        if ($returnfromfunc instanceof agpu_exception) {
            throw $returnfromfunc;
        } else if (!is_number($returnfromfunc)) {
            throw new \agpu_exception('invalidfunction', '', course_get_url($course, $moduleinfo->section));
        } else {
            throw new \agpu_exception('cannotaddnewmodule', '', course_get_url($course, $moduleinfo->section),
 ...
 * Please note this api does not trigger events as of agpu 2.6. Please trigger events before calling this api.
 *
...
function edit_module_post_actions($moduleinfo, $course) {
    ...
    if (plugin_supports('mod', $moduleinfo->modulename, FEATURE_ADVANCED_GRADING, false)
            and has_capability('agpu/grade:managegradingforms', $modcontext)) {
...
function can_add_moduleinfo($course, $modulename, $sectionnum) {
    ...
    if (!course_allowed_module($course, $module->name)) {
        throw new \agpu_exception('moduledisable');
    }
 ...
 * @throws agpu_exception if user is not allowed to perform the action
 */
...
function update_moduleinfo($cm, $moduleinfo, $course, $mform = null) {
    ...
    $modcontext = context_module::instance($moduleinfo->coursemodule);
    if (has_capability('agpu/course:setforcedlanguage', $modcontext)) {
    ...
    if (!$updateinstancefunction($moduleinfo, $mform)) {
        throw new \agpu_exception('cannotupdatemod', '', course_get_url($course, $cm->section), $moduleinfo->modulename);
    }
    ...
    if (!empty($data->grade_rescalegrades) && $data->grade_rescalegrades == 'yes') {
        ...
        if ($newgradeitem && $oldgradeitem->gradetype == GRADE_TYPE_VALUE && $newgradeitem->gradetype == GRADE_TYPE_VALUE) {
            ...
            if (!component_callback('mod_' . $moduleinfo->modulename, 'rescale_activity_grades', $params)) {
                throw new \agpu_exception('cannotreprocessgrades', '', course_get_url($course, $cm->section),
                    $moduleinfo->modulename);
            }
        }
    }
 ...
 * @throws agpu_exception if lib.php file for the module does not exist
 */
function include_modulelib($modulename) {
    ...
    if (file_exists($modlib)) {
        ...
        throw new agpu_exception('modulemissingcode', '', '', $modlib);
    }
}
...
function get_moduleinfo_data($cm, $course) {
    ...
    if (plugin_supports('mod', $data->modulename, FEATURE_ADVANCED_GRADING, false)
            and has_capability('agpu/grade:managegradingforms', $context)) {
        require_once($CFG->dirroot.'/grade/grading/lib.php');
