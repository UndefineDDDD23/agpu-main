...
final class externallib_test extends externallib_advanced_testcase {
    ...
    public function test_create_categories(): void {
        ...
        $roleid = $this->assignUserCapability('agpu/category:manage', $contextid);
        ...
        $this->unassignUserCapability('agpu/category:manage', $contextid, $roleid);
    ...
    public function test_get_categories(): void {
        ...
        $category1data['descriptionformat'] = FORMAT_agpu;
        ...
        $roleid = $this->assignUserCapability('agpu/category:manage', $context->id);
        $this->assignUserCapability('agpu/category:viewhiddencategories', $context->id, $roleid);
        ...
        foreach ($categories as $category) {
            ...
            $this->assertEquals($category['description'], format_text($generatedcat->description, FORMAT_agpu, array('para' => false)));
        ...
        $this->unassignUserCapability('agpu/category:viewhiddencategories', $context->id, $roleid);
        ...
        $this->expectException('agpu_exception');
        $categories = core_course_external::get_categories(array(
            array('key' => 'id', 'value' => $category1->id),
            array('key' => 'idnumber', 'value' => $category1->idnumber),
            array('key' => 'visible', 'value' => 1)), 0);
    ...
    public function test_update_categories_moving(): void {
        ...
        $roleid = $this->assignUserCapability('agpu/category:manage', $contextcata->id);
    ...
    public function test_create_course_numsections(): void {
        ...
        // Set the required capabilities by the external function.
        $contextid = context_system::instance()->id;
        $roleid = $this->assignUserCapability('agpu/course:create', $contextid);
        $this->assignUserCapability('agpu/course:visibility', $contextid, $roleid);
    ...
    public function test_create_courses(): void {
        ...
        $roleid = $this->assignUserCapability('agpu/course:create', $contextid);
        $this->assignUserCapability('agpu/course:visibility', $contextid, $roleid);
        $this->assignUserCapability('agpu/course:setforcedlanguage', $contextid, $roleid);
        ...
        $course2['summaryformat'] = FORMAT_agpu;
        ...
        foreach ($createdcourses as $createdcourse) {
            ...
            if ($createdcourse['shortname'] == $course2['shortname']) {
                ...
                throw new agpu_exception('Unexpected shortname');
            }
        }

        // Call without required capability
        $this->unassignUserCapability('agpu/course:create', $contextid, $roleid);
        $this->expectException('required_capability_exception');
    ...
    public function test_get_courses(): void {
        ...
        $coursedata['summaryformat'] = FORMAT_agpu;
        ...
        // Set the required capabilities by the external function.
        $context = context_system::instance();
        $roleid = $this->assignUserCapability('agpu/course:view', $context->id);
        $this->assignUserCapability('agpu/course:update',
        ...
        foreach ($courses as $course) {
            ...
            $this->assertEquals($course['summary'], format_text($dbcourse->summary, FORMAT_agpu, array('para' => false)));
    ...
    private function prepare_get_course_contents_test() {
        ...
        $roleid = $this->assignUserCapability('agpu/course:view', $context->id);
        $this->assignUserCapability('agpu/course:update', $context->id, $roleid);
        $this->assignUserCapability('mod/data:view', $context->id, $roleid);

        $conditions = array('course' => $course->id, 'section' => 2);
        $DB->set_field('course_sections', 'summary', 'Text with iframe <iframe src="https://agpu.org"></iframe>', $conditions);
    ...
    public function test_duplicate_course(): void {
        ...
        $roleid = $this->assignUserCapability('agpu/course:create', $categorycontext->id);
        $this->assignUserCapability('agpu/course:view', $categorycontext->id, $roleid);
        $this->assignUserCapability('agpu/restore:restorecourse', $categorycontext->id, $roleid);
        $this->assignUserCapability('agpu/backup:backupcourse', $coursecontext->id, $roleid);
        $this->assignUserCapability('agpu/backup:configure', $coursecontext->id, $roleid);
        // Optional capabilities to copy user data.
        $this->assignUserCapability('agpu/backup:userinfo', $coursecontext->id, $roleid);
        $this->assignUserCapability('agpu/restore:userinfo', $categorycontext->id, $roleid);
    ...
    public function test_update_courses(): void {
        ...
        // Set the required capabilities by the external function.
        $contextid = context_system::instance()->id;
        $roleid = $this->assignUserCapability('agpu/course:update', $contextid);
        $this->assignUserCapability('agpu/course:changecategory', $contextid, $roleid);
        $this->assignUserCapability('agpu/course:changelockedcustomfields', $contextid, $roleid);
        $this->assignUserCapability('agpu/course:changefullname', $contextid, $roleid);
        $this->assignUserCapability('agpu/course:changeshortname', $contextid, $roleid);
        $this->assignUserCapability('agpu/course:changeidnumber', $contextid, $roleid);
        $this->assignUserCapability('agpu/course:changesummary', $contextid, $roleid);
        $this->assignUserCapability('agpu/course:visibility', $contextid, $roleid);
        $this->assignUserCapability('agpu/course:viewhiddencourses', $contextid, $roleid);
        $this->assignUserCapability('agpu/course:setforcedlanguage', $contextid, $roleid);
        ...
        foreach ($courses as $course) {
            ...
            if ($course['id'] == $course2['id']) {
                ...
                $this->assertEquals(FORMAT_agpu, $courseinfo->summaryformat);
        ...
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        $this->unassignUserCapability('agpu/course:update', $contextid, $roleid);
        ...
        $this->assignUserCapability('agpu/course:update', $contextid, $roleid);
        $this->unassignUserCapability('agpu/course:changecategory', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        ...
        $this->assignUserCapability('agpu/course:changecategory', $contextid, $roleid);
        $this->unassignUserCapability('agpu/course:changefullname', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        $updatedcoursewarnings = core_course_external::update_courses($courses);
        ...
        $this->assignUserCapability('agpu/course:changefullname', $contextid, $roleid);
        $this->unassignUserCapability('agpu/course:changeshortname', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        $updatedcoursewarnings = core_course_external::update_courses($courses);
        ...
        $this->assignUserCapability('agpu/course:changeshortname', $contextid, $roleid);
        $this->unassignUserCapability('agpu/course:changeidnumber', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        $updatedcoursewarnings = core_course_external::update_courses($courses);
        ...
        $this->assignUserCapability('agpu/course:changeidnumber', $contextid, $roleid);
        $this->unassignUserCapability('agpu/course:changesummary', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        $updatedcoursewarnings = core_course_external::update_courses($courses);
        ...
        $this->unassignUserCapability('agpu/course:visibility', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
        self::getDataGenerator()->enrol_user($user->id, $course1['id'], $roleid);
        $course1['summaryformat'] = FORMAT_agpu;
        $courses = array($course1);
        $updatedcoursewarnings = core_course_external::update_courses($courses);
        $updatedcoursewarnings = external_api::clean_returnvalue(core_course_external::update_courses_returns(),
                                                                    $updatedcoursewarnings);
        ...
        $this->unassignUserCapability('agpu/course:changelockedcustomfields', $contextid, $roleid);
        $user = self::getDataGenerator()->create_user();
        $this->setUser($user);
    ...
    public function test_delete_modules(): void {
        ...
        $this->assignUserCapability('agpu/course:manageactivities', $modcontext->id);

        // Assign capabilities to delete module 2.
        $modcontext = context_module::instance($module2->cmid);
        $newrole = create_role('Role 2', 'role2', 'Role 2 description');
        $this->assignUserCapability('agpu/course:manageactivities', $modcontext->id, $newrole);
    ...
    public function test_get_recent_courses(): void {
        ...
        $this->assignUserCapability('agpu/user:viewdetails', $usercontext, $teacherroleid);
