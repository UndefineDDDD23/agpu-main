...
class category_test extends \advanced_testcase {
    ...
    public function test_visibility(): void {
        $this->assign_capability('agpu/category:viewhiddencategories');
        $this->assign_capability('agpu/category:manage');
    ...
    public function test_delete(): void {
        ...
        $this->assign_capability('agpu/category:manage');
        $this->assign_capability('agpu/course:create');
        ...
        $this->assign_capability('agpu/course:create', CAP_ALLOW, \context_coursecat::instance($category3->id));
        $this->assign_capability('agpu/category:manage');
        ...
        // Add necessary capabilities.
        $this->assign_capability('agpu/course:delete', CAP_ALLOW, \context_coursecat::instance($category3->id));
    ...
    public function test_get_search_courses(): void {
        ...
        // Add necessary capabilities.
        $this->assign_capability('agpu/course:create', CAP_ALLOW, \context_coursecat::instance($cat2->id));
        // Do another search with restricted capabilities.
        $reqcaps = array('agpu/course:create');
    ...
    public function test_coursecat_is_uservisible(): void {
        ...
        // User $user should still have the agpu/category:viewhiddencategories capability.
        $this->assertTrue($category1->is_uservisible($userid));
        $this->assertTrue($category2->is_uservisible($userid));

        $this->assign_capability('agpu/category:viewhiddencategories', CAP_INHERIT);
    ...
    public function test_current_user_coursecat_get(): void {
        ...
        // Expecting to get an exception as this new user does not have the agpu/category:viewhiddencategories capability.
    ...
    public function test_get_nearest_editable_subcategory(): void {
        ...
        $this->assertEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['agpu/course:create']));
        $this->assertEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['manage']));
        $this->assertEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['agpu/category:manage']));
        ...
        $this->assertNotEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['agpu/course:create']));
        $this->assertEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['manage']));
        $this->assertEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['agpu/category:manage']));
        $this->assertEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['create', 'manage']));
        ...
        $this->assertEquals($category1->id,
            core_course_category::get_nearest_editable_subcategory($coursecat, ['agpu/course:create'])->id);
        ...
        $this->setUser($user2);
        // Remove the agpu/course:create capability for the manager role.
        unassign_capability('agpu/course:create', $managerrole->id);
        $coursecat = core_course_category::user_top();
        $this->assertEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['create']));
        $this->assertEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['agpu/course:create']));
        $this->assertNotEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['manage']));
        $this->assertNotEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['agpu/category:manage']));
        $this->assertEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['create', 'manage']));
        ...
        $this->assertEquals($category3->id,
            core_course_category::get_nearest_editable_subcategory($coursecat, ['agpu/category:manage'])->id);
        // End scenario 3.

        // Start scenario 4.
        // user2 has both permission to create course and manage category.
        // Add the agpu/course:create capability back again for the manager role.
        assign_capability('agpu/course:create', CAP_ALLOW, $managerrole->id, $category3context->id);
        $this->setUser($user2);
        $coursecat = core_course_category::user_top();
        $this->assertNotEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['create']));
        $this->assertNotEmpty(core_course_category::get_nearest_editable_subcategory($coursecat, ['agpu/course:create']));
        ...
        $this->assertEquals($category3->id, core_course_category::get_nearest_editable_subcategory($coursecat,
            ['agpu/course:create', 'agpu/category:manage'])->id);
        // End scenario 4.
