...
class search_test extends \advanced_testcase {
     ...
     *
     * @param agpu_recordset $rs Recordset to convert (and close)
     * @return array Array of IDs from records indexed by number (0, 1, 2, ...)
     */
    protected static function recordset_to_ids(\agpu_recordset $rs) {
        $results = [];
        foreach ($rs as $rec) {
            $results[] = $rec->id;
        }
    ...
    public function test_customfield_access(): void {
        ...
        assign_capability('agpu/category:viewcourselist', CAP_PREVENT, $userrole, \context_system::instance()->id, true);
