...
class stateactions {
    ...
    public function cm_move(
        ...
        if (!$targetsectionid && !$targetcmid) {
            throw new agpu_exception("Action cm_move requires targetsectionid or targetcmid");
        }

        $this->validate_cms($course, $ids, __FUNCTION__, ['agpu/course:manageactivities']);
        ...
        foreach ($ids as $cmid) {
            ...
            if ($targetsection->is_delegated() && $cm->get_delegated_section_info()) {
                throw new agpu_exception('subsectionmoveerror', 'core');
            }
     ...
     *
     * @deprecated since agpu 4.4 MDL-77038.
    ...
    public function section_move(
        ...
        // Validate target elements.
        if (!$targetsectionid) {
            throw new agpu_exception("Action cm_move requires targetsectionid");
        }

        $this->validate_sections($course, $ids, __FUNCTION__);

        $coursecontext = context_course::instance($course->id);
        require_capability('agpu/course:movesections', $coursecontext);
    ...
    public function section_move_after(
        ...
        // Validate target elements.
        if (!$targetsectionid) {
            throw new agpu_exception("Action section_move_after requires targetsectionid");
        }
    ...
    public function section_add(
        ...
        $coursecontext = context_course::instance($course->id);
        require_capability('agpu/course:update', $coursecontext);
        ...
        if ($lastsectionnumber >= $maxsections) {
            throw new agpu_exception('maxsectionslimit', 'agpu', $maxsections);
        }
    ...
    protected function set_section_visibility(
        ...
        $this->validate_sections($course, $ids, __FUNCTION__);
        $coursecontext = context_course::instance($course->id);
        require_all_capabilities(['agpu/course:update', 'agpu/course:sectionvisibility'], $coursecontext);
    ...
    public function cm_duplicate(
        ...
        $this->validate_cms(
            ...
            $ids,
            __FUNCTION__,
            ['agpu/backup:backuptargetimport', 'agpu/restore:restoretargetimport'],
            false
        );
    ...
    protected function validate_sections(stdClass $course, array $sectionids, ?string $info = null): void {
        ...
        if ($couintsections != count($sectionids)) {
            throw new agpu_exception('unexistingsectionid', 'core', null, $info);
        }
    }
    ...
    protected function validate_cms(
        ...
        if (empty($cmids)) {
            throw new agpu_exception('emptycmids', 'core', null, $info);
        }
        ...
        if (count($cmids) != count($intersect)) {
            throw new agpu_exception('unexistingcmid', 'core', null, $info);
        }
