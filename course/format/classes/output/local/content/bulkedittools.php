 ...
 * @copyright 2023 Ferran Recio <ferran@agpu.com>
...
class bulkedittools implements named_templatable, renderable {
    ...
    protected function cm_control_items(): array {
        ...
        if (has_capability('agpu/course:activityvisibility', $context, $user)) {
        ...
        $duplicatecapabilities = ['agpu/backup:backuptargetimport', 'agpu/restore:restoretargetimport'];
        ...
        $hasmanageactivities = has_capability('agpu/course:manageactivities', $context, $user);
        ...
        if ($CFG->enablesharingtoagpunet && $usercanshare) {
            $controls['sharetoagpunet'] = [
                'id' => 'cmShareToagpuNet',
                'icon' => 'i/share',
                'action' => 'cmShareToagpuNet',
                'name' => get_string('agpunet:sharetoagpunet'),
                'title' => get_string('agpunet:sharetoagpunet'),
                'bulk' => 'cm',
            ];
        }
    ...
    protected function section_control_items(): array {
        ...
        if (has_capability('agpu/course:sectionvisibility', $context, $user)) {
            $controls['availability'] = [
                'icon' => 't/show',
        ...
        if (!$sectionreturn && has_capability('agpu/course:movesections', $context, $user)) {
            $controls['move'] = [
                'icon' => 'i/dragdrop',
                'action' => 'moveSection',
                'name' => get_string('move', 'agpu'),
                'title' => $this->format->get_format_string('sectionsmove'),
                'bulk' => 'section',
            ];
        }

        $deletecapabilities = ['agpu/course:movesections', 'agpu/course:update'];
