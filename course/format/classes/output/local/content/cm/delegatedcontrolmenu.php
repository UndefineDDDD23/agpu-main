 ...
 * @copyright 2024 Amaia Anabitarte <amaia@agpu.com>
...
class delegatedcontrolmenu extends basecontrolmenu {
    ...
    public function delegated_control_items() {
        ...
        $cmbaseurl = new agpu_url('/course/mod.php');
        $cmbaseurl->param('sesskey', sesskey());

        $hasmanageactivities = has_capability('agpu/course:manageactivities', $coursecontext);
        ...
        if (!$isheadersection) {
            $controls['view'] = [
                'url'   => new agpu_url('/course/section.php', ['id' => $section->id]),
                'icon' => 'i/viewsection',
        ...
        if (has_capability('agpu/course:update', $coursecontext, $user)) {
            ...
            $controls['edit'] = [
                'url'   => new agpu_url('/course/editsection.php', $params),
                'icon' => 'i/settings',
                'name' => $streditsection,
                'pixattr' => ['class' => ''],
                'attr' => ['class' => 'edit'],
        ...
        $availablevisibility = has_capability('agpu/course:sectionvisibility', $coursecontext, $user) && $parentsection->visible;
        ...
        if (!$isheadersection && $hasmanageactivities && $usecomponents) {
            $controls['move'] = [
                'url'   => new agpu_url('/course/mod.php', ['copy' => $cm->id]),
                'icon' => 'i/dragdrop',
