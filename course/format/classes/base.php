 ...
 *
 * If you are upgrading from agpu 2.3 start with copying the class format_legacy and renaming
...
abstract class base {
    ...
    public function get_max_sections() {
        $maxsections = get_config('agpucourse', 'maxsections');
     ...
     * @todo MDL-80116 This will be deleted in agpu 4.8.
     */
     ...
     * @return null|agpu_url
     */
    public function get_view_url($section, $options = array()) {
        $course = $this->get_course();
        $url = new agpu_url('/course/view.php', ['id' => $course->id]);
        ...
        if ((!empty($options['navigation']) || array_key_exists('sr', $options)) && $sectionno !== null) {
            ...
            return new agpu_url('/course/section.php', ['id' => $sectioninfo->id]);
        }
     ...
     * @return agpu_url
     */
    public function get_non_ajax_cm_action_url(string $action, cm_info $cm): agpu_url {
        $nonajaxactions = [
            'cmDelete' => 'delete',
     ...
     *    arguments in the agpuform::addElement() method
     ...
     *
     * @param \agpuQuickForm $mform form the elements are added to
    ...
    public function create_edit_form_elements(&$mform, $forsection = false) {
        ...
        if (!$forsection && empty($this->courseid)) {
            ...
            // agpuQuickForm_date_time_selector class, otherwise it's disabled by default.
            if (get_config('agpucourse', 'courseenddateenabled')) {
     ...
     *
     * @param stdClass|array $data return value from agpuform::get_data() or array with data
    ...
    /**
     * Return an instance of agpuform to edit a specified section
     *
    ...
    /**
     * Allows course format to execute code on agpu_page::set_course()
     *
     * @param agpu_page $page instance of page calling set_course
     */
    public function page_set_course(agpu_page $page) {
    }

    /**
     * Allows course format to execute code on agpu_page::set_cm()
     *
     * Current module can be accessed as $page->cm (returns instance of cm_info)
     *
     * @param agpu_page $page instance of page calling set_cm
     */
    public function page_set_cm(agpu_page $page) {
    }
     ...
     */
    public function get_renderer(agpu_page $page) {
        try {
            ...
            debugging(
                "The '{$formatname}' course format does not define the {$expectedrenderername} renderer class. This is required since agpu 4.0.",
                 DEBUG_DEVELOPER
            );
    ...
    public function is_section_visible(section_info $section): bool {
        ...
        if ($section->is_orphan() && !has_capability('agpu/course:viewhiddensections', $this->get_context())) {
            return false;
        }
        // Previous to agpu 4.0 thas logic was hardcoded. To prevent errors in the contrib plugins
     ...
     *  If null or not specified, the user needs to have 'agpu/course:manageactivities'.
     * @return bool true if edit controls must be displayed
     */
    public function show_editor(?array $capabilities = ['agpu/course:manageactivities']): bool {
        global $PAGE;
        $course = $this->get_course();
        $coursecontext = context_course::instance($course->id);
        if ($capabilities === null) {
            $capabilities = ['agpu/course:manageactivities'];
        }
    ...
    public function show_groupmode(cm_info $cm): bool {
        ...
        if (!has_capability('agpu/course:manageactivities', $cm->context)) {
            return false;
        }
        return true;
    }
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if ($editable === null) {
            $editable = !empty($USER->editing) && has_capability('agpu/course:update',
     ...
     * agpucourse/courseduration setting. Course formats like format_weeks for
    ...
    public function get_default_course_enddate($mform, $fieldnames = array()) {
        ...
        $courseduration = intval(get_config('agpucourse', 'courseduration'));
        if (!$courseduration) {
    ...
    public function section_action($section, $action, $sr) {
        ...
        if (!$this->uses_sections() || !$section->section) {
            ...
            throw new agpu_exception('sectionactionnotsupported', 'core', null, s($action));
        }
        ...
        switch($action) {
            case 'hide':
            case 'show':
                require_capability('agpu/course:sectionvisibility', $coursecontext);
    ...
    public function duplicate_section(section_info $originalsection): section_info {
        if (!$this->uses_sections()) {
            throw new agpu_exception('sectionsnotsupported', 'core_courseformat');
        }
        ...
        if (!empty($originalsection->name)) {
            $newsectiondata->name = get_string('duplicatedsection', 'agpu', $originalsection->name);
        } else {
