...
$url = new agpu_url('/course/management.php');
...
if ($courseid) {
    ...
    if (empty($topchildren)) {
        throw new agpu_exception('cannotviewcategory', 'error');
    }
...
if (!core_course_category::has_capability_on_any(array('agpu/category:manage', 'agpu/course:create'))) {
    // The user isn't able to manage any categories. Lets redirect them to the relevant course/index.php page.
    $url = new agpu_url('/course/index.php');
    if ($categoryid) {
        $url->param('categoryid', $categoryid);
    }
    redirect($url);
...
if (!$issearching && $category !== null) {
    ...
    foreach ($parents as $parent) {
        $PAGE->navbar->add(
            ...
            new agpu_url('/course/index.php', array('categoryid' => $parent->id))
        );
    }
...
if ($category && !has_any_capability($capabilities, $systemcontext)) {
    ...
    navigation_node::override_active_url(new agpu_url('/course/index.php', array('categoryid' => $category->id)));
    ...
    navigation_node::override_active_url(new agpu_url('/course/index.php'));
}
...
if ($action !== false && confirm_sesskey()) {
    ...
    switch ($action) {
        ...
        case 'deletecategory':
            ...
            echo $renderer->heading(get_string('deletecategory', 'agpu', $category->get_formatted_name()));

            if ($data = $mform->get_data()) {
                // The form has been submit handle it.
                if ($data->fulldelete == 1 && $category->can_delete_full()) {
                    $continueurl = new agpu_url('/course/management.php');
                    ...
                    $continueurl = new agpu_url('/course/management.php', array('categoryid' => $data->newparent));
            ...
            if ($bulkmovecourses) {
                ...
                try {
                    ...
                    if ($redirectback) {
                        ...
                        $redirectmessage = get_string('bulkmovecoursessuccess', 'agpu', $a);
                    }
                ...
                if ($movecount > 1) {
                    ...
                    $notificationspass[] = get_string($movesuccessstrkey, 'agpu', $a);
                } else if ($movecount === 1) {
                    $a = new stdClass;
                ...
                if ($for === 'thiscategory') {
                    ...
                    $categorieslist = core_course_category::make_categories_list('agpu/category:manage');
                ...
                if ($category === null && count($categoryids) === 1) {
                    ...
                    redirect(new agpu_url($PAGE->url, array('categoryid' => reset($categoryids))));
                }
            }
    }
    if ($redirectback) {
