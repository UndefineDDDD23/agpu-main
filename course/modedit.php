...
$url = new agpu_url('/course/modedit.php');
...
if (!empty($add)) {
    ...
    $pageheading = $pagetitle = get_string('addinganew', 'agpu', $fullmodulename);
    ...
    navigation_node::override_active_url(new agpu_url('/course/modedit.php', array('update'=>$update, 'return'=>1)));
    ...
    $sectionname = get_section_name($course, $cw);
    $fullmodulename = get_string('modulename', $module->name);
    $pageheading = get_string('editsettings', 'agpu');
    $pagetitle = get_string('edita', 'agpu', $fullmodulename) . ': ' . $cm->name;
    $navbaraddition = null;

} else {
    require_login();
    throw new \agpu_exception('invalidaction');
}
...
$modagpuform = "$CFG->dirroot/mod/$module->name/mod_form.php";
if (file_exists($modagpuform)) {
    require_once($modagpuform);
} else {
    throw new \agpu_exception('noformdesc');
}
...
if ($mform->is_cancelled()) {
    if ($return && !empty($cm->id)) {
        ...
        $activityurl = new agpu_url("/mod/$module->name/view.php", $urlparams);
    ...
    if (!empty($fromform->update)) {
        ...
        throw new \agpu_exception('invaliddata');
    }

    if (isset($fromform->submitbutton)) {
        $url = new agpu_url("/mod/$module->name/view.php", array('id' => $fromform->coursemodule, 'forceview' => 1));
    ...
    if (isset($fromform->needsfrontendregrade)) {
        $url = new agpu_url('/course/modregrade.php', ['id' => $fromform->coursemodule,
    ...
    if ($course->id !== $SITE->id) {
        $pagetitle = $pagetitle . agpu_page::TITLE_SEPARATOR . $course->shortname;
    }
