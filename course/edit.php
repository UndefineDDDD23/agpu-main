...
if ($returnto === 'url' && confirm_sesskey() && $returnurl) {
    ...
    if (!empty($id)) {
        $returnurl = new agpu_url($CFG->wwwroot . '/course/view.php', array('id' => $id));
    } else {
        $returnurl = new agpu_url($CFG->wwwroot . '/course/');
    }
    if ($returnto !== 0) {
        switch ($returnto) {
            case 'category':
                $returnurl = new agpu_url($CFG->wwwroot . '/course/index.php', array('categoryid' => $categoryid));
                break;
            case 'catmanage':
                $returnurl = new agpu_url($CFG->wwwroot . '/course/management.php', array('categoryid' => $categoryid));
                break;
            case 'topcatmanage':
                $returnurl = new agpu_url($CFG->wwwroot . '/course/management.php');
                break;
            case 'topcat':
                $returnurl = new agpu_url($CFG->wwwroot . '/course/');
                break;
            case 'pending':
                $returnurl = new agpu_url($CFG->wwwroot . '/course/pending.php');
                break;
        }
    }
}
...
if ($id) {
    ...
    if ($id == SITEID){
        ...
        throw new \agpu_exception('cannoteditsiteform');
    }
    ...
    require_capability('agpu/course:create', $catcontext);
...
if ($editform->is_cancelled()) {
    ...
    if (empty($course->id)) {
        ...
        if (is_siteadmin($USER->id)) {
            ...
            $enroluser = !is_viewing($context, null, 'agpu/role:assign');
        }

        if (!empty($CFG->creatornewroleid) and $enroluser and !is_enrolled($context, null, 'agpu/role:assign')) {
        ...
        $courseurl = new agpu_url('/course/view.php', array('id' => $course->id));
    } else {
...
if (!empty($course->id)) {
    ...
    $managementurl = new agpu_url('/course/management.php');
    // These are the caps required in order to see the management interface.
    $managementcaps = array('agpu/category:manage', 'agpu/course:create');
    ...
    navigation_node::override_active_url(new agpu_url('/course/index.php', ['categoryid' => $category->id]), true);
