...
function build_mnet_logs_array($hostid, $course, $user=0, $date=0, $order="l.time ASC", $limitfrom='', $limitnum='',
    ...
    //if ($course->groupmode == SEPARATEGROUPS and !has_capability('agpu/course:managegroups', context_course::instance($course->id))) {
...
function can_edit_in_category($categoryid = 0) {
    ...
    return has_any_capability(array('agpu/category:manage', 'agpu/course:create'), $context);
}
...
function course_add_cm_to_section($courseorid, $cmid, $sectionnum, $beforemod = null) {
    ...
    if (empty($section->sequence)) {
        $newsequence = "$cmid";
    } else if ($beforemod && ($key = agpu_array_keys_filter($modarray, $beforemod))) {
...
function course_delete_module($cmid, $async = false) {
    ...
    if (file_exists($modlib)) {
        ...
        throw new agpu_exception('cannotdeletemodulemissinglib', '', '', null,
    ...
    if (!function_exists($deleteinstancefunction)) {
        throw new agpu_exception('cannotdeletemodulemissingfunc', '', '', null,
    ...
    if (!$deleteinstancefunction($cm->instance)) {
        throw new agpu_exception('cannotdeletemoduleinstance', '', '', null,
    ...
    if (!delete_mod_from_section($cm->id, $cm->section)) {
        throw new agpu_exception('cannotdeletemodulefromsection', '', '', null,
...
function delete_mod_from_section($modid, $sectionid) {
    ...
    if ($section = $DB->get_record("course_sections", array("id"=>$sectionid)) ) {
        ...
        if ($key = agpu_array_keys_filter($modarray, $modid)) {
...
function course_can_delete_section($course, $section) {
    ...
    if (!has_all_capabilities(array('agpu/course:movesections', 'agpu/course:update'), $context)) {
        return false;
    }
    ...
    if (!empty($modinfo->sections[$section])) {
        foreach ($modinfo->sections[$section] as $cmid) {
            if (!has_capability('agpu/course:manageactivities', context_module::instance($cmid))) {
                return false;
            }
        }
    }
...
function course_get_cm_edit_actions(cm_info $mod, $indent = -1, $sr = null) {
    ...
    $editcaps = array('agpu/course:manageactivities', 'agpu/course:activityvisibility', 'agpu/role:assign');
    $dupecaps = array('agpu/backup:backuptargetimport', 'agpu/restore:restoretargetimport');
    ...
    $hasmanageactivities = has_capability('agpu/course:manageactivities', $modcontext);
    ...
    $baseurl = new agpu_url('/course/mod.php', array('sesskey' => sesskey()));
    ...
    if ($hasmanageactivities) {
        $actions['update'] = new action_menu_link_secondary(
            new agpu_url($baseurl, array('update' => $mod->id)),
            new pix_icon('i/settings', '', 'agpu', ['class' => 'iconsmall']),
            $str->editsettings,
    ...
    if ($hasmanageactivities && $usecomponents) {
        $actions['move'] = new action_menu_link_secondary(
            new agpu_url($baseurl, [
                'sesskey' => sesskey(),
                'copy' => $mod->id,
            ]),
            new pix_icon('i/dragdrop', '', 'agpu', ['class' => 'iconsmall']),
            $str->move,
            [
    ...
    if ($hasmanageactivities && $indent >= 0 && !$hasdelegatesection) {
        ...
        $actions['moveright'] = new action_menu_link_secondary(
            new agpu_url($baseurl, ['id' => $mod->id, 'indent' => '1']),
            new pix_icon($rightarrow, '', 'agpu', ['class' => 'iconsmall']),
            $str->moveright,
            [
        ...
        $actions['moveleft'] = new action_menu_link_secondary(
            new agpu_url($baseurl, ['id' => $mod->id, 'indent' => '-1']),
            new pix_icon($leftarrow, '', 'agpu', ['class' => 'iconsmall']),
            $str->moveleft,
            [
    ...
    if (has_capability('agpu/course:activityvisibility', $modcontext)) {
    ...
    if (has_all_capabilities($dupecaps, $coursecontext) &&
            plugin_supports('mod', $mod->modname, FEATURE_BACKUP_agpu2) &&
            course_allowed_module($mod->get_course(), $mod->modname)) {
        $actions['duplicate'] = new action_menu_link_secondary(
            new agpu_url($baseurl, ['duplicate' => $mod->id]),
            new pix_icon('t/copy', '', 'agpu', array('class' => 'iconsmall')),
            $str->duplicate,
            [
    ...
    // Assign.
    if (has_capability('agpu/role:assign', $modcontext) && !$hasdelegatesection) {
        $actions['assign'] = new action_menu_link_secondary(
            new agpu_url('/admin/roles/assign.php', array('contextid' => $modcontext->id)),
            new pix_icon('t/assignroles', '', 'agpu', array('class' => 'iconsmall')),
            $str->assign,
    ...
    if ($courseformat->show_groupmode($mod) && $usecomponents  && !$mod->coursegroupmodeforce) {
        ...
        $actions['groupmode'] = new action_menu_subpanel(
            ...
            new pix_icon('t/groupv', '', 'agpu', ['class' => 'iconsmall'])
        );
    }
    ...
    if ($hasmanageactivities) {
        $actions['delete'] = new action_menu_link_secondary(
            new agpu_url($baseurl, ['delete' => $mod->id]),
            new pix_icon('t/delete', '', 'agpu', ['class' => 'iconsmall']),
            $str->delete,
            [
...
function course_get_cm_move(cm_info $mod, $sr = null) {
    ...
    if ($hasmanageactivities) {
        ...
        return html_writer::link(
            new agpu_url($baseurl, ['copy' => $mod->id]),
            $OUTPUT->pix_icon($pixicon, '', 'agpu', ['class' => 'iconsmall']),
            $attributes
        );
    }
    return '';
 ...
 * the course format supports agpu course ajax features.
 *
 * @param string $format
 * @return stdClass
 */
...
function can_delete_course($courseid) {
    ...
    if (has_capability('agpu/course:delete', $context)) {
        return true;
    }

    // hack: now try to find out if creator created this course recently (1 day)
    if (!has_capability('agpu/course:create', $context)) {
        return false;
    }
...
function create_course($data, $editoroptions = NULL) {
    ...
    if (!empty($data->shortname)) {
        if ($DB->record_exists('course', array('shortname' => $data->shortname))) {
            throw new agpu_exception('shortnametaken', '', '', $data->shortname);
        }
    }
    ...
    if (!empty($data->idnumber)) {
        if ($DB->record_exists('course', array('idnumber' => $data->idnumber))) {
            throw new agpu_exception('courseidnumbertaken', '', '', $data->idnumber);
        }
    }
    ...
    if ($errorcode = course_validate_dates((array)$data)) {
        throw new agpu_exception($errorcode);
    }
    ...
    $courseconfig = get_config('agpucourse');
...
function update_course($data, $editoroptions = NULL) {
    ...
    if ($data->id == SITEID) {
        throw new agpu_exception('invalidcourse', 'error');
    }
...
class course_request {
    ...
    public static function create($data) {
        ...
        if ($users = get_users_from_config($CFG->courserequestnotify, 'agpu/site:approvecourse')) {
    ...
    public function __construct($properties) {
        ...
        if (empty($properties->requester)) {
            if (!($this->properties = $DB->get_record('course_request', array('id' => $properties->id)))) {
                throw new \agpu_exception('unknowncourserequest');
            }
        } else {
     ...
     * misconfigured), we check capabilities 'agpu/site:approvecourse' and 'agpu/course:changecategory'.
     *
     * @return bool
     */
    public function can_approve() {
        ...
        if ($category) {
            return has_capability('agpu/site:approvecourse', $category->get_context());
        }
        ...
        return has_all_capabilities(['agpu/site:approvecourse', 'agpu/course:changecategory'], context_system::instance());
    }
     ...
     * hidden categories if he has capabilities 'agpu/site:approvecourse' and
     * 'agpu/course:changecategory'
     *
    ...
    public function approve() {
        ...
        if (!empty($CFG->creatornewroleid) and !is_viewing($context, $user, 'agpu/role:assign') and !is_enrolled($context, $user, 'agpu/role:assign')) {
        ...
        $this->notify($user, $USER, 'courserequestapproved', get_string('courseapprovedsubject'), get_string('courseapprovedemail2', 'agpu', $a), $course->id);
    ...
    public function reject($notice) {
        ...
        $this->notify($user, $USER, 'courserequestrejected', get_string('courserejectsubject'), get_string('courserejectemail', 'agpu', $notice));
        $this->delete();
    }
...
function include_course_ajax($course, $usedmodules = array(), $enabledmodules = null, $config = null) {
    ...
    if (!$format->supports_components() && course_format_uses_sections($course->format)) {
        ...
        $PAGE->requires->yui_module('agpu-course-dragdrop', 'M.course.init_section_dragdrop',
            array(array(
                'courseid' => $course->id,
        ...
        $PAGE->requires->yui_module('agpu-course-dragdrop', 'M.course.init_resource_dragdrop',
            array(array(
                'courseid' => $course->id,
 ...
 * @return agpu_url The url of course
 */
 ...
 * @throws agpu_exception if user is not allowed to perform the action or module is not allowed in this course
 */
function create_module($moduleinfo) {
    ...
    foreach($mandatoryfields as $mandatoryfield) {
        if (!isset($moduleinfo->{$mandatoryfield})) {
            throw new agpu_exception('createmodulemissingattribut', '', '', $mandatoryfield);
        }
    }
 ...
 * @throws agpu_exception if current user is not allowed to update the module
 */
 ...
 * @param int $sr The section to link back to (used for creating the links)
 * @throws agpu_exception if the plugin doesn't support duplication
...
function duplicate_module($course, $cm, ?int $sectionid = null, bool $changename = true): ?cm_info {
    ...
    if (!plugin_supports('mod', $cm->modname, FEATURE_BACKUP_agpu2)) {
        throw new agpu_exception('duplicatenosupport', 'error', '', $a);
    }

    // Backup the activity.

    $bc = new backup_controller(backup::TYPE_1ACTIVITY, $cm->id, backup::FORMAT_agpu,
    ...
    if ($newcmid) {
        ...
        if ($changename) {
            ...
            $newname = get_string('duplicatedmodule', 'agpu', $newcm->name);
...
function course_get_user_navigation_options($context, $course = null) {
    ...
    $options->blogs = !empty($CFG->enableblogs) &&
                        ...
                        && has_capability('agpu/blog:view', $sitecontext);

    $options->notes = !empty($CFG->enablenotes) && has_any_capability(array('agpu/notes:manage', 'agpu/notes:view'), $context);
    ...
    if ($isfrontpage) {
        ...
        $options->badges = !empty($CFG->enablebadges) && has_capability('agpu/badges:viewbadges', $sitecontext);
        $options->tags = !empty($CFG->usetags) && $isloggedin;
        $options->search = !empty($CFG->enableglobalsearch) && has_capability('agpu/search:query', $sitecontext);
    } else {
        ...
        if (!empty($CFG->enablebadges) && !empty($CFG->badges_allowcoursebadges)) {
            ...
            if (!$canmanage) {
                ...
                $canview = has_capability('agpu/badges:viewbadges', $context);
                if ($canview) {
        ...
        if (has_capability('agpu/grade:viewall', $context)) {
            $grades = true;
    ...
    if (\core_communication\api::is_available()) {
        $options->communication = has_capability('agpu/course:configurecoursecommunication', $context);
    }

    if (\core_competency\api::is_enabled()) {
        $capabilities = array('agpu/competency:coursecompetencyview', 'agpu/competency:coursecompetencymanage');
...
function course_get_user_administration_options($course, $context) {
    ...
    $options->update = has_capability('agpu/course:update', $context);
    $options->editcompletion = $CFG->enablecompletion && $course->enablecompletion &&
        ($options->update || $hascompletionoptions);
    $options->filters = has_capability('agpu/filter:manage', $context) &&
                        count(filter_get_available_in_context($context)) > 0;
    $options->reports = has_capability('agpu/site:viewreports', $context);
    $options->backup = has_capability('agpu/backup:backupcourse', $context);
    $options->restore = has_capability('agpu/restore:restorecourse', $context);
    $options->copy = \core_course\management\helper::can_copy_course($course->id);
    $options->files = ($course->legacyfiles == 2 && has_capability('agpu/course:managefiles', $context));

    if (!$isfrontpage) {
        $options->tags = has_capability('agpu/course:tag', $context);
        $options->gradebook = has_capability('agpu/grade:manage', $context);
        $options->outcomes = !empty($CFG->enableoutcomes) && has_capability('agpu/course:update', $context);
        $options->badges = !empty($CFG->enablebadges);
        $options->import = has_capability('agpu/restore:restoretargetimport', $context);
        $options->reset = has_capability('agpu/course:reset', $context);
        $options->roles = has_capability('agpu/role:switchroles', $context);
    } else {
...
function course_filter_courses_by_timeline_classification(
    ...
    if (!in_array($classification,
        ...
        throw new agpu_exception($message);
    }
...
function course_can_view_participants($context) {
    $viewparticipantscap = 'agpu/course:viewparticipants';
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        $viewparticipantscap = 'agpu/site:viewparticipants';
    }

    return has_any_capability([$viewparticipantscap, 'agpu/course:enrolreview'], $context);
}
...
function can_download_from_backup_filearea($filearea, \context $context, ?stdClass $user = null) {
    ...
    switch ($filearea) {
        case 'course':
        case 'backup':
            $candownload = has_capability('agpu/backup:downloadfile', $context, $user);
            break;
        case 'automated':
            ...
            $candownload = has_capability('agpu/backup:downloadfile', $context, $user) &&
                           has_capability('agpu/restore:userinfo', $context, $user);
            break;
        default:
            break;
...
function course_get_recent_courses(?int $userid = null, int $limit = 0, int $offset = 0, ?string $sort = null) {
    ...
    if ($userid != $USER->id && isset($hiddenfields['lastaccess'])) {
        $recentcourses = array_filter($recentcourses, function($course) {
            ...
            // fields.
            return has_capability('agpu/course:viewhiddenuserfields', $context);
        });
    }
...
function course_get_communication_instance_data(int $courseid): array {
    ...
    $context = context_course::instance($course->id);
    require_capability('agpu/course:configurecoursecommunication', $context);

    $heading = $course->fullname;
    $returnurl = new agpu_url('/course/view.php', ['id' => $courseid]);
