...
class category_action_bar extends manage_categories_action_bar {
     ...
     */
    public function __construct(agpu_page $page, object $category, ?object $course = null, ?string $searchvalue = null) {
    ...
    protected function get_category_select(\renderer_base $output): ?object {
        if (!$this->searchvalue && !core_course_category::is_simple_site()) {
            ...
            if (count($categories) > 1) {
                ...
                $currenturl = new agpu_url($this->page->url, ['categoryid' => $this->category->id]);
    ...
    protected function get_additional_category_options(): array {
        ...
        if ($this->category->is_uservisible()) {
            ...
            if (has_capability('agpu/course:create', $context)) {
                ...
                $options[0] = [
                    'url' => new agpu_url('/course/edit.php', $params),
            ...
            if (!empty($CFG->enablecourserequests)) {
                ...
                if (course_request::can_request($context)) {
                    ...
                    $options[3] = [
                        'url' => new agpu_url('/course/request.php', $params),
                ...
                if (has_capability('agpu/site:approvecourse', $context)) {
                    ...
                    if (!$disabled) {
                        $options[4] = [
                            'url' => new agpu_url('/course/pending.php'),
        ...
        if ($this->category->can_create_course() || $this->category->has_manage_capability()) {
            ...
            $options[2] = [
                'url' => new agpu_url('/course/management.php', ['categoryid' => $this->category->id]),
                'string' => get_string('managecourses')
            ];

            if ($this->category->has_manage_capability()) {
                $addsubcaturl = new agpu_url('/course/editcategory.php', array('parent' => $this->category->id));
                $options[1] = [
                    'url' => $addsubcaturl,
