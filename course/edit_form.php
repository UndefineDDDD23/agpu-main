 ...
 */
class course_edit_form extends agpuform {
    protected $course;
    protected $context;
    ...
    function definition() {
        ...
        if (!empty($course->id) and !has_capability('agpu/course:changefullname', $coursecontext)) {
        ...
        if (!empty($course->id) and !has_capability('agpu/course:changeshortname', $coursecontext)) {
        ...
        if (empty($course->id)) {
            if (has_capability('agpu/course:create', $categorycontext)) {
                $displaylist = core_course_category::make_categories_list('agpu/course:create');
            ...
            if (has_capability('agpu/course:changecategory', $coursecontext)) {
                $displaylist = core_course_category::make_categories_list('agpu/course:changecategory');
        ...
        if (!empty($course->id)) {
            if (!has_capability('agpu/course:visibility', $coursecontext)) {
                $mform->hardFreeze('visible');
                $mform->setConstant('visible', $course->visible);
            }
        } else {
            if (!guess_if_creator_will_have_course_capability('agpu/course:visibility', $categorycontext)) {
                $mform->hardFreeze('visible');
        ...
        if ($CFG->downloadcoursecontentallowed) {
            ...
            if ((!empty($course->id) && !has_capability('agpu/course:configuredownloadcontent', $coursecontext)) ||
                    (empty($course->id) &&
                    !guess_if_creator_will_have_course_capability('agpu/course:configuredownloadcontent', $categorycontext))) {
        ...
        if (!empty($course->id) and !has_capability('agpu/course:changeidnumber', $coursecontext)) {
        ...
        if (!empty($course->id) and !has_capability('agpu/course:changesummary', $coursecontext)) {
        ...
        if (isset($course->format)) {
            ...
            if (!in_array($course->format, $courseformats)) {
                ...
                $formcourseformats->add_option(
                    $course->format,
                    get_string('withdisablednote', 'agpu', get_string('pluginname', 'format_'.$course->format)),
                );
            }
        }
        ...
        if ((empty($course->id) && guess_if_creator_will_have_course_capability('agpu/course:setforcedlanguage', $categorycontext))
                || (!empty($course->id) && has_capability('agpu/course:setforcedlanguage', $coursecontext))) {
        ...
        if (core_tag_tag::is_enabled('core', 'course') &&
                ((empty($course->id) && guess_if_creator_will_have_course_capability('agpu/course:tag', $categorycontext))
                || (!empty($course->id) && has_capability('agpu/course:tag', $coursecontext)))) {
