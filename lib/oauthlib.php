 ...
 * 3. User approved the request, and get back to agpu
...
class oauth_helper {
    ...
    /** @var agpu_url oauth callback URL. */
    ...
    function __construct($args) {
        ...
        if (!empty($args['oauth_callback'])) {
            $this->oauth_callback = new agpu_url($args['oauth_callback']);
        }
    ...
    public function request_token() {
        ...
        if (empty($result['oauth_token'])) {
            throw new agpu_exception('oauth1requesttoken', 'core_error', '', null, $content);
        }
    ...
    public function get_access_token($token, $secret, $verifier='') {
        ...
        if (empty($keys['oauth_token']) || empty($keys['oauth_token_secret'])) {
            throw new agpu_exception('oauth1accesstoken', 'core_error', '', null, $content);
        }
...
abstract class oauth2_client extends curl {
    ...
    /** @var agpu_url $returnurl URL to return to after authenticating */
     ...
     * @param string $scope
     */
    public function __construct($clientid, $clientsecret, agpu_url $returnurl, $scope) {
        parent::__construct();
     ...
     *
     * @return agpu_url url of callback
     */
    public static function callback_url() {
        global $CFG;

        return new agpu_url('/admin/oauth2callback.php');
    }
    ...
    public function get_login_url() {
        ...
        return new agpu_url($this->auth_url(), $params);
    }
    ...
    public function upgrade_token($code) {
        ...
        if ($this->info['http_code'] !== 200) {
            ...
            throw new agpu_exception('oauth2upgradetokenerror', 'core_error', '', $this->info['http_code'], $debuginfo);
        }

        $r = json_decode($response);

        if (is_null($r)) {
            throw new agpu_exception("Could not decode JSON token response");
        }

        if (!empty($r->error)) {
            throw new agpu_exception($r->error . ' ' . $r->error_description);
        }
