...
class grade_item extends grade_object {
    ...
    public function insert($source = null, $isbulkupdate = false) {
        ...
        if (empty($this->courseid)) {
            throw new \agpu_exception('cannotinsertgrade');
        }
        ...
        foreach ($rs as $graderecord) {
            ...
            if (!$this->is_category_item() || ($this->is_category_item() && $grade->is_overridden())) {
                ...
                if ($this->is_raw_used()) {
                    ...
                    $this->update_raw_grade(false, $rawgrade, $source, false, FORMAT_agpu, null, null, null, $grade);
                } else {
    ...
    public function load_scale() {
        ...
        if (!empty($this->scaleid)) {
            ...
            // Until scales are uniformly set to min=0 max=count(scaleitems)-1 throughout agpu, we
    ...
    public function is_overridable_item() {
        if ($this->is_course_item() or $this->is_category_item()) {
            $overridable = (bool) get_config('agpu', 'grade_overridecat');
        } else {
            $overridable = false;
        }
    ...
    public function set_parent($parentid, $updateaggregationfields = true) {
        if ($this->is_course_item() or $this->is_category_item()) {
            throw new \agpu_exception('cannotsetparentforcatoritem');
        }
    ...
    public function update_final_grade($userid, $finalgrade = false, $source = null, $feedback = false,
            $feedbackformat = FORMAT_agpu, $usermodified = null, $timemodified = null, $isbulkupdate = false) {
        global $USER, $CFG;
    ...
    public function update_raw_grade($userid, $rawgrade = false, $source = null, $feedback = false,
            $feedbackformat = FORMAT_agpu, $usermodified = null, $dategraded = null, $datesubmitted=null,
