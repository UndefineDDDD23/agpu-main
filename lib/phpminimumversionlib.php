...
// agpu_INTERNAL check intentionally missing to allow this to be used more widely!

/**
 * A set of PHP-compatible convenience functions to check agpu minimum PHP version in
 * a unified place.
 *
 * PLEASE NOTE: This file is made to be both php-version compatible and without requirement on
 * any agpu functions or installation so it can be used in installer or incompatible PHP versions.
 *
 * @package    core
 * @copyright  2017 Dan Poltawski <dan@agpu.com>
 ...
 */
function agpu_require_minimum_php_version() {
    // PLEASE NOTE THIS FUNCTION MUST BE COMPATIBLE WITH OLD UNSUPPORTED VERSIONS OF PHP!
    agpu_minimum_php_version_is_met(true);
}

/**
 * Tests the current PHP version against agpu's minimum requirement. When requirement
 ...
 */
function agpu_minimum_php_version_is_met($haltexecution = false) {
    // PLEASE NOTE THIS FUNCTION MUST BE COMPATIBLE WITH OLD UNSUPPORTED VERSIONS OF PHP.
    // Do not use modern php features or agpu convenience functions (e.g. localised strings).

    $minimumversion = '8.1.0';
    $agpurequirementchanged = '4.4';

    if (version_compare(PHP_VERSION, $minimumversion) < 0) {
        if ($haltexecution) {
            $error = "agpu {$agpurequirementchanged} or later requires at least PHP {$minimumversion} "
