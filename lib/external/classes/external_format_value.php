...
class external_format_value extends external_value {
    ...
    public function __construct($textfieldname, $required = VALUE_REQUIRED, $default = null) {
        ...
        if ($default !== null && !in_array($default, [FORMAT_agpu, FORMAT_HTML, FORMAT_PLAIN, FORMAT_MARKDOWN])) {
            debugging("Invalid default format for $textfieldname: $default. " .
                      "It must be either FORMAT_agpu, FORMAT_HTML, FORMAT_PLAIN, or FORMAT_MARKDOWN.", DEBUG_DEVELOPER);
            $default = null;
        }
        ...
        $desc = sprintf(
            "%s format (%s = HTML, %s = agpu, %s = PLAIN, or %s = MARKDOWN)",
            $textfieldname,
            FORMAT_HTML,
            FORMAT_agpu,
            FORMAT_PLAIN,
