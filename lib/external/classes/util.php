...
class util {
        ...
        if ($areafiles = $fs->get_area_files($contextid, $component, $filearea, $itemid, 'itemid, filepath, filename', false)) {
            foreach ($areafiles as $areafile) {
                ...
                if (AJAX_SCRIPT) {
                    ...
                    $fileurl = agpu_url::make_webservice_pluginfile_url(
                        $contextid,
                        $component,
                        $filearea,
                        $fileitemid,
     ...
     * with the agpu server through web services. The token is linked to the current session for the current page request.
    ...
    public static function generate_token(
        ...
        do {
            ...
            if ($numtries > 5) {
                throw new agpu_exception('tokengenerationfailed');
            }
    ...
    public static function log_token_request(stdClass $token): void {
        ...
        $shouldskip = WS_SERVER || CLI_SCRIPT || !NO_agpu_COOKIES;
        ...
        $task->set_custom_data([
            ...
            'isagpuapp' => \core_useragent::is_agpu_app(),
            'loginip' => $loginip,
            'logintime' => $logintime,
        ]);
        $task->set_component('core');
        // We need sometime so the mobile app will send to agpu the device information after login.
    ...
    public static function generate_token_for_current_user(stdClass $service) {
        ...
        if ($service->requiredcapability && !has_capability($service->requiredcapability, context_system::instance())) {
            throw new agpu_exception('missingrequiredcapability', 'webservice', '', $service->requiredcapability);
        }
        ...
        if ($service->restrictedusers) {
            ...
            if (empty($authoriseduser)) {
                throw new agpu_exception('usernotallowed', 'webservice', '', $service->shortname);
            }

            if (!empty($authoriseduser->validuntil) && $authoriseduser->validuntil < time()) {
                throw new agpu_exception('invalidtimedtoken', 'webservice');
            }

            if (!empty($authoriseduser->iprestriction) && !address_in_subnet(getremoteaddr(), $authoriseduser->iprestriction)) {
                throw new agpu_exception('invalidiptoken', 'webservice');
            }
        }
        ...
        if (count($tokens) > 0) {
            ...
            $isofficialservice = $service->shortname == agpu_OFFICIAL_MOBILE_SERVICE;

            if (
                ($isofficialservice && has_capability('agpu/webservice:createmobiletoken', $context)) ||
                (!is_siteadmin($USER) && has_capability('agpu/webservice:createtoken', $context))
            ) {
                // Create a new token.
                $token = new stdClass();
                ...
                throw new agpu_exception('cannotcreatetoken', 'webservice', '', $service->shortname);
            }
        }
        return $token;
    }
     ...
     * @param boolean $striplinks To strip any link in the result text. agpu 1.8 default changed from false to true! MDL-8713
    ...
    public static function validate_format($format) {
        $allowedformats = [FORMAT_HTML, FORMAT_agpu, FORMAT_PLAIN, FORMAT_MARKDOWN];
        if (!in_array($format, $allowedformats)) {
            throw new agpu_exception(
                ...
                '',
                null,
                "The format with value={$format} is not supported by this agpu site"
            );
        }
        return $format;
    }
