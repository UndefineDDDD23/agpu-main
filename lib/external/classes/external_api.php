...
class external_api {
    ...
    public static function call_external_function($function, $args, $ajaxonly = false) {
        ...
        try {
            ...
            if ($externalfunctioninfo->loginrequired && !WS_SERVER) {
                if (defined('NO_agpu_COOKIES') && NO_agpu_COOKIES && !PHPUNIT_TEST) {
                    throw new agpu_exception('servicerequireslogin', 'webservice');
                }
                if (!isloggedin()) {
                    throw new agpu_exception('servicerequireslogin', 'webservice');
                } else {
        ...
        if ($description instanceof external_value) {
            ...
            if (!is_array($params)) {
                throw new invalid_parameter_exception(
                    // phpcs:ignore agpu.PHP.ForbiddenFunctions.Found
    ...
    protected static function get_context_from_params($param) {
        if (!empty($param['contextid'])) {
            ...
            // Numbers and short names are supported since agpu 4.2.
    ...
    protected static function get_context_parameters() {
        ...
        $level = new external_value(
            PARAM_ALPHANUM, // Since agpu 4.2 numeric context level values are supported too.
