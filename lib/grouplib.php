 ...
 *
 * If a user does not have agpu/course:viewhiddengroups, the list of groups and members will be restricted based on the
...
function groups_get_all_groups($courseid, $userid=0, $groupingid=0, $fields='g.*', $withmembers=false, $participationonly = false) {
    ...
    $visibilityfrom = '';
    $visibilitywhere = '';
    $viewhidden = has_capability('agpu/course:viewhiddengroups', context_course::instance($courseid));
    if (!$viewhidden) {
...
function groups_get_my_groups() {
    ...
    $viewhidden = has_capability('agpu/course:viewhiddengroups', context_system::instance());
    $visibilitywhere = '';
    if (!$viewhidden) {
...
function groups_get_user_groups(int $courseid, int $userid = 0, bool $includehidden = false): array {
    ...
    $usergroups = false;
    $viewhidden = $includehidden || has_capability('agpu/course:viewhiddengroups', context_course::instance($courseid));
 ...
 * @category group
 * @param stdClass $course course object
 * @param mixed $urlroot return address. Accepts either a string or a agpu_url
...
function groups_print_course_menu($course, $urlroot, $return=false) {
    ...
    if (count($groupsmenu) == 1) {
        ...
        $select = new single_select(new agpu_url($urlroot), 'group', $groupsmenu, $activegroup, null, 'selectgroup');
 ...
 *
 * @param stdclass          $course  course object.
 * @param string|agpu_url $urlroot return address. Accepts either a string or a agpu_url.
 ...
 * @category group
 * @param stdClass|cm_info $cm course module object
 * @param string|agpu_url $urlroot return address that users get to if they choose an option;
...
function groups_print_activity_menu($cm, $urlroot, $return=false, $hideallparticipants=false) {
    ...
    if ($urlroot instanceof agpu_url) {
...
function groups_get_activity_allowed_groups($cm,$userid=0) {
    ...
    $context = context_module::instance($cm->id);
    if ($groupmode == VISIBLEGROUPS or has_capability('agpu/site:accessallgroups', $context, $userid)) {
...
function groups_group_visible($groupid, $course, $cm = null, $userid = null) {
    ...
    if (has_capability('agpu/site:accessallgroups', $context, $userid)) {
...
function groups_get_groups_members($groupsids, $extrafields=null, $sort='lastname ASC') {
    ...
    if (!has_capability('agpu/course:viewhiddengroups', $context)) {
        list($visibilitywhere, $visibilityparams) = \core_group\visibility::sql_member_visibility_where();
        $params = array_merge($params, $visibilityparams);
