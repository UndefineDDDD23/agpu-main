 ...
 * @since     agpu 2.0
 */
class block_not_on_page_exception extends agpu_exception {
    /**
     * Constructor
...
/**
 * This class keeps track of the block that should appear on a agpu_page.
 *
...
/// Field declarations =========================================================
    ...
    /**
     * the agpu_page we are managing blocks for.
     * @var agpu_page
     */
    protected $page;
...
/// Constructor ================================================================
    ...
    /**
     * Constructor.
     * @param object $page the agpu_page object object we are managing the blocks for,
...
/// Getter methods =============================================================
     ...
     * @return agpu_url URL for moving block $this->movingblock to this position.
     */
    protected function get_move_target_url($region, $weight) {
        return new agpu_url($this->page->url, array('bui_moveid' => $this->movingblock,
...
/// Inner workings =============================================================
    ...
    public function ensure_content_created($region, $output) {
        ...
        if (!has_capability('agpu/block:view', $this->page->context) ) {
...
/// Process actions from the URL ===============================================
    ...
    public function edit_controls($block) {
        ...
        if ($this->page->user_can_edit_blocks()) {
            ...
            $controls[] = new action_menu_link_primary(
                new agpu_url($actionurl, array('bui_moveid' => $block->instance->id)),
                new pix_icon('t/move', $str, 'agpu', array('class' => 'iconsmall', 'title' => '')),
                $str,
        ...
        if ($this->page->user_can_edit_blocks() || $block->user_can_edit()) {
            ...
            $editactionurl = new agpu_url($actionurl, ['bui_editid' => $block->instance->id]);
            ...
            if ($editactionurl->compare(new agpu_url('/admin/index.php'), URL_MATCH_BASE)) {
                $editactionurl->param('cache', 1);
            }

            $controls[] = new action_menu_link_secondary(
                $editactionurl,
                new pix_icon('i/settings', $str, 'agpu', ['class' => 'iconsmall', 'title' => '']),
                $str,
                [
                    'class' => 'editing_edit',
        ...
        if ($this->page->user_can_edit_blocks() && $block->instance_can_be_hidden()) {
            ...
            if ($block->instance->visible) {
                ...
                $url = new agpu_url($actionurl, array('bui_hideid' => $block->instance->id));
                $icon = new pix_icon('t/hide', $str, 'agpu', array('class' => 'iconsmall', 'title' => ''));
                $attributes = array('class' => 'editing_hide');
            } else {
                $str = new lang_string('showblock', 'block', $blocktitle);
                $url = new agpu_url($actionurl, array('bui_showid' => $block->instance->id));
                $icon = new pix_icon('t/show', $str, 'agpu', array('class' => 'iconsmall', 'title' => ''));
        ...
        if (get_assignable_roles($block->context, ROLENAME_SHORT)) {
            $rolesurl = new agpu_url('/admin/roles/assign.php', array('contextid' => $block->context->id,
                'returnurl' => $this->page->url->out_as_local_url()));
            $str = new lang_string('assignrolesinblock', 'block', $blocktitle);
            $controls[] = new action_menu_link_secondary(
                $rolesurl,
                new pix_icon('i/assignroles', $str, 'agpu', array('class' => 'iconsmall', 'title' => '')),
                $str, array('class' => 'editing_assignroles')
            );
        }

        // Permissions.
        if (has_capability('agpu/role:review', $block->context) or get_overridable_roles($block->context)) {
            $rolesurl = new agpu_url('/admin/roles/permissions.php', array('contextid' => $block->context->id,
                'returnurl' => $this->page->url->out_as_local_url()));
            $str = get_string('permissions', 'role');
            $controls[] = new action_menu_link_secondary(
                $rolesurl,
                new pix_icon('i/permissions', $str, 'agpu', array('class' => 'iconsmall', 'title' => '')),
                $str, array('class' => 'editing_permissions')
            );
        }

        // Change permissions.
        if (has_any_capability(array('agpu/role:safeoverride', 'agpu/role:override', 'agpu/role:assign'), $block->context)) {
            $rolesurl = new agpu_url('/admin/roles/check.php', array('contextid' => $block->context->id,
                'returnurl' => $this->page->url->out_as_local_url()));
            $str = get_string('checkpermissions', 'role');
            $controls[] = new action_menu_link_secondary(
                $rolesurl,
                new pix_icon('i/checkpermissions', $str, 'agpu', array('class' => 'iconsmall', 'title' => '')),
        ...
        if ($this->user_can_delete_block($block)) {
            ...
            $deleteactionurl = new agpu_url($actionurl, ['bui_deleteid' => $block->instance->id]);
            ...
            if ($deleteactionurl->compare(new agpu_url('/admin/index.php'), URL_MATCH_BASE)) {
                $deleteactionurl->param('cache', 1);
            }

            $deleteconfirmationurl = new agpu_url($actionurl, [
            ...
            $controls[] = new action_menu_link_secondary(
                $deleteactionurl,
                new pix_icon('t/delete', $str, 'agpu', array('class' => 'iconsmall', 'title' => '')),
                $str,
                [
        ...
        if (!empty($CFG->contextlocking) && has_capability('agpu/site:managecontextlocks', $block->context)) {
            ...
            if (empty($parentcontext) || empty($parentcontext->locked)) {
                ...
                $controls[] = new action_menu_link_secondary(
                    ...
                    new pix_icon($lockicon, $lockstring, 'agpu', array('class' => 'iconsmall', 'title' => '')),
                    $lockstring,
                    ['class' => 'editing_lock']
                );
            }
    ...
    public function process_url_add() {
        ...
        if (!$this->page->user_can_edit_blocks()) {
            throw new agpu_exception('nopermissions', '', $this->page->url->out(), get_string('addblock'));
        }
        ...
        if ($blocktype === '') {
            ...
            if (!$addableblocks) {
                ...
                $url = new agpu_url($addpage->url, array('sesskey' => sesskey()));
        ...
        if (!array_key_exists($blocktype, $addableblocks)) {
            throw new agpu_exception('cannotaddthisblocktype', '', $this->page->url->out(), $blocktype);
        }
    ...
    public function process_url_delete() {
        ...
        if (!$this->user_can_delete_block($block)) {
            throw new agpu_exception('nopermissions', '', $this->page->url->out(), get_string('deleteablock'));
        }

        if (!$confirmdelete) {
            $deletepage = new agpu_page();
            ...
            $PAGE = $deletepage;
            //some functions like agpuQuickForm::addHelpButton use $OUTPUT so we need to replace that too
            /** @var core_renderer $output */
            ...
            $PAGE->set_heading($site->fullname);
            echo $OUTPUT->header();
            $confirmurl = new agpu_url($deletepage->url, array('sesskey' => sesskey(), 'bui_deleteid' => $block->instance->id, 'bui_confirm' => 1));
            $cancelurl = new agpu_url($deletepage->url);
    ...
    public static function get_block_edit_form_class(string $blockname): string {
        global $CFG;
        require_once("$CFG->dirroot/blocks/agpublock.class.php");
    ...
    public function process_url_show_hide() {
        ...
        if (!$this->page->user_can_edit_blocks()) {
            throw new agpu_exception('nopermissions', '', $this->page->url->out(), get_string('hideshowblocks'));
    ...
    public function process_url_edit() {
        ...
        if (!$block->user_can_edit() && !$this->page->user_can_edit_blocks()) {
            throw new agpu_exception('nopermissions', '', $this->page->url->out(), get_string('editblock'));
        }

        $editpage = new agpu_page();
        ...
        // overwrite global $PAGE ready for this. (Formslib refers to it.)
        $PAGE = $editpage;
        //some functions like agpuQuickForm::addHelpButton use $OUTPUT so we need to replace that to
        ...
        if ($mform->is_cancelled()) {
            ...
            $strheading = get_string('blockconfiga', 'agpu', $block->get_title());
            ...
            if ($bits[0] == 'tag' && !empty($this->page->subpage)) {
                ...
                $editpage->navbar->add(get_string('tags'), new agpu_url('/tag/'));
    ...
    public function save_block_data(block_base $block, stdClass $data): void {
        ...
        if (has_capability('agpu/site:manageblocks', $parentcontext)) { // Check permissions in destination.
    ...
    public function process_url_move() {
        ...
        if (!$this->is_known_region($newregion)) {
            throw new agpu_exception('unknownblockregion', '', $this->page->url, $newregion);
        }
 ...
 * @param agpu_page $page the page this block is appearing on.
...
/**
 * Generates the page type list for the my agpu page
 *
 * @param string $pagetype
 * @param stdClass $parentcontext
 * @param stdClass $currentcontext
...
function block_add_block_ui($page, $output) {
    ...
    $actionurl = new agpu_url($page->url, array('sesskey'=>sesskey()));
 ...
 * @param agpu_page $page the back to set the visibility with respect to.
