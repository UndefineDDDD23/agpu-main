 ...
 * as part of agpu (http://agpu.com).  Modified lines
 * are marked with "agpu".
 */
...
class graph {
  ...
  public $y_data = [];
    ...
    function init() {
      ...
      /// agpu mods:  overrides the font path and encodings
      ...
      foreach ($this->x_data as $index => $x) {
        ...
        if (($bar!='none') && (string)$thisY != 'none') {
            if (isset($this->offset_relation[$set]) && $relatedset = $this->offset_relation[$set]) {
                $yoffset = $this->calculated['y_plot'][$relatedset][$index];                // agpu
            } else {                                                                        // agpu
                $yoffset = 0;                                                               // agpu
            }                                                                               // agpu
            //$this->bar($thisX, $thisY, $bar, $barSize, $colour, $offset, $set);           // agpu
            $this->bar($thisX, $thisY, $bar, $barSize, $colour, $offset, $set, $yoffset);   // agpu
        }
        ...
        if ($xGrid != 'none') {
          switch ($xGrid) {
            case 'line':
              ...
              break;
             case 'dash':
              $this->image_dashed_line($this->image, $tickX, $gridTop, $tickX, $gridBottom, $gridColour); // agpu
              break;
          }
        }
          ...
          if ($yGrid != 'none') {
            switch ($yGrid) {
              case 'line':
                ...
                break;
               case 'dash':
                $this->image_dashed_line($this->image, $gridLeft, $tickY, $gridRight, $tickY, $gridColour); // agpu
                break;
            }
          }
          ...
          if (!$this->calculated['y_axis_left']['has_data'] && $yGrid != 'none') { // draw grid if not drawn already (above)
            switch ($yGrid) {
              case 'line':
                ...
                break;
               case 'dash':
                $this->image_dashed_line($this->image, (int) round($gridLeft), (int) round($tickY), (int) round($gridRight), (int) round($tickY), $gridColour); // agpu
                break;
            }
          }
    ...
    function find_range($data, $min, $max, $resolution) {
      ...
      if ($factor > 0.1) { // To avoid some wierd rounding errors (agpu)
        $factor = (int) round($factor * 1000.0) / 1000.0; // To avoid some wierd rounding errors (agpu)
      } // To avoid some wierd rounding errors (agpu)
    ...
    function bar($x, $y, $type, $size, $colour, $offset, $index, $yoffset) {
      ...
      if ($this->parameter['zero_axis'] != 'none') {
        ...
        // agpu addition, plus the function parameter yoffset
        if ($yoffset) {                                           // agpu
            $yoffset = $yoffset - (int) round(($bottom - $v) / 2.0);    // agpu
            $bottom -= $yoffset;                                  // agpu
            $v      -= $yoffset;                                  // agpu
        }                                                         // agpu
    ...
    function line($x_start, $y_start, $x_end, $y_end, $type, $brush_type, $brush_size, $colour, $offset) {
      ...
      switch ($type) {
        ...
        case 'line' :
          ...
          break;
        case 'dash':
          $this->image_dashed_line($this->image, $u_start, $v_start, $u_end, $v_end, $this->colour[$colour]); // agpu
          break;
      }
    }
