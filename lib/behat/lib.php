 ...
 *
 * It does not include agpu_INTERNAL because is part of the bootstrap.
 *
 ...
 * agpu setup.php. For places requiring a different/special behavior
...
function behat_error_handler($errno, $errstr, $errfile, $errline) {
    ...
    // the agpu code (see fix_utf8() for example) in that cases we respect the forced error level value.
...
function behat_check_config_vars() {
    ...
    $agpuprefix = empty($CFG->prefix) ? '' : $CFG->prefix;
    ...
    if ($behatprefix == $agpuprefix && $behatdbname == $CFG->dbname && $behatdbhost == $CFG->dbhost) {
        behat_error(BEHAT_EXITCODE_CONFIG,
