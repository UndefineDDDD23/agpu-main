...
class behat_field_manager {
    ...
    public static function get_form_field_from_label($label, RawMinkContext $context) {
        // There are agpu form elements that are not directly related with
     ...
     *
     * Not all the fields are part of a agpu form, in this
    ...
    public static function get_form_field(NodeElement $fieldnode, Session $session) {
        ...
        // Get the field type if is part of a agpuform.
        if (self::is_agpuform_field($fieldnode)) {
            $type = self::get_field_node_type($fieldnode, $session);
        }

        // If is not a agpuforms field use the base field type.
        if (empty($type)) {
            $type = 'field';
        }
    ...
    public static function get_field_instance($type, NodeElement $fieldnode, Session $session) {
        ...
        // If the field is not part of a agpuform, we should still try to find out
     ...
     * agpuform-specific elements have been detected.
     *
     * @param NodeElement $fieldnode
    ...
    /**
     * Detects when the field is a agpuform field type.
     *
     * Note that there are fields inside agpuforms that are not
     * agpuform element; this method can not detect this, this will
     ...
     * @return bool
     */
    protected static function is_agpuform_field(NodeElement $fieldnode) {

        // We already waited when getting the NodeElement and we don't want an exception if it's not part of a agpuform.
    ...
    protected static function get_field_type(\DOMDocument $document, \DOMElement $node, Session $session): ?string {
        ...
        if ($class = $parentnode->getAttribute('class')) {
            ...
            // Stop propagation through the DOM, if it does not have a felement is not part of a agpu form.
