...
class behat_partial_named_selector extends \Behat\Mink\Selector\PartialNamedSelector {
    ...
    public function __construct() {
        ...
        foreach (static::$agpuselectors as $name => $xpath) {
     ...
     * will be replaced by the locator. These are agpu's own xpaths.
     *
     * @var array XPaths for agpu elements.
     */
    protected static $agpuselectors = array(
        'activity' => <<<XPATH
...
XPATH
        , 'dialogue' => <<<XPATH
.//div[contains(concat(' ', normalize-space(@class), ' '), ' agpu-dialogue ') and
    not(contains(concat(' ', normalize-space(@class), ' '), ' agpu-dialogue-hidden ')) and
    normalize-space(descendant::div[
        contains(concat(' ', normalize-space(@class), ' '), ' agpu-dialogue-hd ')
        ]) = %locator%] |
...
XPATH
    ...
    protected static $customreplacements = [
        ...
        '%ariaLabelMatch%' => [
            'agpu' => 'contains(./@aria-label, %locator%)',
        ],
        '%exactTagTextMatch%' => [
            ...
            'agpu' => 'normalize-space(text())=%locator%',
        ],
    ];
