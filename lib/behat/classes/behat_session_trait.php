...
EOF;
     ...
     * @see agpu\BehatExtension\Tester\agpuStepTester
     */
    public function look_for_exceptions() {
        ...
        try {
            ...
            if ($errormsg = $this->getSession()->getPage()->find('xpath', $exceptionsxpath)) {
                ...
                if (empty($errorinfoboxes)) {
                    $errorinfoboxes = $this->getSession()->getPage()->findAll('css', 'div.agpu-exception-message');
                ...
                $msg = "agpu exception: " . $errormsg->getText() . "\n" . $errorinfo;
    ...
    protected function get_session_user() {
        ...
        $sid = $this->getSession()->getCookie('agpuSession');
        if (empty($sid)) {
            throw new coding_exception('failed to get agpu session');
        }
    ...
    /**
     * Gets the internal agpu context id from the context reference.
     *
    ...
    protected function js_trigger_click($node) {
        ...
        if ($driver instanceof \agpu\BehatExtension\Driver\WebDriver) {
     ...
     */
    protected function resolve_page_url(string $page): agpu_url {
