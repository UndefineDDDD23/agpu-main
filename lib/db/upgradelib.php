 ...
 * These functions must use SQL and database related functions only- no other agpu API,
...
/**
 * Upgrade core licenses shipped with agpu.
 */
...
function upgrade_calendar_site_status(bool $output = true): bool {
    ...
    if ($output) {
        ...
        if ($needsfix) {
            ...
            mtrace('');
            mtrace("You can use this CLI tool or upgrade to a version of agpu that includes");
...
function upgrade_calendar_events_get_teacherid(int $courseid): int {
    ...
    if ($context = context_course::instance($courseid, IGNORE_MISSING)) {
        if ($havemanage = get_users_by_capability($context, 'agpu/course:manageactivities', 'u.id')) {
