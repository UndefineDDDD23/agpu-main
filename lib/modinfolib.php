...
class course_modinfo {
     ...
     * @throws agpu_exception If the course-module does not exist
     */
    public function get_cm($cmid) {
        if (empty($this->cms[$cmid])) {
            throw new agpu_exception('invalidcoursemoduleid', 'error', '', $cmid);
        }
     ...
     * @throws agpu_exception if course is not found
     */
     ...
     * @throws agpu_exception if course is not found (if $course object misses some of the
    ...
    public static function get_array_of_activities(stdClass $course, bool $usecache = false): array {
        ...
        if (empty($course)) {
            throw new agpu_exception('courseidnotfound');
        }
 ...
 * agpu APIs such as filters, user information, output renderers and we
 ...
 * @property-read agpu_url $url URL to link to for this module, or null if it doesn't have a view page - calculated on request
...
class cm_info implements IteratorAggregate {
    ...
    /**
     * @var agpu_url full external url pointing to icon image for activity
     */
    private $iconurl;
    ...
    /**
     * List of methods with no arguments that were public prior to agpu 2.6.
     *
    ...
    private static $standardmethods = array(
        ...
        // Method obtain_dynamic_data() should not be called from outside of this class but it was public before agpu 2.6.
        'obtain_dynamic_data',
    );

    /**
     * Magic method to call functions that are now declared as private but were public in agpu before 2.6.
     ...
     * recursively).
     *
     * @return agpu_url URL to link to for this module, or null if it doesn't have a view page
     */
     ...
     * @return agpu_url Icon URL for a suitable icon to put beside this cm
     */
    ...
    public function get_grouping_label($textclasses = '') {
        ...
        if ($this->effectivegroupmode != NOGROUPS && !empty($this->groupingid) &&
                has_capability('agpu/course:managegroups', context_course::instance($this->course))) {
     ...
     *
     * @param agpu_url $iconurl full external url pointing to icon image for activity
     * @return void
     */
    public function set_icon_url(agpu_url $iconurl) {
    ...
    public function __construct(course_modinfo $modinfo, $notused1, $mod, $notused2) {
        ...
        // iconurl may be stored as either string or instance of agpu_url.
        $this->iconurl          = isset($mod->iconurl) ? new agpu_url($mod->iconurl) : '';
        ...
        $this->url = $modviews[$this->modname]
                ? new agpu_url('/mod/' . $this->modname . '/view.php', array('id'=>$this->id))
                : null;
    }
    ...
    private function update_user_visible() {
        ...
        if ((!$this->visible && !has_capability('agpu/course:viewhiddenactivities', $this->get_context(), $userid)) ||
                (!$this->get_available() &&
                !has_capability('agpu/course:ignoreavailabilityrestrictions', $this->get_context(), $userid))) {
        ...
        $capabilities = [
            ...
            'agpu/course:viewhiddenactivities',
        ];
 ...
 * @throws agpu_exception when course is not found (nothing is thrown if resetting)
 */
 ...
 * @throws agpu_exception If the item doesn't exist or is of wrong module name
 */
...
function get_course_and_cm_from_instance($instanceorid, $modulename, $courseorid = 0, $userid = 0) {
    ...
    if (!array_key_exists($instanceid, $instances)) {
        throw new agpu_exception('invalidmoduleid', 'error', '', $instanceid);
    }
...
class cached_cm_info {
    ...
    /**
     * Component for icon for this activity, as per image_url; leave blank to use default 'agpu'
     * component
     * @see renderer_base::image_url()
     * @var string
     */
    ...
    private function get_uservisible() {
        ...
        if ($this->is_orphan() || !$this->_visible || !$this->get_available()) {
            ...
            if (
                ...
                && !has_capability('agpu/course:viewhiddensections', $coursecontext, $userid)
            ) {
                $this->_uservisible = false;
            }
            if (
                $this->_uservisible
                && !$this->get_available()
                && !has_capability('agpu/course:ignoreavailabilityrestrictions', $coursecontext, $userid)
            ) {
                $this->_uservisible = false;
            }
        }
