 ...
 * @return void
 */
function enrol_course_edit_form(agpuQuickForm $mform, $data, $context) {
...
function enrol_add_course_navigation(navigation_node $coursenode, $course) {
    ...
    if (has_capability('agpu/course:enrolreview', $coursecontext)) {
    ...
    if ($course->id != SITEID) {
        ...
        if (has_capability('agpu/course:enrolconfig', $coursecontext) or has_capability('agpu/course:enrolreview', $coursecontext)) {
            $url = new agpu_url('/enrol/instances.php', array('id'=>$course->id));
        } else {
            $url = NULL;
        }
        ...
        if (has_capability('agpu/course:renameroles', $coursecontext)) {
            $url = new agpu_url('/enrol/renameroles.php', array('id' => $course->id));
            $instancesnode->add(
    ...
    if (($course->groupmode || !$course->groupmodeforce) && has_capability('agpu/course:managegroups', $coursecontext)) {
        $url = new agpu_url('/group/index.php', array('id'=>$course->id));
        $usersnode->add(get_string('groups'), $url, navigation_node::TYPE_SETTING, null, 'groups', new pix_icon('i/group', ''));
    }

    if (has_any_capability(
        [ 'agpu/role:assign', 'agpu/role:safeoverride', 'agpu/role:override', 'agpu/role:review'],
        $coursecontext
    )) {
        // Override roles
        if (has_capability('agpu/role:review', $coursecontext)) {
            $url = new agpu_url('/admin/roles/permissions.php', array('contextid'=>$coursecontext->id));
        } else {
            $url = NULL;
        }
        ...
        if ($course->id == SITEID or (!empty($CFG->adminsassignrolesincourse) and is_siteadmin())) {
            if (has_capability('agpu/role:assign', $coursecontext)) {
                $url = new agpu_url('/admin/roles/assign.php', array('contextid'=>$coursecontext->id));
        ...
        if (has_any_capability(array('agpu/role:assign', 'agpu/role:safeoverride', 'agpu/role:override'), $coursecontext)) {
            $url = new agpu_url('/admin/roles/check.php', array('contextid'=>$coursecontext->id));
    ...
    if ($course->id != SITEID) {
        ...
        if (has_capability('agpu/course:reviewotherusers', $coursecontext)) {
            $url = new agpu_url('/enrol/otherusers.php', array('id'=>$course->id));
            ...
            if (is_viewing($coursecontext)) {
                ...
                foreach ($instances as $instance) {
                    ...
                    if ($plugin->show_enrolme_link($instance)) {
                        $url = new agpu_url('/enrol/index.php', array('id'=>$course->id));
...
function enrol_get_my_courses($fields = null, $sort = null, $limit = 0, $courseids = [], $allaccessible = false,
    ...
    if ($allaccessible) {
        if (is_siteadmin()) {
            ...
            // Include courses where the current user has agpu/course:view capability.
            $courses = get_user_capability_course('agpu/course:view', null, false);
            if (!$courses) {
                $courses = [];
            }
    ...
    foreach ($courses as $id=>$course) {
        ...
        if (!$course->visible) {
            ...
            if (!has_capability('agpu/course:viewhiddencourses', $context)) {
                unset($courses[$id]);
                continue;
            }
        }
    ...
    if ($onlyactive) {
        foreach ($courses as $id=>$course) {
            ...
            if (!$course->visible) {
                ...
                if (!has_capability('agpu/course:viewhiddencourses', $context, $userid)) {
                    unset($courses[$id]);
                    continue;
                }
            }
...
function enrol_user_sees_own_courses($user = null) {
    ...
    foreach($courses as $course) {
        ...
        if (has_capability('agpu/course:viewhiddencourses', $context, $user)) {
            return true;
        }
    }
...
abstract class enrol_plugin {
    ...
    /**
     * @deprecated since agpu 2.8 MDL-35864 - please use can_delete_instance() instead.
     */
     ...
     *
     * @param stdClass $instance
     * @return ?agpu_url or NULL if self unenrolment not supported
     */
    public function get_unenrolself_link($instance) {
        ...
        return new agpu_url("/enrol/$name/unenrolself.php", array('enrolid'=>$instance->id));
    }
     ...
     * @return void
     */
    public function course_edit_form($instance, agpuQuickForm $mform, $data, $context) {
     ...
     * @return void
     */
    public function edit_instance_form($instance, agpuQuickForm $mform, $context) {
    ...
    public function add_course_navigation($instancesnode, stdClass $instance) {
        if ($this->use_standard_editing_ui()) {
            ...
            if (has_capability($cap, $context)) {
                ...
                $managelink = new agpu_url('/enrol/editinstance.php', $linkparams);
    ...
    public function get_action_icons(stdClass $instance) {
        ...
        if ($this->use_standard_editing_ui()) {
            ...
            if (has_capability($cap, $context)) {
                $linkparams = array('courseid' => $instance->courseid, 'id' => $instance->id, 'type' => $instance->enrol);
                $editlink = new agpu_url("/enrol/editinstance.php", $linkparams);
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params = $manager->get_agpupage()->url->params();
        $params['ue'] = $ue->id;
        ...
        if ($this->allow_manage($instance) && has_capability("enrol/{$instance->enrol}:manage", $context)) {
            ...
            $url = new agpu_url('/enrol/editenrolment.php', $params);
            $actionparams = [
                'class' => 'editenrollink',
                'rel' => $ue->id,
        ...
        if ($this->allow_unenrol_user($instance, $ue) && has_capability("enrol/{$instance->enrol}:unenrol", $context)) {
            ...
            $url = new agpu_url('/enrol/unenroluser.php', $params);
            $actionparams = [
                'class' => 'unenrollink',
                'rel' => $ue->id,
    ...
    protected function fetch_users_and_notify_expiry(int $timenow, string $name, progress_trace $trace): void {
        ...
        foreach ($rs as $ue) {
            ...
            $users[] = [
                'fullname' => fullname($user, has_capability('agpu/site:viewfullnames', $context, $enroller)),
                'timeend' => $ue->timeend,
            ];
    ...
    protected function notify_expiry_enrolled($user, $ue, progress_trace $trace) {
        ...
        $message->contexturl        = (string)new agpu_url('/course/view.php', ['id' => $ue->courseid]);
    ...
    protected function get_expiry_message_body(stdClass $user, stdClass $ue, string $name,
        ...
        $a->enroller = fullname($enroller, has_capability('agpu/site:viewfullnames', $context, $user));
    ...
    protected function notify_expiry_enroller($eid, $users, progress_trace $trace) {
        ...
        $a->users     = implode("\n", $users);
        $a->extendurl = (string)new agpu_url('/user/index.php', array('id'=>$instance->courseid));
    ...
    public function send_course_welcome_message_to_user(
        ...
        if ($message && trim($message) !== '') {
            ...
            if (strpos($message, '<') === false) {
                ...
                // This is most probably the tag/newline soup known as FORMAT_agpu.
                $messagehtml = format_text($message, FORMAT_agpu,
        ...
        $message->userto = $user;
        $message->subject = get_string('welcometocourse', 'agpu', $a->coursename);
