...
class agpuODSWorkbook {
     ...
     * @return agpuODSWorksheet
     */
    public function add_worksheet($name = '') {
        $ws = new agpuODSWorksheet($name, $this->worksheets);
        $this->worksheets[] = $ws;
        return $ws;
    }

    /**
     * Create one agpu Format.
     *
     ...
     * @return agpuODSFormat
     */
    public function add_format($properties = array()) {
        return new agpuODSFormat($properties);
    }
    ...
    public function close() {
        ...
        $writer = new agpuODSWriter($this->worksheets);
...
class agpuODSWorksheet {
    ...
    public function write_string($row, $col, $str, $format = null) {
        if (!isset($this->data[$row][$col])) {
            $this->data[$row][$col] = new agpuODSCell();
        }
        if (is_array($format)) {
            $format = new agpuODSFormat($format);
        }
    ...
    public function merge_cells($first_row, $first_col, $last_row, $last_col) {
        ...
        if (!isset($this->data[$first_row][$first_col])) {
            $this->data[$first_row][$first_col] = new agpuODSCell();
        }
...
<office:document-meta xmlns:office="urn:oasis:names:tc:opendocument:xmlns:office:1.0"
                      ...
                      office:version="1.2">
    <office:meta>
        <meta:generator>agpu '.$CFG->release.'</meta:generator>
