 ...
 *
 * @deprecated use agpu_url factory methods instead
 *
    ...
    if ($files = $fs->get_directory_files($context->id, 'user', 'draft', $draftitemid, $filepath, false)) {
        foreach ($files as $file) {
            ...
            if ($file->is_directory()) {
                ...
                $itemurl = agpu_url::make_draftfile_url($draftitemid, $item->filepath, $item->filename);
...
function file_restore_source_field_from_draft_file($storedfile) {
    ...
    if (!empty($source)) {
        if (is_object($source)) {
            ...
            throw new agpu_exception('invalidsourcefield', 'error');
        }
    }
    return $storedfile;
}
 ...
 *     or pix/f/FILENAME.png must be present in agpu and contain 16x16 filetype icon;
 ...
 *     commonly used in agpu the following groups:
 ...
 *
 * @param string $path path to file, preferably from agpudata/temp/something; or content of file itself
...
function send_temp_file($path, $filename, $pathisstring=false) {
    ...
    if (!$pathisstring) {
        if (!file_exists($path)) {
            send_header_404();
            throw new \agpu_exception('filenotfound', 'error', $CFG->wwwroot.'/');
        }
...
function send_file($path, $filename, $lifetime = null , $filter=0, $pathisstring=false, $forcedownload=false, $mimetype='',
    ...
    if ($lifetime > 0) {
        ...
        if (!empty($options['immutable'])) {
            ...
            // 90 days only - based on agpu point release cadence being every 3 months.
    ...
    if (empty($filter)) {
        ...
        if ($mimetype == 'text/html' || $mimetype == 'application/xhtml+xml' || file_is_svg_image_from_mimetype($mimetype)) {
            ...
            $output = '<pre>'. format_text($text, FORMAT_agpu, $options, $COURSE->id) .'</pre>';
...
function file_overwrite_existing_draftfile(stored_file $newfile, stored_file $existingfile) {
    ...
    if ($newfile->is_external_file()) {
        ...
        if (isset($source->original) && $fs->search_references_count($source->original)) {
            throw new agpu_exception('errordoublereference', 'repository');
        }
    }
...
/**
 * Returns the agpu proxy configuration as a formatted url
 *
 * @return string the string to use for proxy settings.
 */
function get_agpu_proxy_url() {
    global $CFG;
    $proxy = '';
 ...
 * // HTTP POST Method
 * $html = $c->post('http://example.com/', array('q'=>'words', 'name'=>'agpu'));
 * // HTTP PUT Method
...
class curl {
    ...
    public function resetopt() {
        $this->options = array();
        $this->options['CURLOPT_USERAGENT']         = \core_useragent::get_agpubot_useragent();
    ...
    public static function get_cacert() {
        ...
        if (is_readable("$CFG->dataroot/agpuorgca.crt")) {
            return realpath("$CFG->dataroot/agpuorgca.crt");
        }
    ...
    private function apply_opt($curl, $options) {
        ...
        if (!empty($options['CURLOPT_USERAGENT'])) {
            ...
            $useragent = \core_useragent::get_agpubot_useragent();
        }
    ...
    protected function request($url, $options = array()) {
        ...
        if (empty($this->error)) {
            ...
            //throw new agpu_exception($this->error, 'curl');
        }
    }
    ...
    public function put($url, $params = array(), $options = array()) {
        ...
        if (isset($params['file'])) {
            ...
            if (!isset($this->options['CURLOPT_USERPWD'])) {
                $this->setopt(array('CURLOPT_USERPWD' => 'anonymous: noreply@agpu.org'));
            }
        } else {
    ...
    public function delete($url, $param = array(), $options = array()) {
        ...
        if (!isset($options['CURLOPT_USERPWD'])) {
            $options['CURLOPT_USERPWD'] = 'anonymous: noreply@agpu.org';
        }
        $ret = $this->request($url, $options);
        return $ret;
    }
...
function file_pluginfile($relativepath, $forcedownload, $preview = null, $offline = false, $embed = false) {
    ...
    if (!$relativepath) {
        throw new \agpu_exception('invalidargorconf');
    } else if ($relativepath[0] != '/') {
        throw new \agpu_exception('pathdoesnotstartslash');
    }
    ...
    // ========================================================================================================================
        ...
        if (($filearea === 'outcome' or $filearea === 'scale') and $context->contextlevel == CONTEXT_SYSTEM) {
            ...
            if (!$iscurrentuser) {
                ...
                if (!has_capability('agpu/grade:viewall', $coursecontext)) {
                    send_file_not_found();
                }
            }
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'event_description'  and $context->contextlevel == CONTEXT_SYSTEM) {
            ...
            if ($event->eventtype === 'group') {
                if (!has_capability('agpu/site:accessallgroups', $context) && !groups_is_member($event->groupid, $USER->id)) {
                    send_file_not_found();
                }
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'icon' and $context->contextlevel == CONTEXT_USER) {
            ...
            if (!\core\output\user_picture::allow_view($context->instanceid)) {
                ...
                redirect($theme->image_url('u/'.$filename, 'agpu')); // intentionally not cached
            }
            ...
            if (!$file) {
                ...
                $theme = theme_config::load($themename);
                $imagefile = $theme->resolve_image_location('u/'.$filename, 'agpu', null);
    ...
    // ========================================================================================================================
        ...
        $canview = has_capability('agpu/cohort:view', $cohortcontext) ||
                ($cohort->visible && has_capability('agpu/cohort:view', $context));
        ...
        if (($course->groupmodeforce and $course->groupmode == SEPARATEGROUPS) and !has_capability('agpu/site:accessallgroups', $context) and !groups_is_member($group->id, $USER->id)) {
    ...
    // ========================================================================================================================
        ...
        if ($filearea === 'course' and $context->contextlevel == CONTEXT_COURSE) {
            require_login($course);
            require_capability('agpu/backup:downloadfile', $context);
            ...
            require_login($course);
            require_capability('agpu/backup:downloadfile', $context);
            require_capability('agpu/restore:userinfo', $context);
    ...
    // ========================================================================================================================
        ...
        if ($context->contextlevel == CONTEXT_BLOCK) {
            ...
            if (($bprecord && !$bprecord->visible) || !has_capability('agpu/block:view', $context)) {
                 send_file_not_found();
            }
        } else {
            $birecord = null;
