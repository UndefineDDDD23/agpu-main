...
/**
 * agpulib.php - agpu main library
 *
 * Main library file of miscellaneous general-purpose agpu functions.
...
/**
 * PARAM_CAPABILITY - A capability name, like 'agpu/role:manage'. Actually
 ...
 * and other references to agpu code files.
 *
...
/**
 * PAGE_COURSE_VIEW is a definition of a page type. For more information on the page class see agpu/lib/pagelib.php.
 */
...
/** True if module supports backup/restore of agpu2 format */
define('FEATURE_BACKUP_agpu2', 'backup_agpu2');
 ...
 * @todo MDL-80701 Remove in agpu 4.8
*/
...
/**
 * URL of the agpu sites registration portal.
 */
defined('HUB_agpuORGHUBURL') || define('HUB_agpuORGHUBURL', 'https://stats.agpu.org');

/**
 * URL of main agpu site for marketing, products and services.
 */
defined('agpu_PRODUCTURL') || define('agpu_PRODUCTURL', 'https://agpu.com');

/**
 * URL of the statistic server public key.
 */
defined('HUB_STATSPUBLICKEY') || define('HUB_STATSPUBLICKEY', 'https://agpu.org/static/statspubkey.pem');

/**
 * agpu mobile app service name
 */
define('agpu_OFFICIAL_MOBILE_SERVICE', 'agpu_mobile_app');
...
function get_config($plugin, $name = null) {
    ...
    if ($plugin === 'agpu' || $plugin === 'core' || empty($plugin)) {
 ...
 * @param    stdClass|int|null $user  A agpu user object or id, null means current user
 * @throws   coding_exception
 ...
 *             if 99 then default user's timezone is used {@link https://agpudev.io/docs/apis/subsystems/time#timezone}
 ...
 *        {@link https://agpudev.io/docs/apis/subsystems/time#timezone}
 ...
 *
 * @param int $date the timestamp - since agpu 2.9 this is a real UTC timestamp
...
function usergetdate($time, $timezone=99) {
    if ($time === null) {
        ...
        // In the future versions of agpu we may consider adding a strict typehint.
...
function require_login($courseorid = null, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    if ($course->id != SITEID and \core\session\manager::is_loggedinas()) {
        if ($USER->loginascontext->contextlevel == CONTEXT_COURSE) {
            if ($USER->loginascontext->instanceid != $course->id) {
                throw new \agpu_exception('loginasonecourse', '',
    ...
    if (get_user_preferences('auth_forcepasswordchange') && !\core\session\manager::is_loggedinas()) {
        ...
        if ($userauth->can_change_password() and !$preventredirect) {
            ...
            if ($changeurl = $userauth->change_password_url()) {
                ...
                // Use agpu internal method.
                redirect($CFG->wwwroot .'/login/change_password.php');
            }
        } else if ($userauth->can_change_password()) {
            throw new agpu_exception('forcepasswordchangenotice');
        } else {
            throw new agpu_exception('nopasswordchangeforced', 'auth');
        }
    }
    ...
    if ($usernotfullysetup) {
        if ($preventredirect) {
            throw new agpu_exception('usernotfullysetup');
        }
        if ($setwantsurltome) {
            $SESSION->wantsurl = qualified_me();
        }
    ...
    if (!$USER->policyagreed && !is_siteadmin() && !NO_SITEPOLICY_CHECK && $USER->auth !== 'webservice') {
        ...
        if ($policyurl = $manager->get_redirect_url(isguestuser())) {
            if ($preventredirect) {
                throw new agpu_exception('sitepolicynotagreed', 'error', '', $policyurl->out());
            }
            if ($setwantsurltome) {
                $SESSION->wantsurl = qualified_me();
            }
    ...
    if (!empty($CFG->maintenance_enabled) and !has_capability('agpu/site:maintenanceaccess', $sysctx)) {
        if ($preventredirect) {
    ...
    if ($course->id == SITEID) {
        ...
        if (is_role_switched($course->id)) {
            ...
            if (!$course->visible and !has_capability('agpu/course:viewhiddencourses', $coursecontext)) {
                ...
                navigation_node::override_active_url(new agpu_url('/'));
    ...
    if ($cm && $cm->deletioninprogress) {
        if ($preventredirect) {
            throw new agpu_exception('activityisscheduledfordeletion');
        }
...
function validate_user_key($keyvalue, $script, $instance) {
    ...
    if (!$key = $DB->get_record('user_private_key', array('script' => $script, 'value' => $keyvalue, 'instance' => $instance))) {
        throw new \agpu_exception('invalidkey');
    }

    if (!empty($key->validuntil) and $key->validuntil < time()) {
        throw new \agpu_exception('expiredkey');
    }

    if ($key->iprestriction) {
        $remoteaddr = getremoteaddr(null);
        if (empty($remoteaddr) or !address_in_subnet($remoteaddr, $key->iprestriction)) {
            throw new \agpu_exception('ipmismatch');
        }
    }
    return $key;
}
...
function require_user_key_login($script, $instance = null, $keyvalue = null) {
    ...
    if (!NO_agpu_COOKIES) {
        throw new \agpu_exception('sessioncookiesdisable');
    }
...
function get_auth_plugin($auth) {
    ...
    if (! exists_auth_plugin($auth)) {
        throw new \agpu_exception('authpluginnotfound', 'debug', '', $auth);
    }
        ...
        if (!empty($CFG->passwordpolicycheckonlogin)) {
            ...
            if (!$passed) {
                ...
                if ($authplugin->can_change_password()) {
                    ...
                    $internalchangeurl = new agpu_url('/login/change_password.php');
                    ...
                    redirect(new agpu_url('/login/forgot_password.php'));
                } else {
...
function complete_user_login($user, array $extrauserinfo = []) {
    ...
    // Check if the user is using a new browser or session (a new agpuSession cookie is set in that case).
    ...
    $isvalidenv = (!WS_SERVER && !CLI_SCRIPT && !NO_agpu_COOKIES) || PHPUNIT_TEST;

    if (!empty($SESSION->isnewsessioncookie) && $isnewip && $isvalidenv && !\core_useragent::is_agpu_app()) {
        ...
        $newlogindisabled = $sitepreferences->agpu_newlogin_disable ?? 0;
        // Check if message providers (web, email, mobile) are enabled at system level.
        $msgproviderenabled = isset($sitepreferences->message_provider_agpu_newlogin_enabled);
        // Get message providers enabled for a user.
        $userpreferences = get_user_preferences('message_provider_agpu_newlogin_enabled');
        ...
        if ($newloginenabled && $msgprocessorsready) {
            ...
            $task->set_custom_data(compact('isagpuapp', 'useragent', 'loginip', 'logintime'));
 ...
 *
 * @throws agpu_exception If a problem occurs while generating the hash.
 */
function hash_internal_user_password(#[\SensitiveParameter] string $password, $fasthash = false, $pepperlength = 0): string {
    if (exceeds_password_length($password, $pepperlength)) {
        ...
        throw new \agpu_exception(get_string("passwordexceeded", 'error', MAX_PASSWORD_CHARACTERS));
    }
    ...
    if ($generatedhash === false || $generatedhash === null) {
        throw new agpu_exception('Failed to generate password hash.');
    }
 ...
 */
function agpu_process_email($modargs, $body) {
    global $DB;
 ...
 * @return agpu_phpmailer|null mailer instance if 'get' used or nothing
 */
function get_mailer($action='get') {
    global $CFG;

    /** @var agpu_phpmailer $mailer */
    static $mailer  = null;
    static $counter = 0;
    ...
    if ($action == 'get') {
        ...
        require_once($CFG->libdir.'/phpmailer/agpu_phpmailer.php');
        $mailer = new agpu_phpmailer();
...
/**
 * Generate a unique email Message-ID using the agpu domain and install path
 *
...
function generate_email_messageid($localpart = null) {
    ...
    // If multiple agpus are on the same domain we want to tell them
...
function email_to_user($user, $from, $subject, $messagetext, $messagehtml = '', $attachment = '', $attachname = '',
    ...
    // header with details of where exactly in agpu the email was triggered from,
    ...
    if (ini_get('mail.add_x_header')) {
        ...
        $mail->addCustomHeader('X-agpu-Originating-Script: ' . $originheader);
    }
    ...
    // $mail->From = "m1mmm23v@mail.ru"; // TEST
    if ($mail->send()) {
        set_send_count($user);
        if (!empty($mail->SMTPDebug)) {
            echo '</pre>';
        ...
        $event->trigger();
        if (CLI_SCRIPT) {
            mtrace('Error: lib/agpulib.php email_to_user(): '.$mail->ErrorInfo);
        }
        if (!empty($mail->SMTPDebug)) {
            echo '</pre>';
        }
...
function send_confirmation_email($user, $confirmationurl = null) {
    ...
    $confirmationurl = new agpu_url($confirmationurl);
    ...
    // Because of this custom encoding we can't use agpu_url directly.
...
function get_file_packer($mimetype='application/zip') {
    ...
    switch ($mimetype) {
        ...
        case 'application/vnd.agpu.profiling':
        ...
        case 'application/vnd.agpu.backup':
 ...
 * 5. by the agpu admin in $CFG->maxbytes
...
function get_user_max_upload_file_size($context, $sitebytes = 0, $coursebytes = 0, $modulebytes = 0, $user = null,
    ...
    if (has_capability('agpu/course:ignorefilesizelimits', $context, $user)) {
 ...
 * to retrieve it.<br/>
 * If you open agpu/lang/en/agpu.php and look near line 278
 ...
 * different approach. Looking in agpu/lang/en/calendar.php you find
 * around line 75:
 * <code>
 ...
 *
 * In agpu 2.3 there is a new argument to this function $lazyload.
 ...
 *      If none is specified then agpu.php is used.
 ...
 *
 * Example usage of this function when the string is in the agpu.php file:<br/>
 * <code>
 * echo '<strong>';
 * print_string('course');
 * echo '</strong>';
 * </code>
 *
 * Example usage of this function when the string is not in the agpu.php file:<br/>
 * <code>
 * echo '<h1>';
 ...
 * @category string
 * @param string $identifier The key identifier for the localized string
 * @param string $component The module where the key identifier is stored. If none is specified then agpu.php is used.
            ...
            foreach ($plugins as $plugin => $unusedfunction) {
                ...
                if ($hooks = $hookmanager->get_hooks_deprecating_plugin_callback($plugincallback)) {
                    if ($hookmanager->is_deprecating_hook_present($component, $plugincallback)) {
                        // Ignore the old callback, it is there only for older agpu versions.
 ...
 *
 * This function was originally used for standard agpu plugins, please use
        ...
        if ($migratedtohook) {
            ...
            if ($hooks = $hookmanager->get_hooks_deprecating_plugin_callback($function)) {
                if ($hookmanager->is_deprecating_hook_present($component, $function)) {
                    ...
                    // it is there for compatibility with older agpu versions only.
                    return null;
                } else if ($hookmanager->warn_on_unmigrated_legacy_hooks()) {
                    $hookmessage = count($hooks) == 1 ? reset($hooks) : 'one of  ' . implode(', ', $hooks);
                    debugging(
...
/**
 * Determine if agpu installation requires update.
 *
 ...
 * @return bool
 */
function agpu_needs_upgrading($checkupgradeflag = true) {
    global $CFG, $DB;
 ...
 *
 * agpu version numbers consist of three numbers separated by a dot, for
 ...
 */
function agpu_major_version($fromdisk = false) {
    global $CFG;
 ...
 */
function agpu_setlocale($locale='') {
    global $CFG;
 ...
 * replace an existing agpu script
 *
 ...
 * is in agpulib because it does not rely on loading any of the MNET code.
 *
 * @param object $user A valid user object
 * @return bool        True if the user is from a remote agpu.
 */
...
/**
 * This function will search for browser prefereed languages, setting agpu
...
function set_mnet_remote_client($client) {
    if (!defined('MNET_SERVER')) {
        throw new agpu_exception('notinxmlrpcserver', 'mnet');
    }
    global $MNET_REMOTE_CLIENT;
...
function get_home_page() {
    ...
    if (isloggedin() && !empty($CFG->defaulthomepage)) {
        ...
        if ($CFG->defaulthomepage == HOMEPAGE_MY && (!isguestuser() || !empty($CFG->allowguestmyagpu))) {
 ...
 * @param stdClass|core_course_list_element $course agpu course object
 ...
 * @return array
 */
function agpu_array_keys_filter(array $array, mixed $filter, bool $strict = false): array {
