...
class grades_external_test extends \externallib_advanced_testcase {
    ...
    protected function load_test_data($assignmentname, $student1rawgrade, $student2rawgrade) {
        ...
        $parentroleid = $this->assignUserCapability('agpu/grade:viewall', $student1context->id);
    ...
    public function test_update_grades(): void {
        ...
        // Give the student role 'agpu/grade:hide' and they should now be able to hide the grade item.
        $studentrole = $DB->get_record('role', array('shortname' => 'student'));
        $coursecontext = \context_course::instance($course->id);
        assign_capability('agpu/grade:hide', CAP_ALLOW, $studentrole->id, $coursecontext->id);
