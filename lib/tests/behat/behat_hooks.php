...
class behat_hooks extends behat_base {
    ...
    /**
     * Gives access to agpu codebase, ensures all is ready and sets up the test lock.
     *
     * Includes config.php to use agpu codebase with $CFG->behat_* instead of $CFG->prefix and $CFG->dataroot, called
     * once per suite.
     *
     * @BeforeSuite
    ...
    public static function before_suite_hook(BeforeSuiteScope $scope) {
        ...
        // Defined only when the behat CLI command is running, the agpu init setup process will
        ...
        if (!behat_util::is_test_data_updated()) {
            ...
            $message = <<<EOF
Your behat test site is outdated, please run the following command from your agpu dirroot to drop, and reinstall the Behat test site.
...
EOF;
    ...
    /**
     * Gives access to agpu codebase, to keep track of feature start time.
     *
    ...
    /**
     * Gives access to agpu codebase, to keep track of feature end time.
     *
    ...
    /**
     * Gives access to agpu codebase, to keep track of suite timings.
     *
...
EOF;
    ...
    public function before_scenario_hook(BeforeScenarioScope $scope) {
        ...
        // If scenario requires the agpu app to be running, set this up.
        if ($this->has_tag('app')) {
    ...
    public function before_step(BeforeStepScope $scope) {
        ...
        if (!$this->scenariorunning) {
            ...
            // Checking that the root path is a agpu test site.
            if (self::is_first_scenario()) {
...
EOF;
     ...
     * @see agpu\BehatExtension\EventDispatcher\Tester\ChainedStepTester
     */
