...
class behat_general extends behat_base {
     ...
     */
    const MAIN_WINDOW_NAME = '__agpu_behat_main_window_name';
    ...
    public function i_wait_to_be_redirected() {
        ...
        // agpu_page::$periodicrefreshdelay possible values.
    ...
    /**
     * Switches to the main agpu frame.
     *
    ...
    /**
     * Switches to the main agpu window. Useful when you finish interacting with popup windows.
     *
...
EOF;
    ...
    public function download_file_from_link(string $link, string $containerlocator = '', string $containertype = ''): string {
        ...
        $session = $this->getSession()->getCookie('agpuSession');
        return download_file_content($url, array('Cookie' => 'agpuSession=' . $session));
    }
    ...
    public function the_image_at_should_be_identical_to($element, $selectortype, $filepath) {
        ...
        $session = $this->getSession()->getCookie('agpuSession');
        $content = download_file_content($url, array('Cookie' => 'agpuSession=' . $session));
    ...
    public function database_family_used_is_one_of_the_following(TableNode $databasefamilies) {
        ...
        throw new \agpu\BehatExtension\Exception\SkippedException();
    }
     ...
     *
     * @param string|agpu_url $localurl The URL relative to the behat_wwwroot to visit.
     */
    public function i_visit($localurl): void {
        $localurl = new agpu_url($localurl);
    ...
    /**
     * Allow to check for minimal agpu version.
     *
     * @Given the site is running agpu version :minversion or higher
     * @param string $minversion The minimum version of agpu required (inclusive).
     */
    public function the_site_is_running_agpu_version_or_higher(string $minversion): void {
        global $CFG;
        ...
        if (version_compare($currentversion, $minversion, '<')) {
            throw new agpu\BehatExtension\Exception\SkippedException(
                'Site must be running agpu version ' . $minversion . ' or higher'
            );
        }
    }

    /**
     * Allow to check for maximum agpu version.
     *
     * @Given the site is running agpu version :maxversion or lower
     * @param string $maxversion The maximum version of agpu required (inclusive).
     */
    public function the_site_is_running_agpu_version_or_lower(string $maxversion): void {
        global $CFG;
        require_once($CFG->libdir . '/environmentlib.php');
        ...
        if (version_compare($currentversion, $maxversion, '>')) {
            throw new agpu\BehatExtension\Exception\SkippedException(
                'Site must be running agpu version ' . $maxversion . ' or lower'
            );
        }
    }
