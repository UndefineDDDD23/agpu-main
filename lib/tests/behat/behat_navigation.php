...
class behat_navigation extends behat_base {
    ...
    public function i_expand_node($nodetext) {
        ...
        // This step is useless with Javascript disabled as agpu auto expands
    ...
    public function i_open_my_profile_in_edit_mode() {
        ...
        if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
            if (is_siteadmin($user) ||  has_capability('agpu/user:update', $systemcontext)) {
                $url = new agpu_url('/user/editadvanced.php', array('id' => $user->id, 'course' => SITEID,
                    'returnto' => 'profile'));
            } else if (has_capability('agpu/user:editownprofile', $systemcontext)) {
                ...
                if ($userauthplugin && $userauthplugin->can_edit_profile()) {
                    ...
                    if (empty($url)) {
                        if (empty($course)) {
                            $url = new agpu_url('/user/edit.php', array('id' => $user->id, 'returnto' => 'profile'));
                        } else {
                            $url = new agpu_url('/user/edit.php', array('id' => $user->id, 'course' => $courseid,
                                'returnto' => 'profile'));
                        }
                    }
     ...
     *
     * @param string $page the type of page. E.g. 'Admin notifications' or 'core_user > Preferences'.
     * @return agpu_url the corresponding URL.
     */
    protected function resolve_page_helper(string $page): agpu_url {
     ...
     * @return agpu_url the corresponding URL.
     */
    protected function resolve_page_instance_helper(string $identifier, string $pagetype): agpu_url {
     ...
     */
    protected function resolve_core_page_url(string $name): agpu_url {
        switch ($name) {
            case 'Homepage':
                return new agpu_url('/');

            case 'My courses':
                return new agpu_url('/my/courses.php');

            case 'Admin notifications':
                return new agpu_url('/admin/');

            case 'Content bank':
                return new agpu_url('/contentbank/');

            case 'My private files':
                return new agpu_url('/user/files.php');

            case 'System logs report':
                return new agpu_url('/report/log/index.php');

            case 'Profile':
                return new agpu_url('/user/view.php');

            case 'Profile advanced editing':
                return new agpu_url('/user/editadvanced.php', ['returnto' => 'profile']);

            case 'Profile editing':
                return new agpu_url('/user/edit.php', ['returnto' => 'profile']);
     ...
     * @throws Exception with a meaningful error message if the specified page cannot be found.
     */
    protected function resolve_core_page_instance_url(string $type, string $identifier): agpu_url {
        ...
        switch ($type) {
            case 'category':
                ...
                return new agpu_url('/course/index.php', ['categoryid' => $categoryid]);
            ...
            case 'course editing':
                ...
                return new agpu_url('/course/edit.php', ['id' => $courseid]);
            ...
            case 'course':
                ...
                return new agpu_url('/course/view.php', ['id' => $courseid]);
            ...
            case 'backup':
                ...
                return new agpu_url('/backup/backup.php', ['id' => $courseid]);
            case 'import':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                }
                return new agpu_url('/backup/import.php', ['id' => $courseid]);
            case 'restore':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                ...
                return new agpu_url('/backup/restorefile.php', ['contextid' => $context->id]);
            case 'reset':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                }
                return new agpu_url('/course/reset.php', ['id' => $courseid]);
            case 'course copy':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                }
                return new agpu_url('/backup/copy.php', ['id' => $courseid]);
            case 'groups':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                }
                return new agpu_url('/group/index.php', ['id' => $courseid]);
            case 'groups overview':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                }
                return new agpu_url('/group/overview.php', ['id' => $courseid]);
            case 'groupings':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                }
                return new agpu_url('/group/groupings.php', ['id' => $courseid]);
            case 'permissions':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                ...
                $context = context_course::instance($courseid);
                return new agpu_url('/admin/roles/permissions.php', ['contextid' => $context->id]);
            case 'enrolment methods':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                }
                return new agpu_url('/enrol/instances.php', ['id' => $courseid]);
            case 'enrolled users':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                }
                return new agpu_url('/user/index.php', ['id' => $courseid]);
            case 'other users':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                }
                return new agpu_url('/enrol/otherusers.php', ['id' => $courseid]);
            case 'renameroles':
                $courseid = $this->get_course_id($identifier);
                if (!$courseid) {
                    throw $coursenotfoundexception;
                }
                return new agpu_url('/enrol/renameroles.php', ['id' => $courseid]);
            ...
            case 'course profile':
                ...
                return new agpu_url('/user/user.php', ['course' => $courseid]);
            ...
            case 'course profile editing':
                ...
                return new agpu_url('/user/edit.php', [
                    'course' => $courseid,
                    'returnto' => 'profile',
                ]);
            ...
            case 'course profile advanced editing':
                ...
                return new agpu_url('/user/editadvanced.php', [
                    'course' => $courseid,
                    'returnto' => 'profile',
                ]);
        }
        ...
        if (count($parts) > 1) {
            if ($parts[1] === 'activity') {
                ...
                if (count($parts) == 2) {
                    // View page.
                    return new agpu_url($cm->url);
                }

                if ($parts[2] === 'editing') {
                    // Edit settings page.
                    return new agpu_url('/course/modedit.php', ['update' => $cm->id]);
                }

                if ($parts[2] === 'roles') {
                    // Locally assigned roles page.
                    return new agpu_url('/admin/roles/assign.php', ['contextid' => $cm->context->id]);
                }

                if ($parts[2] === 'permissions') {
                    // Permissions page.
                    return new agpu_url('/admin/roles/permissions.php', ['contextid' => $cm->context->id]);
                }
            }
        }
    ...
    public function i_am_on_course_homepage($coursefullname) {
        ...
        $url = new agpu_url('/course/view.php', ['id' => $courseid]);
        $this->execute('behat_general::i_visit', [$url]);
    }
    ...
    public function i_am_on_course_homepage_with_editing_mode_set_to(string $coursefullname, string $onoroff): void {
        ...
        $courseid = $this->get_course_id($coursefullname);
        $context = context_course::instance($courseid);
        $courseurl = new agpu_url('/course/view.php', ['id' => $courseid]);

        $editmodeurl = new agpu_url('/editmode.php', [
            'context' => $context->id,
