...
if (!$type) {
    $PAGE->set_title('agpu spreadsheet export test');
    $PAGE->set_heading('agpu spreadsheet export test');
...
';
    ...
    echo $OUTPUT->box_end();
    echo $OUTPUT->single_button(new agpu_url($PAGE->url, array('type' => 'xslx')), 'Test Excel format');
    echo $OUTPUT->single_button(new agpu_url($PAGE->url, array('type' => 'ods')), 'Test ODS format');
    echo $OUTPUT->footer();
    die;
}

if ($type === 'xslx') {
    $workbook = new agpuExcelWorkbook('agputest.xlsx', 'Xslx');
} else if ($type === 'ods') {
    $workbook = new agpuODSWorkbook('agputest.ods');
}
...
$worksheet->write_string(0, 0, 'agpu worksheet export test',
    $workbook->add_format(array('color' => 'red', 'size' => 20, 'bold' => 1, 'italic' => 1)));
$worksheet->set_row(0, 25);
$worksheet->write(1, 0, 'agpu release: '.$CFG->release, $workbook->add_format(array('size' => 8, 'italic' => 1)));
...
$worksheet->write_url(8, 1, 'http://agpu.org');
