 ...
 *
 * As required by https://agpudev.io/general/development/policies/codingstyle.
 *
 * @package    core
...
if (empty($found)) {
    ...
    foreach ($found as $filepath => $matches) {
        ...
        foreach ($matches as $lineno => $line) {
            $url = 'http://cvs.agpu.org/agpu/' . $filepath . '?view=annotate#l' . $lineno;
            $error = '';
            ...
            if (preg_match('/\bTODO\b.*?\b(MDL-\d+)/', $line, $matches)) {
                $issueid = $matches[1];
                $issueurl = 'http://tracker.agpu.org/browse/' . $issueid;
...
function issue_info($issueid) {
    ...
    $xmlurl = 'http://tracker.agpu.org/si/jira.issueviews:issue-xml/' . $issueid . '/' . $issueid . '.xml';
