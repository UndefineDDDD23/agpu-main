...
class accesslib_test extends advanced_testcase {
    ...
    public function test_capabilities_mutation(): void {
        $oldcap = get_capability_info('agpu/site:config');
        $cap = get_capability_info('agpu/site:config');
        unset($cap->name);
        $newcap = get_capability_info('agpu/site:config');
    ...
    public function test_is_safe_capability(): void {
        ...
        $capability = $DB->get_record('capabilities', array('name'=>'agpu/site:config'), '*', MUST_EXIST);
    ...
    public function test_assign_capability(): void {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'agpu/backup:backupcourse'))); // Any capability assigned to student by default.
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$syscontext->id, 'roleid'=>$student->id, 'capability'=>'agpu/backup:backupcourse')));
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$student->id, 'capability'=>'agpu/backup:backupcourse')));

        $this->setUser($user);
        $result = assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $student->id, $frontcontext->id);
        $this->assertTrue($result);
        $permission = $DB->get_record('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$student->id, 'capability'=>'agpu/backup:backupcourse'));
        ...
        $this->setUser(0);
        $result = assign_capability('agpu/backup:backupcourse', CAP_PROHIBIT, $student->id, $frontcontext->id, false);
        $this->assertTrue($result);
        $permission = $DB->get_record('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$student->id, 'capability'=>'agpu/backup:backupcourse'));
        $this->assertNotEmpty($permission);
        $this->assertEquals(CAP_ALLOW, $permission->permission);
        ...
        $result = assign_capability('agpu/backup:backupcourse', CAP_PROHIBIT, $student->id, $frontcontext->id, true);
        $this->assertTrue($result);
        $permission = $DB->get_record('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$student->id, 'capability'=>'agpu/backup:backupcourse'));
        $this->assertNotEmpty($permission);
        ...
        $result = assign_capability('agpu/backup:backupcourse', CAP_INHERIT, $student->id, $frontcontext->id);
        $this->assertTrue($result);
        $permission = $DB->get_record('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$student->id, 'capability'=>'agpu/backup:backupcourse'));
        ...
        $sink = $this->redirectEvents();
        $capability = 'agpu/backup:backupcourse';
    ...
    public function test_unassign_capability(): void {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'agpu/backup:backupcourse'))); // Any capability assigned to manager by default.
        assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $manager->id, $frontcontext->id);

        $this->assertTrue($DB->record_exists('role_capabilities', array('contextid'=>$syscontext->id, 'roleid'=>$manager->id, 'capability'=>'agpu/backup:backupcourse')));
        $this->assertTrue($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$manager->id, 'capability'=>'agpu/backup:backupcourse')));

        $result = unassign_capability('agpu/backup:backupcourse', $manager->id, $syscontext->id);
        $this->assertTrue($result);
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$syscontext->id, 'roleid'=>$manager->id, 'capability'=>'agpu/backup:backupcourse')));
        $this->assertTrue($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$manager->id, 'capability'=>'agpu/backup:backupcourse')));
        unassign_capability('agpu/backup:backupcourse', $manager->id, $frontcontext);
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$manager->id, 'capability'=>'agpu/backup:backupcourse')));

        assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $manager->id, $syscontext->id);
        assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $manager->id, $frontcontext->id);
        $this->assertTrue($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$manager->id, 'capability'=>'agpu/backup:backupcourse')));

        $result = unassign_capability('agpu/backup:backupcourse', $manager->id);
        $this->assertTrue($result);
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$syscontext->id, 'roleid'=>$manager->id, 'capability'=>'agpu/backup:backupcourse')));
        $this->assertFalse($DB->record_exists('role_capabilities', array('contextid'=>$frontcontext->id, 'roleid'=>$manager->id, 'capability'=>'agpu/backup:backupcourse')));
    ...
    public function test_role_assign(): void {
        ...
        $this->assertInstanceOf('agpu_url', $event->get_url());
    }
    ...
    public function test_get_roles_with_capability(): void {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'agpu/backup:backupcourse'))); // Any capability is ok.
        $DB->delete_records('role_capabilities', array('capability'=>'agpu/backup:backupcourse'));

        $roles = get_roles_with_capability('agpu/backup:backupcourse');
        $this->assertEquals(array(), $roles);

        assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $manager->id, $syscontext->id);
        assign_capability('agpu/backup:backupcourse', CAP_PROHIBIT, $manager->id, $frontcontext->id);
        assign_capability('agpu/backup:backupcourse', CAP_PREVENT, $teacher->id, $frontcontext->id);

        $roles = get_roles_with_capability('agpu/backup:backupcourse');
        $this->assertEqualsCanonicalizing(array($teacher->id, $manager->id), array_keys($roles), true);

        $roles = get_roles_with_capability('agpu/backup:backupcourse', CAP_ALLOW);
        $this->assertEqualsCanonicalizing(array($manager->id), array_keys($roles), true);

        $roles = get_roles_with_capability('agpu/backup:backupcourse', null, $syscontext);
        $this->assertEqualsCanonicalizing(array($manager->id), array_keys($roles), true);
    }
    ...
    public function test_get_overridable_roles(): void {
        ...
        $DB->insert_record('role_names', $teacherename);
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'agpu/backup:backupcourse'))); // Any capability is ok.
        assign_capability('agpu/backup:backupcourse', CAP_PROHIBIT, $teacherrole->id, $coursecontext->id);
        ...
        foreach ($users as $user) {
            ...
            foreach ($contexts as $contextid => $unused) {
                ...
                foreach ($allroles as $roleid => $role) {
                    $hascap = has_any_capability(array('agpu/role:safeoverride', 'agpu/role:override'), $context);
                    if (is_siteadmin()) {
                        $this->assertTrue(isset($roles[$roleid]));
                    } else {
                        $parents = $context->get_parent_context_ids(true);
    ...
    public function test_has_capability_and_friends(): void {
        ...
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'agpu/backup:backupsection')));
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'agpu/backup:backupcourse')));
        $this->assertTrue($DB->record_exists('capabilities', array('name'=>'agpu/site:approvecourse')));

        $sca = array('agpu/backup:backupsection', 'agpu/backup:backupcourse', 'agpu/site:approvecourse');
        $sc = array('agpu/backup:backupsection', 'agpu/backup:backupcourse');

        $this->setUser(0);
        $this->assertFalse(has_capability('agpu/backup:backupsection', $coursecontext));
        $this->assertFalse(has_capability('agpu/backup:backupcourse', $coursecontext));
        $this->assertFalse(has_capability('agpu/site:approvecourse', $coursecontext));
        $this->assertFalse(has_any_capability($sca, $coursecontext));
        $this->assertFalse(has_all_capabilities($sca, $coursecontext));

        $this->assertTrue(has_capability('agpu/backup:backupsection', $coursecontext, $teacher));
        $this->assertTrue(has_capability('agpu/backup:backupcourse', $coursecontext, $teacher));
        $this->assertFalse(has_capability('agpu/site:approvecourse', $coursecontext, $teacher));
        ...
        $this->assertTrue(has_capability('agpu/backup:backupsection', $coursecontext, $admin));
        $this->assertTrue(has_capability('agpu/backup:backupcourse', $coursecontext, $admin));
        $this->assertTrue(has_capability('agpu/site:approvecourse', $coursecontext, $admin));
        ...
        $this->assertFalse(has_capability('agpu/backup:backupsection', $coursecontext, $admin, false));
        $this->assertFalse(has_capability('agpu/backup:backupcourse', $coursecontext, $admin, false));
        $this->assertFalse(has_capability('agpu/site:approvecourse', $coursecontext, $admin, false));
        ...
        $this->setUser($teacher);
        $this->assertTrue(has_capability('agpu/backup:backupsection', $coursecontext));
        $this->assertTrue(has_capability('agpu/backup:backupcourse', $coursecontext));
        $this->assertFalse(has_capability('agpu/site:approvecourse', $coursecontext));
        ...
        $this->assertTrue(has_capability('agpu/backup:backupsection', $coursecontext));
        $this->assertTrue(has_capability('agpu/backup:backupcourse', $coursecontext));
        $this->assertTrue(has_capability('agpu/site:approvecourse', $coursecontext));
        $this->assertTrue(has_any_capability($sca, $coursecontext));
        $this->assertTrue(has_all_capabilities($sc, $coursecontext));
        $this->assertTrue(has_all_capabilities($sca, $coursecontext));

        $this->assertFalse(has_capability('agpu/backup:backupsection', $coursecontext, 0));
        $this->assertFalse(has_capability('agpu/backup:backupcourse', $coursecontext, 0));
        $this->assertFalse(has_capability('agpu/site:approvecourse', $coursecontext, 0));
    ...
    public function test_fake_capability(): void {
        ...
        $fakecapname = 'agpu/fake:capability';
    ...
    public function test_fake_capability_assign(): void {
        ...
        $capability = 'agpu/fake:capability';
    ...
    public function test_role_definition_caching(): void {
        ...
        // Alter a role.
        role_change_permission($studentrole->id, $coursecontext, 'agpu/course:tag', CAP_ALLOW);
    ...
    public function test_get_user_capability_course(): void {
        ...
        $cap = 'agpu/course:view';

        // The structure being created here is this:
        //
        // All tests work with the single capability 'agpu/course:view'.
        //
    ...
    public function test_get_user_capability_contexts(): void {
        ...
        $generator = $this->getDataGenerator();
        $cap = 'agpu/contentbank:access';
        $defaultcategoryid = 1;

//         The structure being created here is this:
//
//         All tests work with the single capability 'agpu/contentbank:access'.
//         ROLE DEF/OVERRIDE                                                    .
...
//                 cat2        PROHIBIT    PROHIBIT        PROHIBIT             .
    ...
    public function test_guess_if_creator_will_have_course_capability(): void {
        ...
        $this->assertFalse(has_capability('agpu/course:view', $categorycontext, $creator));
        $this->assertFalse(has_capability('agpu/role:assign', $categorycontext, $creator));
        $this->assertFalse(has_capability('agpu/course:visibility', $categorycontext, $creator));
        $this->assertFalse(has_capability('agpu/course:visibility', $coursecontext, $creator));
        $this->assertFalse(guess_if_creator_will_have_course_capability('agpu/course:visibility', $categorycontext, $creator));
        $this->assertFalse(guess_if_creator_will_have_course_capability('agpu/course:visibility', $coursecontext, $creator));

        $this->assertTrue(has_capability('agpu/role:assign', $categorycontext, $manager));
        $this->assertTrue(has_capability('agpu/course:visibility', $categorycontext, $manager));
        $this->assertTrue(has_capability('agpu/course:visibility', $coursecontext, $manager));
        $this->assertTrue(guess_if_creator_will_have_course_capability('agpu/course:visibility', $categorycontext, $manager->id));
        $this->assertTrue(guess_if_creator_will_have_course_capability('agpu/course:visibility', $coursecontext, $manager->id));

        $this->assertEquals(0, $USER->id);
        $this->assertFalse(has_capability('agpu/course:view', $categorycontext));
        $this->assertFalse(has_capability('agpu/role:assign', $categorycontext));
        $this->assertFalse(has_capability('agpu/course:visibility', $categorycontext));
        $this->assertFalse(has_capability('agpu/course:visibility', $coursecontext));
        $this->assertFalse(guess_if_creator_will_have_course_capability('agpu/course:visibility', $categorycontext));
        $this->assertFalse(guess_if_creator_will_have_course_capability('agpu/course:visibility', $coursecontext));

        $this->setUser($manager);
        $this->assertTrue(has_capability('agpu/role:assign', $categorycontext));
        $this->assertTrue(has_capability('agpu/course:visibility', $categorycontext));
        $this->assertTrue(has_capability('agpu/course:visibility', $coursecontext));
        $this->assertTrue(guess_if_creator_will_have_course_capability('agpu/course:visibility', $categorycontext));
        $this->assertTrue(guess_if_creator_will_have_course_capability('agpu/course:visibility', $coursecontext));
        ...
        $this->assertFalse(has_capability('agpu/course:visibility', $categorycontext, $creator));
        $this->assertFalse(has_capability('agpu/course:visibility', $coursecontext, $creator));
        $this->assertTrue(guess_if_creator_will_have_course_capability('agpu/course:visibility', $categorycontext, $creator));
        $this->assertTrue(guess_if_creator_will_have_course_capability('agpu/course:visibility', $coursecontext, $creator));

        $this->setUser($creator);
        $this->assertFalse(has_capability('agpu/role:assign', $categorycontext, null));
        $this->assertFalse(has_capability('agpu/course:visibility', $categorycontext, null));
        $this->assertFalse(has_capability('agpu/course:visibility', $coursecontext, null));
        $this->assertTrue(guess_if_creator_will_have_course_capability('agpu/course:visibility', $categorycontext, null));
        $this->assertTrue(guess_if_creator_will_have_course_capability('agpu/course:visibility', $coursecontext, null));
        $this->setUser(0);
        ...
        role_change_permission($managerrole->id, $categorycontext, 'agpu/course:visibility', CAP_PREVENT);
        role_assign($creatorrole->id, $manager->id, $categorycontext);

        $this->assertTrue(has_capability('agpu/course:view', $categorycontext, $manager));
        $this->assertTrue(has_capability('agpu/course:view', $coursecontext, $manager));
        $this->assertTrue(has_capability('agpu/role:assign', $categorycontext, $manager));
        $this->assertTrue(has_capability('agpu/role:assign', $coursecontext, $manager));
        $this->assertFalse(has_capability('agpu/course:visibility', $categorycontext, $manager));
        $this->assertFalse(has_capability('agpu/course:visibility', $coursecontext, $manager));
        $this->assertFalse(guess_if_creator_will_have_course_capability('agpu/course:visibility', $categorycontext, $manager));
        $this->assertFalse(guess_if_creator_will_have_course_capability('agpu/course:visibility', $coursecontext, $manager));

        role_change_permission($managerrole->id, $categorycontext, 'agpu/course:view', CAP_PREVENT);
        $this->assertTrue(has_capability('agpu/role:assign', $categorycontext, $manager));
        $this->assertFalse(has_capability('agpu/course:visibility', $categorycontext, $manager));
        $this->assertFalse(has_capability('agpu/course:visibility', $coursecontext, $manager));
        $this->assertTrue(guess_if_creator_will_have_course_capability('agpu/course:visibility', $categorycontext, $manager));
        $this->assertTrue(guess_if_creator_will_have_course_capability('agpu/course:visibility', $coursecontext, $manager));
        ...
        try {
            guess_if_creator_will_have_course_capability('agpu/course:visibility', $syscontext, $creator);
    ...
    public function test_require_capability(): void {
        ...
        $this->setUser(0);
        $this->assertFalse(has_capability('agpu/site:config', $syscontext));
        try {
            require_capability('agpu/site:config', $syscontext);
            $this->fail('Exception expected from require_capability()');
        } catch (agpu_exception $e) {
            $this->assertInstanceOf('required_capability_exception', $e);
        }
        $this->setAdminUser();
        $this->assertFalse(has_capability('agpu/site:config', $syscontext, 0));
        try {
            require_capability('agpu/site:config', $syscontext, 0);
            $this->fail('Exception expected from require_capability()');
        } catch (agpu_exception $e) {
            $this->assertInstanceOf('required_capability_exception', $e);
        }
        $this->assertFalse(has_capability('agpu/site:config', $syscontext, null, false));
        try {
            require_capability('agpu/site:config', $syscontext, null, false);
            $this->fail('Exception expected from require_capability()');
        } catch (agpu_exception $e) {
            $this->assertInstanceOf('required_capability_exception', $e);
        }
    ...
    public function test_permission_evaluation(): void {
        ...
        foreach ($testcontexts as $context) {
            ...
            $this->assertInstanceOf('agpu_url', $context->get_url());
        ...
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'agpu/site:accessallgroups'));
        $this->assertFalse($rc);
        assign_capability('agpu/site:accessallgroups', CAP_ALLOW, $allroles['teacher'], $frontpagecontext->id);
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'agpu/site:accessallgroups'));
        $this->assertEquals(CAP_ALLOW, $rc->permission);
        assign_capability('agpu/site:accessallgroups', CAP_PREVENT, $allroles['teacher'], $frontpagecontext->id);
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'agpu/site:accessallgroups'));
        $this->assertEquals(CAP_ALLOW, $rc->permission);
        assign_capability('agpu/site:accessallgroups', CAP_PREVENT, $allroles['teacher'], $frontpagecontext, true);
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'agpu/site:accessallgroups'));
        $this->assertEquals(CAP_PREVENT, $rc->permission);

        assign_capability('agpu/site:accessallgroups', CAP_INHERIT, $allroles['teacher'], $frontpagecontext);
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'agpu/site:accessallgroups'));
        $this->assertFalse($rc);
        assign_capability('agpu/site:accessallgroups', CAP_ALLOW, $allroles['teacher'], $frontpagecontext);
        unassign_capability('agpu/site:accessallgroups', $allroles['teacher'], $frontpagecontext, true);
        $rc = $DB->get_record('role_capabilities', array('contextid'=>$frontpagecontext->id, 'roleid'=>$allroles['teacher'], 'capability'=>'agpu/site:accessallgroups'));
        $this->assertFalse($rc);
        unassign_capability('agpu/site:accessallgroups', $allroles['teacher'], $frontpagecontext->id, true);
        unset($rc);
        ...
        assign_capability('agpu/site:accessallgroups', CAP_ALLOW, $CFG->defaultuserroleid, $frontpageblockcontext, true);
        assign_capability('agpu/site:accessallgroups', CAP_ALLOW, $CFG->defaultfrontpageroleid, $frontpageblockcontext, true);
        assign_capability('agpu/block:view', CAP_PROHIBIT, $allroles['guest'], $frontpageblockcontext, true);
        ...
        assign_capability('agpu/site:accessallgroups', CAP_PREVENT, $CFG->defaultuserroleid, $frontpagepagecontext, true);
        assign_capability('agpu/site:accessallgroups', CAP_ALLOW, $CFG->defaultfrontpageroleid, $frontpagepagecontext, true);
        ...
        assign_capability('agpu/site:accessallgroups', CAP_ALLOW, $CFG->defaultuserroleid, $frontpagecontext, true);
        assign_capability('agpu/site:accessallgroups', CAP_ALLOW, $CFG->defaultfrontpageroleid, $frontpagecontext, true);
        ...
        assign_capability('agpu/course:update', CAP_PROHIBIT, $allroles['teacher'], context_course::instance($testcourses[17]), true);
        ...
        $this->assertFalse(has_capability('agpu/block:view', $frontpageblockcontext, $guestid));
        ...
        $this->assertFalse(has_capability('agpu/block:view', $frontpageblockcontext, 0));
        ...
        $this->assertFalse(has_capability('agpu/course:create', $systemcontext, $testusers[11]));
        $this->assertTrue(has_capability('agpu/course:create', context_coursecat::instance($testcategories[2]), $testusers[11]));
        $this->assertFalse(has_capability('agpu/course:create', context_course::instance($testcourses[1]), $testusers[11]));
        $this->assertTrue(has_capability('agpu/course:create', context_course::instance($testcourses[19]), $testusers[11]));

        $this->assertFalse(has_capability('agpu/course:update', context_course::instance($testcourses[1]), $testusers[9]));
        $this->assertFalse(has_capability('agpu/course:update', context_course::instance($testcourses[19]), $testusers[9]));
        $this->assertFalse(has_capability('agpu/course:update', $systemcontext, $testusers[9]));

        // Test prohibits.
        $this->assertTrue(has_capability('agpu/course:update', context_system::instance(), $testusers[19]));
        $ids = get_users_by_capability(context_system::instance(), 'agpu/course:update', 'u.id');
        $this->assertArrayHasKey($testusers[19], $ids);
        $this->assertFalse(has_capability('agpu/course:update', context_course::instance($testcourses[17]), $testusers[19]));
        $ids = get_users_by_capability(context_course::instance($testcourses[17]), 'agpu/course:update', 'u.id');
        ...
        $enrolled = get_enrolled_users($coursecontext, 'agpu/course:update');
        $this->assertCount(1, $enrolled);
        ...
        load_all_capabilities();
        $coursecontext = context_course::instance($course1->id);
        $this->assertTrue(has_capability('agpu/course:update', $coursecontext));
        ...
        $this->assertFalse(has_capability('agpu/course:update', $coursecontext));
        ...
        $this->assertTrue(has_capability('agpu/course:update', $blockcontext));
        role_switch($allroles['student'], $coursecontext);
        $this->assertEquals($allroles['student'], $USER->access['rsw'][$coursecontext->path]);
        $this->assertFalse(has_capability('agpu/course:update', $blockcontext));
        reload_all_capabilities();
        $this->assertFalse(has_capability('agpu/course:update', $blockcontext));
        load_all_capabilities();
        $this->assertTrue(has_capability('agpu/course:update', $blockcontext));
        ...
        $this->assertFalse(has_capability('agpu/course:update', $coursecontext, $userid));
        load_temp_course_role($coursecontext, $roleid);
        reload_all_capabilities();
        $this->assertFalse(has_capability('agpu/course:update', $coursecontext, $userid));
        $USER = new stdClass();
    ...
    public function test_context_module_caps_returned_by_get_capabilities_in_sys_context(): void {
        ...
        $expectedcapabilities = ['agpu/site:accessallgroups', 'agpu/site:viewfullnames',
    ...
    public function test_context_block_caps_returned_by_get_capabilities_block_context(): void {
        ...
        // Just test a few representative capabilities.
        $expected = ['block/online_users:addinstance', 'agpu/block:edit', 'agpu/block:view'];
    ...
    public function test_context_user_caps_returned_by_get_capabilities_user_context(): void {
        ...
        // Just test a few representative capabilities.
        $expected = ['agpu/user:editmessageprofile', 'agpu/user:editprofile', 'agpu/user:viewalldetails'];
        $this->assert_capability_list_contains($expected, $capabilities);
    ...
    public function test_update_capabilities(): void {
        ...
        $this->assertFalse(isset($existingcaps['agpu/site:restore']));         // Moved to new 'agpu/restore:restorecourse'.
        $this->assertTrue(isset($existingcaps['agpu/restore:restorecourse'])); // New cap from 'agpu/site:restore'.
        $this->assertTrue(isset($existingcaps['agpu/site:sendmessage']));      // New capability.
        $this->assertTrue(isset($existingcaps['agpu/backup:backupcourse']));
        $this->assertTrue(isset($existingcaps['agpu/backup:backupsection']));  // Cloned from 'agpu/backup:backupcourse'.
        $this->assertTrue(isset($existingcaps['agpu/site:approvecourse']));    // Updated bitmask.
        $this->assertTrue(isset($existingcaps['agpu/course:manageactivities']));
        $this->assertTrue(isset($existingcaps['mod/page:addinstance']));         // Cloned from core 'agpu/course:manageactivities'.

        // Fake state before upgrade.
        $DB->set_field('capabilities', 'name', 'agpu/site:restore', array('name'=>'agpu/restore:restorecourse'));
        $DB->set_field('role_capabilities', 'capability', 'agpu/site:restore', array('capability'=>'agpu/restore:restorecourse'));
        assign_capability('agpu/site:restore', CAP_PROHIBIT, $teacher->id, $froncontext->id, true);
        $perms1 = array_values($DB->get_records('role_capabilities', array('capability'=>'agpu/site:restore', 'roleid'=>$teacher->id), 'contextid, permission', 'contextid, permission'));

        $DB->delete_records('role_capabilities', array('capability'=>'agpu/site:sendmessage'));
        $DB->delete_records('capabilities', array('name'=>'agpu/site:sendmessage'));

        $DB->delete_records('role_capabilities', array('capability'=>'agpu/backup:backupsection'));
        $DB->delete_records('capabilities', array('name'=>'agpu/backup:backupsection'));
        assign_capability('agpu/backup:backupcourse', CAP_PROHIBIT, $student->id, $froncontext->id, true);
        assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $teacher->id, $froncontext->id, true);

        $DB->set_field('capabilities', 'riskbitmask', 0, array('name'=>'agpu/site:approvecourse'));
        ...
        assign_capability('agpu/course:manageactivities', CAP_PROHIBIT, $student->id, $froncontext->id, true);
        assign_capability('agpu/course:manageactivities', CAP_ALLOW, $teacher->id, $froncontext->id, true);

        // Execute core.
        update_capabilities('agpu');

        // Only core should be upgraded.
        $caps = $DB->get_records('capabilities', array(), 'id', 'name, captype, contextlevel, component, riskbitmask');

        $this->assertFalse(isset($existingcaps['agpu/site:restore']));
        $this->assertTrue(isset($caps['agpu/restore:restorecourse']));
        $this->assertEquals($existingcaps['agpu/restore:restorecourse'], $caps['agpu/restore:restorecourse']);
        $perms2 = array_values($DB->get_records('role_capabilities', array('capability'=>'agpu/restore:restorecourse', 'roleid'=>$teacher->id), 'contextid, permission', 'contextid, permission'));
        $this->assertEquals($perms1, $perms2);

        $this->assertTrue(isset($caps['agpu/site:sendmessage']));
        $this->assertEquals($existingcaps['agpu/site:sendmessage'], $caps['agpu/site:sendmessage']);

        $this->assertTrue(isset($caps['agpu/backup:backupsection']));
        $this->assertEquals($existingcaps['agpu/backup:backupsection'], $caps['agpu/backup:backupsection']);
        $roles = $DB->get_records_sql('SELECT DISTINCT roleid AS id FROM {role_capabilities} WHERE capability=? OR capability=?', array('agpu/backup:backupcourse', 'agpu/backup:backupsection'));
        foreach ($roles as $role) {
            $perms1 = array_values($DB->get_records('role_capabilities', array('capability'=>'agpu/backup:backupcourse', 'roleid'=>$role->id), 'contextid, permission', 'contextid, permission'));
            $perms2 = array_values($DB->get_records('role_capabilities', array('capability'=>'agpu/backup:backupsection', 'roleid'=>$role->id), 'contextid, permission', 'contextid, permission'));
            $this->assertEquals($perms1, $perms2);
        }

        $this->assertTrue(isset($caps['agpu/site:approvecourse']));
        $this->assertEquals($existingcaps['agpu/site:approvecourse'], $caps['agpu/site:approvecourse']);
        ...
        $roles = $DB->get_records_sql('SELECT DISTINCT roleid AS id FROM {role_capabilities} WHERE capability=? OR capability=?', array('agpu/course:manageactivities', 'mod/page:addinstance'));
        foreach ($roles as $role) {
            $perms1 = array_values($DB->get_records('role_capabilities', array('capability'=>'agpu/course:manageactivities', 'roleid'=>$role->id), 'contextid, permission', 'contextid, permission'));
    ...
    public function test_update_capabilities_clone_performance(): void {
        ...
        for ($i = 0; $i < 100; $i++) {
            ...
            assign_capability('agpu/course:manageactivities', CAP_PREVENT, $roleid, $contextid);
        }

        // Get rid of one of the capabilities for forum, which clones agpu/course:manageactivities.
    ...
    public function test_update_capabilities_clone_existing(): void {
        ...
        // agpu/course:manageactivities. In one of them, also override mod/forum:addinstance
        ...
        assign_capability('agpu/course:manageactivities', CAP_PREVENT, $roleid, $context1->id);
        $page2 = $generator->create_module('page', ['course' => $course->id]);
        $context2 = context_module::instance($page2->cmid);
        assign_capability('agpu/course:manageactivities', CAP_PREVENT, $roleid, $context2->id);
    ...
    public function test_get_users_by_capability(): void {
        ...
        $users = get_users_by_capability($coursecontext, 'agpu/backup:backupcourse');
        ...
        $users = get_users_by_capability($coursecontext, 'agpu/site:approvecourse');
        ...
        // Test role override.
        assign_capability('agpu/backup:backupcourse', CAP_PROHIBIT, $teacherrole->id, $coursecontext, true);
        assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $studentrole->id, $coursecontext, true);
    ...
    public function test_get_with_capability_sql(): void {
        ...
        list($sql, $params) = get_with_capability_sql($coursecontext, 'agpu/backup:backupcourse');
        $users = $DB->get_records_sql($sql, $params);
        ...
        list($sql, $params) = get_with_capability_sql($coursecontext, 'agpu/site:approvecourse');
        $users = $DB->get_records_sql($sql, $params);
    ...
    public function get_get_with_capability_join_override_cases() {
        return [
                'no overrides' => [true, []],
                'one override' => [true, ['agpu/course:viewscales']],
                'both overrides' => [false, ['agpu/course:viewscales', 'agpu/question:flag']],
        ];
    }
    ...
    public function test_get_with_capability_join_when_overrides_present(
        ...
        $coursecontext = context_course::instance($course->id);
        $this->assertTrue(has_capability('agpu/course:viewscales', $coursecontext, $student));
        $this->assertTrue(has_capability('agpu/question:flag', $coursecontext, $student));
        ...
        $sqljoin = get_with_capability_join($coursecontext,
                ['agpu/course:viewscales', 'agpu/question:flag'], 'u.id');
    ...
    public function test_require_all_capabilities(): void {
        ...
        $sca = array('agpu/backup:backupsection', 'agpu/backup:backupcourse');
