...
class ldaplib_test extends \advanced_testcase {
    ...
    public function ldap_normalise_objectclass_provider() {
        return array(
            ...
            'Supplied complex' => array(
                array('(&(objectClass=cheetah)(enabledagpuUser=1))'),
                '(&(objectClass=cheetah)(enabledagpuUser=1))',
            ),
        );
    }

    /**
     * Tests for ldap_get_entries_agpu.
     *
     ...
     *
     */
    public function test_ldap_get_entries_agpu(): void {
        $this->resetAfterTest();
        ...
        $debuginfo = '';
        if (!$connection = ldap_connect_agpu(TEST_LDAPLIB_HOST_URL, 3, 'rfc2307', TEST_LDAPLIB_BIND_DN,
        ...
        if (!($containerdn = $this->create_test_container($connection, 'agputest'))) {
            $this->markTestSkipped('Can not create test LDAP container.');
        }

        // Add all the test objects.
        $testobjects = $this->get_ldap_get_entries_agpu_test_objects();
        ...
        foreach ($testobjects as $object) {
            ...
            $entries = ldap_get_entries_agpu($connection, $sr);
    ...
    /**
     * Provide the array of test objects for the ldap_get_entries_agpu test case.
     *
     * @return array of test objects
     */
    protected function get_ldap_get_entries_agpu_test_objects() {
        $testobjects = array(
            // Test object 1.
            array(
     ...
     *              by ldap_get_entries_agpu_test_objects() member function.
     *
