...
/**
 * Unit tests for (some of) ../agpulib.php.
 *
 * @package    core
 * @category   phpunit
 * @copyright  &copy; 2006 The Open University
 * @author     T.J.Hunt@open.ac.uk
 * @author     nicolas@agpu.com
 */
final class agpulib_test extends \advanced_testcase {
    ...
    public function test_optional_param(): void {
        ...
        // Make sure warning is displayed if array submitted - TODO: throw exception in agpu 2.3.
    ...
    public function test_required_param(): void {
        ...
        try {
            ...
            $this->fail('agpu_exception expected');
        } catch (\agpu_exception $ex) {
            $this->assertInstanceOf('agpu_exception', $ex);
        }
    ...
    public function test_clean_param_url(): void {
        ...
        $this->assertSame('', clean_param('mailto:support@agpu.org', PARAM_URL));
        $this->assertSame('', clean_param('mailto:support@agpu.org?subject=Hello%20agpu', PARAM_URL));
        $this->assertSame('', clean_param('mailto:support@agpu.org?subject=Hello%20agpu&cc=feedback@agpu.org', PARAM_URL));
     ...
     * output using agpu TZ/DST support and PHP native one.
     *
     ...
     */
    public function test_some_agpu_special_dst(): void {
        ...
        $agpures = userdate($stamp, '%Y/%m/%d %H:%M:%S', 'Europe/Tallinn', false); // agpu result.
        $this->assertSame($expectation, $phpres);
        $this->assertSame($expectation, $agpures);
        ...
        $phpres = $phpdt->format('Y/m/d H:i:s'); // PHP result.
        $agpures = userdate($stamp, '%Y/%m/%d %H:%M:%S', 'America/St_Johns', false); // agpu result.
        $this->assertSame($expectation, $phpres);
        $this->assertSame($expectation, $agpures);
    ...
    /**
     * Make sure the DST changes happen at the right time in agpu.
     */
    ...
    public function test_get_string(): void {
        ...
        $numdays = get_string('numdays', 'agpu', 0, true);
    ...
    public static function generate_confirmation_link_provider(): array {
        ...
        return [
            ...
            "Custom external confirmation url" => [
                "username" => "many_-.@characters@_@-..-..",
                "confirmationurl" => "http://agpu.org/custom/external/url.php",
                "expected" => "http://agpu.org/custom/external/url.php?data=/many_-%2E%40characters%40_%40-%2E%2E-%2E%2E"
            ],
            "Custom external confirmation url with parameters" => [
                "username" => "many_-.@characters@_@-..-..",
                "confirmationurl" => "http://agpu.org/ext.php?with=some&param=eters",
                "expected" => "http://agpu.org/ext.php?with=some&param=eters&data=/many_-%2E%40characters%40_%40-%2E%2E-%2E%2E"
            ],
            "Custom external confirmation url with parameters (again)" => [
                "username" => "many_-.@characters@_@-..-..",
                "confirmationurl" => "http://agpu.org/ext.php?with=some&data=test",
                "expected" => "http://agpu.org/ext.php?with=some&data=/many_-%2E%40characters%40_%40-%2E%2E-%2E%2E"
            ],
        ];
    }
...
EOT;
        ...
        return [
            ...
            [1, '<span>a</span><span>b</span>', FORMAT_agpu],
    ...
    public function test_generate_email_processing_address(): void {
        ...
        $data = (object)[
            'id' => 42,
            'email' => 'my.email+from_agpu@example.com',
        ];
    ...
    public function callable_names_provider() {
        return [
            ...
            'static_method_of_object' => [
                ...
                'core\agpulib_test::my_foobar_method',
            ],
            'method_of_object' => [
    ...
    /**
     * Data provider for \core_agpulib_testcase::test_get_complete_user_data().
     *
     * @return array
     */
     ...
     * $param int|null $allowguestmyagpu The $CFG->allowguestmyagpu setting value.
    ...
    public function test_get_home_page(
        ...
        if (isset($allowguestmyagpu)) {
            $CFG->allowguestmyagpu = $allowguestmyagpu;
        }
    ...
    /**
     * Test that the agpu_array_keys_filter method behaves in the same way
     * that array_keys behaved before agpu 8.3.
     *
     * @dataProvider agpu_array_keys_filter_provider
     * @param array $array
     * @param mixed $filter
     * @param bool $strict
     * @param array $expected
     * @covers ::agpu_array_keys_filter
     */
    public function test_agpu_array_keys_filter(
        array $array,
        mixed $filter,
        bool $strict,
        array $expected,
        ...
        $this->assertSame(
            $expected,
            agpu_array_keys_filter($array, $filter, $strict),
        );
    }

    /**
     * Data provider for agpu_array_keys_filter tests.
     *
     * @return array
     */
    public static function agpu_array_keys_filter_provider(): array {
        return [
            [['a', 'b', 'c'], 'b', false, [1]],
            [
                [
