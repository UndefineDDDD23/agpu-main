...
final class outputrequirementslib_test extends \advanced_testcase {
    public function test_string_for_js(): void {
        ...
        $page = new \agpu_page();
        $page->requires->string_for_js('course', 'agpu', 1);
        $page->requires->string_for_js('course', 'agpu', 1);
        $this->expectException('coding_exception');
        $page->requires->string_for_js('course', 'agpu', 2);
    ...
    public function test_one_time_output_different_pages_independent(): void {
        $firstpage = new \agpu_page();
        $secondpage = new \agpu_page();
     ...
     *
     * @param \agpu_url $agpuurl The <u>agpu_url</u> instance pointing to a web resource.
     ...
     * @dataProvider js_fix_url_agpu_url_provider
     */
    public function test_js_fix_url_agpu_url(\agpu_url $agpuurl, int $cfgslashargs, string $expected): void {
        global $CFG;
        ...
        $actualagpuurl = $rcm->invokeArgs($requires, [$agpuurl]);
        $this->assertEquals($expected, $actualagpuurl->out(false));

        $CFG->slasharguments = $defaultslashargs;
    }

    /**
     * Data provider for JavaScript proper Handler using a <u>\agpu_url</url>.
     *
     * @return array
     * @see \page_requirements_manager::js_fix_url()
     * @see \agpu_url
     */
    public function js_fix_url_agpu_url_provider() {
        global $CFG;
        ...
        return [
            ...
            'A custom agpu CSS Handler' => [
                new \agpu_url('/mod/data/css.php?d=1234567890'),
                1,
                $wwwroot . '/mod/data/css.php?d=1234567890'
            ],
            'A custom agpu JS Handler (slasharguments on)' => [
                new \agpu_url('/mod/data/js.php?d=1234567890'),
                1,
                $wwwroot . '/mod/data/js.php?d=1234567890'
            ],
            'A custom agpu JS Handler (slasharguments off)' => [
                new \agpu_url('/mod/data/js.php?d=1234567890'),
                0,
                $wwwroot . '/mod/data/js.php?d=1234567890'
            ],
    ...
    public function test_js_fix_url_plain_string(string $url, int $cfgslashargs, string $expected): void {
        ...
        $rcm = $rc->getMethod('js_fix_url');
        $requires = new \page_requirements_manager();
        $actualagpuurl = $rcm->invokeArgs($requires, [$url]);
        $this->assertEquals($expected, $actualagpuurl->out(false));
     ...
     *
     * @param agpu_url|string|null $url The URL pointing to a web resource.
        ...
        return [
            'Provide a null argument' => [
                ...
                'Coding error detected, it must be fixed by a programmer: '
                    . 'Invalid JS url, it has to be shortened url starting with / or agpu_url instance.'
            ],
            ...
            'A non-existant internal resource using agpu_url. WARN the developer!' => [
                new \agpu_url('/path/to/file2.ext'),
                'Coding error detected, it must be fixed by a programmer: '
