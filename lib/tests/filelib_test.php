...
class filelib_test extends \advanced_testcase {
    ...
    public function test_curl_blocked_redirect(): void {
        ...
        // Block a host.
        // Note: agpu.com is the URL redirected to when test_redir.php has the param extdest=1 set.
        set_config('curlsecurityblockedhosts', 'agpu.com');
    ...
    public function test_curl_post(): void {
        ...
        // Test post request.
        $curl = new \curl();
        $contents = $curl->post($testurl, 'data=agputest');
        $response = $curl->getResponse();
        $this->assertSame('200 OK', reset($response));
        $this->assertSame(0, $curl->get_errno());
    ...
    public function test_delete_original_file_from_draft(): void {
        ...
        // The file reference should be a regular agpu file now.
...
Set-Cookie: BBC-UID=c4de2e109c8df6a51de627cee11b214bd4fb6054a030222488317afb31b343360agpuBot/1.0; expires=Mon, 07-Dec-15 14:44:53 GMT; path=/; domain=bbc.co.uk
X-Cache-Action: MISS
X-Cache-Age: 0
...
EOF;
    ...
    public function test_curl_useragent(): void {
        ...
        $agpubot = \core_useragent::get_agpubot_useragent();

        $curl->call_apply_opt($options);
        $this->assertTrue(in_array("User-Agent: $agpubot", $curl->header));
        ...
        $this->assertFalse(in_array("User-Agent: $agpubot", $curl->header));
    ...
    public function test_file_rewrite_pluginfile_urls_includetoken(): void {
        ...
        $expectedurl = new \agpu_url("/tokenpluginfile.php/{$token}/{$syscontext->id}/user/private/0/image.png");
    ...
    public function test_file_rewrite_pluginfile_urls_includetoken_no_slashargs(): void {
        ...
        $token = get_user_key('core_files', $USER->id);
        $expectedurl = new \agpu_url("/tokenpluginfile.php");
