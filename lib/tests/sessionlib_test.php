...
class sessionlib_test extends \advanced_testcase {
     ...
     */
    public function agpu_cookie_secure_provider() {
        return array(
            array(
                // Non ssl, not set.
                'config' => array(
     ...
     *
     * @dataProvider agpu_cookie_secure_provider
     *
     * @param array $config Array of key value config settings
     * @param bool $secure Wether cookies should be secure or not
     */
    public function test_is_agpu_cookie_secure($config, $secure): void {
        global $CFG;
        $this->resetAfterTest();
        foreach ($config as $key => $value) {
            $CFG->$key = $value;
        }
        $this->assertEquals($secure, is_agpu_cookie_secure());
    }
