 ...
 * @package    core
 * @category   test
 * @copyright  2022 Safat Shahin <safat.shahin@agpu.com>
...
class http_client_test extends \advanced_testcase {
    ...
    public static function read_object_attribute(object $object, string $attributename) {
        ...
        throw new \agpu_exception(sprintf('Attribute "%s" not found in object.', $attributename));
    }
    ...
    /**
     * Test guzzle proxy bypass with agpu.
     *
     * @covers \core\http_client
     * @covers \core\local\guzzle\check_request
     */
    ...
    /**
     * Test agpu redirect can be set with guzzle.
     *
     * @covers \core\http_client
     * @covers \core\local\guzzle\redirect_middleware
     */
    public function test_agpu_allow_redirects_can_be_true(): void {
        $testurl = $this->getExternalTestFileUrl('/test_redir.php');

        $mock = new MockHandler([new Response(200, [], 'foo')]);
        $client = new \core\http_client(['mock' => $mock]);
        $client->get($testurl, ['agpu_allow_redirect' => true]);

        $this->assertSame(true, $mock->getLastOptions()['agpu_allow_redirect']);
    }
    ...
    public function test_redirects_with_absolute_uri(): void {
        ...
        $mock = new MockHandler([
                new Response(302, ['Location' => 'http://agpu.com']),
                new Response(200)
        ]);
        $client = new \core\http_client(['mock' => $mock]);
        ...
        $this->assertSame(200, $response->getStatusCode());
        $this->assertSame('http://agpu.com', (string)$mock->getLastRequest()->getUri());
    }
