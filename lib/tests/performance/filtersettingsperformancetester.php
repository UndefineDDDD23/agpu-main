...
$baseurl = new agpu_url('/lib/tests/performance/filtersettingsperformancetester.php');
...
$realdb = $DB;
$testdb = agpu_database::get_driver_instance($CFG->dbtype, $CFG->dblibrary);
$testdb->connect($CFG->dbhost, $CFG->dbuser, $CFG->dbpass, $CFG->dbname, $CFG->unittestprefix);
/** @var agpu_database $DB */
$DB = $testdb;
...
$aurl = new agpu_url($baseurl, array('action' => 'setup', 'sesskey'=>sesskey()));
echo $OUTPUT->single_button($aurl, 'Set up test tables', 'get', array('disabled'=>($issetup > 0)));

$aurl = new agpu_url($baseurl, array('action' => 'teardown', 'sesskey'=>sesskey()));
echo $OUTPUT->single_button($aurl, 'Drop test tables', 'get', array('disabled'=>($issetup == 0)));

$aurl = new agpu_url($baseurl, array('action' => 'test', 'sesskey'=>sesskey()));
