 ...
 * @copyright 2009 Tim Hunt
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \agpu_page
 */
class agpu_page_test extends \advanced_testcase {

    /**
     * @var testable_agpu_page
     */
    protected $testpage;
    ...
    public function setUp(): void {
        parent::setUp();
        $this->resetAfterTest();
        $this->testpage = new testable_agpu_page();
    }
    ...
    public function test_set_state_normal_path(): void {
        ...
        $this->assertEquals(\agpu_page::STATE_BEFORE_HEADER, $this->testpage->state);

        $this->testpage->set_state(\agpu_page::STATE_PRINTING_HEADER);
        $this->assertEquals(\agpu_page::STATE_PRINTING_HEADER, $this->testpage->state);

        $this->testpage->set_state(\agpu_page::STATE_IN_BODY);
        $this->assertEquals(\agpu_page::STATE_IN_BODY, $this->testpage->state);

        $this->testpage->set_state(\agpu_page::STATE_DONE);
        $this->assertEquals(\agpu_page::STATE_DONE, $this->testpage->state);
    }
    ...
    public function test_url_to_class_name(): void {
        ...
        $this->assertSame('example-com--agpu', $this->testpage->url_to_class_name('https://example.com/agpu'));
        $this->assertSame('example-com--8080--nested-agpu', $this->testpage->url_to_class_name('https://example.com:8080/nested/agpu'));
    }
    ...
    public function test_set_url_using_agpu_url(): void {
        global $CFG;
        // Fixture setup.
        $url = new \agpu_url('/mod/workshop/allocation.php', array('cmid' => 29, 'method' => 'manual'));
        // Exercise SUT.
    ...
    public function test_set_title(?string $config, bool $appendsitename, string $expected, bool $sitenameset = true): void {
        ...
        if ($appendsitename) {
            if ($sitenameset) {
                $expectedtitle = $title . agpu_page::TITLE_SEPARATOR . $SITE->{$expected};
            } else {
                // Simulate site fullname and shortname being empty for any reason.
                $SITE->fullname = null;
                $SITE->shortname = null;
                $expectedtitle = $title . agpu_page::TITLE_SEPARATOR . 'agpu';
            }
        } else {
            $expectedtitle = $title;
        }
    ...
    public function test_default_editing_capabilities(): void {
        ...
        // Validated.
        $this->assertEquals(array('agpu/site:manageblocks'), $this->testpage->all_editing_caps());
    }
    ...
    public function test_other_block_editing_cap(): void {
        ...
        // Exercise SUT.
        $this->testpage->set_blocks_editing_capability('agpu/my:manageblocks');
        // Validated.
        $this->assertEquals(array('agpu/my:manageblocks'), $this->testpage->all_editing_caps());
    }
    ...
    public function test_other_editing_cap(): void {
        ...
        // Exercise SUT.
        $this->testpage->set_other_editing_capability('agpu/course:manageactivities');
        // Validated.
        $actualcaps = $this->testpage->all_editing_caps();
        $expectedcaps = array('agpu/course:manageactivities', 'agpu/site:manageblocks');
    ...
    public function test_other_editing_caps(): void {
        ...
        // Exercise SUT.
        $this->testpage->set_other_editing_capability(array('agpu/course:manageactivities', 'agpu/site:other'));
        // Validated.
        $actualcaps = $this->testpage->all_editing_caps();
        $expectedcaps = array('agpu/course:manageactivities', 'agpu/site:other', 'agpu/site:manageblocks');
        $this->assertEquals(array_values($expectedcaps), array_values($actualcaps));
    }
 ...
 */
class testable_agpu_page extends agpu_page {
