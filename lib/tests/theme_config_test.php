...
class theme_config_test extends advanced_testcase {
    ...
    public function test_svg_image_use(): void {
        ...
        $imagefile = $theme->resolve_image_location('i/test', 'agpu', true);
        $this->assertSame('test.svg', basename($imagefile));
        $imagefile = $theme->resolve_image_location('i/test', 'agpu', false);
        ...
        foreach ($useragents as $agent => $expected) {
            ...
            $imagefile = $testtheme->resolve_image_location('i/test', 'agpu', null);
            if ($expected) {
