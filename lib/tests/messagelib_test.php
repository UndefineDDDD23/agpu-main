...
class messagelib_test extends \advanced_testcase {
    ...
    public function test_message_provider_disabled(): void {
        ...
        $disableprovidersetting = 'agpu_instantmessage_disable';
    ...
    public function test_message_get_providers_for_user_more(): void {
        ...
        $this->assertFalse(has_capability('agpu/site:config', $modulecontext));
        $providers = message_get_providers_for_user($teacher->id);
        $this->assertFalse($this->message_type_present('agpu', 'backup', $providers));

        // Give the user the required capability in an activity module.
        // They should now be able to see the backup message.
        assign_capability('agpu/site:config', CAP_ALLOW, $teacherrole->id, $modulecontext->id, true);
        accesslib_clear_all_caches_for_unit_testing();
        $modulecontext = \context_module::instance($assign->cmid);
        $this->assertTrue(has_capability('agpu/site:config', $modulecontext));

        $providers = message_get_providers_for_user($teacher->id);
        $this->assertTrue($this->message_type_present('agpu', 'backup', $providers));
        ...
        assign_capability('agpu/site:config', CAP_PROHIBIT, $teacherrole->id, $coursecontext->id, true);
        accesslib_clear_all_caches_for_unit_testing();
        $modulecontext = \context_module::instance($assign->cmid);
        $this->assertFalse(has_capability('agpu/site:config', $modulecontext));
    ...
    public function test_send_message_redirection(): void {
        ...
        $this->assertCount(0, $sink->get_messages());
        $this->assertDebuggingCalled('Attempt to send msg from a provider agpu/xxx '.
            'that is inactive or not allowed for the user id='.$user2->id);
        $sink->close();
    ...
    public function test_send_message(): void {
        ...
        set_user_preference('message_provider_agpu_instantmessage_enabled', 'none', $user2);
        ...
        $messageid = message_send($message);
        $this->assertFalse($messageid);
        $this->assertDebuggingCalled('Attempt to send msg from a provider agpu/instantmessage '.
            'that is inactive or not allowed for the user id='.$user2->id);
        $emails = $sink->get_messages();
        $this->assertCount(0, $emails);
        $sink->clear();
        ...
        // Will always use the pop-up processor.
        set_user_preference('message_provider_agpu_instantmessage_enabled', 'email', $user2);
        ...
        set_user_preference('message_provider_agpu_instantmessage_enabled', 'email,popup', $user2);
        ...
        set_user_preference('message_provider_agpu_instantmessage_enabled', 'popup', $user2);
    ...
    public function test_message_send_to_self_conversation(): void {
        ...
        // Ensure we're going to hit the email processor for this user.
        $DB->set_field_select('message_processors', 'enabled', 0, "name <> 'email'");
        set_user_preference('message_provider_agpu_instantmessage_enabled', 'email', $user1);
    ...
    public function test_message_send_to_conversation_group(): void {
        ...
        $DB->set_field_select('message_processors', 'enabled', 0, "name <> 'email'");
        set_user_preference('message_provider_agpu_instantmessage_enabled', 'email', $user2);
        set_user_preference('message_provider_agpu_instantmessage_enabled', 'email', $user3);
