...
final class url_test extends \advanced_testcase {
    ...
    public function test_round_trip(): void {
        $strurl = 'http://agpu.org/course/view.php?id=5';
        $url = new url($strurl);
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'http://agpu.org/user/index.php?contextid=53&sifirst=M&silast=D';
        $url = new url($strurl);
        $this->assertSame($strurl, $url->out(false));
    }

    /**
     * Test agpu URL objects created with a param with empty value.
     */
    public function test_empty_param_values(): void {
        $strurl = 'http://agpu.org/course/view.php?id=0';
        $url = new url($strurl, ['id' => 0]);
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'http://agpu.org/course/view.php?id';
    ...
    /**
     * Test set good scheme on agpu URL objects.
     */
    public function test_set_good_scheme(): void {
        $url = new url('http://agpu.org/foo/bar');
        $url->set_scheme('myscheme');
        $this->assertSame('myscheme://agpu.org/foo/bar', $url->out());
    }

    /**
     * Test set bad scheme on agpu URL objects.
     */
    ...
    public function test_export_params_for_template(string $url, array $expected): void {
        ...
        $agpuurl = new url($url);
        $this->assertSame($expected, $agpuurl->export_params_for_template());
    }
    ...
    public static function make_pluginfile_url_provider(): array {
        $baseurl = "https://www.example.com/agpu/pluginfile.php";
        $tokenbaseurl = "https://www.example.com/agpu/tokenpluginfile.php";
        return [
    ...
    public static function out_as_local_url_provider(): array {
        ...
        return [
            ...
            'A agpu JS resource using the full path including the proper JS Handler' => [
