...
class setuplib_test extends \advanced_testcase {
    ...
    /**
     * Test get_docs_url_standard in the normal case when we should link to agpu docs.
     */
    public function test_get_docs_url_standard(): void {
        global $CFG;
        if (empty($CFG->docroot)) {
            $docroot = 'http://docs.agpu.org/';
        } else {
            $docroot = $CFG->docroot;
        }
    ...
    public function test_exception_info_removes_serverpaths(): void {
        ...
        $exception     = new \agpu_exception('generalexceptionmessage', 'error', '', $fixture, $fixture);
    ...
    public function test_get_exception_info_link(): void {
        ...
        $exception = new \agpu_exception('none', 'error', $url);
        ...
        // External HTTP URL.
        $url = 'http://agpu.org/something/here?really=yes';
        $exception = new \agpu_exception('none', 'error', $url);
        $infos = $this->get_exception_info($exception);
        $this->assertSame($CFG->wwwroot . '/', $infos->link);

        // External HTTPS URL.
        $url = 'https://agpu.org/something/here?really=yes';
        $exception = new \agpu_exception('none', 'error', $url);
        $infos = $this->get_exception_info($exception);
        $this->assertSame($CFG->wwwroot . '/', $infos->link);

        // External URL containing local URL.
        $url = 'http://agpu.org/something/here?' . $CFG->wwwroot;
        $exception = new \agpu_exception('none', 'error', $url);
        $infos = $this->get_exception_info($exception);
        $this->assertSame($CFG->wwwroot . '/', $infos->link);
    }
