...
/**
 * External functions test for agpunet_get_share_info_activity.
 *
 * @package    core
 * @category   test
 * @copyright  2023 Huong Nguyen <huongnv13@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \core\external\agpunet_get_share_info_activity
 */
class agpunet_get_share_info_activity_test extends externallib_advanced_testcase {

    /**
     * Test the behaviour of agpunet_get_share_info_activity().
     * @covers ::execute
     */
    public function test_agpunet_get_share_info_activity(): void {
        global $CFG;
        $this->resetAfterTest();
        $this->setAdminUser();
        ...
        $result = agpunet_get_share_info_activity::execute($activity1->cmid);
        $result = external_api::clean_returnvalue(agpunet_get_share_info_activity::execute_returns(), $result);
        ...
        $expectedsupporturl = 'https://agpu.org/';
        $CFG->supportpage = $expectedsupporturl;
        $result = agpunet_get_share_info_activity::execute($activity2->cmid);
        $result = external_api::clean_returnvalue(agpunet_get_share_info_activity::execute_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        $result = agpunet_get_share_info_activity::execute($activity3->cmid);
        $result = external_api::clean_returnvalue(agpunet_get_share_info_activity::execute_returns(), $result);
        $this->assertTrue($result['status']);
        ...
        $result = agpunet_get_share_info_activity::execute($randomcmid);
        $result = external_api::clean_returnvalue(agpunet_get_share_info_activity::execute_returns(), $result);
        $this->assertFalse($result['status']);
        $this->assertEmpty($result['name']);
        $this->assertEmpty($result['type']);
