 ...
 *
 * @coversDefaultClass \core\agpunet\utilities
 * @package core
...
class utilities_test extends \advanced_testcase {
    ...
    public function test_is_valid_instance(): void {
        ...
        $CFG->enablesharingtoagpunet = false;
        ...
        set_config('oauthservice', random_int(1, 30), 'agpunet');
        $this->assertFalse(utilities::is_valid_instance($issuer));
        ...
        // Can not share if the OAuth 2 service type is not agpunet.
        $issuer->set('enabled', 1);
        $this->assertFalse(utilities::is_valid_instance($issuer));

        // All good now.
        $issuer->set('servicetype', 'agpunet');
    ...
    public function test_can_user_share(): void {
        ...
        assign_capability('agpu/agpunet:shareactivity', CAP_ALLOW, $teacherrole->id, $this->coursecontext);
        assign_capability('agpu/backup:backupactivity', CAP_ALLOW, $teacherrole->id, $this->coursecontext);
        ...
        assign_capability('agpu/agpunet:shareactivity', CAP_PROHIBIT, $editingteacherrole->id, $this->coursecontext);
        ...
        assign_capability('agpu/agpunet:sharecourse', CAP_ALLOW, $teacherrole->id, $this->coursecontext);
        assign_capability('agpu/backup:backupcourse', CAP_ALLOW, $teacherrole->id, $this->coursecontext);
        ...
        assign_capability('agpu/agpunet:sharecourse', CAP_PROHIBIT, $editingteacherrole->id, $this->coursecontext);
    ...
    public function test_does_user_have_capability_in_any_course(): void {
        ...
        // Compare to cache.
        $teachercachedvalue = \cache::make('core', 'agpunet_usercanshare')->get($teacher1->id);
        $this->assertSame('yes', $teachercachedvalue);
        $studentcachedvalue = \cache::make('core', 'agpunet_usercanshare')->get($student1->id);
        $this->assertSame('no', $studentcachedvalue);

        // Change the teacher's role and check the agpunet_usercanshare cache is invalidated for everyone.
