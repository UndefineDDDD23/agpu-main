...
/**
 * Test coverage for agpunet course sender.
 *
 * @package   core
 * @copyright 2023 Safat Shahin <safat.shahin@agpu.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \core\agpunet\course_sender
 */
class course_sender_test extends \advanced_testcase {
    ...
    public function test_prepare_share_contents(): void {
        ...
        $this->expectExceptionMessage(get_string('agpunet:invalidshareformat', 'error'));

        $httpclient = new http_client();
        $agpunetclient = new agpunet_client($httpclient, $this->mockoauthclient);
    ...
    public function test_get_resource_description(): void {
        ...
        $course = $this->generator->create_course([
            'summary' => '<p>This is an example agpu course description.</p>
<p>&nbsp;</p>
...
<p>The last word of this sentence is in <strong>bold</strong></p>'
        ...
        $this->assertEquals('This is an example agpu course description.
 
...
The last word of this sentence is in bold', $processeddescription);
     ...
     * @covers ::log_event
     * @covers \core\agpunet\agpunet_client::create_resource_from_stored_file
     * @covers \core\agpunet\agpunet_client::prepare_file_share_request_data
    ...
    public function test_share_resource(ResponseInterface $httpresponse, array $expected): void {
        ...
        set_config('oauthservice', $this->issuer->get('id'), 'agpunet');
        ...
        $event = end($events);
        $this->assertInstanceOf('\core\event\agpunet_resource_exported', $event);
        $this->assertEquals($USER->id, $event->userid);

        if ($result['responsecode'] == 201) {
            $description = "The user with id '{$USER->id}' successfully shared course to agpuNet with the " .
                "following course id, from context with id '{$this->coursecontext->id}': '{$this->course->id}'.";
        } else {
            $description = "The user with id '{$USER->id}' failed to share course to agpuNet with the " .
                "following course id, from context with id '{$this->coursecontext->id}': '{$this->course->id}'.";
        }
        ...
        return [
            'Success' => [
                'http_response' => new Response(
                    ...
                    json_encode([
                        'homepage' => 'https://agpunet.example.com/drafts/view/testcourse_backup_1.mbz',
                    ]),
                ),
                'expected' => [
                    'response_code' => 201,
                    'resource_url' => 'https://agpunet.example.com/drafts/view/testcourse_backup_1.mbz',
                ],
            ],
            'Fail with 200 status code' => [
                'http_response' => new Response(
                    ...
                    ['Content-Type' => 'application/json'],
                    json_encode([
                        'homepage' => 'https://agpunet.example.com/drafts/view/testcourse_backup_2.mbz',
                    ]),
                ),
                'expected' => [
                    'response_code' => 200,
                    'resource_url' => 'https://agpunet.example.com/drafts/view/testcourse_backup_2.mbz',
                ],
            ],
            'Fail with 401 status code' => [
                ...
                'expected' => [
                    ...
                    'exception' => 'Client error: ' .
                        '`POST https://agpunet.example.com/.pkg/@agpunet/ed-resource/basic/v1/create` ' .
