...
class navigationlib_test extends \advanced_testcase {
    ...
    protected function setup_node() {
        ...
        $activeurl = $PAGE->url;
        $inactiveurl = new \agpu_url('http://www.agpu.com/');
    ...
    public function test_node__construct(): void {
        ...
        $fakeproperties = array(
            ...
            'action' => new \agpu_url('http://www.agpu.org/'));
    ...
    public function test_node_add(): void {
        ...
        $node1 = $this->node->add('test_add_1', 'http://www.agpu.org/', navigation_node::TYPE_COURSE, 'testadd1', 'key', new pix_icon('i/course', ''));
        // Add a node with the minimum args required.
        $node2 = $this->node->add('test_add_2', null, navigation_node::TYPE_CUSTOM, 'testadd2');
        $node3 = $this->node->add(str_repeat('agpu ', 15), str_repeat('agpu', 15));
    ...
    public function test_node_check_if_active(): void {
        ...
        // Demo1 -> action is http://www.agpu.org/, thus should be true.
        ...
        // Demo2 -> action is http://www.agpu.com/, thus should be false.
    ...
    public function test_node_make_active(): void {
        ...
        $node2 = $this->node->add('active node 2', new \agpu_url($CFG->wwwroot), navigation_node::TYPE_COURSE, null, 'anode2');
        $node1->make_active();
    ...
    public function test_navbar_prepend_and_add(): \agpu_page {
        global $PAGE;
        ...
        $page->set_url(new \agpu_url('/course/view.php', array('id' => $course->id)));
     ...
     * @param $node
     */
    public function test_navbar_has_items(\agpu_page $page): void {
        $this->resetAfterTest();
    ...
    public function test_cache_set(): void {
        ...
        $cache->set('software', 'agpu');
        $this->assertTrue($cache->cached('software'));
        $this->assertEquals($cache->software, 'agpu');
    }
    ...
    public function is_action_link_provider(): array {
        return [
            'The navigation node has an action link.' =>
                [
                    navigation_node::create('Node', new action_link(new \agpu_url('/'), '',
                        new popup_action('click', new \agpu_url('/'))), navigation_node::TYPE_SETTING),
                    true
                ],

            'The navigation node does not have an action link.' =>
                [
                    navigation_node::create('Node', new \agpu_url('/'), navigation_node::TYPE_SETTING),
                    false
                ],
        ];
    }
    ...
    public function action_link_actions_provider(): array {
        return [
            ...
            'The navigation node has an action link without an action.' =>
                [
                    navigation_node::create('Node', new action_link(new \agpu_url('/'), '', null),
...
class exposed_global_navigation extends global_navigation {
    ...
    public function __construct(?\agpu_page $page=null) {
        global $PAGE;
        if ($page === null) {
            $page = $PAGE;
        }
...
class navigation_exposed_agpu_page extends \agpu_page {
