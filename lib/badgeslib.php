 ...
 * @todo Final deprecation in agpu 6.0. See MDL-82332.
 */
...
function badges_notify_badge_award(badge $badge, $userid, $issued, $filepathhash) {
    ...
    $badgeurl = new agpu_url('/badges/badge.php', ['hash' => $issued]);
    ...
    $eventdata->customdata        = [
        'notificationiconurl' => agpu_url::make_pluginfile_url(
...
function badges_prepare_badge_for_external(stdClass $badge, stdClass $user): object {
    ...
    $canconfiguredetails = has_capability('agpu/badges:configuredetails', $context);
...
function badges_prepare_badgeclass_for_external(core_badges\output\badgeclass $badgeclass): stdClass {
    ...
    $badgeurl = new \agpu_url('/badges/badgeclass.php', [
        'id' => $badgeclass->badge->id,
    ]);
    $badgeurl = $badgeurl->out(false);
    $file = \agpu_url::make_webservice_pluginfile_url(
...
function badges_add_course_navigation(navigation_node $coursenode, stdClass $course) {
    ...
    $canmanage = has_any_capability(array('agpu/badges:viewawarded',
                                          ...
                                          'agpu/badges:deletebadge'), $coursecontext);
    ...
    if (!empty($CFG->enablebadges) && !empty($CFG->badges_allowcoursebadges) && !$isfrontpage && $canmanage) {
        ...
        $url = new agpu_url('/badges/index.php', array('type' => BADGE_TYPE_COURSE, 'id' => $course->id));

        $coursenode->get('coursebadges')->add(get_string('managebadges', 'badges'), $url,
            navigation_node::TYPE_SETTING, null, 'coursebadges');

        if (has_capability('agpu/badges:createbadge', $coursecontext)) {
            $url = new agpu_url('/badges/edit.php', ['action' => 'new', 'courseid' => $course->id]);
...
function print_badge_image(badge $badge, stdClass $context, $size = 'small') {
    ...
    $imageurl = agpu_url::make_pluginfile_url($context->id, 'badges', 'badgeimage', $badge->id, '/', $fsize, false);
 ...
 * @return string|agpu_url|null Returns either new file path hash or new file URL
 */
function badges_bake($hash, $badgeid, $userid = 0, $pathhash = false) {
    ...
    $fileurl = agpu_url::make_pluginfile_url($user_context->id, 'badges', 'userbadge', $badge->id, '/', $hash, true);
    return $fileurl;
}
...
function badges_create_site_backpack($data) {
    global $DB;
    $context = context_system::instance();
    require_capability('agpu/badges:manageglobalsettings', $context);
...
function badges_save_external_backpack(stdClass $data) {
    ...
    if ($data->apiversion == OPEN_BADGES_V2P1) {
        ...
        // In the future, this request can be moved to the agpu-exttests repository.
        if (!PHPUNIT_TEST) {
 ...
 *
 * @throws \agpu_exception if attempting to use invalid direction value.
 */
...
function badges_get_default_issuer() {
    ...
    $issuerid = new agpu_url('/badges/issuer_json.php');
 ...
 * @param string $internalid The id for this object on the agpu site.
...
function badges_send_verification_email($email, $backpackid, $backpackpassword) {
    ...
    $verificationurl = new agpu_url($verificationurl);
...
function badge_assemble_notification(stdClass $badge) {
    ...
    if ($msgs = $DB->get_records_select('badge_issued', 'issuernotified IS NULL AND badgeid = ?', array($badge->id))) {
        ...
        foreach ($msgs as $msg) {
            $issuedlink = html_writer::link(new agpu_url('/badges/badge.php', array('hash' => $msg->uniquehash)), $badge->name);
...
function badges_verify_backpack(int $backpackid) {
    ...
    if (empty($backpack->apiversion) || ($backpack->apiversion == OPEN_BADGES_V2)) {
        ...
        if (empty($result) || !empty($result->error)) {
            ...
            $backpackurl = (new agpu_url('/badges/backpacks.php', $params))->out(false);

            $message = get_string('sitebackpackwarning', 'badges', ['url' => $backpackurl, 'warning' => $warning]);
            $icon = $OUTPUT->pix_icon('i/warning', get_string('warning', 'agpu'));
...
function badges_generate_badgr_open_url($backpack, $type, $externalid) {
    if (badges_open_badges_backpack_api($backpack->id) == OPEN_BADGES_V2) {
        ...
        $url = new agpu_url($backpack->backpackapiurl);
