...
/**
 * A javascript module to handle agpuNet ajax actions.
 *
 * @module     core/agpunet/service
...
export const getActivityInformation = (cmId) => {
    const request = {
        methodname: 'core_agpunet_get_share_info_activity',
        args: {
            cmid: cmId
        }
    };
...
export const getCourseInformation = (courseID) => {
    const request = {
        methodname: 'core_agpunet_get_shared_course_info',
        args: {
            courseid: courseID
        }
    };
...
export const sendCourse = (issuerId, courseId, shareFormat) => {
    const request = {
        methodname: 'core_agpunet_send_course',
        args: {
            issuerid: issuerId,
            courseid: courseId,
...
export const sendActivity = (issuerId, cmId, shareFormat) => {
    const request = {
        methodname: 'core_agpunet_send_activity',
        args: {
            issuerid: issuerId,
            cmid: cmId,
            shareformat: shareFormat,
...
/**
 * Send the selected activities in a course to agpuNet.
 *
 * @param {Integer} issuerId The OAuth 2 issuer ID.
 * @param {Integer} courseId The course ID.
...
/**
 * Check if the user is already authorized with agpuNet.
 *
 * @param {Integer} issuerId The OAuth 2 issuer ID.
 * @param {Integer} courseId The course ID.
 * @return {promise}
...
export const authorizationCheck = (issuerId, courseId) => {
    const request = {
        methodname: 'core_agpunet_auth_check',
        args: {
            issuerid: issuerId,
            courseid: courseId,
        }
