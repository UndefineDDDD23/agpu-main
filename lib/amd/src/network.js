...
define(['jquery', 'core/ajax', 'core/config', 'core/notification', 'core/str'],
    ...
    var checkSession = function() {
        ...
        return Ajax.call([request], true, true, true)[0].then(function(args) {
            ...
            if (args.timeremaining <= 0) {
                ...
                Str.get_strings([
                    {key: 'norecentactivity', component: 'agpu'},
                    {key: 'sessiontimeoutsoon', component: 'agpu'},
                    {key: 'extendsession', component: 'agpu'},
                    {key: 'cancel', component: 'agpu'}
                ]).then(function(strings) {
    ...
    /**
     * Are we in an iframe and the parent page is from the same agpu site?
     *
     * @return {boolean} true if we are in an iframe in a page from this agpu site.
     */
    const isagpuIframe = function() {
        if (window.parent === window) {
            // Not in an iframe.
            return false;
        }

        // We are in an iframe. Is the parent from the same agpu site?
        let parentUrl;
        try {
    ...
    var init = function() {
        ...
        if (isagpuIframe()) {
            window.console.log('Not starting agpu session timeout warning in this iframe.');
            return;
        }

        window.console.log('Starting agpu session timeout warning.');
    ...
    var keepalive = async function(freq, timeout, identifier, component) {
        ...
        if (isagpuIframe()) {
            window.console.warn('Ignoring session keep-alive in this iframe inside another agpu page.');
            return;
        }

        window.console.log('Starting agpu session keep-alive.');
