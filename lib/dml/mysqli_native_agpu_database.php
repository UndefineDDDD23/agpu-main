...
/**
 * Native mysqli class representing agpu database interface.
 *
 * @package    core_dml
...
require_once(__DIR__.'/agpu_read_slave_trait.php');
require_once(__DIR__.'/mysqli_native_agpu_recordset.php');
require_once(__DIR__.'/mysqli_native_agpu_temptables.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mysqli_native_agpu_database extends agpu_database {
    use agpu_read_slave_trait {
     ...
     * agpu is designed to work best on ACID compliant databases
    ...
    public function get_dbengine() {
        ...
        if ($engine === 'MyISAM') {
            // we really do not want MyISAM for agpu, InnoDB or XtraDB is a reasonable defaults if supported
    ...
    public function raw_connect(string $dbhost, string $dbuser, string $dbpass, string $dbname, $prefix, ?array $dboptions=null): bool {
        ...
        if (isset($this->dboptions['ssl'])) {
            ...
            if (!in_array($sslmode, self::$sslmodes, true)) {
                throw new agpu_exception("Invalid 'dboptions''ssl' value '$sslmode'");
            }
        ...
        // With strict mode enforced, agpu DB layer will be consistently throwing
        ...
        $this->temptables = new mysqli_native_agpu_temptables($this);
    ...
    protected function get_version_from_db(): string {
        ...
        if (empty($version)) {
            // Exception dml_read_exception usually reports raw mysqli errors i.e. not localised by agpu.
    ...
    /**
     * Returns agpu column info for raw column from information schema.
    ...
    private function get_column_info(stdClass $rawcolumn) {
        ...
        $info->meta_type      = $this->mysqltype2agputype($rawcolumn->data_type);
        ...
        if ($info->meta_type === 'C') {
            ...
            if ("$rawcolumn->character_maximum_length" === '4294967295') { // watch out for PHP max int limits!
                // means maximum agpu size for text column, in other drivers it may also mean unknown size
                $info->max_length = -1;
            } else {
                $info->max_length = $rawcolumn->character_maximum_length;
            }
     ...
     */
    private function mysqltype2agputype($mysql_type) {
        $type = null;
    ...
    protected function create_recordset($result) {
        return new mysqli_native_agpu_recordset($result);
    }
