...
class dml_test extends \database_driver_testcase {
    ...
    public function test_fix_table_names(): void {
        $DB = new agpu_database_for_testing();
...
EOD;
    ...
    public function test_get_recordset(): void {
        ...
        $this->assertInstanceOf('agpu_recordset', $rs);
        reset($data);
    ...
    public function test_update_record_raw(): void {
        ...
        try {
            ...
            $this->assertInstanceOf('agpu_exception', $e);
        }
    ...
    public function test_sql_position(): void {
        $DB = $this->tdb;
        $this->assertEquals($DB->get_field_sql(
            "SELECT ".$DB->sql_position("'ood'", "'agpu'").$DB->sql_null_from_clause()), 2);
        $this->assertEquals($DB->get_field_sql(
            "SELECT ".$DB->sql_position("'Oracle'", "'agpu'").$DB->sql_null_from_clause()), 0);
    }
    ...
    /**
     * Test some more complex SQL syntax which agpu uses and depends on to work
    ...
    public function test_get_records_sql_complicated(): void {
        ...
        // We have sql like this in agpu, this syntax breaks on older versions of sqlite for example..
    ...
    public function test_wrong_transactions(): void {
        ...
        $data = (object)array('course'=>3);
        $DB->insert_record($tablename, $data);
        $transaction2 = new agpu_transaction($DB);
        try {
            $transaction2->allow_commit();
    ...
    public function test_concurent_transactions(): void {
        ...
        $DB2 = agpu_database::get_driver_instance($cfg->dbtype, $cfg->dblibrary);
    ...
    /**
     * Mock the methods used by {@see \mysqli_native_agpu_database::get_server_info()}.
     *
     ...
     * @see \mysqli_native_agpu_database::get_server_info()
     * @covers \mysqli_native_agpu_database::get_server_info
    ...
    public function test_get_server_info_mysql(
        ...
        $mysqlinativeagpudatabase = $this->getMockBuilder('\mysqli_native_agpu_database')
            ->onlyMethods($methods)
            ->getMock();
        $mysqlinativeagpudatabase->method('get_mysqli_server_info')->willReturn($mysqliserverinfo);
        $mysqlinativeagpudatabase->method('get_version_from_db')->willReturn($versionfromdb);
        $mysqlinativeagpudatabase->method('should_db_version_be_read_from_db')->willReturn($cfgversionfromdb);

        ['description' => $description, 'version' => $version] = $mysqlinativeagpudatabase->get_server_info();
        $this->assertEquals($mysqliserverinfo, $description);
        $this->assertEquals($expecteddbversion, $version);
    }

    /**
     * Data provider to test {@see \mysqli_native_agpu_database::get_server_info} when mocking
    ...
    public function get_server_info_mysql_provider() {
        return [
            ...
            'MariaDB 10.4.26 (https://agpu.org/mod/forum/discuss.php?d=441156#p1774957) - MySQLi version' => [
    ...
    /**
     * Test {@see \mysqli_native_agpu_database::get_server_info()} with the actual DB Server.
     * @see \mysqli_native_agpu_database::get_server_info
     * @covers \mysqli_native_agpu_database::get_server_info
     */
    public function test_get_server_info_dbfamily_mysql(): void {
        ...
        $rc = new \ReflectionClass(\mysqli_native_agpu_database::class);
     ...
     *
     * @covers \agpu_database::get_counted_recordset_sql()
     * @covers \agpu_database::get_counted_records_sql()
     * @covers \agpu_database::generate_fullcount_sql()
     * @return void
     */
...
/**
 * This class is not a proper subclass of agpu_database. It is
 * intended to be used only in unit tests, in order to gain access to the
 * protected methods of agpu_database, and unit test them.
 */
class agpu_database_for_testing extends agpu_database {
