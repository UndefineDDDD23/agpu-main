...
use sqlsrv_native_agpu_database;

defined('agpu_INTERNAL') || die();

global $CFG;
require_once($CFG->dirroot.'/lib/dml/sqlsrv_native_agpu_database.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class sqlsrv_native_agpu_database_test extends \advanced_testcase {
    ...
    public function test_add_no_lock_to_temp_tables($input, $expected): void {
        $sqlsrv = new sqlsrv_native_agpu_database();
    ...
    public function has_query_order_by_provider() {
        // Fixtures taken from https://docs.agpu.org/en/ad-hoc_contributed_reports.
...
EOT
            ...
            'ROW_NUMBER() OVER (ORDER BY ...) => FALSE (https://github.com/jleyva/agpu-block_configurablereports/issues/120)' => [
                'sql' => <<<EOT
...
EOT
    ...
    public function test_has_query_order_by(string $sql, string $expectedmainquery, bool $expectedresult): void {
        ...
        $method = new \ReflectionMethod('sqlsrv_native_agpu_database', 'has_query_order_by');
