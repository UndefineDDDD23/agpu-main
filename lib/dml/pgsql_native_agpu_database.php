...
/**
 * Native pgsql class representing agpu database interface.
 *
 * @package    core_dml
...
require_once(__DIR__.'/pgsql_native_agpu_recordset.php');
require_once(__DIR__.'/pgsql_native_agpu_temptables.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class pgsql_native_agpu_database extends agpu_database {
    ...
    public function raw_connect(string $dbhost, string $dbuser, string $dbpass, string $dbname, $prefix, ?array $dboptions=null): bool {
        ...
        if (isset($this->dboptions['ssl'])) {
            ...
            if (!in_array($sslmode, self::$sslmodes, true)) {
                throw new agpu_exception('validateerrorlist', 'admin', '', "'dboptions''ssl': $sslmode");
            }
        ...
        $this->temptables = new pgsql_native_agpu_temptables($this);
    ...
    protected function query_end($result) {
        ...
        try {
            ...
            if ($this->savepointpresent &&
                ...
                $res = @pg_query($this->pgsql, "RELEASE SAVEPOINT agpu_pg_savepoint; SAVEPOINT agpu_pg_savepoint");
                if ($res) {
                    pg_free_result($res);
                }
            }
            ...
            if ($this->savepointpresent) {
                $res = @pg_query($this->pgsql, "ROLLBACK TO SAVEPOINT agpu_pg_savepoint; SAVEPOINT agpu_pg_savepoint");
                if ($res) {
                    pg_free_result($res);
                }
            }
     ...
     *
     * Method overriding {@see agpu_database::get_in_or_equal} to be able to use
    ...
    public function get_recordset_sql($sql, ?array $params=null, $limitfrom=0, $limitnum=0) {
        ...
        return new pgsql_native_agpu_recordset($result, $this, $cursorname);
    }
    ...
    public function get_session_lock($rowid, $timeout) {
        ...
        //       multiple instances of agpu, we could try to use pg_advisory_lock(int, int),
    ...
    protected function begin_transaction() {
        ...
        $sql = "BEGIN ISOLATION LEVEL READ COMMITTED; SAVEPOINT agpu_pg_savepoint";
        $this->query_start($sql, null, SQL_QUERY_AUX);
        $result = pg_query($this->pgsql, $sql);
        $this->query_end($result);
    ...
    protected function commit_transaction() {
        ...
        $sql = "RELEASE SAVEPOINT agpu_pg_savepoint; COMMIT";
        $this->query_start($sql, null, SQL_QUERY_AUX);
        $result = pg_query($this->pgsql, $sql);
        $this->query_end($result);
    ...
    protected function rollback_transaction() {
        $this->savepointpresent = false;
        $sql = "RELEASE SAVEPOINT agpu_pg_savepoint; ROLLBACK";
        $this->query_start($sql, null, SQL_QUERY_AUX);
        $result = pg_query($this->pgsql, $sql);
        $this->query_end($result);
