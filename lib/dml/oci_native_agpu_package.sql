 ...
 * cross-db compatibility in the agpu 2.x DB API with some operations
 * not natively supported by Oracle, namely:
 *  - locking: Application locks used by agpu DB sessions. It uses
 *             the DBMS_LOCK package so execution must be granted
 *             to the agpu DB user by SYS to work properly.
 ...
 *
 * agpu will not parse this file correctly if it uses Windows line endings.
 */

CREATE OR REPLACE PACKAGE agpuLIB AS
...
CREATE OR REPLACE PACKAGE BODY agpuLIB AS
...
BEGIN
    RETURN agpuLIB.BITOR(value1,value2) - BITAND(value1,value2);
END BITXOR;
...
BEGIN
    ...
    stringresult := CONCAT(CONCAT(agpuLIB.UNDO_DIRTY_HACK(string1), agpuLIB.UNDO_DIRTY_HACK(string2)), agpuLIB.UNDO_DIRTY_HACK(string3));
