...
/**
 * formslib.php - library of classes for creating forms in agpu, based on PEAR QuickForms.
 *
 ...
 * extend agpuform overriding abstract classes definition and optionally defintion_after_data
 * and validation.
 *
...
/**
 * Wrapper that separates quickforms syntax from agpu code
 *
 * agpu specific wrapper that separates quickforms syntax from agpu code. You won't directly
 * use this class you should write a class definition which extends this class or a more specific
 * subclass such a agpuform_mod for each form you want to display and/or process with formslib.
 *
 ...
 */
abstract class agpuform {
    /** @var string name of the form */
    protected $_formname;       // form name

    /** @var agpuQuickForm quickform object definition */
    protected $_form;
     ...
     * the name you gave the class extending agpuform. You should call your class something
     * like
     *
     * @param mixed $action the action attribute for the form. If empty defaults to auto detect the
     *              current url. If a agpu_url object then outputs params as hidden variables.
    ...
    public function __construct($action=null, $customdata=null, $method='post', $target='', $attributes=null, $editable=true,
                                $ajaxformdata=null) {
        global $CFG, $FULLME;
        // no standard mform in agpu should allow autocomplete with the exception of user signup
        if (empty($attributes)) {
        ...
        $this->_form = new agpuQuickForm($this->_formname, $method, $action, $target, $attributes, $ajaxformdata);
        if (!$editable){
     ...
     */
    public function agpuform($action=null, $customdata=null, $method='post', $target='', $attributes=null, $editable=true) {
    ...
    function validate_defined_fields($validateonnosubmit=false) {
        ...
        if ($this->no_submit_button_pressed() && empty($validateonnosubmit)){
            ...
            $agpu_val = $this->validation($data, $files);
            if ((is_array($agpu_val) && count($agpu_val)!==0)) {
                // non-empty array means errors
                foreach ($agpu_val as $element=>$msg) {
                    $mform->setElementError($element, $msg);
                }
                $agpu_val = false;
            ...
            $this->_validated = ($internal_val and $element_val and $agpu_val and $file_val);
        }
     ...
     *
     * @deprecated since agpu 2.0
     * @todo MDL-31294 remove this api
     * @see agpuform::save_stored_file()
     * @see agpuform::save_file()
    ...
    function get_new_filename($elname=null) {
        ...
        if ($element instanceof agpuQuickForm_filepicker || $element instanceof agpuQuickForm_filemanager) {
     ...
     * This is a protected method which will be used only inside agpuforms
     *
     * @param string $elname name of element
    ...
    function save_stored_file($elname, $newcontextid, $newcomponent, $newfilearea, $newitemid, $newfilepath='/',
        ...
        if ($element instanceof agpuQuickForm_filepicker) {
            $values = $this->_form->exportValues($elname);
            if (empty($values[$elname])) {
                return false;
            }
     ...
     * IntermediateClass extends agpuform'.
     *
    ...
    function add_checkbox_controller($groupid, $text = null, $attributes = null, $originalValue = 0) {
        ...
        if (!is_null($contollerbutton) || is_null($selectvalue)) {
            foreach ($mform->_elements as $element) {
                if (($element instanceof agpuQuickForm_advcheckbox) &&
        ...
        $PAGE->requires->yui_module('agpu-form-checkboxcontroller', 'M.form.checkboxcontroller',
                array(
        ...
        $submitlink = new agpuQuickForm_submit($checkboxcontrollername, $attributes);
     ...
     * @deprecated since agpu 3.3 MDL-57471
     */
    ...
    /**
     * Used by tests to generate valid submit keys for agpu forms that are
     * submitted with ajax data.
     *
     * @throws \agpu_exception If called outside unit test environment
    ...
    public static function mock_generate_submit_keys($data = []) {
        if (!defined('PHPUNIT_TEST') || !PHPUNIT_TEST) {
            throw new \agpu_exception("This function can only be used for unit testing.");
        }
...
/**
 * agpuQuickForm implementation
 *
 * You never extend this class directly. The class methods of this class are available from
 * the private $this->_form property on agpuform and its children. You generally only
 * call methods on this class from within abstract methods that you override on agpuform such
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class agpuQuickForm extends HTML_QuickForm_DHTMLRulesTableless {
    ...
    /**
     * String with the html for hidden params passed in as part of a agpu_url
     ...
     * @param string|agpu_url $action Form's action
     ...
     * @deprecated since agpu 3.1
     */
    public function agpuQuickForm($formName, $method, $action, $target='', $attributes=null) {
        debugging('Use of class name as constructor is deprecated', DEBUG_DEVELOPER);
    ...
    */
     ...
     *        {@link lib/agpulib.php} for defined parameter types
     */
    ...
    public function getCleanedValue($value, $type) {
        if (is_array($type) && is_array($value)) {
            ...
            throw new coding_exception('Unexpected type or value received in agpuQuickForm::getCleanedValue()');
        }
        return $value;
    }
     ...
     * no data is loaded in using agpuform::set_data()
     *
     * note: $slashed param removed
     *
                ...
                if ('client' == $rule['validation']) {
                    ...
                    if (is_object($element) && $element->getType() == 'editor') {
                        if ($element->getType() == 'editor') {
                            ...
                            // Add format to rule as agpuform check which format is supported by browser
        ...
        if (is_a($element, 'HTML_QuickForm_group')) {
            ...
            foreach ($elsInGroup as $elInGroup){
                if (is_a($elInGroup, 'HTML_QuickForm_group')) {
                    ...
                    // We will be appending group name again in agpuQuickForm_group::export_for_template().
...
/**
 * agpuQuickForm renderer
 *
 * A renderer for agpuQuickForm that only uses XHTML and CSS and no
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class agpuQuickForm_Renderer extends HTML_QuickForm_Renderer_Tableless{
     ...
     * @deprecated since agpu 3.1
     */
    public function agpuQuickForm_Renderer() {
        debugging('Use of class name as constructor is deprecated', DEBUG_DEVELOPER);
        self::__construct();
    }
     ...
     *
     * @param agpuQuickForm $form reference of the form
     */
    function startForm(&$form){
        ...
        if (!empty($this->_collapsibleElements)) {
            ...
            $PAGE->requires->yui_module('agpu-form-shortforms', 'M.form.shortforms', array(array('formid' => $formid)));
        }
     ...
     *
     * @param agpuQuickForm_group $group Passed by reference
     ...
     *
     * @global agpu_page $PAGE
     * @param agpuQuickForm $form Passed by reference
     */
    function finishForm(&$form){
        ...
        if (!$form->isFrozen()) {
            ...
            if (count($args[1]) > 0) {
                $PAGE->requires->js_init_call('M.form.initFormDependencies', $args, true, agpuform::get_js_module());
            }
        }
    }
   /**
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class agpuQuickForm_Rule_Required extends HTML_QuickForm_Rule {
    /**
 ...
 */
$GLOBALS['_HTML_QuickForm_default_renderer'] = new agpuQuickForm_Renderer();

/** Please keep this list in alphabetical order. */
agpuQuickForm::registerElementType('advcheckbox', "$CFG->libdir/form/advcheckbox.php", 'agpuQuickForm_advcheckbox');
agpuQuickForm::registerElementType('autocomplete', "$CFG->libdir/form/autocomplete.php", 'agpuQuickForm_autocomplete');
agpuQuickForm::registerElementType('button', "$CFG->libdir/form/button.php", 'agpuQuickForm_button');
agpuQuickForm::registerElementType('cancel', "$CFG->libdir/form/cancel.php", 'agpuQuickForm_cancel');
agpuQuickForm::registerElementType('course', "$CFG->libdir/form/course.php", 'agpuQuickForm_course');
agpuQuickForm::registerElementType('cohort', "$CFG->libdir/form/cohort.php", 'agpuQuickForm_cohort');
agpuQuickForm::registerElementType('searchableselector', "$CFG->libdir/form/searchableselector.php", 'agpuQuickForm_searchableselector');
agpuQuickForm::registerElementType('checkbox', "$CFG->libdir/form/checkbox.php", 'agpuQuickForm_checkbox');
agpuQuickForm::registerElementType('choicedropdown', "$CFG->libdir/form/choicedropdown.php", 'agpuQuickForm_choicedropdown');
agpuQuickForm::registerElementType('date_selector', "$CFG->libdir/form/dateselector.php", 'agpuQuickForm_date_selector');
agpuQuickForm::registerElementType('date_time_selector', "$CFG->libdir/form/datetimeselector.php", 'agpuQuickForm_date_time_selector');
agpuQuickForm::registerElementType('duration', "$CFG->libdir/form/duration.php", 'agpuQuickForm_duration');
agpuQuickForm::registerElementType('editor', "$CFG->libdir/form/editor.php", 'agpuQuickForm_editor');
agpuQuickForm::registerElementType('filemanager', "$CFG->libdir/form/filemanager.php", 'agpuQuickForm_filemanager');
agpuQuickForm::registerElementType('filepicker', "$CFG->libdir/form/filepicker.php", 'agpuQuickForm_filepicker');
agpuQuickForm::registerElementType('filetypes', "$CFG->libdir/form/filetypes.php", 'agpuQuickForm_filetypes');
agpuQuickForm::registerElementType('float', "$CFG->libdir/form/float.php", 'agpuQuickForm_float');
agpuQuickForm::registerElementType('grading', "$CFG->libdir/form/grading.php", 'agpuQuickForm_grading');
agpuQuickForm::registerElementType('group', "$CFG->libdir/form/group.php", 'agpuQuickForm_group');
agpuQuickForm::registerElementType('header', "$CFG->libdir/form/header.php", 'agpuQuickForm_header');
agpuQuickForm::registerElementType('hidden', "$CFG->libdir/form/hidden.php", 'agpuQuickForm_hidden');
agpuQuickForm::registerElementType('listing', "$CFG->libdir/form/listing.php", 'agpuQuickForm_listing');
agpuQuickForm::registerElementType('defaultcustom', "$CFG->libdir/form/defaultcustom.php", 'agpuQuickForm_defaultcustom');
agpuQuickForm::registerElementType('modgrade', "$CFG->libdir/form/modgrade.php", 'agpuQuickForm_modgrade');
agpuQuickForm::registerElementType('modvisible', "$CFG->libdir/form/modvisible.php", 'agpuQuickForm_modvisible');
agpuQuickForm::registerElementType('password', "$CFG->libdir/form/password.php", 'agpuQuickForm_password');
agpuQuickForm::registerElementType('passwordunmask', "$CFG->libdir/form/passwordunmask.php", 'agpuQuickForm_passwordunmask');
agpuQuickForm::registerElementType('questioncategory', "$CFG->libdir/form/questioncategory.php", 'agpuQuickForm_questioncategory');
agpuQuickForm::registerElementType('radio', "$CFG->libdir/form/radio.php", 'agpuQuickForm_radio');
agpuQuickForm::registerElementType('recaptcha', "$CFG->libdir/form/recaptcha.php", 'agpuQuickForm_recaptcha');
agpuQuickForm::registerElementType('select', "$CFG->libdir/form/select.php", 'agpuQuickForm_select');
agpuQuickForm::registerElementType('selectgroups', "$CFG->libdir/form/selectgroups.php", 'agpuQuickForm_selectgroups');
agpuQuickForm::registerElementType('selectwithlink', "$CFG->libdir/form/selectwithlink.php", 'agpuQuickForm_selectwithlink');
agpuQuickForm::registerElementType('selectyesno', "$CFG->libdir/form/selectyesno.php", 'agpuQuickForm_selectyesno');
agpuQuickForm::registerElementType('static', "$CFG->libdir/form/static.php", 'agpuQuickForm_static');
agpuQuickForm::registerElementType('submit', "$CFG->libdir/form/submit.php", 'agpuQuickForm_submit');
agpuQuickForm::registerElementType('tags', "$CFG->libdir/form/tags.php", 'agpuQuickForm_tags');
agpuQuickForm::registerElementType('text', "$CFG->libdir/form/text.php", 'agpuQuickForm_text');
agpuQuickForm::registerElementType('textarea', "$CFG->libdir/form/textarea.php", 'agpuQuickForm_textarea');
agpuQuickForm::registerElementType('url', "$CFG->libdir/form/url.php", 'agpuQuickForm_url');
agpuQuickForm::registerElementType('warning', "$CFG->libdir/form/warning.php", 'agpuQuickForm_warning');

agpuQuickForm::registerRule('required', null, 'agpuQuickForm_Rule_Required', "$CFG->libdir/formslib.php");
