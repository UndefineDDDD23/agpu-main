...
M.form_dndupload.init = function(Y, options) {
    var dnduploadhelper = {
        ...
        update_progress: function(filename, percent) {
            ...
            this.progressbars[filename].progressinnertext.setContent(percent + '% ' + M.util.get_string('complete', 'agpu'));
        },
    ...
    Y.extend(dnduploader, Y.Base, {
        ...
        print_msg: function(msg, type) {
            var header = M.util.get_string('error', 'agpu');
            if (type != 'error') {
                type = 'info'; // one of only two types excepted
                header = M.util.get_string('info', 'agpu');
            }
            if (!this.msg_dlg) {
        ...
        display_size: function(size) {
            // This is snippet of code (with some changes) is from the display_size function in agpulib.
            var gb = M.util.get_string('sizegb', 'agpu'),
                mb = M.util.get_string('sizemb', 'agpu'),
                kb = M.util.get_string('sizekb', 'agpu'),
                b  = M.util.get_string('sizeb', 'agpu');
        ...
        add_to_upload_queue: function(file, filename, overwrite) {
            ...
            if (this.options.maxfiles >= 0 && this.getTotalNumberOfFiles() > this.options.maxfiles) {
                ...
                this.uploadqueue = [];
                this.renamequeue = [];
                this.print_msg(M.util.get_string('maxfilesreached', 'agpu', this.options.maxfiles), 'error');
                return false;
            }
            ...
            if (this.options.areamaxbytes > -1) {
                ...
                if (sizereached > this.options.areamaxbytes) {
                    this.uploadqueue = [];
                    this.renamequeue = [];
                    this.print_msg(M.util.get_string('maxareabytesreached', 'agpu'), 'error');
                    return false;
                }
            }
        ...
        process_renames: function() {
            ...
            node.one('.fp-dlg-text').setContent(M.util.get_string('fileexists', 'agpu', file.name));
