...
/**
 * Unit tests for agpuQuickForm_course
 *
 * Contains test cases for testing agpuQuickForm_course.
 *
 * @package    core_form
 * @category   test
...
class course_test extends \basic_testcase {
    ...
    public function test_constructor_attributes(): void {
        $attributes = [
            'exclude' => [1, 2],
            'requiredcapabilities' => ['agpu/course:update'],
        ];

        $element = new \agpuQuickForm_course('testel', null, $attributes);
        $html = $element->toHtml();
        $this->assertStringContainsString('data-exclude="1,2"', $html);
        $this->assertStringContainsString('data-requiredcapabilities="agpu/course:update"', $html);
