YUI.add('agpu-form-shortforms', function (Y, NAME) {

/**
 * Provides the form shortforms class.
 *
 * @module agpu-form-shortforms
 */
...
Y.extend(SHORTFORMS, Y.Base, {
    ...
    initializer: function() {
        ...
        if (!form) {
            Y.log('Could not locate the form', 'warn', 'agpu-form-shortforms');
            return;
        }
    ...
    set_state: function(fieldset, collapsed) {
        ...
        if (!statuselement) {
            Y.log("M.form.shortforms::switch_state was called on an fieldset without a status field: '" +
                fieldset.get('id') + "'", 'debug', 'agpu-form-shortforms');
            return this;
        }
...
}, '@VERSION@', {"requires": ["node", "base", "selector-css3", "agpu-core-event"]});
