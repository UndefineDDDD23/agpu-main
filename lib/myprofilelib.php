...
function core_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    ...
    if (!empty($course)) {
        if (user_can_view_profile($user, null, $usercontext)) {
            $url = new agpu_url('/user/profile.php', array('id' => $user->id));
    ...
    if (isloggedin() && !isguestuser($user) && !is_mnet_remote_user($user)) {
        if (($iscurrentuser || is_siteadmin($USER) || !is_siteadmin($user)) && has_capability('agpu/user:update',
                    $systemcontext)) {
            $url = new agpu_url('/user/editadvanced.php', array('id' => $user->id, 'course' => $courseid,
                'returnto' => 'profile'));
            ...
            $tree->add_node($node);
                   ...
                   || ($iscurrentuser && has_capability('agpu/user:editownprofile', $systemcontext))) {
    ...
    if (!$iscurrentuser && $PAGE->settingsnav->can_view_user_preferences($user->id)) {
        $url = new agpu_url('/user/preferences.php', array('userid' => $user->id));
        $title = get_string('preferences', 'agpu');
    ...
    if (!$user->deleted && !$iscurrentuser &&
                !\core\session\manager::is_loggedinas() && has_capability('agpu/user:loginas',
                $courseorsystemcontext) && !is_siteadmin($user->id)) {
        $url = new agpu_url('/course/loginas.php',
    ...
    // Contact details.
    if (has_capability('agpu/user:viewhiddendetails', $courseorusercontext)) {
        $hiddenfields = array();
    } else {
    ...
    if ($iscurrentuser
        or (!isset($hiddenfields['email']) and (
            ...
            or has_capability('agpu/course:useremail', $courseorusercontext) // TODO: Deprecate/remove for MDL-37479.
        ))
    ...
    if (!isset($hiddenfields['agpunetprofile']) && $user->agpunetprofile) {
        $node = new core_user\output\myprofile\node('contact', 'agpunetprofile', get_string('agpunetprofile', 'user'), null,
                null, $user->agpunetprofile);
        $tree->add_node($node);
    }
        ...
        if ($mycourses = enrol_get_all_users_courses($user->id, true, null)) {
            ...
            foreach ($mycourses as $mycourse) {
                if ($mycourse->category) {
                    ...
                    if (!isset($course) || $mycourse->id != $course->id) {
                        ...
                        if ($mycourse->visible == 0) {
                            if (!has_capability('agpu/course:viewhiddencourses', $ccontext)) {
                                continue;
                            }
                        ...
                        $url = new agpu_url('/user/view.php', $params);
                ...
                if (!$showallcourses && $shown == $CFG->navcourselimit) {
                    $url = null;
                    if (isset($course)) {
                        $url = new agpu_url('/user/view.php',
    ...
    if (!empty($course)) {
        ...
        if (!isset($hiddenfields['groups']) && !empty($course)) {
            $accessallgroups = has_capability('agpu/site:accessallgroups', $courseorsystemcontext);
    ...
    // Last ip.
    if (has_capability('agpu/user:viewlastip', $usercontext) && !isset($hiddenfields['lastip'])) {
        if ($user->lastip) {
            $iplookupurl = new agpu_url('/iplookup/index.php', array('ip' => $user->lastip, 'user' => $user->id));
