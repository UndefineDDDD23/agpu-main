...
abstract class testing_module_generator extends component_generator_base {
    ...
    public function create_instance($record = null, ?array $options = null) {
        ...
        if (empty($record->introeditor) && empty($record->introformat)) {
            $record->introformat = FORMAT_agpu;
        }
        ...
        // Before agpu 2.6 it was possible to create a module with completion tracking when
        ...
        if (!empty($record->lang) && !has_capability('agpu/course:setforcedlanguage', context_course::instance($course->id))) {
            throw new coding_exception('Attempt to generate an activity when the current user does not have ' .
                    'permission agpu/course:setforcedlanguage. This does not work.');
        }
