 ...
 * @param string $agpupath starting with "/", ex: "/admin/tool/cli/init.php"
 * @return string path relative to current directory or absolute path
 */
function testing_cli_argument_path($agpupath) {
    global $CFG;

    if (isset($CFG->admin) and $CFG->admin !== 'admin') {
        $agpupath = preg_replace('|^/admin/|', "/$CFG->admin/", $agpupath);
    }
    ...
    $path = realpath($CFG->dirroot.$agpupath);
...
function testing_update_composer_dependencies(bool $selfupdate = true, bool $updatedependencies = true): void {
    ...
    // Switch to agpu's dirroot for easier path handling.
    chdir($dirroot);
 ...
 * OS. agpu_DIRECTORY_SEPARATOR should only be used when parsing
