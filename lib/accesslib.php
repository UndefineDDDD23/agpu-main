...
/** Capability allow management of trusts - NOT IMPLEMENTED YET - see {@link https://agpudev.io/docs/apis/subsystems/roles} */
define('RISK_MANAGETRUST', 0x0001);
/** Capability allows changes in system configuration - see {@link https://agpudev.io/docs/apis/subsystems/roles} */
define('RISK_CONFIG',      0x0002);
/** Capability allows user to add scripted content - see {@link https://agpudev.io/docs/apis/subsystems/roles} */
define('RISK_XSS',         0x0004);
/** Capability allows access to personal user information - see {@link https://agpudev.io/docs/apis/subsystems/roles} */
define('RISK_PERSONAL',    0x0008);
/** Capability allows users to add content others may see - see {@link https://agpudev.io/docs/apis/subsystems/roles} */
define('RISK_SPAM',        0x0010);
/** capability allows mass delete of data belonging to other users - see {@link https://agpudev.io/docs/apis/subsystems/roles} */
...
function has_capability($capability, context $context, $user = null, $doanything = true) {
    ...
    if (strpos($capability, 'agpu/legacy:') === 0) {
    ...
    if (!empty($CFG->contextlocking)) {
        if ($capinfo->captype === 'write' && $context->locked) {
            ...
            // It does not apply to agpu/site:managecontextlocks - this is to allow context locking to be unlocked.
            if ($capinfo->name !== 'agpu/site:managecontextlocks') {
...
function guess_if_creator_will_have_course_capability($capability, context $context, $user = null) {
    ...
    if (!has_capability('agpu/course:create', $context, $user)) {
        return false;
    }
    ...
    if ($context->contextlevel == CONTEXT_COURSE) {
        if (is_viewing($context, $user, 'agpu/role:assign') or is_enrolled($context, $user, 'agpu/role:assign')) {
            return false;
        }
    } else {
        if (has_capability('agpu/course:view', $context, $user) and has_capability('agpu/role:assign', $context, $user)) {
            return false;
        }
    }
 ...
 *
 * NOTE before agpu 2.0, this function attempted to make an appropriate
...
function reload_all_capabilities() {
    ...
    foreach ($sw as $path => $roleid) {
        if ($record = $DB->get_record('context', array('path'=>$path))) {
            ...
            if (has_capability('agpu/role:switchroles', $context)) {
...
function assign_legacy_capabilities($capability, $legacyperms) {
    ...
    foreach ($legacyperms as $type => $perm) {
        ...
        if (!array_key_exists($type, $archetypes)) {
            throw new \agpu_exception('invalidlegacy', '', '', $type);
        }
...
function create_role($name, $shortname, $description, $archetype = '') {
    ...
    if (strpos($archetype, 'agpu/legacy:') !== false) {
...
function is_guest(context $context, $user = null) {
    ...
    if (has_capability('agpu/course:view', $coursecontext, $user)) {
...
/**
 * Returns true if the user has agpu/course:view capability in the course,
...
function can_access_course(stdClass $course, $user = null, $withcapability = '', $onlyactive = false) {
    ...
    if (!$course->visible and !has_capability('agpu/course:viewhiddencourses', $coursecontext, $userid)) {
        return false;
    }
 ...
 *
 * @access private
 * @param string $component full plugin name, examples: 'agpu', 'mod_forum'
 ...
 *
 * @access private
 * @param string $component - examples: 'agpu', 'mod_forum'
 * @return array array of capabilities
 */
function get_cached_capabilities($component = 'agpu') {
    global $DB;
 ...
 * If no parameters are given, the function updates the core agpu
 * capabilities.
 *
 ...
 *
 * @access private
 * @param string $component examples: 'agpu', 'mod_forum', 'block_activity_results'
 * @return boolean true if success, exception in case of any problems
 */
function update_capabilities($component = 'agpu') {
    global $DB, $OUTPUT;
                ...
                if ($roles = get_roles_with_capability($cachedcap->name)) {
                    foreach ($roles as $role) {
                        if (!unassign_capability($cachedcap->name, $role->id)) {
                            throw new \agpu_exception('cannotunassigncap', 'error', '',
...
/**
 * Return a link to agpu docs for a given capability name
 *
 * @param stdClass $capability a capability - a row from the mdl_capabilities table.
 * @return string the human-readable capability name as a link to agpu Docs.
 */
...
function get_component_string($component, $contextlevel) {
    ...
    if ($component === 'agpu' || $component === 'core') {
...
function get_profile_roles(context $context) {
    ...
    if (has_capability('agpu/role:assign', $context)) {
    ...
    if ($roles = $DB->get_records_sql($sql, $params)) {
        ...
        foreach ($roles as $roleid => $unused) {
            if (isset($viewableroles[$roleid])) {
                $url = new agpu_url('/user/index.php', ['contextid' => $context->id, 'roleid' => $roleid]);
 ...
 * to cause an out-of-memory error on large agpu sites, so this combination is deprecated and
...
function get_users_roles(context $context, $userids = [], $checkparentcontexts = true, $order = 'c.contextlevel DESC, r.sortorder ASC') {
    ...
    if (!$userids && $checkparentcontexts) {
        debugging('Please do not call get_users_roles() with $checkparentcontexts = true ' .
                'and $userids array not set. This combination causes large agpu sites ' .
...
function get_assignable_roles(context $context, $rolenamedisplay = ROLENAME_ALIAS, $withusercounts = false, $user = null) {
    ...
    if (!has_capability('agpu/role:assign', $context, $userid)) {
        if ($withusercounts) {
 ...
 * test the agpu/role:switchroles to see if the user is allowed to switch in the first place.
 *
...
function get_overridable_roles(context $context, $rolenamedisplay = ROLENAME_ALIAS, $withcounts = false) {
    ...
    if (!has_any_capability(array('agpu/role:safeoverride', 'agpu/role:override'), $context)) {
        if ($withcounts) {
            return array(array(), array(), array());
        } else {
            return array();
...
function get_with_capability_join(context $context, $capability, $useridcolumn) {
    ...
    if (!empty($CFG->contextlocking) && $context->locked) {
        ...
        $excludelockedcapsparams['managelockscap'] = 'agpu/site:managecontextlocks';
    }
 ...
 *               have capability both $capability and agpu/site:accessallgroups
...
function get_users_by_capability(context $context, $capability, $fields = '', $sort = '', $limitfrom = '', $limitnum = '',
    ...
    if ($groups) {
        ...
        if ($useviewallgroups) {
            $viewallgroupsusers = get_users_by_capability($context, 'agpu/site:accessallgroups', 'u.id, u.id', '', '', '', '', $exceptions);
 ...
 *
 * Note that agpu is based on capabilities and it is usually better
