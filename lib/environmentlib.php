 ...
 * tests of required versions and libraries to run agpu. It can be
 ...
 */
function check_agpu_environment($version, $env_select = ENV_SELECT_NEWER) {
...
/**
 * This function will return the list of agpu versions available
 *
...
function get_list_of_environment_versions($contents) {
    ...
    if (isset($contents['COMPATIBILITY_MATRIX']['#']['agpu'])) {
        foreach ($contents['COMPATIBILITY_MATRIX']['#']['agpu'] as $version) {
...
/**
 * This function will return the xmlized data belonging to one agpu version
 *
 * @param string $version top version from which we start to look backwards
 * @param int|string $env_select one of ENV_SELECT_NEWER | ENV_SELECT_DATAROOT | ENV_SELECT_RELEASE decide xml to use. String means plugin name.
 * @return mixed the xmlized structure or false on error
...
/// Detect available versions
    ...
    // be matched against any version of agpu.
...
/// We now we have it. Extract from full contents.
    ...
    return $contents['COMPATIBILITY_MATRIX']['#']['agpu'][$fl_arr[$version]];
}
...
/// Only run the agpu versions checker on upgrade, not on install
    if (!empty($CFG->version)) {
        $results[] = environment_check_agpu($version, $env_select);
    }
...
/// Get current agpu version (release) for later compare
...
/**
 * This function will check if agpu requirements are satisfied
 *
 * @uses NO_VERSION_DATA_FOUND
 * @param string $version xml version we are going to use to test this server
 * @param int|string $env_select one of ENV_SELECT_NEWER | ENV_SELECT_DATAROOT | ENV_SELECT_RELEASE decide xml to use. String means plugin name.
 ...
 */
function environment_check_agpu($version, $env_select) {

    $result = new environment_results('agpu');
...
/// Extract the agpu part
    if (!isset($data['@']['requires'])) {
        $needed_version = '1.0'; /// Default to 1.0 if no agpu requires is found
    } else {
    /// Extract required agpu version
...
/**
 * Looks for buggy PCRE implementation, we need unicode support in agpu...
 * @param string $version xml version we are going to use to test this server
 * @param int|string $env_select one of ENV_SELECT_NEWER | ENV_SELECT_DATAROOT | ENV_SELECT_RELEASE decide xml to use. String means plugin name.
