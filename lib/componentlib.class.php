 ...
 * download and install feature of agpu, used to keep updated some
 * items like languages, pear, enviroment... i.e, components.
 *
 * It has been developed harcoding some important limits that are
 * explained below:
 *    - It only can check, download and install items under agpudata.
 ...
 *     if ($cd = new component_installer('https://download.agpu.org', 'langpack/2.0',
 ...
 *                     $a = new stdClass();
 *                     $a->url = 'https://download.agpu.org/langpack/2.0/es.zip';
 *                     $a->dest= $CFG->dataroot.'/lang';
 *                     throw new \agpu_exception($cd->get_error(), 'error', '', $a);
 *                 } else {
 *                     throw new \agpu_exception($cd->get_error(), 'error');
 *                 }
 *                 break;
 ...
 * download.agpu.org to the agpudata directory.
 *
...
class component_installer {
    ...
    var $destpath;     /// Relative path (from agpudata) where the .zip
     ...
     *               than zipfilename)
     * @param string $destpath Relative path (from agpudata) where the .zip file will
     *               be expanded (default='' = agpudataitself)
     * @return object
     */
    ...
    /// Check for correct sourcebase (this will be out in the future)
        if (!PHPUNIT_TEST and $this->sourcebase != 'https://download.agpu.org') {
...
class lang_installer {
    ...
    /** @var string this agpu major version */
    protected $version;
    ...
    public function __construct($langcode = '') {
        ...
        $this->version = agpu_major_version(true);
    ...
    public function lang_pack_url($langcode = '') {
        ...
        if (empty($langcode)) {
            return 'https://download.agpu.org/langpack/'.$this->version.'/';
        } else {
            return 'https://download.agpu.org/download.php/langpack/'.$this->version.'/'.$langcode.'.zip';
        }
    }

    /**
     * Returns the list of available language packs from download.agpu.org
     *
     * @return array|bool false if can not download
     */
    public function get_remote_list_of_languages() {
        $source = 'https://download.agpu.org/langpack/' . $this->version . '/languages.md5';
    ...
    protected function install_language_pack($langcode) {
        ...
        $installer = new component_installer('https://download.agpu.org', 'download.php/direct/langpack/' . $this->version,
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class lang_installer_exception extends agpu_exception {
