...
class auth_plugin_base {
    ...
    /**
     * agpu custom fields to sync with.
     * @var array()
     */
    ...
    function user_login($username, $password) {
        throw new \agpu_exception('mustbeoveride', 'debug', '', 'user_login()' );
    }
     ...
     *
     * @return ?agpu_url url of the profile page or null if standard used
     */
     ...
     *
     * Internal plugins use password hashes from agpu user table for authentication.
     *
     * @return bool
     */
    ...
    /**
     * Indicates if agpu should automatically update internal user
     ...
     *
     * In previous versions of agpu, the function
    ...
    function user_signup($user, $notify=true) {
        ...
        throw new \agpu_exception('mustbeoveride', 'debug', '', 'user_signup()' );
    }

    /**
     * Return a form to capture user details for account creation.
     * This is used in /login/signup.php.
     * @return agpuform A form which edits a record from the user table.
     */
    ...
    function user_confirm($username, $confirmsecret) {
        ...
        throw new \agpu_exception('mustbeoveride', 'debug', '', 'user_confirm()' );
    }
     ...
     * this information to agpu user-table you should honour synchronisation flags
     *
     ...
     * string keys:
     *
     * - url => (agpu_url|string) URL of the page to send the user to for authentication
     * - name => (string) Human readable name of the IdP
     * - iconurl => (agpu_url|string) URL of the icon representing the IdP (since agpu 3.3)
     *
     ...
     * Post logout hook.
     *
     * This method is used after agpu logout by auth classes to execute server logout.
     *
     ...
     * This is a lighter version of the one in agpulib -- won't do
    ...
    protected function update_user_record($username, $updatekeys = false, $triggerevent = false, $suspenduser = false) {
        ...
        if (empty($user)) { // Trouble.
            ...
            throw new \agpu_exception('auth_usernotexist', 'auth', '', $username);
            die;
        }
    ...
    public static function prepare_identity_providers_for_output($identityproviders, renderer_base $output) {
        ...
        foreach ($identityproviders as $idp) {
            ...
            if ($idp['iconurl'] instanceof agpu_url) {
                $idp['iconurl'] = $idp['iconurl']->out(false);
            }
            unset($idp['icon']);
            if ($idp['url'] instanceof agpu_url) {
    ...
    public function get_password_change_info(stdClass $user): array {
        ...
        if (!empty($data->link) and has_capability('agpu/user:changeownpassword', $systemcontext, $user->id)) {
    ...
    /**
     * Find an OS level admin agpu user account
     *
    ...
    /**
     * Find and login as an OS level admin agpu user account
     *
    ...
    /**
     * Identify a agpu account on the CLI
     *
...
function login_attempt_failed($user) {
    ...
    if ($lock = $lockfactory->get_lock($resource, 10)) {
        ...
        throw new agpu_exception('locktimeout');
    }
}
...
function signup_validate_data($data, $files) {
    ...
    if (! validate_email($data['email'])) {
        ...
        // Emails in agpu as case-insensitive and accents-sensitive. Such a combination can lead to very slow queries
        ...
        if ($DB->record_exists_sql($sql, $params)) {
            $forgotpasswordurl = new agpu_url('/login/forgot_password.php');
            $forgotpasswordlink = html_writer::link($forgotpasswordurl, get_string('emailexistshintlink'));
            $errors['email'] = get_string('emailexists') . ' ' . get_string('emailexistssignuphint', 'agpu', $forgotpasswordlink);
        }
    }
 ...
 * @param admin_settingpage $settings agpu admin settings instance
...
function display_auth_lock_options($settings, $auth, $userfields, $helptext, $mapremotefields, $updateremotefields, $customfields = array()) {
    ...
    foreach ($userfields as $field) {
        ...
        if ($fieldnametoolong) {
            ...
            $url = new agpu_url('/user/profile/index.php');
