...
class flexible_table {
    ...
    /**
     * Sets $this->baseurl.
     * @param agpu_url|string $url the url with params needed to call up this page
     */
    public function define_baseurl($url) {
        $this->baseurl = new agpu_url($url);
    }
    ...
    public function col_fullname($row) {
        ...
        $name = fullname($row, has_capability('agpu/site:viewfullnames', $this->get_context()));
        if ($this->download) {
            return $name;
        }
        ...
        if ($COURSE->id == SITEID) {
            $profileurl = new agpu_url('/user/profile.php', ['id' => $userid]);
        } else {
            $profileurl = new agpu_url(
                '/user/view.php',
     ...
     * options are changed.
     */
    public function format_text($text, $format = FORMAT_agpu, $options = null, $courseid = null) {
            ...
            switch ($column) {
                ...
                default:
                    if (array_search($column, $this->userfullnamecolumns) !== false) {
                        ...
                        if (has_capability('agpu/site:viewfullnames', $this->get_context())) {
    ...
    protected function get_dynamic_table_html_end(): string {
        ...
        if (is_a($this, dynamic::class)) {
            ...
            $perpageurl = new agpu_url($PAGE->url);
