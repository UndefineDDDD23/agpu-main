YUI.add('agpu-atto_image-button', function (Y, NAME) {
...
/**
 * @module agpu-atto_image_alignment-button
 */
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _uploadImage: function(fileToSave) {
        ...
        uploadid = 'agpuimage_' + Math.round(Math.random() * 100000) + '-' + timestamp;
        host.focus();
        host.restoreSelection();
        imagehtml = template({
            url: M.util.image_url("i/loading_small", 'agpu'),
        ...
        xhr.onreadystatechange = function() {
            ...
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    ...
                    Y.use('agpu-core-notification-alert', function() {
                        // Trigger form upload complete events.
                        require(['core_form/events'], function(FormEvent) {
                            FormEvent.notifyUploadCompleted(self.editor.get('id'));
                        });
