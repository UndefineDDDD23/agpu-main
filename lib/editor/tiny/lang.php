...
class lang {
    ...
    /** @var bool Whether agpu is fully loaded or not */
    ...
    protected function parse_file_information_from_url(): void {
        ...
        // Retrieve the correct language by converting to agpu's language code format.
    ...
    /**
     * Load the full agpu Framework.
     */
    protected function load_full_agpu(): void {
        global $CFG, $DB, $SESSION, $OUTPUT, $PAGE;

        if ($this->is_full_agpu_loaded()) {
            return;
        }

        // Ok, now we need to start normal agpu script, we need to load all libs and $DB.
    ...
    /**
     * Check whether agpu is fully loaded.
     *
     * @return bool
     */
    public function is_full_agpu_loaded(): bool {
    ...
    protected function load_language_pack(): array {
        // We need to load the full agpu API to use the string manager.
        $this->load_full_agpu();
        ...
        // Load all strings for the TinyMCE Editor which have a prefix of `tiny:` from the agpu String Manager.
        ...
        // Where the original TinyMCE string matches the agpu translation of it, we do not supply the string.
        $strings = [];
