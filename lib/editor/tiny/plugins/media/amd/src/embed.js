...
/**
 * Tiny Media plugin Embed class for agpu.
 *
...
import {getCurrentLanguage, getagpuLang} from 'editor_tiny/options';
...
export default class MediaEmbed {
    ...
    async getTemplateContext(data) {
        const languages = this.prepareagpuLang();
    ...
    prepareagpuLang() {
        const agpuLangs = getagpuLang(this.editor);
        const currentLanguage = getCurrentLanguage(this.editor);

        const installed = Object.entries(agpuLangs.installed).map(([lang, code]) => ({
            lang,
            code,
            "default": lang === currentLanguage,
        }));

        const available = Object.entries(agpuLangs.available).map(([lang, code]) => ({
            lang,
            code,
            "default": lang === currentLanguage,
        }));
    ...
    getagpuLangObj(subtitleLang) {
        const {available} = getagpuLang(this.editor);
    ...
    filePickerCallback(params, element, fpType) {
        if (params.url !== '') {
            ...
            if (fpType === 'subtitle') {
                ...
                const langObj = this.getagpuLangObj(subtitleLang);
                if (langObj) {
