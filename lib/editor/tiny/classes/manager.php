...
class manager {
    ...
    public function get_plugin_configuration(
        ...
        // Fetch configuration for agpu plugins.
        $agpuplugins = \core_component::get_plugin_list_with_class('tiny', 'plugininfo');
        $enabledplugins = \editor_tiny\plugininfo\tiny::get_enabled_plugins();
        foreach ($agpuplugins as $plugin => $classname) {
            ...
            // We suffix the plugin name for agpu plugins with /plugin to avoid conflicts with Tiny plugins.
    ...
    /**
     * Return a list of all available plugins, including both TinyMCE shipped, and agpu add-onis.
     *
    ...
    /**
     * Return a list of all available plugins built into TinyMCE and not shipped as separate agpu plugins.
     *
     * Each plugin is returned as an array element containing:
     * - a list of buttons (if applicable); and
     * - a list of menuitems (if applicable).
     ...
     *
     * These are usually disabled because we have replaced them, or they are not compatible with agpu in some way.
     *
     * @return string[]
     */
    protected function get_disabled_tinymce_plugins(): array {
        return [
            ...
            // These are not generally compatible with agpu.
            'image',
            'media',

            // Use the agpu autosave plugin instead.
            'autosave',

            // Disable the preview plugin as it does not support agpu filters.
            'preview',

            // Use the agpu link plugin instead.
            'link',
        ];
    }

    /**
     * Get a list of the agpu plugins with their button, and menuitem, configuration.
     *
     * @return array[]
     */
    protected function get_agpu_plugins(): array {
     ...
     * Some premium plugins *may not* be compatible with agpu, and some may require additional configuration.
     *
     * @return array[]
     */
