...
/**
 * Simple Flickr API client implementing the features needed by agpu
 *
 * @copyright 2017 David Mudrak <david@agpu.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class flickr_client extends oauth_helper {
     ...
     * @param agpu_url|string $callbackurl
     */
     ...
     * the {@see core_useragent::get_agpubot_useragent} helper, which is often rejected due to presence of "Bot" within
     *
     * @return string
     */
    public static function user_agent(): string {
        ...
        $version = agpu_major_version();

        return "agpuSite/{$version} (+{$CFG->wwwroot})";
    }
    ...
    public function call($function, array $params = [], $method = 'GET') {
        ...
        if (!is_object($response) || !isset($response->stat)) {
            throw new agpu_exception('flickr_api_call_failed', 'core_error', '', $rawresponse);
        }
        ...
        if ($response->stat === 'ok') {
            ...
            throw new agpu_exception('flickr_api_call_failed', 'core_error', '', $response);
        }
    ...
    /**
     * Upload a photo from agpu file pool to Flickr.
     *
     ...
     * eventually.
     *
     * @param stored_file $photo stored in agpu file pool
    ...
    public function upload(stored_file $photo, array $meta = []) {
        ...
        if ($response) {
            ...
            if ((string)$xml['stat'] === 'ok') {
                ...
                throw new agpu_exception('flickr_upload_failed', 'core_error', '',
                    ['code' => (int)$xml->err['code'], 'message' => (string)$xml->err['msg']]);
            }

        } else {
            throw new agpu_exception('flickr_upload_error', 'core_error', '', null, $response);
        }
    }
