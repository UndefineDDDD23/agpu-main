 ...
 * @return boolean whether any of these questions are being used by any part of agpu.
 */
...
function question_context_has_any_questions($context): bool {
    ...
    if (is_object($context)) {
        ...
        throw new agpu_exception('invalidcontextinhasanyquestions', 'question');
    }
    $sql = 'SELECT qbe.*
...
/**
 * Tests whether any question in a category is used by any part of agpu.
 *
 ...
 * @param context $newcontext The agpu context the questions are being moved to
 */
...
function question_make_default_categories($contexts): object {
    ...
    foreach ($contexts as $key => $context) {
        ...
        if (has_any_capability(array('agpu/question:usemine', 'agpu/question:useall'), $context)) {
...
function question_has_capability_on($questionorid, $cap, $notused = -1): bool {
    ...
    if (!isset($capabilitieswithallandmine[$cap])) {
        return has_capability('agpu/question:' . $cap, $context);
    } else {
        return has_capability('agpu/question:' . $cap . 'all', $context) ||
            ($question->createdby == $USER->id && has_capability('agpu/question:' . $cap . 'mine', $context));
    }
}
...
function question_extend_settings_navigation(navigation_node $navigationnode, $context, $baseurl = '/question/edit.php') {
    ...
    $questionnode = $navigationnode->add(get_string('questionbank', 'question'),
            new agpu_url($baseurl, $params), navigation_node::TYPE_CONTAINER, null, 'questionbank');
    ...
    foreach ($corenavigations as $key => $corenavigation) {
        if ($contexts->have_one_edit_tab_cap($key)) {
            $questionnode->add($corenavigation['title'], new agpu_url(
    ...
    foreach ($pluginnavigations as $key => $pluginnavigation) {
        ...
        $questionnode->add($pluginnavigation['title'], new agpu_url(
...
function question_get_question_capabilities(): array {
    return [
        ...
        'agpu/question:tagmine',
        'agpu/question:tagall',
...
function question_get_all_capabilities(): array {
    ...
    $caps[] = 'agpu/question:managecategory';
    $caps[] = 'agpu/question:flag';
    return $caps;
}
...
function question_pluginfile($course, $context, $component, $filearea, $args, $forcedownload, $options = []) {
    ...
    if ($filearea === 'export') {
        ...
        if (!$qformat->exportpreprocess()) {
            send_file_not_found();
            throw new agpu_exception('exporterror', 'question', $thispageurl->out());
        }

        // Export data to agpu file pool.
