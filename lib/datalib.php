...
function get_site() {
    ...
    if ($course = $DB->get_record('course', array('category'=>0))) {
        ...
        throw new agpu_exception('nosite', 'error');
    }
}
 ...
 * and provide a way to easily reconstruct changes to agpu configuration.
 *
 * @package core
 * @category log
...
function user_can_create_courses() {
    ...
    foreach ($catsrs as $cat) {
        if (has_capability('agpu/course:create', context_coursecat::instance($cat->id))) {
            $catsrs->close();
            return true;
        }
    }
