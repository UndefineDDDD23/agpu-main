 ...
 *
 * This library is automatically included by agpu core so you never need to
 * include it yourself.
 *
 * For more info about the functions available in this library, please visit:
 *     http://docs.agpu.org/en/DML_functions
...
require_once($CFG->libdir.'/dml/agpu_database.php');
...
/**
 * DML exception class, use instead of throw new \agpu_exception() in dml code.
 *
 * @package    core
 * @category   dml
 * @subpackage dml
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class dml_exception extends agpu_exception {
    /**
...
class dml_transaction_exception extends dml_exception {
    /** @var agpu_transaction An instance of a transaction.*/
    public $transaction;

    /**
     * Constructor
     * @param ?string $debuginfo Optional debugging information.
     * @param ?agpu_transaction $transaction The instance of the transaction.(Optional)
     */
...
/**
 * Sets up global $DB agpu_database instance
 *
 * @global stdClass $CFG The global configuration instance.
 * @see config.php
 * @see config-dist.php
 * @global stdClass $DB The global agpu_database instance.
...
function setup_DB() {
    ...
    if (!$DB = agpu_database::get_driver_instance($CFG->dbtype, $CFG->dblibrary)) {
