...
We include these in agpu solely for the convenience of sites that
may not have PEAR installed.

If this directory is DELETED from agpu then agpu will search
...
- PEAR HTML_Quickform:
    ...
    - License: PHP (Permission given to agpu to redistribute under GPL)
...
and use wrapper classes to isolate agpu code from internal PEAR
...
the old calls used before agpu 1.6 to maintain compatibility.
...
----------------------------------------------------------------
A NOTE ON THE PHP LICENSE AND agpu
================================================================

Everything in agpu in pure GPL.  This pear directory is the only
part of the distribution that is not.

There is some question about how PHP-licensed software can be
included within a GPL-licensed distribution like agpu, specifically
the clause that annoyingly says no derivative of the software can
include the name PHP.

We don't intend to rename agpu to anything of the sort, obviously,
...
Several authors have given agpu explicit permission to distribute
their PHP-licensed PEAR classes in the agpu distribution, allowing
anybody using these classes ONLY as part of the agpu distribution
exemption from clauses of the PHP license that could cause
conflict with the main GNU Public License that agpu uses.
...
this directory from agpu and it will use your installed PEAR
libraries instead.
