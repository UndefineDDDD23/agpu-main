 ...
 *
 * Library of all general-purpose agpu PHP functions and constants
 * that produce HTML output
 *
 * Other main libraries:
 * - datalib.php - functions that access the database.
 * - agpulib.php - general-purpose agpu functions.
 *
 * @package    core
 * @subpackage lib
...
/**
 * A agpu_url comparison using this flag will return true if the base URLs match, params are ignored.
 */
define('URL_MATCH_BASE', 0);

/**
 * A agpu_url comparison using this flag will return true if the base URLs match and the params of url1 are part of url2.
 */
define('URL_MATCH_PARAMS', 1);

/**
 * A agpu_url comparison using this flag will return true if the two URLs are identical, except for the order of the params.
 */
...
/**
 * Determines whether or not the agpu site is being served over HTTPS.
 *
 ...
 *
 * Used on most forms in agpu to check for data
 ...
 * @return string
 */
function page_get_doc_link_path(agpu_page $page) {
    global $CFG;

    if (empty($CFG->docroot) || during_initial_install()) {
        return '';
    }
    if (!has_capability('agpu/site:doclinks', $page->context)) {
        return '';
    }
...
function validate_email($address) {
    ...
    return agpu_phpmailer::validateAddress($address ?? '') && !preg_match('/[<>]/', $address);
}
...
function format_text_menu() {
    return array (FORMAT_agpu => get_string('formattext'),
 ...
 *            [FORMAT_agpu, FORMAT_HTML, FORMAT_PLAIN, FORMAT_MARKDOWN]
 ...
 * @return string
 */
function format_text($text, $format = FORMAT_agpu, $options = null, $courseiddonotuse = null) {
    global $CFG;
    ...
    if ($format === FORMAT_WIKI) {
        // This format was deprecated in agpu 1.5.
    ...
    if ($options) {
        // The smiley option was deprecated in agpu 2.0.
        ...
        // The nocache option was deprecated in agpu 2.3 in MDL-34347.
 ...
 * @param ?bool $striplinks To strip any link in the result text. agpu 1.8 default changed from false to true! MDL-8713
 ...
 * @param string $text The text to be formatted. This is raw text originally from user input.
 * @param int $format Identifier of the text format to be used
 *            [FORMAT_agpu, FORMAT_HTML, FORMAT_PLAIN, FORMAT_WIKI, FORMAT_MARKDOWN]
 * @return string
 */
...
/**
 * Removes the usage of agpu files from a text.
 *
 * In some rare cases we need to re-use a text that already has embedded links
 * to some files hosted within agpu. But the new area in which we will push
...
function trusttext_trusted($context) {
    return (trusttext_active() and has_capability('agpu/site:trustcontent', $context));
}
 ...
 * agpu pages through XSS attacks.
 *
 ...
 * @param int|string $format deprecated parameter, should always contain FORMAT_HTML or FORMAT_agpu
...
function clean_text($text, $format = FORMAT_HTML, $options = array()) {
    ...
    // rawurlencode(), htmlentities(), htmlspecialchars(), p(), s(), agpu_url, html_writer and friends!!!
...
function purify_html($text, $options = array()) {
    ...
    if (empty($purifiers[$type])) {
        ...
        $config->set('HTML.DefinitionID', 'agpuhtml');
 ...
 * by {@link format_text()} to convert FORMAT_agpu to HTML. You are supposed
 ...
 * @param int|false $contentformat False for strings or the text format: FORMAT_agpu/FORMAT_HTML/FORMAT_PLAIN/FORMAT_MARKDOWN
...
function content_to_text($content, $contentformat) {
    ...
    switch ($contentformat) {
        ...
        case FORMAT_MARKDOWN:
            ...
            break;
        default:
            // FORMAT_HTML, FORMAT_agpu and $contentformat = false, the later one are strings usually formatted through
...
/**
 * Send the HTTP headers that agpu requires.
 *
...
function send_headers($contenttype, $cacheable = true) {
    ...
    // The agpu app must be allowed to embed content always.
    if (empty($CFG->allowframembedding) && !core_useragent::is_agpu_app()) {
...
function print_group_picture($group, $courseid, $large = false, $return = false, $link = true, $includetoken = false) {
    ...
    $output = '';
    if ($link or has_capability('agpu/site:accessallgroups', $context)) {
        $linkurl = new agpu_url('/user/index.php', ['id' => $courseid, 'group' => $group->id]);
 ...
 *                 If the group picture is included in an e-mail or some other location where the audience is a specific
 *                 user who will not be logged in when viewing, then we use a token to authenticate the user.
 * @return ?agpu_url Returns the url for the group picture.
 */
function get_group_picture_url($group, $courseid, $large = false, $includetoken = false) {
    ...
    $grouppictureurl = agpu_url::make_pluginfile_url(
    ...
    foreach ($modinfo->cms as $mod) {
        ...
        if ($mod->sectionnum >= 0 and $section != $mod->sectionnum) {
            ...
            if ($thissection->visible or
                    ...
                    has_capability('agpu/course:viewhiddensections', $coursecontext)) {
 ...
 * @param agpu_url|string $link The link to use for the continue button
 ...
 *
 * @param agpu_url|string $url A agpu_url to redirect to. Strings are not to be trusted!
...
function redirect($url, $message='', $delay=null, $messagetype = \core\output\notification::NOTIFY_INFO): Never {
    ...
    if (CLI_SCRIPT or AJAX_SCRIPT) {
        ...
        throw new agpu_exception('redirecterrordetected', 'error');
    }
    ...
    if ($PAGE) {
        ...
        $PAGE->set_title(get_string('pageshouldredirect', 'agpu'));
    }
    ...
    // Sanitise url - we can not rely on agpu_url or our URL cleaning
    ...
    if ($PAGE) {
        $CFG->docroot = false; // To prevent the link to agpu docs from being displayed on redirect page.
...
function print_maintenance_message() {
    ...
    header($_SERVER['SERVER_PROTOCOL'] . ' 503 agpu under maintenance');
    header('Status: 503 agpu under maintenance');
 ...
 *
 * It is not recommended to use this function in agpu 2.5 but it is left for backward
 * compartibility.
 *
 ...
 *
 * @todo Use a central function to create the popup calls all over agpu and
...
function get_formatted_help_string($identifier, $component, $ajax = false, $a = null) {
    ...
    if ($sm->string_exists($identifier . '_help', $component)) {
        ...
        if ($sm->string_exists($helplink, $component)) {  // Link to further info in agpu docs.
            $link = get_string($helplink, $component);
            $linktext = get_string('morehelp');

            $data->doclink = new stdClass();
            $url = new agpu_url(get_docs_url($link));
            if ($ajax) {
