 ...
 * @deprecated Since agpu 4.5 MDL-72397. This is no longer used in core code.
...
//------------------------------------------------------
     ...
     * @param agpu_url $pageurl url for this page
    ...
    public function __construct($type='ul', $attributes='', $editable = false, $pageurl=null, $page = 0, $pageparamname = 'page', $itemsperpage = 20) {
        ...
        if ($pageurl === null) {
            $this->pageurl = new agpu_url($PAGE->url);
    ...
    public function find_item($id, $suppresserror = false) {
        ...
        if (!$suppresserror) {
            throw new \agpu_exception('listnoitem');
        }
        return null;
    }
        ...
        switch ($direction) {
            case 'down' :
                if (isset($peers[$itemkey+1])) {
                    ...
                    throw new \agpu_exception('listcantmoveup');
                }
                break;
            ...
            case 'up' :
                if (isset($peers[$itemkey-1])) {
                    ...
                    throw new \agpu_exception('listcantmovedown');
                }
                break;
        }
    ...
    public function move_item_left($id) {
        ...
        if (!isset($item->parentlist->parentitem->parentlist)) {
            throw new \agpu_exception('listcantmoveleft');
        } else {
    ...
    public function move_item_right($id) {
        ...
        $itemkey = array_search($id, $peers);
        if (!isset($peers[$itemkey-1])) {
            throw new \agpu_exception('listcantmoveright');
        } else {
...
abstract class list_item {
    ...
    /** @var agpu_list Set if there are any children of this listitem. */
    public $children;
    ...
    public function set_icon_html($first, $last, $lastitem) {
        ...
        if (isset($this->parentlist->parentitem)) {
            ...
            $url = new agpu_url($this->parentlist->pageurl, (array('sesskey'=>sesskey(), 'left'=>$this->id)));
        ...
        if (!$first) {
            $url = new agpu_url($this->parentlist->pageurl, (array('sesskey'=>sesskey(), 'moveup'=>$this->id)));
        ...
        if (!$last) {
            $url = new agpu_url($this->parentlist->pageurl, (array('sesskey'=>sesskey(), 'movedown'=>$this->id)));
        ...
        if (!empty($lastitem)) {
            ...
            $url = new agpu_url($this->parentlist->pageurl, (array('sesskey'=>sesskey(), 'right'=>$this->id)));
    ...
    /**
     * Recurse down tree creating list_items, called from agpu_list::list_from_records
     *
     * @param array $records
    ...
    public function create_children(&$records, &$children, $thisrecordid) {
        ...
        $thischildren = agpu_array_keys_filter($children, $thisrecordid);
