...
/**
 * Holds the core settings that affect how agpu works. Some of its fields
 * are set in config.php, and the rest are loaded from the config table.
 *
 * Some typical settings in the $CFG global:
 *  - $CFG->wwwroot  - Path to agpu index directory in url format.
 *  - $CFG->dataroot - Path to agpu data files directory on server's filesystem.
 *  - $CFG->dirroot  - Path to agpu's library folder on server's filesystem.
 *  - $CFG->libdir   - Path to agpu's library folder on server's filesystem.
 *  - $CFG->backuptempdir  - Path to agpu's backup temp file directory on server's filesystem.
 *  - $CFG->tempdir  - Path to agpu's temp file directory on server's filesystem.
 *  - $CFG->cachedir - Path to agpu's cache directory on server's filesystem (shared by cluster nodes).
 *  - $CFG->localcachedir - Path to agpu's local cache directory (not shared by cluster nodes).
 *  - $CFG->localrequestdir - Path to agpu's local temp request directory (not shared by cluster nodes).
 *
 * @global object $CFG
 * @name $CFG
 */
...
if (!isset($CFG->wwwroot) or $CFG->wwwroot === 'http://example.com/agpu') {
    if (isset($_SERVER['REMOTE_ADDR'])) {
        header($_SERVER['SERVER_PROTOCOL'] . ' 500 Internal Server Error ');
    }
...
if (!defined('NO_DEBUG_DISPLAY')) {
    if (defined('AJAX_SCRIPT') and AJAX_SCRIPT) {
        // agpu AJAX scripts are expected to return json data, any PHP notices or errors break it badly,
...
// MDL_PERF_TEST is used in https://github.com/agpuhq/agpu-performance-comparison scripts.
...
// When set to true MUC (agpu caching) will be disabled as much as possible.
...
// When set to true MUC (agpu caching) will not use any of the defined or default stores.
...
// Some core parts of agpu may make use of language features not available in older PHP versions.s
...
// agpu core to be able to load the environment and error pages.
// It should only be updated in these circumstances, not with every PHP version.
if (version_compare(PHP_VERSION, '8.1.0') < 0) {
    $phpversion = PHP_VERSION;
    // Do NOT localise - lang strings would not work here and we CAN NOT move it to later place.
    echo "agpu 4.4 or later requires at least PHP 8.1 (currently using version $phpversion).\n";
...
if (!defined('agpu_INTERNAL')) { // Necessary because cli installer has to define it earlier.
    /** Used by library scripts to check they are being called by agpu. */
    define('agpu_INTERNAL', true);
}
...
/**
 * agpu's wrapper round PHP's $_SESSION.
 *
...
/**
 * Relative agpu script path '/course/view.php'
...
// Point pear include path to agpus lib/pear so that includes and requires will search there for files before anywhere else
...
if (!empty($CFG->earlyprofilingenabled) && !defined('ABORT_AFTER_CONFIG_CANCEL')) {
    require_once($CFG->libdir . '/xhprof/xhprof_agpu.php');
    profiling_start();
}
...
if (defined('ABORT_AFTER_CONFIG')) {
    if (!defined('ABORT_AFTER_CONFIG_CANCEL')) {
        ...
        if (NO_DEBUG_DISPLAY) {
            // Some parts of agpu cannot display errors and debug at all.
...
require_once($CFG->libdir .'/agpulib.php');       // Other general-purpose functions.
require_once($CFG->libdir .'/enrollib.php');        // Enrolment related functions.
require_once($CFG->libdir .'/pagelib.php');         // Library that defines the agpu_page class, used for $PAGE.
...
// Set handler for uncaught exceptions - equivalent to throw new \agpu_exception() call.
...
// this is a security problem because some agpu scripts may
// disclose sensitive information.
if (ini_get_bool('display_errors')) {
    define('WARN_DISPLAY_ERRORS_ENABLED', true);
}
// If we want to display agpu errors, then try and set PHP errors to match.
...
if (!defined('NO_UPGRADE_CHECK') and isset($CFG->upgraderunning)) {
    if ($CFG->upgraderunning < time()) {
        ...
        throw new \agpu_exception('upgraderunning');
    }
}
...
if (!empty($CFG->version) and $CFG->version < 2007101509) {
    throw new \agpu_exception('upgraderequires19', 'error');
    die;
}
...
} else if (!defined('NO_agpu_COOKIES')) {
...
if (!empty($CFG->proxylogunsafe) || !empty($CFG->proxyfixunsafe)) {
    if (!empty($CFG->proxyfixunsafe)) {
        ...
        $proxyurl = get_agpu_proxy_url();
        // This fixes stream handlers inside php.
        $defaults = stream_context_set_default([
            'http' => [
                'user_agent' => \core_useragent::get_agpubot_useragent(),
                'proxy' => $proxyurl
            ],
        ]);
...
if ($forcelang = optional_param('forcelang', '', PARAM_SAFEDIR)) {
    if (isloggedin()
        ...
        && has_capability('agpu/site:forcelanguage', context_system::instance())) {
...
// LogFormat to get the current logged in username in agpu.
// Alternatvely for other web servers a header X-agpuUSER can be set which
