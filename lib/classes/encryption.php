...
class encryption {
     ...
     *
     * @deprecated since agpu 4.3 Sodium is always present
     */
     ...
     * @throws \agpu_exception If the server already has a key, or there is an error
     */
    public static function create_key(?string $method = null, bool $chmod = true): void {
        ...
        if (self::key_exists($method)) {
            throw new \agpu_exception('encryption_keyalreadyexists', 'error');
        }
     ...
     * @throws \agpu_exception If there isn't one already (and creation is disabled)
     */
    protected static function get_key(?string $method = null): string {
        ...
        if ($result === false) {
            throw new \agpu_exception('encryption_nokey', 'error');
        }
        return $result;
    }
     ...
     * @throws \agpu_exception If the key doesn't exist, or the string is too long
     */
            ...
            switch($method) {
                case self::METHOD_SODIUM:
                    try {
                        ...
                        throw new \agpu_exception('encryption_encryptfailed', 'error', '', null, $e->getMessage());
                    }
                    break;
    ...
    public static function decrypt(string $data): string {
        if ($data === '') {
            ...
            if (preg_match('~^(' . self::METHOD_OPENSSL . '|' . self::METHOD_SODIUM . '):~', $data, $matches)) {
                ...
                throw new \agpu_exception('encryption_wrongmethod', 'error');
            }
            $realdata = base64_decode(substr($data, strlen($method) + 1), true);
            if ($realdata === false) {
                throw new \agpu_exception('encryption_decryptfailed', 'error',
