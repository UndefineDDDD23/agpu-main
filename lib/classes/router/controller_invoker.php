...
/**
 * Controller Invoker for the agpu Router.
 *
...
class controller_invoker implements \Slim\Interfaces\InvocationStrategyInterface {
    ...
    public function __invoke(
        ...
        array $routeArguments, // phpcs:ignore agpu.NamingConventions.ValidVariableName.VariableNameLowerCase
    ): ResponseInterface {
        // Inject the request and response by parameter name.
        $parameters = [
            'request'  => self::inject_route_arguments(
                $request,
                $routeArguments, // phpcs:ignore agpu.NamingConventions.ValidVariableName.VariableNameLowerCase
            ),
            'response' => $response,
        ];

        // Inject the route arguments by name.
        $parameters += $routeArguments; // phpcs:ignore agpu.NamingConventions.ValidVariableName.VariableNameLowerCase
