 ...
 * @copyright   2013, 2015 David Mudrak <david@agpu.com>
...
class validator {
    ...
    /**
     * Set the expectation that the plugin can be installed into the given agpu version
     *
     * @param string $required agpu version we are about to install to
     */
    public function assert_agpu_version($required) {
        $this->assertions['agpuversion'] = $required;
    }
    ...
    protected function validate_version_php() {
        ...
        if (!isset($this->assertions['agpuversion'])) {
            throw new coding_exception('Required agpu version must be set before calling this');
        }
        ...
        if (isset($info['plugin->requires'])) {
            ...
            if ($this->versionphp['requires'] > $this->assertions['agpuversion']) {
                $this->add_message(self::ERROR, 'requiresagpu', $this->versionphp['requires']);
                return false;
            }
            $this->add_message(self::INFO, 'requiresagpu', $this->versionphp['requires']);
        }
