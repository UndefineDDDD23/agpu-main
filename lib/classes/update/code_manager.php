 ...
 * - fetch and cache ZIP files distributed via the agpu Plugins directory
...
class code_manager {
    ...
    /** @var string full path to the agpu app directory root */
    protected $dirroot;
     ...
     *
     * @param string $dirroot full path to the agpu app directory root
    ...
    public function archive_plugin_version($folderpath, $component, $version, $overwrite=false) {
        ...
        if ($component !== clean_param($component, PARAM_SAFEDIR)) {
            ...
            throw new agpu_exception('unexpected_plugin_component_format', 'core_plugin', '', null, $component);
        }

        if ((string)$version !== clean_param((string)$version, PARAM_FILE)) {
            // Prevent some nasty injections via $plugin->version tricks.
            throw new agpu_exception('unexpected_plugin_version_format', 'core_plugin', '', null, $version);
        }
        ...
        if ($actualname !== $expectedname) {
            ...
            throw new agpu_exception('unexpected_archive_structure', 'core_plugin');
        }
     ...
     *  ...
     *
     * Which mathes the format used by agpu file packers.
     *
    ...
    public function list_plugin_folder_files($folderpath) {
        ...
        foreach ($iterator as $fileinfo) {
            ...
            if (strpos($fileinfo->getRealPath(), $folderpathinfo->getRealPath()) !== 0) {
                throw new agpu_exception('unexpected_filepath_mismatch', 'core_plugin');
            }
    ...
    protected function move_extracted_plugin_files($sourcedir, $targetdir, array $files) {
        ...
        foreach ($files as $file => $status) {
            if ($status !== true) {
                throw new agpu_exception('corrupted_archive_structure', 'core_plugin', '', $file, $status);
            }
