 ...
 *
 * @package     core
 * @copyright   2015 David Mudrak <david@agpu.com>
...
/**
 * General purpose client for https://download.agpu.org/api/
 *
 * The API provides proxy access to public information about plugins available
 * in the agpu Plugins directory. It is used when we are checking for
 ...
 * - locate the most suitable plugin version for the given agpu branch
 *
 * TODO:
 *
...
class api {
    ...
    const APIROOT = 'https://download.agpu.org/api';
     ...
     * agpu plugins directory. This is typically used to handle the
     ...
     * name, that suits best for the given agpu core branch. Minimal required
     ...
     * @param int $branch agpu core branch such as 29, 30, 31 etc, defaults to $CFG->branch
     * @return \core\update\remote_info|bool
     */
    ...
    public function validate_pluginfo_format($data) {
        ...
        if (!empty($data->version)) {
            ...
            $versionproperties = array('id' => 1, 'version' => 1, 'release' => 0, 'maturity' => 0,
                ...
                'vcsrepositoryurl' => 0, 'vcsbranch' => 0, 'vcstag' => 0, 'supportedagpus' => 0);
            ...
            if (!empty($data->version->supportedagpus)) {
                if (!is_array($data->version->supportedagpus)) {
                    return false;
                }
                foreach ($data->version->supportedagpus as $supportedagpu) {
                    if (!is_object($supportedagpu)) {
                        return false;
                    }
                    if (empty($supportedagpu->version) or empty($supportedagpu->release)) {
                        return false;
                    }
                }
            }
     ...
     *
     * The syntax of $CFG->branch uses the XY format that suits the agpu docs
     * versioning and stable branches numbering scheme. The API at
     * download.agpu.org uses the X.Y numbering scheme.
     *
     * @param int $branch agpu branch in the XY format (e.g. 29, 30, 31 etc)
     * @return string agpu branch in the X.Y format (e.g. 2.9, 3.0, 3.1 etc)
     */
