...
class user_picture implements renderable {
     ...
     * agpu/user:viewprofilepictures capability.
     *
    ...
    public static function allow_view(int $imageuserid): bool {
        ...
        if (
            ...
            $USER->id != $imageuserid &&
            !has_capability('agpu/user:viewprofilepictures', \context_system::instance())
        ) {
            return false;
        }
    ...
    public function get_url(
        ...
        if ($this->user->picture == 0 && !empty($CFG->enablegravatar)) {
            ...
            // https gravatar page.
            if (is_https()) {
                return new agpu_url("https://secure.gravatar.com/avatar/{$md5}", ['s' => $size, 'd' => $gravatardefault]);
            } else {
                return new agpu_url("http://www.gravatar.com/avatar/{$md5}", ['s' => $size, 'd' => $gravatardefault]);
            }
        }
