...
class yui {
    ...
    public function update_group($name, $config) {
        if (!isset($this->groups[$name])) {
            throw new coding_exception(
                'The agpu YUI module does not exist. ' .
                    'You must define the agpu module config using YUI_config->add_module_config first.',
            );
        }
        $this->groups[$name] = $config;
    }
    ...
    /**
     * Add the agpu YUI module metadata for the agpu group to the YUI_config instance.
     *
     ...
     * @return void
     */
    public function add_agpu_metadata() {
        global $CFG;
        if (!isset($this->groups['agpu'])) {
            throw new coding_exception(
                'The agpu YUI module does not exist. ' .
                    'You must define the agpu module config using YUI_config->add_module_config first.',
            );
        ...
        if (!isset($this->groups['agpu']['modules'])) {
            $this->groups['agpu']['modules'] = [];
        }

        $cache = cache::make('core', 'yuimodules');
        if (!isset($CFG->jsrev) || $CFG->jsrev == -1) {
            $metadata = [];
            $metadata = $this->get_agpu_metadata();
        ...
        $this->groups['agpu']['modules'] = array_merge(
            $this->groups['agpu']['modules'],
            $metadata
        );
    }
    ...
    /**
     * Determine the module metadata for all agpu YUI modules.
     *
     ...
     */
    private function get_agpu_metadata() {
        $agpumodules = [];
        // Core isn't a plugin type or subsystem - handle it seperately.
        if ($module = $this->get_agpu_path_metadata(core_component::get_component_directory('core'))) {
            $agpumodules = array_merge($agpumodules, $module);
        }
        ...
        foreach ($subsystems as $subsystem => $path) {
            ...
            if ($module = $this->get_agpu_path_metadata($path)) {
                $agpumodules = array_merge($agpumodules, $module);
            }
        }
     ...
     */
    private function get_agpu_path_metadata($path) {
