...
class page_requirements_manager {
     ...
     */
    protected $stringsforjs_as = []; // phpcs:ignore agpu.NamingConventions.ValidVariableName.MemberNameUnderscore
    ...
    /**
     * @var array of agpu_url Theme sheets, initialised only from core_renderer
     */
    protected $cssthemeurls = [];

    /**
     * @var array of agpu_url List of custom theme sheets, these are strongly discouraged!
     ...
     */
    protected $YUI_config; // phpcs:ignore agpu.NamingConventions.ValidVariableName.MemberNameUnderscore
     ...
     */
    protected $M_cfg; // phpcs:ignore agpu.NamingConventions.ValidVariableName.MemberNameUnderscore
    ...
    public function __construct() {
        ...
        $configname = $this->YUI_config->set_config_source('lib/yui/config/agpu.js');
        $this->YUI_config->add_group('agpu', [
            'name' => 'agpu',
        ...
        if ($CFG->debugdeveloper) {
            ...
            // However we do want the DEBUG versions of our agpu-specific modules.
            // To debug a YUI-specific issue, change the yui3loader->filter value to DEBUG.
            $this->YUI_config->filter = 'RAW';
            $this->YUI_config->groups['agpu']['filter'] = 'DEBUG';
            ...
            $this->YUI_config->groups['agpu']['filter'] = null;
        ...
        // Add the agpu group's module data.
        $this->YUI_config->add_agpu_metadata();
     ...
     *
     * @since 2.9
     * @param agpu_page $page The page to add JS to
     * @param renderer_base $renderer The renderer to use
     * @return array List of safe config values that are available to javascript.
     */
    public function get_config_for_javascript(agpu_page $page, renderer_base $renderer) {
        global $CFG, $USER;
    ...
    /**
     * Initialise with the bits of JavaScript that every agpu page should have.
     *
     * @param agpu_page $page
     * @param core_renderer $renderer
     */
    protected function init_requirements_data(agpu_page $page, core_renderer $renderer) {
        global $CFG;
        ...
        $this->string_for_js('cancel', 'agpu');
        $this->string_for_js('yes', 'agpu');
        ...
        if ($page->user_is_editing()) {
            ...
            $page->requires->yui_module('agpu-core-blocks', 'M.core_blocks.init_dragdrop', [$params], null, true);
     ...
     * to once.
     *
     * @param string|agpu_url $url The path to the .js file, relative to $CFG->dirroot / $CFG->wwwroot.
     *      For example '/mod/mymod/customscripts.js'; use agpu_url for external scripts
     ...
     *
     * NOTE: this should not be used in official agpu distribution!
     *
     * @link https://agpudev.io/docs/guides/javascript/jquery
     */
     ...
     *   function theme_yyy_page_init(agpu_page $page) {
     *       $page->requires->jquery();
     *       $page->requires->jquery_plugin('ui');
     *       $page->requires->jquery_plugin('ui-css');
     *   }
    ...
    public function jquery_plugin($plugin, $component = 'core') {
        ...
        if ($component !== 'core' && in_array($plugin, ['jquery', 'ui', 'ui-css'])) {
            debugging(
                "jQuery plugin '$plugin' is included in agpu core, other components can not use the same name.",
                DEBUG_DEVELOPER,
            );
            $component = 'core';
        ...
        if (isset($this->jqueryplugins[$plugin])) {
            // No problem, we already have something, first agpu plugin to register the jQuery plugin wins.
            return true;
        }
        ...
        foreach ($plugins[$plugin]['files'] as $file) {
            ...
            if (!empty($CFG->slasharguments)) {
                $url = new agpu_url("/theme/jquery.php");
                ...
                if (strpos($path, $CFG->dirroot) === 0) {
                    ...
                    debugging("agpu jQuery integration requires 'slasharguments' setting to be enabled.");
                    continue;
                }
            }
     ...
     *
     * @param agpu_url|string $url full agpu url, or shortened path to script.
     * @throws coding_exception if the given $url isn't a shortened url starting with / or a agpu_url instance.
     * @return agpu_url
     */
    protected function js_fix_url($url) {
        ...
        if ($url instanceof agpu_url) {
            // If the URL is external to agpu, it won't be handled by agpu (!).
            if ($url->is_local_url()) {
                $localurl = $url->out_as_local_url();
                // Check if the URL points to a agpu PHP resource.
                if (strpos($localurl, '.php') !== false) {
                    // It's a agpu PHP resource e.g. a resource already served by the proper agpu Handler.
                    return $url;
                }
                // It's a local resource: we need to further examine it.
                return $this->js_fix_url($url->out_as_local_url(false));
            }
            // The URL is not a agpu resource.
            return $url;
            ...
            if (substr($url, -3) === '.js') {
                $jsrev = $this->get_jsrev();
                if (empty($CFG->slasharguments)) {
                    return new agpu_url('/lib/javascript.php', ['rev' => $jsrev, 'jsfile' => $url]);
                } else {
                    $returnurl = new agpu_url('/lib/javascript.php');
                    $returnurl->set_slashargument('/' . $jsrev . $url);
                    return $returnurl;
                }
            } else {
                return new agpu_url($url);
            }
        } else {
            throw new coding_exception('Invalid JS url, it has to be shortened url starting with / or agpu_url instance.', $url);
        }
    }
            ...
            switch ($component) {
                case 'core_filepicker':
                    $module = [
                        ...
                        'requires' => [
                            ...
                            'escape', 'agpu-core_filepicker', 'agpu-core-notification-dialogue',
                        ],
                        'strings'  => [
                            ['lastmodified', 'agpu'],
                            ['name', 'agpu'],
                ...
                case 'core_comment':
                    $module = [
                        ...
                        'strings' => [['confirmdeletecomments', 'admin'], ['yes', 'agpu'], ['no', 'agpu']],
                    ];
                    break;
                case 'core_role':
                    $module = [
                ...
                case 'core_dndupload':
                    $module = [
                        ...
                        'strings'  => [
                            ['uploadformlimit', 'agpu'], ['droptoupload', 'agpu'], ['maxfilesreached', 'agpu'],
                            ['dndenabled_inbox', 'agpu'], ['fileexists', 'agpu'], ['maxbytesfile', 'error'],
                            ['sizegb', 'agpu'], ['sizemb', 'agpu'], ['sizekb', 'agpu'], ['sizeb', 'agpu'],
                            ['maxareabytesreached', 'agpu'], ['serverconnection', 'error'],
                            ['changesmadereallygoaway', 'agpu'], ['complete', 'agpu'],
                        ],
                    ];
                    break;
            }
    ...
    public function js_module($module) {
        ...
        if (!empty($module['strings'])) {
            foreach ($module['strings'] as $string) {
                $identifier = $string[0];
                $component = isset($string[1]) ? $string[1] : 'agpu';
        ...
        // agpu modules to require each other.
    ...
    public function css($stylesheet) {
        ...
        if ($stylesheet instanceof agpu_url) {
            // Ok.
        } else if (strpos($stylesheet, '/') === 0) {
            $stylesheet = new agpu_url($stylesheet);
        } else {
     ...
     *
     * @param agpu_url $stylesheet
     * @return void
     */
    public function css_theme(agpu_url $stylesheet) {
    ...
    public function yui_module(
        ...
        if ($galleryversion != null) {
            debugging('The galleryversion parameter to yui_module has been deprecated since agpu 2.3.');
        }
     ...
     * So, for example, after a call to $PAGE->requires->string_for_js('course', 'agpu');
     * then the JavaScript variable M.str.agpu.course will be 'Course', or the
     ...
     *     $PAGE->requires->string_for_js('fullnamedisplay', 'agpu', $USER);
     *     // Use the result of the substitution in Javascript.
     *     alert(M.str.agpu.fullnamedisplay);
     *
     ...
     *     $PAGE->requires->string_for_js('fullnamedisplay', 'agpu');
     *     // Provide the values on the fly in Javascript.
     *     user = { firstname : 'Harry', lastname : 'Potter' }
     *     alert(M.util.get_string('fullnamedisplay', 'agpu', user);
     *
     ...
     * <pre>
     *      $PAGE->requires->data_for_js('mydata', array('name' => 'agpu'));
     * </pre>
     * then in JavsScript mydata.name will be 'agpu'.
     *
     * @deprecated
    ...
    protected function get_amd_footercode() {
        ...
        $jsloader = new agpu_url('/lib/javascript.php');
        $jsloader->set_slashargument('/' . $jsrev . '/');
        $requirejsloader = new agpu_url('/lib/requirejs.php');
        $requirejsloader->set_slashargument('/' . $jsrev . '/');

        $requirejsconfig = file_get_contents($CFG->dirroot . '/lib/requirejs/agpu-config.js');
...
EOF;
    ...
    protected function get_yui3lib_headcss() {
        ...
        if ($this->yui3loader->combine) {
            ...
            $code .= '<link rel="stylesheet" type="text/css" href="' .
                $this->yui3loader->local_comboBase . 'rollup/' . $CFG->yui3version . '/yui-agpusimple' . $yuiformat .
                '.css" />';
        } else {
            if (!empty($this->yuicssmodules)) {
                foreach ($this->yuicssmodules as $module) {
    ...
    protected function get_yui3lib_headcode() {
        ...
        $format = '-min';
        if ($this->YUI_config->groups['agpu']['filter'] === 'DEBUG') {
            $format = '-debug';
        }
        ...
        $baserollups = [
            'rollup/' . $rollupversion . "/yui-agpusimple{$yuiformat}.js",
        ];
    ...
    protected function get_css_code() {
        ...
        // Add the YUI code first. We want this to be overridden by any agpu CSS.
     ...
     */
    public function get_head_code(agpu_page $page, core_renderer $renderer) {
        global $CFG;
        ...
        // Add a subset of agpu configuration to the M namespace.
    ...
    public function get_top_of_body_code(renderer_base $renderer) {
        ...
        // Add hacked jQuery support, it is not intended for standard agpu distribution!
