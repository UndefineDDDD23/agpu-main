...
// phpcs:disable agpu.NamingConventions.ValidVariableName.VariableNameUnderscore
// phpcs:disable agpu.NamingConventions.ValidVariableName.MemberNameUnderscore

/**
 * This class represents the configuration variables of a agpu theme.
 *
 ...
 * workings of agpu's themes system. If you are just editing a themes config.php
 ...
 * However, normally you don't need to bother, because agpu_page (that is, $PAGE)
...
class theme_config {
     ...
     * Pages in agpu are using several different layouts like 'normal', 'course', 'home',
     * 'popup', 'form', .... The most reliable way to get a complete list is to look at
     * {@link http://cvs.agpu.org/agpu/theme/base/config.php?view=markup the base theme config.php file}.
     ...
     *
     * This is an advanced feature. agpu output is generated by 'renderers',
     * you can customise the HTML that is output by writing custom renderers,
     * and then you need to specify 'renderer factory' so that agpu can find
     * your renderers.
     *
     * There are some renderer factories supplied with agpu. Please follow these
     ...
     *  - 'html5' the only officially supported doctype in agpu
    ...
    public static function load($themename) {
        ...
        try {
            ...
            // Most probably agpu tables not created yet.
        ...
        if (is_readable($rendererfile)) {
            ...
            if (is_readable($this->dir . '/renderer.php')) {
                debugging('Developer hint: ' . $this->dir . '/renderer.php should be renamed to ' . $this->dir . "/renderers.php.
                    See: http://docs.agpu.org/dev/Output_renderers#Theme_renderers.", DEBUG_DEVELOPER);
            }
        }
     ...
     */
    public function init_page(agpu_page $page) {
    ...
    public function editor_css_url($encoded = true) {
        ...
        if ($rev > -1) {
            ...
            $url = new agpu_url("/theme/styles.php");
            ...
            $url = new agpu_url('/theme/styles_debug.php', [
                'theme' => $this->name,
                'type' => $type,
            ]);
        }
     ...
     *
     * @param agpu_page $page Not used... deprecated?
     * @return agpu_url[]
     */
    public function css_urls(agpu_page $page) {
        global $CFG;
        ...
        if ($rev > -1) {
            ...
            $baseurl = new agpu_url('/theme/styles_debug.php');
            ...
            if (core_useragent::is_ie()) {
                ...
                $urls[] = new agpu_url($baseurl, ['theme' => $this->name, 'type' => 'ie', 'subtype' => 'plugins']);
                ...
                if ($this->get_scss_property()) {
                    ...
                    $urls[] = new agpu_url($baseurl, ['theme' => $this->name, 'type' => 'scss']);
                }
                $urls[] = new agpu_url($baseurl, ['theme' => $this->name, 'type' => 'ie', 'subtype' => 'theme']);
            } else {
                foreach ($css['plugins'] as $plugin => $unused) {
                    $urls[] = new agpu_url($baseurl, ['theme' => $this->name, 'type' => 'plugin', 'subtype' => $plugin]);
                }
                foreach ($css['parents'] as $parent => $sheets) {
                ...
                foreach ($css['theme'] as $sheet => $filename) {
                    if ($sheet === self::SCSS_KEY) {
                        ...
                        $urls[] = new agpu_url($baseurl, ['sheet' => $sheet, 'theme' => $this->name, 'type' => 'theme']);
                    }
                }
            }
        }
    ...
    public function javascript_url($inhead) {
        ...
        if (!empty($CFG->slasharguments) && $rev > 0) {
            $url = new agpu_url("/theme/javascript.php");
            $url->set_slashargument('/' . $this->name . '/' . $rev . '/' . $params['type'], 'noparam', true);
            return $url;
        } else {
            return new agpu_url('/theme/javascript.php', $params);
        }
    }
    ...
    public function image_url($imagename, $component) {
        ...
        if (empty($component) || $component === 'agpu' || $component === 'core') {
        ...
        $url = new agpu_url("/theme/image.php");
        if (!empty($CFG->slasharguments) && $rev > 0) {
    ...
    public function font_url($font, $component) {
        ...
        $url = new agpu_url("/theme/font.php");
        if (!empty($CFG->slasharguments) && $rev > 0) {
    ...
    public function setting_file_url($setting, $filearea) {
        ...
        $url = agpu_url::make_file_url(
            "$CFG->wwwroot/pluginfile.php",
            "/$syscontext->id/$component/$filearea/$itemid" . $filepath,
        );

        // Now this is tricky because the we can not hardcode http or https here, lets use the relative link.
        // Note: unfortunately agpu_url does not support //urls yet.
    ...
    public function resolve_image_location($image, $component, $svg = false) {
        ...
        if ($component === 'agpu' || $component === 'core' || empty($component)) {
            ...
            // In agpu 4.0 we introduced a new image format.
     ...
     * At that point the were the following image counts in agpu core:
     *
    ...
    /**
     * Get the renderer for a part of agpu for this theme.
     *
     * @param agpu_page $page the page we are rendering
     * @param string $component the name of part of agpu. E.g. 'core', 'quiz', 'qtype_multichoice'.
