...
class participants_action_bar implements renderable {
     ...
     */
    public function __construct(object $course, agpu_page $page, ?string $renderedcontent) {
        $this->course = $course;
        $this->page = $page;
        $node = 'users';
    ...
    protected function get_content_for_select(): array {
        ...
        if (
            ...
            if ($canviewparticipants) {
                $participantsurl = (new agpu_url('/user/index.php', ['id' => $this->course->id]))->out();
                $formattedcontent[] = [
                    $enrolmentsheading => [
            ...
            foreach ($content as $key) {
                if ($node = $this->node->find($key, null)) {
                    ...
                    if ($key === 'groups') {
                        ...
                        $items += [
                            (new agpu_url('/group/groupings.php', $params))->out() => get_string('groupings', 'group'),
                            (new agpu_url('/group/overview.php', $params))->out() => get_string('overview', 'group'),
                        ];
                    }
                }
            }
    ...
    protected function find_active_page(array $urlcontent, int $strictness = URL_MATCH_EXACT): string {
        foreach ($urlcontent as $key => $value) {
            if (is_array($value) && $activeitem = $this->find_active_page($value, $strictness)) {
                return $activeitem;
            } else if ($this->page->url->compare(new agpu_url($key), $strictness)) {
                return $key;
            }
        }
