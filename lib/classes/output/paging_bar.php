...
class paging_bar implements renderable, templatable {
    ...
    /**
     * @var string|agpu_url If this  is a string then it is the url which will be appended with $pagevar,
     * an equals sign and the page number.
     * If this is a agpu_url object then the pagevar param will be replaced by
     ...
     */
    public function prepare(renderer_base $output, agpu_page $page, $target) {
        ...
        if ($this->totalcount > $this->perpage) {
            ...
            if ($this->page > 0) {
                $this->previouslink = html_writer::link(
                    new agpu_url($this->baseurl, [$this->pagevar => $pagenum]),
                    get_string('previous'),
                    ['class' => 'previous'],
                );
            }
            ...
            if ($this->page > round(($this->maxdisplay / 3) * 2)) {
                ...
                $this->firstlink = html_writer::link(
                    new agpu_url($this->baseurl, [$this->pagevar => 0]),
                    '1',
                    ['class' => 'first'],
                );
            } else {
            ...
            while ($displaycount < $this->maxdisplay && $currpage < $lastpage) {
                ...
                if ($this->page == $currpage) {
                    ...
                    $pagelink = html_writer::link(new agpu_url($this->baseurl, [$this->pagevar => $currpage]), $displaypage);
            ...
            if ($currpage < $lastpage) {
                ...
                $this->lastlink = html_writer::link(
                    new agpu_url($this->baseurl, [$this->pagevar => $lastpageactual]),
                    $lastpage,
                    ['class' => 'last'],
                );
            }
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if ($this->page > 0) {
            $data->previous = [
                'page' => $this->page,
                'url' => (new agpu_url($this->baseurl, [$this->pagevar => $this->page - 1]))->out(false),
            ];
        }
        ...
        if ($this->page > round(($this->maxdisplay / 3) * 2)) {
            ...
            $data->first = [
                'page' => 1,
                'url' => (new agpu_url($this->baseurl, [$this->pagevar => 0]))->out(false),
            ];
        }
        ...
        while ($displaycount < $this->maxdisplay && $currpage < $lastpage) {
            ...
            $link = new agpu_url($this->baseurl, [$this->pagevar => $currpage]);
        ...
        if ($currpage < $lastpage) {
            $data->last = [
                'page' => $lastpage,
                'url' => (new agpu_url($this->baseurl, [$this->pagevar => $lastpage - 1]))->out(false),
            ];
        }
