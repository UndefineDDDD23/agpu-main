...
class context_header implements renderable, templatable {
     ...
     *                       url => Link for the button to head to. Should be a agpu_url.
    ...
    protected function format_button_images() {
        ...
        foreach ($this->additionalbuttons as $buttontype => $button) {
            ...
            if (!isset($button['image'])) {
                ...
                // Check to see if this is an internal agpu icon.
                $internalimage = $page->theme->resolve_image_location('t/' . $button['image'], 'agpu');
                if ($internalimage) {
