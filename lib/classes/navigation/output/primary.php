...
class primary implements renderable, templatable {
    /** @var \agpu_page $page the agpu page that the navigation belongs to */
    private $page = null;
    ...
    public function get_user_menu(renderer_base $output): array {
        ...
        if ($info->metadata['asotheruser']) {
            ...
            $usermenudata['metadata'][] = [
                'content' => get_string('loggedinas', 'agpu', $info->metadata['userfullname']),
                'classes' => 'viewingas'
            ];
        }
        ...
        if (!empty($langmenu)) {
            ...
            if (!empty($languageitems)) {
                ...
                if (has_capability('agpu/role:switchroles', $PAGE->context)) {
