...
class secondary extends view {
    ...
    protected function load_course_navigation(?navigation_node $rootnode = null): void {
        ...
        if (!empty($nodekeys = $rootnode->children->get_key_list())) {
            $rootnode->add_node(
                navigation_node::create($firstnodeidentifier, new \agpu_url('/course/view.php', ['id' => $course->id]),
     ...
     * $PAGE->secondarynav->set_overflow_selected_node(new agpu_url('restore');
    ...
    protected function load_module_navigation(settings_navigation $settingsnav, ?navigation_node $rootnode = null): void {
        ...
        if ($mainnode) {
            $url = new \agpu_url('/mod/' . $settingsnav->get_page()->activityname . '/view.php',
    ...
    protected function load_category_navigation(): void {
        ...
        if ($mainnode) {
            $url = new \agpu_url('/course/index.php', ['categoryid' => $this->context->instanceid]);
    ...
    protected function load_admin_navigation(): void {
        ...
        $isadminsearchpage = $PAGE->url->compare(new \agpu_url('/admin/search.php', ['query' => '']), URL_MATCH_PARAMS);
        if ($node) {
            ...
            if ($isadminsearchpage) {
                ...
                $siteadminnode->action = new \agpu_url("/admin/search.php", [], "link$node->key");
            }
            foreach ($node->children as $child) {
                if ($child->display && !$child->is_short_branch()) {
                    ...
                    if ($isadminsearchpage) {
                        ...
                        $child->action = new \agpu_url("/admin/search.php", [], "link$child->key");
                    }
    ...
    protected function load_single_activity_course_navigation(): void {
        ...
        if ($page->context instanceof \context_course) {
            ...
            $page->set_url(new \agpu_url('/mod/' . $page->activityname . '/view.php', ['id' => $page->cm->id]));
        }
        ...
        if (count($activitysecondarynode->children) > 1) {
            ...
            $viewactivityurl = new \agpu_url('/mod/' . $page->activityname . '/view.php', ['id' => $page->cm->id]);
