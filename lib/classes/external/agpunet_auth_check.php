...
/**
 * The external API to check whether a user has authorized for a given agpuNet OAuth 2 issuer.
 *
 * @package    core
 * @copyright  2023 Huong Nguyen <huongnv13@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class agpunet_auth_check extends external_api {
    ...
    /**
     * External function to check if the user is already authorized with agpuNet.
     *
    ...
    public static function execute(int $issuerid, int $courseid): array {
        ...
        $returnurl = new agpu_url('/admin/agpunet_oauth2_callback.php');
        $returnurl->param('issuerid', $issuerid);
        $returnurl->param('callback', 'yes');
        $returnurl->param('sesskey', sesskey());

        // Get the OAuth Client.
        if (!$oauthclient = api::get_user_oauth_client($issuer, $returnurl, agpunet_client::API_SCOPE_CREATE_RESOURCE, true)) {
