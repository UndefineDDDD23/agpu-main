...
use core\agpunet\course_partial_sender;
use core\agpunet\course_sender;
use core\agpunet\agpunet_client;
use core\agpunet\share_recorder;
...
/**
 * The external API to send course to agpuNet.
 *
 * @package    core
 * @copyright  2023 Safat Shahin <safat.shahin@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class agpunet_send_course extends external_api {
    ...
    /**
     * External function to send the course to agpuNet.
     *
     * @param int $issuerid The agpuNet OAuth 2 issuer ID
     * @param int $courseid The course ID
     * @param int $shareformat The share format being used, as defined by \core\agpunet\course_sender
    ...
    public static function execute(
        ...
        if (!$usercanshare) {
            return self::return_errors(
                $courseid,
                'errorpermission',
                get_string('nopermissions', 'error', get_string('agpunet:sharetoagpunet', 'agpu'))
            );
        }
        ...
        if (!$oauthclient = api::get_user_oauth_client(
            $issuer,
            new agpu_url($CFG->wwwroot),
            agpunet_client::API_SCOPE_CREATE_RESOURCE
        )) {
            return self::return_errors(
                $issuerid,
                'erroroauthclient',
        ...
        if (!$oauthclient->is_logged_in()) {
            return self::return_errors(
                $issuerid,
                'erroroauthclient',
                get_string('agpunet:issuerisnotauthorized', 'agpu')
            );
        }
        ...
        try {
            ...
            $agpunetclient = new agpunet_client($client, $oauthclient);
            if ($ispartialsharing) {
                $coursesender = new course_partial_sender($courseid, $USER->id, $agpunetclient,
                    $oauthclient, $cmids, $shareformat);
            } else {
                $coursesender = new course_sender($courseid, $USER->id, $agpunetclient, $oauthclient, $shareformat);
            }
            ...
            if (empty($result['drafturl'])) {
                ...
                return self::return_errors(
                    $result['responsecode'],
                    'errorsendingcourse',
                    get_string('agpunet:cannotconnecttoserver', 'agpu')
                );
            }
        } catch (\agpu_exception | \JsonException $e) {
    ...
    public static function execute_returns(): external_single_structure {
        return new external_single_structure([
            ...
            // We used PARAM_TEXT instead of PARAM_URL because the URL return from agpuNet may contain some characters.
            // It does not match with PARAM_URL, but the URL still works.
            // Since we just show the response resource URL to the user for them to navigate to agpuNet, it would be safe.
            'resourceurl' => new external_value(PARAM_TEXT, 'Resource URL from agpuNet'),
