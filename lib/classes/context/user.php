...
class user extends context {
    ...
    public function get_url() {
        ...
        if ($COURSE->id == SITEID) {
            $url = new agpu_url('/user/profile.php', array('id' => $this->_instanceid));
        } else {
            $url = new agpu_url('/user/view.php', array('id' => $this->_instanceid, 'courseid' => $COURSE->id));
        }
        return $url;
    }
    ...
    public function get_capabilities(string $sort = self::DEFAULT_CAPABILITY_SORT) {
        ...
        $extracaps = array('agpu/grade:viewall');
