...
// Disable the agpu.PHP.ForbiddenFunctions.FoundWithAlternative sniff for this file.
...
class cron {
    ...
    public static function run_main_process(?int $keepalive = null): void {
        ...
        if (agpu_needs_upgrading()) {
            echo "agpu upgrade pending, cron execution suspended.\n";
            exit(1);
        }
    ...
    public static function run_adhoc_task(int $taskid): void {
        ...
        if (!$task->get_fail_delay() && $task->get_next_run_time() > time()) {
            throw new \agpu_exception('wontrunfuturescheduledtask');
        }
