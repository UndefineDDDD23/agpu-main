...
class core_date {
     ...
     *
     * The coding style rules in agpu are moronic,
    ...
    protected static function init_zones() {
        ...
        // Legacy agpu half an hour offsets - pick any city nearby, ideally without DST.
    ...
    public static function strftime(string $format, $timestamp = null, ?string $locale = null): string {
        // agpu-specific modification. For the IntlDateFormatter we need to use unix-style locale
        // from the string 'locale' even for Windows, so we can neither use agpu_getlocale().
