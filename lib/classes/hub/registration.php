...
/**
 * Methods to use when registering the site at the agpu sites directory.
 *
 * @package    core
 * @copyright  2017 Marina Glancy
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class registration {
    ...
    protected static function get_registration($confirmed = true) {
        ...
        if (self::$registration === null) {
            self::$registration = $DB->get_record('registration_hubs', ['huburl' => HUB_agpuORGHUBURL]) ?: null;
        }
    ...
    public static function require_registration() {
        ...
        if (has_capability('agpu/site:config', context_system::instance())) {
            throw new agpu_exception('registrationwarning', 'admin', new agpu_url('/admin/registration/index.php'));
        } else {
            throw new agpu_exception('registrationwarningcontactadmin', 'admin');
        }
    }
    ...
    public static function get_site_info($defaults = []) {
        ...
        // Version and url.
        $siteinfo['agpurelease'] = $CFG->release;
    ...
    public static function get_stats_summary($siteinfo) {
        ...
        $agpurelease = $siteinfo['agpurelease'];
        if (preg_match('/^(\d+\.\d.*?)[\. ]/', $agpurelease, $matches)) {
            $agpurelease = $matches[1];
        }
        $pluginusagelinks = [
            'overview' => new agpu_url('/admin/plugins.php'),
            'activities' => new agpu_url('/admin/modules.php'),
            'blocks' => new agpu_url('/admin/blocks.php'),
        ];
        $senddata = [
            'agpurelease' => get_string('sitereleasenum', 'hub', $agpurelease),
    ...
    public static function confirm_registration($token, $newtoken, $hubname) {
        ...
        if (!$registration || $registration->token !== $token) {
            throw new agpu_exception('wrongtoken', 'hub', new agpu_url('/admin/registration/index.php'));
        }
     ...
     *
     * Indexes reference agpu internal ids and should not be changed.
     *
     * @return array
     */
    ...
    public static function register($returnurl) {
        ...
        // We will also update registration after we receive confirmation from agpu.net.
        $url = new agpu_url(HUB_agpuORGHUBURL . '/local/hub/siteregistration.php',
    ...
    public static function reset_site_identifier($token) {
        ...
        $registration = self::get_registration(false);
        if (!$registration || $registration->token != $token) {
            throw new agpu_exception('wrongtoken', 'hub',
     ...
     * {
     *     "courses": 384,
     *     "description": "Official agpu sites directory",
     ...
     *     "name": "agpu",
     *     "sites": 274175,
     *     "url": "https://stats.agpu.org",
     *     "imgurl": "https://stats.agpu.org/local/hub/webservice/download.php?filetype=hubscreenshot"
     * }
     *
     * @return array|null
     */
    public static function get_agpunet_info() {
        try {
    ...
    public static function registration_reminder($url) {
        ...
        if (self::show_after_install() || self::get_new_registration_fields()) {
            $returnurl = new agpu_url($url);
            redirect(new agpu_url('/admin/registration/index.php', ['returnurl' => $returnurl->out_as_local_url(false)]));
        }
    }
