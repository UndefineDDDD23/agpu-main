...
class user {
    ...
    /** @var int Indicates that user profile view should be allowed even if agpu would prevent it */
     ...
     *
     * If you have agpu/user:viewdetails at system level, you can search everyone.
     * Otherwise we check which courses you *do* have that permission and search everyone who is
     * enrolled on those courses.
     *
     * Normally you can only search the user's name. If you have the agpu/site:viewuseridentity
    ...
    public static function search(
        ...
        $viewsystem = has_capability('agpu/user:viewdetails', $systemcontext);
        if ($viewsystem) {
        ...
        if ($extrasql) {
            $identitysystem = has_capability('agpu/site:viewuseridentity', $systemcontext);
            if ($identitysystem) {
    ...
    public static function require_active_user($user, $checksuspended = false, $checknologin = false) {
        ...
        if (isguestuser($user)) {
            throw new agpu_exception('guestsarenotallowed', 'error');
        }

        if ($checksuspended && $user->suspended) {
            throw new agpu_exception('suspended', 'auth');
        }
    ...
    protected static function fill_preferences_cache() {
        ...
        $preferences['auth_forcepasswordchange'] = [
            ...
            'permissioncallback' => function ($user, $preferencename) {
                global $USER;
                $systemcontext = context_system::instance();
                return ($USER->id != $user->id && (has_capability('agpu/user:update', $systemcontext) ||
                        ($user->timecreated > time() - 10 && has_capability('agpu/user:create', $systemcontext))));
            },
        ];
        ...
        $preferences['blogpagesize'] = [
            ...
            'permissioncallback' => function ($user, $preferencename) {
                return self::is_current_user($user) && has_capability('agpu/blog:view', context_system::instance());
            },
        ];
    ...
    protected static function default_preference_permission_check($user, $preferencename) {
        ...
        if (self::is_current_user($user)) {
            // Editing own profile.
            $systemcontext = context_system::instance();
            return has_capability('agpu/user:editownprofile', $systemcontext);
        } else {
            // Teachers, parents, etc.
            $personalcontext = context_user::instance($user->id);
            if (!has_capability('agpu/user:editprofile', $personalcontext)) {
                return false;
            }
    ...
    /**
     * Is the user expected to perform an action to start using agpu properly?
     *
