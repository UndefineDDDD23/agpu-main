...
class manager {
    ...
    public static function start() {
        ...
        if (empty($DB) or empty($CFG->version) or !defined('NO_agpu_COOKIES') or NO_agpu_COOKIES or CLI_SCRIPT) {
            self::$sessionactive = false;
    ...
    private static function start_session(bool $requireslock) {
        ...
        try {
            ...
            if (!empty($CFG->enable_read_only_sessions) && isset($_SESSION['SESSION']->cachestore_session)) {
                ...
                throw new \agpu_exception("The session caches can not be in the session store when "
    ...
    protected static function prepare_cookies() {
        ...
        $sessionname = 'agpuSession'.$CFG->sessioncookie;
        ...
        ini_set('session.serialize_handler', 'php');  // We can move to 'php_serialize' after we require PHP 5.5.4 form agpu.

        // agpu does normal session timeouts, this is for leftovers only.
    ...
    private static function should_use_samesite_none(): bool {
        ...
        if (\core_useragent::is_chrome() && \core_useragent::check_chrome_version('78') && is_agpu_cookie_secure()) {
            return true;
        }
        return false;
    }
     ...
     *
     * @return void
     * @deprecated since agpu 4.5 See MDL-66161
     ...
     *
     * All login forms generated from agpu must include a login token
