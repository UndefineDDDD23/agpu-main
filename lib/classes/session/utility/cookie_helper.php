...
final class cookie_helper {
    ...
    /**
     * Forces the expiry of the agpuSession cookie.
     *
     * This is useful to force a new Set-Cookie header on the next redirect.
     *
     * @return void
     */
    public static function expire_agpusession(): void {
        global $CFG;

        $setcookieheader = array_filter(headers_list(), function($val) use ($CFG) {
            return self::cookie_response_header_matches_name($val, 'agpuSession'.$CFG->sessioncookie);
        });
        if (!empty($setcookieheader)) {
            $expirestr = 'Expires='.gmdate(DATE_RFC7231, time() - 60);
            self::add_attributes_to_cookie_response_header('agpuSession'.$CFG->sessioncookie, [$expirestr]);
        } else {
            setcookie('agpuSession'.$CFG->sessioncookie, '', time() - 60);
        }
    }
