...
class client extends \oauth2_client {
     ...
     * @throws agpu_exception if an invalid response is received or if the response contains errors.
     */
    protected function exchange_refresh_token(string $refreshtoken): ?array {
        ...
        if ($this->info['http_code'] !== 200) {
            ...
            throw new agpu_exception('oauth2refreshtokenerror', 'core_error', '', $this->info['http_code'], $debuginfo);
        }
     ...
     *
     * @return stdClass|false agpu user fields for the logged in user (or false if request failed)
     * @throws agpu_exception if the response is empty after decoding it.
     */
    public function get_raw_userinfo() {
        ...
        if (is_null($userinfo)) {
            ...
            throw new agpu_exception($response);
        }
     ...
     * the fields back into agpu fields.
     *
     * @return stdClass|false agpu user fields for the logged in user (or false if request failed)
     * @throws agpu_exception if the response is empty after decoding it.
     */
    ...
    protected function map_userinfo_to_fields(stdClass $userinfo): array {
        ...
        $user = new stdClass();
        foreach ($map as $openidproperty => $agpuproperty) {
