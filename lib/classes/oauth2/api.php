...
class api {
    ...
    public static function init_standard_issuer($type) {
        ...
        throw new agpu_exception('OAuth 2 service type not recognised: ' . $type);
    }
     ...
     *
     * @param string $type One of google, facebook, microsoft, agpuNet, nextcloud or imsobv2p1
     * @param string|false $baseurl Baseurl (only required for nextcloud, imsobv2p1 and agpunet)
     * @return \core\oauth2\issuer
     */
    public static function create_standard_issuer($type, $baseurl = false) {
        ...
        switch ($type) {
            case 'imsobv2p1':
                if (!$baseurl) {
                    throw new agpu_exception('IMS OBv2.1 service type requires the baseurl parameter.');
                }
            case 'nextcloud':
                if (!$baseurl) {
                    throw new agpu_exception('Nextcloud service type requires the baseurl parameter.');
                }
            case 'agpunet':
                if (!$baseurl) {
                    throw new agpu_exception('agpuNet service type requires the baseurl parameter.');
                }
            case 'google':
            case 'facebook':
            case 'microsoft':
     ...
     * @throws agpu_exception Request for token upgrade failed for technical reasons
     */
     ...
     * @param agpu_url $currenturl The url to the current page.
     ...
     */
    public static function get_user_oauth_client(issuer $issuer, agpu_url $currenturl, $additionalscopes = '',
            $autorefresh = false) {
        $class = self::get_client_classname($issuer->get('servicetype'));
    ...
    /**
     * Get the list of defined mapping from OAuth user fields to agpu user fields.
     *
     * @param \core\oauth2\issuer $issuer The desired OAuth issuer
     * @return \core\oauth2\user_field_mapping[]
     */
     ...
     *
     * Requires agpu/site:config capability at the system context.
     *
     ...
     *
     * @param \core\oauth2\issuer $issuer
     * @param agpu_url $returnurl The url to the current page (we will be redirected back here after authentication).
     * @return boolean
     */
