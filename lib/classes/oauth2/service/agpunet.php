...
/**
 * agpuNet OAuth 2 configuration.
 *
 * @package    core
 * @copyright  2023 Jake Dallimore <jrhdallimore@gmail.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class agpunet implements issuer_interface {
    ...
    public static function discover_endpoints($issuer): int {
        ...
        try {
            $config = $configreader->read_configuration(new \agpu_url($baseurl));
            ...
            throw new \agpu_exception('Could not read service configuration for issuer: ' . $issuer->get('name'));
        }

        try {
            self::client_registration($issuer);
        } catch (\Exception $e) {
            throw new \agpu_exception('Could not register client for issuer: ' . $issuer->get('name'));
        }
    ...
    /**
     * Perform (open) OAuth 2 Dynamic Client Registration with the agpuNet application.
     *
        ...
        if (empty($clientid) && empty($clientsecret)) {
            ...
            if ($url) {
                ...
                $request = [
                    ...
                    'client_uri' => $hosturl,
                    'logo_uri' => $hosturl . '/pix/agpulogo.png',
                    'tos_uri' => $hosturl,
                    'policy_uri' => $hosturl,
                    'software_id' => 'agpu',
                ...
                try {
                    ...
                    if (is_null($decodedbody)) {
                        throw new \agpu_exception('Error: ' . __METHOD__ . ': Failed to decode response body. Invalid JSON.');
                    }
