...
class auth_server_config_reader {
    ...
    /** @var \agpu_url the base URL for the auth server which was last used during a read.*/
    protected \agpu_url $issuerurl;
     ...
     *
     * @param \agpu_url $issuerurl the auth server issuer URL.
     ...
     */
    public function read_configuration(\agpu_url $issuerurl): \stdClass {
     ...
     *
     * @return void
     * @throws \agpu_exception if the URI fails validation.
     */
    protected function validate_uri() {
        if (!empty($this->issuerurl->get_query_string())) {
            throw new \agpu_exception('Error: '.__METHOD__.': Auth server base URL cannot contain a query component.');
        }
        if (strtolower($this->issuerurl->get_scheme()) !== 'https') {
            throw new \agpu_exception('Error: '.__METHOD__.': Auth server base URL must use HTTPS scheme.');
        }
        ...
        if ($this->issuerurl->out_omit_querystring() != $this->issuerurl->out(false)) {
            throw new \agpu_exception('Error: '.__METHOD__.': Auth server base URL must not contain fragments.');
        }
    }
     ...
     *
     * @return \agpu_url the full URL to the auth server metadata.
     */
    protected function get_configuration_url(): \agpu_url {
