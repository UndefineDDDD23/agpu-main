...
class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        // Except for agpunet_share_progress, these tables are really data about site configuration and not user data.
        ...
        // The agpunet_share_progress includes details of an attempted share of a resource to agpuNet.
        $collection->add_database_table('agpunet_share_progress', [
            'type' => 'privacy:metadata:agpunet_share_progress:type',
            'courseid' => 'privacy:metadata:agpunet_share_progress:courseid',
            'cmid' => 'privacy:metadata:agpunet_share_progress:cmid',
            'userid' => 'privacy:metadata:agpunet_share_progress:userid',
            'timecreated' => 'privacy:metadata:agpunet_share_progress:timecreated',
            'resourceurl' => 'privacy:metadata:agpunet_share_progress:resourceurl',
            'status' => 'privacy:metadata:agpunet_share_progress:status',
        ], 'privacy:metadata:agpunet_share_progress');

        // This resourceurl field is an external link from agpuNet.
        $collection->add_external_location_link('agpunet_share_progress', [
            'resourceurl' => 'privacy:metadata:agpunet_share_progress:resourceurl',
        ], 'privacy:metadata:agpunet_share_progress');
    ...
    public static function get_contexts_for_userid(int $userid): contextlist {
        ...
        // agpuNet share progress uses the user context.
        $sql = "SELECT ctx.id
                  FROM {context} ctx
                  JOIN {agpunet_share_progress} msp ON ctx.instanceid = msp.userid
    ...
    public static function get_users_in_context(userlist $userlist) {
        // Except for agpunet_share_progress, don't add any users.
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        // Except for agpunet_share_progress, none of the core tables should be exported.
        global $DB;

        foreach ($contextlist as $context) {
            // agpuNet share progress uses the user context.
            if ($context->contextlevel == CONTEXT_USER && $context->instanceid == $contextlist->get_user()->id) {
                // Get the user's agpuNet share progress data.
                $sharedata = $DB->get_records('agpunet_share_progress', ['userid' => $context->instanceid]);
                $subcontext = get_string('privacy:metadata:agpunet_share_progress', 'agpu');
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        // Except for agpunet_share_progress, none of the the data from these tables should be deleted.
        global $DB;

        // agpuNet share progress uses the user context.
        if ($context->contextlevel == CONTEXT_USER) {
            $DB->delete_records('agpunet_share_progress', ['userid' => $context->instanceid]);
        }
    }
