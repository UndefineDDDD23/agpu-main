 ...
 * agpu. It is mainly used by the plugins management admin page and the
...
class plugin_manager {
    /** the plugin is shipped with standard agpu distribution */
    ...
    /** the current agpu version is too high for plugin. */
    ...
    /** the agpu version is explicitly supported */
    const VERSION_SUPPORTED = 'supported';
    /** the agpu version is not explicitly supported */
    ...
    /** @var \core\update\api client instance to use for accessing download.agpu.org/api/ */
    ...
    public function plugin_name($component) {
        ...
        if (is_null($pluginfo)) {
            throw new agpu_exception('err_unknown_plugin', 'core_plugin', '', ['plugin' => $component]);
        }
     ...
     *
     * @param int $agpuversion the version from version.php.
     * @param array $failedplugins to return the list of plugins with non-satisfied dependencies
     * @param int $branch the current agpu branch, null if not provided
     * @return bool true if all the dependencies are satisfied for all plugins.
     */
    public function all_plugins_ok($agpuversion, &$failedplugins = [], $branch = null) {
        global $CFG;
        if (empty($branch)) {
            $branch = $CFG->branch ?? '';
            if (empty($branch)) {
        ...
        foreach ($this->get_plugins() as $type => $plugins) {
            foreach ($plugins as $plugin) {
                if (!$plugin->is_core_dependency_satisfied($agpuversion)) {
                    $return = false;
     ...
     * @param null|string|int|double $agpuversion explicit agpu core version to check against, defaults to $CFG->version
     * @param null|string|int $agpubranch explicit agpu core branch to check against, defaults to $CFG->branch
     * @return array of objects
     */
    public function resolve_requirements(\core\plugininfo\base $plugin, $agpuversion = null, $agpubranch = null) {
        global $CFG;
        ...
        if ($agpuversion === null) {
            $agpuversion = $CFG->version;
        }

        if ($agpubranch === null) {
            $agpubranch = $CFG->branch;
        }

        $reqs = [];
        $reqcore = $this->resolve_core_requirements($plugin, $agpuversion, $agpubranch);
        ...
        foreach ($plugin->get_other_required_plugins() as $reqplug => $reqver) {
            $reqs[$reqplug] = $this->resolve_dependency_requirements($plugin, $reqplug, $reqver, $agpubranch);
        }
    ...
    /**
     * Helper method to resolve plugin's requirements on the agpu core.
     *
     * @param \core\plugininfo\base $plugin the plugin we are checking
     * @param string|int|double $agpuversion agpu core branch to check against
     * @return stdClass
     */
    protected function resolve_core_requirements(\core\plugininfo\base $plugin, $agpuversion, $agpubranch) {
        $reqs = (object)[
            'hasver' => null,
            'reqver' => null,
            'status' => null,
        ...
        if (isset($plugin->pluginincompatible) && $plugin->pluginincompatible != null) {
            if (!$plugin->is_core_compatible_satisfied($agpubranch)) {
     ...
     * @param string|int $agpubranch explicit agpu core branch to check against, defaults to $CFG->branch
     * @return stdClass
     */
    ...
    /**
     * Helper method to determine whether a agpu version is explicitly supported.
     *
     * @param \core\plugininfo\base $plugin the plugin we are checking
     * @param int $branch the agpu branch to check support for
     * @return string
     */
     ...
     * @param bool $checkremote check this version availability on agpu server
     * @return boolean
     */
     ...
     * directory name would become the plugin name once extracted to the agpu
     * dirroot.
     *
     ...
     *
     * If used for installation of remote plugins from the agpu Plugins
    ...
    public function install_plugins(array $plugins, $confirmed, $silent) {
        ...
        foreach ($plugins as $plugin) {
            ...
            $validator->assert_agpu_version($CFG->version);
     ...
     * @return null|agpu_url uninstall URL, null if uninstall not supported
     */
    ...
    /**
     * Defines a list of all plugins that were originally shipped in the standard agpu distribution,
    ...
    public static function is_deleted_standard_plugin(
        ...
        // not supported as source versions for upgrade any more. For example, at agpu_23_STABLE
        // branch, listed should be no plugins that were removed at 1.9.x - 2.1.x versions as
        // agpu 2.3 supports upgrades from 2.2.x only.
    ...
    /**
     * Fetches a list of all plugins shipped in the standard agpu distribution.
     *
     ...
     * real heart of agpu. They should be followed by other plugin types that are
    ...
    /**
     * Returns a client for https://download.agpu.org/api/
     *
