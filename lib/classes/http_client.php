...
class http_client extends Client {
    ...
    /**
     * Get the custom options and handlers for guzzle integration in agpu.
     *
    ...
    protected function get_options(array $settings): array {
        ...
        if (!isset($settings['headers'])) {
            $settings['headers'] = ['User-Agent' => \core_useragent::get_agpubot_useragent()];
    ...
    protected function get_handlers(array $settings): HandlerStack {
        ...
        $stack->unshift(check_request::setup($settings), 'agpu_check_initial_request');

        // Replace the standard redirect handler with our custom agpu one.
        // This handler checks the block list.
        // It extends the standard 'allow_redirects' handler so supports the same options.
        $stack->after('allow_redirects', redirect_middleware::setup($settings), 'agpu_allow_redirect');
