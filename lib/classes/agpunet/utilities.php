...
/**
 * Class containing static utilities (such as various checks) required by the agpuNet API.
 *
 * @package   core
...
class utilities {
    /**
     * Check whether the specified issuer is configured as a agpuNet instance that can be shared to.
     *
    ...
    public static function is_valid_instance(issuer $issuer): bool {
        ...
        $allowedissuer = get_config('agpunet', 'oauthservice');

        return ($CFG->enablesharingtoagpunet && $issuerid == $allowedissuer && $issuer->get('enabled') &&
            $issuer->get('servicetype') == 'agpunet');
    }

    /**
     * Check whether a user has the capabilities required to share activities or courses to agpuNet.
     *
    ...
    public static function can_user_share(\core\context\course $coursecontext, int $userid, string $type = 'activity'): bool {
        if ($type === 'course') {
            return (has_capability('agpu/agpunet:sharecourse', $coursecontext, $userid) &&
                has_capability('agpu/backup:backupcourse', $coursecontext, $userid));
        } else if ($type === 'activity') {
            return (has_capability('agpu/agpunet:shareactivity', $coursecontext, $userid) &&
                has_capability('agpu/backup:backupactivity', $coursecontext, $userid));
        }
    ...
    public static function does_user_have_capability_in_any_course(int $userid): string {
        ...
        $isallowed = 'no';
        $cache = \cache::make('core', 'agpunet_usercanshare');
