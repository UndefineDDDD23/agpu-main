...
/**
 * Base packager to prepare appropriate backup of a resource to share to agpuNet.
 *
 * @package   core
...
abstract class resource_packager {
    ...
    public function get_package(): stored_file {
        ...
        // Override relevant settings to remove user data when packaging to share to agpuNet.
    ...
    protected function package(backup_controller $controller): stored_file {
        ...
        if (!isset($result['backup_destination'])) {
            throw new \agpu_exception('Failed to package resource.');
        }

        $backupfile = $result['backup_destination'];

        if (!$backupfile->get_contenthash()) {
            throw new \agpu_exception('Failed to package resource (invalid file).');
        }
