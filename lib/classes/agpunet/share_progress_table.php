...
class share_progress_table extends table_sql {
     ...
     * @param agpu_url $url The base URL.
    ...
    protected function define_table_columns() {
        ...
        $cols = [
            ...
            'type' => get_string('agpunet:columntype'),
            'timecreated' => get_string('agpunet:columnsenddate'),
            'status' => get_string('agpunet:columnsendstatus'),
        ];
    ...
    protected function col_name(stdClass $row): string {
        ...
        $class = '';
        // Track deletion of resources on agpu.
        $deleted = false;
        // Courses.
        if ($row->type == share_recorder::TYPE_COURSE) {
            if ($row->fullname) {
                ...
                $name = get_string('agpunet:deletedcourse');
                $deleted = true;
            }
        // Activities.
            ...
            if ($cm = get_coursemodule_from_id('', $row->cmid)) {
                ...
                $name = get_string('agpunet:deletedactivity');
                $deleted = true;
            }
        }
        if ($deleted) {
        ...
        if (!empty($row->resourceurl)) {
            ...
            $icon = $OUTPUT->pix_icon('i/externallink', get_string('opensinnewwindow'), 'agpu', ['class' => 'ms-1']);
            $text = $name . $icon;
            $attributes = [
                'target' => '_blank',
    ...
    protected function get_sql_and_params($count = false) {
        ...
        $sql = "SELECT $select
                  FROM {agpunet_share_progress} msp
    ...
    public function print_nothing_to_display() {
        global $OUTPUT;
        echo $OUTPUT->notification(get_string('agpunet:nosharedresources'), 'info');
    }
}
