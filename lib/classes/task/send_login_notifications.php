...
class send_login_notifications extends adhoc_task {
    ...
    public function execute() {
        ...
        $logindevice = $customdata->isagpuapp ? get_string('mobileapp', 'tool_mobile') : '';
        $logindevice .= ' ' . $customdata->useragent;
        $loginip = $customdata->loginip;
        $logintime = userdate($customdata->logintime);

        $changepasswordlink = (new \agpu_url('/user/preferences.php', ['userid' => $USER->id]))->out(false);
        ...
        if ($userauth->can_change_password()) {
            if ($changepwurl = $userauth->change_password_url()) {
                ...
                $changepasswordlink = (new \agpu_url('/login/change_password.php'))->out(false);
            }
        }
        ...
        $eventdata->subject           = get_string('newloginnotificationtitle', 'agpu', $sitename);
        $eventdata->fullmessageformat = FORMAT_HTML;
        $info = compact('sitename', 'siteurl', 'userfullname', 'username', 'useremail',
            'logindevice', 'logintime', 'loginip', 'changepasswordlink');
        $eventdata->fullmessagehtml   = get_string('newloginnotificationbodyfull', 'agpu', $info);
        $eventdata->fullmessage       = html_to_text($eventdata->fullmessagehtml);
        $eventdata->smallmessage      = get_string('newloginnotificationbodysmall', 'agpu', $username);
