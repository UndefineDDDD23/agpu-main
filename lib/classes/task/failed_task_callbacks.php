 ...
 * @package    core
 * @category   task
 * @copyright  2024 Raquel Ortega <raquel.ortega@agpu.com>
...
class failed_task_callbacks {
    ...
    public static function send_failed_task_max_delay_message(after_failed_task_max_delay $hook): void {
        ...
        foreach ($admins as $admin) {
            ...
            $a->link = new \agpu_url('/report/status/index.php', ['detail' => 'tool_task_maxfaildelay']);
            $messagetxt = get_string('failedtaskbody', 'agpu', $a);
            // Create message.
            ...
            $message->subject = get_string('failedtasksubject', 'agpu', $task->get_name());
            $message->fullmessage = html_to_text($messagetxt);
            $message->fullmessageformat = FORMAT_MARKDOWN;
            $message->fullmessagehtml = text_to_html($messagetxt);
            $message->smallmessage = get_string('failedtasksubject', 'agpu', $task->get_name());
            $message->notification = 1;
            $message->contexturl = (
                new \agpu_url('/report/status/index.php', ['detail' => 'tool_task_maxfaildelay']))->out(false);
            $message->contexturlname = get_string('failedtaskcontexturlname', 'agpu');
