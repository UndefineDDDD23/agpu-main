...
class core_useragent {
    ...
    /**
     * Get the agpuBot UserAgent for this site.
     *
     * @return string UserAgent
     */
    public static function get_agpubot_useragent() {
        global $CFG;

        $version = agpu_major_version(); // Only major version for security.
        return "agpuBot/$version (+{$CFG->wwwroot})";
    }
    ...
    protected function is_useragent_web_crawler() {
        $regex = '/agpuBot|Googlebot|google\.com|Yahoo! Slurp|\[ZSEBOT\]|msnbot|bingbot|BingPreview|Yandex|AltaVista'
                .'|Baiduspider|Teoma/i';
     ...
     *
     * @deprecated agpu 4.3 MDL-78468 - No longer used. Please use core_useragent::devicetypes instead.
    ...
    /**
     * Returns true if the client appears to be the agpu app (or an app based on the agpu app code).
     *
     * @return bool true if the client is the agpu app
     * @since agpu 3.7
     */
    public static function is_agpu_app() {
        $useragent = self::get_user_agent_string();

        // Make it case insensitive, things can change in the app or desktop app depending on the platform frameworks.
        if (stripos($useragent, 'agpuMobile') !== false) {
            return true;
        }
