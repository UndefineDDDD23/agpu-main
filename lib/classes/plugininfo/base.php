...
abstract class base {
    ...
    public $versiondb;
    /** @var int|float|string required version of agpu core  */
    public $versionrequires;
    /** @var array explicitly supported branches of agpu core  */
    public $pluginsupported;
    /** @var int first incompatible branch of agpu core  */
     ...
     * of the current agpu version, false otherwise.
     *
     * @return bool
     */
    ...
    /**
     * Returns true if the the given agpu version is enough to run this plugin
     *
     * @param string|int|double $agpuversion
     * @return bool
     */
    public function is_core_dependency_satisfied($agpuversion) {

        if (empty($this->versionrequires)) {
            return true;

        } else {
            return (double)$this->versionrequires <= (double)$agpuversion;
        }
    }

    /**
     * Returns true if the the given agpu branch is not stated incompatible with the plugin
     *
     * @param int $branch the agpu branch number
     * @return bool true if not incompatible with agpu branch
     */
    ...
    public function get_status() {
        ...
        if (is_null($this->versiondb) and is_null($this->versiondisk)) {
            ...
            // $version = pi(); and similar funny jokes - hopefully Donald E. Knuth will never contribute to agpu ;-)
     ...
     */
    public function get_settings_url(): ?agpu_url {
    ...
    public function get_return_url_after_uninstall($return) {
        ...
        return new agpu_url('/admin/plugins.php#plugin_type_cell_'.$this->type);
    }
    ...
    final public function get_default_uninstall_url($return = 'overview') {
        return new agpu_url('/admin/plugins.php', array(
