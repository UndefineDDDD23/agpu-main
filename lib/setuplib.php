...
/**
 * These functions are required very early in the agpu
...
function get_whoops(): ?\Whoops\Run {
    ...
    $whoops->appendHandler(function ($exception, $inspector, $run) {
        ...
        if ($isdebugging) {
            ...
            // agpu exceptions often have a link to the agpu docs pages for them.
...
function get_exception_info($ex): stdClass {
    ...
    if ($ex instanceof agpu_exception) {
    ...
    // Be careful, no guarantee agpulib.php is loaded.
    if (empty($module) || $module == 'agpu' || $module == 'core') {
        $module = 'error';
    }
    ...
    if (function_exists('get_string_manager')) {
        ...
        } elseif ($module == 'error' && get_string_manager()->string_exists($errorcode, 'agpu')) {
            // Search in agpu file if error specified - needed for backwards compatibility
            $message = get_string($errorcode, 'agpu', $a);
        } else {
    ...
    if (!empty($CFG->errordocroot)) {
        ...
        if (function_exists('current_language')) {
            ...
            $errordoclink = 'https://docs.agpu.org/en/';
        }
    }
...
/**
 * @deprecated since agpu 3.8 MDL-61038 - please do not use this function any more.
...
/**
 * Returns the agpu Docs URL in the users language for a given 'More help' link.
 *
 ...
 * agpuDocs in the user's language, and for the appropriate agpu version.
 * E.g. 'group/import' may become 'http://docs.agpu.org/2x/en/group/import'.
 * The 'http://docs.agpu.org' bit comes from $CFG->docroot.
 *
 * This is the only option that should be used in standard agpu code. The other
 ...
 * @return string The agpuDocs URL in the user's language. for example @link http://docs.agpu.org/2x/en/$path}
 */
function get_docs_url($path = null) {
    ...
    // Otherwise we do the normal case, and construct a agpuDocs URL relative to $CFG->docroot.
...
function setup_validate_php_configuration() {
   ...
   if (ini_get_bool('session.auto_start')) {
        throw new \agpu_exception('sessionautostartwarning', 'admin');
   }
}
...
function hash_local_config_cache() {
    ...
    // This is pretty much {@see agpu_database::get_settings_hash()} that is used
...
function initialise_fullme() {
    ...
    if (substr($CFG->wwwroot, -1) == '/') {
        throw new \agpu_exception('wwwrootslash', 'error');
    }
    ...
    if (empty($rurl['host'])) {
        ...
        if (($rurl['host'] !== $wwwroot['host']) or
            ...
            if (defined('REQUIRE_CORRECT_ACCESS') && REQUIRE_CORRECT_ACCESS) {
                ...
                throw new agpu_exception('requirecorrectaccess', 'error', '', null,
            ...
            if (strpos($rfullpath, $wwwroot['path']) === 0) {
                ...
                $rfullpath = (new agpu_url($rfullpath))->out(false);
            }
    ...
    // (That is, the agpu server uses http, with an external box translating everything to https).
    if (empty($CFG->sslproxy)) {
        if ($rurl['scheme'] === 'http' and $wwwroot['scheme'] === 'https') {
            if (defined('REQUIRE_CORRECT_ACCESS') && REQUIRE_CORRECT_ACCESS) {
                throw new \agpu_exception('sslonlyaccess', 'error');
            } else {
        ...
        $rurl['scheme'] = 'https'; // make agpu believe it runs on https, squid or something else it doing it
    ...
    // Using agpu in "reverse proxy" mode, it's expected that the HTTP Host agpu receives is different
    // from the wwwroot configured host. Those URLs being identical could be the consequence of various
    // issues, including:
    // - Intentionally trying to set up agpu with 2 distinct addresses for intranet and Internet: this
    ...
    if (!empty($CFG->reverseproxy) && $rurl['host'] === $wwwroot['host'] && (empty($wwwroot['port']) || $rurl['port'] === $wwwroot['port'])) {
        throw new \agpu_exception('reverseproxyabused', 'error');
    }
...
/* NOTE: following servers are not fully tested! */
        ...
        throw new agpu_exception('unsupportedwebserver', 'error', '', $_SERVER['SERVER_SOFTWARE']);
    }
...
function workaround_max_input_vars() {
    ...
    if ($max >= 200000) {
        ...
        // because there are very many easier ways to DDoS any agpu server.
        return;
    }
 ...
 * in agpulib.php. This function is here so that we can
...
/**
 * Indicates whether we are in the middle of the initial agpu install.
 *
 * Very occasionally it is necessary avoid running certain bits of code before the
 * agpu installation has completed. The installed flag is set in admin/index.php
 * after agpu core and all the plugins have been installed, but just before
 ...
 * See {@link https://agpudev.io/docs/guides/upgrade#upgrade-code-restrictions}
 *
 * @throws agpu_exception if executed from inside of upgrade script and $warningonly is false
...
function upgrade_ensure_not_running($warningonly = false) {
    ...
    if (!empty($CFG->upgraderunning)) {
        if (!$warningonly) {
            throw new agpu_exception('cannotexecduringupgrade');
        } else {
...
function get_request_storage_directory($exceptiononerror = true, bool $forcecreate = false) {
    ...
    if ($createnewdirectory) {
        ...
        // paths on systems which host multiple agpus. We don't use the full id
...
function set_access_log_user() {
    ...
    if ($USER && isset($USER->username)) {
        ...
        if (!empty($logmethod)) {
            ...
            if ($logmethod == 'apache') {
                apache_note('agpuUSER', $logname);
            }

            if ($logmethod == 'header' && !headers_sent()) {
                header("X-agpuUSER: $logname");
            }
        }
    }
}
 ...
 * the 'http' handler. Ideally 100% of traffic uses the agpu curl
