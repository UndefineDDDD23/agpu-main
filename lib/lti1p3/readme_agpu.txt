...
This library is a patched for use in agpu - it requires the following changes be applied on top of the packback upstream base:
...
2. Apply the changes mentioned above from the agpu-fixes branch of this repository:
https://github.com/snake/lti-1-3-php-library/tree/agpu-fixes
...
7. Update the dependency note in lib/php-jwt/readme_agpu.txt, recording the version of php-jwt lib/lti1p3 depends on, if needed.
8. Check the upstream library's release notes and UPGRADES.md for any backwards incompatible changes to names, etc.
agpu's calling code may require updates if changes are breaking -  so check this and make any changes if needed.
9. Run all unit tests in enrol/lti and auth/lti.
10. Regression test agpu-to-agpu LTI using LTI Advantage (not legacy) using the relevant MDLQA tests as a guide.
