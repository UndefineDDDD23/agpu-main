...
function profiling_stop() {
    ...
    // config.php before agpu is installed. Rare but...
...
/**
 * Export profiling runs to a .mpr (agpu profile runs) file.
 *
 ...
 *     - agpu_profiling_runs.xml: Metadata about the information
 *         exported. Contains some header information (version and
 *         release of agpu, database, git hash - if available, date
 ...
 * profiling table for each run (agpu version, database, git hash...).
 *
...
/**
 * Import a .mpr (agpu profile runs) file into agpu.
 *
...
function profiling_import_runs($file, $commentprefix = '') {
    ...
    $fp = get_file_packer('application/vnd.agpu.profiling');
    $status = $fp->extract_to_pathname($file, $tmpdir);

    // Look for master file and verify its format.
    if ($status) {
        $mfile = $tmpdir . '/agpu_profiling_runs.xml';
...
function profiling_export_generate(array $runids, $tmpdir) {
    ...
    // Calculate the header information to be sent to agpu_profiling_runs.xml.
    $release = $CFG->release;
    $version = $CFG->version;
    $dbtype = $CFG->dbtype;
    ...
    $mainxo = new file_xml_output($tmpdir . '/agpu_profiling_runs.xml');
    $mainxw = new xml_writer($mainxo);

    // Output begins.
    $mainxw->start();
    $mainxw->begin_tag('agpu_profiling_runs');
    ...
    foreach ($runids as $runid) {
        ...
        $runxw->start();
        $runxw->begin_tag('agpu_profiling_run');
        ...
        $runxw->end_tag('agpu_profiling_run');
        $runxw->stop();
    }
    $mainxw->end_tag('runs');
    $mainxw->end_tag('agpu_profiling_runs');
    $mainxw->stop();
...
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
  <xs:element name="agpu_profiling_runs">
    <xs:complexType>
      <xs:sequence>
        <xs:element ref="info"/>
        <xs:element ref="runs"/>
...
function profiling_get_import_run_schema() {
    $schema = <<<EOS
<xs:schema xmlns:xs="http://www.w3.org/2001/XMLSchema" elementFormDefault="qualified">
  <xs:element name="agpu_profiling_run">
    <xs:complexType>
      <xs:sequence>
 ...
 */
class agpu_xhprofrun implements iXHProfRuns {
...
class xhprof_table_sql extends table_sql {
    ...
    protected function col_url($row) {
        ...
        $scripturl = new agpu_url('/admin/tool/profiling/index.php', array('script' => $row->url, 'listurl' => $row->url));
        ...
        if ($this->listurlmode) {
            ...
            $detailsurl = new agpu_url('/admin/tool/profiling/index.php', array('listurl' => $row->url));
    ...
    protected function col_timecreated($row) {
        ...
        $url = new agpu_url('/admin/tool/profiling/index.php', array('runid' => $row->runid, 'listurl' => $row->url));
