 ...
 *  fullmessageformat int the format if the full message (FORMAT_agpu, FORMAT_HTML, ..)
...
function message_send(\core\message\message $eventdata) {
    ...
    if (!$eventdata->notification && !$eventdata->convid) {
        ...
        if (empty($CFG->messaging) || $eventdata->component !== 'agpu' || $eventdata->name !== 'instantmessage') {
 ...
 *
 * @param string $component For example 'agpu', 'mod_forum' or 'block_activity_results'
 * @return boolean True on success
 */
function message_update_providers($component='agpu') {
    global $DB;
 ...
 *
 * @param  string $component   The name of component (e.g. agpu, mod_forum, etc.)
 ...
 * @param string $component A agpu component like 'agpu', 'mod_forum', 'block_activity_results'
 * @return array An array of message providers
 */
