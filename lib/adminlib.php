 ...
 *   -it defines the necessary objects and interfaces to build the agpu
 *    admin hierarchy
 *   -it defines the admin_externalpage_setup()
 *
 *  ADMIN_SETTING OBJECTS
 *
 *  agpu settings are represented by objects that inherit from the admin_setting
 ...
 *  appear in the agpu admin tree block. All interaction with admin_settingpage
 *  objects is handled by the admin/settings.php file.
 *
 *  ADMIN_EXTERNALPAGE OBJECTS
 *
 *  There are some settings in agpu that are too complex to (efficiently) handle
...
/**
 * Returns names of all known tables == tables that agpu knows about.
 *
...
function is_dataroot_insecure($fetchtest=false) {
    ...
    foreach($rp as $r) {
        if (strpos($siteroot, '/'.$r.'/') === 0) {
            $siteroot = substr($siteroot, strlen($r)+1); // agpu web in subdirectory
        } else {
...
class admin_externalpage implements part_of_admin_tree, linkable_settings_page {
     ...
     * @param mixed $req_capability The role capability/permission a user must have to access this external page. Defaults to 'agpu/site:config'.
     ...
     */
    public function __construct($name, $visiblename, $url, $req_capability='agpu/site:config', $hidden=false, $context=NULL) {
        $this->name        = $name;
        $this->visiblename = $visiblename;
        $this->url         = $url;
...
class admin_settingpage implements part_of_admin_tree, linkable_settings_page {
     ...
     *      if you specify something other than system or front page. Defaults to system.
     */
    public function __construct($name, $visiblename, $req_capability='agpu/site:config', $hidden=false, $context=NULL) {
...
abstract class admin_setting {
    ...
    private function parse_setting_name($name) {
        $bits = explode('/', $name);
        if (count($bits) > 2) {
            throw new agpu_exception('invalidadminsettingname', '', '', $name);
        }
    ...
    public function get_defaultsetting() {
        ...
        if (!empty($adminroot->custom_defaults)) {
            $plugin = is_null($this->plugin) ? 'agpu' : $this->plugin;
        ...
        if ($parentvalidation === true) {
            if ($this->maxlength > 0) {
                ...
                if ($length > $this->maxlength) {
                    return get_string('maximumchars', 'agpu',  $this->maxlength);
                }
                return true;
            } else {
 ...
 * get_users_from_config($CFG->mysetting, $capability); function in agpulib.php.
 *
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
class admin_setting_emoticons extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $context->reseturl = new agpu_url('/admin/resetemoticons.php');
 ...
 * @package     core
 * @category    admin
 * @copyright   2020 David Mudrák <david@agpu.com>
...
class admin_settings_num_course_sections extends admin_setting_configselect {
    ...
    /** Lazy-load the available choices for the select box */
    public function load_choices() {
        $max = get_config('agpucourse', 'maxsections');
 ...
 *
 * @copyright 2015 Adrian Greeve <adrian@agpu.com>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
class admin_setting_manageenrols extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $url = new agpu_url('/admin/enrol.php', array('sesskey'=>sesskey()));
        $printed = array();
        foreach($allenrols as $enrol => $unused) {
            ...
            if (isset($active_enrols[$enrol])) {
                $aurl = new agpu_url($url, array('action'=>'disable', 'enrol'=>$enrol));
                $hideshow = "<a href=\"$aurl\">";
                $hideshow .= $OUTPUT->pix_icon('t/hide', $strdisable) . '</a>';
                $enabled = true;
                $displayname = $name;
            } else if (isset($enrols_available[$enrol])) {
                $aurl = new agpu_url($url, array('action'=>'enable', 'enrol'=>$enrol));
                $hideshow = "<a href=\"$aurl\">";
            ...
            if ($PAGE->theme->resolve_image_location('icon', 'enrol_' . $name, false)) {
                ...
                $icon = $OUTPUT->pix_icon('spacer', '', 'agpu', array('class' => 'icon pluginicon noicon'));
            }
            ...
            if ($enabled) {
                if ($updowncount > 1) {
                    $aurl = new agpu_url($url, array('action'=>'up', 'enrol'=>$enrol));
                    $updown .= "<a href=\"$aurl\">";
                ...
                if ($updowncount < $enrolcount) {
                    $aurl = new agpu_url($url, array('action'=>'down', 'enrol'=>$enrol));
                    $updown .= "<a href=\"$aurl\">";
            ...
            if (!empty($enrols_available[$enrol]) and method_exists($enrols_available[$enrol], 'test_settings')) {
                $testsettingsurl = new agpu_url('/enrol/test_settings.php', ['enrol' => $enrol]);
...
class admin_page_managemessageoutputs extends admin_externalpage {
    ...
    public function __construct() {
        ...
        parent::__construct('managemessageoutputs',
            ...
            new agpu_url('/admin/message.php')
        );
    }
...
class admin_page_manageqbehaviours extends admin_externalpage {
    ...
    public function __construct() {
        ...
        parent::__construct('manageqbehaviours', get_string('manageqbehaviours', 'admin'),
                new agpu_url('/admin/qbehaviours.php'));
    }
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($displayauths as $auth => $name) {
            ...
            if (!empty($authplugins[$auth]) and method_exists($authplugins[$auth], 'test_settings')) {
                $testurl = new agpu_url('/auth/test_settings.php', ['auth' => $auth]);
...
class admin_setting_manageantiviruses extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $baseurl = new agpu_url('/admin/antiviruses.php', array('sesskey' => sesskey()));
        foreach ($antivirusesavailable as $antivirus => $name) {
            ...
            if (file_exists($CFG->dirroot.'/lib/antivirus/'.$antivirus.'/settings.php')) {
                $eurl = new agpu_url('/admin/settings.php', array('section' => 'antivirussettings'.$antivirus));
...
class admin_setting_manageformats extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $cnt = 0;
        $defaultformat = get_config('agpucourse', 'format');
        $spacer = $OUTPUT->pix_icon('spacer', '', 'agpu', array('class' => 'iconsmall'));
        foreach ($formats as $format) {
            $url = new agpu_url('/admin/courseformats.php',
            ...
            if ($format->is_enabled()) {
                ...
                if ($defaultformat === $format->name) {
                    ...
                    $hideshow = html_writer::link($url->out(false, array('action' => 'disable')),
                            $OUTPUT->pix_icon('t/hide', $txt->disable, 'agpu', array('class' => 'iconsmall')));
                }
            } else {
                $strformatname = $format->displayname;
                $class = 'dimmed_text';
                $hideshow = html_writer::link($url->out(false, array('action' => 'enable')),
                    $OUTPUT->pix_icon('t/show', $txt->enable, 'agpu', array('class' => 'iconsmall')));
            }
            $updown = '';
            if ($cnt) {
                $updown .= html_writer::link($url->out(false, array('action' => 'up')),
                    $OUTPUT->pix_icon('t/up', $txt->up, 'agpu', array('class' => 'iconsmall'))). '';
            } else {
                $updown .= $spacer;
            }
            if ($cnt < count($formats) - 1) {
                $updown .= '&nbsp;'.html_writer::link($url->out(false, array('action' => 'down')),
                    $OUTPUT->pix_icon('t/down', $txt->down, 'agpu', array('class' => 'iconsmall')));
            } else {
                $updown .= $spacer;
            }
            $cnt++;
        ...
        $return .= html_writer::table($table);
        $link = html_writer::link(new agpu_url('/admin/settings.php', array('section' => 'coursesettings')), new lang_string('coursesettings'));
...
class admin_setting_managecustomfields extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($fields as $field) {
            $url = new agpu_url('/admin/customfields.php',
...
class admin_setting_managedataformats extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($formats as $format) {
            ...
            $url = new agpu_url('/admin/dataformats.php',
...
abstract class admin_setting_manage_plugins extends admin_setting {
    ...
    protected function get_manage_url() {
        return new agpu_url('/admin/updatesetting.php');
    }
    ...
    public function output_html($data, $query = '') {
        ...
        $context = (object) [
            'manageurl' => new agpu_url($this->get_manage_url(), [
        ...
        foreach ($plugins as $key => $plugin) {
            $pluginlink = new agpu_url($context->manageurl, ['plugin' => $key]);
            ...
            // Enable/Disable link.
            $togglelink = new agpu_url($pluginlink);
            if ($plugin->is_enabled()) {
                ...
                if (count($context->plugins)) {
                    ...
                    $pluginkey->moveuplink = new agpu_url($pluginlink, ['action' => 'up']);
                }

                if (count($enabled) > count($context->plugins) + 1) {
                    // This is not the last plugin.
                    $pluginkey->movedownlink = new agpu_url($pluginlink, ['action' => 'down']);
                }
...
class admin_setting_managemediaplayers extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $updowncount = 1;
        $url = new agpu_url('/admin/media.php', array('sesskey' => sesskey()));
        $printed = array();
        $spacer = $OUTPUT->pix_icon('spacer', '', 'agpu', array('class' => 'iconsmall'));
        ...
        foreach ($plugins as $name => $plugin) {
            ...
            if (!$plugininfo->is_installed_and_upgraded()) {
                ...
                if ($enabled) {
                    $hideshow = html_writer::link(new agpu_url($url, array('action' => 'disable')),
                        $OUTPUT->pix_icon('t/hide', $strdisable, 'agpu', array('class' => 'iconsmall')));
                } else {
                    $hideshow = html_writer::link(new agpu_url($url, array('action' => 'enable')),
                        $OUTPUT->pix_icon('t/show', $strenable, 'agpu', array('class' => 'iconsmall')));
                    $class = 'dimmed_text';
                }
            ...
            if ($enabled) {
                if ($updowncount > 1) {
                    $updown = html_writer::link(new agpu_url($url, array('action' => 'up')),
                        $OUTPUT->pix_icon('t/up', $strup, 'agpu', array('class' => 'iconsmall')));
                } else {
                    $updown = $spacer;
                }
                if ($updowncount < count($enabledplugins)) {
                    $updown .= html_writer::link(new agpu_url($url, array('action' => 'down')),
                        $OUTPUT->pix_icon('t/down', $strdown, 'agpu', array('class' => 'iconsmall')));
                } else {
                    $updown .= $spacer;
                }
                ++$updowncount;
...
class admin_setting_managecontentbankcontenttypes extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        foreach ($types as $type) {
            $url = new agpu_url('/admin/contentbank.php',
...
function admin_externalpage_setup($section, $extrabutton = '', ?array $extraurlparams = null, $actualurl = '', array $options = array()) {
    ...
    if (empty($extpage) or !($extpage instanceof admin_externalpage)) {
        ...
        if (!$hassiteconfig) {
            ...
            throw new \agpu_exception('sectionerror', 'admin', "$CFG->wwwroot/$CFG->admin/");
        }
    }
    ...
    if ($PAGE->user_allowed_editing() && !$PAGE->theme->haseditswitch) {
        if ($PAGE->user_is_editing()) {
            ...
            $url = new agpu_url($PAGE->url, array('adminedit'=>'0', 'sesskey'=>sesskey()));
        } else {
            $caption = get_string('blocksediton');
            $url = new agpu_url($PAGE->url, array('adminedit'=>'1', 'sesskey'=>sesskey()));
        }
        $PAGE->set_button($OUTPUT->single_button($url, $caption, 'get'));
    }

    $PAGE->set_title(implode(agpu_page::TITLE_SEPARATOR, $extpage->visiblepath));
    ...
    if ($hassiteconfig && empty($options['nosearch'])) {
        $PAGE->add_header_action($OUTPUT->render_from_template('core_admin/header_search_input', [
            'action' => new agpu_url('/admin/search.php'),
 ...
 *
 * NOTE: previous "internal" parameters $admindefaultsettings, $settingsoutput were removed in agpu 4.3.
 *
        ...
        foreach ((array)$node->settings as $setting) {
            ...
            if ($error === '') {
                ...
                if (is_int($defaultsetting) || $defaultsetting instanceof lang_string
                    || $defaultsetting instanceof agpu_url) {
...
function admin_search_settings_html($query) {
    ...
    foreach ($findings as $found) {
        ...
        if ($page instanceof admin_externalpage) {
            $headingurl = new agpu_url($page->url);
        } else if ($page instanceof admin_settingpage) {
            $headingurl = new agpu_url('/admin/settings.php', ['section' => $page->name]);
        } else {
            continue;
        }
...
/** @var string */
    ...
    /**
     * Helper function that generates a agpu_url object
     * relevant to the repository
     */

    function repository_action_url($repository) {
        return new agpu_url($this->baseurl, array('sesskey'=>sesskey(), 'repos'=>$repository));
    }
        ...
        if (!empty($repositorytypes)) {
            ...
            foreach ($repositorytypes as $i) {
                ...
                if (!empty($typeoptionnames) || !empty($instanceoptionnames)) {
                    // Calculate number of instances in order to display them for the agpu administrator
...
class admin_setting_enablemobileservice extends admin_setting_configcheckbox {
    ...
    public function get_setting() {
        ...
        $mobileservice = $webservicemanager->get_external_service_by_shortname(agpu_OFFICIAL_MOBILE_SERVICE);
    ...
    public function write_setting($data) {
        ...
        $servicename = agpu_OFFICIAL_MOBILE_SERVICE;
...
class admin_setting_webservicesoverview extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        /// One system controlling agpu with Token
        ...
        $row = array();
        $url = new agpu_url("/admin/search.php?query=enablewebservices");
        ...
        /// 2. Enable protocols
        $row = array();
        $url = new agpu_url("/admin/settings.php?section=webserviceprotocols");
        ...
        $row = array();
        $url = new agpu_url("/user/editadvanced.php?id=-1");
        ...
        $row = array();
        $url = new agpu_url("/admin/roles/check.php?contextid=1");
        ...
        $row = array();
        $url = new agpu_url("/admin/settings.php?section=externalservices");
        ...
        $row = array();
        $url = new agpu_url('/admin/webservice/tokens.php', ['action' => 'create']);
        ...
        $row = array();
        $url = new agpu_url("/admin/search.php?query=enablewsdocumentation");
        ...
        /// 10. Test the service
        $row = array();
        $url = new agpu_url("/admin/webservice/testclient.php");
...
class admin_setting_configcolourpicker extends admin_setting {
    ...
    public function output_html($data, $query = '') {
        ...
        $icon = new pix_icon('i/loading', get_string('loading', 'admin'), 'agpu', ['class' => 'loadingicon']);
        $context = (object) [
            'id' => $this->get_id(),
            'name' => $this->get_full_name(),
            'value' => $data,
...
class admin_setting_configstoredfile extends admin_setting {
    ...
    public function output_html($data, $query = '') {
        ...
        $fm = new agpuQuickForm_filemanager($elname, $this->visiblename, ['id' => $id], $fmoptions);
 ...
 *
 * @deprecated agpu 4.3 MDL-78468 - No longer used since the devicedetectregex was removed.
 * @todo Final deprecation on agpu 4.7 MDL-79052
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
...
class admin_setting_searchsetupinfo extends admin_setting {
    ...
    public function output_html($data, $query='') {
        ...
        $row = array();
        $url = new agpu_url('/admin/settings.php?section=manageglobalsearch#admin-searchengine');
        ...
        // Available areas.
        $row = array();
        $url = new agpu_url('/admin/searchareas.php');
        ...
        if (empty($CFG->searchengine)) {
            ...
            if ($ADMIN->locate('search' . $CFG->searchengine)) {
                $url = new agpu_url('/admin/settings.php?section=search' . $CFG->searchengine);
        ...
        $row = array();
        $url = new agpu_url("/admin/search.php?query=enableglobalsearch");
        ...
        $row = array();
        $url = new agpu_url("/admin/search.php?query=searchincludeallcourses");
 ...
 *
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @copyright 2016 Dan Poltawski <dan@agpu.com>
 */
 ...
 * @copyright 2017 David Mudrák <david@agpu.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
 ...
 *
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @copyright 2019 Bas Brands <bas@agpu.com>
 */
 ...
 *
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @copyright 2020 Sara Arjona <sara@agpu.com>
 */
