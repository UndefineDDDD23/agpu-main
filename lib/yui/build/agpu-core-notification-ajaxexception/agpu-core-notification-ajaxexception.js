YUI.add('agpu-core-notification-ajaxexception', function (Y, NAME) {
 ...
 * thrown in the agpu codebase was reported during an AJAX request.
 *
 * @module agpu-core-notification
 * @submodule agpu-core-notification-ajaxexception
 */

var AJAXEXCEPTION_NAME = 'agpu AJAX exception',
    AJAXEXCEPTION;
...
AJAXEXCEPTION = function(config) {
    config.name = config.name || M.util.get_string('error', 'agpu');
...
Y.extend(AJAXEXCEPTION, M.core.notification.info, {
    ...
    initializer: function(config) {
        ...
        this.get(BASE).addClass('agpu-dialogue-exception');
        this.setStdModContent(Y.WidgetStdMod.HEADER,
                '<h5 id="agpu-dialogue-' + this.get('COUNT') + '-wrap-header-text">'
                    + Y.Escape.html(config.name) + '</h5>',
                Y.WidgetStdMod.REPLACE);
        content = Y.Node.create('<div class="agpu-ajaxexception" data-rel="fatalerror"></div>')
                .append(Y.Node.create('<div class="agpu-exception-message">' + Y.Escape.html(this.get('error')) + '</div>'))
                .append(Y.Node.create('<div class="agpu-exception-param hidden param-debuginfo"><label>' +
                        M.util.get_string('url', 'agpu') + labelsep + '</label> ' +
                        this.get('reproductionlink') + '</div>'))
                .append(Y.Node.create('<div class="agpu-exception-param hidden param-debuginfo"><label>' +
                        M.util.get_string('debuginfo', 'debug') + labelsep + '</label> ' +
                        Y.Escape.html(this.get('debuginfo')) + '</div>'))
                .append(Y.Node.create('<div class="agpu-exception-param hidden param-stacktrace"><label>' +
                        M.util.get_string('stacktrace', 'debug') + labelsep + '</label> <pre>' +
                        Y.Escape.html(this.get('stacktrace')) + '</pre></div>'));
        if (M.cfg.developerdebug) {
            content.all('.agpu-exception-param').removeClass('hidden');
        }
    ...
    ATTRS: {
        ...
        error: {
            ...
            value: M.util.get_string('unknownerror', 'agpu')
        },
...
}, '@VERSION@', {"requires": ["agpu-core-notification-dialogue"]});
