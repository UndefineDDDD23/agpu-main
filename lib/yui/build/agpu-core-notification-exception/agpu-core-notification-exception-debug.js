YUI.add('agpu-core-notification-exception', function (Y, NAME) {
 ...
 * @submodule agpu-core-notification-exception
 */

var EXCEPTION_NAME = 'agpu exception',
    EXCEPTION;
...
Y.extend(EXCEPTION, M.core.notification.info, {
    ...
    initializer: function(config) {
        ...
        this.setStdModContent(Y.WidgetStdMod.HEADER,
                '<h5 id="agpu-dialogue-' + this.get('COUNT') + '-wrap-header-text">' + Y.Escape.html(config.name) + '</h5>',
                Y.WidgetStdMod.REPLACE);
        content = Y.Node.create('<div class="agpu-exception" data-rel="fatalerror"></div>')
                .append(Y.Node.create('<div class="agpu-exception-message">' + Y.Escape.html(this.get('message')) + '</div>'))
                .append(Y.Node.create('<div class="agpu-exception-param hidden param-filename"><label>' +
                        M.util.get_string('file', 'agpu') + labelsep + '</label> ' +
                        Y.Escape.html(this.get('fileName')) + '</div>'))
                .append(Y.Node.create('<div class="agpu-exception-param hidden param-linenumber"><label>' +
