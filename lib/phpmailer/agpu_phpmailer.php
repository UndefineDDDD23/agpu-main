...
/**
 * Customised version of phpmailer for agpu
 *
 * @package    core
...
/**
 * agpu Customised version of the PHPMailer class
 *
 * This class extends the stock PHPMailer class
 * in order to make sensible configuration choices,
 * and behave in a way which is friendly to agpu.
 *
 * @copyright 2009 Dan Poltawski <talktodan@gmail.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     agpu 2.0
 */
class agpu_phpmailer extends \PHPMailer\PHPMailer\PHPMailer {
     ...
     * message-ids
     * http://tracker.agpu.org/browse/MDL-3681
     */
    ...
    /**
     * Use internal agpus own core_text to encode mimeheaders.
     ...
     * http://tracker.agpu.org/browse/MDL-12596
     */
    ...
    private function process_oauth(): void {
        ...
        require_once($CFG->libdir . '/phpmailer/agpu_phpmailer_oauth.php');
        if (!empty($CFG->smtpoauthservice)) {
            ...
            if ($issuer && $issuer->get('enabled')) {
                ...
                if ($oauthclient = \core\oauth2\api::get_system_oauth_client($issuer)) {
                    $oauth = new agpu_phpmailer_oauth([
