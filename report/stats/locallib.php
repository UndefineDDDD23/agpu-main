...
function report_stats_mode_menu($course, $mode, $time, $url) {
    ...
    if (empty($timeoptions)) {
        throw new \agpu_exception('nostatstodisplay', '', $CFG->wwwroot.'/course/view.php?id='.$course->id);
    }
    */
    ...
    $select = new single_select(new agpu_url($popupurl), 'mode', $options, $mode, null);
...
function report_stats_report($course, $report, $mode, $user, $roleid, $time) {
    ...
    if (!empty($report) && !empty($time) && ($mode !== STATS_MODE_DETAILED || !empty($userid))) {
        if ($report == STATS_REPORT_LOGINS && $course->id != SITEID) {
            throw new \agpu_exception('reportnotavailable');
        }
        ...
        if (empty($stats)) {
            ...
            $table->head = array(get_string('periodending','agpu',$period));
