...
$PAGE->set_url(new agpu_url('/report/stats/index.php', array('course' => $course->id,
                                                               'report' => $report,
                                                               'time'   => $time,
                                                               'mode'   => $mode,
                                                               'userid' => $userid)));
navigation_node::override_active_url(new agpu_url('/report/stats/index.php', array('course' => $course->id)));
...
if (empty($CFG->enablestats)) {
    if (has_capability('agpu/site:config', context_system::instance())) {
        ...
        throw new \agpu_exception('statsdisable');
    }
}
