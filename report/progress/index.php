...
$url = new agpu_url('/report/progress/index.php', array('course'=>$id));
...
foreach($progress as $user) {
    // User name
    if ($csv) {
        print csv_quote(fullname($user, has_capability('agpu/site:viewfullnames', $context)));
        foreach ($extrafields as $field) {
    ...
    foreach($activities as $activity) {
        ...
        $a->state=$describe;
        $a->date=$date;
        $a->user = fullname($user, has_capability('agpu/site:viewfullnames', $context));
        ...
        if ($csv) {
            ...
            if (has_capability('agpu/course:overridecompletion', $context) &&
                ...
                $url = new agpu_url($PAGE->url, ['sesskey' => sesskey()]);
