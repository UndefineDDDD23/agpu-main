...
class infectedfiles_table extends \table_sql implements \renderable {
     ...
     * @param \agpu_url $url page url
     * @param int $page current page
     * @param int $perpage number or record per page
     * @throws \coding_exception
     */
    public function __construct($uniqueid, \agpu_url $url, $page = 0, $perpage = 30) {
     ...
     * @param \agpu_url $url
     */
    protected function define_table_configs(\agpu_url $url) {
        // Set table url.
    ...
    protected function col_author($row): string {
        ...
        $url = new \agpu_url('/user/profile.php', ['id' => $row->userid]);
    ...
    protected function col_actions($row): string {
        ...
        $links = '';
        $managefilepage = new \agpu_url('/report/infectedfiles/index.php');

        // Download.
        $downloadparams = ['file' => $fileid, 'action' => 'download', 'sesskey' => sesskey()];
        $downloadurl = new \agpu_url($managefilepage, $downloadparams);
        ...
        $deleteurl = new \agpu_url($managefilepage, $deleteparams);
    ...
    public function display($pagesize, $useinitialsbar, $downloadhelpbutton='') {
        ...
        $deleteallurl = new \agpu_url($managefilepage, $deleteallparams);
        ...
        $downloadallurl = new \agpu_url($managefilepage, $downloadallparams);
