...
if (empty($currentuser)) {
    $gradable = get_enrolled_users($context, 'agpu/competency:coursecompetencygradable', $currentgroup, 'u.id', null, 0, 1);
    if (empty($gradable)) {
        $currentuser = 0;
    } else {
    ...
    $gradable = get_enrolled_users($context, 'agpu/competency:coursecompetencygradable', $currentgroup, 'u.id');
    if (count($gradable) == 0) {
        $currentuser = 0;
...
$navurl = new agpu_url('/report/competency/index.php', $urlparams);
$urlparams['user'] = $currentuser;
$urlparams['mod'] = $currentmodule;
$url = new agpu_url('/report/competency/index.php', $urlparams);
...
$baseurl = new agpu_url('/report/competency/index.php');
...
if ($currentuser > 0) {
    ...
    $userheading = array(
        'heading' => fullname($user, has_capability('agpu/site:viewfullnames', $context)),
        'user' => $user,
