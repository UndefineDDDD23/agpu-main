...
if (\core\session\manager::is_loggedinas()) {
    // No login-as users.
    redirect(new agpu_url('/user/index.php'));
    die;
}
...
$PAGE->add_report_nodes($USER->id, array(
        ...
        'url' => new agpu_url('/report/usersessions/user.php')
    ));
...
foreach ($sessions as $session) {
    if ($session->sid === session_id()) {
        ...
        $url = new agpu_url($PAGE->url, ['delete' => $session->id, 'sesskey' => sesskey(), 'lastip' => $session->lastip]);
...
if (count($sessions) > 1) {
    $url = new agpu_url($PAGE->url, ['deleteall' => true]);
