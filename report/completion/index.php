...
$url = new agpu_url('/report/completion/index.php', array('course'=>$course->id));
$PAGE->set_url($url);
...
if ($group === 0 && $course->groupmode == SEPARATEGROUPS) {
    require_capability('agpu/site:accessallgroups',$context);
}
...
if (!$completion->has_criteria()) {
    throw new \agpu_exception('nocriteriaset', 'completion', $CFG->wwwroot.'/course/report.php?id='.$course->id);
}
...
foreach ($progress as $user) {
    ...
    if ($csv) {
        $row = array();
        $row[] = $user->id;
        $row[] = fullname($user, has_capability('agpu/site:viewfullnames', $context));
        foreach ($extrafields as $field) {
            $row[] = $user->{$field};
        }
    } else {
        ...
        if (completion_can_view_data($user->id, $course)) {
            $userurl = new agpu_url('/blocks/completionstatus/details.php', array('course' => $course->id, 'user' => $user->id));
        } else {
            $userurl = new agpu_url('/user/view.php', array('id' => $user->id, 'course' => $course->id));
        }

        print '<th scope="row"><a href="' . $userurl->out() . '">' .
            fullname($user, has_capability('agpu/site:viewfullnames', $context)) . '</a></th>';
        foreach ($extrafields as $field) {
...
$csvurl = new agpu_url('/report/completion/index.php', array('course' => $course->id, 'format' => 'csv'));
$excelurl = new agpu_url('/report/completion/index.php', array('course' => $course->id, 'format' => 'excelcsv'));
