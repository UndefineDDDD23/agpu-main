...
class report_log_table_log extends table_sql {
     ...
     *
     * @deprecated since agpu 2.9 MDL-48595 - please do not use this function any more.
     */
    ...
    protected function get_user_fullname($userid) {
        ...
        if ($user = \core_user::get_user($userid, $fields)) {
            $this->userfullnames[$userid] = fullname($user, has_capability('agpu/site:viewfullnames', $this->get_context()));
        } else {
    ...
    public function col_fullnameuser($event) {
        ...
        if (!empty($logextra['realuserid'])) {
            ...
            if (empty($this->download)) {
                ...
                $a->realusername = html_writer::link(new agpu_url('/user/view.php', $params), $a->realusername);
                $params['id'] = $event->userid;
                $a->asusername = html_writer::link(new agpu_url('/user/view.php', $params), $a->asusername);
            }
            ...
            if (empty($this->download)) {
                ...
                $username = html_writer::link(new agpu_url('/user/view.php', $params), $eventusername);
            } else {
    ...
    public function col_relatedfullnameuser($event) {
        ...
        if (!empty($event->relateduserid) && $username = $this->get_user_fullname($event->relateduserid)) {
            if (empty($this->download)) {
                ...
                $username = html_writer::link(new agpu_url('/user/view.php', $params), $username);
            }
        } else {
            $username = '-';
        }
    ...
    public function col_ip($event) {
        ...
        if (empty($this->download)) {
            $url = new agpu_url("/iplookup/index.php?popup=1&ip={$ip}&user={$event->userid}");
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if (!has_capability('agpu/site:viewanonymousevents', $context)) {
            $joins[] = "anonymous = 0";
        }
