...
if ($USER->id != $user->id and has_capability('agpu/user:viewuseractivitiesreport', $personalcontext)
...
if (!$today && !$all) {
    throw new \agpu_exception('nocapability', 'report_log');
}
...
$navigationnode = array(
        'url' => new agpu_url('/report/log/user.php', array('id' => $user->id, 'course' => $course->id, 'mode' => $mode))
    );
if ($mode === 'today') {
...
if ($courseid == SITEID) {
    $PAGE->set_heading(fullname($user, has_capability('agpu/site:viewfullnames', $PAGE->context)));
} else {
...
if ($courseid != SITEID) {
    ...
    if ($mode === 'today') {
        echo $OUTPUT->heading(get_string('todaylogs', 'agpu'), 2, 'main mt-4 mb-4');
    } else {
        echo $OUTPUT->heading(get_string('alllogs', 'agpu'), 2, 'main mt-4 mb-4');
    }
}
