...
function gradereport_singleview_get_report_link(context_course $context, int $courseid,
    ...
    if ($mode == 'gradeitem') {
        ...
        if (get_capability_info('gradereport/singleview:view')) {
            if (has_all_capabilities(['gradereport/singleview:view', 'agpu/grade:viewall',
                'agpu/grade:edit'], $context)) {

                $url = new agpu_url('/grade/report/singleview/index.php', [
                    'id' => $courseid,
                    'item' => 'grade',
        ...
        if (get_capability_info('gradereport/singleview:view')) {
            $canseesingleview = has_all_capabilities(['gradereport/singleview:view',
                'agpu/grade:viewall', 'agpu/grade:edit'], $context);
        }
