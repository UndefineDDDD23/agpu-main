...
class grade_report_grader extends grade_report {
     ...
     * @deprecated Since agpu 4.5 MDL-84245. Use grade_report_grader::get_max_students_per_page() instead.
     */
    ...
    public function __construct($courseid, $gpr, $context, $page=null, $sortitemid=null, string $sort = '') {
        ...
        $this->canviewhidden = has_capability('agpu/grade:viewhidden', context_course::instance($this->course->id));
        ...
        $this->baseurl = new agpu_url('index.php', array('id' => $this->courseid));
        ...
        $this->pbarurl = new agpu_url('/grade/report/grader/index.php', array('id' => $this->courseid));

        $this->setup_groups();
        $this->setup_users();
        $this->setup_sortitemid($sort);

        $this->overridecat = (bool)get_config('agpu', 'grade_overridecat');
    }
    ...
    public function process_data($data) {
        ...
        $mygroups       = array();
        if ($this->groupmode == SEPARATEGROUPS and !has_capability('agpu/site:accessallgroups', $this->context)) {
            $separategroups = true;
            ...
            foreach ($students as $userid => $items) {
                ...
                foreach ($items as $itemid => $postedvalue) {
                    ...
                    if (!$gradeitem = grade_item::fetch(array('id'=>$itemid, 'courseid'=>$this->courseid))) {
                        throw new \agpu_exception('invalidgradeitemid');
                    }
    ...
    /**
     * Gets html toggle
     * @deprecated since agpu 2.4 as it appears not to be used any more.
     */
    ...
    public function get_right_rows(bool $displayaverages): array {
        ...
        foreach ($this->users as $userid => $user) {
            ...
            foreach ($this->gtree->items as $itemid => $unused) {
                ...
                // doesn't have 'agpu/grade:viewhidden'.
    ...
    /**
     * @deprecated since agpu 4.4 - Call calculate_average instead.
     ...
     *
     * @deprecated since agpu 2.9 MDL-46662 - please do not use this function any more.
     */
    ...
    public function get_sort_arrows(array $extrafields = []) {
        ...
        if (has_capability('agpu/site:viewfullnames', $this->context)) {
     ...
     *
     * @param agpu_url $url Url to grader report page
     ...
     * @return string|null
     */
    public function get_category_view_mode_link(agpu_url $url, string $title, string $action, bool $active = false): ?string {
        $urlnew = $url;
    ...
    public function get_default_sortable(): string {
        $sortlink = new agpu_url('/grade/report/grader/index.php', [
            'id' => $this->courseid,
...
function gradereport_grader_get_report_link(context_course $context, int $courseid,
    ...
    if ($mode == 'category') {
        ...
        $url = new agpu_url($gpr->get_return_url(null, ['target' => $element['eid'], 'sesskey' => sesskey()]));
