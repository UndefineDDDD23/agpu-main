...
$PAGE->set_url(new agpu_url('/grade/report/user/index.php', ['id' => $courseid]));
...
if ($userid === 0) {
    require_capability('agpu/grade:viewall', $context);
} else if ($userid) {
...
if (has_capability('agpu/grade:viewall', $context)) {
    ...
    $access = true;
} else if (($userid == $USER->id || is_null($userid)) && has_capability('agpu/grade:view', $context) && $course->showgrades) {
    // User can view own grades.
    $access = true;
} else if (has_capability('agpu/grade:viewall', context_user::instance($userid)) && $course->showgrades) {
...
if (!$access) {
    ...
    throw new \agpu_exception('nopermissiontoviewgrades', 'error',  $CFG->wwwroot.'/course/view.php?id='.$courseid);
}
...
if (has_capability('agpu/grade:viewall', $context)) {
    ...
    if ($groupmode) {
        $baseurl = new agpu_url('/grade/report/user/index.php', ['id' => $courseid]);
    ...
    $isseparategroups = ($course->groupmode == SEPARATEGROUPS && !has_capability('agpu/site:accessallgroups', $context));
