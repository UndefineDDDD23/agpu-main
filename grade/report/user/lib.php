...
function gradereport_user_myprofile_navigation(tree $tree, stdClass $user, bool $iscurrentuser, ?stdClass $course) {
    ...
    $anyreport = has_capability('agpu/user:viewuseractivitiesreport', $usercontext);
    ...
    if ($anyreport || $iscurrentuser) {
        ...
        if (has_capability('agpu/grade:viewall', $coursecontext)) {
            ...
            if ($iscurrentuser && has_capability('agpu/grade:view', $coursecontext)) {
                // Can view own grades.
                $gradeaccess = true;
            } else if (has_capability('agpu/grade:viewall', $usercontext)) {
...
function gradereport_user_get_report_link(context_course $context, int $courseid, array $element,
    ...
    if ($mode == 'user') {
        ...
        if ($canseeuserreport) {
            $url = new agpu_url('/grade/report/' . $CFG->grade_profilereport . '/index.php',
