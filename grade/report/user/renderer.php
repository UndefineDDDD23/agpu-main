...
class gradereport_user_renderer extends plugin_renderer_base {
     ...
     * @return string The raw HTML to render.
     * @throws coding_exception
     * @deprecated since agpu 4.5. See user_selector use in \gradereport_user\output\action_bar::export_for_template.
     */
    public function users_selector(object $course, ?int $userid = null, ?int $groupid = null, string $usersearch = ''): string {
        ...
        $resetlink = new agpu_url('/grade/report/user/index.php', ['id' => $course->id, 'group' => 0]);
        $baseurl = new agpu_url('/grade/report/user/index.php', ['id' => $course->id]);
    ...
    public function user_navigation(graded_users_iterator $gui, int $userid, int $courseid): string {
        ...
        if ($keynumber !== 0) {
            ...
            $navigationdata['previoususer'] = [
                ...
                'url' => (new agpu_url('/grade/report/user/index.php', ['id' => $courseid, 'userid' => $previoususer->id]))
                    ->out(false),
        ...
        if ($keynumber < count($users) - 1) {
            ...
            $navigationdata['nextuser'] = [
                ...
                'url' => (new agpu_url('/grade/report/user/index.php', ['id' => $courseid, 'userid' => $nextuser->id]))
                    ->out(false),
                'nextarrow' => $nextarrow
            ];
        }
     ...
     * @param int $courseid The course ID.
     * @return string The raw HTML to render.
     * @deprecated since agpu 4.5 See select_menu use in \gradereport_user\output\action_bar::export_for_template.
     */
    public function view_mode_selector(int $userid, int $userview, int $courseid): string {
        ...
        $viewasotheruser = new agpu_url('/grade/report/user/index.php', ['id' => $courseid, 'userid' => $userid,
            'userview' => GRADE_REPORT_USER_VIEW_USER]);
        $viewasmyself = new agpu_url('/grade/report/user/index.php', ['id' => $courseid, 'userid' => $userid,
