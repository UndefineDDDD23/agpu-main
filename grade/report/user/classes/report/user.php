...
class user extends grade_report {
    ...
    public function __construct(int $courseid, ?object $gpr, object $context, int $userid, ?bool $viewasuser = null) {
        ...
        if ($viewasuser) {
            ...
            $this->canviewhidden = has_capability('agpu/grade:viewhidden', $coursecontext, $this->user->id);
        } else {
            $this->modinfo = $this->gtree->modinfo;
            $this->canviewhidden = has_capability('agpu/grade:viewhidden', $coursecontext);
        }
        ...
        $this->baseurl = new \agpu_url('/grade/report', ['id' => $courseid, 'userid' => $userid]);
    ...
    private function fill_table_recursive(array &$element) {
        ...
        if ($type == 'item' || $type == 'categoryitem' || $type == 'courseitem') {
            ...
            if (!$hide) {
                $canviewall = has_capability('agpu/grade:viewall', $this->context);
