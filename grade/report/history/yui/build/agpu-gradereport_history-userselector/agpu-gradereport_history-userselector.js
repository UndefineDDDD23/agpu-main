YUI.add('agpu-gradereport_history-userselector', function (Y, NAME) {
 ...
 *
 * @module     agpu-gradereport_history-userselector
 * @package    gradereport_history
 * @copyright  2013 NetSpot Pty Ltd (https://www.netspot.com.au)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @main       agpu-gradereport_history-userselector
 */
...
Y.namespace('M.gradereport_history').UserSelector = Y.extend(USERSELECTOR, M.core.dialogue, {
    ...
    initializer: function() {
                    ...
                    '<div class="{{CSS.SEARCH}}" role="search">' +
                        '<form>' +
                            '<input type="text" class="{{CSS.SEARCHFIELD}}" ' +
                                'aria-label="{{get_string "search" "agpu"}}" value="" />' +
                            '<input type="submit" class="{{CSS.SEARCHBTN}}"' +
                                'value="{{get_string "search" "agpu"}}">' +
                        '</form>' +
        ...
        content = Y.Node.create(
            tpl({
                COMPONENT: COMPONENT,
                CSS: CSS,
                loadingIcon: M.util.image_url('i/loading', 'agpu')
            })
        );
        ...
        this.get('boundingBox').one('.agpu-dialogue-wrap').addClass('agpu-dialogue-content');
    ...
    processSearchResults: function(tid, outcome, args) {
        ...
        if (!args.append) {
            if (totalUsers === 0) {
                bb.one(SELECTORS.RESULTSCOUNT).setHTML(M.util.get_string('noresults', 'agpu'));
                content = '';
            } else {
                if (totalUsers === 1) {
