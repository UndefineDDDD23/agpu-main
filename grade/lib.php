 ...
 *
 * @author Petr Skoda <skodak@agpu.org>
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
        ...
        if (!empty($this->grade_items)) {
            foreach ($this->grade_items as $grade_item) {
                ...
                if (array_key_exists($grade_item->id, $grade_records)) {
                    ...
                    $feedbacks[$grade_item->id]->feedbackformat = FORMAT_agpu;
...
function grade_get_graded_users_select($report, $course, $userid, $groupid, $includeall) {
    ...
    $showonlyactiveenrol = $showonlyactiveenrol || !has_capability('agpu/course:viewsuspendedusers', $coursecontext);
    ...
    $select = new single_select(
        new agpu_url('/grade/report/'.$report.'/index.php', $gpr->get_options()),
        'userid', $menu, $userid
    );
    $select->label = $label;
...
function print_natural_aggregation_upgrade_notice($courseid, $context, $thispage, $return=false) {
    ...
    if (!$hidenaturalwarning && $shownaturalwarning) {
        ...
        $urlparams = array( 'id' => $courseid,
                            ...
                            'sesskey' => sesskey());
        $goawayurl = new agpu_url($thispage, $urlparams);
    ...
    if ($showminmaxwarning) {
        ...
        if ($minmaxtouse == GRADE_MIN_MAX_FROM_GRADE_ITEM) {
            ...
            $urlparams = array('id' => $courseid,
                               ...
                               'sesskey' => sesskey());
            $reverturl = new agpu_url($thispage, $urlparams);
            ...
            $urlparams = array('id' => $courseid,
                               ...
                               'sesskey' => sesskey());
            $fixurl = new agpu_url($thispage, $urlparams);
...
function print_grade_page_head(int $courseid, string $active_type, ?string $active_plugin = null, string|bool $heading = false,
    ...
    if ($active_type === 'preferences') {
        // In agpu 2.8 report preferences were moved under 'settings'. Allow backward compatibility for 3rd party grade reports.
    ...
    $PAGE->set_title(implode(agpu_page::TITLE_SEPARATOR, $titlecomponents));
    ...
    if (isset($headerhelpidentifier) && !empty($heading)) {
        ...
        $showuserbuttons = $user->id != $USER->id && !empty($CFG->messaging) &&
            has_capability('agpu/site:sendmessage', $PAGE->context);
...
class grade_plugin_return {
     ...
     *
     * @param object &$mform agpu form object
     *
     * @return void
     */
     ...
     *
     * @param agpu_url $url A URL
     * @return agpu_url with return tracking params
     */
    public function add_url_params(agpu_url $url): agpu_url {
        if (empty($this->type)) {
            return $url;
        }
...
function grade_build_nav($path, $pagename=null, $id=null) {
    ...
    $PAGE->navbar->add($strgrades, new agpu_url('/grade/index.php', array('id'=>$COURSE->id)));
    ...
    switch ($path_elements[1]) {
        ...
        case 'report':
            ...
            if (!is_null($id)) {
                $link = new agpu_url('/grade/report/index.php', $linkparams);
            }
    ...
    switch ($numberofelements) {
        ...
        case 4:
            if ($path_elements[2] == 'grader' AND $path_elements[3] != 'index.php') {
                $PAGE->navbar->add(get_string('pluginname', 'gradereport_grader'), new agpu_url('/grade/report/grader/index.php', $linkparams));
            }
...
class grade_structure {
    ...
    */
     ...
     * @deprecated since agpu 4.4 - please use {@see grade_helper::get_element_icon()}
     * @todo MDL-79907 This will be deleted in agpu 4.8.
     */
     ...
     * @deprecated since agpu 4.4 - please use {@see grade_helper::get_element_type_string()}
     * @todo MDL-79907 This will be deleted in agpu 4.8.
     */
     ...
     * @param agpu_url|null $sortlink Link to sort column.
     *
     * @return string header
     * @deprecated since agpu 4.4 - please use {@see grade_helper::get_element_header()}
     * @todo MDL-79907 This will be deleted in agpu 4.8.
     */
    ...
    /**
     * @deprecated since agpu 4.4 - please use {@see grade_helper::get_activity_link()}
     * @todo MDL-79907 This will be deleted in agpu 4.8.
     */
     ...
     *
     * @return agpu_url|null URL or null if unable to construct it
     */
    public function get_grade_analysis_url(grade_grade $grade) {
        ...
        $url = new agpu_url('/mod/'.$item->itemmodule.'/grade.php', array(
            'id'         => $cm->id,
            'itemid'     => $item->id,
     ...
     * @return string
     * @deprecated since agpu 4.2 - The row is not shown anymore - we have actions menu.
     * @todo MDL-77307 This will be deleted in agpu 4.6.
     */
    ...
    public function get_reset_weights_link(array $element, object $gpr): ?string {
        ...
        $str = get_string('resetweights', 'grades', $this->get_params_for_iconstr($element));
        $url = new agpu_url('/grade/edit/tree/action.php', [
            'id' => $this->courseid,
            'action' => 'resetweights',
            'eid' => $element['eid'],
            'sesskey' => sesskey(),
    ...
    public function get_delete_link(array $element, object $gpr): ?string {
        if ($element['type'] == 'item' || ($element['type'] == 'category' && $element['depth'] > 1)) {
            if (grade_edit_tree::element_deletable($element)) {
                $deleteconfirmationurl = new agpu_url('index.php', [
                    'id' => $this->courseid,
                    'action' => 'delete',
                    'confirm' => 1,
                    'eid' => $element['eid'],
    ...
    public function get_duplicate_link(array $element, object $gpr): ?string {
        if ($element['type'] == 'item' || ($element['type'] == 'category' && $element['depth'] > 1)) {
            if (grade_edit_tree::element_duplicatable($element)) {
                ...
                $url = new agpu_url('index.php', $duplicateparams);
    ...
    public function get_edit_icon($element, $gpr, $returnactionmenulink = false) {
        ...
        if (!has_capability('agpu/grade:manage', $this->context)) {
            if ($element['type'] == 'grade' and has_capability('agpu/grade:edit', $this->context)) {
        ...
        switch ($element['type']) {
            ...
            case 'courseitem':
                ...
                if (empty($object->outcomeid) || empty($CFG->enableoutcomes)) {
                    $url = new agpu_url('/grade/edit/tree/item.php',
                            array('courseid' => $this->courseid, 'id' => $object->id));
                } else {
                    $url = new agpu_url('/grade/edit/tree/outcomeitem.php',
                            array('courseid' => $this->courseid, 'id' => $object->id));
                }
                break;
            ...
            case 'category':
                $stredit = get_string('editverbose', 'grades', $strparams);
                $url = new agpu_url('/grade/edit/tree/category.php',
                        array('courseid' => $this->courseid, 'id' => $object->id));
                break;
            ...
            case 'grade':
                ...
                if (empty($object->id)) {
                    $url = new agpu_url('/grade/edit/tree/grade.php',
    ...
    public function get_edit_link(array $element, object $gpr): ?string {
        ...
        $title = '';
        if ((!has_capability('agpu/grade:manage', $this->context) &&
            (!($element['type'] == 'grade') || !has_capability('agpu/grade:edit', $this->context)))) {
                return null;
        }
            ...
            if (!empty($instances[$itemmodule][$iteminstance])) {
                ...
                if ($cm->uservisible) {
                    ...
                    if ($hasgradephp[$itemmodule]) {
                        ...
                        $url = new agpu_url('/mod/' . $itemmodule . '/grade.php', $args);
    ...
    public function get_hiding_icon($element, $gpr, $returnactionmenulink = false) {
        ...
        if (!has_capability('agpu/grade:manage', $this->context) and
            !has_capability('agpu/grade:hide', $this->context)) {
            return $returnactionmenulink ? null : '';
        }
        ...
        $url = new agpu_url('/grade/edit/tree/action.php', array('id' => $this->courseid, 'sesskey' => sesskey(), 'eid' => $element['eid']));
        $url = $gpr->add_url_params($url);
        ...
        if ($element['object']->is_hidden()) {
            ...
            if ($returnactionmenulink) {
                ...
                $hideicon = $OUTPUT->action_icon($url, new pix_icon('t/'.$type, $tooltip, 'agpu', array('alt'=>$strshow, 'class'=>'smallicon')));
            }
    ...
    public function get_hiding_link(array $element, object $gpr): ?string {
        if (!$element['object']->can_control_visibility() || !has_capability('agpu/grade:manage', $this->context) ||
            !has_capability('agpu/grade:hide', $this->context)) {
            return null;
        }
    ...
    public function get_locking_icon($element, $gpr) {
        ...
        if ($element['type'] == 'grade' && $element['object']->grade_item->is_locked()) {
            ...
            if (!has_capability('agpu/grade:manage', $this->context) and !has_capability('agpu/grade:unlock', $this->context)) {
                $action = '';
            } else {
                $url->param('action', 'unlock');
                $action = $OUTPUT->action_icon($url, new pix_icon('t/'.$type, $tooltip, 'agpu', array('alt'=>$strunlock, 'class'=>'smallicon')));
            }

        } else {
            if (!has_capability('agpu/grade:manage', $this->context) and !has_capability('agpu/grade:lock', $this->context)) {
                $action = '';
            } else {
    ...
    public function get_locking_link(array $element, object $gpr): ?string {
        ...
        if (has_capability('agpu/grade:manage', $this->context) && isset($element['object'])) {
            $title = '';
            $url = new agpu_url('/grade/edit/tree/action.php',
                ['id' => $this->courseid, 'sesskey' => sesskey(), 'eid' => $element['eid']]);
            $url = $gpr->add_url_params($url);
            ...
            if ($element['type'] == 'category') {
                ...
                if ($alllocked && has_capability('agpu/grade:unlock', $this->context)) {
                    $title = get_string('unlock', 'grades');
                    $url->param('action', 'unlock');
                } else if (!$alllocked && has_capability('agpu/grade:lock', $this->context)) {
                ...
                if (has_capability('agpu/grade:unlock', $this->context)) {
                    $title = get_string('unlock', 'grades');
                    $url->param('action', 'unlock');
                } else {
                    return null;
                ...
                if (has_capability('agpu/grade:lock', $this->context)) {
                    $title = get_string('lock', 'grades');
                    $url->param('action', 'lock');
                } else {
                    return null;
    ...
    public function get_calculation_icon($element, $gpr, $returnactionmenulink = false) {
        ...
        if ($type == 'item' or $type == 'courseitem' or $type == 'categoryitem') {
            ...
            if (!$object->is_external_item() and ($is_scale or $is_value)) {
                ...
                $url = new agpu_url('/grade/edit/tree/calculation.php', array('courseid' => $this->courseid, 'id' => $object->id));
                $url = $gpr->add_url_params($url);
                if ($returnactionmenulink) {
    ...
    public function get_edit_calculation_link(array $element, object $gpr): ?string {
        ...
        if (has_capability('agpu/grade:manage', $this->context) && isset($element['object'])) {
            ...
            if (!$object->is_external_item() && ($isscale || $isvalue)) {
                ...
                $url = new agpu_url('/grade/edit/tree/calculation.php',
                    ['courseid' => $this->courseid, 'id' => $object->id]);
                $url = $gpr->add_url_params($url);
        ...
        if ($mode == 'gradeitem' || $mode == 'setup') {
            ...
            if ($element['type'] == 'grade') {
                ...
                if ($item->is_course_item() || $item->is_category_item()) {
                    $editable = (bool)get_config('agpu', 'grade_overridecat');;
                }
     ...
     *
     * @param agpu_url $sortlink A base link for sorting
     * @param object $gpr A grade_plugin_return object
     * @param string $direction Direction od sorting
     * @return string
     */
    public function get_sorting_link(agpu_url $sortlink, object $gpr, string $direction = 'asc'): string {
...
function grade_button($type, $courseid, $object) {
    ...
    if ($type == 'delete') {
        $url = new agpu_url('index.php', array('id' => $courseid, $objectidstring => $object->id, 'action' => 'delete', 'sesskey' => sesskey()));
    } else if ($type == 'edit') {
        $url = new agpu_url('edit.php', array('courseid' => $courseid, 'id' => $object->id));
    }
...
abstract class grade_helper {
     ...
     * @todo MDL-78780 This will be deleted in agpu 4.7.
    ...
    public static function get_info_manage_settings($courseid) {
        ...
        if ($courseid != SITEID && has_capability('agpu/grade:manage', $context)) {
            self::$managesetting['gradebooksetup'] = new grade_plugin_info('setup',
                new agpu_url('/grade/edit/tree/index.php', array('id' => $courseid)),
                get_string('gradebooksetup', 'grades'));
            self::$managesetting['coursesettings'] = new grade_plugin_info('coursesettings',
                new agpu_url('/grade/edit/settings/index.php', array('id'=>$courseid)),
    ...
    public static function get_plugins_reports($courseid) {
        ...
        foreach (core_component::get_plugin_list('gradereport') as $plugin => $plugindir) {
            ...
            if ($plugin === 'singleview' && !has_all_capabilities(array('agpu/grade:viewall',
                    'agpu/grade:edit'), $context)) {
                continue;
            }

            $pluginstr = get_string('pluginname', 'gradereport_'.$plugin);
            $url = new agpu_url('/grade/report/'.$plugin.'/index.php', array('id'=>$courseid));
            ...
            if (file_exists($plugindir.'/preferences.php')) {
                $url = new agpu_url('/grade/report/'.$plugin.'/preferences.php', array('id' => $courseid));
    ...
    public static function get_info_scales($courseid) {
        ...
        if (has_capability('agpu/course:managescales', context_course::instance($courseid))) {
            $url = new agpu_url('/grade/edit/scale/index.php', array('id'=>$courseid));
    ...
    public static function get_info_outcomes($courseid) {
        ...
        $context = context_course::instance($courseid);
        $canmanage = has_capability('agpu/grade:manage', $context);
        $canupdate = has_capability('agpu/course:update', $context);
        if (!empty($CFG->enableoutcomes) && ($canmanage || $canupdate)) {
            $outcomes = array();
            if ($canupdate) {
                if ($courseid!=$SITE->id) {
                    $url = new agpu_url('/grade/edit/outcome/course.php', array('id'=>$courseid));
                    $outcomes['course'] = new grade_plugin_info('course', $url, get_string('outcomescourse', 'grades'));
                }
                $url = new agpu_url('/grade/edit/outcome/index.php', array('id'=>$courseid));
                $outcomes['edit'] = new grade_plugin_info('edit', $url, get_string('editoutcomes', 'grades'));
                $url = new agpu_url('/grade/edit/outcome/import.php', array('courseid'=>$courseid));
    ...
    public static function get_info_letters($courseid) {
        ...
        $context = context_course::instance($courseid);
        $canmanage = has_capability('agpu/grade:manage', $context);
        $canmanageletters = has_capability('agpu/grade:manageletters', $context);
        if ($canmanage || $canmanageletters) {
            ...
            self::$letterinfo = array(
                'view' => new grade_plugin_info('view', new agpu_url('/grade/edit/letter/index.php', array('id'=>$context->id)), get_string('view')),
                'edit' => new grade_plugin_info('edit', new agpu_url('/grade/edit/letter/index.php', $param), get_string('edit'))
            );
        } else {
    ...
    public static function get_plugins_import($courseid) {
        ...
        if (has_capability('agpu/grade:import', $context)) {
            foreach (core_component::get_plugin_list('gradeimport') as $plugin => $plugindir) {
                ...
                $url = new agpu_url('/grade/import/'.$plugin.'/index.php', array('id'=>$courseid));
            ...
            if ($CFG->gradepublishing && has_capability('gradeimport/xml:publish', $context)) {
                $url = new agpu_url('/grade/import/keymanager.php', array('id'=>$courseid));
    ...
    public static function get_plugins_export($courseid) {
        ...
        $exportplugins = array();
        $canpublishgrades = 0;
        if (has_capability('agpu/grade:export', $context)) {
            foreach (core_component::get_plugin_list('gradeexport') as $plugin => $plugindir) {
                ...
                $url = new agpu_url('/grade/export/'.$plugin.'/index.php', array('id'=>$courseid));
            ...
            if ($CFG->gradepublishing && $canpublishgrades != 0) {
                $url = new agpu_url('/grade/export/keymanager.php', array('id'=>$courseid));
    ...
    public static function get_user_profile_fields($courseid, $includecustomfields = false) {
        ...
        $context = context_course::instance($courseid);
        $canseehiddenfields = has_capability('agpu/course:viewhiddenuserfields', $context);
     ...
     *
     * @param array $element An array representing an element in the grade_tree
     * @return agpu_url|null link to activity or null if not found
     */
    public static function get_activity_link(array $element): ?agpu_url {
        $itemtype = $element['object']->itemtype;
        $itemmodule = $element['object']->itemmodule;
        $iteminstance = $element['object']->iteminstance;
        ...
        return new agpu_url('/mod/' . $itemmodule . '/view.php', ['id' => $cm->id]);
    }
}
