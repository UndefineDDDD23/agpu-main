...
class course_outcomes_action_bar extends action_bar {
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        $generalnavselector = new general_action_bar($this->context,
            new agpu_url('/grade/edit/outcome/course.php', ['id' => $courseid]), 'outcome', 'course');
        $data = $generalnavselector->export_for_template($output);

        if (has_capability('agpu/grade:manageoutcomes', $this->context)) {
            // Add a button to the action bar with a link to the 'manage outcomes' page.
            $manageoutcomeslink = new agpu_url('/grade/edit/outcome/index.php', ['id' => $courseid]);
