...
abstract class gradingform_controller {
     ...
     *
     * @param agpu_url $returnurl optional URL of a page where the user should be sent once they are finished with editing
     * @return agpu_url
     */
    public function get_editor_url(?agpu_url $returnurl = null) {
        ...
        return new agpu_url('/grade/grading/form/'.$this->get_method_name().'/edit.php', $params);
    }
    ...
    public function update_definition(stdClass $definition, $usermodified = null) {
        ...
        if (!empty($this->definition->id)) {
            ...
            if (empty($record->descriptionformat)) {
                $record->descriptionformat = FORMAT_agpu; // field can not be empty
            }
     ...
     * @return string
     */
    abstract public function render_preview(agpu_page $page);
 ...
 * and validation of this form element. See {@link agpuQuickForm_grading}
 *
 * @package    core_grading
...
abstract class gradingform_instance {
    ...
    public static function create_new($definitionid, $raterid, $itemid) {
        ...
        $instance->feedbackformat = FORMAT_agpu;
     ...
     *
     * @see agpuQuickForm_grading in lib/form/grading.php
     *
     * @param agpu_page $page
