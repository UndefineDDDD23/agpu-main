...
if ($courseid) {
    ...
    navigation_node::override_active_url(new agpu_url('/grade/edit/outcome/course.php', ['id' => $courseid]));
    $PAGE->navbar->add(get_string('manageoutcomes', 'grades'),
        new agpu_url('/grade/edit/outcome/index.php', ['id' => $courseid]));
    $PAGE->navbar->add(get_string('importoutcomes', 'grades'),
        new agpu_url('/grade/edit/outcome/import.php', ['courseid' => $courseid]));
...
if ($upload_form->is_cancelled()) {
    redirect(new agpu_url('/grade/edit/outcome/index.php', ['id' => $courseid]));
    die;
}
...
if (isset($courseid) && ($scope  == 'custom')) {
    // custom scale
    $local_scope = true;
} elseif (($scope == 'global') && has_capability('agpu/grade:manage', context_system::instance())) {
    // global scale
    $local_scope = false;
} else {
...
if ($handle = fopen($imported_file, 'r')) {
    ...
    while ( $csv_data = fgetcsv($handle, 8192, ';', '"')) { // if the line is over 8k, it won't work...
        ...
        if ($scale) {
            ...
            if (!has_capability('agpu/course:managescales', $context)) {
                echo $OUTPUT->notification(get_string('importskippedoutcome', 'grades',
    ...
    if ($fatal_error) {
        ...
        echo $OUTPUT->single_button(new agpu_url('/grade/edit/outcome/import.php', ['courseid' => $courseid]),
            get_string('back'), 'get');
    } else {
        echo $OUTPUT->single_button(new agpu_url('/grade/edit/outcome/index.php', ['id' => $courseid]),
