...
$url = new agpu_url('/grade/edit/outcome/index.php', ['id' => $courseid]);
$PAGE->set_url($url);
...
if ($courseid) {
    ...
    require_capability('agpu/grade:manageoutcomes', $context);

    if (empty($CFG->enableoutcomes)) {
        redirect('../../index.php?id='.$courseid);
    }
    // This page doesn't exist on the navigation so map it to another
    navigation_node::override_active_url(new agpu_url('/grade/edit/outcome/course.php', array('id'=>$courseid)));
...
switch ($action) {
    case 'delete':
        ...
        if (empty($outcome->courseid)) {
            require_capability('agpu/grade:manage', context_system::instance());
        ...
        if(!$deleteconfirmed){
            ...
            echo $OUTPUT->header();
            $confirmurl = new agpu_url('index.php', array(
...
$caneditsystemscales = has_capability('agpu/course:managescales', $systemcontext);

if ($courseid) {

    $caneditcoursescales = has_capability('agpu/course:managescales', $context);
    ...
    foreach($outcomes as $outcome) {
        ...
        if (empty($scale->id)) {   // hopefully never happens
            ...
            if (empty($scale->courseid)) {
                ...
                $caneditthisscale = has_capability('agpu/course:managescales', $context);
            }
            if ($caneditthisscale) {
...
if ($outcomes = grade_outcome::fetch_all_global()) {
    ...
    foreach($outcomes as $outcome) {
        ...
        $buttons = "";
        if (has_capability('agpu/grade:manage', context_system::instance())) {
            $buttons .= grade_button('edit', $courseid, $outcome);
        }
        if (has_capability('agpu/grade:manage', context_system::instance()) and $outcome->can_delete()) {
            $buttons .= grade_button('delete', $courseid, $outcome);
        }
        $line[] = $buttons;
...
function grade_print_scale_link($courseid, $scale, $gpr) {
    global $CFG, $OUTPUT;
    $url = new agpu_url('/grade/edit/scale/edit.php', array('courseid' => $courseid, 'id' => $scale->id));
