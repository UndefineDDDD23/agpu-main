...
$url = new agpu_url('/grade/edit/outcome/edit.php');
if ($courseid !== 0) {
...
if ($id) {
    ...
    if (!$outcome_rec = $DB->get_record('grade_outcomes', array('id' => $id))) {
        throw new \agpu_exception('invalidoutcome');
    }
    if ($outcome_rec->courseid) {
        ...
        require_capability('agpu/grade:manage', $context);
        $courseid = $course->id;
    } else {
        if ($courseid) {
        ...
        require_login();
        require_capability('agpu/grade:manage', $systemcontext);
