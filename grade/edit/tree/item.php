...
$url = new agpu_url('/grade/edit/tree/item.php', array('courseid'=>$courseid));
if ($id !== 0) {
    $url->param('id', $id);
}
$PAGE->set_url($url);
$PAGE->set_pagelayout('admin');
navigation_node::override_active_url(new agpu_url('/grade/edit/tree/index.php',
    array('id'=>$courseid)));
...
if ($grade_item = grade_item::fetch(array('id'=>$id, 'courseid'=>$courseid))) {
    ...
    if (!empty($grade_item->outcomeid) && !empty($CFG->enableoutcomes)) {
        $url = new agpu_url('/grade/edit/tree/outcomeitem.php', ['id' => $id, 'courseid' => $courseid]);
        redirect($gpr->add_url_params($url));
    }
    if ($grade_item->is_course_item() or $grade_item->is_category_item()) {
        $grade_category = $grade_item->get_item_category();
        $url = new agpu_url('/grade/edit/tree/category.php', ['id' => $grade_category->id, 'courseid' => $courseid]);
        redirect($gpr->add_url_params($url));
    }
