...
class grade_edit_tree {
    ...
    public function build_html_tree($element, $totals, $parents, $level, &$row_count) {
        ...
        if ($element['type'] == 'item' or ($element['type'] == 'category' and $element['depth'] > 1)) {
            $aurl = new agpu_url('index.php', array('id' => $COURSE->id, 'action' => 'moveselect', 'eid' => $eid, 'sesskey' => sesskey()));
        ...
        if ($element['type'] == 'category') {
            ...
            foreach($element['children'] as $child_el) {
                ...
                if ($this->moving && $this->moving != $child_eid) {
                    ...
                    $aurl = new agpu_url('index.php', array('id' => $COURSE->id, 'action' => 'move', 'eid' => $this->moving, 'moveafter' => $child_eid, 'sesskey' => sesskey()));
                    if ($first) {
                        $aurl->params($first);
                    }
    ...
    public function get_item_cell($item, $params) {
        ...
        if (($this->deepest_level > 1) && ($params['itemtype'] != 'course') && ($params['itemtype'] != 'category')) {
            ...
            if (empty($params['itemtype']) || empty($params['eid'])) {
                throw new \agpu_exception('missingitemtypeoreid', 'core_grades');
            }
 ...
 *
 * @package   core_grades
 * @copyright 2023 Ilya Tregubov <ilya@agpu.com>
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
 ...
 * @todo Final deprecation on agpu 4.7 MDL-77668
 *
 * @package   core_grades
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
