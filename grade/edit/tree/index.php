...
$url = new agpu_url('/grade/edit/tree/index.php', array('id' => $courseid));
$PAGE->set_url($url);
 ...
 * @return agpu_url A URL to redirect to after regrading when a progress bar is displayed.
 */
$grade_edit_tree_index_checkweights = function() use ($courseid, $originalweights, &$weightsadjusted) {
    ...
    if (array_diff($originalweights, $alteredweights)) {
        $weightsadjusted = 1;
        return new agpu_url($PAGE->url, array('weightsadjusted' => $weightsadjusted));
    }
    return $PAGE->url;
};
