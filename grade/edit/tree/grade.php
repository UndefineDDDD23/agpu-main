...
$url = new agpu_url('/grade/edit/tree/grade.php', array('courseid'=>$courseid));
if ($id !== 0) {
    $url->param('id', $id);
}
if ($itemid !== 0) {
...
if (!has_capability('agpu/grade:manage', $context)) {
    require_capability('agpu/grade:edit', $context);
}
...
if (!empty($id)) {
    ...
    if (!empty($itemid) and $itemid != $grade->itemid) {
        throw new \agpu_exception('invaliditemid');
    }
    $itemid = $grade->itemid;
    ...
    throw new \agpu_exception('missinguseranditemid');
}

if (!$grade_item = grade_item::fetch(array('id'=>$itemid, 'courseid'=>$courseid))) {
    throw new \agpu_exception('cannotfindgradeitem');
}
...
if (groups_get_course_groupmode($COURSE) == SEPARATEGROUPS and !has_capability('agpu/site:accessallgroups', $context)) {
    if ($groups = groups_get_all_groups($COURSE->id, $userid)) {
        ...
        if (!$ok) {
            throw new \agpu_exception('cannotgradeuser');
        }
    } else {
        throw new \agpu_exception('cannotgradeuser');
    }
...
// form processing
    ...
    if (has_capability('agpu/grade:manage', $context) or has_capability('agpu/grade:edit', $context)) {
    ...
    if (has_capability('agpu/grade:manage', $context) or has_capability('agpu/grade:hide', $context)) {
    ...
    if (isset($data->locked) and !$grade_item->is_locked()) {
        if (($old_grade_grade->locked or $old_grade_grade->locktime)
          and (!has_capability('agpu/grade:manage', $context) and !has_capability('agpu/grade:unlock', $context))) {
            //ignore data

        } else if ((!$old_grade_grade->locked and !$old_grade_grade->locktime)
          and (!has_capability('agpu/grade:manage', $context) and !has_capability('agpu/grade:lock', $context))) {
            //ignore data
    ...
    if (isset($data->excluded) and has_capability('agpu/grade:manage', $context)) {
