...
if ($courseid) {
    ...
    require_capability('agpu/course:managescales', $context);
...
switch ($action) {
    case 'delete':
        ...
        if (empty($scale->courseid)) {
            require_capability('agpu/course:managescales', context_system::instance());
        ...
        if (!$deleteconfirmed) {
            if ($courseid) {
                $PAGE->navbar->add(get_string('scales'), new agpu_url('/grade/edit/scale/index.php',
                    ['id' => $courseid]));
            }
...
if ($scales = grade_scale::fetch_all_global()) {
    ...
    foreach($scales as $scale) {
        ...
        $buttons = "";
        if (has_capability('agpu/course:managescales', context_system::instance())) {
            $buttons .= grade_button('edit', $courseid, $scale);
        }
        if (!$used and has_capability('agpu/course:managescales', context_system::instance())) {
            $buttons .= grade_button('delete', $courseid, $scale);
        }
        $line[] = $buttons;
        $data[] = $line;
