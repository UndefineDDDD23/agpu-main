...
$url = new agpu_url('/grade/edit/letter/index.php', array('id' => $contextid));
...
//if viewing
if (!$edit) {
    if (!has_capability('agpu/grade:manage', $context) and !has_capability('agpu/grade:manageletters', $context)) {
        throw new \agpu_exception('nopermissiontoviewletergrade');
    }
} else {//else we're editing
    require_capability('agpu/grade:manageletters', $context);
...
if ($context->contextlevel == CONTEXT_SYSTEM or $context->contextlevel == CONTEXT_COURSECAT) {
    ...
    throw new \agpu_exception('invalidcourselevel');
}
