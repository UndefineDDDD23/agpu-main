...
class default_handler extends handler {
     ...
     *
     * This is a regular interactive page for web users. It should have normal agpu header/footers, it should
     * allow user to view policies and accept them.
     *
     * @param bool $forguests
     * @return agpu_url|null (returns null if site policy is not defined)
     */
    public static function get_redirect_url($forguests = false) {
        return static::is_defined($forguests) ? new agpu_url('/user/policy.php') : null;
    }
    ...
    public static function get_embed_url($forguests = false) {
        ...
        if ($forguests && !empty($CFG->sitepolicyguest)) {
            return new agpu_url($CFG->sitepolicyguest);
        } else if (!$forguests && !empty($CFG->sitepolicy)) {
            return new agpu_url($CFG->sitepolicy);
        }
        return null;
    }
}
