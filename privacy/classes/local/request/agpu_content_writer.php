...
/**
 * The agpu_content_writer is the default agpu implementation of a content writer.
 *
 * It exports data to a rich tree structure using agpu's context system,
 ...
 */
class agpu_content_writer implements content_writer {
    /**
     ...
     * @throws  \agpu_exception           If the file cannot be written for some reason.
     */
    protected function write_data(string $path, string $data) {
        ...
        if (file_put_contents($targetpath, $data) === false) {
            throw new \agpu_exception('cannotsavefile', 'error', '', $targetpath);
        }
    ...
    protected function make_tree_object(array $tree, array $treekey, array $currentkey = []): Array {
        ...
        $addfinalfile = function($directory, $treeleaf, $file) use ($treekey) {
            ...
            if (isset($treekey[$gokey]) && $treekey[$gokey] !== 'No var') {
                ...
                $treeleaf->url = new \agpu_url($url . '/' . $file);
            }
        };
    ...
    protected function write_html_data() {
        ...
        $agpuimgpath = ['pix', 'agpulogo.svg'];
        $this->copy_data($agpuimgpath, ['pix', 'agpulogo.svg']);

        // Additional required css.
        $csspath = ['theme', 'boost', 'style', 'agpu.css'];
        $destination = ['agpu.css'];
     ...
     * @throws \agpu_exception If the file cannot be opened.
     */
    protected function get_file_content(string $filepath): String {
        $content = file_get_contents($filepath);
        if ($content === false) {
            throw new \agpu_exception('cannotopenfile', 'error', '', $filepath);
        }
        return $content;
    }
}
