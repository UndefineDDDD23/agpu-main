...
class core_files_renderer extends plugin_renderer_base {
    ...
    public function render_files_tree_viewer(files_tree_viewer $tree) {
        $html = $this->output->heading_with_help(get_string('coursefiles'), 'courselegacyfiles', 'agpu');
        ...
        $html .= $this->output->single_button(new agpu_url('/files/coursefilesedit.php', array('contextid'=>$tree->context->id)), get_string('coursefilesedit'), 'get');
    ...
    public function render_form_filemanager($fm) {
        ...
        $module = array(
            ...
            'requires' => array('agpu-core-notification-dialogue', 'core_filepicker', 'base', 'io-base', 'node', 'json', 'core_dndupload', 'panel', 'resize-plugin', 'dd-plugin'),
            'strings' => array(
                array('error', 'agpu'), array('info', 'agpu'), array('confirmdeletefile', 'repository'),
                ...
                array('confirmrenamefile', 'repository'), array('newfolder', 'repository'), array('edit', 'agpu'),
                ...
                ['confirmdeleteselectedfile', 'repository'], ['selectall', 'agpu'], ['deselectall', 'agpu'],
...
<div class="filemanager fp-mkdir-dlg" role="dialog" aria-live="assertive" aria-labelledby="fp-mkdir-dlg-title">
    ...
    protected function fm_print_restrictions($fm) {
        ...
        if ($hasmaxfiles && $hasarealimit) {
            $maxsize = get_string('maxsizeandattachmentsandareasize', 'agpu', $strparam);
        } else if ($hasmaxfiles) {
            $maxsize = get_string('maxsizeandattachments', 'agpu', $strparam);
        } else if ($hasarealimit) {
            $maxsize = get_string('maxsizeandareasize', 'agpu', $strparam);
        } else {
            $maxsize = get_string('maxfilesize', 'agpu', $maxbytes);
        }
...
<div class="file-picker fp-msg" role="alertdialog" aria-live="assertive" aria-labelledby="fp-msg-labelledby">
    ...
    protected function create_license_help_icon_context(): stdClass {
        ...
        $iconcontext->alt = get_string('helpprefix2', 'agpu', get_string('chooselicense', 'repository'));
...
/**
 * Data structure representing a general agpu file tree viewer
 *
 * @copyright 2010 Dongsheng Cai
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @since     agpu 2.0
...
class files_tree_viewer implements renderable {
    ...
    /**
     * Constructor of agpu_file_tree_viewer class
    ...
    public function __construct(file_info $file_info, ?array $options = null) {
        ...
        while ($level) {
            ...
            unset($params['itemid']);
            $url = new agpu_url('/files/index.php', $params);
