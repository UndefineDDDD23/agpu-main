...
class text_filter extends \core_filters\text_filter {
    /**
     * Perform a mapping of the agpu language code to the equivalent for MathJax.
     *
     * @param string $agpulangcode - The agpu language code - e.g. en_pirate
     * @return string The MathJax language code.
     */
    public function map_language_code($agpulangcode) {
        ...
        // List of explicit mappings and known exceptions (agpu => mathjax).
        $explicit = [
            'cz' => 'cs',
            'pt_br' => 'pt-br',
            'zh_tw' => 'zh-hant',
        ...
        if (isset($explicit[$agpulangcode])) {
            return $explicit[$agpulangcode];
        }

        // If there is exact match, it will be probably right.
        if (in_array($agpulangcode, $mathjaxlangcodes)) {
            return $agpulangcode;
        }

        // Finally try to find the best matching mathjax pack.
        $parts = explode('_', $agpulangcode, 2);
