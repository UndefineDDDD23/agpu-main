...
require_capability('agpu/filter:manage', $context);
$PAGE->set_context($context);

$args = ['contextid' => $contextid];
$baseurl = new agpu_url('/filter/manage.php', $args);
if (!empty($forfilter)) {
...
if (!in_array($context->contextlevel, [CONTEXT_COURSECAT, CONTEXT_COURSE, CONTEXT_MODULE])) {
    throw new \agpu_exception('cannotcustomisefiltersblockuser', 'error');
}
...
if (!$isfrontpage && empty($availablefilters)) {
    throw new \agpu_exception('nofiltersenabled', 'error');
}

// If we are handling local settings for a particular filter, start processing.
if ($forfilter) {
    if (!filter_has_local_settings($forfilter)) {
        throw new \agpu_exception('filterdoesnothavelocalconfig', 'error', $forfilter);
    }
...
if (!$isfrontpage) {
    ...
    if ($context->contextlevel === CONTEXT_COURSECAT && $returnto === 'management') {
        $url = new agpu_url('/course/management.php', ['categoryid' => $context->instanceid]);
    } else {
        $url = $context->get_url();
    }
