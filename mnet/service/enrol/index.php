...
$roamingusers = get_users_by_capability(context_system::instance(), 'agpu/site:mnetlogintoremote', 'u.id');
if (empty($roamingusers)) {
    $capname = get_string('site:mnetlogintoremote', 'role');
    $url = new agpu_url('/admin/roles/manage.php');
...
foreach ($hosts as $host) {
    $hostlink = html_writer::link(new agpu_url($host->hosturl), s($host->hosturl));
    $editbtn  = $OUTPUT->single_button(new agpu_url('/mnet/service/enrol/host.php', array('id'=>$host->id)),
