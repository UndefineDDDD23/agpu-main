...
class mnetservice_enrol {
    ...
    public function get_remote_courses($mnethostid, $usecache=true) {
        ...
        if ($request->send($peer)) {
            ...
            foreach ($response as &$remote) {
                ...
                $course->summaryformat  = empty($remote['summaryformat']) ? FORMAT_agpu : (int)$remote['summaryformat'];
    ...
    /**
     * Prepares error messages returned by our XML-RPC requests to be send as debug info to {@see \agpu_exception()}
     *
...
class mnetservice_enrol_potential_users_selector extends user_selector_base {
     ...
     *
     * Our users must have 'agpu/site:mnetlogintoremote' capability assigned.
    ...
    public function find_users($search) {
        ...
        $userids = get_users_by_capability($systemcontext, 'agpu/site:mnetlogintoremote', 'u.id');
