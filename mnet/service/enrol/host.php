...
admin_externalpage_setup('mnetenrol', '', array('id'=>$hostid, 'usecache'=>1),
        new agpu_url('/mnet/service/enrol/host.php'));
...
if (empty($hosts[$hostid])) {
    throw new \agpu_exception('wearenotsubscribedtothishost', 'mnetservice_enrol');
}
$host = $hosts[$hostid];
...
if (is_string($courses)) {
    throw new \agpu_exception('fetchingcourses', 'mnetservice_enrol', '', null, $service->format_error_message($courses));
}
...
if (empty($courses)) {
    ...
    if ($usecache) {
        echo $OUTPUT->single_button(new agpu_url($PAGE->url, array('usecache'=>0, 'sesskey'=>sesskey())),
...
foreach ($courses as $course) {
    ...
    $editbtn = $OUTPUT->single_button(new agpu_url('/mnet/service/enrol/course.php',
