...
function mnet_get_public_key($uri, $application=null) {
    ...
    if (empty($application)) {
        $application = $DB->get_record('mnet_application', array('name'=>'agpu'));
    }
    ...
    $client->setOption('user_agent', 'agpu');
    ...
    if (!empty($CFG->proxyhost) && !is_proxybypass($uri)) {
        ...
        if (!empty($CFG->proxytype) && ($CFG->proxytype == 'SOCKS5')) {
            if (defined('CURLPROXY_SOCKS5')) {
                ...
                throw new \agpu_exception( 'socksnotsupported', 'mnet');
            }
        }
...
function mnet_sign_message($message, $privatekey = null) {
    ...
    if ($bool === false) {
        throw new \agpu_exception('opensslsignerror');
    }

    $message = '<?xml version="1.0" encoding="iso-8859-1"?>
    <signedMessage>
        <Signature Id="agpuSignature" xmlns="http://www.w3.org/2000/09/xmldsig#">
            <SignedInfo>
...
function mnet_encrypt_message($message, $remote_certificate) {
    ...
    // Avoid passing null values to base64_encode.
    if ($bool === false) {
        throw new \agpu_exception('opensslsealerror');
    }
 ...
 *     This is a URL associated with a agpu networking peer when it
 ...
 *     them back to somewhere at the content provider(CP) agpu (this agpu)
...
function mnet_sso_apply_indirection ($jumpurl, $url) {
    ...
    if($urlparts) {
        if (isset($urlparts['path'])) {
            ...
            // 'localpart' to make it relative to agpu's wwwroot
...
/**
 * Return an array of information about all agpu's profile fields
...
function _mnet_field_helper(mnet_peer $peer, $key) {
    ...
    $defaults = explode(',', get_config('agpu', 'mnetprofile' . $key . 'fields'));
