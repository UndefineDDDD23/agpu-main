...
/**
 * comment_manager is helper class to manage agpu comments in admin page (Reports->Comments)
 *
...
class comment_manager {
    ...
    function get_comments($page) {
        ...
        foreach ($rs as $item) {
            ...
            $item->content = format_text($item->content, FORMAT_agpu, $formatoptions);
     ...
     *
     * @deprecated since agpu 4.2 - please do not use this function any more
     */
    public function print_comments($page = 0) {
        ...
        if (count($comments) == 0) {
            echo $OUTPUT->notification(get_string('nocomments', 'agpu'));
            return false;
        }
        ...
        $link = new agpu_url('/comment/index.php', array('action' => 'delete', 'sesskey' => sesskey()));
        foreach ($comments as $c) {
            $userdata = html_writer::link(new agpu_url('/user/profile.php', ['id' => $c->userid]), $c->fullname);
            $this->setup_plugin($c);
            ...
            $action = html_writer::link(new agpu_url($link, array('commentid' => $c->id)), get_string('delete'));
