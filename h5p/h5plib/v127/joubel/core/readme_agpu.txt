...
Import procedure:
 * Remove the content in this folder (but the readme_agpu.txt)
...
Changed:
 ...
 value of the coreApi minor and major versions. If they are different from the values in the current agpu version, instead of
 ...
 1. Replace the $_SESSION references with $SESSION. That implies that the information is saved to backends, so only the agpu one
...
2. Add namespace to this library to avoid collision. It means:
  - Add the "namespace agpu;" line at the top of all the h5p*.php files in the root folder.
...
6. Open js/h5p.js and in function contentUserDataAjax() add the following patch:
  function contentUserDataAjax(contentId, dataType, subContentId, done, data, preload, invalidate, async) {
    ...
    // agpu patch to let override this method.
