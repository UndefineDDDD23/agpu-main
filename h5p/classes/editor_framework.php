 ...
 *
 * @package    core_h5p
 * @copyright  2020 Victor Deniz <victor@agpu.com>, base on code by Joubel AS
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace core_h5p;

use agpu\H5peditorStorage;
use stdClass;

/**
 * agpu's implementation of the H5P Editor storage interface.
 *
 * Makes it possible for the editor's core library to communicate with the
 * database used by agpu.
 *
 * @package    core_h5p
 * @copyright  2020 Victor Deniz <victor@agpu.com>, base on code by Joubel AS
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class editor_framework implements H5peditorStorage {
    ...
    private function get_language_record(string $name, int $major, int $minor, string $lang) {
        ...
        if ($result === false) {
            ...
            // Normalise agpu language using underscore, as opposed to H5P which uses dash.
            $agpulanguage = str_replace('-', '_', $lang);

            $dependencies = get_string_manager()->get_language_dependencies($agpulanguage);
