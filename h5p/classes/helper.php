...
class helper {
     ...
     * @param \stored_file $file agpu file instance
    ...
    public static function can_deploy_package(\stored_file $file): bool {
        ...
        return has_capability('agpu/h5p:deploy', $context, $userid);
    }
    ...
    public static function can_update_library(\stored_file $file): bool {
        ...
        return has_capability('agpu/h5p:updatelibraries', $context, $userid);
    }
        ...
        if ($task = \core\task\manager::get_scheduled_task('\core\task\h5p_get_content_types_task')) {
            ...
            if ($status && \core\task\manager::is_runnable() && get_config('tool_task', 'enablerunnow')) {
                $statusaction = \html_writer::link(
                    new \agpu_url('/admin/tool/task/schedule_task.php',
     ...
     * @param \agpu_url $link The URL to management page
    ...
    public static function get_core_settings(?string $component = null): array {
        ...
        $usersettings = [];
        if (isloggedin()) {
            $usersettings['name'] = fullname($USER, has_capability('agpu/site:viewfullnames', $systemcontext));
    ...
    public static function get_core_assets(?string $component = null): array {
        ...
        foreach (core::$styles as $style) {
            ...
            $PAGE->requires->css(new \agpu_url($liburl . $style . $cachebuster));
        }
        // Add core JavaScript.
     ...
     * @param  \agpu_url $url The URL of the exported file.
     * @param  factory $factory The \core_h5p\factory object
     * @return array|null The information export file otherwise null.
     */
    public static function get_export_info(string $exportfilename, ?\agpu_url $url = null, ?factory $factory = null): ?array {
