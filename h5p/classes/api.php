...
class api {
    ...
    public static function create_content_from_pluginfile_url(string $url, \stdClass $config, factory $factory,
        ...
        if ($h5p) {
            ...
            if (!self::is_library_enabled($mainlibrary)) {
                ...
                if ($displayoptions != $h5p->displayoptions && has_capability('agpu/h5p:setdisplayoptions', $context)) {
            ...
            if (!$h5pid && $file->get_userid() != $USER->id && has_capability('agpu/h5p:updatelibraries', $context)) {
    ...
    protected static function can_access_pluginfile_hash(string $url, bool $preventredirect = true): bool {
        ...
        $url = new \agpu_url(urldecode($url));
        ...
        try {
            ...
            throw new \agpu_exception('invalidcontextid', 'core_h5p');
        }
        ...
        if ($context->contextlevel == CONTEXT_USER && $USER->id !== $context->instanceid) {
            throw new \agpu_exception('h5pprivatefile', 'core_h5p');
        }
    ...
    public static function is_library_enabled(\stdClass $librarydata): bool {
        ...
        if (empty($params)) {
            throw new \agpu_exception("Missing 'machinename' or 'id' in librarydata parameter");
        }
