...
require_capability('agpu/h5p:updatelibraries', $context);

$pagetitle = get_string('h5pmanage', 'core_h5p');
$url = new \agpu_url("/h5p/libraries.php");
...
if ($deletelibrary) {
    ...
    if ($confirm) {
        ...
        redirect(new agpu_url('/h5p/libraries.php'));
    }
    ...
    echo $OUTPUT->confirm(
        ...
        new agpu_url($PAGE->url, ['deletelibrary' => $deletelibrary, 'confirm' => 1]),
        new agpu_url('/h5p/libraries.php')
    );
    echo $OUTPUT->footer();
    die();
}
