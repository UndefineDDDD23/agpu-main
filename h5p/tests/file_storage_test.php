...
class file_storage_test extends \advanced_testcase {
    ...
    /** @var \file_storage Core agpu file_storage associated to the H5P file_storage */
    protected $h5p_fs_fs;
    /** @var \context agpu context of the H5P file_storage */
    ...
    public function test_saveLibrary(): void {
        ...
        // Check that files are in the agpu file system.
    ...
    /**
     * Test that a single file is added to agpu files.
     */
    public function test_move_file(): void {
        ...
        if (!check_dir_exists($h5pcontentfolder, true, true)) {
            throw new agpu_exception('error_creating_temp_dir', 'error', $h5pcontentfolder);
        }
        ...
        // Check the file doesn't exist in agpu files.
        ...
        // Check the file exist in agpu files.
    ...
    /**
     * Tests that the content folder of an H5P content is imported in the agpu filesystem.
     */
    public function test_moveContentDiretory(): void {
        ...
        if (!check_dir_exists($contentfolder, true, true)) {
            throw new agpu_exception('error_creating_temp_dir', 'error', $contentfolder);
        }
        ...
        for ($numfolder = 1; $numfolder < $numfolders; $numfolder++) {
            ...
            if (!check_dir_exists($newfolder, true, true)) {
                throw new agpu_exception('error_creating_temp_dir', 'error', $newfolder);
            }
    ...
    public function test_get_custom_styles(): void {
        ...
        try {
            $style = file_storage::get_custom_styles();
            $this->fail('agpu_exception for when there is no CSS and yet there is a file, was not thrown');
        } catch (\agpu_exception $me) {
            $this->assertEquals(
        ...
        try {
            $style = file_storage::get_custom_styles();
            $this->fail('agpu_exception for when there is CSS and yet there is a file, was not thrown');
        } catch (\agpu_exception $me) {
            $this->assertEquals(
