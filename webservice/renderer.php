...
class core_webservice_renderer extends plugin_renderer_base {
    ...
    public function admin_authorised_user_selector(&$options) {
        ...
        $actionurl = new agpu_url('/' . $CFG->admin . '/webservice/service_users.php',
    ...
    public function admin_authorised_user_list($users, $serviceid) {
        ...
        foreach ($users as $user) {
            $settingsurl = new agpu_url('/admin/webservice/service_user_settings.php',
    ...
    public function admin_remove_service_function_confirmation($function, $service) {
        ...
        $formcontinue = new single_button(new agpu_url('service_functions.php',
                                $optionsyes), get_string('remove'));
        $formcancel = new single_button(new agpu_url('service_functions.php',
    ...
    public function admin_remove_service_confirmation($service) {
        ...
        $formcontinue = new single_button(new agpu_url('service.php', $optionsyes),
                        get_string('delete'), 'post');
        $formcancel = new single_button(
                        new agpu_url($CFG->wwwroot . "/" . $CFG->admin . "/settings.php", $optionsno),
    ...
    public function admin_service_function_list($functions, $service) {
        ...
        if (!empty($functions)) {
            ...
            foreach ($functions as $function) {
                ...
                //display remove function operation (except for build-in service)
                if (empty($service->component)) {
                    $removeurl = new agpu_url('/' . $CFG->admin . '/webservice/service_functions.php',
        ...
        if (empty($service->component)) {
            ...
            $addurl = new agpu_url('/' . $CFG->admin . '/webservice/service_functions.php',
    ...
    public function user_webservice_tokens_box($tokens, $userid, $documentation = false) {
        ...
        if (!empty($tokens)) {
            foreach ($tokens as $token) {
                ...
                $userprofilurl = new agpu_url('/user/view.php?id=' . $token->creatorid);
                ...
                if ($documentation) {
                    $doclink = new agpu_url('/webservice/wsdoc.php',
                            array('id' => $token->id));
...
EOF;
        ...
        $docurl = new agpu_url('http://docs.agpu.org/dev/Creating_a_web_service_client');
        ...
        $url = new agpu_url($parenturl, $authparams); // Required
