...
class webservice_rest_client {
    ...
    /** @var agpu_url the REST server url */
    private $serverurl;
     ...
     * Constructor
     *
     * @param string $serverurl a agpu URL
    ...
    public function __construct($serverurl, $token, $format = 'xml') {
        $this->serverurl = new agpu_url($serverurl);
        $this->token = $token;
        $this->format = $format;
    }
    ...
    public function call($functionname, $params) {
        global $DB, $CFG;
         ...
         if ($this->format == 'json') {
             $formatparam = '&agpuwsrestformat=json';
             $this->serverurl->param('agpuwsrestformat','json');
         } else {
