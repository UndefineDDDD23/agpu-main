...
/**
 * WEBSERVICE_AUTHMETHOD_SESSION_TOKEN - token for embedded application (requires agpu session)
 */
 ...
 *
 * @package    core_webservice
 * @copyright  2010 Jerome Mouneyrac <jerome@agpu.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class webservice {
    /**
    ...
    public function authenticate_user($token) {
        ...
        if (!$token = $DB->get_record('external_tokens', array('token' => $token))) {
            //client may want to display login form => agpu_exception
            throw new agpu_exception('invalidtoken', 'webservice');
        }
        ...
        $hasmaintenanceaccess = has_capability('agpu/site:maintenanceaccess', context_system::instance(), $user);
        if (!empty($CFG->maintenance_enabled) and !$hasmaintenanceaccess) {
            //this is usually temporary, client want to implement code logic  => agpu_exception
        ...
        if (!empty($user->suspended)) {
            ...
            $event->add_record_snapshot('external_tokens', $token);
            $event->trigger();
            throw new agpu_exception('wsaccessusersuspended', 'agpu', '', $user->username);
        }
        ...
        if ($user->auth == 'nologin') {
            ...
            $event->add_record_snapshot('external_tokens', $token);
            $event->trigger();
            throw new agpu_exception('wsaccessusernologin', 'agpu', '', $user->username);
        }
    ...
    public function generate_user_ws_tokens($userid) {
        ...
        if (!is_siteadmin() && has_capability('agpu/webservice:createtoken', context_system::instance(), $userid) && !empty($CFG->enablewebservices)) {
     ...
     *
     * @param string $token
     * @throws agpu_exception if there is multiple result
     */
     ...
     *    (
     *       [0] => agpu/course:managegroups
     *    )
     *
     *    [core_enrol_get_enrolled_users] => Array
     *    (
     *       [0] => agpu/user:viewdetails
     *       [1] => agpu/user:viewhiddendetails
     *       [2] => agpu/course:useremail
     *       [3] => agpu/user:update
     *       [4] => agpu/site:accessallgroups
     *    )
     *  )
     * @param int $serviceid service id
     * @return array
     ...
     *
     * @deprecated since agpu 3.11 in MDL-67748 without a replacement.
     * @todo MDL-70187 Please delete this method completely in agpu 4.3, thank you.
     * @param int $userid user id
     * @return array
     */
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class webservice_access_exception extends agpu_exception {
...
abstract class webservice_server implements webservice_server_interface {
    ...
    protected function authenticate_user() {
        ...
        if ($this->authmethod == WEBSERVICE_AUTHMETHOD_USERNAME) {
            ...
            if (!$this->username) {
                throw new agpu_exception('missingusername', 'webservice');
            }

            if (!$this->password) {
                throw new agpu_exception('missingpassword', 'webservice');
            }
            ...
            if (!$auth->user_login_webservice($this->username, $this->password)) {
                ...
                throw new agpu_exception('wrongusernamepassword', 'webservice');
            }
        ...
        if (!empty($user->deleted)) {
            ...
            $event = \core\event\webservice_login_failed::create($params);
            $event->trigger();
            throw new agpu_exception('wsaccessuserdeleted', 'webservice', '', $user->username);
        }
        ...
        if (empty($user->confirmed)) {
            ...
            $event = \core\event\webservice_login_failed::create($params);
            $event->trigger();
            throw new agpu_exception('wsaccessuserunconfirmed', 'webservice', '', $user->username);
        }
        ...
        if (!empty($user->suspended)) {
            ...
            $event = \core\event\webservice_login_failed::create($params);
            $event->trigger();
            throw new agpu_exception('wsaccessusersuspended', 'webservice', '', $user->username);
        }
        ...
        if (!empty($auth->config->expiration) and $auth->config->expiration == 1) {
            ...
            if (intval($days2expire) < 0 ) {
                ...
                $event = \core\event\webservice_login_failed::create($params);
                $event->trigger();
                throw new agpu_exception('wsaccessuserexpired', 'webservice', '', $user->username);
            }
        }
        ...
        if ($user->auth=='nologin') {
            ...
            $event = \core\event\webservice_login_failed::create($params);
            $event->trigger();
            throw new agpu_exception('wsaccessusernologin', 'webservice', '', $user->username);
        }
    ...
    /**
     * Intercept some agpuwssettingXXX $_GET and $_POST parameter
    ...
    protected function set_web_service_call_settings() {
        ...
        // Must be the same XXX ws parameter name as 'agpuwssettingXXX'.
        ...
        foreach ($externalsettings as $name => $settingdata) {
            ...
            $wsparamname = 'agpuwssetting' . $name;
...
abstract class webservice_base_server extends webservice_server {
    ...
    protected function execute() {
        ...
        // Allow any agpu plugin a chance to override this call. This is a convenient spot to
    ...
    protected function get_virtual_method_code($function) {
        ...
        foreach ($function->parameters_desc->keys as $name => $keydesc) {
            ...
            if ($keydesc->required == VALUE_OPTIONAL) {
                ...
                throw new agpu_exception('erroroptionalparamarray', 'webservice', '', $name);
                ...
                if ($keydesc instanceof external_value) {
                    ...
                    if (isset($keydesc->default) && is_array($keydesc->default) && empty($keydesc->default)) {
                        ...
                        throw new agpu_exception('errornotemptydefaultparamarray', 'webservice', '', $name);
                    }
                }
            }
