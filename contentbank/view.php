...
if (!$content->is_view_allowed()) {
    $cburl = new \agpu_url('/contentbank/index.php', ['contextid' => $context->id, 'errormsg' => 'notavailable']);
    redirect($cburl);
}
...
$PAGE->set_url(new \agpu_url('/contentbank/view.php', ['id' => $id]));
    ...
    if ($statusmsg == 'contentvisibilitychanged') {
        switch ($content->get_visibility()) {
            ...
            case content::VISIBILITY_UNLISTED:
                ...
                break;
            default:
                throw new \agpu_exception('contentvisibilitynotfound', 'error', $returnurl, $content->get_visibility());
                break;
        }
