 ...
 * @copyright  2020 Victor Deniz <victor@agpu.com>
...
$returnurl = new \agpu_url('/contentbank/view.php', ['id' => $id]);
...
if (!empty($id)) {
    ...
    $heading = get_string('addinganew', 'agpu', get_string('description', $contenttypename));
    $content = null;
...
if (!$plugin || !$plugin->is_enabled()) {
    throw new \agpu_exception('unsupported', 'core_contentbank', $returnurl);
}
...
if (!$contenttype->can_edit($content)) {
    throw new \agpu_exception('contenttypenoedit', 'core_contentbank', $returnurl);
}
...
$PAGE->set_url(new \agpu_url('/contentbank/edit.php', $values));
...
if ($content) {
    $PAGE->navbar->add($content->get_name(), new \agpu_url('/contentbank/view.php', ['id' => $id]));
}
...
if ($editorform->is_cancelled()) {
    if (empty($id)) {
        $returnurl = new \agpu_url('/contentbank/index.php', ['contextid' => $context->id]);
    }
    redirect($returnurl);
