...
class content extends \core_contentbank\content {
    ...
    public function is_view_allowed(): bool {
        ...
        if (!empty($file)) {
            ...
            if (empty($h5p)) {
                ...
                if (!has_capability('agpu/contentbank:manageanycontent', $context)) {
                    return false;
                }
     ...
     * an exception.
     *
     * @throws \agpu_exception If file operations fail
    ...
    public function import_file(\stored_file $file): ?\stored_file {
        ...
        if (!\core_h5p\api::is_valid_package($file, $onlyupdatelibs)) {
            ...
            if (empty($errors) || count($errors) > 1) {
                throw new \agpu_exception('notvalidpackage', 'h5p');
            }
            throw new \agpu_exception($errors[0]->code, 'h5p');
        }
