...
class core_cohort_external extends external_api {
                    ...
                    new external_single_structure(
                        array(
                            ...
                            'theme' => new external_value(PARAM_THEME,
                                'the cohort theme. The allowcohortthemes setting must be enabled on agpu',
                                VALUE_OPTIONAL
                            ),
            ...
            if (isset($cohort->theme)) {
                if (!empty($CFG->allowcohortthemes)) {
                    if (empty($availablethemes[$cohort->theme])) {
                        throw new agpu_exception('errorinvalidparam', 'webservice', '', 'theme');
                    }
                }
            }
    ...
    public static function get_cohorts($cohortids = array()) {
        ...
        foreach ($cohorts as $cohort) {
            ...
            self::validate_context($context);
            if (!has_any_capability(array('agpu/cohort:manage', 'agpu/cohort:view'), $context)) {
                throw new required_capability_exception($context, 'agpu/cohort:view', 'nopermissions', '');
            }
    ...
    public static function search_cohorts($query, $context, $includes = 'parents', $limitfrom = 0, $limitnum = 25) {
        ...
        $manager = has_capability('agpu/cohort:manage', $context);
        if (!$manager) {
            require_capability('agpu/cohort:view', $context);
        }
    ...
    public static function update_cohorts($cohorts) {
        ...
        foreach ($params['cohorts'] as $cohort) {
            ...
            require_capability('agpu/cohort:manage', $oldcontext);
            ...
            if (!empty($cohort->theme) && !empty($CFG->allowcohortthemes)) {
                if (empty($availablethemes[$cohort->theme])) {
                    ...
                    throw new agpu_exception('errorinvalidparam', 'webservice', '', 'theme', $debuginfo);
                }
            }
    ...
    public static function add_cohort_members($members) {
        ...
        foreach ($params['members'] as $member) {
            ...
            try {
                ...
                throw new agpu_exception('Error', 'cohort', '', $e->getMessage());
            }
            if (!has_any_capability(array('agpu/cohort:manage', 'agpu/cohort:assign'), $context)) {
                throw new required_capability_exception($context, 'agpu/cohort:assign', 'nopermissions', '');
            }
