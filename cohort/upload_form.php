 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class cohort_upload_form extends agpuform {
    ...
    protected function get_cohort_file($draftid) {
        global $USER;
        // We can not use agpuform::get_file_content() method because we need the content before the form is validated.
        if (!$draftid) {
            return null;
        }
        $fs = get_file_storage();
    ...
    protected function preview_uploaded_cohorts() {
        ...
        foreach ($previewdrows as $idx) {
            ...
            foreach ($columns as $key => $value) {
                if ($key === 'contextid') {
                    $text = html_writer::link(new agpu_url('/cohort/index.php', array('contextid' => $context->id)),
