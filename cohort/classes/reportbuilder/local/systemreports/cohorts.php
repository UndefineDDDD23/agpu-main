...
class cohorts extends system_report {
    ...
    protected function initialise(): void {
        ...
        $this->set_checkbox_toggleall(static function(stdClass $cohort): ?array {
            if (!has_capability('agpu/cohort:manage', context::instance_by_id($cohort->contextid))) {
                return null;
            }

            return [
                $cohort->id,
                get_string('selectitem', 'agpu', $cohort->name),
            ];
        });
    ...
    protected function can_view(): bool {
        ...
        return has_any_capability(['agpu/cohort:manage', 'agpu/cohort:view'], $context);
    }
            ...
            $this->add_column_from_entity('cohort:context')
                ->add_callback(static function(string $value, stdClass $cohort): string {
                    ...
                    if ($context instanceof context_coursecat) {
                        return html_writer::link(new agpu_url('/cohort/index.php',
    ...
    protected function add_actions(): void {
        ...
        $contextid = $this->get_parameter('contextid', 0, PARAM_INT);
        $showall = $this->get_parameter('showall', true, PARAM_BOOL);
        $returnurl = (new agpu_url('/cohort/index.php',
            ['id' => ':id', 'contextid' => $contextid, 'showall' => $showall]))->out(false);

        // Hide action. It will be only shown if the property 'visible' is true and user has 'agpu/cohort:manage' capabillity.
        $this->add_action((new action(
            new agpu_url('/cohort/edit.php', ['id' => ':id', 'sesskey' => sesskey(), 'hide' => 1, 'returnurl' => $returnurl]),
            ...
            return empty($row->component) && $row->visible
                && has_capability('agpu/cohort:manage', context::instance_by_id($row->contextid));
        }));

        // Show action. It will be only shown if the property 'visible' is false and user has 'agpu/cohort:manage' capabillity.
        $this->add_action((new action(
            new agpu_url('/cohort/edit.php', ['id' => ':id', 'sesskey' => sesskey(), 'show' => 1, 'returnurl' => $returnurl]),
        ...
        // Edit action. It will be only shown if user has 'agpu/cohort:manage' capabillity.
        $this->add_action((new action(
            new agpu_url('/cohort/edit.php', ['id' => ':id', 'returnurl' => $returnurl]),
            new pix_icon('t/edit', '', 'core'),
            [],
            false,
            new lang_string('edit')
        ))->add_callback(function(stdClass $row): bool {
            return empty($row->component) && has_capability('agpu/cohort:manage', context::instance_by_id($row->contextid));
        }));

        // Delete action. It will be only shown if user has 'agpu/cohort:manage' capabillity.
        $this->add_action((new action(
            new agpu_url('#'),
        ...
        // Assign members to cohort action. It will be only shown if user has 'agpu/cohort:assign' capabillity.
        $this->add_action((new action(
            new agpu_url('/cohort/assign.php', ['id' => ':id', 'returnurl' => $returnurl]),
            new pix_icon('i/users', '', 'core'),
            [],
            false,
            new lang_string('assign', 'core_cohort')
        ))->add_callback(function(stdClass $row): bool {
            return empty($row->component) && has_capability('agpu/cohort:assign', context::instance_by_id($row->contextid));
        }));
    }
