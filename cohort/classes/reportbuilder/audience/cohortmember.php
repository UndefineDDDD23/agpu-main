...
class cohortmember extends base {
     ...
     *
     * @param agpuQuickForm $mform The form to add elements to
     */
    public function get_config_form(agpuQuickForm $mform): void {
    ...
    public function user_can_add(): bool {
        ...
        if (has_capability('agpu/cohort:view', context_system::instance())) {
            return true;
        }
        // If there is at least one category with given permissions, user can add.
        return !empty(core_course_category::make_categories_list('agpu/cohort:view'));
    }
    ...
    public function user_can_edit(): bool {
        ...
        foreach ($cohorts as $cohort) {
            ...
            $canedit = $canedit && has_capability('agpu/cohort:view', $context);
            if ($canedit === false) {
                break;
            }
        }
    ...
    private static function get_cohorts(): array {
        ...
        $cohorts = array_filter($usercohorts['cohorts'], static function(stdClass $cohort) use ($syscontext): bool {
            return ($cohort->contextid != $syscontext->id) || has_capability('agpu/cohort:view', $syscontext);
        });
