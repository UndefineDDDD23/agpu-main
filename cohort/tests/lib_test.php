...
class lib_test extends \advanced_testcase {
    ...
    public function test_cohort_add_cohort_event(): void {
        ...
        $url = new \agpu_url('/cohort/index.php', array('contextid' => $event->contextid));
    ...
    public function test_cohort_update_cohort_event(): void {
        ...
        $url = new \agpu_url('/cohort/edit.php', array('id' => $event->objectid));
        $this->assertEquals($url, $event->get_url());
        $this->assertEquals($cohort, $event->get_record_snapshot('cohort', $id));
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_cohort_add_member_event(): void {
        ...
        $url = new \agpu_url('/cohort/assign.php', array('id' => $event->objectid));
        $this->assertEquals($url, $event->get_url());
        $this->assertEventContextNotUsed($event);
    }
    ...
    public function test_cohort_get_all_cohorts(): void {
        ...
        role_change_permission($managerrole->id, $context2, 'agpu/cohort:view', CAP_PROHIBIT);
        role_change_permission($managerrole->id, $context2, 'agpu/cohort:manage', CAP_PROHIBIT);
        $this->assertFalse(has_any_capability(array('agpu/cohort:view', 'agpu/cohort:manage'), $context2));
