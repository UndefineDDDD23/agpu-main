 ...
 *
 * @package    core_group
 * @copyright  2017 Adrian Greeve <adrian@agpu.com>
...
class group_details implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if (!empty($this->group->description) || (!empty($this->group->picture))) {
            ...
            $descriptionformat = $this->group->descriptionformat ?? FORMAT_agpu;
            $options = [
                'overflowdiv' => true,
                'context'     => $context
            ];
            ...
            if (has_capability('agpu/course:managegroups', $context)) {
                $url = new agpu_url('/group/group.php', ['id' => $this->group->id, 'courseid' => $this->group->courseid]);
