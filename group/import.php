...
navigation_node::override_active_url(new agpu_url('/group/index.php', array('id' => $course->id)));
$PAGE->set_title("$course->shortname: $strimportgroups");
$PAGE->set_heading($course->fullname);
$PAGE->set_pagelayout('admin');

$returnurl = new agpu_url('/group/index.php', array('id'=>$id));
...
if ($importform->is_cancelled()) {
    ...
    if ($readcount === false) {
        throw new \agpu_exception('csvfileerror', 'error', $PAGE->url, $csvimport->get_error());
    } else if ($readcount == 0) {
        throw new \agpu_exception('csvemptyfile', 'error', $PAGE->url, $csvimport->get_error());
    } else if ($readcount == 1) {
        throw new \agpu_exception('csvnodata', 'error', $PAGE->url);
    }
    ...
    foreach ($header as $i => $h) {
        ...
        if (!isset($required[$h]) && !isset($optionaldefaults[$h]) && !isset($optional[$h]) && !isset($customfieldnames[$h])) {
            throw new \agpu_exception('invalidfieldname', 'error', $PAGE->url, $h);
        }
        if (isset($required[$h])) {
            $required[$h] = 2;
        }
    ...
    foreach ($required as $key => $value) {
        if ($value < 2) {
            throw new \agpu_exception('fieldrequired', 'error', $PAGE->url, $key);
        }
    }
        ...
        if (trim(implode($line)) !== '') {
            ...
            foreach ($record as $name => $value) {
                ...
                if (isset($required[$name]) and !$value) {
                    throw new \agpu_exception('missingfield', 'error', $PAGE->url, $name);
            ...
            if (isset($newgroup->courseid)) {
                ...
                if (!has_capability('agpu/course:managegroups', $newgrpcoursecontext) or (!is_enrolled($newgrpcoursecontext) and !has_capability('agpu/course:view', $newgrpcoursecontext))) {
                    ...
                    if (isset($newgroup->groupidnumber)) {
                        ...
                        if (has_capability('agpu/course:changeidnumber', $newgrpcoursecontext)) {
