...
$url = new agpu_url('/group/overview.php', array('id'=>$courseid));
if ($groupid !== 0) {
...
$popupurl = new agpu_url($rooturl.'&group='.$groupid);
...
$popupurl = new agpu_url($rooturl.'&grouping='.$groupingid);
...
foreach ($members as $gpgid=>$groupdata) {
    ...
    foreach ($groupdata as $gpid=>$users) {
        ...
        foreach ($users as $user) {
            ...
            $fullnames[] = html_writer::link(new agpu_url('/user/view.php', ['id' => $user->id, 'course' => $course->id]),
                $displayname);
        }
