...
$url = new agpu_url('/group/grouping.php');
if ($id) {
    $url->param('id', $id);
...
if ($id and $delete) {
    if (!empty($grouping->idnumber) && !has_capability('agpu/course:changeidnumber', $context)) {
        throw new \agpu_exception('groupinghasidnumber', '', '', $grouping->name);
    }
    if (!$confirm) {
        ...
        $formcontinue = new single_button(new agpu_url('grouping.php', $optionsyes), get_string('yes'), 'get');
        $formcancel = new single_button(new agpu_url('groupings.php', $optionsno), get_string('no'), 'get');
        ...
        if (groups_delete_grouping($id)) {
            ...
            throw new \agpu_exception('erroreditgrouping', 'group', $returnurl);
        }
    }
}
...
if ($editform->is_cancelled()) {
    ...
    $success = true;
    if (!has_capability('agpu/course:changeidnumber', $context)) {
...
$PAGE->navbar->add($strparticipants, new agpu_url('/user/index.php', array('id'=>$courseid)));
$PAGE->navbar->add($strgroupings, new agpu_url('/group/groupings.php', array('id'=>$courseid)));
