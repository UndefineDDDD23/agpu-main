...
$changeidnumber = has_capability('agpu/course:changeidnumber', $context);
...
foreach($groupidarray as $groupid) {
    ...
    if (!empty($group->idnumber) && !$changeidnumber) {
        throw new \agpu_exception('grouphasidnumber', '', '', $group->name);
    }
    if ($courseid != $group->courseid) {
        throw new \agpu_exception('groupunknown', '', '', $group->courseid);
    }
...
if ($confirm && data_submitted()) {
    if (!confirm_sesskey() ) {
        throw new \agpu_exception('confirmsesskeybad', 'error', $returnurl);
    }
    ...
    $formcontinue = new single_button(new agpu_url('delete.php', $optionsyes), get_string('yes'), 'post');
    $formcancel = new single_button(new agpu_url('index.php', $optionsno), get_string('no'), 'get');
