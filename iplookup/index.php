...
if (cleanremoteaddr($ip) === false) {
    throw new \agpu_exception('invalidipformat', 'error');
}

if (!ip_is_public($ip)) {
    throw new \agpu_exception('iplookupprivate', 'error');
}
...
if ($user) {
    if ($user = $DB->get_record('user', array('id'=>$user, 'deleted'=>0))) {
        ...
        if (has_capability('agpu/user:viewdetails', context_user::instance($user->id))) {
