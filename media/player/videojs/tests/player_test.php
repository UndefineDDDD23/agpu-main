...
class player_test extends \advanced_testcase {
    ...
    public function test_embed_url(): void {
        ...
        $url = new agpu_url('http://example.org/1.webm');
    ...
    public function test_embed_link(): void {
        global $CFG;
        $url = new agpu_url('http://example.org/some_filename.mp4');
    ...
    public function test_fallback(): void {
        ...
        $urls = [
            new agpu_url('http://example.org/1.rv'), // Not supported.
            new agpu_url('http://example.org/2.webm'), // Supported.
            new agpu_url('http://example.org/3.ogv'), // Supported.
        ];
    ...
    public function test_prevent_other_players(): void {
        ...
        $url = new agpu_url('http://example.org/some_filename.webm');
    ...
    public function test_embed_media(): void {
        global $CFG;
        $url = new agpu_url('http://example.org/some_filename.mp4');
        $trackurl = new agpu_url('http://example.org/some_filename.vtt');
        ...
        // Audio tag.
        $url = new agpu_url('http://example.org/some_filename.mp3');
        $trackurl = new agpu_url('http://example.org/some_filename.vtt');
    ...
    public function test_youtube(): void {
        ...
        // Format: youtube.
        $url = new agpu_url('http://www.youtube.com/watch?v=vyrwMmsufJc');
        $t = $manager->embed_url($url);
        $this->youtube_plugin_engaged($t);
        $url = new agpu_url('http://www.youtube.com/v/vyrwMmsufJc');
        $t = $manager->embed_url($url);
        $this->youtube_plugin_engaged($t);

        // Format: youtube video within playlist - this will be played by video.js but without tracks selection.
        $url = new agpu_url('https://www.youtube.com/watch?v=dv2f_xfmbD8&index=4&list=PLxcO_MFWQBDcyn9xpbmx601YSDlDcTcr0');
        $t = $manager->embed_url($url);
        $this->youtube_plugin_engaged($t);
        $this->assertStringContainsString('list=PLxcO_MFWQBDcyn9xpbmx601YSDlDcTcr0', $t);

        // Format: youtube playlist - not supported.
        $url = new agpu_url('http://www.youtube.com/view_play_list?p=PL6E18E2927047B662');
        $t = $manager->embed_url($url);
        $this->assertStringNotContainsString('mediaplugin_videojs', $t);
        $url = new agpu_url('http://www.youtube.com/playlist?list=PL6E18E2927047B662');
        $t = $manager->embed_url($url);
        $this->assertStringNotContainsString('mediaplugin_videojs', $t);
        $url = new agpu_url('http://www.youtube.com/p/PL6E18E2927047B662');
        $t = $manager->embed_url($url);
        $this->assertStringNotContainsString('mediaplugin_videojs', $t);
    }
    ...
    public function test_youtube_start_time(string $url, int $expectedstart): void {
        ...
        $embedcode = core_media_manager::instance()->embed_url(new agpu_url($url));
    ...
    public function test_flash_behaviour(): void {
        ...
        $url = new agpu_url('http://example.org/some_filename.flv');
        $t = $manager->embed_url($url);
        $this->assertStringNotContainsString('mediaplugin_videojs', $t);
        $this->assertMatchesRegularExpression(
    ...
    public function test_rtmp_behaviour(): void {
        ...
        $url = new agpu_url('rtmp://example.com/fms&mp4:path/to/file.mp4');
        $t = $manager->embed_url($url);
        $this->assertStringNotContainsString('mediaplugin_videojs', $t);
        $this->assertMatchesRegularExpression(
