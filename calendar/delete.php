...
/**
 * This file is part of the Calendar section agpu
...
if (!calendar_delete_event_allowed($event)) {
    throw new \agpu_exception('nopermissions', 'error', $PAGE->url, $title);
}
...
$viewcalendarurl = new agpu_url(CALENDAR_URL.'view.php', array('view'=>'upcoming'));
...
if ($confirm) {
    ...
    if (!confirm_sesskey()) {
        throw new \agpu_exception('confirmsesskeybad');
    }
...
$url = new agpu_url(CALENDAR_URL.'delete.php', array('id'=>$event->id, 'confirm'=>true));
...
if (!empty($event->eventrepeats) && $event->eventrepeats > 0) {
    $url = new agpu_url(CALENDAR_URL.'delete.php', array('id'=>$event->repeatid, 'confirm'=>true, 'repeats'=>true));
