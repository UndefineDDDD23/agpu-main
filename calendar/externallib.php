...
class core_calendar_external extends external_api {
    ...
    public static function delete_calendar_events($events) {
        ...
        foreach ($params['events'] as $event) {
            ...
            if (!calendar_delete_event_allowed($eventobj)) {
                throw new agpu_exception('nopermissions', 'error', '', get_string('deleteevent', 'calendar'));
            }
    ...
    public static function get_calendar_events($events = array(), $options = array()) {
        ...
        $hassystemcap = has_capability('agpu/calendar:manageentries', context_system::instance());
        $warnings = array();
            ...
            if (!empty($params['events']['categoryids'])) {
                ...
                foreach ($catobjs as $catobj) {
                    if (isset($coursecategories[$catobj->id]) ||
                            has_capability('agpu/category:manage', $catobj->get_context())) {
    ...
    public static function get_calendar_action_events_by_timesort($timesortfrom = 0, $timesortto = null,
        ...
        if ($params['userid'] && $USER->id !== $params['userid'] && !has_capability('agpu/calendar:manageentries', $context)) {
            throw new \required_capability_exception($context, 'agpu/calendar:manageentries', 'nopermission', '');
        }
     ...
     * @since agpu 2.5
     * @throws agpu_exception if user doesnt have the permission to create events.
     */
    ...
    public static function get_calendar_event_by_id($eventid) {
        ...
        if ($event = $eventvault->get_event_by_id($params['eventid'])) {
            ...
            if (!calendar_view_event_allowed($mapper->from_event_to_legacy_event($event))) {
                throw new agpu_exception('nopermissiontoviewcalendar', 'error');
            }
        }
        ...
        if (!$event) {
            ...
            throw new \required_capability_exception($syscontext, 'agpu/course:view', 'nopermissions', 'error');
        }
    ...
    public static function update_event_start_day($eventid, $daytimestamp) {
        ...
        if (!$event) {
            throw new \agpu_exception('Unable to find event with id ' . $eventid);
        }
    ...
    public static function get_calendar_access_information($courseid = 0) {
        ...
        return [
            'canmanageentries' => has_capability('agpu/calendar:manageentries', $context),
            'canmanageownentries' => has_capability('agpu/calendar:manageownentries', $context),
            'canmanagegroupentries' => has_capability('agpu/calendar:managegroupentries', $context),
            'warnings' => [],
        ];
    }
     ...
     * @throws agpu_exception If one or more of the dates provided does not convert to a valid timestamp.
     */
    public static function get_timestamps($datetimes) {
        ...
        foreach ($params['data'] as $key => $datetime) {
            ...
            try {
                ...
                throw new agpu_exception('One or more of the dates provided were invalid');
            }
        }
