...
/**
 * agpu calendar import
 *
 * @package    core_calendar
 * @copyright  agpu Pty Ltd
...
$data = [];
$pageurl = new agpu_url('/calendar/import.php');
...
if (!empty($courseid) && $courseid != SITEID) {
    ...
    navigation_node::override_active_url(new agpu_url('/course/index.php', ['categoryid' => $category]));
    $PAGE->set_category_by_id($category);
    $PAGE->navbar->add(
        $calendarstr,
        new agpu_url('/calendar/view.php', ['view' => 'month', 'category' => $category])
    );
} else {
    $course = get_site();
    $PAGE->navbar->add($calendarstr, new agpu_url('/calendar/view.php', ['view' => 'month']));
}
...
if (!calendar_user_can_add_event($course)) {
    throw new \agpu_exception('errorcannotimport', 'calendar');
}
...
if (!empty($formdata)) {
    ...
    if ($formdata->importfrom == CALENDAR_IMPORT_FROM_FILE) {
        ...
        try {
            ...
            throw new \agpu_exception($e->errorcode, $e->module, $PAGE->url);
        }
    }
