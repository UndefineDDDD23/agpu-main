...
$url = new agpu_url('/calendar/managesubscriptions.php');
if ($courseid != SITEID && !empty($courseid)) {
    $url->param('course', $courseid);
    navigation_node::override_active_url(new agpu_url('/course/view.php', ['id' => $courseid]));
    $PAGE->navbar->add(
        get_string('calendar', 'calendar'),
        new agpu_url('/calendar/view.php', ['view' => 'month', 'course' => $courseid])
    );
} else if ($categoryid) {
...
if ($usedefaultfilters) {
    ...
    if (!empty($types['category'])) {
        list($categoryinsql, $categoryparams) = $DB->get_in_or_equal(
                array_keys(\core_course_category::make_categories_list('agpu/category:manage')), SQL_PARAMS_NAMED, 'category');
