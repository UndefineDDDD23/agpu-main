...
/**
 * This file contains the renderers for the calendar within agpu
 *
...
class core_calendar_renderer extends plugin_renderer_base {
    ...
    public function event(calendar_event $event, $showactions=true) {
        ...
        if (calendar_edit_event_allowed($event) && $showactions) {
            if (calendar_delete_event_allowed($event)) {
                $editlink = new agpu_url(CALENDAR_URL.'event.php', array('action' => 'edit', 'id' => $event->id));
                $deletelink = new agpu_url(CALENDAR_URL.'delete.php', array('id' => $event->id));
                ...
                $editlink = new agpu_url('/course/mod.php', $params);
                $deletelink = null;
            }
        ...
        if (!empty($event->actionurl)) {
            $actionlink = html_writer::link(new agpu_url($event->actionurl), $event->actionname);
     ...
     *
     * @param agpu_url $returnurl The URL that the user should be taken too upon selecting a course.
     ...
     * @return string
     */
    public function course_filter_selector(agpu_url $returnurl, $label = null, $courseid = null, ?int $calendarinstanceid = null) {
        global $CFG, $DB;
        ...
        $courseurl = new agpu_url($returnurl);
    ...
    public function render_subscriptions_header(): string {
        $importcalendarbutton = new single_button(new agpu_url('/calendar/import.php', calendar_get_export_import_link_params()),
                get_string('importcalendar', 'calendar'), 'get', single_button::BUTTON_PRIMARY);
        $importcalendarbutton->class .= ' float-sm-end float-end';
        $exportcalendarbutton = new single_button(new agpu_url('/calendar/export.php', calendar_get_export_import_link_params()),
    ...
    public function render_no_calendar_subscriptions(): string {
        ...
        $importlink = (new agpu_url('/calendar/import.php', calendar_get_export_import_link_params()))->out();
