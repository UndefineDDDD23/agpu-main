...
Feature: Export calendar events
  In order to be able to use my calendar events outside of agpu
  As a user
  ...
  Scenario: Export calendar in ics format
    ...
    # We need to split the step in two because Bennu library use days with leading zero and agpu removes it.
