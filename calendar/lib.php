 ...
 * It was introduced as part of agpu 2.0 and was created in order to provide a
...
class calendar_event {
     ...
     * @param bool $checkcapability If agpu should check the user can manage the calendar events for this call or not.
        ...
        if (empty($this->properties->id) || $this->properties->id < 1) {
            if ($checkcapability) {
                if (!calendar_add_event_allowed($this->properties)) {
                    throw new \agpu_exception('nopermissiontoupdatecalendar');
                }
            }
        ...
        if ($prepareeditor) {
            ...
            if (!empty($properties->id)) {
                ...
                if ($properties->eventtype === 'site') {
                    ...
                    if (!$course) {
                        throw new \agpu_exception('invalidcourse');
                    }
                    // Course context.
                    $this->editorcontext = $this->get_context();
...
class calendar_information {
    ...
    public static function create($time, int $courseid, ?int $categoryid = null): calendar_information {
        ...
        if ($courseid != SITEID && !empty($courseid)) {
            ...
            if (!$course->visible && !is_role_switched($course->id)) {
                require_capability('agpu/course:viewhiddencourses', $calendar->context);
            }
            ...
            if ($this->categories === false) {
                ...
                foreach (\core_course_category::get_all() as $category) {
                    if (isset($coursecategories[$category->id]) ||
                            has_capability('agpu/category:manage', $category->get_context(), $USER, false)) {
     ...
     * or throws a agpu_exception if not
     *
    ...
    public function checkdate($defaultonow = true) {
        if (!checkdate($this->month, $this->day, $this->year)) {
            if ($defaultonow) {
                ...
                throw new agpu_exception('invaliddate');
            }
        }
        return true;
    }
    ...
    public function add_sidecalendar_blocks(core_calendar_renderer $renderer, $showfilters=false, $view=null) {
        ...
        if (!has_capability('agpu/block:view', $PAGE->context) ) {
            return;
        }
 ...
 * @throws agpu_exception for an invalid id
 */
    ...
    switch ($type) {
        case 'frontpage':
            ...
            $nextlink = calendar_get_link_next(get_string('monthnext', 'calendar'), $urlbase, false, false, false, true,
                $nextmonthtime);
            $calendarlink = calendar_get_link_href(new \agpu_url(CALENDAR_URL . 'view.php', array('view' => 'month')),
        ...
        case 'course':
            ...
            break;
        case 'upcoming':
            $calendarlink = calendar_get_link_href(new \agpu_url(CALENDAR_URL . 'view.php', array('view' => 'upcoming')),
                false, false, false, $time);
            if (!empty($data['id'])) {
                $calendarlink->param('course', $data['id']);
            }
...
/**
 * Adds day, month, year arguments to a URL and returns a agpu_url object.
 *
 * @param string|agpu_url $linkbase
...
function calendar_get_link_href($linkbase, $d, $m, $y, $time = 0) {
    ...
    if (!($linkbase instanceof \agpu_url)) {
        $linkbase = new \agpu_url($linkbase);
    }
 ...
 * @param string|agpu_url $linkbase The URL stub.
...
function calendar_get_link_previous($text, $linkbase, $d, $m, $y, $accesshide = false, $time = 0) {
    ...
    $href = calendar_get_link_href(new \agpu_url($linkbase), $d, $m, $y, $time);
...
function calendar_set_filters(array $courseeventsfrom, $ignorefilters = false, ?stdClass $user = null) {
    ...
    $allgroupscaps = array('agpu/site:accessallgroups', 'agpu/calendar:manageentries');
...
function calendar_can_manage_non_user_event_in_system($event) {
    ...
    $canmanageentries = has_capability('agpu/calendar:manageentries', $sitecontext);
...
function calendar_view_event_allowed(calendar_event $event) {
    ...
    if (!empty($event->groupid)) {
        ...
        if (has_capability('agpu/calendar:manageentries', $event->context) ||
                has_capability('agpu/calendar:managegroupentries', $event->context)) {
            return true;
        }
        ...
        throw new agpu_exception('unknown event type');
    }
...
function calendar_edit_event_allowed($event, $manualedit = false) {
    ...
    if ($manualedit && !empty($event->modulename)) {
        ...
        return has_capability('agpu/course:manageactivities', $context);
    }
    ...
    if (!empty($event->groupid)) {
        ...
        return $group && (
                has_capability('agpu/calendar:manageentries', $event->context) ||
                (has_capability('agpu/calendar:managegroupentries', $event->context)
                    && groups_is_member($event->groupid)));
    } else if (!empty($event->courseid)) {
        // If groupid is not set, but course is set, it's definitely a course event.
        return has_capability('agpu/calendar:manageentries', $event->context);
        ...
        return (has_capability('agpu/calendar:manageownentries',
...
function calendar_can_manage_user_event($event): bool {
    ...
    $canmanage = has_capability('agpu/calendar:manageentries', $event->context);
    $canmanageown = has_capability('agpu/calendar:manageownentries', $event->context);
...
function calendar_get_default_courses($courseid = null, $fields = '*', $canmanage = false, ?int $userid = null) {
    ...
    if ((!empty($CFG->calendar_adminseesall) || $canmanage) &&
            has_capability('agpu/calendar:manageentries', context_system::instance(), $userid)) {
    ...
    if ($courseid && $courseid != SITEID) {
        if (empty($courses[$courseid]) && has_capability('agpu/calendar:manageentries', context_system::instance(), $userid)) {
    ...
    if ($event->timeduration) {
        ...
        if ($usermidnightstart == $usermidnightend) {
            ...
            if (!$showtime) {
                ...
                $url = calendar_get_link_href(new \agpu_url(CALENDAR_URL . 'view.php', $linkparams), 0, 0, 0, $endtime);
            ...
            if ($now >= $usermidnightstart && $now < strtotime('+1 day', $usermidnightstart)) {
                ...
                $url = calendar_get_link_href(new \agpu_url(CALENDAR_URL . 'view.php', $linkparams), 0, 0, 0, $starttime);
 ...
 * @param stdClass|int $user agpu user object or id, null means current user
 */
...
function calendar_get_allowed_types(&$allowed, $course = null, $groups = null, $category = null) {
    ...
    $allowed->user = has_capability('agpu/calendar:manageownentries', \context_system::instance());
    $allowed->groups = false;
    $allowed->courses = false;
    $allowed->categories = false;
    $allowed->site = has_capability('agpu/calendar:manageentries', \context_course::instance(SITEID));
    ...
    if (!empty($course)) {
        ...
        if ($course->id != SITEID) {
            ...
            $allowed->user = has_capability('agpu/calendar:manageownentries', $coursecontext);

            if (has_capability('agpu/calendar:manageentries', $coursecontext)) {
                $allowed->courses = array($course->id => 1);
                $allowed->groups = $getgroupsfunc($course, $coursecontext, $USER);
            } else if (has_capability('agpu/calendar:managegroupentries', $coursecontext)) {
                $allowed->groups = $getgroupsfunc($course, $coursecontext, $USER);
            }
        }
    }
...
function calendar_add_event_allowed($event) {
    ...
    switch ($event->eventtype) {
        case 'category':
            return has_capability('agpu/category:manage', $event->context);
        case 'course':
            return has_capability('agpu/calendar:manageentries', $event->context);
        case 'group':
            // Allow users to add/edit group events if -
...
function calendar_add_subscription($sub) {
    ...
    if (!empty($sub->name)) {
        ...
        throw new \agpu_exception('errorbadsubscription', 'importcalendar');
    }
}

/**
 * Add an iCalendar event to the agpu calendar.
 *
...
function calendar_add_icalendar_event($event, $unused, $subscriptionid, $timezone='UTC') {
    ...
    if ($updaterecord = $DB->get_record('event', array('uuid' => $eventrecord->uuid,
        ...
        // Compare iCal event data against the agpu event to see if something has changed.
...
function calendar_get_icalendar($url) {
    ...
    if (!$calendar || $curl->info['http_code'] != 200 || !empty($curl->errorno)) {
        throw new \agpu_exception('errorinvalidicalurl', 'calendar');
    }
...
function calendar_get_allowed_event_types(?int $courseid = null) {
    ...
    if (!empty($courseid) && $courseid != SITEID) {
        ...
        $types['user'] = has_capability('agpu/calendar:manageownentries', $context);
        calendar_internal_update_course_and_group_permission($courseid, $context, $types);
    }

    if (has_capability('agpu/calendar:manageentries', \context_course::instance(SITEID))) {
        $types['site'] = true;
    }

    if (has_capability('agpu/calendar:manageownentries', \context_system::instance())) {
        $types['user'] = true;
    }
    ...
    if ($types['course'] == false || $types['group'] == false) {
        if ($CFG->calendar_adminseesall && has_capability('agpu/calendar:manageentries', context_system::instance())) {
            ...
            foreach ($courseswithgroups as $course) {
                ...
                if (has_capability('agpu/calendar:manageentries', $context)) {
                    if (has_any_capability(['agpu/site:accessallgroups', 'agpu/calendar:managegroupentries'], $context)) {
...
function calendar_internal_update_course_and_group_permission(int $courseid, context $context, array &$types) {
    ...
    if (!$types['group'] && (has_capability('agpu/calendar:manageentries', $context) ||
            has_capability('agpu/calendar:managegroupentries', $context))) {
