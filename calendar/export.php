...
/**
 * This file is part of the User section agpu
 *
 * @copyright 2003-2004 Jon Papaioannou (pj@agpu.org)
...
$url = new agpu_url('/calendar/export.php', array('time' => $time));
$managesubscriptionsurl = new agpu_url('/calendar/managesubscriptions.php');
...
if ($courseid != SITEID && !empty($courseid)) {
    ...
    navigation_node::override_active_url(new agpu_url('/course/view.php', ['id' => $course->id]));
    $PAGE->navbar->add(
        get_string('calendar', 'calendar'),
        new agpu_url('/calendar/view.php', ['view' => 'month', 'course' => $course->id])
    );
} else {
    $course = get_site();
    ...
    if (!empty($categoryid)) {
        ...
        navigation_node::override_active_url(new agpu_url('/course/index.php', ['categoryid' => $categoryid]));
        $PAGE->set_category_by_id($categoryid);
        $PAGE->navbar->add(
            get_string('calendar', 'calendar'),
            new agpu_url('/calendar/view.php', ['view' => 'month', 'category' => $categoryid])
        );
    } else {
        $PAGE->navbar->add(get_string('calendar', 'calendar'), new agpu_url('/calendar/view.php', ['view' => 'month']));
    }
}
...
if ($data = $exportform->get_data()) {
    ...
    $link = new agpu_url('/calendar/export_execute.php', $params);
