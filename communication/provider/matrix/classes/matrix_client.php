 ...
 *
 * Matrix uses a versioned API, and a handshake occurs between the Client (agpu) and server, to determine the APIs available.
 *
...
abstract class matrix_client {
    ...
    public static function instance(
        ...
        if (count($versions) === 0) {
            ...
            throw new \agpu_exception('No supported Matrix API versions found.');
        }
        asort($versions);
    ...
    public function require_feature(array|string $feature): void {
        if (!$this->implements_feature($feature)) {
            ...
            throw new \agpu_exception("The requested feature is not implemented in this Matrix Client: '{$feature}'");
        }
    }
    ...
    public function requires_version(string $minversion): void {
        ...
        throw new \agpu_exception("Matrix API version {$minversion} or higher is required for this command.");
    }
}
