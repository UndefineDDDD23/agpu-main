...
class communication_feature implements
     ...
     * This does not involve a remote call, but checks whether agpu is aware of the room id.
     * @return bool
     */
     ...
     * Create members.
     *
     * @param array $userids The agpu user ids to create
     */
        ...
        foreach ($userids as $userid) {
            ...
            if (!empty($user->email) && !empty($userfullname)) {
                ...
                if (!empty($matrixuserid = $body->name)) {
                    // Then create matrix user id in agpu.
                    matrix_user_manager::set_matrix_userid_in_agpu($userid, $qualifiedmuid);
    ...
    public function update_room_membership(array $userids): void {
        ...
        if (isset($body->joined)) {
            foreach ($userids as $key => $userid) {
                $matrixuserid = matrix_user_manager::get_matrixid_from_agpu(
                    userid: $userid,
                );
    ...
    public function add_members_to_room(array $userids): void {
        ...
        foreach ($userids as $userid) {
            $matrixuserid = matrix_user_manager::get_matrixid_from_agpu($userid);
     ...
     *
     * @param array $userids The agpu user ids to remove
     */
    ...
    public static function set_form_definition(\agpuQuickForm $mform): void {
     ...
     *
     * Users with a non-agpu power level are not typically removed unless specified in the $forceremoval param.
    ...
    private function set_matrix_power_levels(
        ...
        $userlist = array_combine(
            array_map(
                fn ($userid) => matrix_user_manager::get_matrixid_from_agpu($userid),
                $userlist,
            ),
            $userlist,
        );
        ...
        if (!empty($forceremoval)) {
            ...
            foreach ($forceremoval as $userid) {
                $muid = matrix_user_manager::get_matrixid_from_agpu($userid);
    ...
    /**
     * Filter the list of users provided to remove those with a agpu-related power level.
     *
     * @param array $users
     * @return array
     */
        ...
        return array_filter(
            ...
            function ($level): bool {
                switch ($level) {
                    ...
                    case matrix_constants::POWER_LEVEL_agpu_SITE_ADMIN:
                    case matrix_constants::POWER_LEVEL_agpu_MODERATOR:
                        return false;
                    default:
                        return true;
                }
    ...
    public function get_user_allowed_power_level(int $userid): int {
        ...
        if (has_capability('communication/matrix:moderator', $this->processor->get_context(), $userid)) {
            $powerlevel = matrix_constants::POWER_LEVEL_agpu_MODERATOR;
        }

        // If site admin, override all caps.
        if (is_siteadmin($userid)) {
            $powerlevel = matrix_constants::POWER_LEVEL_agpu_SITE_ADMIN;
        }
