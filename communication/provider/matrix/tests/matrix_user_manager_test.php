 ...
 * @category   test
 * @copyright  2023 Stevani Andolo <stevani.andolo@agpu.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @covers \communication_matrix\matrix_user_manager
 */
class matrix_user_manager_test extends \advanced_testcase {
    /**
     * Test fetcihing a users matrix userid from agpu.
     */
    public function test_get_matrixid_from_agpu_without_field(): void {
        $user = get_admin();

        $this->assertNull(matrix_user_manager::get_matrixid_from_agpu($user->id));
    }

    /**
     * Test fetching a user's matrix userid from agpu.
     */
    public function test_get_matrixid_from_agpu(): void {
        $this->resetAfterTest();
        ...
        matrix_user_manager::set_matrix_userid_in_agpu(
            $user1->id,
            '@someexampleuser:matrix.agpu.org',
        );

        matrix_user_manager::set_matrix_userid_in_agpu(
            $user2->id,
            '@someotherexampleuser:matrix.agpu.org',
        );

        // And confirm that they're fetched back.
        $this->assertEquals(
            '@someexampleuser:matrix.agpu.org',
            matrix_user_manager::get_matrixid_from_agpu($user1->id),
        );
        $this->assertEquals(
            '@someotherexampleuser:matrix.agpu.org',
            matrix_user_manager::get_matrixid_from_agpu($user2->id),
        );
    }

    /**
     * Test fetching a formatted matrix userid from agpu when no server is set.
     */
     ...
     * @param string $server
     * @param string $username The agpu username to turn into a Matrix username
    ...
    /**
     * Data provider for set_matrix_userid_in_agpu.
     *
     * @return array
     */
    public static function set_matrix_userid_in_agpu_provider(): array {
    ...
    /**
     * Test setting of a user's matrix userid in agpu.
     *
     * @dataProvider set_matrix_userid_in_agpu_provider
     * @param string $expectedusername
     */
    public function test_set_matrix_userid_in_agpu(
        string $expectedusername,
    ): void {
        $this->resetAfterTest();

        $user = $this->getDataGenerator()->create_user();
        matrix_user_manager::set_matrix_userid_in_agpu($user->id, $expectedusername);

        // Get created matrixuserid from agpu.
        $this->assertEquals(
            $expectedusername,
            matrix_user_manager::get_matrixid_from_agpu($user->id),
        );
    }
