...
class manage_communication_providers_page extends admin_setting {
    ...
    public function output_html($data, $query = ''): string {
        ...
        foreach ($plugins as $plugin) {
            $class = '';
            $actionurl = new agpu_url('/admin/communication.php', ['sesskey' => sesskey(), 'name' => $plugin->name]);
            if (
            ...
            if ($plugin->is_enabled()) {
                $hideshow = html_writer::link(
                    ...
                    $OUTPUT->pix_icon('t/hide', get_string('disable'), 'agpu', ['class' => 'iconsmall'])
                );
            } else {
                $class = 'dimmed_text';
                $hideshow = html_writer::link(
                    $actionurl->out(false, ['action' => 'enable']),
                    $OUTPUT->pix_icon('t/show', get_string('enable'), 'agpu', ['class' => 'iconsmall'])
                );
            }
