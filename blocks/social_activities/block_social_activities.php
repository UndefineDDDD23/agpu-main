...
class block_social_activities extends block_list {
    ...
    function get_content() {
        ...
        $isediting = $this->page->user_is_editing() && has_capability('agpu/course:manageactivities', $context);
        ...
        if ($ismoving) {
            ...
            $cancelurl = new agpu_url('/course/mod.php', array('cancelcopy' => 'true', 'sesskey' => sesskey()));
            ...
            foreach ($modinfo->sections[0] as $modnumber) {
                ...
                if (!$ismoving) {
                    ...
                    $moveaction = html_writer::link(
                        new agpu_url('/course/mod.php', ['sesskey' => sesskey(), 'copy' => $mod->id]),
                ...
                if ($mod->visible || has_capability('agpu/course:viewhiddenactivities', $mod->context)) {
                    if ($ismoving) {
                        if ($mod->id == $USER->activitycopy) {
                            continue;
                        }
                        $movingurl = new agpu_url('/course/mod.php', array('moveto' => $mod->id, 'sesskey' => sesskey()));
        ...
        if ($ismoving) {
            $movingurl = new agpu_url('/course/mod.php', array('movetosection' => $section->id, 'sesskey' => sesskey()));
