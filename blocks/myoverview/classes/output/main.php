 ...
 * Class containing data for my overview block.
 *
 * @copyright  2018 Bas Brands <bas@agpu.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class main implements renderable, templatable {
    ...
    public function export_for_template(renderer_base $output) {
        ...
        if ($coursecat && ($category = \core_course_category::get_nearest_editable_subcategory($coursecat, ['create']))) {
            $newcourseurl = new \agpu_url('/course/edit.php', ['category' => $category->id]);
        }
    ...
    public function export_for_zero_state_template(renderer_base $output) {
        ...
        if ($coursecat) {
            ...
            $category = \core_course_category::get_nearest_editable_subcategory($coursecat, ['agpu/course:request']);
            if ($category && $category->can_request_course()) {
                $requestbutton = new \single_button(
                    new \agpu_url('/course/request.php', ['category' => $category->id]),
            ...
            if ($coursecat) {
                ...
                if ($categorytomanage = \core_course_category::get_nearest_editable_subcategory($coursecat, ['manage'])) {
                    $managebutton = new \single_button(
                        new \agpu_url('/course/management.php', ['category' => $categorytomanage->id]),
                        $managebuttonname,
                    );
            ...
            if ($category = \core_course_category::get_nearest_editable_subcategory($coursecat, ['create'])) {
                $createbutton = new \single_button(
                    new \agpu_url('/course/edit.php', ['category' => $category->id]),
     ...
     *
     * @param \agpu_url $imageurl The URL to the image to show
     ...
     * @return array Context variables for the template
     */
    private function generate_zero_state_data(\agpu_url $imageurl, array $buttons, array $strings) {
        global $CFG;
        // Documentation data.
        $dochref = new \agpu_url($CFG->docroot, ['lang' => current_language()]);
        $docparams = [
            'dochref' => $dochref->out(),
        ...
        if ($CFG->coursecreationguide) {
            ...
            $quickstart = new \agpu_url($CFG->coursecreationguide, ['lang' => current_language()]);
            $docparams = [
