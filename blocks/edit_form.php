...
class block_edit_form extends \core_form\dynamic_form {
     ...
     */
    protected function get_page(): agpu_page {
        if (!$this->_page && !empty($this->_customdata['page'])) {
            ...
            if (!$pagehash = $this->optional_param('pagehash', '', PARAM_ALPHANUMEXT)) {
                throw new \agpu_exception('missingparam', '', '', 'pagehash');
            }
            $this->_page = agpu_page::retrieve_edited_page($pagehash, MUST_EXIST);
     ...
     * @param \agpuQuickForm $mform the form being built.
     */
     ...
     * @return agpu_url
     */
    protected function get_page_url_for_dynamic_submission(): agpu_url {
