...
$returnurl = new agpu_url('/course/view.php', array('id' => $id));

// Don't display if completion isn't enabled.
if (!$info->is_enabled()) {
    throw new \agpu_exception('completionnotenabled', 'completion', $returnurl);
}

// Check this user is enroled.
if (!$info->is_tracked_user($user->id)) {
    if ($USER->id == $user->id) {
        throw new \agpu_exception('notenroled', 'completion', $returnurl);
    } else {
        throw new \agpu_exception('usernotenroled', 'completion', $returnurl);
    }
}
