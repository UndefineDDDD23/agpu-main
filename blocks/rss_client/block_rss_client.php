 ...
 */
 ...
 class block_rss_client extends block_base {
    ...
    protected function get_footer($feedrecords) {
        ...
        if ($this->hasfailedfeeds) {
            if (has_any_capability(['block/rss_client:manageownfeeds', 'block/rss_client:manageanyfeeds'], $this->context)) {
                ...
                $manageurl = new agpu_url('/blocks/rss_client/managefeeds.php',
    ...
    function get_content() {
        ...
        if (has_any_capability(['block/rss_client:manageanyfeeds', 'block/rss_client:manageownfeeds'], $this->context)) {
            $managefeedfooterlink = html_writer::link(
                new agpu_url('/blocks/rss_client/managefeeds.php', ['courseid' => $this->page->course->id]),
    ...
    public function get_feed($feedrecord, $maxentries, $showtitle) {
        ...
        $simplepiefeed = new agpu_simplepie($feedrecord->url);
        ...
        if(empty($feedrecord->preferredtitle)){
            ...
            // agpu custom title does not does escape HTML entities.
            ...
            foreach ($simplepieitems as $simplepieitem) {
                try {
                    $item = new \block_rss_client\output\item(
                        ...
                        new agpu_url($simplepieitem->get_link()),
                        $simplepieitem->get_title(),
                        $simplepieitem->get_description(),
                        new agpu_url($simplepieitem->get_permalink()),
                    ...
                    // want to crash the page. Specifically, agpu_url can
        ...
        if ($imageurl = $simplepiefeed->get_image_url()) {
            try {
                $image = new \block_rss_client\output\channel_image(
                    new agpu_url($imageurl),
                    $simplepiefeed->get_image_title(),
                    new agpu_url($simplepiefeed->get_image_link())
                );

                $feed->set_image($image);
            } catch (agpu_exception $e) {
                // If there is an error with the RSS image, we don'twant to
                // crash the page. Specifically, agpu_url can throw an
        ...
        if ($this->config->block_rss_client_show_channel_link) {
            ...
            $feed->set_channellink($channellink ? new agpu_url($channellink) : null);
        }
