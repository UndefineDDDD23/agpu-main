...
class block_activity_results extends block_base {
                ...
                if (!empty($best)) {
                    ...
                    foreach ($best as $userid => $gradeid) {
                        switch ($nameformat) {
                            ...
                            default:
                            case B_ACTIVITYRESULTS_NAME_FORMAT_FULL:
                                if (has_capability('agpu/user:viewdetails', $context)) {
                                    $thisname = html_writer::link(new agpu_url('/user/view.php',
    ...
    private function activity_link($activity, $cm) {
        ...
        $o .= html_writer::link(new agpu_url('/mod/'.$activity->itemmodule.'/view.php',
